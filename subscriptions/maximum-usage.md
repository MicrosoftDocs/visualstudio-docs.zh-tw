...
答：[概觀] 中的資訊顯示每個訂用帳戶層級目前指派和可用的訂用帳戶。  這和合約在任何時點的已指派訂用帳戶數目上限會有極大差異。  [使用量上限] 功能可讓您查看何時達到指派層級上限以及哪個層級。  這是重要的區別，因為訂用帳戶的計費校正期間是以一年內任何時點已指派訂用帳戶數目上限為基礎。 
