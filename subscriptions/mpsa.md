...
如果您已透過 MPSA 方案購買 Visual Studio 訂用帳戶，您必須注意幾件事，才能成為 Visual Studio 訂用帳戶管理員，並將訂用帳戶指派給您的使用者。 如果您已將設定為系統管理員，您可以直接前往 Visual Studio 訂用帳戶 [管理入口網站](https://manage.visualstudio.com/)。
...
此外，Visual Studio 訂用帳戶 [管理入口網站](https://manage.visualstudio.com/) 可讓您在成為 Visual studio 訂用帳戶系統管理員之後，指派訂用帳戶。在 MPSA 中，您必須將 Visual Studio 訂用帳戶布建到其個別的入口網站，也就是 Visual Studio 訂用帳戶的管理入口網站。 若要這樣做，您必須將「購買帳戶」與租用戶 (亦即 contoso.onmicrosoft.com) 建立關聯。
...
以下是在完成租使用者關聯之前，成為 Visual Studio 訂用帳戶管理員的幾個需求/步驟。

## <a name="pre-tenant-association-managed-tenant"></a>建立租用戶關聯之前 (受控租用戶)
- 您必須是「商務中心」的已註冊使用者。
- 您必須是所屬租用戶的「使用者管理員」(至少需具備此角色) 或「全域管理員」。 (這適用於公司已經使用「雲端服務」的情況)。 需要有任一個角色才能成為 Visual Studio 訂閱系統管理員。
...
在關聯時，所有符合準則的使用者通常會在幾分鐘內布建為 Visual Studio 訂用帳戶管理員。 不過，有時可能會花費長達 24 小時的時間。 布建您的租使用者之後，您將能夠存取 Visual Studio 訂用帳戶的系統管理入口網站。 如果這花費的時間超過24小時，請聯絡 [商務中心支援](https://businessaccount.microsoft.com/Customer/ContactUs)人員。

> [!NOTE]
> 如果在建立關聯之後，有符合步驟 2 和 5 的新使用者，您必須連絡 MPSA 支援服務。 MPSA 支援人員會提供提供新 Visual Studio 訂用帳戶管理員的協助。
...
> [!NOTE]
> 當您完成網域接管程式時，您必須遵守「租使用者關聯」 (受控) 一節中五個步驟的準則。 符合準則之後，您只需要聯繫 MPSA 支援人員，就可以布建其他 Visual Studio 訂用帳戶管理員。
