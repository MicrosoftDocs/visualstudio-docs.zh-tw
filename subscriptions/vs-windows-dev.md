...
2. 在 [工具] 區段找到 [Windows 開發人員帳戶] 磚，並按一下權益磚底部的 [取得代碼]**** 連結。  將程式碼複製到剪貼簿，然後按一下 [**啟動**]。
   > [!div class="mx-imgBorder"]
   > ![Windows 開發人員權益磚](_img/vs-windows-dev/vs-windows-dev-tile.png)
3. 在 [https://developer.microsoft.com/store/register/](https://developer.microsoft.com/store/register/) 網站上，您可以開始註冊程式。  按一下 [註冊]**** 繼續。
   > [!div class="mx-imgBorder"]
...
<sup>1</sup>  *包括：禁止轉售（NFR）、FTE、Visual Studio 產業夥伴（VSIP）、BizSpark、MCT Software & Services 開發人員、最有價值專家（MVP）、區域主管（RD）。排除： Microsoft 合作夥伴網路（MPN），試想，MCT Software & Services。*
