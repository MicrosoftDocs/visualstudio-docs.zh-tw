...
當您從 [**資料來源**] 視窗將專案拖曳至 [[!INCLUDE[wpfdesigner_current_short](../data-tools/includes/wpfdesigner_current_short_md.md)]] 時，Visual Studio 會產生 [!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)] 來定義新的資料繫結控制項（或將現有的控制項系結至資料來源）。 針對某些資料來源，Visual Studio 也會在程式碼後置檔案中產生可將資料填入資料來源的程式碼。

下表列出 Visual Studio 為 [**資料來源**] 視窗中每個資料來源類型產生的 [!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)] 和程式碼。
...
當您從 [**資料來源**] 視窗將資料表或資料行拖曳至設計工具時，Visual Studio 會產生會執行下列動作的 [!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)]：
...
- 針對包含控制項的 <xref:System.Windows.FrameworkElement.Loaded> 項目，建立 [!INCLUDE[TLA2#tla_ui](../data-tools/includes/tla2sharptla_ui_md.md)] 事件處理常式。 事件處理常式會將資料填入資料表，從容器的資源擷取 <xref:System.Windows.Data.CollectionViewSource>，然後讓第一個資料項目成為目前的項目。 如果 <xref:System.Windows.FrameworkElement.Loaded> 事件處理常式已經存在，Visual Studio 就會將此程式碼加入至現有的事件處理常式。

### <a name="entity-data-models"></a>實體資料模型

當您從 [**資料來源**] 視窗將實體或實體屬性拖曳至設計工具時，Visual Studio 會產生會執行下列動作的 [!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)]：
...
- 建立控制項的資料繫結。 如果您將項目拖曳至設計工具中的現有控制項，[!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)] 會將控制項繫結至項目。 如果您將專案拖曳至容器，[!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)] 會建立已針對拖曳專案選取的控制項，並將控制項系結至專案。 控制項是在新的 <xref:System.Windows.Controls.Grid> 內建立。

Visual Studio 也會對程式碼後置檔案進行下列變更：

- 加入新方法，其針對您拖曳至設計工具的實體 (或包含拖曳至設計工具之屬性的實體)，傳回查詢。 新方法的名稱為 `Get<EntityName>Query`，其中 `\<EntityName>` 是實體的名稱。

- 針對包含控制項的 <xref:System.Windows.FrameworkElement.Loaded> 項目，建立 [!INCLUDE[TLA2#tla_ui](../data-tools/includes/tla2sharptla_ui_md.md)] 事件處理常式。 事件處理常式會呼叫 `Get<EntityName>Query` 方法，將資料填入實體，從容器的資源中抓取 <xref:System.Windows.Data.CollectionViewSource>，然後讓第一個資料項目成為目前的專案。 如果 <xref:System.Windows.FrameworkElement.Loaded> 事件處理常式已經存在，Visual Studio 就會將此程式碼加入至現有的事件處理常式。

### <a name="services"></a>服務

當您從 [**資料來源**] 視窗將服務物件或屬性拖曳至設計工具時，Visual Studio 會產生 [!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)]，以建立資料繫結控制項（或將現有控制項系結至物件或屬性）。 不過，Visual Studio 不會產生將資料填滿 proxy 服務物件的程式碼。 您必須自行撰寫此程式碼。 如需示範如何執行這種操作的範例，請參閱將[WPF 控制項系結至 WCF 資料服務](../data-tools/bind-wpf-controls-to-a-wcf-data-service.md)。
...
當您從 [**資料來源**] 視窗將物件或屬性拖曳至設計工具時，Visual Studio 會產生 [!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)]，以建立資料繫結控制項（或將現有控制項系結至物件或屬性）。 不過，Visual Studio 不會產生程式碼來填入具有資料的物件。 您必須自行撰寫此程式碼。
...
Visual Studio 會產生會執行下列動作的 [!INCLUDE[TLA#tla_titlexaml](../data-tools/includes/tlasharptla_titlexaml_md.md)]：
