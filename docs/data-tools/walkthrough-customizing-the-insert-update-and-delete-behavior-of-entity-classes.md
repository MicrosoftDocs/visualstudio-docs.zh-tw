...
此逐步解說提供您必須遵循的步驟，以覆寫預設 LINQ to SQL 執行時間行為，以使用預存程式將資料儲存回資料庫。

在此逐步解說中，您將瞭解如何執行下列工作：
...
2. 依照下列步驟安裝 Northwind 範例資料庫：
    ...
    1. 在 Visual Studio 中，開啟 [ **SQL Server 物件總管**] 視窗。 （**SQL Server 物件總管**在**Visual Studio 安裝程式**中安裝為**資料儲存和處理**工作負載的一部分）。展開 [ **SQL Server** ] 節點。 以滑鼠右鍵按一下您的 LocalDB 實例，然後選取 [追加**查詢**]。
...
由於您是使用 LINQ to SQL 類別，並在 Windows Form 上顯示資料，因此請建立新的 Windows Forms 應用程式，並新增 LINQ to SQL 類別檔案。
...
6. 按一下 [加入]。
     ...
     系統會將空的 LINQ to SQL 類別檔案（**Northwind**）新增至專案，並開啟**O/R 設計**工具。

## <a name="create-the-customer-entity-class-and-object-data-source"></a>建立 Customer 實體類別和物件資料來源

藉由將資料表從**伺服器總管**或**資料庫總管**拖曳至**O/R 設計**工具，來建立對應至資料庫資料表的 LINQ to SQL 類別。 結果會產生對應至資料庫中各資料表的 LINQ to SQL 實體類別。 建立實體類別之後，實體類別就和其他具有公用 (Public) 屬性的類別一樣，可以當成物件資料來源使用。

### <a name="to-create-a-customer-entity-class-and-configure-a-data-source-with-it"></a>若要建立 Customer 實體類別和將它設為資料來源

1. 在**伺服器總管**或**資料庫總管**中，找出 SQL Server 版本的 Northwind 範例資料庫中的**Customer**資料表。

2. 將 [ **Customers** ] 節點從**伺服器總管**或**資料庫總管**拖曳到 **O/R 設計*工具介面上。
...
預設不會啟用儲存按鈕，也不會實作儲存功能。 同時，為物件資料來源建立資料繫結控制項時，並不會自動加入程式碼以將變更的資料儲存至資料庫。 本節說明如何啟用 [儲存] 按鈕並執行 LINQ to SQL 物件的儲存功能。
...
## <a name="override-the-default-behavior-for-performing-updates-inserts-updates-and-deletes"></a>覆寫執行更新的預設行為（插入、更新和刪除）
...
4. 在**O/R 設計**工具中，選取 [ **Customer** ] 實體類別。
...
- 在更新期間實作並行檢查。 如需相關資訊，請參閱[開放式平行存取：總覽](/dotnet/framework/data/adonet/sql/linq/optimistic-concurrency-overview)。
