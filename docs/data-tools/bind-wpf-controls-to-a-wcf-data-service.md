...
2.  在已安裝的範本 清單中，按一下**資料**，然後選取**ADO.NET 實體資料模型**專案項目。
...
10. 在  **Services**方塊中，按一下**AdventureWorksService.svc**，然後按一下**確定**。
     ...
     Visual Studio 會下載服務資訊，然後傳回給**資料來源組態**精靈。
...
4.  從**資料來源** 視窗中，拖曳**SalesOrderHeaders**包含按鈕的資料列底下的方格資料列的節點。
     ...
     Visual Studio 會產生 XAML 和程式碼會建立一組中的資料繫結的控制項**產品**資料表。 如需有關產生的 XAML 和程式碼的詳細資訊，請參閱 <<c0> [ 繫結 WPF 控制項新增至 Visual Studio 中的資料](../data-tools/bind-wpf-controls-to-data-in-visual-studio.md)。
...
3.  在 **方案總管**，以滑鼠右鍵按一下**AdventureWorksSalesEditor**專案。
