...
如果資料集內已進行變更，資料集的 <xref:System.Data.DataSet.HasChanges%2A> 方法會傳回 `true`。 判斷變更的資料列存在之後，您可以呼叫 <xref:System.Data.DataSet> 或 <xref:System.Data.DataTable> 的 `GetChanges` 方法，以傳回一組已變更的資料列。
