...
建立適用于 c # 或 Visual Basic 的新**Windows Forms 應用程式**專案。 將專案命名為**ReadingXML**。
...
- <xref:System.Windows.Forms.TextBox>顯示 xml 檔案之 xml 架構的控制項。

- 兩個 <xref:System.Windows.Forms.Button> 控制項。

  - 一個按鈕會將 XML 檔案讀入資料集，並將它顯示在 <xref:System.Windows.Forms.DataGridView> 控制項中。

  - 第二個按鈕會從資料集提取架構，並透過在 <xref:System.IO.StringWriter> 控制項中顯示它 <xref:System.Windows.Forms.TextBox> 。
...
[**讀取 xml** ] 按鈕會將 XML 檔案讀入資料集。 然後，它會在控制項上設定系結 <xref:System.Windows.Forms.DataGridView> 至資料集的屬性。
...
[**顯示架構**] 按鈕會建立一個 <xref:System.IO.StringWriter> 已填入架構的物件，並顯示在 <xref:System.Windows.Forms.TextBox> 控制項中。
