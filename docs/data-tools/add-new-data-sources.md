...
> [!NOTE]
> 本文中所述的功能適用于 .NET Framework Windows Forms 和 WPF 開發。 在 Visual Studio 2019 （和之前的版本）中，WPF 和 Windows Forms 都不支援 .NET Core 開發的功能。
:::moniker-end
...
您可以使用 Windows Forms 或 Windows Presentation Foundation 應用程式中的 [**資料來源設定]** ，來建立和編輯資料來源。 針對 Entity Framework，請先建立您的實體類別，然後選取 [專案] **Project**  >  [新增] [**新資料來源**] 來啟動精靈（本文稍後會詳細說明）。
...
> [!TIP]
> 若要開啟 [**資料來源**] 視窗，請確定您的專案已開啟，然後按**Shift** + **Alt** + **D** ，或選擇 [**查看**  >  **其他 Windows**  >  **資料來源**]。
...
> [!NOTE]
> 為了支援拖放資料系結，實作為或介面的物件 <xref:System.ComponentModel.ITypedList> <xref:System.ComponentModel.IListSource> 必須有預設的函式。 否則，Visual Studio 無法具現化資料來源物件，而且當您將專案拖曳至設計介面時，就會顯示錯誤。
