...
您所選擇的事件取決於您想要驗證的細微程度。 如果資料行變更時，請務必立即攔截錯誤，請使用 <xref:System.Data.DataTable.ColumnChanging> 事件來建立驗證。 否則，請使用 <xref:System.Data.DataTable.RowChanging> 事件，這可能會一次捕捉數個錯誤。 此外，如果您的資料是結構化的，因此會根據另一個資料行的內容來驗證一個資料行的值，請在 <xref:System.Data.DataTable.RowChanging> 事件期間執行驗證。
...
驗證也可以在 <xref:System.Data.DataTable.RowChanging> 事件期間執行。
...
當對資料列進行變更時，資料集會同時保留原始（<xref:System.Data.DataRowVersion.Original>）和資料列的新（<xref:System.Data.DataRowVersion.Current>）版本。 例如，在呼叫 `AcceptChanges` 方法之前，您的應用程式可以存取不同版本的記錄（如 <xref:System.Data.DataRowVersion> 列舉中所定義），並據此處理變更。
