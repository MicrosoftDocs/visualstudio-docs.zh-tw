...
這個逐步解說會建立繫結至兩張資料表中資料的查閱控制項。 此範例使用 Northwind 範例資料庫的 `Customers` 和 `Orders` 資料表。 查閱控制項會系結至 `Orders` 資料表中的 `CustomerID` 欄位。 它會使用這個值來查閱 `Customers` 資料表的 `CompanyName`。
...
若要在 Windows Form 上建立資料繫結控制項，請將 [ **Orders** ] 節點從 [**資料來源**] 視窗拖曳至 windows form，然後確認 [ **LookupBox** ] 控制項是用來顯示 [`CustomerID`] 資料行中的資料。

## <a name="bind-the-control-to-look-up-companyname-from-the-customers-table"></a>系結控制項以查詢 Customers 資料表中的 [公司名稱]

若要設定查閱系結，請選取 [**資料來源**] 視窗中的 [主要**客戶**] 節點，並將它拖曳至 [ **Form1**] 上 [ **CustomerIDLookupBox** ] 的下拉式方塊中。
