...
這個逐步解說會建立繫結至兩張資料表中資料的查閱控制項。 此範例使用 Northwind 範例資料庫的 `Customers` 和 `Orders` 資料表。 查閱控制項所繫結`CustomerID`欄位從`Orders`資料表。 它會使用此值來查閱`CompanyName`從`Customers`資料表。
