...
### <a name="overview"></a>概觀
 ...
 當使用者的內容變更時（例如從設計視窗切換至程式碼編輯視窗），與新內容不相關的功能就會消失。 同時，新功能會與相關的動態資訊（例如屬性和工具箱選項）一起出現。 使用者應該不會注意到可用的命令集交換。 如果使用者在出現或消失的情況下受到干擾或混淆，則 UI 設計需要調整。 使用者目前的內容一律會以一或多個方式指出，例如在 IDE 標題列、屬性視窗或 [屬性頁] 對話方塊中。
...
- 使用者很可能會透過 [**自訂 ...** ] 將使用者新增至工具列 對話方塊中，新增使用者帳戶。
...
- 使用的分隔符號不超過六個。 分組應遵循下圖：
     ...
     ![主功能表群組的方針](../../extensibility/ux-guidelines/media/0501-b-mainmenus.png "0501-b_MainMenus")
...
- 飛出視窗功能表是可接受的，但不能超過一層級的深度–絕對不要使用串聯式 flyouts。
...
- 請考慮將放置命令的位置。 它是在最上層功能表還是飛出視窗中？ 例如，當您在飛出視窗中將對齊命令分組時，最上層命令應該是「對齊」，而飛出視窗命令應該是「左」、「右」、「中心」、「對齊」等等。 這是多餘的，可將飛出視窗的命令「靠左對齊」或「靠右對齊」。
     ...
     ![Visual Studio 的 [格式] 功能表](../../extensibility/ux-guidelines/media/0502-a-formatmenu.png "0502-a_FormatMenu")
...
- 此命令是一種特殊命令，使用者可能會使用 [**自訂 ...]** 將其新增至工具列 對話方塊中，新增使用者帳戶。
