---
title: 如何：搭配專案範本使用嚮導 |Microsoft Docs
ms.date: 11/15/2016
...
## <a name="creating-a-project-template-project-with-a-vsix-project"></a>使用 VSIX 專案建立專案範本專案  
 您可以開始使用專案範本專案建立自訂範本，這是 Visual Studio SDK 的一部分。 在此程式中，我們將使用 c # 專案範本專案，但也會有 Visual Basic 專案範本專案。 然後，將 VSIX 專案加入包含專案範本專案的方案中。  
  
1. 建立 c # 專案範本專案 (在 Visual Studio、檔案 **/新增/專案/Visual c #/擴充性/c # 專案範本**) 。 將它命名為 **MyProjectTemplate**。  
  
    > [!NOTE]
    > 系統可能會要求您安裝 Visual Studio SDK。 如需詳細資訊，請參閱 [安裝 VISUAL STUDIO SDK](../extensibility/installing-the-visual-studio-sdk.md)。  
  
2. 將新的 VSIX 專案加入 (檔案 **/新增/專案/Visual c #/擴充性/VSIX 專案**) 在與 **方案總管**中專案範本專案 (相同的方案中，選取方案節點、按一下滑鼠右鍵，然後選取 [新增 **/新增專案** ]) 。 將它命名為 **MyProjectWizard。**  
  
3. 將 VSIX 專案設定為啟始專案。 在 [ **方案總管**中，選取方案節點、按一下滑鼠右鍵，然後選取 [ **設定為啟始專案**]。  
...
8. 在 Visual Studio 的第二個實例中，嘗試使用新範本建立新專案。  (檔案] **/[新增]/[專案]/[Visual c #/MyProject] 範本**) 。 新的專案應該會顯示一個名為 **Class1**的類別。 您現在已建立自訂專案範本！ 立即停止調試。  
  
## <a name="creating-a-custom-template-wizard"></a>建立自訂範本嚮導  
 本主題說明如何建立自訂的 wizard，以在建立專案之前開啟 Windows Form。 表單可讓使用者在建立專案期間加入自訂參數值，並新增至原始程式碼。  
  
1. 設定 VSIX 專案，以允許它建立元件。  
  
2. 在 [ **方案總管**中，選取 [VSIX 專案] 節點。 在方案總管下方，您應該會看到 [ **屬性** ] 視窗。 如果沒有，請選取 [ **視圖/屬性] 視窗**，或按 **F4**。 在 [屬性視窗中，選取下欄欄位以 `true` ：  
...
3. 將元件做為資產新增至 VSIX 專案。 開啟 extension.vsixmanifest 檔案，然後選取 [**資產**] 索引標籤。在 [**加入新資產**] 視窗的 [ **VisualStudio**]**類型**中，針對 [**來源**] 選取 [**目前方案中的專案**]，然後針對 [**專案**] 選取 [ **MyTemplateWizard**]。  
  
4. 將下列參考加入至 VSIX 專案。  (在 [ **方案總管**] 的 [VSIX 專案] 節點下，選取 [ **參考**]、按一下滑鼠右鍵，然後選取 [ **加入參考**]。 ) 在 [ **加入參考** ] 對話方塊的 [ **架構** ] 索引標籤中，尋找 [ **System. Windows Forms** 元件]，然後選取它。 現在請選取 [ **擴充** 功能] 索引標籤。尋找 **EnvDTE** 元件，然後選取它。 也請尋找並選取 **VisualStudio TemplateWizardInterface** 元件。 按一下 [確定]。  
  
5. 將 wizard 執行的類別加入至 VSIX 專案。  (在方案總管中，以滑鼠右鍵按一下 VSIX 專案節點，然後依序選取 [ **加入**]、[ **新增專案**] 和 [ **類別**]。 ) 將類別命名為 **WizardImplementation**。  
...
6. 將 **WizardImplementationClass.cs** 檔案中的程式碼取代為下列程式碼：  
   ...
   - <xref:System.Object>陣列，其中包含一組透過 Visual Studio 傳遞給 wizard 的參數。  
     ...
     此範例會將使用者輸入表單中的參數值加入至 <xref:System.Collections.Generic.Dictionary%602> 參數。 專案中每個 `$custommessage$` 參數的實例將會取代為使用者輸入的文字。 您必須將下列元件新增至您的專案：  
  
7. 現在建立 **UserInputForm**。 在 **WizardImplementation.cs** 檔案中，將下列程式碼新增至 **WizardImplementation** 類別的結尾之後。  
...
4. 將 [ **將組建輸出複製到輸出目錄** ] 欄位設定為 [ **true**]。 這可在重建方案時，將元件複製到輸出目錄中。 它仍然包含在 .vsix 檔案中。 您必須查看元件，才能找出其簽署金鑰。  
  
5. 重建方案。  
  
6. 您現在可以在 MyProjectWizard 專案目錄中找到金鑰 .snk 檔案， (** \<your disk location> \MyProjectTemplate\MyProjectWizard\key.snk**) 。 複製金鑰 .snk 檔案。  
  
7. 移至輸出目錄，並找出元件 (** \<your disk location> \ MyProjectTemplate/MyProjectWizard\bin\Debug\MyProjectWizard.dll**) 。 在這裡貼上金鑰 .snk 檔案。  (這並非絕對必要，但可讓您更輕鬆地進行下列步驟 )   
...
9. 尋找 **sn.exe** 簽章工具。 例如，在 Windows 10 64 位作業系統上，一般的路徑如下：  
     ...
     **C:\Program Files (x86) \Microsoft SDKs\Windows\v10.0A\bin\NETFX 4.6.1 Tools**  
     ...
     如果您找不到此工具，請嘗試執行 **where/r。 ** 在命令視窗中 sn.exe。 記下路徑。  
  
10. 將公開金鑰從金鑰 .snk 檔案解壓縮。 在命令視窗中，輸入  
...
12. 將自訂嚮導的參考新增至專案範本的 .vstemplate 檔案。 在方案總管中，尋找名為 MyProjectTemplate 的檔案，然後開啟它。 在區段結尾之後 \<TemplateContent> ，新增下列區段：  
...
1. 在方案總管中，移至 **MyProjectTemplate** 專案，然後開啟 **Class1.cs**。  
...
## <a name="using-the-custom-wizard"></a>使用自訂嚮導  
 現在您可以從範本建立專案，並使用自訂的 wizard。  
  
1. 重建方案並開始進行調試。 Visual Studio 的第二個執行個體應該會出現。  
  
2. 建立新的 MyProjectTemplate 專案。  (**File/New/Project/Visual c #/MyProjectTemplate**)   
  
3. 在 [ **新增專案** ] 對話方塊中，找出您的範本、輸入名稱，然後按一下 **[確定]**。  
