...
包含在 Visual Studio 擴充性工具 (VS SDK) 的 Windows Forms 工具箱控制項專案範本，可讓您建立在安裝擴充功能時自動新增至 **工具箱** 的控制項。 本主題說明如何使用範本建立可散發給其他使用者的簡單計數器控制項。  
...
1. 建立名為的 VSIX 專案 `MyWinFormsControl` 。 您可以在 [ **新增專案** ] 對話方塊中的 **Visual c #/** 擴充性下找到 VSIX 專案範本。  
...
2. 移除「按一下這裡！」 當您加入 Windows Forms 工具箱控制項專案範本時，預設會包含的**按鈕**。  
...
3. 建立下列公用屬性宣告。  
    ...
    ```  
     ...
     呼叫端可以存取這些屬性來取得和設定計數器的顯示文字，以及顯示或隱藏 `Reset` 按鈕。 呼叫端可以取得唯讀屬性的目前值 `Value` ，但無法直接設定該值。  
...
7. 返回 [設計檢視]，然後按兩下 `Reset` 按鈕以產生 `btnReset_Click` 事件處理常式，然後在其中填入，如下列範例所示。  
...
1. 按下 F5。  
     ...
     這會建立專案，並開啟已安裝控制項之 Visual Studio 的第二個實驗實例。  
...
12. 在下一行中，新增下列 `if` / `else` 區塊來設定按鈕的可見度 `Reset` 。  
...
16. 按一下 [ **測試** ]，直到計數器每次到達 **5** 次之後，再關閉訊息方塊。  
...
## <a name="next-steps"></a>後續步驟  
 當您建置 [工具箱] **** 控制項時，Visual Studio 會在專案的 \bin\debug\ 資料夾中建立名為 *ProjectName*.vsix 的檔案。 您可以將 .vsix 檔案上傳到到網路或網站以部署控制項。 當使用者開啟 .vsix 檔案時，控制項就會安裝並新增至使用者電腦上的 Visual Studio **工具箱** 。 或者，您可以將 .vsix 檔上傳至 [Visual Studio Marketplace](https://marketplace.visualstudio.com/) 網站，讓使用者可以在 [ **工具/擴充功能和更新** ] 對話方塊中流覽，以找到該檔案。  
  
## <a name="see-also"></a>另請參閱  
 [擴充工具箱](../misc/extending-the-toolbox.md)   
 [建立 WPF 工具箱控制項](../extensibility/creating-a-wpf-toolbox-control.md)   
 [擴充 Visual Studio 的其他部分](../extensibility/extending-other-parts-of-visual-studio.md)   
 [Windows Form 控制項開發的基本概念](https://msdn.microsoft.com/library/6277bb81-90f7-4c5b-9f4b-b02bb42dd316)
