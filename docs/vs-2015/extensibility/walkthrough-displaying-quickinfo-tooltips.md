...
## <a name="implementing-the-quickinfo-source"></a>執行 QuickInfo 來源  
...
8. 您也必須執行 Dispose ( # A1 方法，因為它 <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSource> 會 <xref:System.IDisposable> 執行：  
...
## <a name="implementing-a-quickinfo-source-provider"></a>執行 QuickInfo 來源提供者  
...
## <a name="implementing-a-quickinfo-controller"></a>執行 QuickInfo 控制器  
 QuickInfo 控制器會決定何時應該顯示 QuickInfo。 在此範例中，當指標在對應到其中一個方法名稱的單字上方時，就會顯示 QuickInfo。 QuickInfo 控制器會執行可觸發 QuickInfo 會話的滑鼠停留事件處理常式。  
