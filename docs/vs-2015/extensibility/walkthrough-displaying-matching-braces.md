...
ms.locfileid: "68165782"
---
# <a name="walkthrough-displaying-matching-braces"></a>逐步解說︰顯示成對括弧
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

您可以藉由定義要比對的大括弧來執行以語言為基礎的功能，例如大括弧比對，然後當插入號位於其中一個大括弧時，將文字標記標記加入至相符的大括弧。 您可以在語言的內容中定義大括弧，也可以定義自己的副檔名和內容類型，並只將標記套用至該類型，或者您可以將標記套用至現有的內容類型， (例如「文字」 ) 。 下列逐步解說示範如何將大括弧比對標記套用至 "text" 內容類型。  
...
## <a name="implementing-a-brace-matching-tagger"></a>執行括弧配對的標記  
 若要取得類似于 Visual Studio 中所使用的大括弧醒目提示效果，您可以執行類型的標記 <xref:Microsoft.VisualStudio.Text.Tagging.TextMarkerTag> 。 下列程式碼示範如何在任何嵌套層級上定義大括弧配對的標記。 在此範例中，大括弧配對 []。 [] 和定義于標記的函式中 {} ，但在完整語言的執行中，會在語言規格中定義相關的大括弧組。  
...
4. 加入文字視圖、來源緩衝區和目前快照點的屬性，以及一組大括弧配對。  
...
## <a name="implementing-a-brace-matching-tagger-provider"></a>執行括弧對稱的標記提供者  
 除了執行標記之外，您還必須執行和匯出標記提供者。 在此情況下，提供者的內容類型為 "text"。 這表示括弧對稱將會出現在所有類型的文字檔中，但更完整的執行只會將大括弧比對套用至特定的內容類型。  
