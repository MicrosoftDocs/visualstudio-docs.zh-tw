...
## <a name="template-scanning"></a>掃描的範本  
 先前**devenv /setup**或是**devenv /installvstemplates**掃描本機磁碟，若要尋找專案和項目範本。 從 Preview 4 中，掃描會僅針對執行使用者層級位置 (**%USERPROFILE%\Documents\\< Visual Studio 版本\>\My Exported Templates\\**) 所用的所產生的範本**檔案] / [匯出範本**命令。  
  
 其他 （非使用者） 的位置，您必須包含指定的位置和範本的其他特性 manifest(.vstman) 檔案。 .Vstman 檔案會產生以及用於範本的.vstemplate 檔案。 如果您安裝您使用.vsix 的延伸模組，您可以完成這個 Visual Studio"15"Preview 2 中的擴充功能需要重新編譯。 但如果您使用.msi 時，您需要以手動方式進行變更。 如需您要如何進行這些變更的清單，請參閱**升級與安裝的擴充功能。MSI**本主題稍後的。  
  
## <a name="how-to-update-a-vsix-extension-with-project-or-item-templates"></a>如何更新 VSIX 擴充功能專案或項目範本  
 此程序說明如何讓 Visual Studio"15"Preview 2 更新您現有的 VSIX 擴充功能。  
  
1.  在 Visual Studio"15"Preview 2 中開啟的方案。 系統會要求您升級的程式碼。 按一下 [確定 **Deploying Office Solutions**]。  
  
2.  在升級完成之後，您可能需要變更安裝目標版本。 在 VSIX 專案中，開啟 source.extension.vsixmanifest 檔案中，然後選取**安裝目標**] 索引標籤。如果**版本範圍**欄位是 **[14.0]**，按一下 [**編輯**並將它變更為包含 Visual Studio"15"。 例如，您可以將它設定為 **[14.0,15.0]** Visual Studio 2015 或 Visual Studio"15"，或安裝延伸模組 **[15.0]** 將其安裝到 Visual Studio"15"。  
...
## <a name="file-changes-in-project-and-item-templates"></a>專案和項目範本中的檔案變更  
 如此您就可以正確地建立新的檔案，我們將說明 Visual Studio 2015 版本與範本檔案，Visual Studio"15"版本之間差異的點。  
