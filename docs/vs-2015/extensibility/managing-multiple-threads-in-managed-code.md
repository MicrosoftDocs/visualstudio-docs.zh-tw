...
如果您的 managed VSPackage 延伸模組會呼叫非同步方法，或具有在 Visual Studio UI 執行緒以外的執行緒上執行的作業，您應該遵循下面所述的指導方針。 您可以讓 UI 執行緒保持回應，因為它不需要在另一個執行緒上等候工作完成。 您可以讓程式碼更有效率，因為您沒有額外的執行緒佔用堆疊空間，而且您可以讓它更可靠且更容易進行偵錯工具，因為您可以避免鎖死和無回應。  
...
1. 如果您是在 UI 執行緒上，並且想要在背景執行緒上執行非同步工作，請使用工作。執行 ( # A1：  
