...
針對任何特定的程式設計語言，一次只能有一個語言服務的實例。 不過，單一語言服務可以提供一個以上的編輯器。  
 ...
 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 不會將語言服務與任何特定編輯器產生關聯。 因此，當您要求語言服務作業時，您必須將適當的編輯器識別為參數。  
...
## <a name="additional-language-service-interfaces"></a>其他語言服務介面  
 您可以使用語言服務來提供其他介面。 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 針對每個文字緩衝區實例要求這些介面的個別實例。 因此，您應該在它自己的物件上執行這些介面。 下表顯示每個文字緩衝區實例需要一個實例的介面。  
