...
### <a name="creating-a-topic-f1-support"></a> (F1 支援建立主題) 
...
- 若為目錄，請新增此主題的節點順序。 當父節點有 n 個子節點數目時，請依照子節點的位置定義順序。 例如，本主題為4個子主題的第4個。 ) 
 ...
 **主題主體**
 ...
 本文 (不包括主題的頁首和頁尾) 包含頁面連結、附注區段、可折迭區域、程式碼片段，以及特定語言的文字區段。  請參閱商標一節，以取得所顯示主題中這些區域的相關資訊。
...
5. 加入程式碼語言特定文字：  `<LanguageSpecificText devLangcs="CS" devLangvb="VB" devLangcpp="C++" devLangnu="F#" />` 請注意，devLangnu = 可讓您輸入其他語言。 例如，devLangnu = "Fortran" 會在程式碼片段 DisplayLanguage = Fortran 時顯示 Fortran
 ...
 **範例說明檢視器主題** 此程式碼說明如何定義中繼資料、程式碼片段、可折迭區域，以及特定語言文字。
 ...
 **F1 支援**
 ...
 在 Visual Studio 中，選取 F1 會產生從 IDE 中的資料指標位置所提供的值，並根據游標位置，以提供的值 (填入「屬性包」。 當游標在功能 x 上時，功能 x 是使用中/焦點，並以值填入屬性包。  當選取 F1 時，會填入屬性包，Visual Studio F1 程式碼會查看客戶預設的說明來源是否為本機或線上 (online 是預設的) ，然後根據使用者的設定建立適當的字串 (online 是預設的) – shell execute (如需本機說明，請參閱 [exe 啟動參數] 的 [說明系統管理員指南]。如果本機說明是預設值，則為，而在參數清單中使用關鍵字的 MSDN URL。)  (
...
- 方法：這會變成傳遞的關鍵字。
  ...
  **在 IDE 中啟用 Help 調試工具**
  ...
  HKEY_CURRENT_USER \Software\Microsoft\VisualStudio\12.0\Dynamic 說明金鑰：以零售值顯示調試輸出值：是
...
|\< meta name="Description" content="[topic description]" />|提供本主題內容的簡短摘要。 如果在主題中使用此標籤，則只能使用一次。 這個屬性是由查詢程式庫直接存取;它不會儲存在索引檔中。|
 meta name = "TocParent" content = "[parent_Id]"/>|在目錄中指定本主題的父主題。 此標記是必要的，而且必須只在主題中使用一次。 值是父系的 Microsoft.Help.Id。 主題在目錄中只能有一個位置。 "-1" 被視為 TOC 根目錄的主題識別碼。 在中 [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] ，該頁面是說明檢視器首頁。 這是我們特別將 TocParent =-1 新增至某些主題，以確保它們會顯示在最上層的原因。說明檢視器首頁是系統頁面，因此不可取代。 如果 .VSP 嘗試新增識別碼為-1 的頁面，可能會將它新增至內容集，但說明檢視器一律會使用 system page – Help Viewer 首頁|
...
-  () Branding_ 的「退回品牌」套件 \<locale> 會安裝在說明檢視器2.1 應用程式根目錄 (範例： C:\Program Files (x86) \Microsoft help Viewer\v2.1) 由說明檢視器語言套件提供。  這適用于未安裝產品品牌套件 (未安裝任何內容) 或安裝的商標套件已損毀的情況。  使用應用程式的根後備商標套件時，會忽略 Visual Studio 元素 (標誌和意見反應) 。
...
- Branding.xml –一致地在目錄內容中使用的字串。  注意：若要在 branding.xml 中 Visual Studio 當地語系化文字元素，請包含 _locID = " \<unique value> "
...
- 或當 SelfBranded = false，且在 .MSHA 中定義了唯一的商標套件，且在內容安裝時可供使用
  ...
  Branding.xml 檔案包含的清單元素，可在主題包含的主題中，用來在主題中一致地呈現特定專案 \<meta name="Microsoft.Help.SelfBranded" content="false"/> 。  以下列出 Branding.xml 檔案中的 Visual Studio 元素清單。  這份清單的目的是作為 ISO Shell 採用者的範本，在其中修改這些元素 (例如標誌、意見反應和著作權) ，以符合自己的產品商標需求。

  注意： "{n}" 記下的變數具有程式碼相依性–移除或變更這些值將會導致錯誤，而且可能會導致應用程式損毀。當地語系化識別碼 (範例 _locID = "codesnippet. n" ) 包含在 Visual Studio 商標套件中。

  **Branding.xml**

功能： **CollapsibleArea** 使用：展開折迭內容控制項文字
...
功能：**CodeSnippet** 使用：程式碼片段控制項文字。  注意：具有「非中斷」空間的程式碼片段內容將會變更為「空格」。
...
功能： **意見反應、頁尾和標誌** 使用：提供意見反應控制項，讓客戶透過電子郵件提供目前主題的意見反應。  內容的著作權文字。  標誌定義。
...
功能： **免責聲明** 使用：一組適用于機器翻譯內容的一組案例特定免責聲明。
...
功能： **未安裝的內容控制項** 使用： Text 元素 (字串) 用於轉譯 contentnotinstalled.htm
...
功能： **找不到控制項** 使用的主題： Text 元素 (字串) 用於轉譯 topicnotfound.htm
...
功能： **主題損毀控制項** 使用： Text 元素 (字串) 用於轉譯 topiccorrupted.htm
...
功能： **首頁控制項** 使用：支援說明檢視器最上層節點內容顯示的文字。
...
- .Css –包含用於轉譯的 css 元素，其中 SelfBranded = false
...
### <a name="deploying-a-set-of-topics"></a>部署一組主題
 ...
 .MSHA 的名稱（基於此入門的目的）是 Helpcontentsetup.msha. .msha (檔案的名稱可以是任何副檔名為的檔案。.MSHA) 。 Helpcontentsetup.msha. .msha (範例) 應包含可用的 cab 或 MSHCs 清單。  檔案類型在 .MSHA 中必須是一致的 (不支援 .MSHA 和 CAB 檔案類型的組合) 。 對於每個 CAB 或 .MSHC，都應該有 \<div class="package"> .。。\</div>  (請參閱以下) 的範例。
...
3. 將下列 Helpcontentsetup.msha 建立為文字檔， (記事本用來建立) 的檔案，並將其儲存至上述的已注明資料夾 (請參閱步驟 1) 。
   ...
   若要取得 Visual Studio 商標套件，請將 C:\Program 檔案中的 Branding_en-.mshc 檔案複製 (x86) \Microsoft Help Viewer\v2.1\ 至您的工作資料夾。
...
### <a name="adding-help-to-the-visual-studio-shell-integrated-and-isolated"></a>將說明新增至 Visual Studio Shell (整合和隔離) 
 **簡介**
...
2. [Visual Studio 2013 獨立模式 Shell 可轉散發套件](https://aka.ms/VS2013/IsoShell-LP/all)
   ...
   **概觀**
   ...
   [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)]Shell 是 [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] 您可以用來作為應用程式基礎的 IDE 版本。 這類應用程式包含隔離的 Shell 以及您所建立的擴充功能。 使用 SDK 中包含的獨立 Shell 專案範本 [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] 來建立擴充功能。

   建立隔離式 Shell 型應用程式及其說明的基本步驟：

3. [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] (Microsoft 下載中心) 取得 ISO Shell 可轉散發套件。

4. 在 Visual Studio 中，建立以隔離式 Shell 為基礎的說明延伸模組，例如本逐步解說稍後所述的 Contoso 說明延伸模組。

5. 將延伸模組和 ISO Shell 可轉散發套件包裝到部署 MSI (應用程式安裝) 。 本逐步解說不包含設定步驟。
...
- 建立名為 CatalogType.xml 的檔案，並將它新增至資料夾。 檔案應該包含下列程式程式碼：
  ...
  在登錄中定義內容存放區。 針對整合式 Shell，將 VisualStudio12 變更為產品目錄名稱：
...
8. 在您要部署 Contoso 的電腦上，從上述) ISO Shell 安裝下載的 (。

9. 在 \Program 檔案中建立資料夾 \\ (x86) \\ ，並為其命名 `Contoso` 。

10. 將 ContosoHelpShell 版本資料夾中的內容複寫到 \\ \Program 檔案 (x86) \contoso\ 資料夾。

11. 選擇 [**開始**] 功能表中的 [**執行**]，然後輸入，啟動登錄編輯程式 `Regedit` 。 在 [登錄編輯程式] 中 **，選擇 [** 檔案]，然後匯 **入**。 流覽至 ContosoHelpShell 專案資料夾。 在 [ContosoHelpShell] 子資料夾中，選擇 [ContosoHelpShell]。

12. 建立內容存放區：

     針對 ISO Shell-建立 Contoso 內容存放區 C:\ProgramData\Microsoft\HelpLibrary2\Catalogs\ContosoDev12

     若為 [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] 整合式 Shell，請建立資料夾 C:\ProgramData\Microsoft\HelpLibrary2\Catalogs\VisualStudio12

13. 建立 CatalogType.xml，並新增至內容存放區 (上一個步驟) 包含：
...
16. 用於測試內容存放區的整合式 Shell 命令列範例。 針對 ISO Shell，請視需要變更目錄和 launchingApp 值以符合產品。
...
17. 從 Contoso 應用程式根) 啟動 Contoso 應用程式 (。 在 [ISO Shell] 中，選擇 [說明 **] 功能表項目** ，然後將 [ **設定說明喜好設定** ] 變更為 [ **使用本機**說明]。

18. 在 shell 中，選擇 [說明 **] 功能表項目** ，然後 **查看**[說明]。 本機說明檢視器應該會啟動。 選擇 [ **管理內容** ] 索引標籤。在 [ **安裝來源**] 下，選擇 [ **磁片** ] 選項按鈕。 選擇 [ **...** ] 按鈕，然後流覽至包含 Contoso 內容的本機資料夾， (複製到上述步驟中的本機資料夾) 。 選擇 [Helpcontentsetup.msha] .msha。 Contoso 現在應該會顯示為書籍選取專案中的書。 選擇 [ **新增**]，然後選擇 [ **更新** ] 按鈕 (右下角) 。

19. 在 Contoso IDE 中，選擇 F1 鍵以測試 F1 功能。

### <a name="additional-resources"></a>其他資源

如需執行時間 API 的詳細資訊，請參閱 [Windows 說明 api](https://msdn.microsoft.com/library/windows/desktop/hh447318\(v=vs.85\).aspx)。
...
如需中斷問題的更新，請參閱說明 [檢視器讀我檔案](https://go.microsoft.com/fwlink/?LinkId=255960)。
