...
## <a name="implementing-statement-completion"></a>執行語句完成  
 ...
 若要執行語句完成，您的語言必須有可剖析的語句完成觸發程式。 例如， [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] 使用點 (. ) 運算子，但 [!INCLUDE[vcprvc](../../includes/vcprvc-md.md)] 使用箭號 ( >) 運算子。 語言服務可以使用一個以上的觸發程式來起始語句完成。 這些觸發程式是在命令篩選器中進行程式設計。  
