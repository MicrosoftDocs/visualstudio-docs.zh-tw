...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistHierarchyItem2>|可讓專案管理其專案專案的所有持續性層面，例如磁片上的檔案或其他儲存系統中的物件。 `IVsPeristHierarchyItem2`介面是用於不會執行介面的專案 <xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2> 。|  
