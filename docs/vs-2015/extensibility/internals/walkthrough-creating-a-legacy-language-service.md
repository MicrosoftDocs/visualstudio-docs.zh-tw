...
使用 managed package framework (MPF) 語言類別在中執行語言服務 [!INCLUDE[csprcs](../../includes/csprcs-md.md)] 相當簡單。 您需要 VSPackage 來裝載語言服務、語言服務本身，以及適用于您語言的剖析器。  
...
3. 選取 [!INCLUDE[csprcs](../../includes/csprcs-md.md)] 作為 [語言] 和 [選項]，以產生新的金鑰檔案。 按一下 [下一步]  。  
...
6. 在 MyLanguageService.cs 檔案中，新增下列 `using` 語句。  
...
1. 開啟 MyLanguagePackagePackage.cs 檔案並加入下列 `using` 語句：  
