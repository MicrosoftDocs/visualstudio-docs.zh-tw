...
ms.locfileid: "74299722"
---
# <a name="visual-studio-isolated-shell"></a>Visual Studio 獨立模式 Shell
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Visual Studio 獨立模式 shell 可讓您建立獨立的應用程式，以便與其他版本的 Visual Studio 並存執行。 它主要是用來裝載可使用 Visual Studio 服務，但也有自訂外觀和商標的特殊工具。 您可以輕鬆地開啟和關閉 Visual Studio 功能和功能表命令群組。 應用程式標題、應用程式圖示和啟動顯示畫面可以完全自訂。 如需可自訂功能的清單，請參閱[自訂獨立模式 Shell](../extensibility/customizing-the-isolated-shell.md)。  
 ...
 若要建立獨立的 shell 應用程式，請從 Visual Studio Shell 隔離專案開始。 此專案包含開發和測試您自己的獨立 shell 應用程式所需的所有專案。 當您準備好要撰寫可部署應用程式的安裝程式時，您必須從[Microsoft Visual Studio shell （獨立模式）](https://go.microsoft.com/fwlink/?LinkId=616022)可轉散發套件取得獨立的 shell 可轉散發套件。  
  
> [!NOTE]
> 您必須先填寫簡短的客戶問卷，才可以存取獨立的 shell 可轉散發套件。  填寫問卷後，系統會將您導向到 Visual Studio Connect 頁面，其中包含可轉散發套件下載連結。  您可以在後續造訪   **&#124; VISUAL Studio 2015 整合模式和獨立模式 SHELL**  索引標籤底下的 Visual Studio Connect 網站上找到下載連結。  
