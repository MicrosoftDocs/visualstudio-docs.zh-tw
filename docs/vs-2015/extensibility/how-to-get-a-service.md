...
您通常需要取得 Visual Studio 服務，才能存取不同的功能。 一般來說，Visual Studio 服務會提供一或多個您可以使用的介面。 您可以從 VSPackage 取得大部分的服務。  
 ...
 任何衍生自且已 <xref:Microsoft.VisualStudio.Shell.Package> 正確放置的 VSPackage，都可以要求任何全域服務。 因為封裝類別 <xref:System.IServiceProvider> 會執行，所以任何衍生自封裝的 VSPackage 也是服務提供者。  
...
1. 每個 Visual Studio 擴充功能都會以 VSIX 部署專案開始，其中將包含延伸模組資產。 建立 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 名為的 VSIX 專案 `GetServiceExtension` 。 您可以在 [ **新增專案** ] 對話方塊中的 **Visual c #/** 擴充性下找到 VSIX 專案範本。  
  
2. 現在，新增名為 **GetServiceCommand**的自訂命令專案範本。 在 [ **加入新專案** ] 對話方塊中，移至 **Visual c #/** 擴充性，然後選取 [ **自訂命令**]。 在視窗底部的 [ **名稱** ] 欄位中，將命令檔名稱變更為 **GetServiceCommand.cs**。 如需如何建立自訂命令的詳細資訊，請[使用功能表命令來建立擴充](../extensibility/creating-an-extension-with-a-menu-command.md)功能  
  
3. 在 GetServiceCommand.cs 中，移除 MenuItemCommand 方法的主體，並加入下列程式碼：  
