...
2. 在 `<Commands>` 之後建立空的 `<KeyBindings>` 區段（如果尚未存在）。  
   ...
   若要讓命令只能在自訂編輯器中使用，請在建立包含自訂編輯器的 VSPackage 時，將 `editor` 屬性設定為 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 封裝範本所產生的自訂編輯器名稱。 若要尋找 [名稱] 值，請在 [`<Symbols>`] 區段中尋找其 `name` 屬性以 "`editorfactory`" 結尾的 `<GuidSymbol>` 節點。這是自訂編輯器的名稱。  
  
## <a name="example"></a>範例  
 這個範例會將鍵盤快速鍵 CTRL + ALT + C 系結至名為 `MyPackage`的封裝中名為 `cmdidMyCommand` 的命令。  
...
## <a name="example"></a>範例  
 這個範例會將鍵盤快速鍵 CTL + B 系結至名為 `TestEditor`的專案中名為 `cmdidBold` 的命令。 命令只能在自訂編輯器中使用，而不是在其他編輯器中提供。  
