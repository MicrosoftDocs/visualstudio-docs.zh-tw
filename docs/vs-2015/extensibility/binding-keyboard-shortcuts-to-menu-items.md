...
4. 請嘗試不同的金鑰組合，直到找出未對應的金鑰。  
   ...
   下列程式假設您有一個具有功能表命令的現有 VSPackage。 如果您需要協助，請參閱 [使用功能表命令來建立延伸](../extensibility/creating-an-extension-with-a-menu-command.md)模組。  
...
2. `<KeyBindings>`如果不存在，請在後面建立空白區段 `<Commands>` 。  
   ...
   虛擬金鑰程式碼可讓您存取沒有相關字元的特殊索引鍵，例如，函式索引鍵和 **倒退鍵** 。 如需詳細資訊，請參閱 [虛擬金鑰代碼](https://msdn2.microsoft.com/library/ms645540.aspx)。  
  
   若要讓命令可在 Visual Studio 編輯器中使用，請將 `editor` 屬性設定為 `guidVSStd97` 。  
  
   若只要在自訂編輯器中提供命令，請將 `editor` 屬性設定為 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 當您建立包含自訂編輯器的 VSPackage 時，封裝範本所產生的自訂編輯器名稱。 若要尋找名稱值，請查看 `<Symbols>` `<GuidSymbol>` 其 `name` 屬性結尾為 "." 的節點區段 `editorfactory` 。這是自訂編輯器的名稱。  
  
## <a name="example"></a>範例  
 此範例會將鍵盤快速鍵 CTRL + ALT + C 系結至名為的封裝中名為的命令 `cmdidMyCommand` `MyPackage` 。  
...
## <a name="example"></a>範例  
 此範例會將鍵盤快速鍵 CTL + B 系結至名為的專案中名為的命令 `cmdidBold` `TestEditor` 。 此命令僅適用于自訂編輯器，而不能在其他編輯器中使用。  
