...
1. 開啟 FirstToolWindow.cs，並加入下列 `using` 語句。  
...
5. 此時 FirstToolWindow 的檢查點應該如下所示：  
...
6. 將功能表命令加入至工具列。 在 FirstToolWindowCommand.cs 類別中，新增下列 using 語句  
...
## <a name="testing-the-tool-window"></a>測試控管視窗  
...
3. 按一下該按鈕， (工具視窗中) 的搜尋圖示。 選取支援的音效或影片檔案，例如 C:\windows\media\chimes.wav，然後按 [ **開啟**]。  
