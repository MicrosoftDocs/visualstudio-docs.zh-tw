...
## <a name="remarks"></a>備註  
 ...
 `lpAuxPathLabel`參數是用來做為批註的字串，用來識別儲存在方案檔中的輔助專案路徑，並傳遞至[SccOpenProject](../extensibility/sccopenproject-function.md)呼叫中的原始檔控制外掛程式。 [!INCLUDE[vsvss](../includes/vsvss-md.md)] 使用 "SourceSafe Project：" 字串。其他原始檔控制外掛程式應避免使用這個特定的字串。  
