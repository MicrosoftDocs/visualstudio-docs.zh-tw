...
1. `TestToolWindowSearch`使用名為**TestSearch**的工具視窗，建立名為的 VSIX 專案。 如果您需要協助來執行這項作業，請參閱 [Creating an Extension with a Tool Window](../extensibility/creating-an-extension-with-a-tool-window.md)。  
...
3. 在 TestSearchControl.xaml.cs 檔案中，新增下列 using 語句：  
...
6. 在功能表列上，選擇 [ **View**]、[ **其他視窗**]、[ **TestSearch**]。  
...
1. 在 TestSearch.cs 檔案中，新增下列 `using` 語句：  
...
2. 在 `TestSearchTask` 類別中，取消批註方法中的 matchCase 行 `OnStartSearch` ：  
