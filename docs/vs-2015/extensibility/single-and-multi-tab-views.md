...
## <a name="accessing-physical-views"></a>存取實體視圖  
 ...
 若要讓您的編輯器可以存取所有可用的實體視圖，您必須為編輯器 factory 可建立的每個檔視圖物件類型建立唯一的實體視圖字串。 例如， [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 編輯器 factory 可以建立程式碼視窗和表單設計工具視窗的檔視圖物件。  
