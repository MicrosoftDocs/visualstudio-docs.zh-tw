...
|---------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Attribute** |                                                                                                                                                                                                                                                                                                                                                                                                                                                                            **定義**                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|      Uri      |                                                                                                                                                                                                                                                                                                               必URI，定義可從中載入映射的位置。 可以是下列其中一項：<br /><br /> -使用 application:///授權單位的[套件 URI](https://msdn.microsoft.com/library/aa970069\(v=vs.100\).aspx)<br />-絕對元件資源參考<br />-包含原生資源之檔案的路徑                                                                                                                                                                                                                                                                                                               |
|  背景   | 參數指出來源預定要使用的背景內容。<br /><br /> 可以是下列其中一項：<br /><br /> *Light：* 來源可以在淺背景使用。<br /><br /> <em>深色：</em>來源可以在深色背景上使用。<br /><br /> *Systeminformation.highcontrast：* 來源可以在高對比模式中的任何背景使用。<br /><br /> *HighContrastLight：* 來源可以在高對比模式中的淺色背景上使用。<br /><br /> *HighContrastDark：* 來源可以在高對比模式的深色背景上使用。<br /><br /> 如果省略 Background 屬性，則可以在任何背景使用來源。<br /><br /> 如果背景是 *淺色*、 *深色*、 *HighContrastLight*或 *HighContrastDark*，則不會反轉來源的色彩。 如果省略背景或設定為 *systeminformation.highcontrast*，則會以影像的 **AllowColorInversion** 屬性來控制來源的色彩反轉。 |
|               |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- **Microsoft.VisualStudio.ImageCatalog.dll**  
  ...
  - 如果您使用內建映射目錄 KnownMonikers，則為必要項  
...
## <a name="how-do-i-use-image-monikers-in-a-new-tool-window"></a>如何? 在新的工具視窗中使用影像的名字？  
 已更新 Visual Studio 2015 的 VSIX 封裝專案範本。 若要建立新的工具視窗，請以滑鼠右鍵按一下 VSIX 專案，然後選取 [加入新專案 ...]  (Ctrl + Shift + A) 。 在專案語言的擴充性節點下，選取 [自訂工具視窗]，並指定工具視窗的名稱，然後按 [新增] 按鈕。  

 這些是在工具視窗中使用名字標記的主要位置。 遵循各項操作的指示：  

1. 當索引標籤夠小 (也用於 Ctrl + Tab 視窗切換器) 時，工具視窗索引標籤。  
...
3. 當索引標籤夠小 (也用於 Ctrl + Tab 視窗切換器) 時，工具視窗索引標籤。  
   ...
   2. 請參閱「如何在新的工具視窗中使用影像名字？」的步驟 #1。 一節。  

4. 用來開啟工具視窗的命令。  

   - 請參閱「如何在新的工具視窗中使用影像名字？」的步驟 #2。 一節。  
...
2. 如果您只使用 **KnownMonikers**，請執行下列動作：  
   ...
   - 將 <匯入資訊清單 = "$ (ManifestFolder) \\<相對安裝目錄路徑 \> \Microsoft.VisualStudio.ImageCatalog.imagemanifest」/ \> 到 \<Symbols> 區段頂端。  
...
## <a name="testing-your-images"></a>測試您的映射  
 您可以使用影像庫檢視器工具來測試影像資訊清單，以確定一切都已正確撰寫。 您可以在 [Visual Studio 2015 SDK](visual-studio-sdk.md)中找到此工具。 您可以在 [這裡](internals/vssdk-utilities.md)找到此工具和其他工具的檔。  
...
### <a name="tooling"></a>Tooling  
 建立映射服務的一組支援工具，以協助建立/更新可搭配 Image 服務使用的 UI。 如需每個工具的詳細資訊，請參閱工具隨附的檔。 這些工具組含在 [Visual Studio 2015 SDK 中。](https://msdn.microsoft.com/library/bb166441.aspx)  
