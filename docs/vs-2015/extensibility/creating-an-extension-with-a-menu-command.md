...
1. 建立名為 **FirstMenuCommand**的 VSIX 專案。 您可以在 [ **新增專案** ] 對話方塊中的 **Visual c #/** 擴充性下找到 VSIX 專案範本。  
...
4. 在實驗實例中，開啟 [  **工具/擴充功能和更新** ] 視窗。 您應該會在這裡看到 **FirstMenuCommand** 延伸模組。  (如果您在 Visual Studio 的工作實例中開啟 [ **擴充功能和更新** ]，就不會看到 **FirstMenuCommand**) 。  
     ...
     現在，移至實驗實例中的 [ **工具** ] 功能表。 您應該會看到 **Invoke FirstCommand** 命令。 此時只會顯示一個訊息方塊，指出「FirstCommandPackage 內部 FirstMenuCommand. FirstCommand. MenuItemCallback ( # A1」。 在下一節中，我們將瞭解如何從此命令實際啟動「記事本」。  
...
3. 移除 MenuItemCallback 方法，並新增只會啟動「記事本」的 StartNotepad 方法：  
...
4. 現在就試試看。當您開始對專案進行偵錯工具並按一下 [ **工具]/**[叫用 FirstCommand] 時，您應該會看到 [記事本] 的實例。  
     ...
     您可以使用類別的實例 <xref:System.Diagnostics.Process> 來執行任何可執行檔，而不只是 [記事本]。 例如，使用 calc.exe 來試用。  
  
## <a name="cleaning-up-the-experimental-environment"></a>清除實驗環境  
 如果您正在開發多個擴充功能，或只是使用不同版本的延伸模組程式碼探索結果，您的實驗環境可能會以它的方式停止運作。 在此情況下，您應該執行重設腳本。 這稱為「 **重設」 Visual Studio 2015 實驗實例**，並隨附 Visual Studio SDK 的一部分。 此腳本會從實驗環境中移除擴充功能的所有參考，讓您可以從頭開始。  
  
 您可以透過下列兩種方式之一來取得此腳本：  
  
1. 在桌面上，尋找 **[重設 Visual Studio 2015 實驗實例**]。  
 ...
 **\<code directory>\FirstMenuCommand\FirstMenuCommand\bin\Release\ FirstMenuCommand .vsix**  
 ...
 若要安裝此擴充功能，您的 friend 必須關閉 Visual Studio 的所有開啟的實例，然後按兩下 .vsix 檔，這會顯示 **Vsix 安裝程式**。 這些檔案會複製到 **%LocalAppData%\Microsoft\VisualStudio\14.0\Extensions** 目錄中。  
  
 當您的朋友再次帶出 Visual Studio 時，他會在 [ **工具/擴充功能和更新**] 中找到 FirstMenuCommand 延伸模組。 他也可以前往 [ **延伸模組和更新** ] 來卸載或停用延伸模組。  
...
1. 您可以使用簡單的功能表命令來完成更多工：  
   ...
   2. 變更功能表命令的文字：變更 [功能表命令的文字](../extensibility/changing-the-text-of-a-menu-command.md)  
  
   3. 新增命令的功能表快捷方式：將 [鍵盤快速鍵系結至功能表項目](../extensibility/binding-keyboard-shortcuts-to-menu-items.md)  
...
5. 將選項和屬性頁面和使用者設定新增至您的延伸模組： [擴充屬性和屬性視窗](../extensibility/extending-properties-and-the-property-window.md) ，以及 [擴充使用者設定和選項](../extensibility/extending-user-settings-and-options.md)  
  
   其他類型的延伸模組需要更多工作，例如建立新的專案類型 ([擴充專案](../extensibility/extending-projects.md)) 、建立新的編輯器類型 ([建立自訂編輯器和設計](../extensibility/creating-custom-editors-and-designers.md) 工具) ，或是在隔離的 shell 中執行您的延伸模組： [Visual Studio 獨立模式 shell](../extensibility/visual-studio-isolated-shell.md)
