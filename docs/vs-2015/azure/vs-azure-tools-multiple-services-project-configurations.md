...
在 [設定] 頁面上，您可以利用成對的名稱和數值形式將設定加入組態。 在角色中執行的程式碼可以在執行階段使用 [Azure 受控程式庫](/previous-versions/azure/dn602775(v=azure.11)) 所提供的類別 (具體而言，使用 [GetConfigurationSettingValue](/previous-versions/azure/reference/ee772857(v=azure.100)) 方法) 讀取組態設定的值。
