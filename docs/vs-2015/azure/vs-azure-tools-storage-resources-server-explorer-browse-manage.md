...
如果您直接在篩選文字方塊中輸入 WCF Data Services 篩選字串，您也可以篩選出現在資料表設計工具中的實體資料。 這種類型的字串與 SQL WHERE 子句相似，但是會傳送至伺服器做為 HTTP 要求。 如需有關如何建立篩選字串的詳細資訊，請參閱為 [資料表設計工具建立篩選字串](/azure/vs-azure-tools-table-designer-construct-filter-strings)。
