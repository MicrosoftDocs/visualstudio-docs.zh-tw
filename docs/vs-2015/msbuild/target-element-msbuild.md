...
|`Inputs`|選擇性屬性。<br /><br /> 構成此目標輸入的檔案。 若有多個檔案，則會以分號分隔。 檔案的時間戳記將會與 `Outputs` 中的檔案時間戳記相比較，以判斷 `Target` 是否為最新狀態。 如需詳細資訊，請參閱 <<c0> [ 累加建置](../msbuild/incremental-builds.md)， [How to:累加建置](../msbuild/how-to-build-incrementally.md)，並[轉換](../msbuild/msbuild-transforms.md)。|  
|`Outputs`|選擇性屬性。<br /><br /> 構成此目標輸出的檔案。 若有多個檔案，則會以分號分隔。 檔案的時間戳記將會與 `Inputs` 中的檔案時間戳記相比較，以判斷 `Target` 是否為最新狀態。 如需詳細資訊，請參閱 <<c0> [ 累加建置](../msbuild/incremental-builds.md)， [How to:累加建置](../msbuild/how-to-build-incrementally.md)，並[轉換](../msbuild/msbuild-transforms.md)。|  
