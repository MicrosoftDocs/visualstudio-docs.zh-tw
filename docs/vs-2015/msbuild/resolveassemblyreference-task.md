...
ms.locfileid: "47491790"
---
# <a name="resolveassemblyreference-task"></a>ResolveAssemblyReference 工作
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

本主題的最新的版本可從[ResolveAssemblyReference 工作](https://docs.microsoft.com/visualstudio/msbuild/resolveassemblyreference-task)。  
...
|`FullFrameworkFolders`|選擇 [String] (<!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  -->)`[]`參數。<br /><br /> 指定包含 RedistList 目錄的資料夾集合。 此目錄代表指定用戶端設定檔的完整架構，例如：%programfiles%\reference assemblies\microsoft\framework\v4.0。|  
...
|`IgnoreDefaultInstalledAssemblyTables`|選擇性的 `Boolean` 參數。<br /><br /> 如果為 `true`，則工作會搜尋並使用在 `TargetFrameworkDirectories`。 預設值為 `false.`|  
|`IgnoreDefaultInstalledAssemblySubsetTables`|選擇性的 `Boolean` 參數。<br /><br /> 如果為 `true`，則工作會搜尋並使用在 `TargetFrameworkDirectories`。 預設值為 `false.`|  
...
|`ProfileName`|選擇性的 <!-- TODO: review code entity reference <xref:assetId:///String?qualifyHint=False&amp;autoUpgrade=True>  --> 參數。<br /><br /> -   指定要作為目標之 Framework 設定檔的名稱。 例如，用戶端、Web 或網路。|  
...
|`SearchPaths`|必要的 `String[]` 參數。<br /><br /> 指定要搜尋的目錄或特殊位置，以找出磁碟上代表組件的檔案。 搜尋路徑的列示順序很重要。 對每個組件而言，都會從左至右搜尋路徑清單。 當找到代表組件的檔案時，此搜尋會停止並開始搜尋下一個組件。<br /><br /> 此參數可接受下列類型的值：<br /><br /> -目錄路徑。<br />-{HintPathFromItem}: 指定此工作將檢查`HintPath`的基底的項目中繼資料。<br />-{CandidateAssemblyFiles}: 指定工作會檢查透過傳遞的檔案`CandidateAssemblyFiles`參數。<br />-{Registry: _AssemblyFoldersBase\_，_RuntimeVersion\_，_AssemblyFoldersSuffix\_}:<br />-{AssemblyFolders}: 指定工作會使用 Visual Studio.NET 2003 尋找組件-從登錄配置。<br />-{GAC}: 指定工作會搜尋在 GAC 中。<br />-{RawFileName}: 指定工作會考慮`Include`是確切的路徑和檔案名稱的項目值。|  
