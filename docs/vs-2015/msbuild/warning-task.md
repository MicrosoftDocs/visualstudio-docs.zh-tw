...
## <a name="remarks"></a>備註  
 ...
 如果 `Warning` 工作的 `Condition` 參數評估為 `true`，則會記錄 `Text` 參數的值，而建置會繼續執行。 如果 `Condition` 參數不存在，則會記錄警告文字。 如需有關記錄的詳細資訊，請參閱 [取得組建記錄](../msbuild/obtaining-build-logs-with-msbuild.md)檔。  
