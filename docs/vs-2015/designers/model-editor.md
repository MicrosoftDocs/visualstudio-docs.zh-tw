...
#### <a name="to-add-a-3-d-model-to-your-project"></a>若要加入 3D 模型至您的專案

1. 在 [方案總管]**** 中，開啟您想要在其中新增影像之專案的捷徑功能表，然後選擇 [新增]****，[新增項目]****。

2. 在 [新增項目]**** 對話方塊中，選取 [已安裝]**** 下方的 [圖形]****，然後選取 [3D 場景 (.fbx)]****。
...
### <a name="axis-orientation"></a>座標軸方向
 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 支援立體軸的每個方向，並從支援的模型檔案格式載入軸方向資訊。 如果沒有指定座標軸方向，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 預設會使用右手座標系統。 [axis indicator] (座標軸指示器)**** 會在設計介面的右下角顯示目前座標軸方向。 在 [axis indicator] (座標軸指示器)**** 上，紅色代表 X 軸、綠色代表 Y 軸，而藍色代表 Z 軸。

### <a name="beginning-your-3-d-model"></a>開始使用 3D 模型
 在模型編輯器中，每個新物件的開頭都是模型編輯器內建的其中一個基本3D 圖形 *（或基本型*別）。 若要建立新的且唯一的物件，您可以在場景中加入基本類型，然後再修改其頂點以變更其圖形。 對於複雜的圖形，您可以使用立體化或細分來加入其他頂點，然後再加以修改。 如需如何將基本物件新增至場景的詳細資訊，請參閱 [建立和匯入3d 物件](#Adding3DObjects)。 如需如何將更多頂點新增至物件的資訊，請參閱[修改物件](#ModifyingObjects)。

## <a name="working-with-the-model-editor"></a>使用模型編輯器
 下列各節說明如何使用模型編輯器來使用 3D 模型。

### <a name="model-editor-toolbars"></a>模型編輯器工具列
 模型編輯器工具列包含可協助您使用 3D 模型的命令。

 影響 [模型編輯器] 狀態的命令位於主要 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 視窗的 [模型編輯器模式]**** 工具列。 模型工具和指令化命令位於 [模型編輯器] 設計介面的 [模型編輯器]**** 工具列。

 以下是 [模型編輯器模式]**** 工具列：

 ![[模型檢視器] 的 [模式] 工具列。](../designers/media/digit-mre-modal-toolbar.png "數位深入-強制回應-工具列")
...
|**移動瀏覽**|讓 3D 場景相對於視窗框架進行移動。 若要移動瀏覽，請選取場景中的點並四處移動它。<br /><br /> 在 [選取]**** 模式中，您可以按住 Ctrl 暫時啟用 [移動瀏覽]**** 模式。|
|**縮放**|使其相對於視窗框架顯示或多或少的場景細節。 在 [縮放]**** 模式中，選取場景中的一點，然後左右移動進行放大，或上下移動進行縮小。<br /><br /> 在 [選取]**** 模式中，您可以在按住 Ctrl 時使用滑鼠滾輪來縮放。|
...
|**檢視**|設定檢視方向。 以下是可用的方向：<br /><br /> **Front**<br /> 將檢視放置在場景前方。<br /><br /> **上一步**<br /> 將檢視放置在場景後方。<br /><br /> **離開**<br /> 將檢視放置在場景左方。<br /><br /> **對**<br /> 將檢視放置在場景右方。<br /><br /> **前幾個**<br /> 將檢視放置在場景上方。<br /><br /> **下層**<br /> 將檢視放置在場景下方。 **注意：** 這是在啟用 [正視]**** 投影時變更檢視方向的唯一方法。|
|**Projection**|設定用來繪製場景的投影。 以下是可用的投影：<br /><br /> **檢視方塊**<br /> 在透視投影中，離觀測點愈遠的物件看起來愈小，最後在遠方聚集成一點。<br /><br /> **正視**<br /> 在正射投影中，不論物件與觀測點的距離為何，物件看起來都是相同大小。 沒有顯示任何聚合。 啟用 [正視]**** 投影時，您不能使用 [軌跡]**** 模式來放置檢視。|
...
### <a name="controlling-the-view"></a>控制檢視
...
|**正視**|在正射投影中，不論物件與觀測點的距離為何，物件看起來都是相同大小。 沒有顯示任何聚合。 啟用 [正視]**** 投影時，您不能使用 [軌跡]**** 模式隨機放置檢視。|
 ...
 您可能會發現從已知位置和角度檢視 3D 場景是很好的作法，例如當您想要比較兩個類似的場景時。 對於這種情況，模型編輯器提供數個預先定義的檢視。 若要使用預先定義的檢視，請在 [模型編輯器模式]**** 工具列上選擇 [檢視]****，然後選擇您想要的預先定義檢視：前面、背面、左邊、右邊、上方或下方。 在這些檢視中，虛擬觀景窗會直接對準場景的原點。 例如，如果您選擇 [檢視最上層]****，虛擬觀景窗會從場景正上方對準其原點。

### <a name="viewing-additional-geometry-details"></a>檢視其他幾何詳細資料
 若要進一步了解 3D 物件或場景，您可以檢視其他幾何詳細資料 (例如每個頂點法線、每個曲面法線、作用中選取範圍的軸點，以及其他詳細資料)。 若要啟用或停用它們，請在 [模型編輯器]**** 工具列上依序選擇 [指令碼]**** 和 [檢視]****，然後選擇您想要的項目。

### <a name="creating-and-importing-3-d-objects"></a><a name="Adding3DObjects"></a> 建立和匯入 3D 物件
 若要將預先定義的3D 圖形新增至場景，請在 [ **工具箱**] 中選取您要的圖形，然後將它移至設計介面。 新的圖形是放在場景的原點上。 模型編輯器提供七種圖形：[圓錐體]****、[立方體]****、[圓柱體]****、[實心圓]****、[平面]****、[圓球]**** 和 [茶壺]****。

 若要從檔案匯入3D 物件，請在 [ **模型編輯器** ] 工具列上，選擇 [ **Advanced**]、[ **場景管理**]、[匯 **入**]，然後指定您要匯入的檔案。
...
### <a name="modifying-objects"></a><a name="ModifyingObjects"></a> 修改物件
...
|光澤度|描述反射反白顯示的寬度和濃度。|
 ...
 您可以使用 **著色器設計** 工具來建立自訂著色器材質，以套用至立體場景中的物件。 如需如何建立自訂著色器材質的資訊，請參閱[著色器設計工具](../designers/shader-designer.md)。 如需如何將自訂著色器材質套用至物件的詳細資訊，請參閱 [如何：將著色器套用至3D 模型](../designers/how-to-apply-a-shader-to-a-3-d-model.md)。
