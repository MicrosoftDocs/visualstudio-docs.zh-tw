...
3. ###### <a name="if-the-product-version-is-140247200-or-140247201-follow-these-steps"></a>如果產品版本為 14.0.24720.0 或 14.0.24720.1，請遵循下列步驟：
   ...
   3. 建立和修改 AdminDeployment.xml 檔案。 若要這樣做，請使用`/CreateAdminFile`\<檔案位置 >命令列參數。 (如需詳細資訊，請參閱本文的「以自動模式部署 Visual Studio」一節。)
...
5. ###### <a name="for-other-product-version-values-follow-these-steps"></a>如需其他產品版本值，請依照下列步驟進行：
   ...
   3. 建立 AdminDeployment.xml 檔案，然後予以修改。 若要這樣做，請使用`/CreateAdminFile`\<檔案位置 >命令列參數。 (如需詳細資訊，請參閱本文的「以自動模式部署 Visual Studio」一節。)
