---
title: 針對 Windows Workflow Foundation (舊版) 叫用偵錯工具 |Microsoft Docs
ms.date: 11/15/2016
...
- 不進入程式：您可以使用**F10**來**跳過活動**。 不進入複合活動時。 偵錯工具會在複合活動的第一個可執行子系上中斷。 當不進入非複合（例如 **CodeActivity** 活動）時，偵錯工具會執行活動及其相關聯的處理常式，並在下一個活動上中斷。 如果執行的活動是複合活動中最後一個子活動，則在執行後，偵錯工具會在父活動上中斷。

## <a name="attaching-to-a-process"></a>附加至處理序
 若要將工作流程附加至進程，請在 [**附加至進程**] 對話方塊的 [**可使用的進程**] 清單方塊中，選取可用的進程。 如果 **自動：工作流程程式碼** 不會顯示在 [ **附加至** ] 文字方塊中，請按一下 [ **選取**]。 在 [ **選取程式碼類型** ] 對話方塊中，按一下 [ **Debug 這些程式碼類型** ]，然後選取 [ **工作流程**]。 然後按一下 **[確定]** ，再按一下 [ **附加**]。

## <a name="debugging-with-f5"></a>以 F5 偵錯
 如果工作流程主應用程式和工作流程 DLL 位於不同的 Visual Studio 專案中，例如，當您使用工作流程活動程式庫時，您必須將工作流程 DLL 專案設定為 Visual Studio 方案啟始專案，以使用 **F5**來偵測工作流程。 您也必須在工作流程 DLL 專案的 [ **啟動外部程式** ] 屬性中，設定主應用程式的路徑。

 若要在方案總管中設定啟始專案，請以滑鼠右鍵按一下專案名稱，然後選取 [ **設定為啟始專案**]。 若要在 [ **啟動外部程式** ] 屬性中設定主機的路徑，請在方案總管中按兩下工作流程專案的 [ **屬性** ] 節點，然後選取 [ **調試** 程式] 索引標籤。在 [ **起始動作**] 下，選取 [ **啟動外部程式** ]，並輸入裝載您要進行偵錯工具之工作流程的 .exe 檔案路徑。
...
## <a name="see-also"></a>另請參閱
 [如何：在工作流程中設定中斷點 (舊版) ](../workflow-designer/how-to-set-breakpoints-in-workflows-legacy.md)偵測 [舊版工作流程](../workflow-designer/debugging-legacy-workflows.md)