...
### <a name="using-the-flowdecision-designer"></a>使用 FlowDecision 設計工具
 [ **FlowDecision** ] 設計工具位於 [**工具箱**] 的 [**流程圖**] 類別中，若要存取，請按一下 (上的 [**工具箱**] 索引標籤， [!INCLUDE[wfd2](../includes/wfd2-md.md)] 或從 [ **VIEW** ] 功能表選取 [**工具列**]，或按 CTRL + ALT + X。 ) 

 **FlowDecision**設計**工具**可以從 [工具箱] 拖曳出來，放到 [ [!INCLUDE[wfd2](../includes/wfd2-md.md)] **Flowchart** ] 活動設計工具內的介面上。 這會在 <xref:System.Activities.Statements.FlowDecision> 活動中建立加上標籤的 **決策** <xref:System.Activities.Statements.Flowchart> 。 滑鼠移到設計工具上，會出現兩個分支的 **True** 和 **False** 方形控點。
