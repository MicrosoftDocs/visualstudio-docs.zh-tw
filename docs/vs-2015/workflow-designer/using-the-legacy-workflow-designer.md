...
[!INCLUDE[wfd2](../includes/wfd2-md.md)] 提供的舊版 [!INCLUDE[vs2010](../includes/vs2010-md.md)] 可用來以 [!INCLUDE[netfx35_long](../includes/netfx35-long-md.md)] 或 [!INCLUDE[vstecwinfx](../includes/vstecwinfx-md.md)] 為目標。
 ...
 您可以在 [**新增專案**] 視窗上方的下拉式清單中，選取 [ **.NET Framework 3.0** ] 選項或 [ **.NET Framework 3.5** ] 選項來存取它。 中的預設選項 [!INCLUDE[vs2010](../includes/vs2010-md.md)] 是 **.NET Framework 4** ，可用來建立以做為 [!INCLUDE[wf](../includes/wf-md.md)] 目標的應用程式 [!INCLUDE[netfx40_short](../includes/netfx40-short-md.md)] 。
...
|功能|描述|
|-------------|-----------------|
|活動拖放|將 [ **工具箱** ] 中的活動拖曳至設計介面，以建立工作流程。|
|屬性瀏覽器|中的 [標準 **屬性** ] 視窗 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 是用來設定活動的屬性。|
|Zoom|雙線 **縮放層級** 圖示位於設計介面右側垂直捲動條的下方。 按一下望遠鏡並選取縮放百分比，使工作流程圖形放大或縮小。您也可以使用 **平移** 圖示放大鏡游標選項來放大和縮小。|
|移動瀏覽|移動 **流覽圖示（** 包含四個十字箭號的圓形會以四個方向表示）位於設計介面右側垂直捲動條的下方，緊接在雙線縮放圖示的正下方。 如果您按一下平移圖示，會顯示快顯功能表來提供以下的游標選項：<br /><br /> - **放大** 放大鏡游標可讓您按一下設計介面來放大。<br />- **縮小** 放大鏡游標可讓您按一下設計介面來縮小。<br />- **導覽工具** 手游標可讓您在設計介面中「抓取」和移動工作流程的視圖。<br />- **預設** 箭號游標可讓您從其他資料指標切換回預設箭號游標。|
...
|智慧標籤|設定不完整或是設定無效的活動會以驚嘆號圖示標記。 您可以按一下圖示，便會看到一個下拉式清單，其中列出活動中的組態需求。 然後，您可以使用 [ **屬性** ] 視窗來適當地設定活動。 當活動的所有屬性都有效時，驚嘆號圖示便會消失。|
...
## <a name="see-also"></a>另請參閱
 [開發工作流程](https://msdn2.microsoft.com/library/bb628448.aspx)
