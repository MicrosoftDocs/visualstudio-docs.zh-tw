---
title: 如何： (舊版) 執行 Windows Communication Foundation 合約作業 |Microsoft Docs
ms.date: 11/15/2016
...
1. 在設計工具中按兩下 [ **ReceiveActivity** ] 活動，或在 [**屬性**] 窗格中按一下 [ **ServiceOperationInfo** ] 屬性旁邊的省略號。
...
2. 執行下列其中一個動作：
   ...
   - 按一下對話方塊右上角的 [匯 **入** ]。 [ [流覽並選取 .Net 類型] 對話方塊會 (舊版) ](../workflow-designer/browse-and-select-a-dotnet-type-dialog-box-legacy.md) 開啟。 搜尋含有您所要之合約的組件或專案。 選取合約，然後按一下 **[確定]**。

     建立或匯入合約之後，您可以將新作業加入至該建立或匯入的合約。 若要加入新的作業，請選取合約，然後按一下對話方塊右上角的 [ **新增** 作業]。 完成新增作業時，請繼續進行步驟 3。
...
3. 選取您要與 **ReceiveActivity** 活動相關聯的作業。 您可以變更作業的名稱、參數、屬性和權限設定，藉此操作作業的定義。
    ...
    按一下 [ **內容] 索引標籤，即可** 存取作業保護層級和作業的支援訊息交換功能。

    按一下 [ **許可權** ] 索引標籤，以指定允許哪些群組 (s) 可以執行此作業。

4. 按一下 **[確定]** ，[ **ReceiveActivity** ] 活動會顯示其正在執行之作業的作業名稱。

5. 將您將用於在 **ReceiveActivity** 活動中執行該作業的工作流程活動。
