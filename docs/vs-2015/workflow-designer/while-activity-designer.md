...
當指定的條件評估為**true**時，<xref:System.Activities.Statements.While> 活動會執行其 <xref:System.Activities.Statements.While.Body%2A> 中包含的活動。 包含的活動可能永遠不會執行。 如果您要包含的活動至少執行一次，請改用 <xref:System.Activities.Statements.DoWhile> 活動。
