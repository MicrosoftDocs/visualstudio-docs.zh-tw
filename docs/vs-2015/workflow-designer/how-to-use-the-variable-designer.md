...
變數設計工具適用於建立變數，以用於資料繫結的狀況以及條件陳述式。 按一下設計工具畫布左下角的 [**變數**] 按鈕，即可存取設計工具。 設計工具組含以表格形式出現的變數清單，而且可以依每個資料行標頭來排序，但**預設**資料行除外。 每個變數都會包含名稱、變數型別、範圍與預設值 (如果有的話)。 名稱和預設值都是可編輯的文字欄位，而型別和範圍都可從下拉式清單中選取。 範圍是叫用變數設計工具時選取的活動。 如果無法在選取範圍內建立變數，則範圍會預設為離選取最近的祖系活動，以便在其範圍內建立變數。 [!INCLUDE[crabout](../includes/crabout-md.md)] 變數，請參閱[變數和自](https://msdn.microsoft.com/library/d03dbe34-5b2e-4f21-8b57-693ee49611b8)變數。
...
## <a name="see-also"></a>請參閱
 [使用工作流程設計工具](../workflow-designer/using-the-workflow-designer.md)[變數和引數](https://msdn.microsoft.com/library/d03dbe34-5b2e-4f21-8b57-693ee49611b8)[如何：使用引數設計](../workflow-designer/how-to-use-the-argument-designer.md)工具