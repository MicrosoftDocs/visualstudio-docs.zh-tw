...
2. 套用全部展開或全部折迭的狀態之後，您可以按一下出現的 [**還原**] 按鈕，返回查看先前套用至每個活動的狀態。
    ...
    > [!WARNING]
    > 如果某個活動（例如 <xref:System.Activities.Statements.Flowchart>）已選擇不就地展開，則會停用 [**流程圖**] 設計工具上與 [全部**展開**] 和 [**全部**折迭] 按鈕相關聯的功能。 [!INCLUDE[crabout](../includes/crabout-md.md)] [**流程圖**] 設計工具中，請參閱[流程圖](../workflow-designer/flowchart-activity-designer.md)主題。
