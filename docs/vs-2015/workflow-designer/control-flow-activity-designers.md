...
[!INCLUDE[wfd1](../includes/wfd1-md.md)] 包含一些系統提供的活動，可讓您在建立工作流程時使用。 本節包含用來控制工作流程中之流程的系統提供活動。 下列主題說明會這些活動，並且提供如何使用的指引。

## <a name="in-this-section"></a>本節內容
 [DoWhile](../workflow-designer/dowhile-activity-designer.md) 執行包含在主體中的活動至少一次，直到指定的條件評估為 **true**為止。

 [ForEach \<T> ](foreach-t-activity-designer.md)針對指定集合中的每個專案，執行其主體中包含的活動。

 [If](../workflow-designer/if-activity-designer.md) 根據評估的結果，評估條件並執行活動。
 ...
 [挑選](../workflow-designer/pick-activity-designer.md) 執行數個分支中的其中一個，以回應提供事件型流量控制的某些事件。

 [PickBranch](../workflow-designer/pickbranch-activity-designer.md) 提供活動內執行的潛在路徑 <xref:System.Activities.Statements.Pick> 。

 [順序](../workflow-designer/sequence-activity-designer.md) 包含依序執行之子活動的已排序集合。

 [切換 \<T> ](switch-t-activity-designer.md)評估指定的運算式，並從關聯索引鍵符合評估所取得值的活動集合執行活動。

 [While](../workflow-designer/while-activity-designer.md) 當指定的條件評估為 **true**時，執行其主體中包含的活動。
