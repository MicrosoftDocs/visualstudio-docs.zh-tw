...
## <a name="using-the-state-activity-designer"></a>使用 State 活動設計工具
 若要將加入 <xref:System.Activities.Statements.State> 至工作流程，請從 [工具箱] 的 [**狀態機器**] 區段中，將 [**狀態**] 活動設計**工具**拖放到 <xref:System.Activities.Statements.StateMachine> 介面上的活動 [!INCLUDE[wfd1](../includes/wfd1-md.md)] 。 <xref:System.Activities.Statements.State> 活動稍後可以放到 <xref:System.Activities.Statements.StateMachine> 和加入的轉換，或是可以在放置 <xref:System.Activities.Statements.State> 活動時建立轉換。 若要 <xref:System.Activities.Statements.State> 在一個步驟中加入活動並建立轉換，請從 [工具箱] 的 **[狀態機器**] 區段中拖曳 [**狀態**] 活動，並將其停留在工作流程設計**工具**中的另一個狀態。 當被拖曳的 <xref:System.Activities.Statements.State> 位在另一個 <xref:System.Activities.Statements.State> 上方時，另一個 <xref:System.Activities.Statements.State> 周圍會出現四個三角形。 如果將 <xref:System.Activities.Statements.State> 拖放到四個三角形的其中一個，就會將它加入到該狀態機器，並建立從來源 <xref:System.Activities.Statements.State> 到所放置之目的地 <xref:System.Activities.Statements.State> 的轉換。 如需詳細資訊，請參閱 [轉換](../workflow-designer/transition-activity-designer.md)。
