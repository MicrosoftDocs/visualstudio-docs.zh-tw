...
## <a name="using-the-sequence-activity-designer"></a>使用 Sequence 活動設計工具
 若要加入 <xref:System.Activities.Statements.Sequence> 活動，請將 [ **Sequence** ] 活動設計**工具**從 [工具箱] 拖曳至 [[!INCLUDE[wfd1](../includes/wfd1-md.md)]] 介面上。 若要將子活動加入此 <xref:System.Activities.Statements.Sequence> 活動，請從 [**工具箱**] 拖曳一些其他活動，並將它放在方塊中的三角形上，並出現提示文字「在此放置活動」。
