...
在 Visual Studio 中建立應用程式以進行使用者帳戶控制 (UAC) 在 Windows Vista 上，通常會產生內嵌的資訊清單，並在應用程式的可執行檔中編碼為二進位 XML 資料。 因為 ClickOnce 和免註冊的 COM 應用程式需要外部資訊清單，所以 Visual Studio 會為這些類型的專案產生檔案，其中包含 UAC 資料，而不是內嵌的資訊清單。 根據預設，Visual Studio 會使用名為 app.config 的檔案中的資訊，為 ClickOnce 和免註冊的 COM 部署) 產生外部 UAC 資訊清單資訊 (，或將它內嵌在應用程式的可執行檔 (，以) 所有其他情況。 Visual Studio 針對資訊清單產生提供下列選項：  
  
- 使用內嵌資訊清單。 將 UAC 資料內嵌在應用程式的可執行檔中，並以一般使用者的形式執行。  
  
   除非您使用 ClickOnce) ，否則這是預設的 (設定。 這項設定將支援在 Windows Vista 上進行 Visual Studio 操作的一般方式;也就是，使用同時產生內部和外部資訊清單 `AsInvoker` 。  
  
- 使用外部資訊清單。 使用 app.config 產生外部資訊清單。  
  
   這只會使用 app.config 中的資訊來產生外部資訊清單。 當您使用 ClickOnce 或免註冊的 COM 發行應用程式時，Visual Studio 會將 app.config 新增至專案，並新增這個選項。  
...
- 不使用資訊清單。 建立沒有資訊清單的應用程式。  
  ...
  請注意，在 Visual Studio IDE 中設定 UAC 資訊清單產生的方法，會因 Visual c # (和 Visual Basic) 的專案類型而有所不同。  
