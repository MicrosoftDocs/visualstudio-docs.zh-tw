---
title: 逐步解說：使用設計工具以 ClickOnce 部署 API 依需求下載附屬元件 |Microsoft Docs
ms.date: 11/15/2016
...
4. 選取 [顯示所有檔案]**** 核取方塊，顯示附屬組件。 所有附屬組件預設都會包含在部署中，而且會顯示在這個對話方塊中。
     ...
     附屬元件的名稱格式會是 *isoCode*\ApplicationName.resources.dll，其中 *isoCode* 是 RFC 1766 格式的語言識別項。

5. 在每個語言識別項的 [**下載群組**] 清單中，按一下 [**新增]。** 在系統提示您輸入下載群組名稱時，請輸入語言識別項。 例如，若為日文附屬元件，您可以指定下載組名 `ja-JP` 。
