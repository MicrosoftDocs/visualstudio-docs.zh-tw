...
[!INCLUDE[vsipsdk](../includes/vsipsdk-md.md)] 可讓 VSPackage (例如編輯器或設計工具) 管理 [工具箱] **** 的成員資格和外觀。  
 ...
 此外，[工具箱] **** 本身可以使用自動化進行管理。 如需透過自動化管理工具箱的詳細資訊，請參閱 [How to: Control the Toolbox](https://msdn.microsoft.com/library/c9d8a18a-d2bc-43d4-a803-601bfc6a6599)。  
...
|介面|描述|  
|---------------|-----------------|  
|<xref:System.Drawing.Design.IToolboxService>|允許應用程式從 [工具箱] 管理、加入和移除 <xref:System.Drawing.Design.ToolboxItem> 物件。 **Toolbox** 也會啟用外觀和 [工具箱] **** 類別的設定。|  
...
- 無法使用將控制項直接加入 [ **工具箱** ] <xref:System.Drawing.Design.IToolboxService> 。  
...
- VSPackage 必須使用 <xref:Microsoft.VisualStudio.Shell.Interop.IVsToolbox2> 來加入控制項，或是將控制項裝載在衍生自 <xref:System.Windows.Forms.AxHost> 的包裝函式控制項中。  
   ...
   Visual Studio 提供 `Aximp.exe` 工具，來自動化在衍生自 <xref:System.Windows.Forms.AxHost> 之控制項中的 ActiveX 控制項包裝。 如需詳細資訊，請參閱 [Aximp.exe (Windows Forms ActiveX 控制項匯入工具) ](https://msdn.microsoft.com/library/482c0d83-7144-4497-b626-87d2351b78d0)。  
