...
編譯單位資訊會在具有標記的符號之間分割 `SymTagCompiland` (低詳細資料) 和 `SymTagCompilandDetails` 標記 (高詳細資料) 。 `SymTagCompilandDetails` 需要載入其他符號。 但是，它會提供有關符號無法使用之編譯單位的豐富資訊 `SymTagCompiland` 。  
...
|[IDiaSymbol::get_hasSecurityChecks](../../debugger/debug-interface-access/idiasymbol-get-hassecuritychecks.md)|`BOOL`|`TRUE` 如果已使用 [/gs (緩衝區安全性檢查 ](https://msdn.microsoft.com/library/8d8a5ea1-cd5e-42e1-bc36-66e1cd7e731e) 來編譯編譯單位，) 編譯器參數僅 (DIA SDK 8.0 或更新版本) 。|  
...
|[IDiaSymbol::get_isHotpatchable](../../debugger/debug-interface-access/idiasymbol-get-ishotpatchable.md)|`BOOL`|`TRUE` 如果編譯單位是使用/hotpatch 編譯的， [ (建立可線上修補映射) ](https://msdn.microsoft.com/library/aad539b6-c053-4c78-8682-853d98327798) 編譯器參數只 (DIA SDK 8.0 版或更新版本) 。|  
|[IDiaSymbol::get_isLTCG](../../debugger/debug-interface-access/idiasymbol-get-isltcg.md)|`BOOL`|`TRUE` 如果編譯單位是以 [/ltcg (連結時產生程式碼，) ](https://msdn.microsoft.com/library/788c6f52-fdb8-40c2-90af-4026ea2cf2e2) 編譯器參數 (只在 DIA SDK 8.0 或更新版本的) 中。|  
