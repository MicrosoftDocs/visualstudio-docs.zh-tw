...
## <a name="how-to-deploy-a-windows-store-app"></a><a name="BKMK_How_to_deploy_a_Windows_Store_app"></a> 如何部署 Windows Store 應用程式
 手動部署應用程式是一個簡單的流程：
 ...
 **允許網路回送** 基於安全性考慮， [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] 不允許以標準方式安裝的應用程式對其安裝所在的裝置進行網路呼叫。 根據預設，Visual Studio 部署會針對部署應用程式建立此規則的豁免。 此豁免可讓您測試在單一機器上的通訊程序。 在將您的應用程式提交至 [!INCLUDE[win8_appstore_long](../includes/win8-appstore-long-md.md)]之前，您應該在沒有豁免的情況下測試您的應用程式。
...
- 在 [JavaScript 偵錯] 屬性頁上，將 [允許網路回送] **** 值設定為 [否] ****。
  ...
  不**會啟動，但會在程式碼啟動 (c # 和 VB) /啟動應用程式 (JavaScript 和 c + + 時進行偵錯工具) **若要將部署設定為在應用程式啟動時自動啟動偵錯工具：
