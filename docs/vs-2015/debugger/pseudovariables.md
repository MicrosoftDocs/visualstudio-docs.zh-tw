...
|`$exceptionstack`|顯示目前 Windows 執行階段例外狀況的堆疊追蹤。 `$ exceptionstack` 只適用于在 Windows 8.1 或更新版本上執行的 Store 應用程式。 針對 C++ 和 SHE 例外狀況，不支援 `$ exceptionstack`。|  
|`$ReturnValue`|顯示 .NET Framework 方法的傳回值。 請參閱 [檢查方法呼叫的傳回值](https://msdn.microsoft.com/library/e3245b37-8e2e-4200-ba84-133726e95f1f)|  
...
|`$` *N* `#`|顯示物件識別碼等於 *N* 的物件。|  
|`$dynamic`|針對實作 `IDynamicMetaObjectProvider` 的物件，顯示特殊 [動態檢視]**** 節點。 介面。 語法為 `$dynamic,` *object*。 此功能僅適用於使用 .NET Framework 第 4 版的程式碼。 請參閱 [動態查看](https://msdn.microsoft.com/library/4c851b17-2c12-46a0-9828-eb6ea6f5c563)。|  
