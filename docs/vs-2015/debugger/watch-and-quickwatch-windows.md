---
title: 觀賞和快速監看式視窗 |Microsoft Docs
ms.date: 11/15/2016
...
ms.locfileid: "90838771"
---
# <a name="watch-and-quickwatch-windows"></a>監看式及快速監看式視窗
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

您可以使用 **Watch** (**Debug/Windows/watch/watch (1、2、3、4) **) 和 [ **快速** 監看式] (以滑鼠右鍵按一下 [變數/ **Debug]/[快速** 監看式]) 視窗，以在調試過程中監看變數和運算式。  其差異在於 [監看式] **** 視窗可以顯示數個變數，而 [快速監看式] **** 視窗一次僅可顯示單一變數。  
...
5. 關閉 [快速監看式] **** 視窗。 現在您可以在觀察 [ **監看** 式] 視窗中的值時繼續進行調試  
...
## <a name="observing-expressions-with-the-watch-window"></a>使用 [監看式] 視窗觀察運算式  
 ...
 一般來說，[監看式] **** 視窗的運算式評估規則與您的程式碼撰寫語言的運算式評估規則相同。 如果您的運算式有語法錯誤，您應會在程式碼編輯器中看到相同的編譯器錯誤。 範例如下：  
  
 ![WatchExpressionError](../debugger/media/watchexpressionerror.png "WatchExpressionError")  
  
## <a name="refreshing-watch-values-that-are-out-of-date"></a><a name="bkmk_refreshWatch"></a> 重新整理已過期的監看值  
...
- •評估運算式時發生錯誤。 例如，發生逾時或變數超出範圍。  
  
- •運算式包含可在應用程式中觸發副作用的函式呼叫 (查看 [副作用和運算式](#bkmk_sideEffects)) 。  
...
3. 您應該會 **$** 在 [ **區域變數** ] 視窗中看到一個數位加上一個數位。 這就是物件 ID。  
...
## <a name="using-registers-in-the-watch-window-c-only"></a>在 [監看式] 視窗中使用暫存器 (僅限 C++)  
 如果您正在進行程式碼的偵錯工具，您可以使用或，加入暫存器名稱以及變數名稱 **$\<register name>** **@\<register name>** 。  如需詳細資訊，請參閱 [Pseudovariables](../debugger/pseudovariables.md)。  
...
## <a name="dynamicview-and-the-watch-window"></a>動態檢視和監看式視窗  
 ...
 當監看式視窗顯示從實作為的型別所建立的或物件時 <xref:System.Dynamic.IDynamicMetaObjectProvider> ，偵錯工具會將特殊**動態視圖**節點新增至**Autos**自動變數顯示。 此節點會顯示動態物件的動態成員，但不允許編輯成員值。  
