...
2. C# 主控台應用程式參考類別庫的類別有兩種方法，其中的一種方法會擲回例外狀況並加以處理，而第二個方法會擲回相同的例外狀況，但不會加以處理：  
   ...
   ```  
    ...
    如果您已**AccessViolationException**簽入**例外狀況設定**，當您在偵錯工具執行時，會在**ThrowHandledException （）** 和**ThrowUnhandledException （）** 的 `throw` 行上中斷。  
...
## <a name="see-also"></a>請參閱  
 ...
 [不使用 C 執行時間程式庫進行執行時間檢查](../debugger/using-run-time-checks-without-the-c-run-time-library.md)   
