---
title: 逐步解說：在 Visual Basic 中撰寫視覺化Microsoft Docs
ms.date: 11/15/2016
...
> [!NOTE]
> 根據目前使用的設定與版本，您所看到的對話方塊與功能表命令可能會與 [說明] 中所描述的不同。 若要變更設定，請在 [工具]**** 功能表中選擇 [匯入和匯出]****。 如需詳細資訊，請參閱 [Visual Studio 中的自訂開發設定](https://msdn.microsoft.com/22c4debb-4e31-47a8-8f19-16f328d7dcd3)。  
...
2. 在 [ **新增專案** ] 對話方塊的 [ **專案類型**s] 下，按一下 [ **Visual Basic**]。  
...
2. 將 Class1.vb 變更成有意義的名稱，例如 DebuggerSide.vb。  
    ...
    > [!NOTE]
    > [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 自動變更 Debuggerside.vb 中的類別宣告，使其符合新的檔案名。  
...
2. 在 [ **加入新專案** ] 對話方塊的 [ **範本** ] 方塊中，按一下 [ **主控台應用程式**]。  
...
2. 將 Module1.vb 編輯為適當的名稱，例如 **TestConsole.vb**。  
    ...
    請注意，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 會自動變更 TestConsole.vb 中的類別宣告，以符合新的檔案名稱。  
