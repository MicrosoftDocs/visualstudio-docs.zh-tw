...
[圖形管線階段] 視窗可協助您了解如何透過 Direct3D 圖形管線各階段轉換個別繪製呼叫。  
 ...
 ![3&#45;D 物件會經歷管線階段。](../debugger/media/gfx-diag-demo-pipeline-stages-orientation.png "gfx_diag_demo_pipeline_stages_orientation")  
...
### <a name="vertex-shader-preview"></a>頂點著色器預覽  
 當您在 [ **圖形管線階段** ] 視窗中選取 [頂點著色器] 階段時，會顯示 [ **輸入緩衝區** ] 面板。 在這裡，您將找到下列頂點清單的詳細資料：透過輸入組合語言組合之後提供給頂點著色器的頂點。  
  
 ![頂點著色器階段輸入緩衝區檢視器](../debugger/media/gfx-diag-vertex-shader-inbuffers.png "gfx_diag_vertex_shader_inbuffers")  
