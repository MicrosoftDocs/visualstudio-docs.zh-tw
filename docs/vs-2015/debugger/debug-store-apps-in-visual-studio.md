...
|[快速入門：對 HTML 和 CSS 進行偵錯](../debugger/quickstart-debug-html-and-css.md)|偵錯 HTML 和 CSS 顯示如何使用即時 DOM 檢查工具檢視和修改 HTML 和 CSS，以便互動式地偵錯 JavaScript 應用程式。|
|[快速入門：針對 JavaScript 進行偵錯](../debugger/quickstart-debug-javascript-using-the-console.md)|使用主控台偵錯 JavaScript 會示範如何以互動方式偵錯 JavaScript 應用程式，使用[JavaScript Console commands](../debugger/javascript-console-commands.md)。|
