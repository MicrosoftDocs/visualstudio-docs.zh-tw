...
#### <a name="parameters"></a>參數  
 `-file` `filename`  
 在 [捕捉模式 (] `-c`) 中， `filename` 指定要記錄圖形資訊的圖形記錄檔名稱。 如果 `filename` 未指定，則會將圖形資訊記錄到依預設命名的檔案 `<appname>-<date>-<time>.vsglog` 。  
  
 在驗證 (-v) 模式中，`filename` 指定要驗證的圖形記錄檔名稱。 如果未指定 `filename`，會再次使用前次驗證的圖形記錄檔。  
  
 `-frame` `frames`  
 在擷取模式下，`frames` 指定您想要擷取的框架。 第一個框架為 1。 您可以使用逗號和範圍來指定多個框架。 例如，如果 `frames` 是 `2, 5, 7-9, 15` ，則會捕獲框架、、、、 `2` `5` `7` `8` `9` 和 `15` 。  
  
 `-period` `periods`  
 在擷取模式下，`periods` 指定您要擷取框架的時間範圍，以秒為單位。 您可以使用逗號和範圍來指定多個期間。 例如，如果 `periods` 是 `2.1-5, 7.0-9.3` ，則會捕捉在和秒之間轉譯的框架，以及介於和秒之間的畫面格 `2.1` `5` `7` `9.3` 。  
  
 `-manual`  
 在 [capture] 模式下， `-manual` 指定畫面格將會按下 Print Screen 鍵手動捕獲。 應用程式啟動時，就可以擷取框架；若要停止擷取框架，請返回到命令列介面並按下 Enter 鍵。  
 ...
 `-debug`  
 在 [播放模式] 下， `-debug` 指定應該在啟用 Direct3D debug layer 的情況下執行播放。  
  
 `-warp`  
 在 [播放模式] 下， `-warp` 指定應該使用變形軟體轉譯器來執行播放。  
  
 `-hw`  
 在 [播放模式] 中， `-hw` 指定應該使用 GPU 硬體來執行播放。  
  
 `-config`  
 在 [播放模式] 下， `-config` 如果此資訊記錄到記錄檔，則會顯示用來捕捉圖形記錄檔之電腦的相關資訊。  
  
 `-rawmode`  
 在 [播放模式] 下， `-rawmode` 指定應該在不修改所記錄事件的情況下執行播放。 在正常操作下，播放模式可能會對播放進行次要變更，以簡化偵錯，並加快播放的速度。 例如，它可能會模擬交換鏈結輸出，而不是執行交換鏈結命令。 通常這並不是問題，但您可能需要讓播放時更忠實呈現記錄的事件；例如，您可以使用這個選項來以全螢幕呈現行為，還原至以全螢幕模式執行時擷取的應用程式。  
 ...
 `-examine` `events`  
 在驗證模式下，`events` 指定會比較其立即結果的圖形事件集。 例如，將 `-examine present,draw,copy,clear` 比較限制為只有屬於這些類別的事件。  
  
> [!TIP]
> 我們建議您從開始， `-examine present,draw,copy,clear` 因為這會顯示大部分的問題，但所花的時間會比一組更廣泛的事件少。 如有必要，您可以指定較大或不同的事件集來驗證這些事件，並且顯示其他種類的問題。  
  
 `-haltonfail`  
 在 [驗證] 模式下， `-haltonfail` 會在硬體和變形轉譯器之間偵測到差異時，中止驗證。 按下按鍵之後，會繼續執行驗證。  
  
 `-exitonfail`  
 在驗證模式下， `-exitonfail` 會在硬體和變形轉譯器之間偵測到差異時立即結束驗證。 當程式以這種方式結束時，它會返回 `0` 環境; 否則會傳回 `1` 。  
  
 `-showprogress`  
 在 [驗證模式] 下， `-showprogress` 顯示驗證會話的進度資訊。 WARP 進度顯示在左邊，而硬體進度顯示在右邊。  
...
## <a name="remarks"></a>備註  
 ...
 播放模式 (-p)  
 從現有的圖形記錄檔播放先前所擷取的圖形事件。 根據預設會以視窗播放，即使圖形記錄檔是從全螢幕應用程式擷取也一樣。 只有當圖形記錄檔是從全螢幕應用程式捕捉並指定時，才會在全螢幕中播放 `–rawmode` 。  
...
```  
 ...
 使用 DXCap.exe 來擷取 Windows 市集應用程式，類似於使用它來擷取從 Windows 桌面應用程式，但您不會依檔名識別桌面應用程式，而是依套件名稱和您要擷取之套件內的可執行檔名稱或識別碼來識別 Windows 市集應用程式。 為了讓您更輕鬆地找出電腦上所安裝的 Windows Store 應用程式，請使用 `–e` DXCap.exe 的選項來列舉它們：  
 ...
 **IsWWA:否**  
 **AppSpec (啟動) ： DXCap.exe-c BingMaps_2. 2914. 1734 _x64__8wekyb3d8bbwe，AppexMaps** 每個列舉應用程式輸出的最後一行會顯示您可用來從中捕獲圖形資訊的命令。  
