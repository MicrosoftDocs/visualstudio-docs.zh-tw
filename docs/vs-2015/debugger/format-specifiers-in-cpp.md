...
```  
 ...
 將 `my_var1` 變數加入至 [**監看**式] 視窗 (在 [調試]、[ **Debug]/[Windows]/[** 監看式]/[監看式]) ，然後在**Hexadecimal Display** [**監看**) 式] 視窗中將 [顯示為十六進位] (，以滑鼠右鍵 現在監看式視窗顯示它包含了值 0x0065。 若希望數值以字元顯示，而非整數，請在 [名稱] 欄位中變數名稱後面，加入字元格式規範 **, c**。 [值] **** 資料行現在出現時會帶有 **101 'e'**。  
...
|s|const char* 字串|\<location> "hello world"|"hello world"|  
|**某人**|const char* 字串|\<location> "hello world"|hello world|  
|s8|const char* 字串|\<location> "hello world"|"hello world"|  
|**s8b**|const char* 字串|\<location> "hello world"|"hello world"|  
|su|const wchar_t * const<br /><br /> char16_t \* 字串|\<location> L "hello world"|L"hello world"<br /><br /> u"hello world"|  
|sub|const wchar_t * const<br /><br /> char16_t \* 字串|\<location> L "hello world"|hello world|  
...
|小時|HRESULT 或 Win32 錯誤碼。 (偵錯工具現在可自動將 HRESULT 解碼，因此這個規範並不需要用於這些狀況中)。|S_OK|S_OK|  
...
### <a name="format-specifiers-memory-locations-in-interop-debugging-with-ccli"></a><a name="BKMK_Format_specifiers_memory_locations_in_interop_debugging_and_C___edit_and_continue"></a> 使用 c + +/CLI 進行 interop 調試的格式規範記憶體位置  
...
### <a name="size-specifier-for-pointers-as-arrays-in-interop-debugging-with-cclit"></a><a name="BKMK_Size_specifier_for_pointers_as_arrays_in_interop_debugging_and_C___edit_and_continue"></a> 在使用 C++/CLI 的 Interop 偵錯中做為陣列之指標的大小規範  
