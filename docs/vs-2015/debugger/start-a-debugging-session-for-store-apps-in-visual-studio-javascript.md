---
title: 啟動存放區應用程式的偵測會話（JavaScript） |Microsoft Docs
ms.date: 11/15/2016
...
> [!NOTE]
> 針對以 XAML 和 Visual c # 撰寫的應用程式、Visual C++ 或 Visual Basic，請參閱[啟動 debug 會話（VB、c #、c + + 和 XAML）](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md)

## <a name="in-this-topic"></a><a name="BKMK_In_this_topic"></a>本主題中的
 [本主題中的](#BKMK_In_this_topic)
...
## <a name="the-easy-way-to-start-debugging"></a><a name="BKMK_The_easy_way_to_start_debugging"></a>開始進行調試的簡單方式
 ![僅適用於 Windows](../debugger/media/windows-only-content.png "windows_only_content")

1. 在 Visual Studio 中開啟應用程式方案。

2. 如果方案包含適用於 Windows 市集和 Windows Phone 市集應用程式的專案，請確定您要偵錯的專案是啟始專案。 在 [方案探索] 中，選取專案，然後從內容功能表選擇 [**設定為啟始專案**]。
...
## <a name="configure-the-debugging-session"></a><a name="BKMK_Configure_the_debugging_session"></a>設定調試會話
 因為未編譯指令碼，所以組建組態和平台設定不適用。 如果您要對 c + + 或受管理元件進行偵錯工具，**請將設定設為** **Debug** ，然後從 [**設定] 對話方塊中選擇**您的目標平臺。
...
### <a name="choose-the-deployment-target"></a><a name="BKMK_Choose_the_deployment_target"></a>選擇部署目標
 您可以在 Visual Studio 電腦、本機電腦上的 Visual Studio 模擬器或遠端電腦上部署和偵錯應用程式。 您可以在專案的 [**調試**程式] 屬性頁上，從 [**要啟動的偵錯工具**] 清單中選擇目標。

 ![僅適用於 Windows](../debugger/media/windows-only-content.png "windows_only_content")

 針對 Windows Store 應用程式，請從 [**目標裝置**] 清單中選擇下列其中一個選項：

|選項|描述|
|-|-|
|**本機電腦**|在本機電腦上對目前工作階段中的應用程式進行偵錯。 請參閱在[本機電腦上執行 Windows Store 應用程式](../debugger/run-windows-store-apps-on-the-local-machine.md)。|
|**類比**|在 [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] 應用程式的 Visual Studio 模擬器中進行應用程式偵錯。 模擬器是可讓您對本機電腦上無法使用的裝置功能 (例如觸控筆勢與裝置旋轉) 進行偵錯的桌面視窗。 請參閱[在模擬器中執行 Windows Store 應用程式](../debugger/run-windows-store-apps-in-the-simulator.md)。|
...
- 在 [**電腦名稱稱**] 方塊中，輸入遠端電腦的名稱或 IP 位址。

- 選擇 [**電腦名稱稱**] 方塊中的向下箭號，然後選擇 [] **\<Locate...>** 。 然後從 [**選取遠端偵錯程式連接**] 對話方塊中選擇遠端電腦。
...
### <a name="optional-delay-starting-the-app-in-the-debug-session"></a><a name="BKMK__Optional__Delay_starting_app_in_the_debug_session"></a>選擇性在 debug 會話中延遲啟動應用程式
 ...
 您可以指定是否要在應用程式專案的 [**調試**程式] 屬性頁上，于 [**啟動應用程式**] 清單中延遲啟動應用程式。 請選擇其中一個選項：

- 選擇 [**否**] 會延遲啟動您的應用程式。

- 選擇 **[是]** 以立即啟動應用程式。

### <a name="optional-disable-network-loopbacks"></a><a name="BKMK__Optional__Disable_network_loopbacks"></a>選擇性停用網路回送
 ![僅適用於 Windows](../debugger/media/windows-only-content.png "windows_only_content")
...
## <a name="start-the-debugging-session"></a><a name="BKMK_Start_the_debugging_session"></a>啟動調試階段

### <a name="start-debugging-f5"></a><a name="BKMK_Start_debugging__F5_"></a>開始調試（F5）
 當您選擇 [**調試**程式] 功能表上的 [**開始**偵測] （鍵盤： F5）時，Visual Studio 會啟動已附加偵錯工具的應用程式。 執行會持續到中斷點為止，若以手動方式暫停執行，就會發生未處理的例外狀況，或結束應用程式。

### <a name="start-debugging-f5-but-delay-the-app-start"></a><a name="BKMK_Start_debugging__F5__but_delay_the_app_start"></a>開始偵錯工具（F5），但延遲應用程式啟動
 您可以將應用程式設定成以偵錯模式執行，但卻是藉由偵錯工具以外的方式啟動。 例如，您可能會希望在應用程式從 [開始] 功能表啟動時進行偵錯，或是偵錯應用程式中的背景處理程序而不啟動應用程式。若要延遲啟動應用程式，請執行下列作業：

1. 在應用程式專案屬性的 [ **Debug** ] 頁面上，從 [**啟動應用程式**] 清單中選擇 [**否**]。
...
## <a name="attach-the-debugger-to-a-running-app"></a><a name="BKMK_Attach_the_debugger_to_a_running_app_"></a>將偵錯工具附加至執行中的應用程式
...
### <a name="set-the-app-to-run-in-debug-mode"></a><a name="BKMK_Set_the_app_to_run_in_debug_mode"></a>將應用程式設定為以 debug 模式執行
...
>
> 處理此情況的最簡單方式是關閉所有其他 JavaScript 應用程式。 否則，您可以在啟動應用程式之前開啟 Windows 工作管理員，並記下 wwahost.exe 處理序的 ID。 當您在 [**可使用的進程**] 對話方塊中指定要附加的進程時，應用程式的 wwahost.exe 將會有不同于您所記下的識別碼。
...
## <a name="see-also"></a>另請參閱
 [在偵錯工具中控制執行（JavaScript）](../debugger/control-execution-of-a-store-app-in-a-visual-studio-debug-session-for-windows-store-apps-javascript.md) [快速入門：](../debugger/quickstart-debug-html-and-css.md) [在 Visual Studio 中](../debugger/debug-store-apps-in-visual-studio.md)的 debug HTML 和 CSS[觸發程式的暫止、繼續和背景事件（適用于 Windows Store）](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md)
