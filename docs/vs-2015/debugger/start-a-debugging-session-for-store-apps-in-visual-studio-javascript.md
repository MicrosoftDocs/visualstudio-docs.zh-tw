...
> [!NOTE]
> 針對以 XAML 和視覺效果C#、視覺C++效果或 Visual Basic 撰寫的應用程式，請參閱[啟動 debug 會話C#（ C++ VB、和 XAML）](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md)
...
- [選擇性在 debug 會話中延遲啟動應用程式](#BKMK__Optional__Delay_starting_app_in_the_debug_session)
...
2. 如果解決方案包含適用於 Windows 市集應用程式和 Windows Phone 應用程式的專案，首先必須確定您要偵錯的專案為啟始專案。 在 [方案探索] 中，選取專案，然後從內容功能表選擇 [**設定為啟始專案**]。

3. 請按 F5。

   ![僅適用于 Windows Phone](../debugger/media/phone-only-content.png "phone_only_content")

   Visual Studio 會建置附加了偵錯工具的應用程式，並會加以啟動。 執行會持續到出現中斷點為止，若以手動方式暫止執行，會發生未處理的例外狀況，或結束應用程式。 如需詳細資訊，請參閱[快速入門： DEBUG HTML 和 CSS](../debugger/quickstart-debug-html-and-css.md)。

## <a name="BKMK_Configure_the_debugging_session"></a> 設定偵錯工作階段
 因為未編譯指令碼，所以不會套用組建組態與平台設定。 如果您要對C++或受管理的元件進行偵錯工具，請將設定設為**Debug** ，然後從 [**設定] 對話方塊**中選擇您的目標平臺。
...
### <a name="BKMK_Choose_the_deployment_target"></a> 選擇部署目標
 您可以在 Visual Studio 電腦、本機電腦上的 Visual Studio 模擬器或遠端電腦上部署應用程式並對其執行偵錯。 您可以在專案的 [**調試**程式] 屬性頁上，從 [**要啟動的偵錯工具**] 清單中選擇目標。
...
- 選擇 [**電腦名稱稱**] 方塊中的向下箭號，然後選擇 [ **\<尋找]。>** 。 然後從 [**選取遠端偵錯程式連接**] 對話方塊中選擇遠端電腦。

   ![選取遠端偵錯程式連接](../debugger/media/vsrun-pro-selectremotedebuggerdlg.png "VSRUN_PRO_SelectRemoteDebuggerDlg")
...
### <a name="BKMK_Choose_the_debugger_to_use"></a> 選擇要使用的偵錯工具
 偵錯工具預設會附加到您應用程式中的 JavaScript 程式碼。 您可以選擇偵錯您應用程式元件的原生 C++ 和 Managed 程式碼，而非 JavaScript 程式碼。 您在應用程式專案的 [ **偵錯** ] 屬性頁面之 [ **偵錯工具類型** ] 清單中，指定要偵錯的程式碼。
...
### <a name="BKMK__Optional__Delay_starting_app_in_the_debug_session"></a>選擇性在 debug 會話中延遲啟動應用程式
 ...
 您可以指定是否要在應用程式專案的 [**調試**程式] 屬性頁上，于 [**啟動應用程式**] 清單中延遲啟動應用程式。 選擇下列其中一個選項：
...
### <a name="BKMK_Start_debugging__F5_"></a> 開始偵錯 (F5)
 當您選擇 [**調試**程式] 功能表上的 [**開始**偵測] （鍵盤： F5）時，Visual Studio 會啟動已附加偵錯工具的應用程式。 執行會持續到出現中斷點為止，若以手動方式暫止執行，會發生未處理的例外狀況，或結束應用程式。
...
2. 啟動該應用程式。 您可以從 [開始] 功能表、執行合約或透過其他方法啟動應用程式。
...
>
> 此狀況最簡單的處理方式，就是關閉所有其他 JavaScript 應用程式。 否則，您可以在啟動應用程式之前開啟 Windows 工作管理員，並記下 wwahost.exe 處理序的 ID。 當您在 [**可使用的進程**] 對話方塊中指定要附加的進程時，應用程式的 wwahost.exe 將會有不同于您所記下的識別碼。
...
4. 在 [**可使用的進程**] 清單中，選擇適當的**wwahost.exe**處理常式。 使用 [**標題**] 資料行來識別您的應用程式。
