...
##  <a name="BKMK_Why_create_visualizations_"></a> 建立 Natvis 視覺化的原因  
 ...
 例如，下圖為偵錯工具中顯示之 [Windows::UI::Xaml::Controls::TextBox](http://go.microsoft.com/fwlink/?LinkId=258422) 類型的變數，其中未套用任何自訂視覺化。  
 ...
 套用自訂視覺化規則之後，相同的 `TextBox` 在變數視窗中看起來更簡潔。 您可以同時檢視類別的所有重要成員，且偵錯工具會顯示自訂字串類型的基礎字串值。  
...
1.  正在進行偵錯的 .pdb 中內嵌的 natvis 檔案 (除非已載入的專案中存在相同名稱的檔案)  
...
## <a name="modifying-natvis-files-while-debugging"></a>在偵錯時修改 .natvis 檔案  
 ...
 如果 .natvis 檔案內嵌於 .pdb，您就無法在偵錯時加以修改。  
...
- Natvis 運算式不允許函式評估或副作用。 這表示已忽略函式呼叫和指派運算子。 由於 [偵錯工具內建函式](../debugger/expressions-in-the-debugger.md#BKMK_Using_debugger_intrinisic_functions_to_maintain_state) 沒有副作用，因此可自由地從任何 Natvis 運算式加以呼叫，即使不允許其他函式呼叫亦然。  
  ...
  若要控制運算式如何顯示變數視窗中，您可以使用任何所述的格式規範[格式規範](../debugger/format-specifiers-in-cpp.md#BKMK_Visual_Studio_2012_format_specifiers)一節[c + + 中的格式規範](../debugger/format-specifiers-in-cpp.md)主題。 請注意，當虛擬化項目 Natvis 在內部使用，例如，會忽略格式規範`Size`ArrayItems 展開的運算式。  
...
```  
 ...
 在這個範例中，不論物件是 `CAtlArray<int>` 或 `CAtlArray<float>`，都會使用相同的視覺化。 如果 `CAtlArray<float>` 有特定視覺化項目，則其優先順序高於一般項目。  
...
#### <a name="priority-attribute"></a>Priority 屬性  
 ...
 在下列範例中，我們會先剖析符合 2015 STL 的項目，如果無法剖析，則我們會使用 STL 之 2013 年版本的替代項目：  
...
```  
 ...
 在這個範例中，視覺化只適用於 `DirectUI::Border` 1.0 至 1.5 版中存在的 `Windows.UI.Xaml.dll` 類型。 請注意，加入版本項目會將視覺化項目範圍設為特定模組和版本，並減少不當的比對不符，但如果類型是在不同模組所使用的通用標頭檔中定義，則只要類型不在指定的模組中，就不會套用已建立版本的視覺化。  

#### <a name="optional-attribute"></a>Optional 屬性  
 `Optional` 屬性可以出現在任何節點上。 如果選擇性節點內的任何子運算式無法剖析，就會忽略該節點，但其他 Type 項目仍然有效。 在下列類型中， `[State]` 是非選擇性的，但 `[Exception]` 是選擇性的。  這表示如果 `MyNamespace::MyClass` 包含一個名為 _`M_exceptionHolder`的欄位，您仍然會同時包含 `[State]` 節點和 `[Exception]` 節點，但如果遺漏 `_M_exceptionHolder` ，您就只會看到 `[State]` 節點。  
...
```  
 ...
 加入 `StringView` 項目將會向偵錯工具表示這個值可以透過文字視覺化檢視：  
...
-   如果未在視覺化項目中指定 `Expand` 節點，則會使用 Visual Studio 的預設展開規則。  
...
```  
 ...
 ![使用 item 項目展開的 CRect](../debugger/media/dbg-natvis-expand-item-crect1.png "DBG_NATVIS_Expand_Item_CRect1")  
...
2. 應指向第一個項目 (必須是不屬於 `ValuePointer` 之項目類型的指標) 的 `void*`運算式。  
   ...
   也可以指定多維陣列。 在這個情況下，偵錯工具只需要再多一點資訊，即可正確地顯示子項目：  
...
```  
 ...
 `Direction` 指定陣列是以資料列還是以資料行做為主要次序。 `Rank` 指定陣列的陣序。 `Size` 項目接受隱含 `$i` 參數，並將參數取代為維度索引，以便在該維度中尋找陣列的長度。 例如，在上述範例中，運算式 `_M_extent.M_base[0]` 上方應提供第 0 維度、第 1 維度等 `_M_extent._M_base[1]` 的長度。  
...
#### <a name="customlistitems-expansion"></a>CustomListItems 展開  
 `CustomListItems` 展開可讓您撰寫周遊資料結構 (例如雜湊表) 的自訂邏輯。 您應該使用 `CustomListItems` 來視覺化資料結構，其中您所需評估的一切皆可以 C++ 運算式表示，但不完全符合 `ArrayItems`, `TreeItems`或 `LinkedListItems.`的模式。  
...
```  
 ...
 ![自動&#95;ptr&#60;向量&#60;int&#62; &#62; ExpandedItem 展開](../debugger/media/dbg-natvis-expand-expandeditem-visualized.png "DBG_NATVIS_Expand_ExpandedItem_Visualized")  
...
```  
 ...
 ![含有 Sythentic 項目 expansio](../debugger/media/dbg-natvis-expand-synthetic.png "DBG_NATVIS_Expand_Synthetic")  
