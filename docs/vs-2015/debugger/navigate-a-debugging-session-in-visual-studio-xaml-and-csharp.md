---
title: 巡覽偵錯工作階段 (Xaml 和C#) |Microsoft Docs
ms.date: 11/15/2016
 ...
 **偵錯工具鍵盤快速鍵。** Visual Studio 偵錯工具巡覽已針對滑鼠和鍵盤進行最佳化。 本主題中的許多步驟都會以括號備註的方式加上鍵盤快速鍵。 例如，(鍵盤：F5) 表示輸入 F5 鍵啟動或繼續偵錯工具執行。
 ...
 **使用範例程式碼取代 MainPage 建構函式。** 刪除 MainPage() 方法。 請依循下列連結：[偵錯工具巡覽範例程式碼 (Xaml 和C#)](../debugger/debugger-navigation-sample-code-xaml-and-csharp.md)，然後將複製中的程式碼C#一節，以剪貼簿。 (選擇**回**瀏覽器或說明檢視器，以返回此快速入門 頁面中。)在 Visual Studio 編輯器中，貼上 `partial class MainPage` 區塊中的程式碼。 選擇 CTRL + S 以儲存檔案。
 ...
 **設定中斷點。** 在 MainPage 建構函式的陳述式 `methodTrack = "Main Page";` 中設定中斷點。 選擇原始程式碼編輯器陰影裝訂邊中的一行 (鍵盤：將游標放在一行上，選擇 F9 鍵。）

 ![逐步執行](../debugger/media/dbg-basics-stepinto.png "DBG_Basics_StepInto")

 中斷點圖示會出現在巡覽邊中。

 **執行至中斷點。** 選擇 開始偵錯工作階段**開始偵錯**上**偵錯**功能表 (鍵盤：F5)。
 ...
 **逐步執行方法。** 在 **偵錯**功能表上，選擇**逐步執行**(鍵盤：F11)。
 ...
 **執行至中斷點。** 選擇 開始偵錯工作階段**開始偵錯**上**偵錯**功能表 (鍵盤：F5)。 偵錯工具會在中斷點暫停執行。

 **不進入程式碼行。** 在 **偵錯**功能表上，選擇**不進入函式**(鍵盤：F10)。 偵錯工具會以逐步執行陳述式的相同方式執行 `methodTrack = "MainPage";` 陳述式。
 ...
 **跳離函式。** 選擇 F11 鍵，逐步執行 Example2_B 方法。 請注意，Example2_B 與 Example2_A 非常不同。 若要跳離方法，請選擇**跳離函式**上**偵錯**功能表 (鍵盤：Shift + F11）。 請注意， `methodTrack` 變數表示已執行 Example2_B，且偵錯工具已經回到 Example2 繼續的點。
 ...
 **執行至中斷點。** 選擇 開始偵錯工作階段**開始偵錯**上**偵錯**功能表 (鍵盤：F5)。 偵錯工具會在 MainPage 方法中的中斷點暫停執行。
 ...
 **執行至中斷點。** 在偵錯 功能表中，選擇 繼續 (鍵盤：F5)。 在 [區域變數] 視窗中，確認 `i` 的目前值為 500。 請注意，變數 `s` 是以單行表示，其長度遠超過視窗。

 **將字串變數將變數視覺化。** 在 **的 [值]** 的 [值] `s`。

 [文字視覺化檢視] 視窗隨即出現，且字串的值會以多行字串呈現。

 **執行至游標處。** 以滑鼠右鍵按一下該行`methodTrack += "->Example3";`，然後選擇 **執行至游標處**(鍵盤：將游標移至行;Ctrl + F10）。 偵錯工具會完成迴圈反覆項目，然後在該行暫停執行。
 ...
 **執行至例外狀況。** 選擇 開始偵錯工作階段**開始偵錯**上**偵錯**功能表 (鍵盤：F5)。 再按 F5 一次，繼續執行。 偵錯工具會在 Example4 方法中的例外狀況暫停執行，並顯示例外狀況對話方塊。
