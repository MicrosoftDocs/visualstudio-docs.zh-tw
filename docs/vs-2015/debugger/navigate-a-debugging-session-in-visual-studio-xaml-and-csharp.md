---
title: '流覽 (Xaml 和 c # ) 的調試階段 |Microsoft Docs'
ms.date: 11/15/2016
...
## <a name="create-the-sample-app"></a><a name="BKMK_CreateTheApplication"></a> 建立範例應用程式
 ...
 **開啟 MainPage.xaml.cs 原始程式檔。** 以滑鼠右鍵按一下 XAML 編輯器中的任何位置，然後選擇 [ **視圖程式碼**]。 MainPage.xaml.cs 程式碼後置檔案便會顯示。 請注意，檔案中僅會列出一個方法，也就是 `MainPage()` 建構函式。

 **使用範例程式碼取代 MainPage 建構函式。** 刪除 MainPage() 方法。 遵循此連結： [偵錯工具流覽範例程式碼 (Xaml 和 c # ) ](../debugger/debugger-navigation-sample-code-xaml-and-csharp.md)，然後將 c # 區段中列出的程式碼複製到剪貼簿。  (在瀏覽器或說明檢視器中選擇 [ **上一頁** ]，返回此快速入門頁面。在 Visual Studio 編輯器中 ) ，將程式碼貼到 `partial class MainPage` 區塊中。 選擇 CTRL + S 以儲存檔案。

 您現在可以依照本主題中的範例進行。

## <a name="set-and-run-to-a-breakpoint-step-into-a-method-and-examine-program-data"></a><a name="BKMK_StepInto"></a> 設定並執行至中斷點、逐步執行方法，以及檢查程式資料
 ...
 **執行至中斷點。** 選擇 [**調試**程式] 功能表上的 [**開始調試**程式]， (鍵盤： F5) 來啟動偵錯工具會話。
 ...
 **逐步執行方法。** 在 [ **調試** ] 功能表上，選擇 [ **逐步** 執行 (鍵盤： F11) ]。

 ![目前的行](../debugger/media/dbg-basics-currentline.png "DBG_Basics_CurrentLine")
...
- 如果該陳述式是您方案中某個函式的呼叫，偵錯工具就會移至已呼叫的函式的進入點，然後暫停執行。
  ...
  **檢查 [區域變數] 視窗中的變數值。** 在 [ **調試** ] 功能表上，指向 [ **視窗**]，然後選擇 [ **區域變數**]。 (鍵盤：Alt+4)。
...
## <a name="step-into-over-and-out-of-methods"></a><a name="BKMK_StepIntoOverOut"></a> 逐步執行、不進入和跳過方法
 ...
 **在 MainPage 建構函式中呼叫 Example2 方法。** 編輯 MainPage 建構函式，並將 `methodTrack = String.Empty;` 後面的那一行取代為 `Example2();`。
 ...
 ![從 Demo 方法呼叫 Example2 方法](../debugger/media/dbg-basics-callexample2.png "DBG_Basics_CallExample2")

 **執行至中斷點。** 選擇 [**調試**程式] 功能表上的 [**開始調試**程式]， (鍵盤： F5) 來啟動偵錯工具會話。 偵錯工具會在中斷點暫停執行。
 ...
 **停止調試。** 在 [偵錯] 功能表上，選擇 [停止偵錯] (鍵盤：Shift+F5)。 這樣會結束偵錯工作階段。

## <a name="set-a-conditional-breakpoint-run-to-the-cursor-and-visualize-a-variable"></a><a name="BKMK_ConditionCursorVisualize"></a> 設定條件中斷點、執行至游標處，並將變數視覺化
 ...
 **在 MainPage 建構函式中呼叫 Example3 方法。** 編輯 MainPage 建構函式，並將 `methodTrack = String.Empty;` 後面的那一行取代為 `Example3();`。
 ...
 ![從 Demo 方法呼叫 Example3 方法](../debugger/media/dbg-basics-callexample3.png "DBG_Basics_CallExample3")

 **執行至中斷點。** 選擇 [**調試**程式] 功能表上的 [**開始調試**程式]， (鍵盤： F5) 來啟動偵錯工具會話。 偵錯工具會在 MainPage 方法中的中斷點暫停執行。
...
## <a name="edit-and-continue-recover-from-an-exception"></a><a name="BKMK_EditContinueRecoverExceptions"></a> 編輯後繼續，從例外狀況中復原
 ...
 **執行至例外狀況。** 選擇 [**調試**程式] 功能表上的 [**開始調試**程式]， (鍵盤： F5) 來啟動偵錯工具會話。 再按 F5 一次，繼續執行。 偵錯工具會在 Example4 方法中的例外狀況暫停執行，並顯示例外狀況對話方塊。

 ![[例外狀況] 對話方塊](../debugger/media/dbg-basics-exceptiondlg.png "DBG_Basics_ExceptionDlg")

 **變更程式邏輯。** 錯誤明顯是在 `if` 條件中：當 `x` 等於 0 時，應該變更 `x` 的值；當 `x` 不等於零時，則不應該變更。 選擇 [中斷] **** 以修正方法的邏輯。 當您嘗試編輯該行時，另一個對話方塊隨即出現。

 ![[編輯後繼續] 對話方塊](../debugger/media/dbg-basics-editandcontinuedlg.png "DBG_Basics_EditAndContinueDlg")
 ...
 **停止調試。** 在 [ **調試** ] 功能表上，選擇 [ **停止調試** ] (鍵盤： Shift + F5) 。 這樣會結束偵錯工作階段。

## <a name="see-also"></a>另請參閱
 [啟動 (VB、c #、c + + 和 XAML 的調試](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md)程式，) [在 Visual Studio 中](../debugger/debug-store-apps-in-visual-studio.md)[觸發 Windows Store) 偵錯工具的暫止、繼續和背景事件](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md)
