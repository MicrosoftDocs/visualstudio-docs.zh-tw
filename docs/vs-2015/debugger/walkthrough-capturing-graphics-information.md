...
- 在 [圖形診斷] 工具列上，選擇 [擷取] **** 按鈕。 ![圖形擷取按鈕圖示](../debugger/media/debuggingdirectxgraphics.png "DebuggingDirectXGraphics")  
  ...
  每次您擷取畫面格的相關資訊，圖形診斷就會記錄 Direct3D 事件及相關聯的狀態，並將該資料加入圖形記錄檔中。 每一個圖形診斷工作階段都會建立新的圖形記錄檔。 如需圖形記錄的詳細資訊，請參閱 [總覽](../debugger/overview-of-visual-studio-graphics-diagnostics.md)。  
