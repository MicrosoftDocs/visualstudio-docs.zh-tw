...
圖形記錄文件是在圖形診斷工作階段下執行應用程式時所發生圖形事件的記錄。 錄製之後，您可以在 Visual Studio 圖形分析器中檢查記錄來診斷轉譯和效能問題。  
 ...
 ![包含兩個已擷取之框架的圖形記錄。](../debugger/media/gfx-diag-demo-graphics-log-orientation.png "gfx_diag_demo_graphics_log_orientation")  
...
- 在圖形記錄文件上方，從呈現目標輸出中選取您要的像素。 選取像素後，您可以使用 [圖形像素記錄]**** 視窗，檢視所選取像素的詳細資訊。 如需詳細資訊，請參閱 [圖元歷程記錄](../debugger/graphics-pixel-history.md)。  
...
## <a name="playback-machine"></a>播放電腦  
 ...
 如需如何指定播放電腦的相關資訊，請參閱 [How to: Change the Graphics Diagnostics Playback Machine](../debugger/how-to-change-the-graphics-diagnostics-playback-machine.md) (如何：變更圖形診斷播放電腦)。  
...
|屬性|描述|  
|--------------|-----------------|  
|**建立者為**|已建立圖形記錄文件的應用程式名稱。 例如，如果已從 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 啟動擷取工作階段 (手動擷取)，則此屬性的值為 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]。|  
