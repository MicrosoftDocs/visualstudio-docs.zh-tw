...
## <a name="find-options"></a>尋找選項
 ...
 符合大小寫選取時，[**尋找結果**] 視窗只會顯示內容和大小寫同時符合的 [**尋找目標**] 字串實例。 例如，若搜尋 "MyObject" 時選取 [大小寫須相符]，則會傳回 "MyObject"，而不是 "myobject" 或 "MYOBJECT"。

 全字相符：選取時，[**尋找結果**] 視窗只會顯示完整文字中相符的 [**尋找目標**] 字串實例。 例如，搜尋 "MyObject" 時會傳回 "MyObject"，而不是 "CMyObject" 或 "MyObjectC"。

 使用正則運算式當選取這個核取方塊時，您可以使用特殊標記法來定義 [**尋找目標**] 或 [**取代成**] 文字方塊中的文字模式。 如需這些標記法的清單，請參閱[在 Visual Studio 中使用規則運算式](../ide/using-regular-expressions-in-visual-studio.md)。

 查看這些檔案類型：此清單指出要在 [**查詢**] 目錄中搜尋的檔案類型。 如果此欄位保持空白，則會搜尋 [查詢] 目錄中的所有檔案。
...
## <a name="result-options"></a>結果選項
 ...
 [尋找結果 1] 視窗選取時，目前搜尋的結果將會取代 [**尋找結果 1** ] 視窗的內容。 這個視窗會自動開啟以顯示搜尋結果。 若要手動開啟此視窗，請從 [檢視] 功能表中選取 [其他視窗]，並選擇 [尋找結果 1]。

 [尋找結果 2] 視窗選取時，目前搜尋的結果將會取代 [**尋找結果 2** ] 視窗的內容。 這個視窗會自動開啟以顯示搜尋結果。 若要手動開啟此視窗，請從 [檢視] 功能表中選取 [其他視窗]，並選擇 [尋找結果 2]。

 只有在選取此核取方塊時才顯示檔案名，[尋找結果] 視窗會列出包含搜尋字串之所有檔案的完整名稱和路徑。 不過，這些結果不含字串出現位置的程式碼行。 此核取方塊僅在 [檔案中尋找] 中提供。

 當選取 [全部取代] 之後，將已修改的檔案保持開啟狀態，就會開啟所有已進行取代的檔案，因此您可以復原或儲存變更。 由於記憶體有限，因此在取代作業之後可以保持開啟的檔案數目有所限制。
