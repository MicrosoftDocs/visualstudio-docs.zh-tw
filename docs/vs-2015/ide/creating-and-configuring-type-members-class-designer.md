...
## <a name="opening-the-class-details-window"></a><a name="OpenClassDetails"></a> 開啟類別細節視窗
...
## <a name="creating-members"></a><a name="CreateMembers"></a> 建立成員
 您可以使用下列任何工具建立成員：
...
## <a name="modifying-type-members"></a><a name="ModifyTypeMembers"></a> 修改類型成員
 [類別設計工具] 可讓您修改圖表中所顯示類型的成員。 只要在類別圖中顯示的類型並非唯讀，您都可以修改其成員。  (看到 [ (類別設計工具) 的唯讀資訊顯示 ](https://msdn.microsoft.com/33e2d3a9-1668-4d10-ae56-fa09b3156e0a)。 ) 您在設計介面、屬性視窗和 [類別細節] 視窗上使用就地編輯，以修改類型成員。
...
- **\<add member>**
     ...
     [類別細節] 視窗中的最後一個資料列包含 [ **\<add member>** **名稱** ] 儲存格中的文字。 如果按一下這個儲存格，便會建立新的成員。 如需詳細資訊，請參閱[建立成員](../ide/creating-and-configuring-type-members-class-designer.md#CreateMembers)。
...
2. 如果未顯示 [類別細節] 視窗，請按一下類別設計工具工具列上的 [ **類別細節視窗** ] 按鈕。
...
## <a name="adding-parameters-to-methods"></a><a name="AddMethodParams"></a> 將參數加入至方法
...
### <a name="to-add-a-parameter-to-a-method"></a><a name="HowToAddParameterToMethod"></a> 將參數加入至方法
...
2. 在 [類別細節] 視窗上，展開要加入參數的方法所屬列。
     ...
     縮排的參數資料列隨即出現，其中只包含一對括弧和單字** \<add parameter> 。**
...
## <a name="class-details-usage-notes"></a><a name="ClassDetailsUsageNotes"></a> 類別細節使用注意事項
...
- 例如，在參考的元件中，整個類型都是唯讀的， (看到 [ (類別設計工具) 顯示唯讀資訊 ](https://msdn.microsoft.com/33e2d3a9-1668-4d10-ae56-fa09b3156e0a)。 ) 當您選取類別設計工具中的圖形時，[類別細節] 視窗會以唯讀狀態顯示其詳細資料。
...
## <a name="display-of-read-only-information"></a><a name="ReadOnlyInfo"></a> 顯示唯讀資訊
