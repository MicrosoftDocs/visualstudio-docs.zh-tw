...
**管理 Visual Studio 效能**對話方塊有兩種類別：**延伸模組**並**工具 Windows**。
...
- **不要在啟動時顯示視窗：** 如果您選擇此選項，指定將一律關閉工具視窗是當您開啟 Visual Studio 中，即使已在前一個工作階段中開啟。 您可以從功能表開啟工具視窗。
- **啟動時自動隱藏視窗：** 如果工具視窗已保留在先前的工作階段中開啟，選擇此選項會摺疊在啟動時的工具視窗的群組，以避免初始化工具視窗。 如果您經常使用工具視窗，則這是不錯的選擇，原因是工具視窗仍然可用，但不再對 Visual Studio 啟動時間造成負面影響。
