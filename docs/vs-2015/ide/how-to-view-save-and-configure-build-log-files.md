...
1. 在**Windows 檔案總管**或**檔案總管**中，開啟下列檔案： \\ . ..\Visual Studio*版本*\Projects \\ *專案* \\ *名稱*\Debug \\ *專案名稱*.txt

### <a name="to-create-a-build-log-file-for-a-managed-code-project"></a>建立受控碼專案的組建記錄檔

1. 在功能表列上，選擇 [建置] ****、[建置方案] ****。

2. 在 [ **輸出** ] 視窗中，反白顯示組建中的資訊，然後將它複製到剪貼簿。
...
3. 在 [MSBuild 專案建置輸出詳細資訊層級]**** 清單中，選擇下列其中一個值，然後選擇 [確定]**** 按鈕。
    ...
    |Diagnostic|顯示組建可用的所有資料。 您可以使用此詳細層級以協助偵錯自訂建置指令碼的問題和其他組建問題。|
     ...
     如需詳細資訊，請參閱 [選項對話方塊、專案和方案、組建和執行](../ide/reference/options-dialog-box-projects-and-solutions-build-and-run.md) 和 <xref:Microsoft.Build.Framework.LoggerVerbosity> 。

    > [!IMPORTANT]
    > 您必須重建專案，您的變更才會在 [ **輸出** ] 視窗中 (所有專案) 和 *專案名稱*.Txt 檔案 (c + + 專案只) 。

## <a name="see-also"></a>另請參閱
 在 Visual Studio[編譯和建立](../ide/compiling-and-building-in-visual-studio.md)時，[取得組建記錄](../msbuild/obtaining-build-logs-with-msbuild.md)[以建立和清除專案與方案](../ide/building-and-cleaning-projects-and-solutions-in-visual-studio.md)
