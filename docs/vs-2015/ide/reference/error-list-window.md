...
> [!NOTE]
> 根據您目前使用的設定或版本，您所看到的對話方塊與功能表命令可能會與此處所描述的不同。 如果要變更設定，請按一下 [工具]/[匯入和匯出設定]。 如需詳細資訊，請參閱在 Visual Studio 中自訂開發設定 [Walkthrough: Calling Code in an VSTO Add-in from VBA](https://msdn.microsoft.com/22c4debb-4e31-47a8-8f19-16f328d7dcd3)。
...
## <a name="ui-element-list"></a>UI 項目清單
 [嚴重性] 會顯示不同類型的**錯誤清單**專案（**錯誤**、**訊息**、**警告**、**警告（** 使用中）、**警告（非**使用中）。
