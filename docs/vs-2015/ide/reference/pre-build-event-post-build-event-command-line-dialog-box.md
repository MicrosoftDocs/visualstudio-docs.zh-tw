 ...
 **命令列編輯方塊**包含要針對預先組建或建立後組建執行的事件。

> [!NOTE]
> 在執行 .bat 檔案的所有建置命令前方，加入 `call` 陳述式。 例如，`call C:\MyFile.bat` 或 `call C:\MyFile.bat call C:\MyFile2.bat`。

 **宏**展開編輯方塊，以顯示要在命令列編輯方塊中插入的宏清單。

 **宏資料表**列出可用的宏及其值。 每個巨集的描述，請參閱底下的＜巨集＞。 您可以一次只選取一個巨集來插入命令列編輯方塊中。

 **插入**在命令列編輯方塊中，插入宏資料表中選取的宏。
...
## <a name="see-also"></a>請參閱
 [在 Visual Studio 組建事件頁面中指定自訂群組建事件](../../ide/specifying-custom-build-events-in-visual-studio.md) [、專案C#設計工具（）](../../ide/reference/build-events-page-project-designer-csharp.md) [如何：指定組建事件（Visual Basic）](../../ide/how-to-specify-build-events-visual-basic.md) [如何：指定組建事件C#（）](../../ide/how-to-specify-build-events-csharp.md)
