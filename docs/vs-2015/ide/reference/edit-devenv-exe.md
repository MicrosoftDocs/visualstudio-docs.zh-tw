...
## <a name="arguments"></a>引數
 `file1` 選擇項。 要在現有 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 執行個體中開啟的檔案。 如果沒有 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 執行個體，則會建立具有簡化視窗版面配置的新執行個體，而且會在新執行個體中開啟 `file1`。

 `file2` 選擇項。 要在現有 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 執行個體中開啟的一或多個其他檔案。
