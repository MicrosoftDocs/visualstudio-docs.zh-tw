...
## <a name="indenting"></a>縮排
 [無] 選取時，不會縮排新行。 插入點會放在新行的第一欄中。

 封鎖選取時，新行會自動縮排。 插入點會放在與前一行相同的起始點。
...
## <a name="tabs"></a>定位點
 [索引標籤大小] 會設定在定位停駐點之間的間距。 預設值為四個空格。

 [縮排大小] 會設定自動縮排的大小（以空格為限）。 預設值為四個空格。 插入定位字元、空格字元或兩者，以填滿指定的大小。

 在選取時插入空格，縮排作業只會插入空白字元，而不是定位字元。 例如，如果 [縮排大小]  設為 5，每次您按下 TAB 鍵，或按一下 [格式]  工具列上的 [增加縮排]  按鈕時，就會插入五個空格字元。

 保留索引標籤選取時，縮排作業會盡可能插入最多的定位字元。 每個定位字元都會以指定的 [定位點大小]  填入空格數。 如果 [縮排大小]  不是 [定位點大小]  的偶數倍數，就會加入空格字元填滿其間的差距。

## <a name="see-also"></a>另請參閱
 選項[對話方塊、環境、[一般](../../ide/reference/general-environment-options-dialog-box.md)]、[文字編輯器]、[[所有語言](../../ide/reference/options-text-editor-all-languages.md)]
