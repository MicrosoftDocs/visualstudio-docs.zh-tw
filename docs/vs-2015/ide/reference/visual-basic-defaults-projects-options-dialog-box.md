 ...
 **Option Explicit** 設定編譯器預設值，因此需要明確宣告變數。 [Option Explicit]**** 預設為 [On]****。 如需詳細資訊，請參閱 [/optionexplicit](https://msdn.microsoft.com/library/5d296ab3-bafe-4c4d-9887-78f162ed86c7)。
 ...
 **選項比較** 設定字串比較的編譯器預設值：二進位 (區分大小寫的) 或文字 (不區分大小寫 ) 。根據預設，[ **選項比較** ] 會設定為 [ **二進位**]。 如需詳細資訊，請參閱 [/optioncompare](https://msdn.microsoft.com/library/7237b766-b44d-4cc5-9a3c-885348a7d9e4)。

 **選項推斷** 設定本機類型推斷的編譯器預設值。 針對新建立專案，[Option Infer]**** 預設為 [On]****，針對舊版 Visual Basic 中所建立的已移轉專案，則設為 [Off]****。 如需詳細資訊，請參閱 [/optioninfer](https://msdn.microsoft.com/library/f6c09db1-0553-464a-abe3-d4510c61d6ed)。
