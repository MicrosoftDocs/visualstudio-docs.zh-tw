 ...
 **啟用新的資料庫引擎** 這應該會自動加速資料庫擴展，並讓所有資料庫作業的速度變得更快 (不會因為作業（例如 [ **移至定義** ] 和 [ **尋找所有參考**]）而遺失精確度) 。 (只要關閉並重新開啟您的方案，即可套用變更，而不必重新啟動 Visual Studio)。

## <a name="intellisense"></a>IntelliSense
 **成員清單點對箭號** 如果適用于成員清單，則以 '-> ' 取代 '. '。

## <a name="refactoring"></a>重構
 **啟用解壓縮** 函式將選取的程式碼解壓縮至它自己的函式，並將程式碼取代為新函式的呼叫。 若要存取這項功能，請以滑鼠右鍵按一下選取的程式碼並選取 [快速動作] ****，或是直接按預設快速鍵 Ctrl+點 [Ctrl+.]。

 **啟用變更** 簽章新增、重新排列和刪除函式的參數，並將變更傳播到所有的呼叫位置。 若要存取這項功能，請以滑鼠右鍵按一下函式並選取 [快速動作] ****，或是直接按預設快速鍵 Ctrl+點 [Ctrl+.]。

## <a name="text-editor"></a>文字編輯器
 **啟用展開範圍** 若已啟用，您可以在文字編輯器中輸入 ' {'，以大括弧括住選取的文字。

 **啟用展開優先順序** 若已啟用，您可以在文字編輯器中輸入 ' ( '，以括弧括住選取的文字。
...
## <a name="see-also"></a>另請參閱
 [在 c + + (VC Blog) 中](https://devblogs.microsoft.com/cppblog/all-about-c-refactoring-in-visual-studio-2015-preview/)[設定語言特定編輯器選項](../../ide/reference/setting-language-specific-editor-options.md)的重構
