 ...
 **使用 web sites 和專案的64位版本 IIS Express** 選取此選項可使用 IIS Express 而不是 Visual Studio 程式開發伺服器。 如需詳細資訊，請參閱 [IIS Express 簡介](https://weblogs.asp.net/scottgu/introducing-iis-express)和 [IIS Express 概觀](/iis/extensions/introduction-to-iis-express/iis-express-overview)。 預設會停用這個選項。

 **當錯誤清單中有錯誤時，于執行 web 應用程式前發出警告** 如果選取此方塊，當您嘗試在未發生錯誤的情況下執行 web 應用程式時，將會收到警告。
