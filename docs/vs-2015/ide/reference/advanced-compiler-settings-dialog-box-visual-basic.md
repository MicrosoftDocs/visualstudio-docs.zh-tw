 ...
 **DLL 基底位址** 此文字方塊會以十六進位格式顯示預設 DLL 基底位址。 在類別庫和控制項程式庫專案中，您可以使用這個文字方塊來指定在建立 DLL 時要使用的基底位址。
