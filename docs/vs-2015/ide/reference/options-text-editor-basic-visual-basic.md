...
- 重新格式化日期
  ...
  **啟用大綱模式**當您在程式碼編輯器中開啟檔案時，您可以在大綱模式中查看檔。 如需詳細資訊，請參閱[大綱](../../ide/outlining.md)。 選取此選項後，在您開啟檔案時將會啟動大綱功能。

  **自動插入介面和 MustOverride 成員**當您認可類別的 `Implements` 語句或 `Inherits` 語句時，文字編輯器會分別插入必須執行或覆寫之成員的原型。

  **顯示程式列分隔符號**文字編輯器會指出程式的視覺化範圍。 在專案的 .vb 原始程式檔中描繪一行，而這些原始程式檔是位於下表所列的位置：
 ...
 **啟用錯誤更正建議**文字編輯器可以建議常見錯誤的解決方案，並可讓您選取適當的更正，然後套用至您的程式碼。

 **啟用參考和關鍵字的**反白顯示文字編輯器可以反白顯示符號的所有實例或子句中的所有關鍵詞，例如 `If..Then`、`While...End While` 或 `Try...Catch...Finally`。 您可以按 CTRL+SHIFT+向下鍵或 CTRL+SHIFT+向上鍵，在反白顯示的參考或關鍵字之間巡覽。
