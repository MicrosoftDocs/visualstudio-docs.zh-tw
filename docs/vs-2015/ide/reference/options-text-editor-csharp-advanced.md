...
## <a name="outlining"></a>大綱
 當檔案開啟時，進入大綱模式，會自動概述程式碼檔案，以建立可折迭的程式碼區塊。 第一次開啟檔案時，會摺疊 #regions 區塊和非現用程式碼區塊。
...
- 無效的[編輯後繼續](../../debugger/edit-and-continue.md)編輯為紫色。
  ...
  顯示即時語意錯誤可識別沒有明確編譯的特定編譯錯誤，例如，宣告和使用未知的類型或參考未知的屬性。

  當游標位於符號內部，或是當您按一下符號時，反白顯示游標下的符號參考時，程式碼檔案中該符號的所有實例都會醒目提示。

## <a name="refactoring"></a>重構
 當您嘗試重構包含組建錯誤的程式碼，或重構會導致程式碼參考系結至與原始系結不同的專案時，請確認重整的結果會顯示 [**驗證結果**] 對話方塊。

 當您嘗試重構的成員名稱與編譯器所產生的參考相同時，在具有編譯器產生之參考的成員上警告會顯示警告對話方塊。

## <a name="xml-documentation-comments"></a>XML 文件註解
 產生///的 XML 檔批註當選取此選項時，會在您輸入///批註簡介之後，自動插入 XML 檔批註的 \<summary > 開始和結束標記。 如需 XML 文件的詳細資訊，請參閱 [XML 文件註解](https://msdn.microsoft.com/library/803b7f7b-7428-4725-b5db-9a6cff273199)。

## <a name="implement-interface"></a>實作介面
 使用時，使用 #region 來括住產生的程式碼，會在執行介面或明確執行介面時，插入 #region \<*介面名稱*> 成員。

## <a name="organize-usings"></a>組合管理 Using
 請先將 ' System ' 指示詞用於選取時排序 using，`System` using 指示詞會出現在其他 using 指示詞之前。 如需詳細資訊，請參閱[排序 Using](../../misc/sort-usings.md)。
