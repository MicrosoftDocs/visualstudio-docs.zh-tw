 ...
 **啟用 ClickOnce 安全性設定**判斷安全性設定是否在設計階段啟用。 清除這個選項後，就無法使用 [安全性] 頁面上的所有其他選項。
