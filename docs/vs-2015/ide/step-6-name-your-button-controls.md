...
1. 在表單上，選擇 [關閉] 按鈕。 （如果您仍然選取所有按鈕，請選擇 ESC 鍵取消選取）。在 [**屬性**] 視窗中滾動，直到您看到 [ **（名稱）** ] 屬性為止。 （當屬性按字母順序排列時，[ **（名稱）** ] 屬性會靠近頂端）。將名稱變更為**closeButton**，如下圖所示。
...
3. 按兩下表單上的 [顯示圖片] 按鈕。 或者，請選擇表單上的 [顯示圖片] 按鈕，然後選擇 ENTER 鍵。 這樣做時，IDE 會在主視窗中開啟另一個名為 **Form1.cs** 的索引標籤 (如果使用 Visual Basic 則為**Form1.vb** )。 這個索引標籤會顯示表單後的程式碼檔案，如下圖所示。
     ...
     ![&#35; ](../ide/media/express-showbuttoncode.png "Express_ShowButtonCode")具有 visual C# code 的 [Form1.cs] 索引標籤與 visual C 程式碼 Form1.cs 索引標籤
