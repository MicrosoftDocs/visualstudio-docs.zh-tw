...
- JavaScript 建構函式。
  ...
  `integer` 選擇項。 如果 `type` 為 `Number` ，則指定傳回值是否為整數。 設定為， `true` 表示傳回值為整數; 否則，請設定為 `false` 。 Visual Studio 未使用這個屬性來提供 IntelliSense 資訊。

  `domElement` 選擇項。 這個屬性已取代為優先於其的 `type` 屬性。 這個屬性會指定記載的傳回值是否為 DOM 元素。 設定為， `true` 以指定傳回值為 DOM 元素; 否則設定為 `false` 。 如果 `type` 屬性未設定且 `domElement` 設定為 `true` ，則 IntelliSense 會 `HTMLElement` 在執行語句完成時，將記載的傳回值視為。

  `mayBeNull` 選擇項。 指定記載的傳回值是否可以設定為 null。 設定為， `true` 表示傳回值可以設定為 null; 否則，請設定為 `false` 。 預設值是 `false`。 Visual Studio 未使用這個屬性來提供 IntelliSense 資訊。
  ...
  `locid` 選擇項。 傳回值之當地語系化資訊的識別碼。 該識別項會是成員識別碼，或對應由 OpenAjax 中繼資料所定義訊息包中的 `name` 屬性值。 識別碼類型取決於在標記中指定的格式 [\<loc>](../ide/loc-javascript.md) 。

  `value` 選擇項。 指定應評估以供 IntelliSense 使用的程式碼，而不是函式程式碼本身。 例如，您可以使用這個屬性來提供非同步回呼的 IntelliSense，例如 `Promise` 。 使用具有專案的 `value` 屬性 `<returns>` 可以略過冗長的程式碼，以改善 IntelliSense 效能。

  `description` 選擇項。 傳回值的描述。

## <a name="remarks"></a>備註
 專案 `<returns>` 必須放在函數主體中的任何語句之前。
