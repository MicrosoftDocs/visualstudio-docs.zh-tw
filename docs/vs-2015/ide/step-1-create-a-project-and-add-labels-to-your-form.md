...
7. 選擇 [ **字型** ] 屬性旁邊的加號 (+) ，然後將 [ **Size** ] 屬性的值變更為 **15.75**。
     ...
     ![顯示字型大小的屬性視窗](../ide/media/express-setfontsize.png "Express_setFontSize") 顯示字型大小的屬性視窗
...
1. 從 [工具箱] 新增**Label**控制項，然後將其 [ **Text** ] 屬性設為 [ **？** ] (問號)。
...
15. 將 **NumericUpDown** 控制項對齊加法問題的標籤控制項。

16. 將**NumericUpDown**控制項的 [ ** (名稱) ** ] 屬性值變更為 [**總和**]。

     您已建立第一個資料列，如下圖所示。

     ![數學測驗的第一列](../ide/media/express-firstrow.png "Express_firstRow") 數學測驗的第一列
...
7. 將 NumericUpDown sum 控制項的 [TabIndex]**** 屬性值設定為 [2]****、針對 difference 控制項設定為 [3]****、針對 product 控制項設定為 [4]****，以及針對 quotient 控制項設定為 [5]****。
     ...
     ![初始數學測驗表單](../ide/media/express-formlaidout.png "Express_FormLaidOut") 初始數學測驗表單
...
- 若要返回 [總覽] 主題，請參閱 [教學課程2：建立計時的數學測驗](../ide/tutorial-2-create-a-timed-math-quiz.md)。
