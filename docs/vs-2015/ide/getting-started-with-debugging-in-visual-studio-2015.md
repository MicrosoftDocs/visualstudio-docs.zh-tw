...
### <a name="configuring-a-build"></a>設定建置  
 ...
 ![Visual Studio 偵錯建置按鈕](../ide/media/vs-ide-gs-debug-build-type1.PNG "Vs_ide_gs_debug_build_type1")  
 ...
 ![Visual Studio 組態檔管理員視窗](../ide/media/vs-ide-gs-debug-build-cf-mgr.PNG "Vs_ide_gs_debug_build_cf_mgr")  
  
 您可以使用 [組態管理員] 來指定目標的建置組態。 請啟動它，並按一下 [組態] 或 [CPU] 下拉式清單，然後選取 [新增...] 來建立新的建置或平台。  
  
 ![Visual Studio 組態管理員視窗](../ide/media/vs-ide-gs-debug-build-cf-mgr-2.PNG "Vs_ide_gs_debug_build_cf_mgr_2")  
...
### <a name="building-your-code"></a>建置您的程式碼  
 ...
 ![Visual Studio 建置專案功能表選取項目](../ide/media/vs-ide-gs-debug-build-menu-item.png "Vs_ide_gs_debug_build_menu_item")  
 ...
 ![Visual Studio 成功組建輸出](../ide/media/vs-ide-gs-debug-success-build.PNG "vs_ide_gs_debug_success_build")  
...
### <a name="reviewing-the-error-list"></a>檢閱錯誤清單  
 ...
 ![Visual Studio 行號選項](../ide/media/vs-ide-gs-debug-options-line-numbers.png "Vs_ide_gs_debug_options_line_numbers")  
  
 使用 Ctrl+G 快速跳至發生錯誤的行號。  
  
 錯誤會用紅色「波浪線」的底線加以識別。 如需其他詳細資料，請將滑鼠游標暫留在這上面。 請予以修正，然後它就會消失 (儘管您在修正時可能會導入新的錯誤)。 (這稱為「迴歸」。)  
  
 ![Visual Studio 錯誤動態顯示](../ide/media/vs-ide-gs-debug-error-hover1.png "Vs_ide_gs_debug_error_hover1")  
  
 逐一查核錯誤清單，並處理您程式碼中的所有錯誤。  
  
 ![Visual Studio 偵錯錯誤視窗](../ide/media/vs-ide-gs-debug-error-list.PNG "Vs_ide_gs_debug_error_list")  
...
### <a name="reviewing-errors-in-detail"></a>詳細檢閱錯誤  
 ...
 ![Visual Studio 錯誤清單 Bing 搜尋](../ide/media/vs-ide-gs-debug-error-list-error-help.png "Vs_ide_gs_debug_error_list_error_help")  
...
### <a name="performing-static-code-analysis"></a>執行靜態程式碼分析  
 ...
 ![Visual Studio 2015 程式碼分析功能表項目](../ide/media/vs-ide-gs-debug-run-code-analysis.png "Vs_ide_gs_debug_run_code_analysis")  
  
 任何新的或已更新的警告都會出現在 IDE 底部的 [錯誤清單] 索引標籤中。 按一下以移至這些警告。  
  
 ![Visual Studio 2015 錯誤清單，但有警告](../ide/media/vs-ide-gs-debug-code-analysis-warning-error-list.PNG "vs_ide_gs_debug_code_analysis_warning_error_list")  
  
 警告會以亮黃綠色波浪線識別，而非紅色。 如需詳細資訊，請將滑鼠游標暫留於此，並以滑鼠右鍵按一下以取得操作功能表，這有助於修正或重構選項。  
  
 ![Visual Studio 程式碼分析警告動態顯示](../ide/media/vs-ide-gs-debug-code-analysis-warning-hover.png "vs_ide_gs_debug_code_analysis_warning_hover")  
...
### <a name="using-light-bulbs-to-fix-or-refactor-code"></a>使用燈泡來修正或重構程式碼。  
 ...
 ![Visual Studio 2015 燈泡快速選項](../ide/media/vs-ide-gs-debug-light-bulb1.png "Vs_ide_gs_debug_light_bulb1")  
  
 您會看到一份可能的修正或重構清單，可套用至該程式碼行。  
  
 ![Visual Studio 2015 燈泡預覽](../ide/media/vs-ide-gs-debug-light-bulb-preview-changes.PNG "Vs_ide_gs_debug_light_bulb_preview_changes")  
 ...
 ![Visual Studio 2015 燈泡 「 無選項 」 文字](../ide/media/vs-ide-gs-debug-light-bulb-no-options.PNG "Vs_ide_gs_debug_light_bulb_no_options")  
...
### <a name="debugging-your-running-code"></a>偵錯您執行的程式碼  
 既然您已成功建置程式碼並已稍加清除，請按 F5 或選取 [偵錯] -> [開始偵錯] 予以執行。 這會在偵錯環境中啟動您的應用程式，如此一來您可詳細觀察其行為。 Visual Studio 2015 IDE 會變更您的應用程式執行時：**輸出** 視窗會取代 （在預設視窗組態中），兩個新的**自動變數/區域變數/模組/監看式**索引標籤式的視窗和**呼叫堆疊/中斷點/例外狀況設定/輸出**索引標籤式的視窗。 這些視窗有多個索引標籤，可讓您在執行時檢查和評估應用程式變數、執行緒、呼叫堆疊以及各種其他行為。  
  
 ![VS2015 自動變數和呼叫堆疊 Windows](../ide/media/vs-ide-gs-debug-autos-and-call-stack.PNG "Vs_ide_gs_debug_autos_and_call_stack")  
  
 請嘗試使用應用程式的各種動作，並觀察變更。 如果某些內容出現異常，請按 Ctrl+Alt+Break (或按一下 [暫停] 按鈕) 暫停應用程式。  
  
 ![Visual Studio 全部中斷按鈕](../ide/media/vs-ide-gs-debug-break-all-button.png "vs_ide_gs_debug_break_all_button")  
  
 按 F5 繼續執行應用程式 (或按一下 [繼續] 按鈕)。  
  
 ![Visual Studio 偵錯繼續按鈕](../ide/media/vs-ide-gs-debug-continue-button.png "Vs_ide_gs_debug_continue_button")  
...
### <a name="setting-simple-breakpoints"></a>設定簡單的中斷點  
 ...
 ![Visual Studio 中斷點](../ide/media/vs-ide-gs-debug-breakpoint1.png "Vs_ide_gs_debug_breakpoint1")  
...
### <a name="setting-conditional-breakpoints"></a>設定條件中斷點  
 ...
 ![Visual Studio 2015 中斷點設定](../ide/media/vs-ide-gs-debug-breakpoint-settings.png "Vs_ide_gs_debug_breakpoint_settings")  
  
 您會看到一個看起來像這樣的對話方塊，您可以在此設定讓中斷發生的特定條件。  
  
 ![Visual Studio 2015 條件式中斷點](../ide/media/vs-ide-gs-debug-breakpoint-conditional.PNG "Vs_ide_gs_debug_breakpoint_conditional")  
  
 如需如何宣告運算式用於評估條件中斷點的詳細資訊，請看 Channel9 的影片[Visual Studio 2015 中的中斷點組態體驗](http://channel9.msdn.com/Events/Visual-Studio/Connect-event-2014/711)。  
...
### <a name="inspecting-your-code-at-run-time"></a>在執行階段檢查您的程式碼  
 ...
 ![執行 visual Studio 2015&#45;值檢查的時間](../ide/media/vs-ide-gs-debug-inspect-value.PNG "vs_ide_gs_debug_inspect_value")  
 ...
 ![Visual Studio 2015 診斷工具視窗](../ide/media/vs-ide-gs-debug-diagnostic-tools.PNG "Vs_ide_gs_debug_diagnostic_tools")  
...
### <a name="running-unit-tests"></a>執行單元測試  
 ...
 若要深入了解 Visual Studio 2015，以及它們如何協助您建立更佳品質的程式碼中的單元測試，請閱讀[單元測試基本概念](../test/unit-test-basics.md)。  
