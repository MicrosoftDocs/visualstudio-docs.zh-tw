...
1. 在方案總管**** 中，以滑鼠右鍵按一下 [參考] 節點，然後選擇 [加入參考]****。
...
## <a name="assemblies-tab"></a>組件索引標籤
 [組件]**** 索引標籤會列出可供參考的所有 .NET Framework 組件。 [ **元件** ] 索引標籤不會列出全域組件快取 (GAC) 的任何元件，因為 gac 中的元件是執行時間環境的一部分。 如果您部署或複製的應用程式中包含在 GAC 中註冊之元件的參考，則不論 [複製到本機] 設定為何，該組件都不會隨著應用程式一起部署或複製。 如需詳細資訊，請參閱[專案參考](https://msdn.microsoft.com/library/ez524kew.aspx)。
 ...
 如果專案類型不支援元件，這個索引標籤就不會出現在 [ **參考管理員** ] 對話方塊中。
...
> 如果專案參考的 SDK 取決於另一個 SDK，除非使用者手動加入另一個 SDK 的參考，否則 Visual Studio 不會使用另一個 SDK。 當使用者在 [延伸模組]**** 索引標籤上選擇 SDK 時，[參考管理員]**** 對話方塊除了列出 SDK 的名稱和版本之外，還會在詳細資料窗格中列出所有 SDK 相依性的名稱，藉此幫助使用者識別 SDK。 如果使用者未注意到相依性而只加入該 SDK，MSBuild 將會提示使用者加入相依性。
 ...
 如果專案類型不支援 **延伸**模組，此索引標籤就不會出現在 [ **參考管理員** ] 對話方塊中。
...
- **使用控制項**：XAML 控制項至少包含 <檔案名稱>**.winmd、<檔案名稱>**.dll、<檔案名稱>**.pri、<XAML 名稱>**.xaml 和 <影像名稱>**.jpg。 建立專案時，與檔案參考相關聯的資源檔將不會複製到專案的輸出目錄中，而且只會複製 *檔案名*Winmd、 *Filename*.dll 和 *filename*。 此時會記錄建置錯誤，通知使用者遺漏 <XAML 名稱>**.xaml 和 <影像名稱>**.jpg 資源。 若要成功，使用者必須手動將這些資源檔複製到專案輸出目錄中供建置和偵錯/執行階段使用。 若要解決這個問題，請遵循[建立軟體開發套件](../extensibility/creating-a-software-development-kit.md)中的步驟建立延伸模組 SDK，或編輯專案檔以新增下列屬性：
...
## <a name="search"></a>搜尋
 [ **參考管理員** ] 對話方塊中的搜尋列會在焦點所在的索引標籤上運作。 例如，如果使用者在 [ **方案** ] 索引標籤成為焦點時于搜尋列中輸入 "system"，除非解決方案包含包含 "System" 的專案名稱，否則搜尋不會傳回任何結果。

## <a name="see-also"></a>另請參閱
 [筆尖 how to：使用 [加入參考] 對話方塊來加入或移除參考](https://msdn.microsoft.com/3bd75d61-f00c-47c0-86a2-dd1f20e231c9)， [以管理專案中的參考](../ide/managing-references-in-a-project.md)
