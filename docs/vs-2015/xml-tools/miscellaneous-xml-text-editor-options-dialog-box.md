---
title: 選項對話方塊、XML、文字編輯器、其他Microsoft Docs
ms.date: 11/15/2016
...
> [!NOTE]
> 當您選取 [**文字編輯器**] 資料夾、[ **XML** ] 資料夾，然後選取 [**選項**] 對話方塊中的 [**其他**] 選項時，就可以使用這些設定。

## <a name="auto-insert"></a>自動插入
 **關閉標記** 如果已核取 [自動完成] 設定，當您輸入右角括弧 ( # A0) 以關閉開始標記時，如果標記尚未關閉，編輯器就會自動加入結束標記。 此為預設行為。

 空白項目的完成不會依賴自動完成設定。 您可以隨時輸入反斜線 (/) 以自動完成空白項目。

 **屬性引號** 撰寫 XML 屬性時，編輯器會插入 `=" "` 字元，並將插入號 (^) 在雙引號內。

 依預設為已選取。

 **命名空間** 宣告編輯器會在需要時，自動插入命名空間宣告。
...
## <a name="network"></a>網路
 **自動下載 dtd 和架構** 系統會自動從 HTTP 位置下載 (Dtd) 的架構和檔案類型定義。 此功能會使用已啟用自動 Proxy 伺服器偵測的 System.Net。
...
## <a name="caching"></a>快取
 **架構** 指定架構快取的位置。 [流覽] 按鈕 (**...**) 會在目前的架構快取位置開啟 **瀏覽目錄** 對話方塊。 您可以選取不同的目錄，也可以在對話方塊中選取資料夾，按一下滑鼠右鍵，然後選擇 [ **開啟** ] 以查看目錄中的內容。
