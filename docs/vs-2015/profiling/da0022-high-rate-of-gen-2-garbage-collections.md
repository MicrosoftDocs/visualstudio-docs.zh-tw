...
|程式碼剖析方法 |所有 |  
|訊息 |沒有相當高比率的 Gen 2 回收。 如果您設計讓大部分程式的資料結構配置並持續一段很長的時間，這通常不是問題。 不過，如果這不是預期的行為，您的應用程式可能正鎖定物件。 如果您不確定，您可以收集.NET 記憶體配置資料和物件存留期的資訊以了解應用程式所使用的記憶體配置模式。 |  
|規則類型 |警告 |  
