...
- GPU 通道。  
  ...
  若要識別要隱藏的其他執行緒，請在作用中圖例中，選擇 [**設定檔報表**] 索引標籤上的 [**每個執行緒摘要**] 報表。這會顯示執行分解圖表，其中顯示目前所選取時間間隔的執行緒狀態。 在一些縮放層級，可能不會顯示某些執行緒。 發生這種情況時，會在右邊顯示省略符號。  
...
### <a name="thread-execution-details"></a>執行緒執行詳細資料  
 ...
 當您在執行區段中選取某個點時，並行視覺化會在相關的呼叫堆疊上尋找該時間點，然後在執行區段中選取的點上方顯示黑色插入號，並在 [ **目前堆疊** ] 索引標籤上顯示呼叫堆疊本身。您可以選取執行區段上的多個點。  
...
#### <a name="io"></a>I/O  
 I/O 報表顯示負責 I/O 區塊的呼叫，以及每個呼叫堆疊的彙總封鎖時間。 如需詳細資訊，請參閱 [I/o Time (執行緒 View) ](../profiling/i-o-time-threads-view.md)。  
