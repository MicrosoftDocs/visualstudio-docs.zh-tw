...
本主題的最新的版本可從[如何： 使用命令列檢測動態編譯的 ASP.NET Web 應用程式並收集詳細計時資料 Profiler](https://docs.microsoft.com/visualstudio/profiling/how-to-instrument-a-dynamically-compiled-aspnet-web-application-and-collect-detailed-timing-data-with-the-profiler-by-using-the-command-line)。  
  
本主題說明如何透過「[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 分析工具」命令列工具，使用檢測分析方法來收集動態編譯之 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] 應用程式的詳細計時資料。  
...
>  程式碼剖析工具的命令列工具位於 [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] 安裝目錄的 \Team Tools\Performance Tools 子目錄中。 在 64 位元電腦上，64 位元和 32 位元版本的工具都可以使用。 若要使用程式碼剖析工具命令列工具，必須將工具路徑加入至命令提示字元視窗的 PATH 環境變數，或將它加入至命令本身。 如需詳細資訊，請參閱[指定命令列工具的路徑](../profiling/specifying-the-path-to-profiling-tools-command-line-tools.md)。  
 ...
 若要從 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] Web 應用程式收集效能資料，您需修改目標應用程式的 web.config 檔案來啟用 [VSInstr.exe](../profiling/vsinstr.md) 工具，以檢測動態編譯的應用程式檔案。 接著，您需使用 [VSPerfCLREnv.cmd](../profiling/vsperfclrenv.md) 工具在 Web 伺服器上設定適當的環境變數以啟用分析，然後重新啟動電腦。  
...
3.  以一般方式啟動 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] Web 應用程式。  
...
## <a name="ending-the-profiling-session"></a>結束程式碼剖析工作階段  
 若要結束分析工作階段，請關閉目標 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] Web 應用程式、重設 IIS 以停止已分析的處理程序，然後關閉分析工具。  
