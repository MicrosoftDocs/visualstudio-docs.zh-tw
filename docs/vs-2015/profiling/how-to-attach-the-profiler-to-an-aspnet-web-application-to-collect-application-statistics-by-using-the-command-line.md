---
title: 如何：使用命令列將分析工具附加至 ASP.NET Web 應用程式以收集應用程式統計資料 | Microsoft Docs
ms.date: 11/15/2016
...
>   
> 程式碼剖析工具的命令列工具位於 [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] 安裝目錄的 \Team Tools\Performance Tools 子目錄中。 在 64 位元電腦上，64 位元和 32 位元版本的工具都可以使用。 若要使用分析工具命令列工具，必須將工具路徑新增至命令提示字元視窗的 PATH 環境變數，或將它新增至命令本身。 如需詳細資訊，請參閱[指定命令列工具的路徑](../profiling/specifying-the-path-to-profiling-tools-command-line-tools.md)。  
...
## <a name="starting-the-profiler-and-attaching-to-an-aspnet-web-application"></a>啟動分析工具並附加至 ASP.NET Web 應用程式  
...
4. 啟動分析工具。 Type：**>vsperfcmd** [/start](../profiling/start.md)**： sample** [/output](../profiling/output.md)**：** `OutputFile` [ `Options` ]  
...
6. 將分析工具附加至 ASP.NET 背景工作處理序。 Type：**>vsperfcmd** [/attach](../profiling/attach.md)**：**{ `PID`&#124;`ProcName` } [ `Sample Event` ] [[/targetclr](../profiling/targetclr.md)**：** `Version` ]  
...
## <a name="ending-the-profiling-session"></a>結束程式碼剖析工作階段  
 若要結束程式碼剖析會話，請關閉 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] Web 應用程式，然後使用 Internet Information Services (IIS) **IISReset** 命令來關閉 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] 工作者進程。 呼叫 **VSPerfCmd** [/shutdown](../profiling/shutdown.md) 選項以關閉分析工具，並關閉分析資料檔案。  
...
1. 執行下列其中一項動作，以從目標應用程式中斷連結分析工具：  
    ...
    - 關閉 [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] 背景工作處理序。  
