...
## <a name="BKMK_What_the_Energy_Consumption_tool_is__how_it_works__and_what_it_measures"></a> 能源消耗分析工具是什麼，它的運作方式為何，以及測量什麼內容  
 在分析工作階段期間，能源消耗分析工具會擷取裝置顯示器、CPU 和網路連接的活動。 然後它會產生這些活動的用電量評估，以及該分析工作階段的總計能源量。  
  
> [!NOTE]
> 能源分析工具使用標準參考裝置硬體的軟體模型來代表您應用程式執行所在的低電源平板裝置，藉此估計電源和能源利用。 為了提供最佳評估結果，建議您在低電源平板裝置上收集分析資料。  
>   
> 雖然模型對於各種低電源的裝置提供良好的預估，但是分析之裝置的實際值可能會不同。 使用這些值找出相對於其他資源來說耗用量較高，也因此可能是合適的最佳化候選項目的顯示器、CPU 和網路活動。  
...
- 「*能源* 」(Energy) 用於測量總功率，以容量或電位、電池的電量，或某段時間的累計總用電量表示。 能源的單位是瓦-小時，一小時內連續產生的一瓦功率數。 在 [ **能源摘要**] 中，單位顯示為毫瓦-小時 [ **mW-h**]。  
  ...
  ![能源容量、耗電量、使用的能源總計](../profiling/media/energyprof-capcitypowerused.png "ENERGYPROF_CapcityPowerUsed")  
  
  例如，平板電腦中完全充電的電池有特定的儲存電能量。 當能源用來執行像是網路通訊、計算值或顯示圖形這類工作時，電池的電力會以不同的速率消耗。 不論任何時段，總耗電量也是由能源測量。  
...
## <a name="BKMK_Identify_scenarios_with_user_marks"></a> 透過使用者標記識別情節  
 ...
 執行方法時，該標記會在時間軸中顯示為橙色三角形。 當您將游標停留在標記上時，訊息和時間就會做為工具提示顯示。 如果兩個以上的使用者標記很接近，標記會合併，並且工具提示資料會結合在一起。 您可以放大時間軸來分隔標記。  
...
>   - 每個開啟的記錄通道都必須具有唯一名稱。 如果嘗試將新的同名記錄通道建立為未處置的通道，就會發生例外狀況。  
...
## <a name="BKMK_Configure_your_environment_for_profiling"></a> 設定您的環境以進行分析  
 為取得精確的預估，您必須在以其電池供電的低電源裝置上，分析應用程式的能源利用。 由於這類裝置大部分不會執行 Visual Studio，因此您需要將 Visual Studio 電腦連接到使用 Visual Studio 遠端工具的裝置。 若要連接到遠端裝置，您必須設定 Visual Studio 專案和遠端裝置。 如需詳細資訊，請參閱[在遠端電腦上執行 Windows 市集應用程式](../debugger/run-windows-store-apps-on-a-remote-machine.md)。  
  
> [!TIP]
> - 我們不建議在 Windows 市集模擬器或 Visual Studio 電腦上進行能源分析。 在實際裝置上的分析可提供較為實際的資料。  
>   - 在以其電池供電的目標裝置上進行分析。  
>   - 關閉可能使用相同資源 (網路、CPU 或顯示器) 的其他應用程式。  
  
## <a name="BKMK_Collect_energy_profile_data_for_your_app"></a> 收集應用程式的能源分析資料  
  
1. 在 [偵錯] 功能表上，選擇 [啟動診斷但不偵錯]。  
  
     ![選擇診斷中樞的能源耗用量](../profiling/media/energyprof-diagnosticshub.png "ENERGYPROF_DiagnosticsHub")  
...
## <a name="BKMK_Collect_energy_profile_data_for_an_installed_app"></a> 收集已安裝應用程式的能源分析資料  
 能源消耗工具只可執行於從 Visual Studio 方案啟動的 Window Store 8.1 應用程式或從 Windows 市集安裝的 Window Store 8.1 應用程式上。 方案在 Visual Studio 中開啟時，預設目標為 [ **啟始專案**]。 目標為已安裝的應用程式：  
...
|||  
|-|-|  
|![步驟1](../profiling/media/procguid-1.png "ProcGuid_1")|報告檔的名稱為 Report*YYYYMMDD-HHMM*.diagsession。 如果您決定要儲存該報告，可以變更其名稱。|  
|![步驟2](../profiling/media/procguid-2.png "ProcGuid_2")|時間軸會顯示分析工作階段的長度、應用程式週期啟用事件，以及使用者標記。|  
|![步驟3](../profiling/media/procguid-3.png "ProcGuid_3")|您可以拖曳藍色巡覽列，選取時間軸的區域，將報告限制在時間軸的一部分。|  
...
## <a name="BKMK_Optimize_energy_use"></a> 最佳化能源利用  
 除了傳輸資料之外，網路連接在初始化、維護及關閉連線時會產生能源成本。 某些網路在資料已傳送或接收後仍然保持連線一段時間，以便讓更多資料透過單一連線傳輸。 您可以使用 [ **資源 (開啟/關閉)** ] 窗格檢查應用程式與連接互動的方式。  
  
 ![關閉&#40; &#41;窗格&#47;上的資源](../profiling/media/energyprof-resources.png "ENERGYPROF_Resources")  
 ...
 對於顯示器的能源成本，能擁有的控制權比較少。 大部分的螢幕顯示淺色比深色需要更多的能源，因此使用深色背景是降低成本的一種方式。  
