...
Visual Studio **能源消耗**分析工具可協助您在完全或部分時間以其電池供電的低電源平板裝置上分析 Windows 市集應用程式的電源和能源消耗。 在電池供電的裝置上，使用太多能源的應用程式可能導致客戶諸多不滿，最後客戶可能會解除安裝應用程式。 最佳化能源利用，可以提高客戶對應用程式的採用率。  
...
- 「*功率* 」(Power) 用於測量在某段時間內完成工作的用電速率。 在電氣科學中，功率標準單位為「*瓦特*」(Watt)，定義為當一安培的電流通過一伏特的電位差時執行工作的速率。 在 [用電量] 圖形上，單位顯示為毫瓦 **mW**，相當於千分之一瓦特。  
...
## <a name="BKMK_Identify_scenarios_with_user_marks"></a> 透過使用者標記識別情節  
 您可以在分析資料中加入「*使用者標記*」(User Mark)，幫助識別時間軸尺規中的區域。  
...
4. 若要停止分析，請切換回 Visual Studio (Alt + Tab) 並選擇 [診斷中樞] 頁面上的 [**停止收集**]。  
...
## <a name="BKMK_Collect_energy_profile_data_for_an_installed_app"></a> 收集已安裝應用程式的能源分析資料  
 [能源消耗] 工具只能在從 Visual Studio 方案啟動或從 Windows 市集安裝的 Windows 8.1 市集應用程式上執行。 當方案在 Visual Studio 中開啟時，預設目標是 [**啟始專案**]。 目標為已安裝的應用程式：  
...
4. 選擇 [ **開始** ]，開始分析。  
   ...
   若要停止分析，請切換回 Visual Studio (Alt + Tab) 並選擇 [診斷中樞] 頁面上的 [**停止收集**]。  
...
|||  
|-|-|  
|![步驟1](../profiling/media/procguid-1.png "ProcGuid_1")|報告檔的名稱為 Report*YYYYMMDD-HHMM*.diagsession。 如果您決定儲存報告，可以變更名稱。|  
|![步驟2](../profiling/media/procguid-2.png "ProcGuid_2")|時間軸會顯示程式碼剖析工作階段的長度、應用程式週期啟用事件，以及使用者標記。|  
...
|![步驟5](../profiling/media/procguid-6.png "ProcGuid_6")|[ **資源 (開啟/關閉)**  ] 圖形提供網路能源成本的詳細資料。 [**網路**] 橫條圖表示網路連接為開啟狀態的時間。 [**資料傳輸**] 子橫條圖是應用程式透過網路接收或傳送資料的時間。|  
