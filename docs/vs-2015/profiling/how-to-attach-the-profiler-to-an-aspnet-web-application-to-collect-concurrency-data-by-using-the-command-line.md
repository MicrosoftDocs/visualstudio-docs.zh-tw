---
title: HOW TO：Profiler 附加至 ASP.NET Web 應用程式以使用命令列收集並行資料 |Microsoft Docs
ms.date: 11/15/2016
...
3. 輸入下列命令，將程式碼剖析工具附加至 ASP.NET 背景工作處理序︰**VSPerfCmd /attach:** `PID` [ **/targetclr:** `Version`]  
   ...
   - [/targetclr](../profiling/targetclr.md) **:** `Version` 指定當應用程式載入多個版本的執行階段時要分析的 Common Language Runtime (CLR) 版本。 這個參數是選擇性的。  
