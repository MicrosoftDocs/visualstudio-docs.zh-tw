...
## <a name="rule-description"></a>規則描述
 以元件物件模型（COM）註冊元件時，會將專案加入至元件中每個 COM 可見類型的登錄。 標記為和屬性的方法 <xref:System.Runtime.InteropServices.ComRegisterFunctionAttribute> <xref:System.Runtime.InteropServices.ComUnregisterFunctionAttribute> 會分別在註冊和取消註冊程式期間呼叫，以執行登錄/取消註冊這些類型的特定使用者程式碼。 這段程式碼不應該在這些進程之外呼叫。

## <a name="how-to-fix-violations"></a>如何修正違規
 若要修正此規則的違規，請將方法的存取範圍變更為 `private` 或 `internal` （ `Friend` 在中為 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ）。
