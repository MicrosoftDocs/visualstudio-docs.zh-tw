...
## <a name="cause"></a>原因
 外部可見的類型會實作為 <xref:System.Runtime.Serialization.ISerializable?displayProperty=fullName> 介面，而類型不會以 <xref:System.SerializableAttribute?displayProperty=fullName> 屬性標記。 此規則會忽略基底類型不可序列化的衍生類型。

## <a name="rule-description"></a>規則描述
 若要由 common language runtime 辨識為可序列化， <xref:System.SerializableAttribute> 即使類型透過介面的實作為使用自訂序列化常式，也必須以屬性標記類型 <xref:System.Runtime.Serialization.ISerializable> 。
