...
## <a name="rule-description"></a>規則描述
 ...
 根據預設，COM 可以看到所有公用實數值型別。 不過，若要減少誤報，此規則需要明確陳述類型的 COM 可見度。 包含的元件必須以 <xref:System.Runtime.InteropServices.ComVisibleAttribute?displayProperty=fullName> 設定為的標記 `false` ，而且類型必須以設定為的標記 <xref:System.Runtime.InteropServices.ComVisibleAttribute> `true` 。

## <a name="how-to-fix-violations"></a>如何修正違規
 若要修正此規則的違規，並將欄位保留為隱藏狀態，請將數值型別變更為參考型別，或 <xref:System.Runtime.InteropServices.ComVisibleAttribute> 從類型中移除屬性。
