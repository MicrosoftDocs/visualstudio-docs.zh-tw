...
## <a name="example"></a>範例
 ...
 另一個選項是從程式碼中移除判斷提示，並讓任何後續的檔案 i/o 許可權要求流程超出 SecurityTransparentMethod 至呼叫者。 這會啟用安全性檢查。 在此情況下，通常不需要安全性審查，因為許可權要求會流向呼叫端和（或）應用程式域。 許可權要求會透過安全性原則、裝載環境和程式碼來源許可權授與來緊密控制。
