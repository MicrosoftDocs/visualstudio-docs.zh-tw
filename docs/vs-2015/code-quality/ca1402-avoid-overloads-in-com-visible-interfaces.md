...
## <a name="how-to-fix-violations"></a>如何修正違規
 若要修正此規則的違規情形，請重新命名多載的方法，讓名稱成為唯一的。 或者，藉由將存取範圍變更為 `internal` （ `Friend` 在中 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ）或套用 <xref:System.Runtime.InteropServices.ComVisibleAttribute?displayProperty=fullName> 屬性設定為 `false` ，讓 COM 無法看到介面。
