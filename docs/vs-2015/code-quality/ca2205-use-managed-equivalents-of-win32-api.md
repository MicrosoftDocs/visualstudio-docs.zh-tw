...
## <a name="cause"></a>原因
 已定義平台叫用方法，而且類別庫中存在具有對等功能的方法 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 。

## <a name="rule-description"></a>規則描述
 平台叫用方法可用來呼叫非受控 DLL 函式，並使用 <xref:System.Runtime.InteropServices.DllImportAttribute?displayProperty=fullName> 屬性或 `Declare` Visual Basic 中的關鍵字來定義。 由於 misnamed 函式、參數和傳回值資料類型的錯誤對應，以及不正確的欄位規格（例如呼叫慣例和字元集），不正確定義的平台叫用方法可能會導致執行時間例外狀況。 如果有的話，呼叫同等的 managed 方法通常比定義和直接呼叫非受控方法更簡單且較不容易發生錯誤。 呼叫平台叫用方法也可能會導致需要解決的其他安全性問題。
