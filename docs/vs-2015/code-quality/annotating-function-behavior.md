...
ms.locfileid: "77277865"
---
# <a name="annotating-function-behavior"></a>註釋函式行為
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

除了批註函式 [參數和傳回值](../code-quality/annotating-function-parameters-and-return-values.md)之外，您還可以標注整個函數的屬性。  
...
|Annotation|描述|  
|----------------|-----------------|  
|`_Called_from_function_class_(name)`|這個述詞不適合單獨使用，而是要搭配 `_When_` 註釋使用。 如需詳細資訊，請參閱 [指定套用注釋的時機和位置](../code-quality/specifying-when-and-where-an-annotation-applies.md)。<br /><br /> `name`參數是任一字元串，也會出現在 `_Function_class_` 部分函式宣告的注釋中。  `_Called_from_function_class_` 如果目前正在分析的函式使用具有相同值的標注，則會傳回非零 `_Function_class_` `name` ，否則會傳回零。|  
...
|`_Raises_SEH_exception_`|標註一定會引發結構化例外狀況處理常式 (SEH) 例外狀況的函式 (受 `_When_` 和 `_On_failure_` 條件所限制)。 如需詳細資訊，請參閱 [指定套用注釋的時機和位置](../code-quality/specifying-when-and-where-an-annotation-applies.md)。|  
...
|`_Use_decl_annotations_`|可用於函式定義 (也稱為函式主體) 用來取代標頭中的注釋清單。  `_Use_decl_annotations_`使用時，會使用出現在相同函式之範圍標頭中的注釋，如同它們也出現在具有批註的定義中 `_Use_decl_annotations_` 。|  
...
|`_Success_(expr)`|`expr` 是產生右值的運算式。 當 `_Success_` 註釋出現在函式宣告或定義上時，函式上和後置條件中每個註釋 (`anno`) 的行為就如同撰寫為 `_When_(expr, anno)` 程式碼一樣。 `_Success_` 註釋只能在函式上使用，而不能在其參數或傳回型別上使用。 函式上最多只能有一個 `_Success_` 註釋，而且不能在任何 `_When_`、`_At_` 或 `_Group_` 中。 如需詳細資訊，請參閱 [指定套用注釋的時機和位置](../code-quality/specifying-when-and-where-an-annotation-applies.md)。|  
