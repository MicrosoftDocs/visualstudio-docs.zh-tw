...
命名警告支援遵循 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 設計指導方針的命名慣例。
...
|[CA1724：類型名稱不應該和命名空間相符](../code-quality/ca1724-type-names-should-not-match-namespaces.md)|類型名稱不得符合 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] Class Library 中定義的命名空間名稱。 違反此規則可能會降低程式庫的可用性。|
