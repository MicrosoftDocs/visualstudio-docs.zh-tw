---
title: 如何：為 Managed 程式碼專案設定程式碼分析 |Microsoft Docs
ms.date: 11/15/2016
...
在 [!INCLUDE[vsUltLong](../includes/vsultlong-md.md)]、[!INCLUDE[vsPreLong](../includes/vsprelong-md.md)] 和 [!INCLUDE[vsPro](../includes/vspro-md.md)] 中，您可以選擇要套用至 managed 程式碼專案的程式碼分析*規則集*清單。 預設規則集是 Microsoft 建議的最低規則。 您可以將另一個規則集套用至專案或方案中的所有專案。

> [!NOTE]
> 如需如何為 ASP.NET Web 應用程式設定規則集的詳細資訊，請參閱[如何：設定 ASP.NET Web 應用程式的程式碼分析](../code-quality/how-to-configure-code-analysis-for-an-aspnet-web-application.md)。
...
5. 根據預設，程式碼分析不會報告從外部工具自動產生的程式碼警告。 若要從產生的程式碼中查看警告，請清除 [**隱藏產生的程式碼的結果**] 核取方塊。
