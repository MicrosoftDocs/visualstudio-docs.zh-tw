...
## <a name="cause"></a>原因
 外部可見的類型會執行 <xref:System.Collections.IEnumerable?displayProperty=fullName> 介面，但不會實作用 <xref:System.Collections.Generic.IEnumerable%601?displayProperty=fullName> 介面和包含元件目標 [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] 。 此規則會忽略執行的類型 <xref:System.Collections.IDictionary?displayProperty=fullName> 。
...
### <a name="description"></a>描述
 下列範例會將集合的基類從非泛型 `CollectionBase` 類別變更為泛型 `Collection<T>` （ `Collection(Of T)` in [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ）類別，藉以修正違規。
...
### <a name="description"></a>描述
 下列範例會藉由執行下列泛型介面來修正違規： `IEnumerable<T>` 、 `ICollection<T>` 和 `IList<T>` （ `IEnumerable(Of T)` 中的、和 `ICollection(Of T)` `IList(Of T)` [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ）。
