...
## <a name="example"></a>範例
 下列範例會定義兩個屬性。 `BadCodeMaintainerAttribute`不正確地省略 <xref:System.AttributeUsageAttribute> 語句，並正確地執行本節稍 `GoodCodeMaintainerAttribute` 早所述的屬性。 請注意，[ `DeveloperName` 設計規則[Ca1019 必須：定義屬性引數的存取](../code-quality/ca1019-define-accessors-for-attribute-arguments.md)子] 和 [完整性] 所包含的屬性是必要的。
