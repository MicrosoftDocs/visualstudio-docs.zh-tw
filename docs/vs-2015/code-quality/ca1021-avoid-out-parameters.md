...
### <a name="description"></a>描述
 執行**Try \<Something >** 模式的方法（例如 <xref:System.Int32.TryParse%2A?displayProperty=fullName>）不會引發這個違規。 下列範例顯示的結構（實值型別）會執行 <xref:System.Int32.TryParse%2A?displayProperty=fullName> 方法。
