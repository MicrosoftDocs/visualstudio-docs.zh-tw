...
- `_In_reads_(s)`  
     ...
     `_bytes_`Variant 會提供以位元組為單位的大小，而不是元素。 只有當大小無法以元素表示時，才使用此專案。  例如，只有在使用的類似函式 `char` 時，字串才會使用 `_bytes_` variant `wchar_t` 。  
...
- `_Inout_updates_(s)`  
     ...
     陣列的指標，在函式中讀取和寫入。  它是大小的 `s` 元素，而且在前置狀態和後置狀態中有效。  
  
     `_bytes_`Variant 會提供以位元組為單位的大小，而不是元素。 只有當大小無法以元素表示時，才使用此專案。  例如，只有在使用的類似函式 `char` 時，字串才會使用 `_bytes_` variant `wchar_t` 。  
  
- `_Inout_updates_z_(s)`  
  
     陣列的指標，其以 null 結束且具有已知的大小。 完成 null 結束字元的元素（必須存在）必須是前置狀態和後置狀態中的有效專案。  後置狀態中的值會假設為與前置狀態中的值不同。這包括 null 結束字元的位置。 如果大小是以位元組為單位，則會 `s` 依元素大小進行調整。  
...
- `_Out_writes_to_(s,c)`  
     ...
     元素陣列的指標 `s` 。  元素在預先狀態中不一定是有效的。  在後置狀態中，最多為 `c` -第個元素的元素必須是有效的。  如果大小是以位元組為單位，則 `s` 會 `c` 依元素大小調整，或使用 `_bytes_` variant，其定義為：  
  
     `_Out_writes_to_(_Old_(s), _Old_(s))    _Out_writes_bytes_to_(_Old_(s), _Old_(s))`  
  
     換句話說，處於前置狀態的緩衝區中的每個專案，在 `s` 後置狀態中都是有效的。  例如：  
...
- `_Inout_updates_to_(s,c)`  
     ...
     陣列的指標，這是函式的讀取和寫入。  它是大小的 `s` 元素，全部都必須在前置狀態中有效，而 `c` 元素在後置狀態中必須是有效的。  
...
- `_In_reads_to_ptr_(p)`  
     ...
     陣列的指標，其運算式 `p` `_Curr_` （也就是 `p` 減號 `_Curr_` ）是由適當的語言標準所定義。  之前的專案 `p` 必須在前置狀態中有效。  
  
- `_In_reads_to_ptr_z_(p)`  
  
     以 null 結束之陣列的指標，其運算式 `p` `_Curr_` （也就是 `p` 減號 `_Curr_` ）是由適當的語言標準所定義。  之前的專案 `p` 必須在前置狀態中有效。  
  
- `_Out_writes_to_ptr_(p)`  
  
     陣列的指標，其運算式 `p` `_Curr_` （也就是 `p` 減號 `_Curr_` ）是由適當的語言標準所定義。  之前的元素 `p` 不一定要在預先狀態中有效，而且在後置狀態中必須是有效的。  
  
- `_Out_writes_to_ptr_z_(p)`  
  
     以 null 結束之陣列的指標，其運算式 `p` `_Curr_` （也就是 `p` 減號 `_Curr_` ）是由適當的語言標準所定義。  之前的元素 `p` 不一定要在預先狀態中有效，而且在後置狀態中必須是有效的。  
  
## <a name="optional-pointer-parameters"></a>選擇性指標參數  
 當指標參數注釋包含時 `_opt_` ，它會指出參數可能是 null。 否則，批註的執行會與不包含的版本相同 `_opt_` 。 以下是 `_opt_` 指標參數注釋的變數清單：  
...
## <a name="output-reference-parameters"></a>輸出參考參數  
 參考參數的常見用法是用於輸出參數。  若是簡單的輸出參考參數（例如） `int&` ， `_Out_` 則會提供正確的語法。  不過，當輸出值是指標時（例如），對 `int *&` 等的指標注釋（如 `_Outptr_ int **` 不提供正確的語義）。  若要以簡明的表示指標類型的輸出參考參數的語法，請使用下列複合批註：  
...
## <a name="see-also"></a>另請參閱  
 [使用 SAL 注釋減少 C/c + + 程式碼缺失](../code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects.md)   
