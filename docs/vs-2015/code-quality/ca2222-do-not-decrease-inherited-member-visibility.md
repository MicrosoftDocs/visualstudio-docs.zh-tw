...
## <a name="rule-description"></a>規則描述
 您不得變更繼承成員的存取修飾詞 (Modifier)。 將繼承成員變更為私用不會防止呼叫端存取方法的基底類別 (Base Class) 實作。 如果成員設為私用且類型為未密封，則繼承類型可以呼叫繼承階層架構中最後一個公用的方法。 如果您必須變更存取修飾詞，則應該將方法標記為 final，或是將它的型別密封以防止覆寫方法。
