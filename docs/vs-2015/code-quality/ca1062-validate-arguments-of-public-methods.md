...
## <a name="cause"></a>原因
 外部可見的方法會對其中一個參考引數取值，而不會驗證該引數是否為 `null` （ `Nothing` 在 Visual Basic 中）。
...
## <a name="example"></a>範例
 在中 [!INCLUDE[vsprvslong](../includes/vsprvslong-md.md)] ，此規則不會偵測到參數會傳遞至另一個執行驗證的方法。
...
## <a name="example"></a>範例
 填入屬於參考物件之欄位或屬性的複製函式，也可能違反 CA1062 規則。 發生違規的原因是，傳遞至複製的函式的複製物件可能是 `null` （ `Nothing` 在 Visual Basic 中）。 若要解決違規，請使用靜態（在 Visual Basic 中為 Shared）方法來檢查複製的物件是否不是 null。
