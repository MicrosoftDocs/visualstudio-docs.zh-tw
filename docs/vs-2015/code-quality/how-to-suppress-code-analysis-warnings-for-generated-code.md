...
Managed 程式碼編譯器通常會產生新增至專案的程式碼，以加速程式碼開發。 此外，開發人員通常會使用協力廠商工具來協助快速開發應用程式。 這些工具也會產生加入至專案的程式碼。

 您可能會想要查看程式碼分析在產生的程式碼中發現的規則違規。 但是，如果您無法查看和維護包含違規的程式碼，您可能不想看到它們。

 專案的 [程式碼分析] 屬性頁上的 [ **隱藏來自產生的程式碼的結果** ] 核取方塊可讓您選取是否要從協力廠商工具產生的程式碼中查看程式碼分析警告。

> [!NOTE]
> 當表單和範本中出現錯誤和警告時，此選項不會抑制產生的程式碼中的程式碼分析錯誤和警告。 您可以同時檢視及維護表單或範本的原始程式碼。
