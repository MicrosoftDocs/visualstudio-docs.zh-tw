---
title: CA1007 建議：在適當的情況下使用泛型 |Microsoft Docs
ms.date: 11/15/2016
...
## <a name="cause"></a>原因
 外部可見的方法包含類型的參考參數 <xref:System.Object?displayProperty=fullName> ，以及包含元件目標 [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] 。

## <a name="rule-description"></a>規則描述
 參考參數是 `ref` `ByRef` 在) 關鍵字中使用 (修改的參數 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 。 提供給參考參數的引數類型必須完全符合參考參數類型。 若要使用衍生自參考參數類型的型別，必須先將型別轉換並指派給 reference 參數型別的變數。 使用泛型方法可將所有類型（受限於條件約束）傳遞給方法，而不需要先將型別轉換成參考參數型別。
