...
|CheckId|CA1008|
|類別|Microsoft. 設計|
|中斷變更|不中斷-當系統提示您將 **None** 值新增至非旗標列舉時。重大-當系統提示您重新命名或移除任何列舉值時。|

## <a name="cause"></a>原因
 沒有套用的列舉不 <xref:System.FlagsAttribute?displayProperty=fullName> 會定義值為零的成員; 或已套用的列舉會 <xref:System.FlagsAttribute> 定義具有值為零但其名稱不是 ' None ' 的成員，或列舉定義多個零值的成員。

## <a name="rule-description"></a>規則描述
 未初始化列舉的預設值就像其他實值型別一樣，是零。 非旗標−屬性化列舉應定義具有零值的成員，如此一來，預設值就是列舉的有效值。 如果有的話，請將成員命名為 ' None '。 否則，請將零指派給最常使用的成員。 請注意，根據預設，如果宣告中未設定第一個列舉成員的值，其值為零。

 如果已套用的列舉 <xref:System.FlagsAttribute> 定義了零值的成員，其名稱應為 ' None '，表示列舉中未設定任何值。 針對任何其他用途使用零值成員，與中的用法相反， <xref:System.FlagsAttribute> 在中，和和或位運算子對成員毫無用處。 這表示只應將值零指派給一個成員。 請注意，如果有多個值為零的成員出現在旗標屬性的列舉中，則 `Enum.ToString()` 會針對非零的成員傳回不正確的結果。

## <a name="how-to-fix-violations"></a>如何修正違規
 若要修正非旗標−屬性化列舉的此規則違規，請定義值為零的成員。這是不中斷的變更。 針對定義零值成員的旗標屬性化列舉，請將這個成員命名為 ' None '，然後刪除任何其他值為零的成員。這是中斷性變更。
