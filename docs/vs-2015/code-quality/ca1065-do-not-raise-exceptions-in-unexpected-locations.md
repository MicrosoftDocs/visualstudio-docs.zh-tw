...
### <a name="property-get-methods"></a>屬性 Get 方法
 屬性基本上是智慧型欄位。 因此，它們的行為應該盡可能與欄位相同。 欄位不會擲回例外狀況，也不會擲回屬性。 如果您有擲回例外狀況的屬性，請考慮將它設為方法。

 下列例外狀況允許從屬性 get 方法擲回：
 ...
 **Dispose (false) **程式碼路徑永遠不會擲回例外狀況，因為這幾乎一律從完成項呼叫。
...
### <a name="implicit-cast-operators"></a>隱含轉換運算子
 由於使用者通常不會察覺到隱含轉型運算子已被呼叫，因此隱含轉換運算子擲回的例外狀況是完全非預期的。 因此，不應該從隱含轉換運算子擲回例外狀況。
...
## <a name="when-to-suppress-warnings"></a>隱藏警告的時機
 如果違規是由例外狀況宣告而非擲回例外狀況所造成，則可以安全地隱藏此規則的警告。
