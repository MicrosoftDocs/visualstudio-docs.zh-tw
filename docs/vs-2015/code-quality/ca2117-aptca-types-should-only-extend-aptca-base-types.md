---
title: CA2117： APTCA 類型應該只擴充 APTCA 基底類型 |Microsoft Docs
ms.date: 11/15/2016
ms.prod: visual-studio-dev14
ms.technology: vs-ide-code-analysis
ms.topic: reference
f1_keywords:
- CA2117
- AptcaTypesShouldOnlyExtendAptcaBaseTypes
helpviewer_keywords:
- AptcaTypesShouldOnlyExtendAptcaBaseTypes
- CA2117
ms.assetid: c505b586-2f1e-47cb-98ee-a5afcbeda70f
caps.latest.revision: 18
author: jillre
ms.author: jillfra
manager: wpickett
ms.openlocfilehash: 90c1f66f36fc689ee077ec66f154487d65ee13a1
ms.sourcegitcommit: 6cfffa72af599a9d667249caaaa411bb28ea69fd
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 09/02/2020
ms.locfileid: "85543607"
---
# <a name="ca2117-aptca-types-should-only-extend-aptca-base-types"></a>CA2117:APTCA 類型應該只擴充 APTCA 基底類型
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

|Item|值|
|-|-|
|TypeName|AptcaTypesShouldOnlyExtendAptcaBaseTypes|
|CheckId|CA2117|
|類別|Microsoft.Security|
|中斷變更|中斷|

## <a name="cause"></a>原因
 元件中的公用或受保護型別，其 <xref:System.Security.AllowPartiallyTrustedCallersAttribute?displayProperty=fullName> 屬性繼承自元件中宣告的型別，而該元件沒有屬性。

## <a name="rule-description"></a>規則描述
 根據預設，具有強式名稱之元件中的公用或受保護類型，會隱含地受到完全信任的 [繼承要求](https://msdn.microsoft.com/28b9adbb-8f08-4f10-b856-dbf59eb932d9) 保護。 以 (APTCA) 屬性標記的強式名稱元件沒有 <xref:System.Security.AllowPartiallyTrustedCallersAttribute> 此保護。 屬性會停用繼承要求。 這會使元件中宣告的公開類型可由沒有完全信任的類型繼承。

 當完全信任的元件上有 APTCA 屬性，而且元件中的型別繼承自不允許部分信任呼叫端的型別時，就可以有安全性惡意探索。 如果有兩種類型 `T1` 且 `T2` 符合下列條件，惡意呼叫端可以使用類型 `T1` 來略過保護的隱含完全信任繼承要求 `T2` ：

- `T1` 是在完全信任的元件中宣告的公用型別，該元件具有 APTCA 屬性。

- `T1` 繼承自 `T2` 其元件之外的型別。

- `T2`的元件沒有 APTCA 屬性，因此不應由部分信任元件中的類型繼承。

  部分信任的型別 `X` 可以繼承自 `T1` ，而這可讓它存取在中宣告的繼承成員 `T2` 。 因為沒有 `T2` APTCA 屬性，所以它的立即衍生型別 (`T1`) 必須滿足完全信任的繼承要求; `T1` 具有完全信任，因此可滿足這種檢查。 安全性風險是因為不 `X` 會參與滿足不受信任子類別保護的繼承要求 `T2` 。 基於這個理由，具有 APTCA 屬性的型別不能延伸沒有屬性的類型。

  另一個安全性問題，可能是較常見的問題，就是衍生型別 (`T1`) 可以透過程式設計人員錯誤，從需要完全信任 () 的類型公開受保護的成員 `T2` 。 發生這種情況時，不受信任的呼叫端會取得應僅適用于完全信任類型之資訊的存取權。

## <a name="how-to-fix-violations"></a>如何修正違規
 如果違規所報告的型別是在不需要 APTCA 屬性的元件中，請將它移除。

 如果需要 APTCA 屬性，請將完全信任的繼承要求加入至類型。 這可防止不受信任的類型繼承。

 您可以藉由將 APTCA 屬性新增至違規所報告之基底類型的元件來修正違規。 請勿先對元件中的所有程式碼進行大量的安全性檢查，以及相依于元件的所有程式碼，再執行此動作。

## <a name="when-to-suppress-warnings"></a>隱藏警告的時機
 若要安全地隱藏此規則的警告，您必須確定您的類型所公開的受保護成員不會直接或間接允許不受信任的呼叫端存取機密資訊、作業或可利用破壞性方式使用的資源。

## <a name="example"></a>範例
 下列範例會使用兩個元件和一個測試應用程式來說明此規則所偵測到的安全性弱點。 第一個元件沒有 APTCA 屬性，而且不應該由部分信任型別繼承 (`T2` 在先前的討論) 中表示。

 [!code-csharp[FxCop.Security.NoAptcaInherit#1](../snippets/csharp/VS_Snippets_CodeAnalysis/FxCop.Security.NoAptcaInherit/cs/FxCop.Security.NoAptcaInherit.cs#1)]

## <a name="example"></a>範例
 第二個元件（ `T1` 在先前的討論中表示）是完全信任的元件，並允許部分信任的呼叫端。

 [!code-csharp[FxCop.Security.YesAptcaInherit#1](../snippets/csharp/VS_Snippets_CodeAnalysis/FxCop.Security.YesAptcaInherit/cs/FxCop.Security.YesAptcaInherit.cs#1)]

## <a name="example"></a>範例
 先前討論中所表示的測試類型 `X` 位於部分信任的元件中。

 [!code-csharp[FxCop.Security.TestAptcaInherit#1](../snippets/csharp/VS_Snippets_CodeAnalysis/FxCop.Security.TestAptcaInherit/cs/FxCop.Security.TestAptcaInherit.cs#1)]

 此範例會產生下列輸出。

 **在 shady glen 2/22/2003 12:00:00 AM 開會！** 
**從測試： sunny meadow** 
**在 sunny meadow 2/22/2003 12:00:00 AM 開會！**
## <a name="related-rules"></a>相關規則
 [CA2116:APTCA 方法應該只呼叫 APTCA 方法](../code-quality/ca2116-aptca-methods-should-only-call-aptca-methods.md)

## <a name="see-also"></a>另請參閱
 [使用部分](https://msdn.microsoft.com/library/dd66cd4c-b087-415f-9c3e-94e3a1835f74)信任程式碼[繼承要求](https://msdn.microsoft.com/28b9adbb-8f08-4f10-b856-dbf59eb932d9)中的程式庫，[安全](https://msdn.microsoft.com/library/4f882d94-262b-4494-b0a6-ba9ba1f5f177)[程式碼撰寫方針 .NET Framework 元件可由部分信任的程式碼](https://msdn.microsoft.com/a417fcd4-d3ca-4884-a308-3a1a080eac8d)呼叫
