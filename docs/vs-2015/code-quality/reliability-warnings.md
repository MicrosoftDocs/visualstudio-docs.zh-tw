...
可靠性警告支援程式庫和應用程式可靠性，例如正確的記憶體和執行緒使用方式。
...
|[CA2004：必須移除對 GC.KeepAlive 的呼叫](../code-quality/ca2004-remove-calls-to-gc-keepalive.md)|如果您要轉換成 SafeHandle 使用方式，請移除所有對 GC 的呼叫。KeepAlive （物件）。 在此情況下，類別應該不需要呼叫 GC。KeepAlive，假設它們沒有完成項，但依賴 SafeHandle 來完成作業系統控制碼。|
