...
您可以使用程式碼分析簽入原則，要求在 team 專案中的所有程式碼專案上執行程式碼分析。 需要程式碼分析可以改善已簽入程式碼基底之程式碼的品質。

> [!NOTE]
> 只有當您使用 Team Foundation Server 時，才可使用這項功能。

 程式碼分析簽入原則會設定于 team 專案設定中，並套用至 team 專案中的每個程式碼專案。 程式碼分析回合是針對程式碼專案的專案（. .xxproj）檔案中的程式碼專案所設定。 程式碼分析回合會在本機電腦上執行。 當您啟用程式碼分析簽入原則時，要簽入之程式碼專案中的檔案必須在其上次編輯後編譯，且包含的程式碼分析執行至少必須在 c 的電腦上執行已進行變更 h)。

- 針對 managed 程式碼，您可以藉由指定包含程式碼分析規則子集的*規則集*來設定簽入原則。

- 針對 C/C++程式碼，簽入原則會要求執行所有程式碼分析規則。 您可以新增預處理器指示詞，以停用 team 專案中個別程式碼專案的特定規則。

  在您指定 managed 程式碼的簽入原則之後，小組成員可以將程式碼專案的程式碼分析設定同步處理至 team 專案原則設定。
...
- 從 [**執行此規則集**] 清單中，使用下列其中一種方法：
  ...
  - 若要選取自訂規則集，請按一下 [**從原始檔控制 \<Select 規則集 ...]。>** ，然後在原始檔控制瀏覽器中輸入規則集的版本控制路徑。 版本控制路徑的語法為：
