---
title: CA1020：避免使用少數類型的命名空間 |Microsoft Docs
ms.date: 11/15/2016
...
## <a name="rule-description"></a>規則描述
 請確定每個命名空間都有邏輯組織，而且存在於稀疏填入的命名空間中的有效原因。 命名空間應該包含在大部分情況下一起使用的類型。 當其應用程式互斥時，類型應該位於不同的命名空間中。 例如， <xref:System.Web.UI> 命名空間包含 Web 應用程式中所使用的型別，而且 <xref:System.Windows.Forms> 命名空間包含以 [!INCLUDE[TLA#tla_mswin](../includes/tlasharptla-mswin-md.md)] 應用程式為基礎的類型。 雖然這兩個命名空間都具有控制使用者介面層面的型別，但是這些型別並不是設計來用於相同的應用程式。 因此，它們位於不同的命名空間中。 小心命名空間組織也會很有説明，因為它會增加功能的可探索性。 藉由檢查命名空間階層，程式庫取用者應該能夠找到執行功能的類型。
