...
## <a name="cause"></a>原因
 類型包含傳回 void 的委派，其簽章包含兩個參數（第一個是物件，第二個是可指派給 EventArgs 的類型），以及包含元件目標 [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] 。

## <a name="rule-description"></a>規則描述
 [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)]在之前，為了將自訂資訊傳遞至事件處理常式，必須宣告新的委派，以指定衍生自類別的類別 <xref:System.EventArgs?displayProperty=fullName> 。 在中，這已不再成立 [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] ，這會引進 <xref:System.EventHandler%601?displayProperty=fullName> 委派。 這個泛型委派可讓衍生自的任何類別 <xref:System.EventArgs> 與事件處理常式一起使用。

## <a name="how-to-fix-violations"></a>如何修正違規
 若要修正此規則的違規，請移除委派，並使用委派取代其使用方式 <xref:System.EventHandler%601?displayProperty=fullName> 。 如果委派是由編譯器自動產生 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] ，請將事件宣告的語法變更為使用 <xref:System.EventHandler%601?displayProperty=fullName> 委派。

## <a name="when-to-suppress-warnings"></a>隱藏警告的時機
 請勿隱藏此規則的警告。

## <a name="example"></a>範例
 下列範例顯示違反規則的委派。 在 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 範例中，批註會說明如何修改範例以符合規則。 在 c # 範例中，會顯示已修改程式碼的範例。
...
## <a name="example"></a>範例
 下列範例會移除上一個範例中符合規則的委派宣告，並 `ClassThatRaisesEvent` 使用委派來取代和方法中的使用 `ClassThatHandlesEvent` <xref:System.EventHandler%601?displayProperty=fullName> 。
