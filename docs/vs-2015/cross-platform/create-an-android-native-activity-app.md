...
## <a name="create-a-new-native-activity-project"></a><a name="Create"></a> 建立新的原生活動專案  
...
3. 指定像是 `MyAndroidApp` 的應用程式名稱，然後選擇 [確定]****。  
    ...
    ![方案總管中的 Native Activity 專案](../cross-platform/media/cppmdd-rc-na-solutionexp.PNG "CPPMDD_RC_NA_SolutionExp")  
...
## <a name="build-and-run-the-default-android-native-activity-app"></a><a name="BuildHello"></a> 建立並執行預設 Android Native Activity 應用程式  
...
1. 如果尚未選取，請從 [**方案平臺**] 下拉式清單中選擇 [ **x86** ]。  
  
     ![方案平台下拉式 x86 選取範圍](../cross-platform/media/cppmdd-rc-na-solution-x86.png "CPPMDD_RC_NA_Solution_x86")  
  
     如果未顯示 [**方案平臺**] 清單，請從 [**新增/移除按鈕**] 清單中選擇 [**方案平臺**]，然後選擇您的平臺。  
...
4. 按 F5 啟動偵錯，或按 Shift+F5 啟動但不偵錯。  
     ...
     Visual Studio 啟動模擬器，這需要幾秒的時間來載入及部署您的程式碼。 在您的應用程式啟動之後，您可以設定中斷點，並使用偵錯工具，以逐步執行程式碼、檢查區域變數及監看值。  
