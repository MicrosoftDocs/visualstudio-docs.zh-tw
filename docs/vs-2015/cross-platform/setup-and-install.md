...
- 若要使用 Windows 和 Android 應用程式：一部已安裝 Visual Studio 2015 和 Xamarin 4 的 Windows 開發電腦 (請參閱下列注意事項)  (您也可以遵循 [直接 Xamarin 安裝](https://developer.xamarin.com/guides/cross-platform/getting_started/requirements/#install) (xamarin.com) 的指示來使用 Visual Studio 2013。 )    
...
如果您在執行此設定和安裝之後遇到使用 Xamarin 的問題，請在 [forums.xamarin.com](https://forums.xamarin.com/) 上提出您的問題。
...
## <a name="pre-requisites"></a><a name="prereq"></a> 先決條件  
...
## <a name="windows-setup-visual-studio-and-xamarin"></a><a name="windows"></a> Windows 安裝程式 (Visual Studio 和 Xamarin)   
  
> [!TIP]
> 這些指示適用於 Visual Studio 2015。 若要使用 Xamarin 搭配 Visual Studio 2013 (Update 2 需要) ，請遵循 [Direct Xamarin 安裝](https://developer.xamarin.com/guides/cross-platform/getting_started/requirements/#install) (xamarin.com) 的指示。  
...
2. (僅限新的安裝) 在安裝程式中，選取 [自訂] **** 安裝：  
    ...
    ![在 Visual Studio 安裝中選擇 [自訂] 選項](../cross-platform/media/cross-plat-xamarin-setup-1.png "跨平臺 Xamarin 安裝程式1")  
  
3. 核取下列方塊：  
  
   1. [跨平台行動開發] > [C#/.NET (Xamarin)]****。 這樣做也會自動選取 [常用工具及軟體開發套件] 下的各種 Android 工具。 此選項應該也會更新任何現有的 Xamarin 安裝。  
  
        ![選取 [跨&#45;平臺行動裝置開發] 底下的 [Xamarin] 選項](../cross-platform/media/cross-plat-xamarin-setup-2.png "跨平臺 Xamarin 設定2")  
...
5. 安裝完成之後，請啟動 Visual Studio，並在出現提示時，以您的 Microsoft 帳戶進行登入 (這是您用於 Windows 的相同帳戶)。 然後透過 [工具] > [選項] > [Xamarin]**** 或 [工具] > [選項] > [Xamarin] > [其他]**** 檢查是否有 Xamarin 更新，您會在此找到 [立即檢查]**** 連結：  
    ...
    ![在 Visual Studio 選項中檢查 Xamarin 更新](../cross-platform/media/cross-plat-xamarin-setup-3.png "跨平臺 Xamarin 安裝程式3")  
...
6. 針對 Windows 7 (含) 之前版本，或是在 Mac 上執行 Windows，如果您沒有實體裝置，請使用 [Android SDK 模擬器 (英文)](https://developer.xamarin.com/guides/android/deployment,_testing,_and_metrics/debug-on-emulator/android-sdk-emulator/)。 請參閱下方注意事項。  
...
## <a name="mac-setup-apple-id-xcode-and-xamarin"></a><a name="mac"></a> Mac 安裝 (Apple ID、Xcode 和 Xamarin)   
  
1. 如果您還沒有 Apple ID，請建立一個 [https://appleid.apple.com](https://appleid.apple.com/) 。 安裝及登入 Xcode 時需要這個 ID。  
  
2. 下載並安裝 Xcode  [https://developer.apple.com/xcode/](https://developer.apple.com/xcode/) ，並依照 [將您的帳戶新增至 Xcode](https://developer.apple.com/library/content/documentation/IDEs/Conceptual/AppStoreDistributionTutorial/AddingYourAccounttoXcode/AddingYourAccounttoXcode.html#//apple_ref/doc/uid/TP40013839-CH40-SW1) (apple.com) 所述，新增您的 Apple ID。  
  
3. 遵循 [安裝和設定 Xamarin.iOS](/xamarin/ios/get-started/installation/mac) (xamarin.com) 上的指示，下載並安裝 Xamarin。  
  
4. 當您在 Windows 和 Mac 電腦上完成安裝 Xamarin 之後，請遵循 [連接到 mac](/xamarin/ios/get-started/installation/windows/connecting-to-mac/) (xamarin.com) 的指示，以便在 windows 電腦上的 Visual Studio 中使用 IOS 和 Mac。  
