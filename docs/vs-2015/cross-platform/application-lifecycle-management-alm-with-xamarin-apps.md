...
|功能|支援 Xamarin|其他註解|  
|-------------|----------------------------|-------------------------|  
|內部部署 TFS 伺服器|是|組建電腦必須安裝 Xamarin，而且必須可連結到 OSX 電腦，以便為 iOS 進行建置。 請參閱設定 [適用于 xamarin 的 TFS](/azure/devops/repos/tfvc/overview?view=azure-devops) (xamarin 網站) |  
...
|程式碼涵蓋範圍|n/a||  
|[對程式碼進行單元測試](../test/unit-test-your-code.md)|是|對於 Windows 和 Android 的目標，可以使用內建的 MSTest 工具。 若要在 Windows、Android 和 iOS 上執行單元測試，Xamarin 會建議使用 NUnit。 請參閱 [為 Xamarin 設定 TFS](/azure/devops/repos/tfvc/overview?view=azure-devops) (Xamarin 網站)。|  
|[使用消費者介面自動化來測試您的程式碼](../test/use-ui-automation-to-test-your-code.md)|僅限 Windows|Visual Studio 的 UI 測試錄製器僅適用 Windows。 對於所有平台，請參閱 [Xamarin 測試錄製器 (英文)](https://www.xamarin.com/test-cloud/recorder)。|  
