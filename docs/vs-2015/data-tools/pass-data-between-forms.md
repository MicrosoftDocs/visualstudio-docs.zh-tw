...
ms.locfileid: "72652917"
---
# <a name="pass-data-between-forms"></a>在表單之間傳遞資料
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

此逐步解說提供將資料從某個表單傳遞至另一個表單的指示。 使用來自 Northwind 的 customers 和 orders 資料表，一種表單可讓使用者選取客戶，而第二個表單則會顯示所選客戶的訂單。 本逐步解說示範如何在第二個表單上建立方法，以接收第一個表單中的資料。

> [!NOTE]
> 此逐步解說只示範一種在表單之間傳遞資料的方法。 還有其他選項可用來將資料傳遞至表單，包括建立第二個用來接收資料的處理常式，或建立可使用第一個表單的資料來設定的公用屬性。

 這個逐步解說中所述的工作包括：

- 建立新的**Windows 應用程式**專案。

- 使用[資料來源設定向導](https://msdn.microsoft.com/library/c4df7de5-5da0-4064-940c-761dd6d9e28f)建立和設定資料集。
...
3. 選取 [ **Windows Forms 應用程式**]，然後按一下 **[確定]** 。 如需詳細資訊，請參閱[用戶端應用程式](https://msdn.microsoft.com/library/2dfb50b7-5af2-4e12-9bbb-c5ade0e39a68)。
...
## <a name="add-a-tableadapter-query-to-form2-to-load-orders-for-the-selected-customer-on-form1"></a>將 TableAdapter 查詢新增至 Form2，以在 Form1 上載入所選客戶的訂單

#### <a name="to-create-a-tableadapter-query"></a>建立 TableAdapter 查詢
...
7. 在 [**填入 DataTableMethod 名稱**] 中，輸入 `FillByCustomerID`。

8. 清除 [傳回 DataTable] 選項，然後按一下 [下一步]。

9. 按一下 [ **完成**]。

## <a name="create-a-method-on-form2-to-pass-data-to"></a>在 Form2 上建立方法以將資料傳遞至
