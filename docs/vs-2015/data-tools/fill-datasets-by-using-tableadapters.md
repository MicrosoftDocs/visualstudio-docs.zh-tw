---
title: 使用 Tableadapter 填滿資料集 |Microsoft Docs
ms.date: 11/15/2016
...
> [!NOTE]
> Tableadapter 由 Visual Studio 的設計師產生。 如果您要以程式設計方式建立資料集，請使用 DataAdapter，也就是 .NET Framework 類別。
...
## <a name="tableadapter-inheritance"></a>TableAdapter 繼承
 Tableadapter 藉由封裝已設定的類別來延伸標準資料配接器的功能 <xref:System.Data.Common.DataAdapter> ？ qualifyHint = False&autoUpgrade = True。 根據預設，TableAdapter 繼承自 <xref:System.ComponentModel.Component> 類別，無法轉換成 <xref:System.Data.Common.DataAdapter> 類別。 將 TableAdapter 轉換成 <xref:System.Data.Common.DataAdapter> 類別會導致 <xref:System.InvalidCastException> 錯誤？ QualifyHint = False&AutoUpgrade = True。 若要變更 TableAdapter 的基類，您可以在 <xref:System.ComponentModel.Component> **DataSet 設計工具**中，于 TableAdapter 的**基類**屬性中輸入衍生自的類別。

## <a name="tableadapter-methods-and-properties"></a>TableAdapter 方法和屬性
 TableAdapter 類別不是的一部分 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 。 這表示您無法在檔或 **物件瀏覽器**中查閱。 當您使用稍早所述的其中一個嚮導時，就會在設計階段建立它。 當您建立時，指派給 TableAdapter 的名稱是以您正在使用之資料表的名稱為基礎。 例如，當您根據名為的資料庫中的資料表建立 TableAdapter 時， `Orders` 會將 tableadapter 命名為 `OrdersTableAdapter` 。 您可以使用**DataSet 設計工具**中的**name**屬性來變更 TableAdapter 的類別名稱。

 以下是 Tableadapter 的常用方法和屬性：

|member|描述|
|------------|-----------------|
|`TableAdapter.Fill`|以 TableAdapter 的 SELECT 命令的結果填入 TableAdapter 的關聯資料表。|
...
## <a name="tableadapter-update-method"></a>TableAdapter update 方法
 Tableadapter 使用資料命令來讀取和寫入資料庫。 TableAdapter 的初始 `Fill` (主要) 查詢是用來做為建立關聯資料表之架構的基礎，以及 `InsertCommand` `UpdateCommand` `DeleteCommand` 與方法相關聯的、和命令 `TableAdapter.Update` 。 呼叫 TableAdapter 的 `Update` 方法會執行先前設定 tableadapter 時所建立的語句，而不是使用 **tableadapter 查詢設定 Wizard**加入的其他查詢之一。
...
> [!NOTE]
> 如果主要查詢中有足夠的資訊，則在 `InsertCommand` `UpdateCommand` `DeleteCommand` 產生 TableAdapter 時，預設會建立、和命令。 如果 TableAdapter 的主要查詢超過單一資料表 SELECT 語句，則設計師可能無法產生  `InsertCommand` 、 `UpdateCommand` 和 `DeleteCommand` 。 如果未產生這些命令，您可能會在執行方法時收到錯誤 `TableAdapter.Update` 。

## <a name="tableadapter-generatedbdirectmethods"></a>TableAdapter GenerateDbDirectMethods
 除了  `InsertCommand` 、和以外 `UpdateCommand` ，也會 `DeleteCommand` 使用可直接對資料庫執行的方法來建立 tableadapter。 您 `TableAdapter.Insert` 可以直接呼叫這些方法 (、 `TableAdapter.Update` 和 `TableAdapter.Delete`) ，以運算元據庫中的資料。 這表示您可以從程式碼呼叫這些個別的方法，而不是呼叫 `TableAdapter.Update` 來處理關聯資料表暫止的插入、更新和刪除。
...
## <a name="tableadapter-support-for-nullable-types"></a>可為 null 類型的 TableAdapter 支援
 Tableadapter 支援可為 null `Nullable(Of T)` 的類型和 `T?` 。 如需 Visual Basic 可為 Null 型別的詳細資訊，請參閱[可為 Null 的實值類型](https://msdn.microsoft.com/library/9ac3b602-6f96-4e6d-96f7-cd4e81c468a6)。 如需 c # 中可為 null 型別的詳細資訊，請參閱 [使用可為 null 的類型](https://msdn.microsoft.com/library/0bacbe72-ce15-4b14-83e1-9c14e6380c28)。

## <a name="security"></a>安全性
 當您使用資料命令，並將 `CommandType` 屬性設定為時 <xref:System.Data.CommandType> ，請仔細檢查從用戶端傳送的資訊，再將其傳遞至您的資料庫。 惡意使用者可能會嘗試傳送 (插入) 修改過或額外的 SQL 陳述式，以獲得未授權的存取或破壞資料庫。 將使用者輸入傳送至資料庫之前，請務必確認該資訊是否有效。 最佳做法是盡可能隨時使用參數化查詢或預存程式。
