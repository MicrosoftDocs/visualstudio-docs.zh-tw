...
依預設，當您將名稱結尾為或**伺服器總管**資料庫總管的資料庫物件拖曳 / **Database Explorer**至[Visual Studio 中的 LINQ to SQL 工具](../data-tools/linq-to-sql-tools-in-visual-studio2.md)時，產生的實體類別名稱會從複數變更為單數。 這是為了更正確地呈現具現化 (Instantiated) 的實體類別對應至單一筆記錄的情況。 例如，將名為 Customers 的資料表加入至 [!INCLUDE[vs_ordesigner_short](../includes/vs-ordesigner-short-md.md)]會產生實體類別 Customer，原因是這個類別只會保留單一客戶的資料。
...
2. 將 [**名稱複數表示**] 設定為 [**啟用**  =  **False** ]，以設定， [!INCLUDE[vs_ordesigner_short](../includes/vs-ordesigner-short-md.md)] 使它不會變更類別名稱。

3. 將 [**名稱的複數表示**]**設定為 [**  =  **True** ]，將複數表示規則套用至已加入至之物件的類別名稱 [!INCLUDE[vs_ordesigner_short](../includes/vs-ordesigner-short-md.md)] 。
