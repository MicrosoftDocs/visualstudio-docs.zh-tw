...
1.  開啟或建立新的 LINQ to SQL 類別檔案 (**.dbml**檔案) 中[!INCLUDE[vs_ordesigner_short](../includes/vs-ordesigner-short-md.md)]。 (按兩下 **.dbml**中的檔案**方案總管 中**。)  
...
4.  如果是 Visual Basic 專案：  
    ...
    1.  因為 C# 專案不會自動產生的事件處理常式，您可以使用 IntelliSense 建立部分`Update` *CLASSNAME*方法。  
  
    2.  輸入 `partial` 和一個空格，以存取可用部分方法的清單。 按一下要加入驗證的類別所適用的更新方法。 當您選取 產生程式碼的下列程式碼類似`Update` *CLASSNAME*部分方法：  
