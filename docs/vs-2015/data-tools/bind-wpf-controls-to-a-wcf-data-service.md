---
title: 將 WPF 控制項系結至 WCF 資料服務 |Microsoft Docs
ms.date: 11/15/2016
...
- 建立 [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)] ，以將實體資料模型中的資料公開給 WPF 應用程式。

- 從 [資料來源]**** 視窗將項目拖曳至 WPF 設計工具，以建立一組資料繫結控制項。

- 建立可向前及向後巡覽客戶記錄的按鈕。

- 建立按鈕，將控制項中的資料變更儲存至 [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)] 和基礎資料來源。
...
5. 在 [名稱]**** 方塊中輸入 `AdventureWorksService`，然後按一下 [確定]****。

     Visual Studio 建立 `AdventureWorksService` 專案。

6. 在 [方案總管]**** 中，以滑鼠右鍵按一下 **Default.aspx**，並選取 [刪除]****。 在此逐步解說中不需要此檔案。

## <a name="create-an-entity-data-model-for-the-service"></a>建立服務的實體資料模型
 若要使用 [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)]將資料公開給應用程式，您必須定義服務的資料模型。 [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)]支援兩種類型的資料模型：實體資料模型，以及使用 common language runtime 定義的自訂資料模型， (CLR) 執行介面的物件 <xref:System.Linq.IQueryable%601> 。 在此逐步解說中，您要建立資料模型的實體資料模型。
...
1. 在程式 `AdventureWorks.svc` 代碼檔案中，將類別宣告取代為下列程式 `AdventureWorksService` 代碼。
     ...
     這段程式碼 `AdventureWorksService` 會更新類別，使其衍生自在 <xref:System.Data.Services.DataService%601> `AdventureWorksLTEntities` 您實體資料模型中的物件內容類別上操作的。 該程式碼也會更新 `InitializeService` 方法，允許服務的用戶端對 `SalesOrderHeader` 實體具有完整的讀取/寫入存取權。
...
10. 在 [服務]**** 方塊中，按一下 **AdventureWorksService.svc**，然後按一下 [確定]****。
     ...
     Visual Studio 下載服務資訊，然後返回 [ **資料來源** 設定] wizard。
...
4. 從 [資料來源]**** 視窗將 [SalesOrderHeaders]**** 節點拖曳至包含按鈕之資料列下方的資料格列。
    ...
    Visual Studio 會產生 XAML 和程式碼，其可建立一組繫結至 [產品]**** 資料表之資料的控制項。 如需有關產生的 XAML 和程式碼的詳細資訊，請參閱將 [WPF 控制項系結至 Visual Studio 中的資料](../data-tools/bind-wpf-controls-to-data-in-visual-studio1.md)。
...
## <a name="saving-changes-to-sales-records"></a>儲存銷售記錄的變更
 加入程式碼，讓使用者可以使用 [ **儲存變更** ] 按鈕來查看和儲存銷售記錄的變更。
