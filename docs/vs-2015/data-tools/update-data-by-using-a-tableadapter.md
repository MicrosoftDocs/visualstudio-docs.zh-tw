...
ms.locfileid: "65692338"
---
# <a name="update-data-by-using-a-tableadapter"></a>使用 TableAdapter 更新資料
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

在資料集中的資料已修改且驗證之後，您可以將更新的資料傳送回 databaseby 呼叫`Update`TableAdapter 的方法。 `Update`方法會更新單一資料表，並執行正確的命令 （INSERT、 UPDATE 或 DELETE） 根據<xref:System.Data.DataRow.RowState%2A>的資料表中每個資料列。 當資料集有相關的資料表時，則 Visual Studio 會產生 TableAdapterManager 類別，您用來進行更新。 TableAdapterManager 類別可確保以正確的順序，根據資料庫所定義的外部索引鍵條件約束會進行更新。 當您使用資料繫結控制項時，資料繫結架構會建立呼叫 tableAdapterManager TableAdapterManager 類別成員變數。 如需詳細資訊，請參閱 <<c0> [ 階層式更新概觀](https://msdn.microsoft.com/library/c4f8e8b9-e4a5-4a02-8462-d03d1e8222d6)。  
...
2. 如果攔截到例外狀況時，找出造成錯誤的資料列。 如需詳細資訊，請參閱[如何：找出有錯誤的資料列](https://msdn.microsoft.com/library/1fa907c5-fe66-4f29-a253-2b97b900050c)。  
