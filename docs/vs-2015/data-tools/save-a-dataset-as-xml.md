...
您可以藉由呼叫資料集上可用的 XML 方法來存取資料集中的 XML 資料。 若要以 XML 格式儲存資料，您可以呼叫 <xref:System.Data.DataSet.GetXml%2A> 方法或的 <xref:System.Data.DataSet.WriteXml%2A> 方法 <xref:System.Data.DataSet> 。
...
- 方法會傳回 <xref:System.Data.DataSet.GetXml%2A> <xref:System.String> 。這表示您會宣告類型的變數 <xref:System.String> ，並為它指派方法的結果 <xref:System.Data.DataSet.GetXml%2A> 。
...
- <xref:System.Data.DataSet.WriteXml%2A>方法有數個多載。 下列程式碼顯示如何將資料儲存至檔案。宣告變數，並為它指派儲存檔案的有效路徑。
