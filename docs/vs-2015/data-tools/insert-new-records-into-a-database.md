---
title: 將新記錄插入資料庫中 |Microsoft Docs
ms.date: 11/15/2016
...
若要將新記錄插入資料庫，您可以使用 `TableAdapter.Update` 方法或其中一個 TableAdapter 的 DBDirect 方法（特別是 `TableAdapter.Insert` 方法）。
 ...
 如果您的 TableAdapter 沒有 `Insert` 方法，則表示 TableAdapter 已設定為使用預存程式，或其 `GenerateDBDirectMethods` 屬性設為 `false`。 嘗試將 TableAdapter 的 `GenerateDBDirectMethods` 屬性設定為從 DataSet 設計工具內 `true`，然後儲存資料集。 這會重新產生 TableAdapter。 如果 TableAdapter 仍然沒有 `Insert` 方法，則資料表可能未提供足夠的架構資訊來區別個別資料列（例如，資料表上可能沒有設定主鍵）。
...
1. 藉由建立新的 <xref:System.Data.DataRow>，並將其加入至 <xref:System.Data.DataTable.Rows%2A> 集合，將新記錄新增至所需的 <xref:System.Data.DataTable>。 如需詳細資訊，請參閱[如何：將資料列加入至 DataTable](https://msdn.microsoft.com/library/78ebbb43-c402-49cf-81da-0715289487bf)。
...
2. 將新的資料列加入至 <xref:System.Data.DataTable> 之後，請呼叫 `TableAdapter.Update` 方法。 您可以控制要更新的資料量，方法是傳入整個 <xref:System.Data.DataSet>、<xref:System.Data.DataTable>、<xref:System.Data.DataRow>s 陣列或單一 <xref:System.Data.DataRow>。
   ...
   下列程式會使用 Northwind 資料庫中的 `Region` 資料表做為範例。
...
## <a name="insert-new-records-by-using-command-objects"></a>使用命令物件插入新記錄
 下列範例會使用 command 物件，將新記錄直接插入資料庫中。
