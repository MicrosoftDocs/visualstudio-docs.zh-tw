...
Entity Framework 是一種物件關聯式對應技術，可讓.NET 開發人員使用網域特有物件來處理關聯式資料。 它不需要開發人員通常需要撰寫的大部分資料存取程式碼。 Entity Framework 是模型對於新的.NET 應用程式的技術建議的物件關聯式對應 (ORM)。  
 ...
 [!INCLUDE[adonet_edm](../includes/adonet-edm-md.md)] 工具的設計可協助您建置[!INCLUDE[adonet_ef](../includes/adonet-ef-md.md)]應用程式。 完整文件[!INCLUDE[adonet_edm](../includes/adonet-edm-md.md)]隆重推出工具： [Entity Framework](https://msdn.microsoft.com/data/jj590134)。  
...
|[使用者入門 （主控台、 WinForms、 WPF 等等） 的完整.NET](https://docs.efproject.net/en/latest/platforms/full-dotnet/getting-started.html)|提供有關如何建立使用 Entity Framework 7 的.NET 桌面應用程式的教學課程。|  
