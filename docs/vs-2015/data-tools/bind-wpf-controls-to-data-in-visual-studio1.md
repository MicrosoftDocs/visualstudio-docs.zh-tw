...
## <a name="generated-xaml-and-code"></a>產生的 XAML 和程式碼
 當您從 [**資料來源**] 視窗將專案拖曳至 [[!INCLUDE[wpfdesigner_current_short](../includes/wpfdesigner-current-short-md.md)]] 時，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 會產生 [!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)] 來定義新的資料繫結控制項（或將現有的控制項系結至資料來源）。 對於某些資料來源，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 也會在程式碼後置檔案中產生可將資料填入資料來源的程式碼。

 下表列出 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 為 [**資料來源**] 視窗中每個資料來源類型產生的 [!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)] 和程式碼。
...
### <a name="datasets"></a>資料集
 當您從 [**資料來源**] 視窗將資料表或資料行拖曳至設計工具時，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 會產生會執行下列動作的 [!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)]：
...
### <a name="entity-data-models"></a>實體資料模型
 當您從 [**資料來源**] 視窗將實體或實體屬性拖曳至設計工具時，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 會產生會執行下列動作的 [!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)]：
...
- 建立控制項的資料繫結。 如果您將項目拖曳至設計工具中的現有控制項，[!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)] 會將控制項繫結至項目。 如果您將專案拖曳至容器，[!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)] 會建立已針對拖曳專案選取的控制項，並將控制項系結至專案。 控制項是在新的 <xref:System.Windows.Controls.Grid> 內建立。
...
- 針對包含控制項的 <xref:System.Windows.FrameworkElement.Loaded> 項目，建立 [!INCLUDE[TLA2#tla_ui](../includes/tla2sharptla-ui-md.md)] 事件處理常式。 事件處理常式會呼叫取得*EntityName*查詢方法，將資料填入實體，從容器的資源中抓取 <xref:System.Windows.Data.CollectionViewSource>，然後讓第一個資料項目成為目前的專案。 如果 <xref:System.Windows.FrameworkElement.Loaded> 事件處理常式已經存在，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 就會將此程式碼加入至現有的事件處理常式。

### <a name="services"></a>服務
 當您從 [**資料來源**] 視窗將服務物件或屬性拖曳至設計工具時，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 會產生 [!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)]，以建立資料繫結控制項（或將現有控制項系結至物件或屬性）。 不過，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 不會產生可將資料填入 Proxy 服務物件的程式碼。 您必須自行撰寫此程式碼。 如需示範如何執行這種操作的範例，請參閱將[WPF 控制項系結至 WCF 資料服務](../data-tools/bind-wpf-controls-to-a-wcf-data-service.md)。
...
### <a name="objects"></a>物件
 當您從 [**資料來源**] 視窗將物件或屬性拖曳至設計工具時，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 會產生 [!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)]，以建立資料繫結控制項（或將現有控制項系結至物件或屬性）。 不過，[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 不會產生可將資料填入物件的程式碼。 您必須自行撰寫此程式碼。
...
> 自訂類別必須是公用的，而且根據預設，具有不含參數的函數。 它們不能是在其語法中具有 "dot" 的嵌套類別。 如需詳細資訊，請參閱[WPF 的 XAML 和自訂類別](https://msdn.microsoft.com/library/e7313137-581e-4a64-8453-d44e15a6164a)。
 ...
 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 會產生會執行下列動作的 [!INCLUDE[TLA#tla_titlexaml](../includes/tlasharptla-titlexaml-md.md)]：
