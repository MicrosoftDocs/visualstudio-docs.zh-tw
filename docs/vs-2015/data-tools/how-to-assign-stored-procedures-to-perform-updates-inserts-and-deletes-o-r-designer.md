...
預存程序 (Stored Procedure) 可以加入至 O/R 設計工具，而且可以當成一般 <xref:System.Data.Linq.DataContext> 方法來執行。 它們也可以用來覆寫預設的 [!INCLUDE[vbtecdlinq](../includes/vbtecdlinq-md.md)] 執行時間行為，當變更從實體類別儲存到資料庫時，會執行插入、更新和刪除（例如，呼叫 <xref:System.Data.Linq.DataContext.SubmitChanges%2A> 方法時）。
...
## <a name="configuring-the-update-behavior-of-an-entity-class"></a>設定實體類別的更新行為
 利用 [!INCLUDE[vbtecdlinq](../includes/vbtecdlinq-md.md)] 實體類別中的資料變更來更新資料庫 (插入、更新和刪除) 的邏輯，預設是由 [!INCLUDE[vbtecdlinq](../includes/vbtecdlinq-md.md)] 執行階段所提供。 執行時間會根據資料表的架構（資料行和主鍵資訊），建立預設的 Insert、Update 和 Delete 命令。 當不需要預設行為時，您可以藉由指派特定的預存程式來執行運算元據表中的資料所需的插入、更新和刪除，以設定更新行為。 未產生預設行為時 (例如，實體類別是對應至檢視時)，同樣可以這樣做。 最後，在資料庫需要透過預存程序進行資料表存取時，也可以覆寫預設更新行為。
...
2. 在 **伺服器總管**/**資料庫總管**中，展開 **預存程式**，並找出您要用於 entity 類別之 Insert、Update 和/或 Delete 命令的預存程式。
...
9. 檢查 [方法引數] 和 [類別屬性] 清單，以確認 [方法引數] 對應至適當的 [類別屬性]。 將原始方法引數（Original_*引數名稱*）對應至 [更新] 和 [刪除] 命令的原始屬性（*PropertyName* （原始））。
...
10. 按一下 [確定] 或 [套用]。
    ...
    > 完成每一項變更後按一下 [套用]，即可繼續設定每個類別/行為組合的行為。 如果您**在按一下 [** 套用] 之前變更了類別或行為，則會出現警告對話方塊，提供套用任何變更的機會。
     ...
     若要還原成使用預設執行時間邏輯來進行更新，請在 [**屬性**] 視窗中按一下 [插入]、[更新] 或 [刪除] 命令旁邊的省略號，然後選取 [**設定行為**] 對話方塊中的 [**使用運行**時間]。
