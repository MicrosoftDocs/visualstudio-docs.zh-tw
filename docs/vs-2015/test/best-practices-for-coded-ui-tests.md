...
- 將您的測試建立為一系列已錄製的方法。 如需如何錄製方法的詳細資訊，請參閱建立自動程式化 [UI 測試](../test/use-ui-automation-to-test-your-code.md#VerifyingCodeUsingCUITCreate)。
...
- 為您應用程式中受測試的每個模組建立個別的 [UIMap](/previous-versions/dd580454(v=vs.140)) 檔案。 如需詳細資訊，請參閱 [使用多個 UI 對應測試大型應用程式](../test/testing-a-large-application-with-multiple-ui-maps.md)。

- 當您建立 UI 控制項時，應在受測試的應用程式中使用有意義的名稱。 這可讓自動產生的控制項名稱有更多的意義和可用性。

- 如果您要使用 API 撰寫程式碼來建立判斷提示，請在檔案中 [UIMap](/previous-versions/dd580454(v=vs.140)) 類別的部分建立每個判斷提示的方法 `UIMap.cs` 。 從您的測試方法中呼叫此方法，以執行判斷提示。
...
- 如果控制項有不再有效的判斷提示：
  ...
  如需如何錄製自動程式碼 UI 測試的詳細資訊，請參閱 [使用消費者介面自動化測試您的程式碼](../test/use-ui-automation-to-test-your-code.md)。
