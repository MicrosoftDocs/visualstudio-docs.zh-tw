...
ms.locfileid: "72657282"
---
# <a name="unit-tests-for-generic-methods"></a>泛型方法的單元測試
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

您可以產生泛型方法的單元測試，就像您為其他方法所進行的測試一樣，如同[如何：建立及執行單元測試](https://msdn.microsoft.com/5e0f43cf-5e51-48e2-9c98-0eb9324bdc48)中所述。 下列章節提供建立泛型方法之單元測試的相關資訊與範例。
...
- 正在[編輯產生的測試程式碼](#EditingGeneratedTestCode)。 本範例包含「產生的測試程式碼」和「編輯的測試程式碼」兩節。 其示範如何將由泛型方法產生之未經處理的測試程式碼，編輯成有用的測試方法。

- [使用類型條件約束](#TypeConstraintNotSatisfied)。 本範例示範的單元測試適用於使用類型條件約束的泛型方法。 本範例中並未滿足類型條件約束。

### <a name="example-1-editing-generated-test-code"></a><a name="EditingGeneratedTestCode"></a> 範例1：編輯產生的測試程式碼
...
> [!NOTE]
> 每次 SizeOfLinkedListTest 測試執行時，都會呼叫其 TestHelper 方法兩次。 Assert 陳述式的計算結果每次都必須為 true，測試才會通過。 如果測試失敗，可能無法確定造成失敗的是指定 `<int>` 的呼叫，還是指定 `<char>` 的呼叫。 如果要找出答案，您可以檢查呼叫堆疊，也可以在測試方法中設定中斷點，然後在執行測試時同時進行偵錯。 如需詳細資訊，請參閱 [如何：在 ASP.NET 方案中執行測試時進行 Debug](https://msdn.microsoft.com/library/de4d7aa1-4a1e-467e-a19b-4a85ec245b8b)。

### <a name="example-2-using-a-type-constraint"></a><a name="TypeConstraintNotSatisfied"></a> 範例2：使用類型條件約束
...
## <a name="see-also"></a>另請參閱
 [測試程式碼](../test/unit-test-your-code.md)[的單元測試單元剖析](https://msdn.microsoft.com/a03d1ee7-9999-4e7c-85df-7d9073976144)
