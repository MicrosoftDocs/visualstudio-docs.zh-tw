...
4. 命名文字方塊控制項。
     ...
     ![為 textbox 控制項命名](../test/media/cuit-phone-nametesxtbox.png "CUIT_Phone_NameTesxtBox")
...
1. 您必須部署應用程式，自動程式碼 UI 測試才能對應應用程式的控制項。
     ...
     ![部署在模擬器上的應用程式](../test/media/cuit-phone-deployed.png "CUIT_Phone_Deployed")
...
1. 將新的自動程式碼 UI 測試專案加入使用 Windows Phone 應用程式的方案中。
    ...
    ![針對 Windows Phone 建立新的自動程式碼 UI 測試](../test/media/cuit-phone-newproject.png "CUIT_Phone_NewProject")
...
5. 若要將文字方塊控制項加入 UI 控制項對應，請重複上一個步驟。
    ...
    ![使用 cross&#45;線工具和地圖 textbox 控制項](../test/media/cuit-phone-maptextboxcontrol.png "CUIT_Phone_MapTextBoxControl")
...
8. 加入判斷提示。 測試將使用此判斷提示來驗證值是否正確。
    ...
    ![將判斷提示加入測試](../test/media/cuit-phone-addassertion.png "CUIT_Phone_AddAssertion")
...
1. 建置您的測試，然後使用測試總管執行測試。
     ...
     ![使用測試總管建立及執行測試](../test/media/cuit-phone-runtestexplorer.png "CUIT_Phone_RunTestExplorer")
     ...
     ![測試總管結果](../test/media/cuit-phone-runtestexplorerresults.png "CUIT_Phone_RunTestExplorerResults")

## <a name="use-data-driven-coded-ui-tests-on-windows-phone-apps"></a><a name="TestingPhoneAppsCodedUI_DataDriven"></a> 在 Windows Phone 應用程式上使用資料驅動的自動程式碼 UI 測試
...
### <a name="q-can-tests-be-executed-on-the-emulator-only-or-can-i-also-use-a-physical-device"></a><a name="TestingPhoneAppsCodedUI_EmulatorDevice"></a> 問：是否可以在模擬器上執行測試，也可以使用實體裝置？
