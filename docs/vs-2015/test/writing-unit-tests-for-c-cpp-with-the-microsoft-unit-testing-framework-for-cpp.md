...
3. 在測試專案中，透過下列方式使用 TEST 巨集和 Assert 類別建立測試類別和測試方法：
    ...
    - `LINE_INFO()` 是選用參數。 若沒有 PDB 檔案，它可以讓測試執行器識別失敗的位置。
...
## <a name="walkthrough-developing-an-unmanaged-dll-with-test-explorer"></a><a name="walkthrough"></a> 逐步解說：使用 Test Explorer 開發非受控 DLL
...
7. [檢查涵蓋範圍](https://msdn.microsoft.com/library/fc8hec9e.aspx)。 單元測試運用愈多程式碼，就愈有用。 您可以探索測試曾經使用過程式碼的哪些部分。
...
### <a name="create-a-native-unit-test-project"></a><a name="unitTestProject"></a> 建立原生單元測試專案
...
2. 在新的專案中，檢查 **unittest1.cpp**
     ...
     ![具有 TEST&#95;類別和測試&#95;方法的測試專案](../test/media/utecpp2.png "UteCpp2")
...
### <a name="create-an-unmanaged-dll-project"></a><a name="createDllProject"></a> 建立非受控 DLL 專案
...
2. 在 [Win32 應用程式精靈] 中選取 [DLL] **** 和 [匯出符號] **** 。
     ...
     ![針對 DLL 和匯出符號設定的 C&#43;&#43; 專案嚮導](../test/media/utecpp06.png "UteCpp06")
...
### <a name="couple-the-test-project-to-the-dll-project"></a><a name="coupleProjects"></a> 將測試專案結合至 DLL 專案
...
5. 在 [測試瀏覽器] 中，選擇 [ **全部執行**]。
    ...
    ![單元測試瀏覽器 &#45; 通過基本測試](../test/media/utecpp10.png "UteCpp10")
...
2. 建立方案，然後在 [Test Explorer] 中選擇 [ **全部執行**]。
...
4. 建立方案，然後在 [Test Explorer] 中選擇 [ **全部執行**]。
     ...
     ![已通過單元測試瀏覽器 &#45; 範圍測試](../test/media/utecpp12.png "UteCpp12")
...
## <a name="see-also"></a>另請參閱
 [使用 VisualStudio](../test/using-microsoft-visualstudio-testtools-cppunittestframework.md) [將單元測試新增至現有的 c + + 應用程式](../test/unit-testing-existing-cpp-applications-with-test-explorer.md)。 >microsoft.visualstudio.testtools.cppunittestframework[管理/非受控程式碼互通性](https://msdn.microsoft.com/library/ms973872.aspx)[調試](../debugger/debugging-native-code.md)程式程式碼[逐步解說：建立和使用動態連結程式庫 (c + +) ](https://msdn.microsoft.com/library/3ae94848-44e7-4955-bbad-7d40f493e941)匯[入和匯出](https://msdn.microsoft.com/library/7c44c2aa-2117-4cec-9615-a65bfd3f8f7b)
