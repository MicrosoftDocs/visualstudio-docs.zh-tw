...
Visual Studio 中的程式碼涵蓋範圍分析工具會收集原生組件和 Managed 組件 (.dll 或 .exe 檔) 的資料。 不過，在某些情況下，[程式碼涵蓋範圍結果] 視窗會顯示類似「產生空白的結果： ...」的錯誤。有幾個可能的原因可能會發生這種情況。 本主題有助於解決這些問題。
...
### <a name="no-tests-were-executed"></a>沒有執行過任何測試
 分析：檢查您的輸出視窗。 在 [顯示輸出來源]**** 下拉式清單中選擇 [測試]****。 檢查是否有記錄任何警告或錯誤。

 說明程式碼涵蓋範圍分析會在執行測試時進行。 這項分析只包括在執行測試時載入記憶體的組件。 如果沒有執行任何測試，程式碼涵蓋範圍就不會產生任何報告。

 在測試瀏覽器中解決問題，選擇 [ **全部執行** ] 以確認測試是否順利執行。 在使用 [分析程式碼涵蓋範圍]**** 之前修正所有錯誤。
...
### <a name="pdb-symbol-files-are-unavailable"></a>.pdb (符號) 檔案無法使用
 分析開啟編譯目的檔案夾 (通常會 bin\debug) ，並確認每個元件都有與 .dll 或 .exe 檔案相同的目錄中的 .pdb 檔。

 說明程式碼涵蓋範圍引擎會要求每個元件在測試回合期間都能夠存取其相關聯的 .pdb 檔案。 如果特定組件沒有 .pdb 檔，就不會分析該組件。

 產生 .pdb 檔的組建必須是產生 .dll 或 .exe 檔案的組建。

 解決方法：確定您的組建設定會產生 .pdb 檔。 如果建立專案時不會更新 .pdb 檔案，請開啟專案屬性，選取 [ **組建** ] 頁面，選擇 [ **Advanced** ]，然後檢查 [ **偵錯工具資訊**]。
...
### <a name="using-an-instrumented-or-optimized-binary"></a>使用已檢測或最佳化的二進位檔
 分析會判斷二進位是否經過任何形式的 advanced 優化，例如特性指引優化，或已由分析工具（例如 vsinstr.exe 或 vsperfmon.exe）進行檢測。

 說明如果元件已經過其他程式碼剖析工具的檢測或優化，程式碼涵蓋範圍分析會省略該元件。
...
### <a name="code-is-not-managed-net-or-native-c-code"></a>程式碼不是 Managed (.NET) 或機器碼 (C++)
 分析確認您正在對 managed 或 c + + 程式碼執行某些測試。

 說明 Visual Studio 中的程式碼涵蓋範圍分析僅適用于 managed 和原生 (c + +) 程式碼。 如果您使用協力廠商工具，部分或所有程式碼可能會在不同平台上執行。
...
### <a name="assembly-has-been-installed-by-ngen"></a>NGen 已安裝組件
 ...
 解決方法是使用元件的 MSIL 版本。 不可用 NGen 處理。

### <a name="custom-runsettings-file-with-bad-syntax"></a>語法不正確的自訂 .runsettings 檔案
 分析如果您使用自訂的 .runsettings 檔案，可能會包含語法錯誤。

 這會導致程式碼涵蓋範圍完全無法執行。 程式碼涵蓋範圍視窗在測試回合結束時未開啟，或是顯示舊的結果。

 說明您可以使用自訂的 .runsettings 檔案來執行單元測試，以設定程式碼涵蓋範圍選項。 這些選項可讓您包含或排除檔案。 如需詳細資訊，請參閱[自訂程式碼涵蓋範圍分析](../test/customizing-code-coverage-analysis.md)。
...
### <a name="custom-runsettings-file-with-incorrect-exclusions"></a>自訂 .runsettings 檔包含不正確的排除項
 ...
 解決方法會移除 `Include` .runsettings 檔案中的所有節點，然後移除所有 `Exclude` 節點。 如果這樣可以解決問題，請將它們放回階段。
...
### <a name="initialization-code-in-native-dlls-is-executed-before-instrumentation"></a>原生 DLL 中的初始化程式碼會在檢測之前執行
 靜態連結機器碼中的分析（初始化函式 **DllMain** 的一部分，以及它所呼叫的程式碼）有時候會顯示為未涵蓋，即使程式碼已執行也是一樣。

 說明程式碼涵蓋範圍工具的運作方式，就是在應用程式開始執行之前，將檢測插入元件中。 在這之前載入的任何組件中，**DllMain** 中的初始化程式碼會於組件載入，以及應用程式執行之前時執行。 該程式碼會顯示為未涵蓋。
