...
3. 以滑鼠右鍵按一下套件或模型，然後按一下 [屬性]。

4. 在 [屬性] 視窗中，將 [設定檔] 屬性設為包含您想要使用之造型的設定檔。
...
1. 以滑鼠右鍵按一下圖表或 [UML 模型總管] 中的模型項目或關聯性，然後按一下 [屬性]。
...
2. 按一下 [造型] 屬性，並選取您想要套用的造型。
    ...
    > [!NOTE]
    > 如果找不到 [造型] 屬性，或您想要的造型未出現，請驗證模型項目位於已經連結適當設定檔的套件或模型內。

3. 有些造型可讓您設定模型項目其他屬性的值。 若要查看這些屬性，請展開 [造型] 屬性。

### <a name="to-create-model-elements-within-a-package"></a>在套件內建立模型項目

1. 在 UML 類別圖或 [UML 模型總管] 中建立套件。
...
2. 使用下列其中一種方式，將模型項目加入套件：
    ...
    - 在 [UML 模型總管] 中，以滑鼠右鍵按一下套件，並指向 [加入]，然後按一下項目類型。
    ...
    - 將圖表連結至套件，然後在圖表內建立項目。
         ...
         若要這麼做，請以滑鼠右鍵按一下圖表的空白部分，然後按一下 [屬性]。 在 [屬性] 視窗中，將 [連結的套件] 設定為您想要的套件。
