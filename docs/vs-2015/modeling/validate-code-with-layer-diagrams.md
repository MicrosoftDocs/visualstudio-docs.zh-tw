...
- 包含具有分層圖的模型專案的方案。 這張分層圖必須與您想要驗證的 Visual C# .NET 或 Visual Basic .NET 的成品連結。 請參閱[從您的程式碼建立分層圖](../modeling/create-layer-diagrams-from-your-code.md)。  
  ...
  您可以在 Visual Studio 中的開放分層圖或從命令提示字元手動驗證程式碼。 您也可以在執行本機組建或 Team Foundation Build 時自動驗證程式碼。 請[參閱 Channel 9 影片:使用分層圖](http://go.microsoft.com/fwlink/?LinkID=252073)設計和驗證您的架構。  
...
2. 選擇下列其中一項：  
   ...
   - 若要根據方案中的所有模型專案來驗證程式碼，請使用下列自訂屬性執行 [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)]：  
     ...
     發生的任何錯誤都將列出。 如需的詳細[!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)]資訊, 請參閱[msbuild](../msbuild/msbuild.md)和[msbuild](../msbuild/msbuild-task.md)工作。  
...
|**錯誤語法**|**錯誤描述**|  
|----------------------|---------------------------|  
|AV0001不正確相依性:*Artifact1*(*ArtifactType1*) --> *Artifact2*(*ArtifactType2*)<br /><br /> 分層*LayerName1*,*和 layername2* &#124;相依性:*DependencyType*|*LayerName1*中的*Artifact1*不應該相依于*和 layername2*中的*Artifact2* , 因為*LayerName1*與*和 layername2*沒有直接相依性。|  
|AV1001不正確命名空間:*專案*<br /><br /> 層*LayerName*&#124;必要的命名空間:*NamespaceName1*&#124;目前的命名空間:*NamespaceName2*|*LayerName*要求其相關成品必須屬於*NamespaceName1*。 成品是在*NamespaceName2*中, 而非*NamespaceName1*。|  
|AV1002取決於禁止的命名空間:*Artifact1*(*ArtifactType1*) &#124; *Artifact2*(*ArtifactType2*)<br /><br /> 層*LayerName*&#124;禁止的命名空間:*NamespaceName*&#124;相依性:*DependencyType*|*LayerName*要求其相關成品不得相依于*NamespaceName*。 *Artifact1*無法相依于*Artifact2* , 因為*Artifact2*在*NamespaceName*中。|  
|AV1003在禁止的命名空間中:*Artifact*(*ArtifactType*)<br /><br /> 層*LayerName*&#124;禁止的命名空間:*NamespaceName*|*LayerName*要求其相關成品不能屬於*NamespaceName*。 成品屬於*NamespaceName*。|  
|AV3001遺失連結:圖層 '*LayerName*' 連結至 ' 成品 ', 但找不到該*專案*。 您是否遺漏了組件參考?|*LayerName*找不到之成品的連結。 例如，類別的連結可能因為模型專案遺漏包含該類別之組件的參考而遺失。|  
