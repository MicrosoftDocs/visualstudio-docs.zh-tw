...
|-|-|-|
|**多邊形**|**目**|**描述和主要屬性**|
|1|**動作**|活動的一個步驟，使用者或軟體在此會執行某些工作。<br /><br /> 此動作會在語彙基元抵達其所有流入流程時啟動。 結束時，語彙基元會傳送到所有的流出流程。<br /><br /> -   **主體**-指定動作的詳細資料。<br />-   **語言**-主體中的運算式語言。<br />-   **本機後置條件**-執行結束時必須滿足的條件約束。 藉由動作所達成的目標。<br />-   **本機前置條件**-開始執行之前必須滿足的條件約束。|
...
|8|**註解**|提供所連結項目的其他資訊。|
|9|**呼叫行為動作**|更詳細定義在另一個活動圖上的動作。<br /><br /> -   **IsSynchronous** -如果為 true，則動作會等到活動終止為止。<br />-   **行為**-叫用活動。|
|(未顯示)|**呼叫操作動作**|呼叫類別執行個體上作業的動作。|
||**活動**|活動圖表描述的工作流程。 若要查看活動的屬性，您必須在 [ **UML 模型瀏覽器**] 中選取它。<br /><br /> -   **為唯讀**-如果為 true，則活動不應該變更任何物件的狀態。<br />-   **是單次執行**-如果為 true，則此圖表最多一次執行。|
...
### <a name="concurrent-flows"></a>並行流程
 ...
 ![顯示並行流程的活動圖](../modeling/media/uml-actovconcurrent.png "UML_ActovConcurrent")

||||
|-|-|-|
|**多邊形**|**目**|**說明**|
|11|**分叉節點**|將單一流程分割成並行流程。 每個傳入的語彙基元都會在每個傳出的連接器上產生語彙基元。|
|12|**加入節點**|將並行流程結合成單一流程。 當每個流入流程都有語彙基元在等候時，輸出上就會產生語彙基元。|
|13|**傳送信號動作**|這個動作會將訊息或信號傳送到另一個活動，或傳送到相同活動中的並行執行緒。 訊息的類型和內容是由動作標題暗示，或在其他註解中指定。<br /><br /> 動作可以在信號中傳送資料，這些資料可以傳遞給物件流程或輸入連接 (16) 中的動作。|
...
|-|-|-|
|**多邊形**|**目**|**說明**|
|15|**物件節點**|表示順著流程傳遞的資料。<br /><br /> -   **排序**-儲存多個權杖的方式。<br />-   **選項**-叫用可在另一個圖表中定義的處理常式，以篩選資料。<br />-   **上限**-0 表示資料必須直接沿著流程傳遞; \* 表示資料可以儲存在流程中。<br />-   **類型**-儲存和傳輸的物件類型。|
|16|**輸入釘選**|表示動作執行時可以接收的資料。<br /><br /> -   **類型**-傳輸的物件類型。|
|17|**輸出釘選**|表示動作執行時所產生的資料。<br /><br /> -   **類型**-傳輸的物件類型。|
|18|**活動參數節點**|物件節點，資料可以透過它被活動接收或產生。<br /><br /> 使用時機為從另一個活動呼叫圖表所表示的活動，或圖表說明作業或函式時。<br /><br /> -   **類型**-傳輸的物件類型。|
|(未顯示)|**物件流程**|顯示動作與物件節點間資料流程的連接器。<br /><br /> 若要建立物件流程，請使用 [**連接器**] 工具，將輸入或輸出連接或物件節點連結至另一個元素。<br /><br /> -   **選項**-叫用可在另一個圖表中定義的處理常式，以篩選資料。<br />-   **轉換**-叫用可在另一個圖表中定義的處理常式，以轉換資料。<br />-   **IsMulticast** -表示可能有數個收件者物件或元件。<br />-   **IsMultiReceive** -表示可能會從數個物件或元件接收輸入。|
