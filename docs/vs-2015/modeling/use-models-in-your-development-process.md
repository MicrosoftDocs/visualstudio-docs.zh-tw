...
|設計層級|圖表類型|  
|------------------|-------------------|  
|商務程序<br /><br /> 了解使用系統的內容可協助您了解使用者的需求。|活動圖會描述達成商務目標的 人員與系統之間的工作流程。<br />-概念性類別圖會描述商務程序內使用的商務概念。|  
|使用者需求<br /><br /> 使用者對於系統需求的定義。|使用案例圖摘要說明使用者和其他外部系統具有您所開發系統的互動。 您可以將其他文件附加至每個使用案例來詳細描述。<br />-UML 類別圖會描述使用者和系統溝通有關的資訊的類型。<br />個別的文件可以描述商務規則和服務需求品質。|  
|高階設計<br /><br /> 系統的整體結構：主要元件以及它們如何結合在一起。|階層圖會描述系統如何結構化為彼此相依的組成部分。 您可以根據分層圖驗證程式碼以確保程式碼符合此架構。<br />元件圖會顯示部分，指定的訊息與服務提供和每個元件所需的介面。<br />-循序圖會顯示此元件如何通訊以實作每個使用案例。<br />-UML 類別圖會描述的元件和元件之間所傳遞的資料類型的介面。|  
|設計模式<br /><br /> 設計問題的解決慣例和方法，這些會在此設計的所有部分中使用。|-UML 類別圖會描述模式的結構<br />-序列或活動圖會顯示此互動和演算法|  
|程式碼分析<br /><br /> 您可以從此程式碼產生許多圖表類型。|-循序圖會顯示在程式碼中的物件之間的互動。<br />階層圖會顯示類別之間的相依性。 您可以針對分層圖驗證更新的程式碼。<br />類別圖表會顯示在程式碼中的類別。|  
