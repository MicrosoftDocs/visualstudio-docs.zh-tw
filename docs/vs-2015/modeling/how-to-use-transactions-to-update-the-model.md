...
交易可確保對存放區所做的變更會視為群組。 您可以使用單一單位來認可或回復已分組的變更。
 ...
 每當您的程式碼修改、新增或刪除視覺效果和模型 SDK 中儲存區中的任何元素時 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ，它必須在交易內進行。 <xref:Microsoft.VisualStudio.Modeling.Transaction>發生變更時，必須有與存放區相關聯的使用中實例。 這適用于所有模型專案、關聯性、圖形、圖表和其屬性。
