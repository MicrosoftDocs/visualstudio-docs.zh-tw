...
## <a name="BasicSteps"></a>繪製順序圖表的基本步驟
 如需順序圖表上專案的完整清單，請參閱[UML 順序圖表：參考](../modeling/uml-sequence-diagrams-reference.md)。
...
2. 在 [**範本**] 下，按一下 [ **UML 順序圖表**]。

3. 命名圖表。

4. 在 [**加入至模型專案**] 中，選取方案中的現有模型專案，或**建立新的模型專案**，然後按一下 **[確定]** 。

    新的順序圖表隨即出現，並顯示 [**順序圖表**] 工具箱。 工具箱包含需要的項目和接頭。
...
1. 將 [**生命線**] （1）從 [**工具箱**] 拖曳到圖表上，以代表類別、元件、動作專案或裝置的實例。
    ...
    > [!NOTE]
    > 您也可以從 [ **UML 模型瀏覽器**] 將現有的類別、介面、動作專案或元件拖曳到圖表上，藉以建立生命線。 這會建立生命線，以代表所選擇類型的執行個體。
...
- 按一下訊息，並使用**向上鍵**和**向下**鍵來調整訊息位置。 使用**shift + 向上鍵**和**shift + 向下鍵**來變更訊息的順序。

#### <a name="to-move-or-copy-message-sequences-on-the-sequence-diagram"></a>移動或複製循序圖上的訊息順序

1. 以滑鼠右鍵按一下訊息（3，4），然後按一下 [**複製**]。

2. 以滑鼠右鍵按一下要從中傳送新訊息的執行出現次數（5）或生命線（1），然後按一下 [**貼**上]。 如果想要的話，新的寄件者可以在不同的圖表上。
...
- 目標生命線必須繫結或對應至類型，才會顯示簽章文字。 若要完成這項作業，請執行下列其中一個步驟：
  ...
  - 選取生命線，按**F4**，然後在 [**屬性**] 視窗中，將 [**類型**] 屬性設定為現有類型，或指定新類型的名稱。 以滑鼠右鍵按一下 [訊息] 標籤，然後選擇 [**建立**作業]。
...
- 以滑鼠右鍵按一下圖表的空白部分，然後按一下 [**重新排列版面**配置]。

- 若要復原操作，請按一下 [**編輯**]，然後按一下 [**復原**]。

#### <a name="to-change-the-package-that-owns-the-interaction"></a>變更擁有互動的套件

1. 在 [ **UML 模型瀏覽器**] 中，尋找順序圖表所顯示的互動。

    > [!NOTE]
    > 在您將第一個生命線新增至順序圖表之前，互動不會出現在 [ **UML 模型瀏覽器**] 中。

2. 將互動拖曳至套件。

     \-或-

     以滑鼠右鍵按一下 [互動]，然後按一下 [**剪**下]。 在封裝上按一下滑鼠右鍵，然後按一下 [**貼**上]。
...
- 使用 [**建立**] 工具來描述寄件者建立接收者的互動（8）。

     建立訊息應該是接收者收到的第一個訊息。

### <a name="annotating-the-interactions"></a>加入互動的附註
 若要描述序列的更多詳細資料，您可以將**批註**放在圖表的任何位置。
...
- 請注意例外狀況或可能會執行為所顯示一般序列的替代方案。 例如，"Customer can choose to skip the rest of this sequence"。
  ...
  - 請考慮使用片段做為這類附註的更正式替代方案。 請參閱[使用片段描述控制結構](#Fragments)
...
- 使用[描述具有片段的控制結構](#Fragments)來顯示迴圈、替代專案等等。
...
- 將類別、元件或介面從 [UML 模型總管] 拖曳至循序圖。
  ...
  1. 以滑鼠右鍵按一下其個別圖表上的 [類別]、[元件] 或 [介面]，然後按一下 [**建立生命線**]。
...
2. 在生命線所收到之每個訊息的類別或介面中建立作業：
    ...
    2. 以滑鼠右鍵按一下其中一個訊息，然後按一下 [**建立方法**]。
         ...
         作業名稱會出現在每個訊息箭號底下，以及訊息的**operation**屬性中。
...
2. 以滑鼠右鍵按一下其中一個訊息，指向 [**環繞**]，然後按一下您需要的片段類型。

     新的片段隨即出現。 它會包含您所選取的訊息。

     如果合併片段類型允許多個片段，則也會出現空的片段。

3. 若要設定片段的防護，請以滑鼠右鍵按一下 [片段框線]，然後按一下 [**屬性**]。 設定**Guard**屬性。

     成立條件用來定義分支或迴圈的條件。

4. 若要將新的片段加入允許多個片段的類型，請以滑鼠右鍵按一下片段的界限，然後指向 [**新增**]。 按一下 [ **Before 互動運算元**] 或 [ **After 互動運算元**]。
