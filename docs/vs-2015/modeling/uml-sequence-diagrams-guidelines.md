---
title: UML 順序圖表：方針 |Microsoft Docs
ms.date: 11/15/2016
...
在 Visual Studio 中，您可以繪製 *「循序圖」* (sequence diagram) 顯示互動。 互動是類別、元件、子系統或行動之一般執行個體間的一連串訊息。

 UML 循序圖是 UML 模型的一部分，而且只存在於 UML 模型專案內。 若要建立 UML 循序圖，請按一下 [架構] 功能表上的 [新增 UML 或分層圖]。 深入了解 [UML 循序圖項目](../modeling/uml-sequence-diagrams-reference.md)或 [UML 模型圖](../modeling/edit-uml-models-and-diagrams.md)的一般資訊。 如需影片示範，請參閱[使用順序圖表的草繪互動（2010）](https://channel9.msdn.com/Blogs/clinted/UML-with-VS-2010-Part-7-Sketching-Interactions-with-Sequence-Diagrams)。
...
## <a name="in-this-topic"></a>本主題內容
 ...
 [繪製順序圖表的基本步驟](#BasicSteps)

 [建立和使用簡單的順序圖表](#Simple)

 [類別和生命線](#ClassesAndLifelines)

 [建立可重複使用的互動順序](#Multiple)

 [折迭生命線群組](#Collapse)

 [描述具有片段的控制結構](#Fragments)

## <a name="Using"></a>使用 UML 順序圖表
 您可以將循序圖用於不同程式詳細程度的各種用途。 繪製循序圖的一般情況如下：

- 如果您的使用案例圖彙總系統使用者和其目標，則可以繪製循序圖來描述系統的主要元件如何互動以滿足每個使用案例的目標。 如需詳細資訊，請參閱 [UML 使用案例圖表：方針](../modeling/uml-use-case-diagrams-guidelines.md)。
...
#### <a name="lifelines-and-types"></a>生命線和類型
 您在循序圖中繪製的生命線可以代表系統中元件或類別的一般執行個體。 您可以從類型建立生命線、從生命線建立類型，並在 UML 類別圖和 UML 元件圖上顯示類型。 如需詳細資訊，請參閱[類型和生命線](#ClassesAndLifelines)。

#### <a name="parameter-types"></a>參數型別
 您也可以在 UML 類別圖中描述參數類型以及生命線之間傳送之訊息中所使用的傳回值。

#### <a name="use-case-details"></a>使用案例詳細資料
 使用案例代表使用者目標，以及達成目標的一連串步驟。 您可以使用數種方式來描述這一連串的步驟。 其中一個選項是繪製循序圖，以顯示使用者與系統主要元件之間的互動。 如需詳細資訊，請參閱 [UML 使用案例圖表：方針](../modeling/uml-use-case-diagrams-guidelines.md)。

## <a name="BasicSteps"></a>繪製順序圖表的基本步驟
 如需循序圖上元素的完整清單，請參閱 [UML 循序圖：參考](../modeling/uml-sequence-diagrams-reference.md)。
...
4. 在 [加入至模型專案] 中，選取方案中的現有模型專案，或按一下 [建立新模型專案]，然後按一下 [確定]。
    ...
    新的循序圖隨即出現，其中包含 [循序圖] 工具箱。 工具箱包含需要的項目和接頭。

   ![順序圖表的元件](../modeling/media/uml-sequence.png "UML_Sequence")

#### <a name="to-draw-a-sequence-diagram"></a>繪製循序圖

1. 將 [生命線] (1) 從 [工具箱] 拖曳至圖表，來代表類別、元件、行動或裝置的執行個體。

    > [!NOTE]
    > 將現有類別、介面、行動或元件從 [UML 模型總管] 拖曳至圖表，也可以建立生命線。 這會建立生命線，以代表所選擇類型的執行個體。
...
3. 若要顯示來自未知事件來源 (9) 或廣播給未知收件者 (10) 的訊息，請繪製圖表上傳送至空白空間或從中接收的非同步訊息。 這些訊息稱為*找到的訊息*（9）和*遺失的訊息*（10）。
    ...
    > [!NOTE]
    > 若要移動具有遺失或尋得訊息的生命線群組，請遵循以下步驟來選取生命線，然後移動它們：在這些生命線周圍繪製距形，或在按一下每個生命線時按住 **CTRL** 鍵。 如果您使用 [**全選**] 或**CTRL**+**A**來選取所有生命線，然後移動它們，則附加至這些生命線的任何遺失或找到的訊息都不會移動。 如果發生此案例，則您可以分別移動這些訊息。
...
- 按一下訊息，然後使用「向上鍵」和「向下鍵」，以調整訊息位置。 使用「SHIFT+向上鍵」和「SHIFT+向下鍵」變更訊息順序。

#### <a name="to-move-or-copy-message-sequences-on-the-sequence-diagram"></a>移動或複製循序圖上的訊息順序

1. 以滑鼠右鍵按一下訊息 (3、4)，然後按一下 [複製]。

2. 以滑鼠右鍵按一下想要從中傳送新訊息的執行出現次數 (5) 生命線 (1)，然後按一下 [貼上]。 如果想要的話，新的寄件者可以在不同的圖表上。
...
- 目標生命線必須繫結或對應至類型，才會顯示簽章文字。 若要完成這項作業，請執行下列其中一個步驟：
  ...
  - 選取生命線，並按 **F4**，然後在 [屬性] 視窗中將 [類型] 屬性設定為現有類型，或指定新類型的名稱。 以滑鼠右鍵按一下訊息標籤，然後選擇 [建立作業]。

    簽章文字會出現在訊息標籤下方。 現在，您可以編輯簽章文字。 如需詳細資訊，請參閱[類型和生命線](#ClassesAndLifelines)。

#### <a name="to-improve-the-layout-of-a-sequence-diagram"></a>改善循序圖的版面配置

- 以滑鼠右鍵按一下圖表的空白部分，然後按一下 [重新整理版面配置]。

- 若要復原作業，請按一下 [編輯]，然後按一下 [復原]。

#### <a name="to-change-the-package-that-owns-the-interaction"></a>變更擁有互動的套件

1. 在 [UML 模型總管] 中，尋找循序圖所顯示的互動。

    > [!NOTE]
    > 除非您在循序圖中加入第一個生命線，否則互動不會出現在 [UML 模型總管] 中。

2. 將互動拖曳至套件。

     \-或-

     以滑鼠右鍵按一下互動，然後按一下 [剪下]。 以滑鼠右鍵按一下套件，再按一下 [貼上]。

## <a name="Simple"></a>建立和使用簡單的順序圖表
...
- 使用 [同步] 工具描述傳送者等待接收者傳回回應 (3) 的互動。

     **<\<傳回 > >** 箭號將會顯示在執行發生的結尾。 它指出將控制權傳回給寄件者。

- 使用 [同步] 工具描述傳送者在其中等待接收者傳回回應 (4) 的互動。

- 使用 [建立] 工具描述傳送者在其中建立接收者 (8) 的互動。

     建立訊息應該是接收者收到的第一個訊息。

### <a name="annotating-the-interactions"></a>加入互動的附註
 若要描述序列的更多詳細資料，您可以將 [註解] 放在圖表的任何位置。
...
- 使用[描述含片段的控制結構](#Fragments)來顯示迴圈、替代項目等。
...
- 將類別、元件或介面從 [UML 模型總管] 拖曳至循序圖。
  ...
  1. 以滑鼠右鍵按一下類別、元件或其個別圖表上的介面，然後按一下 [建立生命線]。

  2. 在 [建立生命線] 對話方塊中，選取循序圖，然後按一下 [確定]。
...
2. 在生命線所收到之每個訊息的類別或介面中建立作業：
    ...
    2. 以滑鼠右鍵按一下其中一個訊息，然後按一下 [建立方法]。

         新的類別或介面都有每個所選取訊息的作業。

         作業名稱會出現在每個訊息箭號的下方，以及訊息的 [作業] 屬性中。
...
3. 若要詳細檢視新的類別或介面，請將它加入類別圖或元件圖。
    ...
    2. 將新的類別或介面從 [UML 模型總管] 拖曳至類別圖。

         類別或介面會出現在類別圖中。

         \-或-

    3. 將新的介面從 [UML 模型總管] 拖曳至元件圖中的元件或連接埠。
...
## <a name="Multiple"></a>建立可重複使用的互動順序
...
### <a name="creating-a-placeholder-with-an-interaction-use"></a>使用互動使用建立預留位置
 您可以建立互動使用，而不需要將它連結至另一個圖表。 您可以使用此做為序列中某個部分的預留位置，其詳細資料尚未被處理。使用互動使用的名稱來指出您想要的結果。

## <a name="Collapse"></a>折迭生命線群組
...
## <a name="Fragments"></a>描述具有片段的控制結構
 您可以在循序圖中使用合併片段 (13) 來定義迴圈、分支和並行處理。 或者，請改成考慮使用活動圖。 活動圖不適用於在行動之間顯示訊息，但在某些情況下適用於顯示迴圈、分支和並行。

 如需片段類型的完整清單，請參閱[使用 UML 順序圖表上的片段描述控制流程](../modeling/describe-control-flow-with-fragments-on-uml-sequence-diagrams.md)。
...
3. 若要設定片段的成立條件，請以滑鼠右鍵按一下邊框，然後按一下 [屬性]。 設定 [成立條件] 屬性。

     成立條件用來定義分支或迴圈的條件。

4. 若要將新的片段加入允許多個片段的種類，請以滑鼠右鍵按一下片段界限，並指向 [加入]。 按一下 [ **Before 互動運算元**] 或 [ **After 互動運算元**]。
