---
title: 建立以 Windows Forms 為基礎的網域特定語言 |Microsoft Docs
ms.date: 11/15/2016
...
您可以使用 Windows Forms 來顯示特定領域語言（DSL）模型的狀態，而不是使用 DSL 圖表。 本主題將逐步引導您使用 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的視覺效果和模型化 SDK，將 Windows Form 系結至 DSL。

 ![DSL&#45;Wpf&#45;2](../modeling/media/dsl-wpf-2.png "DSL-Wpf-2")DSL 實例，顯示 Windows Form UI 和模型瀏覽器。

## <a name="creating-a-windows-forms-dsl"></a>建立 Windows Forms DSL
 **最小的 WinForm 設計**工具 dsl 範本會建立最小的 dsl，讓您可以修改以符合您自己的需求。

#### <a name="to-create-a-minimal-winforms-dsl"></a>建立最小 WinForms DSL
...
2. 試驗範本提供的初始範例：
   ...
   在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的主要實例中，請注意下列關於 DSL 解決方案的要點：
...
- 它可確保當使用者選取資料列時，屬性視窗會顯示對應之模型專案的屬性，而不是資料格資料列。
  ...
  ![DslWpf4](../modeling/media/dslwpf4.png "DslWpf4")資料來源與視圖之間的連結架構。
...
#### <a name="to-make-the-properties-update-immediately"></a>讓屬性立即更新
