...
ms.locfileid: "72659314"
---
# <a name="walkthrough-creating-a-custom-directive-processor"></a>逐步解說：建立自訂指示詞處理器
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

指示詞處理器 * 將程式碼加入至 *產生的轉換類別*以進行工作。 如果您從*文字模板*呼叫指示詞，您在文字*範本中撰寫*的其餘程式碼可以依賴指示詞提供的功能。
...
## <a name="creating-a-custom-directive-processor"></a>建立自訂指示詞處理器
 ...
 自訂指示詞處理器會將變數和屬性加入至產生的轉換類別中。 您撰寫的指示詞會使用 <xref:System.CodeDom> 類別來建立引擎要加入至產生之轉換類別中的程式碼。 <xref:System.CodeDom> 類別會根據 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 指示詞之 `language` 參數中所指定的語言，使用 Visual C# 或 `template` 來建立程式碼。 指示詞處理器的語言與存取指示詞處理器之文字範本的語言不一定要相符。
...
1. 在 Visual Studio 中建立 C# 或 Visual Basic 類別庫專案，並命名為 CustomDP。
    ...
    > [!NOTE]
    > 如果您想要在多台電腦上安裝指示詞處理器，最好是使用 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 擴充功能 (VSIX) 專案，並在擴充功能中包含 .pkgdef 檔。 如需詳細資訊，請參閱 [部署自訂](../modeling/deploying-a-custom-directive-processor.md)指示詞處理器。
...
4. 若 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 只是，請開啟 [ **專案** ] 功能表，然後按一下 [ **CustomDP 屬性**]。 在 [ **應用程式** ] 索引標籤的 [ **根命名空間**] 中，刪除預設值 `CustomDP` 。
...
## <a name="registering-the-directive-processor"></a>註冊指示詞處理器
 您必須先加入指示詞處理器的登錄機碼，才能從中的文字模板呼叫指示詞 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 。

> [!NOTE]
> 如果您想要在多台電腦上安裝指示詞處理器，最好是定義 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 擴充功能 (VSIX)，在其中包含 .pkgdef 檔和組件。 如需詳細資訊，請參閱 [部署自訂](../modeling/deploying-a-custom-directive-processor.md)指示詞處理器。
...
## <a name="testing-the-directive-processor"></a>測試指示詞處理器
 ...
 在本範例中，文字範本會呼叫指示詞，並傳入 XML 檔 (包含類別檔案的文件) 的名稱。 如需詳細資訊，請參閱 [XML 檔批註](https://msdn.microsoft.com/library/803b7f7b-7428-4725-b5db-9a6cff273199)。

 文字範本接著使用指示詞建立的 <xref:System.Xml.XmlDocument> 屬性，巡覽 XML 並印出文件註解。

#### <a name="to-create-an-xml-file-for-use-in-testing-the-directive-processor"></a>若要建立可用於測試指示詞處理器的 XML 檔

1. `DocFile.xml`使用任何文字編輯器來建立名為的文字檔 (例如，[記事本]) 。
...
4. 將 TestDP.tt 的內容變更為下列文字。
    ...
    > [!NOTE]
    > 請務必將字串 <取代為 `YOUR PATH>` DocFile.xml 檔案的路徑。
...
1. 在 **方案總管**中，以滑鼠右鍵按一下 TestDP.tt，然後按一下 [ **執行自訂工具**]。
     ...
     若為 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 使用者，預設可能不會在 **方案總管** 中顯示 TestDP.txt。 若要顯示所有指派給專案的檔案，請開啟 [ **專案** ] 功能表，然後按一下 [ **顯示所有**檔案]。
...
1. 以下列內容取代 TestDP.tt 中的程式碼。 HTML 會反白顯示。 請務必將字串取代為 `YOUR PATH` DocFile.xml 檔的路徑。
