...
2. 在 [**屬性**] 視窗中，選擇 [造型 **] 屬性中的下拉**箭號，然後選取要套用之造型的核取方塊。
...
3. 展開 [造型 **] 屬性，** 查看您可以設定的其他屬性。
   ...
   類型、屬性、作業和關聯的**描述**屬性會寫入至產生的程式碼中 `<summary>` 批註。 連結至類型的註解項目會寫入 `<remarks>` 註解。

## <a name="varying-the-generated-code"></a>各種產生的程式碼
 產生的程式碼會因為每一個類型、屬性或作業的屬性而異。 例如，如果您將類別的 [ **Is Abstract** ] 屬性設為 true，則 `abstract` 關鍵字會出現在產生的類別上。 如果您將屬性的**多重性**設定為**0.. \*** ，則產生的屬性會有 `IEnumerable<>` 類型。
...
4. 在此對話方塊中，設定文字範本繫結的屬性。
   ...
   |    目標名稱     | 產生的檔案名稱。<br /><br /> 您可以將運算式插入此字串中，例如 `{Name}` 或 `{Owner.Name}`。 例如，您可以撰寫： `{Owner.Name}_{Name}`。 在模型項目上評估此運算式。 它可以使用項目的屬性，但不能使用方法。 若要找出可以使用的屬性，請查看 **\* VisualStudio**中的類型屬性。 \* \*Important： \* \* `{Name}` 或 `{Owner.Name}` 只能用在 [**目標名稱**] 屬性中。 若要變更產生之類別的名稱，您必須修改此範本。 如需詳細資訊，請參閱[撰寫文字模板](#writing)。 |
...
- 以方括弧括住的範本部分 `<# ... #>` 由 [**產生程式碼**] 命令執行。 這些括號外面的範本部分會複製到結果檔案。 請務必區分產生程式碼與產生的文字。 產生的文字可以使用任何語言。

- 系統會評估 `<#= Expressions #>`，並將其轉換成字串。

## <a name="see-also"></a>另請參閱
 [UML 類別圖表：@No__t_1UML 類別圖表的參考 ](../modeling/uml-class-diagrams-reference.md)：方針 ](../modeling/uml-class-diagrams-guidelines.md)[從 UML 模型產生](../modeling/generate-files-from-a-uml-model.md)檔案
