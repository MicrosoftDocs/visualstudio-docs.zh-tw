...
|---------------|-----------------|------------------------------|
|引擎|引擎元件控制文字模板轉換進程|否。|
|主機|主機是引擎和使用者環境之間的介面。 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 是文字轉換進程的主控制項。|是。 您可以撰寫自訂主機。|
...
- 處理文字模板轉換錯誤。 例如，主機可以在使用者介面中顯示錯誤，或將它們寫入檔案。  (在中 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ，錯誤會顯示在錯誤訊息視窗中。 ) 
