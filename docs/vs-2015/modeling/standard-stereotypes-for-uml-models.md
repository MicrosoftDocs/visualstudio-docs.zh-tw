...
您可以將造型加入 UML 模型項目，以提供其他資訊給讀取器或電腦處理。 造型在設定檔中定義，且每個設定檔會提供一組造型。 有些設定檔隨 Visual Studio 提供。 您也可以定義自己的設定檔來自己的造型。 如需詳細資訊，請參閱[定義要擴充 UML 的設定檔](../modeling/define-a-profile-to-extend-uml.md)。

 若要查看哪些 Visual Studio 版本支援這項功能，請參閱 [Version support for architecture and modeling tools](../modeling/what-s-new-for-design-in-visual-studio.md#VersionSupport)。

## <a name="the-standard-profiles"></a>標準設定檔
 在支援的 Visual Studio 版本中，您可以在安裝之後，立即使用下列設定檔。

|設定檔|用途|
|-------------|-------------|
|[UML 標準設定檔 L2](#L2)|一組標準造型，可以用來加入項目或關聯性的額外相關資訊。|
|[UML 標準設定檔 L3](#L3)|一組標準造型，可以用來加入項目或關聯性的額外相關資訊。|
|[C#特徵](#NetProfile)|如果您想要以一個類別或 UML 模型中的其他項目代表程式碼，可以套用 C# 設定檔的其中一個造型來表示。<br /><br /> 這些造型也會將屬性加入模型項目。|
...
|造型|適用於|意義|
|----------------|----------------|-------------|
|輔助|執行個體|支援另一個類別的類別，通常是藉由實作額外的邏輯來支援。 其他類別可能具有 «焦點» 造型。|
...
|可執行檔|成品|可執行檔 «檔案»。|
|檔案|成品|實體檔案。|
|焦點|執行個體|定義核心商務邏輯的類別，由數個 «輔助» 類別支援。|
|架構|封裝|此套件會定義可重複使用的設計模式。|
|實作|元件|«規格» 的實作。|
|implementationClass|執行個體|此類別描述實作，並且每個執行階段執行個體都有一個固定的實作類別。 與 «類型» 對照。|
|執行個體化|相依性|用戶端會建立供應商的執行個體。|
|程式庫|成品|程式庫 «檔案»。|
|metaclass|執行個體|此類別的執行個體也是類別。|
|modelLibrary|封裝|包含以匯入套件方式要重複使用的模型項目。 通常定義為設定檔，並且由設定檔的應用程式自動匯入。|
...
|型別|執行個體|指定物件的行為，而不陳述實作的方式。 物件如果符合規格，則會是類型的成員。|
|utility|執行個體|靜態函式的集合 此類別沒有執行個體。|
...
|造型|適用於|內容|
|----------------|----------------|----------------|
|**C#課堂**|UML 類別<br /><br /> 元件|**Clr 屬性**<br /><br /> **為部分**<br /><br /> **已密封**<br /><br /> **為靜態**<br /><br /> **不安全**<br /><br /> **套件可見度**|
|**C#結構**|UML 類別<br /><br /> 元件|**Clr 屬性**<br /><br /> **為部分**<br /><br /> **不安全**<br /><br /> **套件可見度**|
|**C#全域成員**|UML 類別<br /><br /> 元件|**Clr 屬性**|
|**C#介面**|UML 介面|**Clr 屬性**<br /><br /> **為部分**<br /><br /> **套件可見度**|
|**C#列舉**|UML 列舉|**ClrAttributes**<br /><br /> **基底類型**|
|**C#命名空間**|UML 套件|**Clr 屬性**<br /><br /> **基底名稱**<br /><br /> **使用命名空間**|

## <a name="see-also"></a>請參閱
 [將造型加入 UML 模型元素](../modeling/add-stereotypes-to-uml-model-elements.md)[使用設定檔和造型自訂您的模型](../modeling/customize-your-model-with-profiles-and-stereotypes.md)[定義要擴充 UML 的設定檔](../modeling/define-a-profile-to-extend-uml.md)
