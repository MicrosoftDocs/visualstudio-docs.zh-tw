...
當您在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 文字模板中撰寫程式碼時，一定會有數種方法可供您使用。 這些方法會在 <xref:Microsoft.VisualStudio.TextTemplating.TextTransformation> 中定義。

> [!TIP]
> 您也可以在一般（未預先處理的）文字模板中，使用主機環境所提供的其他方法和服務。 例如，您可以解析檔案路徑、記錄錯誤，並取得 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 和任何載入的封裝所提供的服務。  如需詳細資訊，請參閱[從文字模板存取 Visual Studio](https://msdn.microsoft.com/0556f20c-fef4-41a9-9597-53afab4ab9e4)。
...
## <a name="error-and-warning-methods"></a>錯誤和警告方法
 您可以使用錯誤和警告公用程式方法，將訊息新增至 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 錯誤清單。 例如，下列程式碼會將錯誤訊息新增至錯誤清單。
...
## <a name="access-to-host-and-service-provider"></a>存取主機和服務提供者
 ...
 @No__t_0 的類型取決於範本執行所在的主機類型。 在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 中執行的範本中，您可以將 `this.Host` 轉換成 `IServiceProvider`，以取得 IDE 等服務的存取權。 例如:
