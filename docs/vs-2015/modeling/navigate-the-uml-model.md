---
title: 流覽 UML 模型 |Microsoft Docs
ms.date: 11/15/2016
...
## <a name="the-model-elements-model-and-model-store"></a>模型項目、模型和模型存放區
 元件中定義的類型 **Microsoft.VisualStudio.Uml.Interfaces.dll** 對應到 [UML 規格 2.1.2](https://www.omg.org/spec/UML/2.1.2/Superstructure/PDF/)中定義的類型。
...
- 如需 API 的完整詳細資訊，請參閱 UML 模型擴充性的 [Api 參考](../modeling/api-reference-for-uml-modeling-extensibility.md)。
...
### <a name="relationships"></a>關聯性
 ...
 如果「UML 規格」將多重性 0..1 指派給屬性，則值可能是 `null`。 最大值大於1的多重性表示 .NET 屬性的類型為： `IEnumerable<` *type* `>` 。
...
### <a name="the-ownership-tree"></a>擁有權樹狀結構
 ...
 在大部分情況下，其他具有更特定名稱的屬性也會參考 `Owner` 和 `OwnedElements` 屬性的目標。 例如，UML 類別會擁有每個 UML 作業。 因此， [IOperation](/previous-versions/dd481186(v=vs.140)) 具有名為 [IOperation](/previous-versions/dd473473%28v%3dvs.140%29)的屬性，以及每個 [IOperation](/previous-versions/dd481186(v=vs.140)) 物件中的 `Class == Owner` 。

 樹狀結構的最上層元素（沒有擁有者）是 `AuxiliaryConstructs.IModel` 。 IModel 包含在中 `IModelStore` ，它是 [IModelStore 根目錄](/previous-versions/ee789368(v=vs.140))。
...
- 在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 中開啟模型專案和其圖表，然後存取內容。 如需詳細資訊，請參閱 [使用 VISUAL STUDIO API 開啟 UML 模型](../modeling/open-a-uml-model-by-using-the-visual-studio-api.md)。
