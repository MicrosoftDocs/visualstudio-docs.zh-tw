...
## <a name="shapes-and-diagrams"></a>圖案和圖表
 有兩個物件用來表示圖層模型中的每個元素： `ILayerElement` 和[IShape](/previous-versions/ee806673(v=vs.140))。 `IShape` 代表圖案在圖表上的位置和大小。 在圖層模型中，每個 `ILayerElement` 都有一個 `IShape`，而圖層圖表上的每個 `IShape` 都有一個 `ILayerElement`。 `IShape` 也用於 UML 模型。 因此，不是每個 `IShape` 都有圖層項目。

 以相同的方式，`ILayerModel` 會顯示在一個[IDiagram](/previous-versions/ee789658(v=vs.140))上。
