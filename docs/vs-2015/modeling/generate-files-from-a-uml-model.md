...
- [在設計階段產生](#Design)。 您可以使用模型來定義一些應用程式功能，並在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 方案內產生程式碼、資源等。
  ...
  本主題最後會討論 [如何使用文字產生](#What)。 如需詳細資訊，請參閱程式 [代碼產生和 T4 文字模板](../modeling/code-generation-and-t4-text-templates.md)。

## <a name="generating-files-from-a-menu-command"></a><a name="Command"></a> 從功能表命令產生檔案
...
```
 ...
 若要建立這個檔案，請在方案總管中的專案上按一下滑鼠右鍵，指向 [ **加入**]，然後按一下 [ **新增專案**]。 選取 [前置處理過的 **文字模板**]。 這個範例的檔案名應該是 **VdmGen.tt**。 檔案的 **自訂工具** 屬性應該是 **TextTemplatingFilePreprocessor**。 如需前置處理過的文字模板的詳細資訊，請參閱 [使用 T4 文字模板的執行時間文字產生](../modeling/run-time-text-generation-with-t4-text-templates.md)。
...
```
 ...
 若要測試專案，請按 **F5**。 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的新執行個體隨即啟動 。 在這個執行個體中，開啟或建立包含類別圖的 UML 模型。 將部分類別加入圖表中，並將部分屬性加入每個類別中。 以滑鼠右鍵按一下圖表，然後按一下範例命令 `Generate VDM` 。 此命令會建立檔案 `C:\Generated.txt` 。 檢查這個檔案。 其內容應該與下列文字類似，但它會列出您自己的類別和屬性：
...
## <a name="generating-files-from-an-application"></a><a name="Application"></a> 從應用程式產生檔案
 您可以從讀取 UML 模型的應用程式產生檔案。 基於這個目的，存取模型和其元素的最具彈性且健全的方法是 [Visual Studio Modelbus](../modeling/integrate-uml-models-with-other-models-and-tools.md)。

 您也可以使用基本 API 來載入模型中，並使用與前一節相同的技術將模型傳遞給文字範本。 如需有關載入模型的詳細資訊，請參閱 [在程式碼中讀取 UML 模型](../modeling/read-a-uml-model-in-program-code.md)。

## <a name="generating-files-at-design-time"></a><a name="Design"></a> 在設計階段產生檔案
 如果您的專案具有將 UML 解譯為程式碼的標準方法，則可以建立文字範本，讓您透過 UML 模型產生您專案內的程式碼。 通常，您會有包含 UML 模型專案的方案。以及應用程式碼的一個或多個專案。 根據模型的內容，每個程式碼專案都可以包含數個產生程式碼、資源和組態檔的範本。 開發人員可以在方案總管工具列中按一下 [ **轉換所有範本** ]，以執行所有範本。 通常會以部分類別的形式產生程式碼，輕鬆地整合手動撰寫的組件。
...
- 範本檔案的 [自訂工具] 屬性應該是 **TextTemplatingFileGenerator**，且副檔名應為 tt。
...
- 當您儲存範本時，以及當您在方案總管工具列中按一下 [ **轉換所有範本** ] 時，就會執行範本。

- 如需這類範本的詳細資訊，請參閱 [使用 T4 文字模板產生設計階段程式碼](../modeling/design-time-code-generation-by-using-t4-text-templates.md)。
...
2. 在相同的方案中，建立 C# 或 Visual Basic 專案。
   ...
   - 在方案總管中，以滑鼠右鍵按一下方案，指向 [ **加入**]，然後按一下 [ **新增專案**]。 在 [ **已安裝的範本**] 下，按一下 [ **Visual Basic** 或 **Visual c #]，** 然後選取 [ **主控台應用程式**] 之類的專案類型。

3. 將純文字檔加入 C# 或 Visual Basic 專案。 這個檔案將包含您要撰寫數個文字範本時共用的程式碼。

   - 在方案總管中，以滑鼠右鍵按一下專案，指向 [加入]  ，然後按一下 [新增項目]  。 選取 [ **文字檔**]。

     插入下列區段中顯示的文字。

4. 將文字範本檔案加入 C# 或 Visual Basic 專案。

   - 在方案總管中，以滑鼠右鍵按一下專案，指向 [加入]  ，然後按一下 [新增項目]  。 選取 [ **文字模板**]。
...
#### <a name="content-of-the-text-template-file"></a>文字範本檔案的內容
 下列文字放置於 **tt** 檔案中。 這個範例會透過模型中的 UML 類別在 C# 檔案中產生類別。 不過，您可以產生任何類型的檔案。 所產生檔案的語言與用來撰寫文字範本程式碼的語言無關。
...
## <a name="how-to-use-text-generation"></a><a name="What"></a> 如何使用文字產生
...
- **設定檔**。 甚至，在一個商務區域內，項目類型的解譯可能會不同。 例如，在網站圖上，有些類別可能代表網頁，其他類別則代表內容區塊。 若要讓使用者更方便記錄這些差異，請定義造型。 造型也可能會附加套用至該類型之項目的其他屬性。 造型會封裝在設定檔內。 如需詳細資訊，請參閱 [定義擴充 UML 的設定檔](../modeling/define-a-profile-to-extend-uml.md)。
...
- **限制模型**。 並非所有您可以建立的模型都適用於每種用途。 例如，在 Fabrikam 的機場行李模型中，報到櫃檯沒有外送傳送帶是不正確的。 您可以定義驗證函式，幫助使用者觀察這些條件約束。 如需詳細資訊，請參閱 [定義 UML 模型的驗證條件約束](../modeling/define-validation-constraints-for-uml-models.md)。

- **保留手動變更**。 只有一些方案檔才能透過模型產生。 在大部分情況下，您需要可以手動加入或調整產生的內容。 不過，重新執行範本轉換時，一定要保留這些手動變更。

     當您的範本以語言產生程式碼 [!INCLUDE[TLA2#tla_net](../includes/tla2sharptla-net-md.md)] 時，它們應該會產生部分類別，讓開發人員可以加入方法和程式碼。 它也可用來將每個類別產生為一個配對：包含方法的抽象基底類別，以及只包含建構函式的繼承類別。 這可讓開發人員覆寫方法。 為了允許覆寫初始化，這項作業是在不同的方法中進行，而不是在建構函式中。
