...
|屬性|描述|預設|
|--------------|-----------------|-------------|
|預設展開折迭狀態|如果 `Expanded`，則會在建立時顯示區間。 如果 `Collapsed`，則不是。|展開|
|填滿色彩|此圖形的填滿色彩。|份|
|填滿漸層模式|此圖形的填滿漸層模式。|水平|
|幾何|這個形狀的幾何（矩形或圓角矩形）。|矩形|
|具有預設連接點|如果 `True`，圖形將會在產生的設計工具中使用頂端、下、左和右連接點。|偽|
|是否顯示單一區間標頭|如果 `False`，且圖形具有單一區間，則不會顯示區間的標頭。|True|
|外框色彩|此圖形的外框色彩。|黑色|
|外框虛線樣式|此圖形的外框虛線樣式（實線、虛線、點、點、DashDotDot、Custom）。|實線|
|外框粗細|此圖形的外框粗細。|0.03125|
|文字色彩|用於與此圖形相關聯之文字裝飾專案的色彩。|黑色|
|存取修飾詞|區間圖形（`public` 或 `internal`）的存取層級。|公用|
|自訂屬性|用來將屬性新增至從這個區間圖形產生的原始程式碼類別|\<無>|
|產生雙重衍生|如果 `True`，則會產生基類和部分類別（以支援透過覆寫的自訂）。 如需詳細資訊，請參閱覆[寫和擴充產生的類別](../modeling/overriding-and-extending-the-generated-classes.md)。|偽|
|有自訂的函數|如果 `True`，則會在原始程式碼中提供自訂的函式。 如需詳細資訊，請參閱覆[寫和擴充產生的類別](../modeling/overriding-and-extending-the-generated-classes.md)。|偽|
|繼承修飾詞|描述從區間圖形（`none`、`abstract` 或 `sealed`）產生之原始程式碼類別的繼承類型。|無|
|基底區間圖形|這個圖形的基類。|(無)|
|名稱|這個圖形的名稱。|目前名稱|
...
|已公開填滿色彩做為屬性<br /><br /> 公開填滿漸層模式<br /><br /> 公開大綱色彩做為屬性<br /><br /> 將大綱虛線樣式公開為屬性<br /><br /> 公開大綱粗細做為屬性<br /><br /> 公開文字色彩|如果 `True`，使用者可以設定圖形的 [所述] 屬性。 若要進行此設定，請以滑鼠右鍵按一下圖形定義，然後按一下 [新增] [**公開**]。|偽|
|描述|用來記錄產生的設計工具。|\<無>|
