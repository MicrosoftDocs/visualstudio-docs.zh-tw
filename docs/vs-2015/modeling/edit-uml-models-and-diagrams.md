...
您可以透過數種不同類型之圖表所提供的檢視來建立和編輯 UML 模型。 這些圖表在您的系統上提供不同的觀點，協助您了解並討論其設計和需求的不同層面。 Visual Studio 提供其中五個最常用的 UML 圖表類型的範本。
 ...
 本主題描述用於編輯不同圖表類型通用之模型的技巧。 如需特定圖表類型的詳細資訊，請參閱為 [您的應用程式建立模型](../modeling/create-models-for-your-app.md)。
...
## <a name="uml-diagrams-are-views-of-a-uml-model"></a><a name="Views"></a> UML 圖表是 UML 模型的視圖
...
- 您可以在 [ **UML 模型瀏覽器**] 中看到模型。 在 [ **架構** ] 功能表上，指向 [ **Windows**]，然後按一下 [ **UML 模型瀏覽器**]。

- 圖表上的每個圖形都是模型中項目的檢視。 在圖表上放置新圖形時，就會在模型中建立新項目。

- 儲存任何圖表時，Visual Studio 會儲存整個模型、其所有圖表，以及模型專案檔。

## <a name="creating-uml-modeling-diagrams"></a><a name="Creating"></a> 建立 UML 模型圖
...
- 在 [ **檔案] 功能表上，指向 [** **新增**]，再按一下 [ **專案**]，然後在 [ **新增專案** ] 對話方塊中，按兩下 [ **模型專案**]。

  如需如何管理模型專案的詳細資訊，請參閱 [建立 UML 模型專案和圖表](../modeling/create-uml-modeling-projects-and-diagrams.md)。

## <a name="drawing-uml-modeling-diagrams"></a><a name="Drawing"></a> 繪製 UML 模型圖
 模型圖會顯示透過關聯性連結之模型項目的集合。 每個項目都會顯示為圖形，而每個關聯性都會顯示為兩個圖形的接點。

 有兩種工具，一個適用於項目，另一個則適用於關聯性。 例如，在 UML 類別圖表工具箱中，[ **類別** ] 是 [專案] 工具，而 [關聯] 則是 **關聯** 性工具。

> [!NOTE]
> 如果您想要特定圖表類型的特定資訊，請參閱為 [您的應用程式建立模型](../modeling/create-models-for-your-app.md)。
...
1. 若要建立模型項目，請按一下 [工具箱] 中的項目工具，然後按一下您想要顯示它的圖表。 建立項目之後，請拖曳其控點來調整大小和形狀。
   ...
   > [!NOTE]
   > 如果您看不到 [工具箱]，請按一下 [ **View** ] 功能表上的 [**工具箱**]。
...
2. 若要建立關聯性，請按一下關聯性工具，並按一下您想要開始關聯性的項目，然後按一下您想要結束關聯性的項目。
   ...
   > [!NOTE]
   > 若要使用相同的工具數次，請按兩下工具。 當您完成時，請按一下 [ **指標** ] 工具。

   在某些類型的圖表中，您也可以繪製簡單圖形。 這些圖形不是模型的一部分，但是您可以使用它們來強調圖表的各部分，或將圖表分成不同的區域。

## <a name="editing-shapes-and-connectors"></a><a name="Editing"></a> 編輯圖形和連接器
...
> [!TIP]
> 內建命令不包括用於整齊地對齊圖形的命令。 不過，您可以藉由複製範例中的程式碼，在 [圖表上顯示 UML 模型](../modeling/display-a-uml-model-on-diagrams.md)，輕鬆地建立自己的對齊命令。
...
- 按一下元素，並在 [ **屬性** ] 視窗中設定欄位。
     ...
     如果您看不到 [ **屬性** ] 視窗，請在專案上按一下滑鼠右鍵，然後按一下 [ **屬性]。**
...
- 按住 **CTRL + SHIFT**鍵，然後按一下滑鼠左鍵或滑鼠左鍵。

     \- 或 -

- 在 [ **架構設計** 工具] 工具列上，按一下加號 (**+**) 或減號 (**-**) ，或是選擇縮放層級。

## <a name="searching-in-a-diagram"></a><a name="Searching"></a> 在圖表中搜尋
...
1. 按 **CTRL + F**。
    ...
    > [!NOTE]
    > 在 [ **尋找和取代** ] 對話方塊中，您必須將 [ **查詢** ] 欄位設定為 [ **目前檔**]。 不支援其他選項。

2. 輸入您想要尋找的文字，然後按一下 **[找下一個]**。

    > [!NOTE]
    > 如果您想要尋找的文字是在已摺疊的圖形內部，則會反白顯示圖形。 展開圖形，然後再按 **一次 [找下一個]** 。

## <a name="undoing-changes-to-the-model"></a><a name="Undo"></a> 復原模型的變更
...
## <a name="sharing-elements-between-diagrams"></a><a name="Sharing"></a> 在圖表之間共用元素
...
1. 若要開啟 [ **UML 模型瀏覽器**]，請在 [ **架構** ] 功能表上，指向 [ **視窗**]，然後按一下 [ **UML 模型瀏覽器**]。

2. 將模型專案從 **UML 模型瀏覽器** 拖曳至相同專案中的相容圖表。

     即會出現提供模型項目檢視的圖形，也可能會有其他圖表或相同圖表上的檢視。

    > [!NOTE]
    > 將類別或元件拖曳至循序圖時，效果會不同。 在該情況下，會建立其類型是該類別或元件的新生命線。 如需詳細資訊，請參閱 [UML 順序圖表：指導方針](../modeling/uml-sequence-diagrams-guidelines.md)。

#### <a name="to-add-a-new-view-of-a-model-element-by-using-paste-reference"></a>使用貼上參考加入模型項目的新檢視

1. 以滑鼠右鍵按一下現有的元素，然後按一下 [ **複製**]。

    - 您可以同時複製數個項目。 按住 CTRL 鍵，同時按一下每個專案，以滑鼠右鍵按一下其中一個專案，然後按一下 [ **複製**]。
...
2. 以滑鼠右鍵按一下相容圖表的空白部分，然後按一下 [ **貼上參考**]。
    ...
    > [!NOTE]
    > 這與 [ **貼** 上] 命令不同，它會在模型中建立新的元素。 如需詳細資訊，請參閱 [複製元素和相關元素群組](#Copying)。

> [!NOTE]
> 如果您將已透過關聯性連接之兩個模型項目的檢視加入圖表，則關聯性檢視也會出現在圖表上。 只有從圖表中移除其中一個項目，或從模型中刪除關聯性，才能刪除此檢視。

## <a name="copying-elements-and-groups-of-related-elements"></a><a name="Copying"></a> 複製相關元素的專案和群組
 您可以複製並貼上模型項目，而且可以複製並貼上項目群組以及其間的關聯性。

> [!NOTE]
> [ **貼** 上和 **貼上參考** ] 命令具有不同的效果。 **貼** 上會建立新的專案，其屬性就像複製的元素一樣。 [**貼上參考**] 會建立相同元素的新觀點。
...
## <a name="deleting-a-model-element-or-its-views"></a><a name="Deleting"></a> 刪除模型元素或其視圖
 某些種類的項目 (特別是分類器) 可以從圖表中移除，而不從模型中刪除它們。 分類器是在類別圖、元件圖和使用案例圖上顯示的主要項目。 它們會出現在多個圖表上。 針對這些類型的元素，有兩個不同的命令： **從圖表中移除** ，並 **從模型刪除**。

 反之，從圖表刪除關聯性時，一律會從模型予以刪除。

> [!NOTE]
> UML 圖表上特定種類的項目會有標籤。 透過在周圍繪製矩形來選取這類項目時，就可以選取標籤，但不是擁有這些標籤的項目。 不支援刪除透過這種方式選取的項目子集。 若要選取這些元素的子集，請按住 **CTRL** 鍵，同時按一下每個專案。
...
- 按一下圖表上的元素，然後按下 **DELETE** 鍵。
  ...
  - 項目的這個檢視隨即消失。 不過，專案會保留在模型中，而您仍然可以在 [ **UML 模型瀏覽器**] 中找到它。 也會維持相同項目的任何其他檢視。

  - 每個終止於此圖形的接頭都會從圖表中予以移除，但是它代表的關聯性會維持在模型中。 您可以在 [ **UML 模型瀏覽器** ] 中 **的關聯性下，看到**其所連接之每個專案底下的關聯性。
...
- 以滑鼠右鍵按一下圖表或 [ **UML 模型瀏覽器**] 中的關聯性，然後按一下 [ **從模型刪除**]。
...
## <a name="preparing-a-diagram-for-presentation"></a><a name="presentation"></a> 準備圖表以進行簡報
 下列功能可協助您將注意力放在圖表的特定部分、新增說明，或將圖表分成不同的興趣區域。

- 您可以將圖表的任何部分複製至 Word、PowerPoint 或其他文件。 選取您要的圖形和連接器，按一下滑鼠右鍵，然後按一下 [ **複製**]。

- 任何圖形或接頭的色彩隨即變更。 選取一或多個圖形，然後變更 [ **色彩** ] 屬性。 如果看不到 [屬性]**** 視窗，請按 **F4** 鍵。

- 在某些類型的圖表上，您可以從 [工具箱] 的 [ **簡單圖形** ] 區段繪製線條、矩形和省略號。 這些圖形不會形成 UML 模型的一部分。

- 若要為區域加上標籤，您可以從 [工具箱] 拖曳批註，然後將其 [ **透明** ] 屬性設定為 [ **True**]。 與 [簡單圖形] 相同，註解不會形成 UML 模型的一部分，而且不會出現在 [UML 模型總管] 中。
...
## <a name="extending-the-uml-designers"></a><a name="extensions"></a> 擴充 UML 設計工具
 您可以將新功能加入 UML 工具，以及調整圖表標記法使其符合您自己的需求。 如需詳細資訊，請參閱 [擴充 UML 模型和圖表](../modeling/extend-uml-models-and-diagrams.md)。

## <a name="see-also"></a>另請參閱
 [建立 UML 模型專案和圖表](../modeling/create-uml-modeling-projects-and-diagrams.md)[分析和模型化架構](../modeling/analyze-and-model-your-architecture.md)[建立您應用程式的模型](../modeling/create-models-for-your-app.md)
