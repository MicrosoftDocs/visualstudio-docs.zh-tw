...
- 您可以在 [UML 模型總管] 中查看模型。 在 [架構] 功能表上，指向 [Windows]，然後按一下 [UML 模型總管]。
...
3. 在 [加入模型專案] 中，選取現有模型專案，或選取 [建立新模型專案]。

   > [!NOTE]
   > 模型圖必須存在於模型專案內。

   您也可以在 [方案總管] 中將圖表加入現有的模型專案。 以滑鼠右鍵按一下模型專案，並指向 [加入]，然後按一下 [新項目]。

#### <a name="to-create-an-empty-uml-modeling-project"></a>建立空白 UML 模型專案

- 在 [檔案] 功能表上指向 [新增]，並按一下 [專案]，然後按兩下 [新增專案] 對話方塊中的 [模型專案]。
...
## <a name="Drawing"></a>繪製 UML 模型圖表
 ...
 有兩種工具，一個適用於項目，另一個則適用於關聯性。 例如，在 UML 類別圖 [工具箱] 中， [類別] 是項目工具，而 [關聯] 是關聯性工具。
...
2. 若要建立關聯性，請按一下關聯性工具，並按一下您想要開始關聯性的項目，然後按一下您想要結束關聯性的項目。
   ...
   > [!NOTE]
   > 若要使用相同的工具數次，請按兩下工具。 完成時，請按一下 [指標] 工具。
...
## <a name="Searching"></a>在圖表中搜尋
 [快速尋找] 函式會尋找圖表上的項目。 您必須將 [查詢] 設定為 [目前文件]。
...
1. 按 **CTRL+F**。
    ...
    > [!NOTE]
    > 在 [尋找和取代] 對話方塊中，您必須將 [查詢] 欄位設定為 [目前文件]。 不支援其他選項。

2. 輸入您想要尋找的文字，然後按一下 [找下一個]。

    > [!NOTE]
    > 如果您想要尋找的文字是在已摺疊的圖形內部，則會反白顯示圖形。 展開圖形，再按一下 [找下一個]。

## <a name="Undo"></a>復原模型的變更
 使用 [編輯] 功能表上的 [復原] 和 [重做] 命令，即可復原和重做您已對模型和圖表所做的變更。
...
1. 若要開啟 [UML 模型總管]，請在 [架構] 功能表上指向 [Windows]，然後按一下 [UML 模型總管]。
...
1. 以滑鼠右鍵按一下現有項目，然後按一下 [複製]。

    - 您可以同時複製數個項目。 在按一下每個項目時按住 CTRL 鍵，並以滑鼠右鍵按一下其中一個項目，然後按一下 [複製]。
...
2. 以滑鼠右鍵按一下相容圖表的空白部分，然後按一下 [貼上參考]。
    ...
    > [!NOTE]
    > 這與 [貼上] 命令不同，此命令會在模型中建立新的項目。 如需詳細資訊，請參閱[複製項目和一組相關項目](#Copying)。
...
> [!NOTE]
> [貼上] 和 [貼上參考] 命令的效果不同。 [**貼**上] 會建立新的專案，其屬性與所複製專案的內容相似。 **貼上參考**會建立相同元素的新視圖。
...
## <a name="Deleting"></a>刪除模型元素或其 Views
 某些種類的項目 (特別是分類器) 可以從圖表中移除，而不從模型中刪除它們。 分類器是在類別圖、元件圖和使用案例圖上顯示的主要項目。 它們會出現在多個圖表上。 這些類型的項目有兩個不同的命令：[從圖表移除] 和 [從模型刪除]。

 反之，從圖表刪除關聯性時，一律會從模型予以刪除。

> [!NOTE]
> UML 圖表上特定種類的項目會有標籤。 透過在周圍繪製矩形來選取這類項目時，就可以選取標籤，但不是擁有這些標籤的項目。 不支援刪除透過這種方式選取的項目子集。 若要選取這些項目的子集，請在按一下每個項目時按住 **CTRL** 鍵。
...
- 按一下圖表上的項目，然後按 **DELETE** 鍵。
  ...
  - 項目的這個檢視隨即消失。 不過，項目會維持在模型中，但您還是可以在 [UML 模型總管] 中找到它。 也會維持相同項目的任何其他檢視。

  - 每個終止於此圖形的接頭都會從圖表中予以移除，但是它代表的關聯性會維持在模型中。 您可以在 [UML 模型總管] 的 [關聯性] 下看到所連接之每個項目的關聯性。

#### <a name="to-delete-an-element-from-the-model"></a>從模型刪除項目

- 以滑鼠右鍵按一下 [UML 模型總管] 或圖表上的項目，然後按一下 [從模型刪除]。
...
- 以滑鼠右鍵按一下圖表或 [UML 模型總管] 上的關聯性，然後按一下 [從模型刪除]。
...
- 您可以將圖表的任何部分複製至 Word、PowerPoint 或其他文件。 選取您想要的圖形和接頭，並按一下滑鼠右鍵，然後按一下 [複製]。

- 任何圖形或接頭的色彩隨即變更。 選取一或多個圖形，然後變更 [色彩] 屬性。 如果看不到 [屬性]視窗，請按 **F4** 鍵。

- 在某些種類的圖表上，您可以從 [工具箱] 的 [簡單圖形] 區段繪製線條、矩形和橢圓形。 這些圖形不會形成 UML 模型的一部分。

- 若要加上區域的標籤，您可以從 [工具箱] 拖曳 [註解]，並將其 [透明] 屬性設定為 [True]。 與 [簡單圖形] 相同，註解不會形成 UML 模型的一部分，而且不會出現在 [UML 模型總管] 中。
