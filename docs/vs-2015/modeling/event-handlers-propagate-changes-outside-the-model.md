---
title: 事件處理常式會將變更傳播到模型之外 |Microsoft Docs
ms.date: 11/15/2016
...
在視覺化和模型 SDK 中，您可以定義存放區事件處理常式，以將變更傳播到存放區以外的資源，例如非存放區變數、檔案、其他存放區中的模型，或其他 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 延伸模組。 存放區事件處理常式是在觸發事件發生的交易結束之後執行。 它們也會在復原或重做作業中執行。 因此，與存放區規則不同的是，存放區事件最適合用來更新存放區以外的值。 不同于 .NET 事件，系統會註冊存放區事件處理常式來接聽類別：您不需要為每個實例註冊個別的處理常式。 如需如何在不同的處理變更方式之間進行選擇的詳細資訊，請參閱 [回應及傳播變更](../modeling/responding-to-and-propagating-changes.md)。
...
1. 選擇您想要監視的事件種類。 如需完整清單，請查看的屬性 <xref:Microsoft.VisualStudio.Modeling.EventManagerDirectory> 。 每個屬性都對應至事件的類型。 最常使用的事件種類為：
   ...
   - `ElementAdded` –當建立模型專案、關聯性連結、圖形或連接器時，就會觸發。

   - ElementPropertyChanged –在定義域屬性的值變更時觸發 `Normal` 。 只有當新舊值不相等時，才會觸發事件。 事件無法套用至計算和自訂儲存體屬性。
...
## <a name="using-events-to-make-undoable-adjustments-in-the-store"></a>使用事件在存放區中進行可進行的調整
