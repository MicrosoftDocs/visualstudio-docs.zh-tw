...
設計階段 T4 文字範本可讓您在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 專案中產生程式碼和其他檔案。 通常，您會撰寫範本，使其根據*模型*中的資料來改變其所產生的程式碼。 模型是含有您應用程式需求重要資訊的檔案或資料庫。
...
2. 將文字模板檔案新增至您的專案，並為其命名副檔名為**tt**的名稱。
     ...
     若要這麼做，請在**方案總管**中，于專案的快捷方式功能表上，選擇 [**加入**]、[**新增專案**]。 在 [**加入新專案**] 對話方塊中，從中間窗格選取 [**文字模板**]。
...
6. 在**方案總管**中，展開範本檔案節點，您會發現副檔名為 **.txt**的檔案。 此檔案包含從範本產生的文字。
    ...
    > [!NOTE]
    > 如果您的專案是 Visual Basic 專案，您必須按一下 [**顯示所有**檔案]，才能看到輸出檔案。
...
- 按一下 [**建立**] 功能表中的 [**轉換所有範本**]。 這樣會轉換 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 方案中的所有範本。
...
- 在方案總管中，從文字模板檔案的快捷方式功能表選擇 [ **Debug T4 Template** ]。
...
6. 儲存檔案。
     ...
     即會建立具有所指定副檔名的附帶檔案。 其屬性對檔案類型而言正確。 例如，.cs 檔案的**組建動作**屬性會**編譯**。
