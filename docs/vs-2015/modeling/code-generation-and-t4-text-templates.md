...
## <a name="in-this-section"></a>本章節內容
 [使用 T4 文字模板產生執行時間文字](../modeling/run-time-text-generation-with-t4-text-templates.md)在任何產生文字檔的應用程式中，預先編譯的文字模板是定義文字的簡單且可靠的方法。 但是，此方法不可用於會在執行階段變更的文字範本。

 [使用 T4 文字模板產生設計階段程式碼](../modeling/design-time-code-generation-by-using-t4-text-templates.md)從模型產生程式碼和其他資源，可讓您藉由更新模型來更新應用程式。

 [組建流程中的程式碼產生](../modeling/code-generation-in-a-build-process.md)如果您已安裝 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 視覺效果和模型化 SDK，則可以確保產生的軟體會隨時掌握模型中的變更。

 [撰寫 T4 文字模板](../modeling/writing-a-t4-text-template.md)文字模板檔案的語法。

 [逐步解說：使用文字模板產生程式碼](../modeling/walkthrough-generating-code-by-using-text-templates.md)示範使用程式碼產生的其中一種方式。

 [對 T4 文字模板進行調試](../modeling/debugging-a-t4-text-template.md)如何調試文字模板，以及一些常見的文字模板錯誤。

 [使用 TextTransform 公用程式產生](../modeling/generating-files-with-the-texttransform-utility.md)檔案命令列工具，可讓您用來執行文字模板轉換。

 [自訂 T4 文字轉換](../modeling/customizing-t4-text-transformation.md)如何為您自己的資料來源撰寫指示詞處理器和自訂範本化主機。

## <a name="see-also"></a>請參閱
 從從[特定領域語言產生程式碼](../modeling/generating-code-from-a-domain-specific-language.md)[的 UML 模型產生](../modeling/generate-files-from-a-uml-model.md)檔案
