...
6. 在屬性視窗中，請確定至少有一個名為 [ **使用 ...** ] 的屬性為 `true` 。
...
8. 建立並執行方案，以確認它仍在運作中。
   ...
   您的 DSL 現在已啟用 MEF。 您可以撰寫功能表命令、手勢處理常式和驗證條件約束做為 MEF 延伸模組。 您可以將這些擴充功能與其他自訂程式碼一起寫入您的 DSL 解決方案中。 此外，您或其他開發人員可以撰寫 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 擴充 DSL 的個別延伸模組。

## <a name="creating-an-extension-for-a-mef-enabled-dsl"></a>建立已啟用 MEF 之 DSL 的延伸模組
 如果您可以存取您自己或其他人所建立且已啟用 MEF 的 DSL，您可以為其撰寫延伸模組。 擴充功能可以用來加入功能表命令、手勢處理常式或驗證條件約束。 若要撰寫這些擴充功能，您可以使用 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] (VSIX) 方案的延伸模組。 解決方案有兩個部分：建立程式碼元件的類別庫專案，以及封裝元件的 VSIX 專案。

#### <a name="to-create-a-dsl-extension-vsix"></a>建立 DSL 延伸模組 VSIX

1. 建立新的類別庫專案。 若要這樣做，請在 [ **新增專案** ] 對話方塊中，選取 [ **Visual Basic** ] 或 [ **Visual c #** ]，然後選取 [ **類別庫**]。
...
4. 在相同的方案中建立 VSIX 專案。 若要這樣做，請在 [ **新增專案** ] 對話方塊中，依序展開 **Visual Basic** 或 **Visual c #**]、[擴充性 **]，然後**選取 [ **VSIX 專案**]。
...
9. 檢查並更新 **extension.vsixmanifest**的其他欄位。 按一下 [ **選取版本** ]，並確認 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 已設定正確的版本。

10. 將程式碼加入至類別庫專案。 使用下一節中的範例做為指南。

     您可以新增任意數目的命令、手勢和驗證類別。

11. 若要測試擴充功能，請按 **F5**。 在的實驗實例中 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ，建立或開啟 DSL 的範例檔案。
...
### <a name="gesture-handlers"></a>軌跡處理常式
 軌跡處理常式可以處理從任何地方、內部或外部拖曳至圖表的物件 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 。 下列範例可讓使用者將檔案從 Windows 檔案總管拖曳至圖表上。 它會建立包含檔案名的元素。
