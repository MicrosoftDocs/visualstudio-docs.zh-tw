...
## <a name="changing-the-tab-icon"></a>變更索引標籤圖示
 若要變更 explorer 的索引標籤圖示，請在 .bmp 檔案中使用16x16 圖元的圖示。 將圖示檔放在 \DslPackage\Resources\ 資料夾中，然後將檔案名變更為**ModelExplorerToolWindowBitmaps。** 例如，您可以將 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 安裝程式 .ico 圖示檔變更為 .bmp 格式，並將它重新命名為**DSLLanguageName\DslPackage\Resources\ModelExplorerToolWindowBitmaps.bmp**。 產生的設計工具會在您的瀏覽器的索引標籤上，當它與**方案總管**停駐在一起時，顯示此圖示。
...
1. 使用 [工作流程] 方案範本來建立 [!INCLUDE[dsl](../includes/dsl-md.md)] 解決方案。
