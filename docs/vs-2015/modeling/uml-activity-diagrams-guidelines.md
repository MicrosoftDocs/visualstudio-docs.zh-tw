...
在 Visual Studio 中，您可以繪製活動圖表，將商務程序或軟體演算法描述成為經過一系列動作的工作流程。 人員、軟體元件或裝置皆可執行這些動作。 如需示範影片，請參閱：[使用活動圖表來捕捉商務工作流程](http://channel9.msdn.com/posts/clinted/UML-with-VS-2010-Part-4-Capture-Business-Workflows/)。
 ...
 若要建立 UML 活動圖，請在 [**架構**] 功能表上，按一下 [**新增 UML 或分層圖**]。
...
- 在您的系統或其每個主要元件所支援的動作 / 使用案例周圍繪製界限。
  ...
  在活動圖表中，您可以顯示動作之間傳遞的資料流。 請參閱[描述資料流程](#DataFlows)的一節。 但是活動圖表不會描述資料的結構。 針對該目的，您可以繪製 UML 類別圖表。 如需相關資訊，請參閱[UML 類別圖：方針](../modeling/uml-class-diagrams-guidelines.md)。
...
## <a name="SimpleControlFlow"></a>描述控制流程
 ...
 活動圖表會使用**動作**和**連接器**，將您的系統或應用程式描述為一系列的動作，並將控制項從某個動作依序流向下一個動作。

- 針對使用者、系統或兩者在共同作業中執行的每個主要工作，建立**動作**（1）。
...
- 使用 [**決策節點**] （3）來表示決策結果決定下一個步驟的點。 您可以繪製您想要的任意數目的傳出路徑。

- 如果您使用活動圖表來定義應用程式的組件，您應該定義成立條件 (4)，以便清楚每個路徑應該採用的時機。 以滑鼠右鍵按一下連接器，按一下 [**屬性**]，然後在 [**屬性**] 視窗中輸入 [ **Guard** ] 欄位的值。

- 您不一定需要定義成立條件。 例如，如果您使用活動圖表來描述商務程序或互動通訊協定，則分支會定義使用者或互動元件可用的選項範圍。

- 使用 [**合併節點**] （5）來結合兩個或多個在**決策節點**上分支的替代流程。

    > [!NOTE]
    > 您應該使用「**合併節點**」將替代流程結合在一起，而不是讓流程在一個動作中一起運作。 在此範例中，將不會正確地從決策節點直接連接到 **[選擇] 功能表項目**。 這是因為動作不會啟動，直到控制到達所有內送的連接器為止。 因此，您應該只將並行流程結合在動作。 如需詳細資訊，請參閱[並行流程](#Concurrent)。
...
- **接受事件節點**
     ...
     如[並行流程](#Concurrent)一節所述，建立**接受事件節點**，以指示回應特定事件的執行緒開始，例如使用者輸入。 請不要對該節點提供流入流程。 省略傳入的流程表示每當事件發生時，將會啟動執行緒。

     描述對特定外部事件的回應時，這個方法最有用。

### <a name="ending-the-activity"></a>結束活動
 使用 [**活動的最後節點**] （7）來表示活動的結束。

- 當控制執行緒到達 [**活動最終] 節點**時，所有活動的並行動作和子活動都會終止。
...
### <a name="swimlanes"></a>區隔線
 將活動的動作排列成區域，對應到執行動作的不同物件或商務角色，有時是很有用的。 這些區域會依資料行進行排序，並稱為*泳道*。
...
- 使用**輸出**連接和**輸入釘**選。 這個方法可讓您分別描述一個動作的輸出和另一個動作的輸入。 連接就像是程式中的參數。 連接代表連接埠，物件可以在這裡進入及離開動作。
...
### <a name="describing-data-flow-with-object-nodes"></a>使用物件節點描述資料流程
 ...
 您可以在屬性視窗中設定物件節點的**類型**。 類型可以是基本類型，例如整數，或您在類別圖表中定義的類別、介面或列舉。 比方說，您可以建立類別 Shipment Address，並具有屬性 Street Address、City 等等，以及與另一個名為 Customer 之類別的關聯。 如需詳細資訊，請參閱[UML 類別圖：方針](../modeling/uml-class-diagrams-guidelines.md)。
...
### <a name="describing-data-flow-with-input-and-output-pins"></a>使用輸入和輸出連接描述資料流程
 使用**輸出 pin**和**輸入 pin** ，分別描述一個動作的輸出，以及另一個動作的輸入。

 ![輸入和輸出圖釘是動作參數](../modeling/media/uml-actguidepins.png "UML_ActGuidePins")

 若要建立 pin，請按一下 [工具箱] 上的 [**輸入釘**選] 或 [**輸出 pin** ]，然後按一下動作。 接著您可以將連接在動作周圍移動，並變更其名稱。 您可以在任何一種動作上建立輸入和輸出針腳，包括**呼叫行為動作**、**呼叫作業動作**、**傳送信號動作**，以及**接受事件動作**。
...
- 設定動作的 [**本機後置條件**] 和 [**本機前置條件**] 屬性，以描述其在更具體細節中的結果。 如需詳細資訊，請參閱[定義後置條件和前置條件](#Postcondition)。
...
2. 在 [**加入新專案**] 對話方塊的 [**範本**] 下，按一下 [**活動圖**]，然後在 [**名稱**] 方塊中輸入您打算提供給 [**呼叫行為] 動作**的名稱。

3. 繪製子活動的詳細工作流程。 這是被呼叫的行為。

    - 在被呼叫的子活動圖表中，**初始節點**會指出叫用所呼叫的行為時，控制權的開始位置。 [**活動最終] 節點**會顯示控制項應回到父活動的位置。

4. 設定 [**呼叫行為] 動作**的 [**行為**] 屬性，以參考被呼叫的行為圖表。

    > [!NOTE]
    > 子活動圖表必須在其上有一些專案，或者圖表將無法在 [**行為**] 屬性的下拉式清單中使用。 此外，trident 圖示不會出現在您的 [**呼叫行為] 動作**圖形上，直到您設定其 [**行為**] 屬性為止。

5. 設定動作的 [ **Is 同步**] 屬性，以指出您的活動是否等待來電的活動完成。

    - 如果您將 [**同步**] 設定為 [false]，表示流程可以在呼叫的活動完成之前繼續到下一個動作。 您不應該定義動作的輸出連接或傳出的資料流程。
...
### <a name="Postcondition"></a>定義後置條件和前置條件
 您可以使用 [**本機後置**條件] 和 [**本機前置條件**] 屬性，以詳細指定動作的結果。 這些屬性會描述動作的效果，而不描述如何達成此效果。
...
- 您可以設定動作的 [ **Is 同步**] 屬性，以指出您的活動是否等待作業完成。

  - 如果您將 [**同步**] 設定為 [false]，則表示流程可以在呼叫的作業完成之前繼續到下一個動作。 您不應該定義動作的輸出連接或傳出的資料流程。

## <a name="Concurrent"></a>並行流程
 您可以使用 [**分叉] 節點**和 [**聯結] 節點**來描述可以同時執行之活動的兩個或多個執行緒。

 ![[分叉] 和 [聯結] 節點會顯示並行流程](../modeling/media/uml-actguideconcurrent.png "UML_ActGuideConcurrent")

 **分叉節點**（1）的效果是將控制權的執行緒分成兩個或多個執行緒。 前一個動作結束時，分岔輸出端的所有動作便可以開始。

 **聯結節點**（2）會將平行線程結合在一起。 在 [**加入節點**] 之後的動作可能會等到所有導致**聯結節點**的動作都完成之後才會啟動。
...
- 您可以顯示動作的傳出資料流程，以顯示您的活動如何處理訊號中收到的資料。 如果您想要顯示多個輸出流程，您應該設定 [接受事件] 動作的 [ **IsUnmarshall** ] 屬性，這表示動作會將傳入的信號剖析成個別的元件。 如需詳細資訊，請參閱[描述資料流程](#DataFlows)。
...
1. 建立新的活動圖表，以描述轉換或選取流程。
   ...
   - 在**方案總管**中，以滑鼠右鍵按一下您的專案，指向 [**加入**]，再按一下 [**新增專案**]，然後按一下 [**活動圖表**]。 為圖表提供適當的名稱，以表示轉換或選取流程。 按一下 [加入]。
...
3. 在您要使用轉換或選取的任何圖表中：
   ...
   3. 在 [**轉換**] 或 [**選取範圍**] 屬性中，選取您在其中指定轉換或選取流程的圖表。
