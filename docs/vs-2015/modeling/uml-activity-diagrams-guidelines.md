...
- 描述使用案例的執行步驟。 如需詳細資訊，請參閱 [UML 使用案例圖表：方針](../modeling/uml-use-case-diagrams-guidelines.md)。

- 描述在軟體中的方法、函式或作業。 如需詳細資訊，請參閱[模型應用程式的架構](../modeling/model-your-app-s-architecture.md)。

  繪製活動圖表可協助您改善程序。 如果現有程序的圖表會很複雜，您可以考慮如何簡化程序。

  如需活動圖表上元素的參考資訊，請參閱[UML 活動圖：參考](../modeling/uml-activity-diagrams-reference.md)。
...
- 在您的系統或其每個主要元件所支援的動作 / 使用案例周圍繪製界限。
  ...
  在活動圖表中，您可以顯示動作之間傳遞的資料流。 請參閱[描述資料流程](#DataFlows)一節。 但是活動圖表不會描述資料的結構。 針對該目的，您可以繪製 UML 類別圖表。 如需相關資訊，請參閱 [UML 類別圖：方針](../modeling/uml-class-diagrams-guidelines.md)。

## <a name="BasicSteps"></a>繪製活動圖的基本步驟
...
2. 在 [範本] 下，按一下 [UML 活動圖表]。

3. 命名圖表。

4. 在 [加入模型專案] 中，選取方案中的現有模型專案，或按一下 [建立新模型專案]。
...
- 在 [UML 模型總管] 中，將活動拖曳到套件。

     \-或-

- 在 [UML 模型總管] 中，以滑鼠右鍵按一下活動，然後按一下 [剪下]。 然後以滑鼠右鍵按一下套件，再按一下 [貼上]。
...
## <a name="SimpleControlFlow"></a>描述控制流程
 ...
 活動圖表使用 [動作] 和 [連接器]，將您的系統或應用程式描述為一系列動作，且控制會循序從一個動作送至下一個動作。

- 使用者、系統，或兩者共同作業中執行的每一項主要工作，建立 [動作] (1)。

  > [!NOTE]
  > 請試著以幾個動作描述您的程序或演算法。 您可以使用 [呼叫行為動作] 在個別的圖表中，更詳細地定義每個動作，如[使用呼叫行為動作描述子活動](#Subactivities)中所述。

- 確定每個動作的標題清楚地指出它通常會達成的事項。

- 使用 [連接器] (2) 循序連結動作。

- 每個動作會在控制流程中的下一個動作開始之前結束。 如果您想要描述重疊的動作，請使用 [分岔節點]，如[並行流程](#Concurrent)一節中所述。
...
- 使用 [決策節點] (3)，以表示決策結果會指定下個步驟的點。 您可以繪製您想要的任意數目的傳出路徑。

- 如果您使用活動圖表來定義應用程式的組件，您應該定義成立條件 (4)，以便清楚每個路徑應該採用的時機。 以滑鼠右鍵按一下連接器，按一下 [屬性]，然後在 [屬性] 視窗中，輸入 [成立條件] 欄位的值。

- 您不一定需要定義成立條件。 例如，如果您使用活動圖表來描述商務程序或互動通訊協定，則分支會定義使用者或互動元件可用的選項範圍。

- 使用 [合併節點] (5) 來結合兩個或多個於 [決策節點] 分支的替代流程。

    > [!NOTE]
    > 您應該使用 [合併節點] 來結合替代的流程，而不是流程結合在一個動作。 在範例中，從決策節點直接連接回到 [Choose Menu Item] 是不正確的。 這是因為動作不會啟動，直到控制到達所有內送的連接器為止。 因此，您應該只將並行流程結合在動作。 如需詳細資訊，請參閱[並行流程](#Concurrent)。
...
- **接受事件節點**
     ...
     建立 [接受事件節點]，如[並行流程](#Concurrent)一節所述，以表示啟動回應特定事件 (例如使用者輸入) 的執行緒。 請不要對該節點提供流入流程。 省略傳入的流程表示每當事件發生時，將會啟動執行緒。

     描述對特定外部事件的回應時，這個方法最有用。

### <a name="ending-the-activity"></a>結束活動
 使用 [活動的最後節點] (7)，表示活動的結束。

- 當控制執行緒到達 [活動的最後節點] 時，會終止活動的所有並行動作和子活動。
...
### <a name="swimlanes"></a>區隔線
 將活動的動作排列成區域，對應到執行動作的不同物件或商務角色，有時是很有用的。 這些區域通常會排列成資料行，並稱為 *「區隔線」* (swimlane)。

- 從工具箱的 [簡單圖形] 區段使用線條或矩形，來繪製區隔線或其他區域。

- 若要將每一個區隔線都加上標籤，請建立註解，並設定其 [透明] 屬性設為 [True]。
...
- 使用 [輸出連接] 和 [輸入連接]。 這個方法可讓您分別描述一個動作的輸出和另一個動作的輸入。 連接就像是程式中的參數。 連接代表連接埠，物件可以在這裡進入及離開動作。

    > [!NOTE]
    > 如需本節中使用之元素的總覽，請參閱主題中的資料流程一節，請參閱[UML 活動圖：參考](../modeling/uml-activity-diagrams-reference.md)。
...
### <a name="describing-data-flow-with-object-nodes"></a>使用物件節點描述資料流程
 ...
 ![物件節點可以顯示在動作之間傳遞的資料](../modeling/media/uml-actguidedata.png "UML_ActGuideData")

 請注意圓角矩形是代表動作，例如「配送餐點」，在這裡會發生處理作業。 方角矩形，例如「送貨地址」，則代表從一個動作到另一個動作的物件流程。

 提供物件節點可反映節點角色的名稱，做為在動作之間流動的物件管道或緩衝區。

 您可以在 [屬性] 視窗中設定物件節點的 [類型]。 類型可以是基本類型，例如整數，或您在類別圖表中定義的類別、介面或列舉。 比方說，您可以建立類別 Shipment Address，並具有屬性 Street Address、City 等等，以及與另一個名為 Customer 之類別的關聯。 如需詳細資訊，請參閱 [UML 類別圖表：方針](../modeling/uml-class-diagrams-guidelines.md)。

> [!NOTE]
> 如果您輸入尚未定義之類型的名稱，會在 UML 模型總管的 [未指定的類型] 下加入一個項目。 如果接下來您在類別圖表中定義該名稱的類型，則應該重設物件節點的類型，使其參考新的類型。
...
#### <a name="buffering-data-in-object-nodes"></a>在物件節點中緩衝處理資料
 ...
 ![緩衝物件節點中的資料](../modeling/media/uml-actguidebuffer.png "UML_ActGuideBuffer")
...
- 設定 [上限] 屬性來指定緩衝區中可以包含的物件最大數目。 預設值為 *。 這表示沒有任何限制。

### <a name="describing-data-flow-with-input-and-output-pins"></a>使用輸入和輸出連接描述資料流程
 使用 [輸出連接] 和 [輸入連接] 分別描述一個動作的輸出和另一個動作的輸入。

 ![輸入和輸出圖釘是動作參數](../modeling/media/uml-actguidepins.png "UML_ActGuidePins")

 若要建立 pin，請按一下 [工具箱] 上的 [**輸入釘**選] 或 [**輸出 pin** ]，然後按一下動作。 接著您可以將連接在動作周圍移動，並變更其名稱。 您可以在任何一種動作上建立輸入和輸出連接，包括 [呼叫行為動作]、[呼叫作業動作]、[傳送訊號動作] 和 [接受事件動作]。

 兩個連接之間的連接器代表物件流程，就像進出物件節點的流程一樣。

 提供每個連接一個名稱，表示它產生或接受的物件角色，例如參數名稱。

 您可以在 [類型] 屬性中設定傳輸的物件類型。 這必須是您已在 UML 類別圖表中建立的類型。
...
- 在 [主體] 屬性寫入更詳細的描述。 例如，您可以撰寫一段程式碼或虛擬程式碼，或是所達到結果的完整描述。

- 將動作取代為呼叫行為動作，並在個別活動圖表中描述其詳細的行為。 請參閱[使用呼叫行為動作描述子活動](#Subactivities)。

- 設定動作的 [本機後置條件] 和 [本機前置條件] 屬性，以更特定的詳細資料描述其結果。 如需詳細資訊，請參閱[定義後置條件和前置條件](#Postcondition)。
...
1. 若要建立子活動的圖表，請在 [方案總管]中，以滑鼠右鍵按一下您的模型專案、指向 [新增]，然後按一下 [新增項目]。

2. 在 [加入新項目] 對話方塊的 [範本] 下，按一下 [活動圖表]，並在 [名稱] 方塊中輸入您打算提供給 [呼叫行為動作] 的名稱。

3. 繪製子活動的詳細工作流程。 這是被呼叫的行為。

    - 在被呼叫的子活動圖中，[初始節點] 表示叫用被呼叫的行為時，控制權的開始位置。 [活動的最後節點] 顯示控制權應傳回至父活動的位置。

4. 設定 [呼叫行為動作] 的 [行為] 屬性，參考被呼叫的行為圖表。

    > [!NOTE]
    > 子活動圖表必須在其上有一些項目，否則圖表將不會出現在 [行為] 屬性的下拉式清單中。 此外，戟圖示不會出現在您的 [呼叫行為動作] 圖形上，直到您設定其 [行為] 屬性為止。

5. 設定動作的 [Is Synchronous] 屬性，表示您的活動是否等待呼叫的活動完成。

    - 如果您將 [Is Synchronous] 設為 false，即表示流程可以在呼叫的活動完成之前就繼續下一個動作。 您不應該定義動作的輸出連接或傳出的資料流程。
...
- 在子活動圖表中，針對呼叫動作上的每個輸入和輸出連接，建立 [活動參數節點] (2)。 提供每個節點與其對應連接相同的名稱。

  > [!NOTE]
  > 活動參數節點類似於物件節點。 若要檢查您正在查看的節點類型，請以滑鼠右鍵按一下節點，然後按一下 [屬性]。 節點類型會顯示在 [屬性] 視窗的標頭中。

- 在子活動圖表中繪製連接器，顯示流入或流出每個活動參數節點的物件流程。

  ![呼叫行為上的 pin 對應至活動參數](../modeling/media/uml-actguidesub.png "UML_ActGuideSub")
...
- 您可以設定動作的 [Is Synchronous] 屬性，表示您的活動是否等待作業完成。

  - 如果您將 [Is Synchronous] 設為 false，即表示流程可以在呼叫的作業完成之前就繼續下一個動作。 您不應該定義動作的輸出連接或傳出的資料流程。

## <a name="Concurrent"></a>並行流程
 您可以使用 [分岔節點] 和 [聯結節點] 來描述可以同時執行之活動的兩個或多個執行緒。

 ![[分叉] 和 [聯結] 節點會顯示並行流程](../modeling/media/uml-actguideconcurrent.png "UML_ActGuideConcurrent")

 [分岔節點] (1) 的效果是將控制權的執行緒分成兩個或多個執行緒。 前一個動作結束時，分岔輸出端的所有動作便可以開始。

 [聯結節點] (2) 會結合並行執行緒。 [聯結節點] 之後的動作必須等到導向 [聯結節點] 的所有動作都已完成才會開始。
...
- 您可以顯示動作的傳出資料流程，以顯示您的活動如何處理訊號中收到的資料。 如果您想要顯示多個輸出流程，您應該設定接受事件動作的 [IsUnmarshall] 屬性，這表示動作會將傳入的訊號剖析為個別的元件。 如需詳細資訊，請參閱[描述資料流程](#DataFlows)。
...
- 選取會描述如何將僅一部分進入流程的物件傳送至接收的動作。
  ...
  ![在另一個圖表中定義的物件轉換](../modeling/media/uml-actguidetransform.png "UML_ActGuideTransform")
...
1. 建立新的活動圖表，以描述轉換或選取流程。
   ...
   - 在 [方案總管] 中，以滑鼠右鍵按一下專案，並指向 [加入]，然後依序按一下 [新增項目] 和 [活動圖表]。 為圖表提供適當的名稱，以表示轉換或選取流程。 按一下 [加入]。
...
3. 在您要使用轉換或選取的任何圖表中：
   ...
   3. 在 [轉換] 或 [選取] 屬性中，選取您指定轉換或選取流程的圖表。

   您也可以為物件節點，以及在個別的輸入和輸出連接上定義選取。 如同上一個程序所示來定義選取活動，然後設定物件節點或輸入或輸出連接的 [Selection] 屬性。
