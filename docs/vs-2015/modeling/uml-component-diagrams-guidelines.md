...
在 Visual Studio 中，您可以繪製 *元件圖* 來顯示軟體系統的結構。 如需影片示範，請參閱 [使用元件圖設計實體結構](https://channel9.msdn.com/blogs/clinted/uml-with-vs-2010-part-6-designing-a-projects-physical-structure)。
 ...
 元件是模組單位，可在其環境內進行取代。 它的內部是隱藏的，但它有一或多個定義完善的 *介面* ，可讓您存取它的函式。 元件也可以有 *必要的介面*。 需要的介面會定義其需要的其他元件的函式或服務。 透過連接數個元件之提供的介面和需要的介面，可以建構較大型元件。 完整軟體系統可以視為元件。
...
## <a name="relationship-to-other-diagrams"></a><a name="OtherDiagrams"></a> 與其他圖表的關聯性
 您可以與其他圖表一起使用元件圖。

|其他圖表|協助您討論和溝通設計的以下方面|
|-------------------|--------------------------------------------------------------------|
|UML 循序圖表|-系統元件之間的互動<br />-元件內各元件之間的互動。<br /><br /> 如需詳細資訊，請參閱 [UML 順序圖表：指導方針](../modeling/uml-sequence-diagrams-guidelines.md)。|
|UML 類別圖表|-元件的介面。 類別圖可讓您將詳細說明介面的方法。<br />-在元件介面上以參數傳送的資料。<br /><br /> 如需詳細資訊，請參閱 [UML 類別圖：方針](../modeling/uml-class-diagrams-guidelines.md)。|
|活動圖|-元件為了回應傳入訊息所執行的內部處理。<br /><br /> 如需詳細資訊，請參閱 [UML 活動圖：方針](../modeling/uml-activity-diagrams-guidelines.md)。|
|圖層圖表|-元件的邏輯架構層。<br /><br /> 如需詳細資訊，請參閱 [分層圖：參考](../modeling/layer-diagrams-reference.md)。|

## <a name="basic-steps-for-drawing-component-diagrams"></a><a name="Basics"></a> 繪製元件圖的基本步驟
 如需元件圖上元素的參考資訊，請參閱 [UML 元件圖：參考](../modeling/uml-component-diagrams-reference.md)。

 如需如何在設計流程中使用元件圖的詳細資訊，請參閱建立 [應用程式架構模型](../modeling/model-your-app-s-architecture.md)。
 ...
 *埠* (2，3) 代表傳入或傳出元件的一組訊息或作業呼叫。 該群組可在介面中進行說明，而介面會定義連接埠的類型。 連接埠可以提供介面或要求介面。
 ...
 具有 *提供之介面* (2) 提供元件所執行的作業，以及可供其他元件使用的埠。
...
- 複製和貼上圖表中介面的參考。
    ...
    2. 在元件圖上，以滑鼠右鍵按一下元件，然後按一下 [ **貼上參考**]。
        ...
        > [!NOTE]
        > 如果您使用 [ **貼** 上] 而非 [ **貼上參考**]，則會建立具有新名稱的新介面。

    3. 如果您想要建立必要的介面，請按一下動作標籤，然後按一下 [ **轉換為所需的介面**]。

## <a name="showing-the-internal-parts-of-a-component"></a><a name="Parts"></a> 顯示元件的內部部分
 ...
 在父元件內，每個組件都會顯示針對其類型 (4、5) 定義的提供的介面和需要的介面。 一個組件需要的作業或服務可由其他組件提供。 您可以使用 **元件的元件** 連接器來顯示元件如何彼此連接 (6) 。
...
1. 針對視為父元件一部分的每個主要功能單元，建立組件。
    ...
    1. 按一下 [工具箱] 中的 [ **元件** ]，然後按一下父元件 (1) 內。

         新組件 (3) 隨即出現在父元件內。

         在 [ **UML 模型瀏覽器**] 中建立新的元件。 這是新組件的類型。
...
2. 將組件連接至另一個組件。
    ...
    - 使用相依 **性工具，** 將不同元件的埠連接 (6) 。

3. 將組件連接至父元件的連接埠：

    1. 在父元件上建立一個或多個連接埠 (7)。 按一下 [工具箱] 上的 [ **必要介面** ] 或 [ **提供的介面** ]，然後按一下父元件。

    2. 將連接埠委派 (9) 至一個或多個組件。 按一下 [ **委派** ] 工具，再按一下父元件上的埠，然後按一下元件上的埠。 您可以使用相同的方式，來連接提供或需要介面的連接埠。
...
2. 在 [ **UML 模型瀏覽器**] 中找出元件的類型。
     ...
     按一下 [ **View**]，指向 [ **其他視窗**]，然後按一下 [ **UML 模型瀏覽器**]。
...
## <a name="designing-the-component"></a><a name="Designing"></a> 設計元件
...
- 如果工作分佈太廣泛，且具有許多互動，則系統效能可能會變慢，且難於掌握。
  ...
  ![順序圖表，顯示共同合作的組件](../modeling/media/uml-compdescparts.png "UML_CompDescParts")

##### <a name="to-draw-a-sequence-diagram-that-shows-collaboration-between-parts"></a>若要繪製順序圖表，以顯示組件之間的共同合作

1. 建立新的順序圖表。

     如需詳細資訊，請參閱 [UML 順序圖表：指導方針](../modeling/uml-sequence-diagrams-guidelines.md)。

2. 建立將訊息傳送至這個元件的外部元件、使用者、裝置或其他行動 (1) 的生命線。

     您 **可以將此生命線的 [動作** 專案] 屬性設定為 [true]，以指出它是在所考慮元件的外部。 生命線上隨即出現桿狀圖形。
...
### <a name="is-the-component-more-than-its-parts"></a>元件多於組件嗎？
 ...
 您可以藉由設定元件的 [間接具 **現化** ] 屬性，在模型中指出這一點。 在此情況下，所有元件的介面都應在連接埠上，並委派至內部組件。
...
### <a name="describing-the-process-inside-each-part"></a>說明每個組件內的程序
 ...
 ![包含資料緩衝區的活動圖表](../modeling/media/uml-compdescribingproc.png "UML_CompDescribingProc")
...
### <a name="general-dependencies-between-components"></a>元件之間的一般相依性
 ...
 使用相依 **性工具來** 繪製相依性。 這表示一個元件的設計依賴於另一個元件。
...
- 一個元件使用其他元件建立的資訊。
  ...
  您可以使用相依性箭號的名稱，來表示特定類型的用法。 若要設定名稱，請以滑鼠右鍵按一下箭號，然後按一下 [ **屬性**]，然後在 [屬性] 視窗中設定 [ **名稱** ] 欄位。

## <a name="see-also"></a>另請參閱
 [編輯 uml 模型和圖表](../modeling/edit-uml-models-and-diagrams.md) [uml 元件圖：參考](../modeling/uml-component-diagrams-reference.md) [uml 順序圖表](../modeling/uml-sequence-diagrams-reference.md)：參考 uml [使用案例](../modeling/uml-use-case-diagrams-reference.md)圖：參考 uml [類別圖表](../modeling/uml-class-diagrams-reference.md)：參考 Uml [元件圖表](../modeling/uml-component-diagrams-reference.md)：參考 [影片：使用元件圖設計實體結構](https://channel9.msdn.com/blogs/clinted/uml-with-vs-2010-part-6-designing-a-projects-physical-structure)
