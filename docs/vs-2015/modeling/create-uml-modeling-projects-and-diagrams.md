...
1. 在**方案總管**中，按一下 [模型專案] 節點。
    ...
    > [!NOTE]
    > 模型專案包含名為**ModelDefinition**的模型定義資料夾。
...
4. 輸入圖表的名稱，然後按一下 [**新增**]。
    ...
    > [!CAUTION]
    > 不要加入、複製，或將現有的圖表檔案拖曳至其他模型專案或方案中其他位置。 這會造成所複製圖表中的項目消失，或在開啟圖表時發生錯誤。 您必須從其中所建立的模型專案中開啟圖表。 這是因為 UML 圖表是其模型專案所擁有的模型檢視。 若要複製圖表檔案，請建立新的圖表，然後將來源圖表中的項目複製到新的圖表。 如需詳細資訊，請參閱針對[模型專案和圖表進行疑難排解](#TroubleshootingModelingProjects)。
...
4. 將專案命名為，並在 [**名稱**] 和 [**位置**] 方塊中指定位置。

5. 在 [**方案**] 方塊中，選取 [新增**至方案**]，將新專案新增至您已經開啟的方案;或者，**建立新的解決方案**以關閉任何開啟的方案，並將專案新增至新的方案。
...
- 在**方案總管**中，以滑鼠右鍵按一下代表圖表的主要檔案，然後按一下 [**刪除**]。
     ...
     圖表會從專案和檔案系統中移除。 圖表上顯示的元素不會從**UML 模型 Explorer**中移除。
...
- 在**方案總管**中，以滑鼠右鍵按一下圖表檔案，然後按一下 [**從專案排除**]。

     圖表隨即從專案中移除。 它並不會從檔案系統中移除。

    > [!NOTE]
    > 圖表上顯示的元素不會從**UML 模型 Explorer**中移除。
