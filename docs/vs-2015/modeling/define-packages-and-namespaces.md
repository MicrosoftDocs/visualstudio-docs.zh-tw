...
在 Visual Studio 中，*封裝*是 UML 元素定義的容器，例如類別、使用案例和元件。 套件也可以包含其他套件。
...
- 設定圖表的**LinkedPackage**屬性，讓圖表上建立的新專案放在您指定的封裝內。 使用這種方式，可以將類別圖、元件圖和使用案例圖連結至套件。

- 將項目移入或移出 [UML 模型總管] 中的套件。

  套件中的項目會出現在 [UML 模型總管] 中的套件，而其限定名稱的開頭是套件的限定名稱。 若要查看任何元素的限定名稱，請以滑鼠右鍵按一下專案，然後按一下 [**屬性**]。 [**限定名稱**] 屬性會出現在 [**屬性**] 視窗中。

#### <a name="to-create-an-element-in-a-package-in-uml-model-explorer"></a>在 UML 模型總管的套件中建立項目

1. 開啟 [ **UML 模型瀏覽器**]。 在 [ **View** ] 功能表上，指向 [**其他視窗**]，然後按一下 [ **UML 模型 Explorer**]。

2. 以滑鼠右鍵按一下您要加入新項目的套件或模型。

3. 指向 [**新增**]，然後按一下您要新增的專案類型。
...
1. 開啟套件所在的類別圖。
    ...
    - 若要讓現有的套件出現在類別圖上，您可以將封裝從 [ **UML 模型瀏覽器**] 拖曳至類別圖。
...
5. 在圖表中建立新項目。 這些將會放置到套件中。
    ...
    - 每個專案的**限定名稱**會以封裝的限定名稱作為開頭。

    - 在 [ **UML 模型瀏覽器**] 中，每個專案都會出現在套件下方。

## <a name="Moving"></a>將元素移入和移出封裝
 您可以將一或多個項目移入或移出套件。
...
2. 以滑鼠右鍵按一下套件，然後按一下 [**貼**上]。

    > [!NOTE]
    > 套件可以在不同的圖表上。

## <a name="Import"></a>匯入套件之間的關聯性
 您可以使用 [匯**入**] 工具，在封裝之間定義匯入關聯性。

 匯入表示所匯入套件中定義的項目 (即關聯性箭號端的項目) 也有效地定義於匯入中套件。 在匯入套件中也會顯示其可見度定義為**套件**的任何元素。

 請避免在匯入關聯性中建立迴圈。

## <a name="References"></a>從一個命名空間到另一個命名空間的參考
...
- 使用完整名稱 `SalesCommon::CustomerAddress`，指定屬性的類型。 只有當 `CustomerAddress` 將其**可見度**屬性設定為 [**公用**] 時，才可以執行此動作。
...
|**限定名稱**|*Container* ：： *package name*|完整名稱，但前面加上包含此套件之套件或模型的名稱。 如需詳細資訊，請參閱[命名空間](#Namespaces)。|
|**資料**|(空白)|連結至此套件的設定檔清單。 這些設定檔提供可套用至套件內項目的造型。 如需詳細資訊，請參閱[使用設定檔和造型自訂您的模型](../modeling/customize-your-model-with-profiles-and-stereotypes.md)。|
|**可見度**|**Public**|套件在其父套件外部的可見性。|
|**工作專案**|(空白)|連結的工作項目清單。 如需詳細資訊，請參閱[連結模型元素和工作專案](../modeling/link-model-elements-and-work-items.md)。|
|**定義位置**|(名稱)|儲存套件詳細資料的檔案名稱。 這些檔案位於**ModelDefinition**專案資料夾內。 這項資訊可用於進行原始檔控制。|
|**說明**|(空白)|套件的描述。|
|**型**|(空白)|套用至此套件的造型。 可用的造型清單取決於您為此套件所選擇的設定檔，以及包含它的套件。 如需詳細資訊，請參閱[使用設定檔和造型自訂您的模型](../modeling/customize-your-model-with-profiles-and-stereotypes.md)。|

## <a name="how-packages-are-stored"></a>套件的儲存方式
 當您建立新的封裝時，會在**ModelDefinition**專案資料夾中建立新的 **. uml**檔案。 根模型也是封裝，也會儲存在一個**uml**檔案中。

 此外，每個圖表都會儲存在兩個檔案中，其中一個代表圖表的形狀，而**一個配置**檔案則會記錄圖形的位置。

## <a name="see-also"></a>請參閱
 [編輯 uml 模型和圖表](../modeling/edit-uml-models-and-diagrams.md) [uml 類別圖：參考](../modeling/uml-class-diagrams-reference.md) [UML 類別圖：方針](../modeling/uml-class-diagrams-guidelines.md)[在版本控制下管理模型和圖表](../modeling/manage-models-and-diagrams-under-version-control.md)
