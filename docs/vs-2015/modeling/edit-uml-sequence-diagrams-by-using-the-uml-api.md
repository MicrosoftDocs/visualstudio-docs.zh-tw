...
互動是一組的生命線之間的訊息序列。 互動會顯示在 UML 循序圖上。
 ...
 如需 API 的完整詳細資訊，請參閱 [VisualStudio](/previous-versions/dd493373(v=vs.140))。

 如需撰寫 UML 圖表的命令和軌跡處理常式的一般簡介，請參閱 [在模型圖表上定義功能表命令](../modeling/define-a-menu-command-on-a-modeling-diagram.md)。
...
- `ISequenceDiagram.Layout([SequenceDiagramLayoutKinds])` 重新繪製整個圖表。 您可以使用參數來指定生命線、訊息或兩者的重新調整位置。
