---
title: UML 類別圖表：方針 |Microsoft Docs
ms.date: 11/15/2016
...
在 Visual Studio 中，您可以使用 *UML 類別圖* 來描述資料類型及其關聯性，而不是它們的實作為。 該圖表可用來重點描述類別的邏輯方面，而非實作。
...
## <a name="using-uml-class-diagrams"></a><a name="Using"></a> 使用 UML 類別圖
 您可以將 UML 類別圖用於各種目的：
...
### <a name="relationship-to-other-diagrams"></a>與其他圖表的關聯性
 ...
 請參閱 [UML 順序圖表：指導方針](../modeling/uml-sequence-diagrams-guidelines.md)。
...
## <a name="basic-steps-for-drawing-class-diagrams"></a><a name="BasicSteps"></a> 繪製類別圖的基本步驟
...
4. 在 [ **加入至模型專案**] 中，選取方案中的現有模型專案，或 **建立新的模型專案**，然後選擇 **[確定]**。

     新的類別圖隨即出現，其中包含 **UMLClass 圖** 工具箱。 工具箱包含需要的項目和關聯。

#### <a name="to-draw-a-uml-class-diagram"></a>若要繪製 UML 類別圖

1. 若要建立類型，請選擇 [工具箱] 上的 [ **類別**]、[ **介面** ] 或 [ **列舉** ] 工具，然後按一下圖表的空白部分。 (如果看不到工具箱，請按 CTRL+ALT+X)。

2. 若要將屬性或作業加入至類型，或將常值加入至列舉，請選擇類型中的 [ **屬性**]、[ **作業** ] 或 [ **常** 值] 標題，然後按 enter。

     您可以撰寫簽章，例如 `f(x:Boolean):Integer`。 請參閱 [屬性和作業](#AttributesAndOperations)。

     若要快速加入數個項目，請在加入每個項目後按兩次 ENTER 鍵。 您可以使用方向鍵在清單中上下移動。

3. 若要展開或摺疊類型，請選擇左上角的＞形箭號圖示。 您也可以展開和折迭類別或介面的 [ **屬性** ] 和 [ **作業** ] 區段。

4. 若要繪製類型之間的關聯、繼承或相依性連結，請按一下適當的工具，然後按一下來源類型和目標類型。

5. 若要在封裝中建立類型，請使用 [ **封裝** ] 工具建立封裝，然後在套件內建立新的類型和套件。 您也可以使用複製命令來複製類型，並在封裝中貼上它們。

6. 每個圖表都是相同專案中其他圖表之間共用之模型的檢視。 若要查看完整模型的樹狀檢視，請選擇 [ **view**]、[ **其他視窗**]、[ **UML 模型瀏覽器**]。

## <a name="using-classes-interfaces-and-enumerations"></a><a name="UsingTypes"></a> 使用類別、介面和列舉
 工具箱中可以使用三種標準類型的 Classifier。 在這份檔中，這些都稱為 *類型* 。

 ![類別、列舉和介面](../modeling/media/uml-classguidetypes.png "UML_ClassGuideTypes")

- 使用 **類別** (1) 來代表大部分用途的資料或物件類型。

- 使用 **介面** (2) 在您必須區分純介面和具有內部執行之實體類別的內容中。 當圖表的用途是用於說明軟體實作時，此區分非常有用。 當建立被動資料模型，或正在定義用來說明使用者要求的概念時，它不是很有用。

- 使用 **列舉** (3) 來代表具有有限的常值數目的型別，例如 `Stop` 和 `Go` 。

  - 將常值加入至列舉。 為每個項目提供不同的名稱。

  - 需要的話，也可以針對每個常值提供數值。 開啟列舉中之常值的快捷方式功能表，選擇 [**屬性**]，然後在 [**屬性**] 視窗的 [**值**] 欄位中輸入數位。
...
### <a name="getting-types-from-other-diagrams"></a>從其他圖表取得類型
 ...
 您可以讓類別出現在多個 UML 類別圖上。 當您在某個圖表上建立類別時，請從 [ **UML 模型瀏覽器** ] 將該類別拖曳至其他圖表。
 ...
 如果您已在元件圖中的元件上定義介面，您可以將介面從 [ **UML 模型瀏覽器** ] 拖曳至類別圖。 在類別圖中，您可以定義介面中所包含的方法。

 請參閱 [UML 元件圖：方針](../modeling/uml-component-diagrams-guidelines.md)。

 UML 循序圖表

 您可以從順序圖表中的生命線建立類別和介面，然後將類別從 **Uml 模型瀏覽器** 拖曳至 uml 類別圖。 順序圖表中的每個生命線都表示物件、元件或行動的執行個體。

 若要從生命線建立類別，請開啟生命線的快捷方式功能表，然後選擇 [ **建立類別** ] 或 [ **建立介面**]。 請參閱 [UML 順序圖表：指導方針](../modeling/uml-sequence-diagrams-guidelines.md)。

## <a name="attributes-and-operations"></a><a name="AttributesAndOperations"></a> 屬性和作業
 屬性 (4) 是類型的每個執行個體可以具有的已命名的值。 存取屬性不會變更執行個體的狀態。

 作業 (5) 是類型的執行個體可以執行的方法或函式。 它會傳回值。 如果其 **isQuery** 屬性為 true，則無法變更實例的狀態。

 若要將屬性或作業加入至類型，請開啟類型的快捷方式功能表，選擇 [ **加入**]，然後選擇 [ **屬性** ] 或 [ **操作**]。

 若要查看其屬性，請開啟屬性或作業的快捷方式功能表，然後選擇 [ **屬性**]。 屬性會出現在 [ **屬性** ] 視窗中。

 若要查看作業參數的屬性，請選擇 <strong>[...]</strong>在 [ **參數** ] 屬性中。 新的屬性對話方塊隨即出現。
...
### <a name="types-of-attributes-and-operations"></a>屬性和作業的類型
 每種 *類型* 的屬性或運算，以及每個參數類型都可以是下列其中一項：

- ** (無) ** -您可以省略前面的冒號 () ，讓簽章中未指定的類型 `:` 。

- 其中一個標準基本類型： **布林值**、 **整數**、 **字串**。

- 在模型中定義的類型。

- 範本類型的參數化值，寫入的範本 \<Parameter> 。 請參閱 [範本類型](#Templates)。

  您也可以撰寫尚未在模型中定義的類型名稱。 名稱將列在 [UML 模型瀏覽器] 的 [ **未指定的類型** ] 之下。
...
- **IsOrdered** -如果為 true，表示集合具有已定義的順序。

- **IsUnique** -如果為 true，則集合中沒有重複的值。
 ...
 **~**
 ...
 只能在包含這個類型的套件中和明確匯入它的任何套件中存取。 請參閱 [定義命名空間和套件](#Packages)。
...
```
 ...
 通常僅在編輯簽章時，圖表上才會顯示完整簽章。 完成編輯時，會隱藏其他屬性。 如果您想要隨時查看完整簽章，請開啟類型的快捷方式功能表，然後選擇 [ **顯示完整**簽章]。

## <a name="drawing-and-using-associations"></a><a name="Associations"></a> 繪圖和使用關聯
...
### <a name="properties-of-an-association"></a>關聯的屬性
 建立關聯之後，需設定其屬性。 開啟關聯的快捷方式功能表，然後選擇 [ **屬性**]。

 除了關聯的屬性之外，每個 *角色*（也就是關聯的每個端點）都有自己的屬性。 若要加以查看，請展開 [ **第一個角色** ] 和 [ **第二個角色** ] 屬性。

 每個角色的部分屬性都會直接顯示在圖表中。 如下所示：

- 角色名稱。 這會顯示在圖表上關聯的適當一端。 您可以在圖表上或在 [ **屬性** ] 視窗中進行設定。

- **多重性**，預設為 **1**。 這也會顯示在圖表上關聯的適當一端。

- **匯總**。 這會以菱形圖案顯示在連接器的一端。 您可以使用它來指出彙總角色上的執行個體擁有或包含其他項目的執行個體。

- **可**流覽。 如果只有一個角色為 true，則在巡覽方向中顯示箭號。 您可以使用它來指出軟體中連結和資料庫關聯的巡覽性。
...
### <a name="navigability"></a>巡覽性
 當您繪製關聯時，它的一端會有箭頭，表示可在該方向巡覽關聯。 如果類別圖代表軟體類別，而關聯代表指標或參考，這會很有用。 不過，使用類別圖代表實體和關聯或商務概念時，它與代表巡覽性比較不相關。 在這種情況下，您可能會偏好繪製不帶箭頭的關聯。 您可以藉由將關聯兩端的 [ **可** 導覽] 屬性設定為 [True]。
...
### <a name="attributes-and-associations"></a>屬性和關聯
 ...
 ![相等的關聯和屬性](../modeling/media/uml-classguideattrib.png "UML_ClassGuideAttrib")

## <a name="inheritance"></a><a name="Inheritance"></a> 繼承
 使用 [ **繼承** ] 工具來建立下列關聯性：

- 特製 *化* 型別和一般型別之間的一般化關聯性
...
2. 按一下 [動作] 標籤，然後按一下 [覆 **寫操作**]。

    [覆 **寫作業** ] 對話方塊隨即出現。

3. 選取您要顯示在特製化類型中的作業，然後按一下 **[確定]**。
...
### <a name="realization"></a>實現
 ...
 建立實現關聯性之後，可以將其轉換為棒棒糖標記法。 以滑鼠右鍵按一下關聯性，然後選擇 [ **顯示為糖形**]。

 這可讓您顯示類別實作的介面，而不會在類別圖上顯示很多實現連結而使圖表凌亂不堪。 您也可以在個別圖表上顯示實現作業的介面和類別。

 ![與連接器和棒棒糖符號一併顯示的實現](../modeling/media/uml-classguiderealize.png "UML_ClassGuideRealize")

## <a name="template-types"></a><a name="Templates"></a> 範本類型
...
2. 開啟新類型的快捷方式功能表，然後選擇 [ **屬性**]。

3. 在 [**屬性**] 視窗中，按一下 [**範本參數**] 欄位中的 **[...]** 。
...
6. 設定 **參數種類**。 預設值為 **Class**。

7. 如果您希望參數只接受特定基類的衍生類別，請將 **限制值** 設定為您想要的基類。

8. 依您的需要新增多個參數，然後選擇 **[確定]**。

9. 如您在其他類別中所執行動作，將屬性和作業加入至範本類型。

     您可以在屬性和作業的定義中使用類型為 **類別**、 **介面** 或 **列舉** 的參數。 例如，透過使用參數類別 `Key` 和 `Value`，可以在 `Dictionary` 中定義下列作業：

     `Get(k : Key) : Value`

     您可以使用類型為 **整數** 的參數做為多重性的系結。 例如，參數整數最大值可以用來定義屬性的多重性為 `[0..max]`。
...
3. 在 [ **範本** 系結] 屬性中，從下拉式清單中選取範本類型（例如 `Dictionary` ）。
...
## <a name="generating-code-from-uml-class-diagrams"></a><a name="generating"></a> 從 UML 類別圖表產生程式碼
 若要開始實作 UML 類別圖上的類別，您可以產生 C# 程式碼或自訂程式碼產生的範本。 使用提供的 C# 範本來產生程式碼：

- 開啟圖表或專案的快捷方式功能表，選擇 [ **產生程式碼**]，然後設定必要的屬性。

     如需如何設定這些屬性和自訂所提供範本的詳細資訊，請參閱 [從 UML 類別圖表產生程式碼](../modeling/generate-code-from-uml-class-diagrams.md)。

## <a name="see-also"></a>另請參閱
 [編輯 uml 模型和圖表](../modeling/edit-uml-models-and-diagrams.md) [uml 類別圖表：參考](../modeling/uml-class-diagrams-reference.md)[模型使用者需求](../modeling/model-user-requirements.md) [Uml 元件圖：參考](../modeling/uml-component-diagrams-reference.md)Uml[順序圖表：參考](../modeling/uml-sequence-diagrams-reference.md)uml[使用案例圖](../modeling/uml-use-case-diagrams-reference.md)：參考 uml[元件圖表：](../modeling/uml-component-diagrams-reference.md)參考
