...
2. 若要將屬性或作業加入至類型或列舉的常值，請選擇類型中的 [**屬性**]、[**作業**] 或 [**常**值] 標題，然後按 enter 鍵。
...
5. 若要在封裝中建立類型，請使用 [**封裝**] 工具建立封裝，然後在封裝內建立新的類型和套件。 您也可以使用複製命令來複製類型，並在封裝中貼上它們。

6. 每個圖表都是相同專案中其他圖表之間共用之模型的檢視。 若要查看完整模型的樹狀檢視，請選擇 [ **view**]、[**其他視窗**]、[ **UML 模型瀏覽器**]。

## <a name="UsingTypes"></a>使用類別、介面和列舉
 工具箱中可以使用三種標準類型的 Classifier。 這些在這份檔中稱為「*類型*」。

 ![類別、列舉和介面](../modeling/media/uml-classguidetypes.png "UML_ClassGuideTypes")

- 針對大部分用途，請使用**類別**（1）來代表資料或物件類型。

- 在您必須區別具有內部部署的純介面和具象類別的內容中使用**介面**（2）。 當圖表的用途是用於說明軟體實作時，此區分非常有用。 當建立被動資料模型，或正在定義用來說明使用者要求的概念時，它不是很有用。

- 使用**列舉**（3）來代表具有有限數目之常值的類型，例如 `Stop` 和 `Go`。

  - 將常值加入至列舉。 為每個項目提供不同的名稱。

  - 需要的話，也可以針對每個常值提供數值。 在列舉中開啟常值的快捷方式功能表，選擇 [**屬性**]，然後在 [**屬性**] 視窗的 [**值**] 欄位中輸入一個數位。
...
### <a name="getting-types-from-other-diagrams"></a>從其他圖表取得類型
 ...
 您可以從順序圖表中的生命線建立類別和介面，然後從 [ **Uml 模型瀏覽器**] 將該類別拖曳至 uml 類別圖。 順序圖表中的每個生命線都表示物件、元件或行動的執行個體。

 若要從生命線建立類別，請開啟生命線的快捷方式功能表，然後選擇 [**建立類別**] 或 [**建立介面**]。 請參閱[UML 順序圖表：方針](../modeling/uml-sequence-diagrams-guidelines.md)。

## <a name="AttributesAndOperations"></a>屬性和作業
 屬性 (4) 是類型的每個執行個體可以具有的已命名的值。 存取屬性不會變更執行個體的狀態。

 作業 (5) 是類型的執行個體可以執行的方法或函式。 它會傳回值。 如果其**isQuery**屬性為 true，就無法變更實例的狀態。

 若要將屬性或作業加入至類型，請開啟類型的快捷方式功能表，選擇 [**新增**]，然後選擇 [**屬性**] 或 [作業 **]。**
...
### <a name="types-of-attributes-and-operations"></a>屬性和作業的類型
 每種*類型*的屬性或作業以及每個參數類型，都可以是下列其中一項：

- **（無）** -您可以省略前面的冒號（`:`），在簽章中保留未指定的類型。
...
### <a name="properties-of-an-association"></a>關聯的屬性
 ...
 除了關聯的整體屬性外，每個*角色*（也就是關聯的每個端點）都有自己的一些屬性。 若要查看它們，請展開**第一個角色**和**第二個角色**屬性。

 每個角色的部分屬性都會直接顯示在圖表中。 分別為：

- 角色名稱。 這會顯示在圖表上關聯的適當一端。 您可以在圖表上或在 [**屬性**] 視窗中設定它。
...
2. 按一下 [動作] 標記，然後按一下 [覆**寫作業**]。

    [覆**寫作業**] 對話方塊隨即出現。

3. 選取您想要顯示在特製化類型中的作業，然後按一下 **[確定]** 。
...
### <a name="realization"></a>實現
 ...
 建立實現關聯性之後，可以將其轉換為棒棒糖標記法。 以滑鼠右鍵按一下關聯性，然後選擇 [**顯示為棒糖**]。
...
7. 如果您想要讓參數只接受特定基類的衍生類別，請將 [**限制值**] 設定為您想要的基類。

8. 視需要新增多個參數，然後選擇 **[確定]** 。

9. 如您在其他類別中所執行動作，將屬性和作業加入至範本類型。

     在屬性和作業的定義中，您可以使用其種類為**類別**、**介面**或**列舉**的參數。 例如，透過使用參數類別 `Key` 和 `Value`，可以在 `Dictionary` 中定義下列作業：

     `Get(k : Key) : Value`

     您可以使用其類型為**整數**的參數，做為多重性的系結。 例如，參數整數最大值可以用來定義屬性的多重性為 `[0..max]`。
...
- 開啟圖表或元素的快捷方式功能表，選擇 [**產生程式碼**]，然後設定必要的屬性。
