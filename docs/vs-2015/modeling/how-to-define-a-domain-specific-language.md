...
2. 按一下每一個範本可查看說明。 選擇與您要建立的方案最相似的方案。  
   ...
   - 選取 **元件模型**來建立具有連接埠的 DSL。 連接埠是較大圖形邊緣上的小圖形。  
...
3. 在適當的精靈頁面中輸入 DSL 的副檔名。 這是包含 DSL 將使用之執行個體的檔案的副檔名。  
   ...
   - 請選擇未與您公司中的任何應用程式相關聯的副檔名，或是未與您要安裝 DSL 之任何電腦中的任何應用程式相關聯的副檔名。 例如， **docx**並**htm**是無法接受的檔案名稱副檔名。  
...
### <a name="test-the-solution"></a>測試方案  
 ...
 如果您執行解決方案，您已建立從**最小語言**範本，您的實驗性[!INCLUDE[vsprvs](../includes/vsprvs-md.md)]將類似下列的範例：  
...
- 根類別出現在 DSL 定義圖左上底下**類別和關聯性**。 將它重新命名為 DSL 以外的名稱。 例如，名為 DSL **MusicLibrary**可能會有一個名為根類別**音樂**。  
  
- 在圖表類別會出現在 DSL 定義圖右下角**圖表項目**資料行。 您可能必須捲動到右側才能看到它。 它通常會命名為_您的 Dsl_**圖表**。  
  
- 如果您使用**工作流程**範本，並且想要建立具有泳道的圖表、 保留和重新命名 Actor 網域類別和 ActorSwimlane 圖形。  
...
## <a name="classes"></a> 定義網域類別  
 ...
 若要建立網域類別，您可以從拖曳**具名網域類別**圖表中，工具，然後重新命名的類別。  
  
 如需詳細資訊，請參閱 <<c0> [ 屬性的網域類別](../modeling/properties-of-domain-classes.md)。  
...
|不直接顯示。|其形成部分所屬的類別。||  
 ...
 若要建立網域類別和其內嵌在此同時，請按一下**內嵌關聯性**工具，然後按一下父類別，然後按一下圖表的空白部分。  
 ...
 **設定項目名稱屬性。** 選取網域屬性，可用來識別語言總管中的項目。 例如，在可以選取 Title 網域屬性的 Song 網域類別中。 在 **屬性**視窗中，將**Is Element Name**到`true`。  
...
### <a name="create-derived-domain-classes"></a>建立衍生的網域類別  
 ...
 請考慮將**繼承修飾詞**的基底類別**抽象**。 如果您認為您可能需要基底類別的執行個體，請考慮為它們建立分開的衍生類別。  
  
 衍生類別繼承其基底類別的屬性和角色。  
  
### <a name="tidy-the-dsl-definition-diagram"></a>整理 DSL 定義圖  
 加入關聯性時，部分類別將會出現在多個地方。 若要減少外觀數並使圖表寬一些，以滑鼠右鍵按一下 目標類別的關聯性，然後**此處將樹狀結構**。 如需相反的效果，請以滑鼠右鍵按一下目標類別的關聯性，然後按一下 **分割樹狀結構**。 如果沒有看到這些功能表命令，請確定是否只有選取網域類別。  
...
3. **開啟 [總管] 中。** 在圖表的側邊是語言總管視窗中，通常名為*YourLanguage*總管。 如果您沒有看到這個視窗，可能是在 [方案總管] 下方的索引標籤上。 如果您找不到的話**檢視**功能表上，指向**其他 Windows**，然後按一下  _YourLanguage_**總管**。  
  
     您的總管隨即呈現模型的樹狀檢視。  
  
4. **建立新的項目。** 以滑鼠右鍵按一下頂端的根節點，然後按一下**加入新**_YourClass_。  
  
     您的語言總管中隨即出現類別的新執行個體。  
  
5. 請確認您建立新執行個體時，每一個執行個體都有不同的名稱。 這在您已設定時，才會發生**Is Element Name**網域屬性上的旗標。  
  
6. **檢查網域屬性。您選取的類別的執行個體**檢查 [屬性] 視窗。 它應顯示您在此網域類別上所定義的網域屬性。  
  
7. **儲存檔案、 關閉它，並重新開啟**。 在展開節點之後，總管中應該會看到您所建立的所有執行個體。  
  
## <a name="shapes"></a> 在圖表上定義的圖形  
...
1. **定義和測試網域類別中所述**[定義網域類別](#classes) **。**  
   ...
   - 如果您的圖表具有泳道，父系可以是對應到泳道的網域類別。 繼續進行此程序之前，請參閱[定義具有泳道的 DSL](#swimlanes)。  
  
2. **加入圖形類別**來代表在模型圖上的項目。 從下列其中一個工具拖曳到 DSL 定義圖上：  
...
3. **如果您建立了影像圖形定義映像，** 。  
   ...
   5. 在 **選取映像**對話方塊方塊中，按一下  下的下拉式清單功能表**檔案名稱**，選取的映像。  
  
4. **將文字裝飾項目新增至圖形，以顯示網域屬性。**  
  
    若要顯示模型項目的名稱或標題，您可能需要至少一個文字裝飾項目。  
  
    以滑鼠右鍵按一下圖形類別的標頭，指向**新增**，然後按一下**文字裝飾項目**。 設定的名稱裝飾項目，然後在 [屬性] 視窗設定其**位置**。  
  
5. **每個圖形連接至網域類別，它應該會顯示的圖表項目對應與**。  
  
    按一下 **圖表項目對應**工具，然後按一下 網域類別，然後按一下 shape 類別。  
  
6. **將屬性對應至文字的裝飾項目。**  
  
   1. 選取網域類別與代表圖表項目對應的圖形類別之間的灰線。  
  
   2. 在 [ **DSL 詳細資料**] 視窗中，按一下**裝飾項目對應**] 索引標籤。如果您看不見**DSL 詳細資料**視窗，請在**檢視**功能表上，指向**其他 Windows** ，然後按一下 [ **DSL 詳細資料**。 經常需要提高此視窗的頂端才能查看其所有內容。  
  
   3. 選取裝飾項目的名稱。 底下**顯示屬性**，選取網域類別的屬性名稱。 針對每一個裝飾項目重複此動作。  
  
       如果您想要顯示的相關項目屬性，按一下下拉式樹狀巡覽器底下**顯示屬性的路徑**。  
  
   4. 請確定核取記號隨每一個裝飾項目名稱出現。  
  
      ![圖形對應和 DSL 詳細資料 視窗](../modeling/media/dsldetailswindow.png "DslDetailsWindow")  
  
7. **請建立網域類別元素的工具箱項目。**  
  
   1. 在  **DSL Explorer**，展開**編輯器**節點及其所有子節點。  
   ...
   5. 設定**工具箱圖示**圖示將出現在工具箱中。 您可以將它設定為新圖示或已用於另一個工具的圖示。  
  
        若要建立新的圖示，開啟 [Dsl\Resources 中的**方案總管] 中**。 複製並貼上其中一個現有的項目工具 BMP 檔案。 將貼上的複本重新命名，然後按兩下加以編輯。  
  
        返回 DSL 定義圖表、 選取工具，並在 [屬性] 視窗中按一下 **[…]** 中**工具箱圖示**。 在 [**選取點陣圖**] 對話方塊中，選取您。從下拉式選單的 BMP 檔案。  
  
   如需詳細資訊，請參閱 <<c0> [ 幾何圖案的屬性](../modeling/properties-of-geometry-shapes.md)並[影像圖案的屬性](../modeling/properties-of-image-shapes.md)。  
...
5. **確認每一個文字裝飾項目會出現，** 而且：  
   ...
   先測試圖形之後，您可能想要調整其部分屬性以及加入一些其他的進階功能。 如需詳細資訊，請參閱 <<c0> [ 自訂及擴充特定領域語言](../modeling/customizing-and-extending-a-domain-specific-language.md)。  
 ...
 **調整每個角色的多重性**，則為必要。 如果您要每一位人員最多有一位上司，請將顯示在圖表上 Manager 標籤下方的多重性設為 0..1。  
  
 **您可以將網域屬性加入關聯性。** 在圖中，Artist-album 關聯性具有角色的屬性。  
  
 **設定允許重複的屬性關聯性，** 如果相同的模型項目配對之間有一個以上的連結，相同的類別。 例如，您可以允許老師向同一位學生教授多個科目。  
  
 ![圖形對應的連接器](../modeling/media/music-connector.png "Music_Connector")  
...
### <a name="define-a-connector-to-display-the-relationship"></a>定義連接器以顯示關聯性  
 ...
 拖曳**連接器**工具拖曳到 DSL 定義圖。  
  
 如果您要在連接器上顯示標籤，請加入文字裝飾項目。 設定它們的位置。 若要讓使用者移動文字裝飾項目，設定其**Is Moveable**屬性。  
  
 使用**圖表項目對應**工具，以將連接器連結到參考關聯性。  
  
 選取圖表項目對應，開啟**DSL 詳細資料**視窗，然後開啟**裝飾項目對應** 索引標籤。  
  
 選取每個**裝飾項目**並設定**顯示屬性**至正確的網域屬性。  
  
 請確定在每個項目旁邊出現核取記號**裝飾項目**清單。  
  
### <a name="define-a-connection-builder-tool"></a>定義連接產生器工具  
 在 [ **DSL Explorer** ] 視窗中，展開**編輯器**節點和所有子節點。  
  
 以滑鼠右鍵按一下具有您的 DSL 中，同名的節點，然後按一下**新增連接工具**。  
  
 已選取新工具時，在 [屬性] 視窗中：  
  
- 設定**Caption**並**工具提示**。  
  
- 按一下 **連接產生器**，然後選取適當的產生器，為新的關聯性。  
  
- 設定**工具箱圖示**至您想要出現在工具箱中的圖示。 您可以將它設定為新圖示或已用於另一個工具的圖示。  
...
6. **請確認您無法建立不適當的類別之間的連線。** 例如，如果您的關聯性介於 Album 與 Artist，請確認您無法將 Artist 連結到 Artist。  
  
7. **確認多重性正確無誤。例如，驗證，您無法將人員連接至一個以上的管理員。**  
...
## <a name="compartments"></a> 定義包含清單的圖形：區間圖形  
 ...
 如需詳細資訊，請參閱 <<c0> [ 屬性的區間圖形](../modeling/properties-of-compartment-shapes.md)。  
...
1. **建立容器網域類別**。 按一下 **內嵌關聯性**工具中，按一下模型的根類別，然後按一下 DSL 定義圖表的空白部分。 這會在範例圖中建立名為 Album 的網域類別。  
     ...
     之類名稱的網域屬性加入至類別，並設定其**Is Element Name**屬性 視窗中的旗標。  
  
2. **建立清單項目網域類別**。 按一下 **內嵌關聯性**工具中，按一下容器類別 (Album)，然後按一下圖表的空白部分。 這會在範例圖中建立名為 Song 的網域類別。  
  
     新增標題至類別，之類的網域屬性並設定其**Is Element Name**旗標。  
  
     加入其他網域屬性。  
  
     為您要顯示的每一份清單加入另一個清單項目。  
  
3. **若要混合使用數種類型的清單中的項目**，建立繼承自清單類別的類別。 藉由設定，使清單類別抽象及其**繼承修飾詞**。  
  
     例如，如果您要依作者 (而非演奏者) 排序古典音樂，您可以建立 Song 的兩個子類別 (ClassicalSong 和 NonClassicalSong)。  
  
4. **建立區間圖形**。 從拖曳**區間圖形**工具拖曳到 DSL 定義圖。  
...
6. 按一下 **圖表項目對應**工具，按一下容器網域類別，然後按一下 區間圖形。  
  
7. 選取網域類別與圖形之間的圖表項目對應連結。 在  **DSL 詳細資料**視窗：  
  
    1. 按一下 **裝飾項目** 索引標籤。按一下 裝飾項目的名稱，然後選取適當的項目底下**Display 屬性**。 請確定裝飾項目名稱旁邊出現核取記號。  
  
    2. 按一下 [**區間對應**] 索引標籤。  
  
         按一下區間的名稱。  
  
         底下**顯示的項目集合路徑**，瀏覽至清單項目類別 (Song)。 按一下下拉箭號以使用巡覽器工具。  
...
2. 在  **DSL Explorer**，展開**編輯器**節點及其所有子節點。  
...
6. 設定**工具箱圖示**圖示將出現在工具箱中。 您可以將它設定為新圖示或已用於另一個工具的圖示。  
  
     若要建立新的圖示，開啟 [Dsl\Resources 中的**方案總管] 中**。 複製並貼上其中一個現有的項目工具 .BMP 檔案。 將貼上的複本重新命名，然後按兩下加以編輯。  
  
     返回 DSL 定義圖表、 選取工具，並在 [屬性] 視窗中按一下 **[…]** 中**工具箱圖示**。 在 **選取點陣圖**對話方塊方塊中，從下拉式選單中選取 BMP 檔案。  
...
7. 開啟語言總管。 確認您可以看到內部有清單項目節點的容器節點。  
   ...
   先測試區間圖形之後，您可能想要調整其部分屬性以及加入一些其他的進階功能。 如需詳細資訊，請參閱 <<c0> [ 自訂及擴充特定領域語言](../modeling/customizing-and-extending-a-domain-specific-language.md)。  
...
1. **將區間加入區間圖形**。 在 DSL 定義圖表中，以滑鼠右鍵按一下區間圖形類別，指向**新增**，然後按一下**區間**。  
  
2. 設定**顯示的項目集合路徑**瀏覽至該連結，而不是其目標項目。 按一下下拉式功能表，並使用樹狀檢視選取參考關聯性而非其目標。 在此範例中，關聯性是**ArtistAppearedOnAlbums**。  
  
3. 設定**顯示屬性的路徑**連結從瀏覽至目標項目。 在此範例中，這是**演出者**。  
  
4. 設定**Display 屬性**目標項目，如範例的適當屬性**名稱**。  
...
## <a name="ports"></a> 定義在另一個圖形界限上的連接埠  
 連接埠是位於另一個圖形界限上的圖形。  
...
- 有一個從容器網域類別到連接埠網域類別的內嵌關聯性。 如需詳細資訊，請參閱 <<c0> [ 定義網域類別](#classes)。  
  
- 如果您要在同一個容器上混合不同類型的連接埠，可以建立連接埠網域類別的子類別。 在範例中，`InPort` 和 `OutPort` 皆繼承自 `ComponentPort`。  
  
- 容器網域類別可以對應到任何類型的圖形。 在範例中，此圖形為 `ComponentShape`。 如需詳細資訊，請參閱 <<c0> [ 定義圖形](#shapes)。  
  
- 連接埠網域類別會對應到連接埠圖形。 您可以將衍生的類別對應到分開的連接埠圖形類別，或是將基底類別對應到一個連接埠圖形類別。  
  
  在其他方面，連接埠圖形的行為中所述[定義圖形](#shapes)。  
  
  如需詳細資訊，請參閱 <<c0> [ 屬性的通訊埠圖案](../modeling/properties-of-port-shapes.md)。  
...
## <a name="swimlanes"></a> 定義具有泳道的 DSL  
 ...
 如需詳細資訊，請參閱 <<c0> [ 屬性的泳道](../modeling/properties-of-swimlanes.md)。  
...
### <a name="domain-enumerations-and-literals"></a>網域列舉和常值  
 ...
 若要加入網域列舉，以滑鼠右鍵按一下模型中的根**DSL Explorer** ，然後按一下 **新增新的網域列舉**。 項目會出現在**DSL Explorer**下方**網域類型**節點。 此項目不會出現在圖形上。  
  
 您可以將列舉常數加入網域列舉，請以滑鼠右鍵按一下中的網域列舉**DSL 總管]** ，然後按一下 [**新增新的列舉常數**。  
  
 根據預設，具有列舉類型的屬性一次只能設定為一個列舉值。 如果您想要能夠設定任何值-組合的 使用者和程式設計人員要設定 「 位元欄位-" **IsFlags**列舉型別的屬性。  
  
### <a name="external-types"></a>外部類型  
 當您設定的領域屬性的類型，如果找不到類型時，您想**型別**下拉式清單中，您可以加入外部類型。 例如，您可以在其中加入**System.Drawing.Color**類型加入清單。  
  
 加入類型，以滑鼠右鍵按一下 [DSL 總管] 中，模型的根，然後按一下**加入新的外部類型**。 在 [屬性] 視窗中，將名稱設定為**色彩**以及要命名空間**System.Drawing**。 這個類型現在會出現在 [DSL 總管] 的下方**網域類型**。 每當設定網域屬性的類型時，您都可以選擇它。  
...
1. 使用 DSL 定義的更多功能來微調 DSL。 例如，您可以製作可建立數種類型連接器的單一連接器工具，並且可以控制規則，透過該規則，刪除一個項目也會刪除相關項目。 這些技巧大部分是透過設定 DSL 定義中的值來達成，而某些技巧則需要幾行程式碼。  
     ...
     如需詳細資訊，請參閱 <<c0> [ 自訂及擴充特定領域語言](../modeling/customizing-and-extending-a-domain-specific-language.md)。  
  
2. 使用程式碼來達成更進階的效果，即可擴充模型工具。 例如，您可以建立可變更模型的功能表命令，並建立能整合兩個或多個 DSL 的工具。 VMSDK 是為了能讓您輕鬆整合擴充功能與從 DSL 定義產生的程式碼而專門設計的。  如需詳細資訊，請參閱 <<c0> [ 來自訂特定領域語言撰寫的程式碼](../modeling/writing-code-to-customise-a-domain-specific-language.md)。  
...
|問題|建議|  
|-------------|----------------|  
|我在 DSL 定義檔案中所做的變更沒有任何作用。|按一下 [**轉換所有範本**方案總管] 中，然後重建方案上方工具列中。|  
|圖形顯示裝飾項目的名稱而非屬性值。|設定裝飾項目對應。 在 DSL 定義圖上，按一下圖表項目對應，此對應為網域類別與圖形類別之間的灰線。<br /><br /> 開啟**DSL 詳細資料**視窗。 如果您無法看到它，在 [檢視] 功能表上，指向**其他的 Windows**，然後按一下**DSL 詳細資料**。<br /><br /> 按一下 [**裝飾項目對應**] 索引標籤。選取裝飾項目的名稱。 請確定已核取它旁邊的方塊。 底下**顯示屬性**，選取網域屬性的名稱。<br /><br /> 如需詳細資訊，請參閱 <<c0> [ 圖表上的圖形](#shapes)。|  
