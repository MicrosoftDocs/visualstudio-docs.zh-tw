...
1. 使用 [網域指定的語言]範本建立方案，該範本位於 [新增專案]對話方塊中的 [其他專案類型/擴充性]之下。

    ![建立 DSL 對話方塊](../modeling/media/create-dsldialog.png "Create_DSLDialog")

    當您按一下 [確定]時，[網域指定的語言精靈]隨即開啟並顯示範本 DSL 方案清單。
...
2. 按一下每一個範本可查看說明。 選擇與您要建立的方案最相似的方案。
   ...
   - 選取 [元件模型]以建立具有連接埠的 DSL。 連接埠是較大圖形邊緣上的小圖形。

   - 選取 [類別圖]以定義具有區間圖形的 DSL。 區間圖形包含項目清單。

   - 在其他情況下或不確定時，請選取 [最小語言]。

       > [!NOTE]
       > 如果您要建立類別圖或元件圖，請考慮使用 UML 模型。 UML 模型工具提供一組整合在單一模型周圍的圖表。 它們可以擴充，並且可以使用 ModelBus 與 DSL 整合。 如需詳細資訊，請參閱為[您的應用程式建立模型](../modeling/create-models-for-your-app.md)。

   - 選取 [最小 WinForm 設計工具]或 [最小 WPF 設計工具]以建立 Windows Forms 或 WPF 介面上顯示的 DSL。 您將必須撰寫程式碼以定義編輯器。 如需詳細資訊，請參閱下列主題：
...
### <a name="test-the-solution"></a>測試方案
 ...
 若要測試方案，請按 F5 或 CTRL+F5。 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的新執行個體隨即在實驗模式中開啟。

 在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的新執行個體中，在 [方案總管] 開啟範例檔案。 此檔案會開啟為具有工具箱的圖表。

 如果您執行從 [最小語言]範本建立的方案，則實驗 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 將類似以下範例：
...
- 根類別出現在 DSL 定義圖左上角的 [類別和關聯性]之下。 將它重新命名為 DSL 以外的名稱。 例如，名為**MusicLibrary**的 DSL 可能會有一個名為「**音樂**」的根類別。

- 圖表類別顯示在 DSL 定義圖右下角的 [圖表項目]資料行。 您可能必須捲動到右側才能看到它。 它通常名為 _YourDsl_**圖表**。

- 如果您使用 [工作流程]範本，並且想要建立具有泳道的圖表，請保留並重新命名 Actor 網域類別和 ActorSwimlane 圖形。
...
> [!NOTE]
> 在加入功能之後，請不要忘了按一下 [方案總管] 工具列中的 [轉換所有範本]，然後再建置和執行 DSL。
 ...
 ![產生之 DSL 的實例模型](../modeling/media/music-instance.png "Music_Instance")

> [!NOTE]
> 「模型」是指使用者建立的 DSL 執行個體，通常會顯示為圖表。 本主題同時討論 DSL 定義圖以及使用 DSL 時出現的模型圖表。

## <a name="classes"></a>定義網域類別
 網域類別代表 DSL 的概念。 執行個體為 *「模型項目」* (model elements)。 例如，在**MusicLibrary** DSL 中，您可能會有名為**專輯**和**歌曲**的網域類別。

 若要建立網域類別，可以從 [具名網域類別]工具拖曳到圖表，然後重新命名類別。
...
|不直接顯示。|其形成部分所屬的類別。||
 ...
 若要同時建立網域類別及其內嵌，請按一下 [內嵌關聯性]工具，然後按一下父類別，然後再按一下圖表的空白部分。
...
### <a name="add-domain-properties-to-each-domain-class"></a>將網域屬性加入每一個網域類別
 ...
 在類別中按一下 [網域屬性]，按 ENTER 鍵，然後輸入屬性的名稱。 網域屬性的預設類型為字串。 如果您要變更類型，請選取網域屬性，然後在 [屬性]視窗中設定 [類型]。 如果您要的類型不在下拉式清單中，請參閱[加入屬性類型](#addTypes)。
...
### <a name="create-derived-domain-classes"></a>建立衍生的網域類別
 ...
 按一下 [繼承]工具，按一下衍生類別，然後按一下基底類別。

 考慮將基底類別的 [繼承修修飾詞]設為 [抽象]。 如果您認為您可能需要基底類別的執行個體，請考慮為它們建立分開的衍生類別。

 衍生類別繼承其基底類別的屬性和角色。

### <a name="tidy-the-dsl-definition-diagram"></a>整理 DSL 定義圖
 加入關聯性時，部分類別將會出現在多個地方。 若要減少外觀數並使圖表寬一些，請以滑鼠右鍵按一下關聯性的目標類別，然後按一下 [將樹狀結構帶到這裡]。 如需相反效果，請以滑鼠右鍵按一下關聯性的目標類別，然後按一下 [分割樹狀結構]。 如果沒有看到這些功能表命令，請確定是否只有選取網域類別。
...
1. 在 [方案總管] 的工具列中**按一下轉換所有範本**，即可產生 DSL 設計工具程式碼。 您可以自動化此步驟。 如需詳細資訊，請參閱[如何自動化轉換所有範本](https://msdn.microsoft.com/b63cfe20-fe5e-47cc-9506-59b29bca768a)。

2. **建立並執行 DSL。** 按 F5 或 CTRL + F5，以實驗模式執行 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的新實例。 在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的實驗執行個體中，開啟或建立具有您 DSL 之副檔名的檔案。

3. **開啟 [Explorer]。** 在圖表的側邊是語言瀏覽器視窗，通常名為*YourLanguage* explorer。 如果您沒有看到這個視窗，可能是在 [方案總管] 下方的索引標籤上。 如果找不到它，請在 [檢視]功能表上指向 [其他視窗]，然後按一下 _YourLanguage_**總管**。
...
5. 請確認您建立新執行個體時，每一個執行個體都有不同的名稱。 只有已在網域屬性上設定 [Is Element Name] 旗標時，才會發生此情況。

6. **檢查網域屬性。當您選取類別的實例時，請**檢查屬性視窗。 它應顯示您在此網域類別上所定義的網域屬性。

7. **儲存檔案，關閉它，然後重新開啟它**。 在展開節點之後，總管中應該會看到您所建立的所有執行個體。
...
1. **定義和測試網域類別，如**  [定義網域類別](#classes)中所述 **。**
   ...
   - 如果您的圖表具有泳道，父系可以是對應到泳道的網域類別。 繼續進行這項程序之前，請參閱[定義具有泳道的 DSL](#swimlanes)。

2. **加入圖形類別**以表示模型圖上的項目。 從下列其中一個工具拖曳到 DSL 定義圖上：
...
3. **如果您已建立影像圖形，請定義影像**。
   ...
   4. 在 [屬性] 視窗中，按一下 [影像]屬性。

   5. 在 [選取影像]對話方塊中，按一下 [檔案名稱]之下的下拉式功能表，然後選取影像。

4. **將文字裝飾專案加入至圖形，以顯示定義域屬性。**

    若要顯示模型項目的名稱或標題，您可能需要至少一個文字裝飾項目。

    以滑鼠右鍵按一下圖形類別的標頭，指向 [加入]，然後按一下 [文字裝飾項目]。 設定裝飾項目的名稱，然後在 [屬性] 視窗中設定其 [位置]。

5. **將每一個具有 [圖表項目對應] 的圖形連接到它應顯示的網域類別**。

    按一下 [圖表項目對應]工具，然後按一下網域類別，然後按一下圖形類別。
...
6. **將屬性對應至文字裝飾專案。**
   ...
   3. 選取裝飾項目的名稱。 在 [顯示屬性]之下，選取網域類別的屬性名稱。 針對每一個裝飾項目重複此動作。

       如果您想要顯示相關項目的屬性，請按一下 [顯示屬性的路徑]之下的下拉式樹狀巡覽器。
...
7. **建立 [工具箱] 專案，以建立網域類別的元素。**
   ...
   2. 以滑鼠右鍵按一下 [工具箱索引標籤]之下與 DSL 具有相同名稱的節點，例如 MusicLibrary。 按一下 [加入項目工具]。

       > [!NOTE]
       > 如果以滑鼠右鍵按一下 [工具]節點，將不會看到 [加入項目工具]。 請改按一下其上方的節點。

   3. 在選取了新項目工具的 [屬性] 視窗中，將 [類別]設為您最近加入的網域類別。

   4. 設定 [標題]和 [工具提示]。

   5. 將**工具提示圖示**設為將出現在工具箱中的圖示。 您可以將它設定為新圖示或已用於另一個工具的圖示。

        若要建立新圖示，請在 [方案總管]中開啟 [Dsl\資源]。 複製並貼上其中一個現有的項目工具 BMP 檔案。 將貼上的複本重新命名，然後按兩下加以編輯。

        返回 DSL 定義圖，選取工具，然後在 [屬性] 視窗中，按一下**工具箱圖示**中的 **[...]** 。 在 [選取點陣圖]對話方塊中的下拉式功能表中，選取 .BMP 檔案。
...
1. 在 [方案總管] 的工具列中**按一下轉換所有範本**，即可產生 DSL 設計工具程式碼。
...
5. **確認每個文字**裝飾專案都出現，而且：
   ...
   1. 您可以編輯它，除非您已在網域屬性上設定 [Is UI Read Only]旗標。
 ...
 **設定關聯性的 [允許重複專案] 屬性（** 如果同一個類別的多個連結可以存在於同一對模型專案之間）。 例如，您可以允許老師向同一位學生教授多個科目。
 ...
 ![連接器的圖形地圖](../modeling/media/music-connector.png "Music_Connector")
...
### <a name="define-a-connector-to-display-the-relationship"></a>定義連接器以顯示關聯性
 ...
 如果您要在連接器上顯示標籤，請加入文字裝飾項目。 設定它們的位置。 若要讓使用者移動文字裝飾項目，請設定其 [Is Moveable]屬性。

 使用 [圖表項目對應]工具將連接器連結到參考關聯性。

 在已選取圖表項目對應下，開啟 [DSL 詳細資料]視窗，然後開啟 [裝飾項目對應]索引標籤。

 選取每一個 [裝飾項目]，並將 [顯示屬性]設為正確的網域屬性。

 請確定 [裝飾項目]清單中的每一個項目旁邊出現核取記號。

### <a name="define-a-connection-builder-tool"></a>定義連接產生器工具
 在 [DSL Explorer]視窗中，展開 [編輯器]節點及其所有子節點。

 以滑鼠右鍵按一下名稱與 DSL 相同的節點，然後按一下 [加入新的連接工具]。

 已選取新工具時，在 [屬性] 視窗中：

- 設定 [標題]和 [工具提示]。

- 按一下 [連接產生器]，然後為新關聯性選取適當的產生器。

- 將**工具提示圖示**設為要出現在工具箱中的圖示。 您可以將它設定為新圖示或已用於另一個工具的圖示。
...
5. 在圖形之間**建立連接**。 按一下連接器工具，按一下圖形，然後按一下另一個圖形。
...
## <a name="compartments"></a>定義包含清單的圖形：區間圖形
 ...
 ![圖形地圖](../modeling/media/music-mapcomp.png "Music_MapComp")

 如需詳細資訊，請參閱區間[圖形的屬性](../modeling/properties-of-compartment-shapes.md)。

#### <a name="to-define-a-compartment-shape"></a>定義區間圖形

1. **建立容器網域類別**。 按一下 [內嵌關聯性]工具，按一下模型的根類別，然後按一下 DSL 定義圖的空白部分。 這會在範例圖中建立名為 Album 的網域類別。

     您可以將容器內嵌於對應到泳道的網域類別中，而不是內嵌於根類別中。

     將 Name 之類的網域屬性加入類別，並在 [屬性] 視窗中設定其 [Is Element Name]旗標。

2. **建立清單項目網域類別**。 按一下 [內嵌關聯性]工具，按一下容器類別 (Album)，然後按一下圖表的空白部分。 這會在範例圖中建立名為 Song 的網域類別。

     將 Title 之類的網域屬性加入類別，並設定其 [Is Element Name]旗標。

     加入其他網域屬性。

     為您要顯示的每一份清單加入另一個清單項目。

3. **若要在清單中混用數種類型的專案**，請建立繼承自清單類別的類別。 設定清單類別的 [繼承修飾詞]以使其成為抽象類別。

     例如，如果您要依作者 (而非演奏者) 排序古典音樂，您可以建立 Song 的兩個子類別 (ClassicalSong 和 NonClassicalSong)。

4. **建立區間圖形**。 從 [區間圖形]工具拖曳到 DSL 定義圖上。

     加入文字裝飾項目並設定其名稱。

     加入區間並設定其名稱。

5. 若要讓使用者隱藏清單區間，請以滑鼠右鍵按一下圖形類別，指向 [加入]，然後按一下 [展開/摺疊裝飾項目]。 在 [屬性] 視窗中，設定裝飾項目的位置。

6. 按一下[圖表項目對應]工具，按一下容器網域類別，然後按一下區間圖形。
...
7. 選取網域類別與圖形之間的圖表項目對應連結。 在 [DSL 詳細資料]視窗中：
    ...
    2. 按一下 [區間對應]索引標籤。
         ...
         在 [顯示的項目集合路徑]之下，巡覽至清單項目類別 (Song)。 按一下下拉箭號以使用巡覽器工具。

         在 [顯示屬性]之下，選取應顯示在清單中的屬性。 在範例中，此屬性為 Title。
...
3. 以滑鼠右鍵按一下 [工具箱索引標籤]之下與 DSL 具有相同名稱的節點，例如 MusicLibrary。 按一下 [加入項目工具]。

    > [!NOTE]
    > 如果以滑鼠右鍵按一下 [工具]節點，將不會看到 [加入項目工具]。 請改按一下其上方的節點。

4. 在選取了新項目工具的 [屬性] 視窗中，將 [類別]設為您最近加入的網域類別。

5. 設定 [標題]和 [工具提示]。

6. 將**工具提示圖示**設為將出現在工具箱中的圖示。 您可以將它設定為新圖示或已用於另一個工具的圖示。
...
5. 以滑鼠右鍵按一下新圖形的標頭，然後按一下加入*您的清單項目。* 在此範例中， 命令為 Add Song。
...
7. 開啟語言總管。 確認您可以看到內部有清單項目節點的容器節點。
   ...
   ![已產生的 DSL explorer](../modeling/media/music-explorer.png "Music_Explorer")
...
2. 將 [顯示的項目集合路徑]設為巡覽至連結而非其目標項目。 按一下下拉式功能表，並使用樹狀檢視選取參考關聯性而非其目標。 在此範例中，關聯性為**ArtistAppearedOnAlbums**。

3. 將 [顯示屬性的路徑]設為從連結巡覽至目標項目。 在範例中，這是 [**演出者**]。
...
## <a name="ports"></a>定義另一個圖形界限上的埠
 ...
 若要查看使用連接埠的範例，請在建立新的 DSL 方案時選取 [元件圖]範本。 此範例顯示定義連接埠時可以考慮的要點：
...
- 連接埠網域類別會對應到連接埠圖形。 您可以將衍生的類別對應到分開的連接埠圖形類別，或是將基底類別對應到一個連接埠圖形類別。
  ...
  在其他層面中，連接埠圖形的行為方式如[定義圖形](#shapes)中所述。
...
### <a name="domain-enumerations-and-literals"></a>網域列舉和常值
 ...
 若要加入網域列舉，請以滑鼠右鍵按一下 [DSL Explorer]中模型的根，然後按一下 [加入新的網域列舉]。 該項目將出現在 [DSL Explorer]中的 [網域類型]節點之下。 此項目不會出現在圖形上。

 若要將列舉常數加入網域列舉，請以滑鼠右鍵按一下 [DSL Explorer]中的網域列舉，然後按一下 [加入新的列舉常數]。

 根據預設，具有列舉類型的屬性一次只能設定為一個列舉值。 如果您要使用者和程式設計人員能夠設定任何值組合 (「位元欄位」)，請設定列舉的 [IsFlags]屬性。

### <a name="external-types"></a>外部類型
 設定網域屬性的類型時，如果在 [類型]下拉式清單中找不到您要的類型，您可以加入外部類型。 例如，您可以將**system.web. Color**類型加入至清單中。

 若要加入類型，請以滑鼠右鍵按一下 [DSL Explorer] 中模型的根，然後按一下 [加入新的外部類型]。 在屬性視窗中，將名稱設定為**Color** ，並將命名空間設為**system.web**。 這時，此類型會出現在 [DSL Explorer] 中的 [網域類型]之下。 每當設定網域屬性的類型時，您都可以選擇它。
...
2. 使用程式碼來達成更進階的效果，即可擴充模型工具。 例如，您可以建立可變更模型的功能表命令，並建立能整合兩個或多個 DSL 的工具。 VMSDK 是為了能讓您輕鬆整合擴充功能與從 DSL 定義產生的程式碼而專門設計的。  如需詳細資訊，請參閱[撰寫程式碼以自訂域特定語言](../modeling/writing-code-to-customise-a-domain-specific-language.md)。
...
|問題|建議|
|-------------|----------------|
|我在 DSL 定義檔案中所做的變更沒有任何作用。|在 [方案總管] 上方的工具列中按一下 [轉換所有範本]，然後重建方案。|
|圖形顯示裝飾項目的名稱而非屬性值。|設定裝飾項目對應。 在 DSL 定義圖上，按一下圖表項目對應，此對應為網域類別與圖形類別之間的灰線。<br /><br /> 開啟 [DSL 詳細資料]視窗。 如果看不到它，請在 [檢視] 功能表上指向 [其他視窗]，然後按一下 [DSL 詳細資料]。<br /><br /> 按一下 [裝飾專案**對應**] 索引標籤。選取裝飾專案的名稱。 請確定已核取它旁邊的方塊。 在 [顯示屬性]之下，選取網域屬性的名稱。<br /><br /> 如需詳細資訊，請參閱[圖表上的圖形](#shapes)。|
...
|在我的 DSL 總管中，項目只會與其類型名稱一起顯示。|在 DSL 定義中，選取類別的網域屬性，然後在 [屬性] 視窗中，將 [Is Element Name]設為 true。|
|My DSL 一律在 XML 編輯器中開啟。|這種情況的發生原因是在讀取檔案時所發生的錯誤。 然而，即使在修正該錯誤之後，您都必須明確地將編輯器重設為 DSL 設計工具。<br /><br /> 以滑鼠右鍵按一下專案項目，按一下 [開啟方式]，然後選取 _YourLanguage_**設計工具 (預設值)** 。|
