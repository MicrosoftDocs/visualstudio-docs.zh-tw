---
title: 自訂元素的建立和移動 |Microsoft Docs
ms.date: 11/15/2016
...
- 您的程式碼會呼叫 element merge 指示詞。
  ...
  ![DSL&#45;EMD&#95;合併](../modeling/media/dsl-emd-merge.png "DSL-EMD_Merge") Element Merge 指示詞的角色
...
4. 在 [ **建立連結的進程合併**] 下，新增兩個路徑：
   ...
   1. 其中一個路徑會將新元素連結至父模型。 您需要輸入的路徑運算式會從現有的專案中流覽，直到父模型的內嵌關聯性。 最後，它會在新的連結中指定要指派新元素的角色。 路徑如下所示：

       `ExampleModelHasElements.ExampleModel/!ExampleModel/.Elements`

   2. 另一個路徑會將新元素連結至現有的專案。 路徑運算式會指定參考關聯性，以及將指派新元素的角色。 此路徑如下所示：

       `ExampleElementReferencesTargets.Sources`

      您可以使用路徑流覽工具來建立每個路徑：

   3. 在 [在 **路徑上建立連結的進程合併**] 下，按一下 **\<add path>** 。

   4. 按一下清單專案右邊的下拉箭號。 樹狀檢視隨即出現。

   5. 展開樹狀結構中的節點，以形成您要指定的路徑。
...
5. 測試 DSL：
   ...
   2. 當的實驗實例 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 啟動時，開啟 DSL 的模型檔案。 建立一些範例元素。
...
6. 測試 DSL：
    ...
    1. 按 F5 以重建解決方案。 當的實驗實例開啟時 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ，開啟您的 DSL 實例。
...
1. [!INCLUDE[dsl](../includes/dsl-md.md)]使用元件模型範本建立方案。
...
10. 建置並執行解決方案。 的新實例 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 隨即出現。
...
## <a name="see-also"></a>另請參閱
 在程式碼[自訂工具和工具箱](../modeling/customizing-tools-and-the-toolbox.md)[中流覽和更新模型](../modeling/navigating-and-updating-a-model-in-program-code.md)
