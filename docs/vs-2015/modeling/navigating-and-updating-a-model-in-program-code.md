---
title: 在程式碼中流覽和更新模型 |Microsoft Docs
ms.date: 11/15/2016
...
## <a name="in-this-topic"></a>本主題內容
 ...
 圖形、連接器和圖表以及它們與模型專案的關聯性，會在個別的主題中描述。 如需詳細資訊，請參閱 [如何：流覽和更新圖表](../misc/how-to-navigate-and-update-a-diagram.md)。
...
- 旗標為 true 之角色的元素 `PropagatesDelete` 。 例如，會刪除顯示元素的圖形。
  ...
  依預設，在目標角色上的每個內嵌關聯性都是 `PropagatesDelete` true。 刪除 `henry` 並不會刪除 `familyTree` ，但是 `familyTree.Delete()` 會刪除所有 `Persons` 。 如需詳細資訊，請參閱 [自訂刪除行為](../modeling/customizing-deletion-behavior.md)。
...
|圖表|<xref:Microsoft.VisualStudio.Modeling.Diagrams.Diagram>|
 ...
 圖形可以有兩個集合中的子圖案。 集合中的圖形 `NestedChildShapes` 會限制為其父系的周框方塊。 清單中的圖形 `RelativeChildShapes` 可以出現在父系的外部或部分外部，例如標籤或埠。 圖表沒有 `RelativeChildShapes` 和否 `Parent` 。
...
## <a name="see-also"></a>另請參閱
 <xref:Microsoft.VisualStudio.Modeling.ModelElement>[從特定領域語言產生程式碼](../modeling/generating-code-from-a-domain-specific-language.md)的[特定領域語言驗證](../modeling/validation-in-a-domain-specific-language.md)[如何：使用交易來更新模型](../modeling/how-to-use-transactions-to-update-the-model.md)[整合模型，方法是使用 Visual Studio Modelbus](../modeling/integrating-models-by-using-visual-studio-modelbus.md) [回應和傳播變更](../modeling/responding-to-and-propagating-changes.md)
