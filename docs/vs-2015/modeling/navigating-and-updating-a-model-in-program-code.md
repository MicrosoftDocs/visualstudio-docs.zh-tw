...
## <a name="in-this-topic"></a>本主題內容
 ...
 圖形、連接器和圖表, 以及它們與模型專案的關聯性, 會在另一個主題中加以說明。 如需詳細資訊，請參閱[如何：導覽並更新圖表](../misc/how-to-navigate-and-update-a-diagram.md)。
...
- 位於角色的元素, 其`PropagatesDelete`旗標為 true。 例如, 顯示元素的圖形將會被刪除。
  ...
  根據預設, 每個內嵌關聯`PropagatesDelete`性在目標角色上都為 true。 刪除`henry`並不會`familyTree`刪除`familyTree.Delete()` ,`Persons`但會刪除所有。 如需詳細資訊, 請參閱[自訂刪除行為](../modeling/customizing-deletion-behavior.md)。
...
|圖表|<xref:Microsoft.VisualStudio.Modeling.Diagrams.Diagram>|
 ...
 圖形在兩個集合中可以有子圖形。 `NestedChildShapes`集合中的圖形僅限於其父系的周框方塊。 `RelativeChildShapes`清單中的圖形可能會出現在父代之外或部分外部 (例如標籤或埠)。 圖表`RelativeChildShapes`沒有和否`Parent`。
...
## <a name="see-also"></a>另請參閱
 <xref:Microsoft.VisualStudio.Modeling.ModelElement>[以特定領域語言進行驗證](../modeling/validation-in-a-domain-specific-language.md)[從特定領域語言產生程式碼](../modeling/generating-code-from-a-domain-specific-language.md)[如何:使用交易來更新模型](../modeling/how-to-use-transactions-to-update-the-model.md)整合模式,[方法是使用 Visual Studio Modelbus](../modeling/integrating-models-by-using-visual-studio-modelbus.md) [回應及傳播變更](../modeling/responding-to-and-propagating-changes.md)
