...
ms.locfileid: "72654937"
---
# <a name="customizing-the-properties-window"></a>自訂屬性視窗
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

您可以在 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的特定領域語言（DSL）中，自訂 [屬性] 視窗的外觀和行為。 在您的 DSL 定義中，您會在每個網域類別上定義定義域屬性。 根據預設，當您在圖表上或在 [模型瀏覽器] 中選取類別的實例時，每個定義域屬性都會列在 [屬性] 視窗中。 這可讓您查看和編輯定義域屬性的值，即使您未將它們對應到圖表上的 [圖形] 欄位也一樣。
...
## <a name="forwarding-displaying-properties-of-related-elements"></a>轉送：顯示相關元素的屬性
...
##### <a name="to-forward-a-property-from-another-element"></a>若要從另一個元素轉送屬性

1. 建立包含至少兩個類別（在此範例中稱為**Book**和**Author**）的 [!INCLUDE[dsl](../includes/dsl-md.md)] 解決方案。 **書籍**與**作者**之間應該有任一類型的關聯性。
...
```
 ...
 您可以指定您自己的編輯器，或在 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 中提供的編輯器，例如 <xref:System.Windows.Forms.Design.FileNameEditor> 或 <xref:System.Drawing.Design.ImageEditor>。 例如，使用下列程式來擁有屬性，讓使用者可以在其中輸入檔案名。

##### <a name="to-define-a-file-name-domain-property"></a>若要定義檔案名網域屬性
