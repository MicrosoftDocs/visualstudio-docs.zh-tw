...
@No__t_0 中的文字模板提供了一種實用的方式來產生任何種類的文字。 您可以使用文字模板，在執行時間將文字產生為應用程式的一部分，並在設計階段，產生您的某些專案程式碼。 本主題摘要說明最常詢問的「如何? ...？」 問題.
...
- 如果您想要從僅在執行時間提供的範本產生，您可以使用標準文字模板。 如果您要撰寫 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 的延伸模組，您可以叫用文字模板化服務。 如需詳細資訊，請參閱叫[用 VS 擴充功能中的文字轉換](../modeling/invoking-text-transformation-in-a-vs-extension.md)。 在其他內容中，您可以使用文字模板化引擎。 如需詳細資訊，請參閱<xref:Microsoft.VisualStudio.TextTemplating.Engine?displayProperty=fullName>。
...
### <a name="read-another-project-file-from-a-template"></a>從範本讀取另一個專案檔
 若要從與範本相同的 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 專案讀取檔案：
...
### <a name="invoke-methods-from-a-template"></a>從範本叫用方法
 如果方法已經存在，例如，在標準 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 類別中：
...
- 在個別元件（類別庫）中寫入方法，並從您的範本呼叫它們。 使用 `<#@assembly#>` 指示詞載入元件，並 `<#@import#>` 以設定命名空間內容。 請注意，為了在進行偵錯工具時重建元件，您可能必須停止並重新啟動 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]。 如需詳細資訊，請參閱[T4 文字模板](../modeling/t4-text-template-directives.md)指示詞。
...
- 請考慮從 UML 模型產生程式碼。 程式碼不需要直接反映 UML。 例如，您不需要為 UML 模型中的每個類別產生一個類別。 相反地，您可以使用 UML 類別圖來代表網站，並從每個 UML 類別產生網頁。 選擇最接近您需求的圖表類型。 例如，選擇 [活動圖表] 來代表任何類型的工作流程。 您可以定義造型，將您的應用程式適用的資訊新增至每個專案類型。

     從 UML 模型產生，可以讓您以圖表的形式繪製和編輯模型，但不需要設計自己的圖表類型，就像使用 DSL 一樣。

     如需詳細資訊，請參閱為[您的應用程式建立模型](../modeling/create-models-for-your-app.md)和[從 UML 模型產生](../modeling/generate-files-from-a-uml-model.md)檔案。

### <a name="get-data-from-includevsprvsincludesvsprvs-mdmd"></a>從 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 取得資料
 若要使用 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 中提供的服務，請設定 `hostSpecific` 屬性，並載入 `EnvDTE` 元件。 例如:
...
### <a name="what-is-the-benefit-of-using-text-templates"></a>使用文字模板的優點為何？
 一般來說，您會從一個模型產生多個程式碼或其他檔案。 模型所呈現的需求比產生的程式碼更直接。 它會省略「執行詳細資料」，並根據需求（而不是程式碼）來撰寫。 當需求變更時（如同往常一般），您可以更輕鬆且更可靠地更新模型，而不是程式碼的不同部分。
...
- 這裡所述 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 文字模板功能的另一個名稱。 先前的版本（尚未發行）是「文字模板轉換」的縮寫。
