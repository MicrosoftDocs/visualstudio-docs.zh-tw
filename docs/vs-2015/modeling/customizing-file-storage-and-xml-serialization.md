...
ms.locfileid: "85548014"
---
# <a name="customizing-file-storage-and-xml-serialization"></a>自訂檔案儲存體和 XML 序列化
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

當使用者在中儲存特定領域語言（DSL）的實例或*模型*時 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ，就會建立或更新 XML 檔案。 您可以重載該檔案，以在存放區中重新建立模型。
...
- 每個關聯性會序列化為 XML 節點，並嵌套在關聯性的來源端內。 節點與來源角色屬性的名稱相同，但有小寫的初始字元。
     ...
     例如，在 DSL 定義中，名為**人員**的角色是源自于**FamilyTree**類別。  在 XML 中，這是由節點內名為 nested 的節點表示 `people` `familyTreeModel` 。
...
## <a name="understanding-monikers"></a>瞭解名字
 標記是用來表示模型和圖表檔案的不同部分之間的交互參考。 它們也會在檔案中用 `.diagram` 來參考模型檔案中的節點。 有兩種形式的名字標記：
...
- *限定*的索引鍵標記會依據指定的網域屬性值（稱為「名字」索引鍵）來識別目標元素。 Target 元素的名字元前面會加上內嵌關聯性樹狀結構中其父元素的名字標記。
  ...
  ```
   ...
   如果目標類別具有**Is Moniker Key** `true` 在**Xml 序列化行為**中將選項設為 [標記索引鍵] 的網域屬性，將會使用限定的索引鍵名稱。 在此範例中，此選項會設定為網域類別「專輯」和「歌曲」中名為「標題」的網域屬性。
...
1. 請確定在類別及其基類中的每個網域屬性都是「是」標記索引**鍵** `false` 。

    1. 在 [DSL Explorer] 中，展開 [ **Xml 序列化 Behavior\Class \\ 資料** _\<the domain class>_ **\Element 資料**]。

    2. 確認每個網域屬性的**都是名字標記索引鍵** `false` 。
...
- 針對現有網域類別的網域屬性，Set**是「名字**」索引鍵。 屬性的類型必須是 `string` 。
    ...
    1. 在 [DSL Explorer] 中，展開 [ **Xml 序列化] [Behavior\Class 資料 \\ **] _\<the domain class>_ [**\Element 資料**]，然後選取 [網域] 屬性。

    2. 在屬性視窗中，將 [**為名字標記索引鍵**] 設定為 `true` 。

- \- 或 -

     使用**命名網域類別**工具，建立新的網域類別。

     此工具會建立新的類別，其具有名為 Name 的網域屬性。 **是元素名稱**，而且是此網域屬性的**名字標記索引鍵**屬性，會初始化為 `true` 。
...
- 設定**為**  =  `true` 金鑰網域屬性的元素名稱。 選取 DSL 定義圖表上的 [網域] 屬性，然後在 [屬性視窗中設定值。

     當使用者建立類別的新實例時，這個值會自動將不同的值指派給網域屬性。 預設行為會在類別名稱的結尾加上一個數位。 這不會防止使用者將名稱變更為重複的，但在使用者不會在儲存模型之前設定值時，它會有説明。

- 啟用 DSL 的驗證。 在 [DSL Explorer] 中，選取 [Editor\Validation]，然後將 [**使用 ...** ] 屬性設為 `true` 。
...
```
 ...
 如果您知道特定的網域屬性在模型中一律會有唯一的值，您可以將該屬性的設**為標記辨識符號** `true` 。 這會使其當做辨識符號使用，而不使用父系的標記。 例如，如果您將兩者都設定**為 [標記辨識符號**]，而針對 [專輯] 類別的 [標題網域] 屬性設為 [名字] 索引**鍵**，則模型的名稱或識別碼不會用於專輯的名字和其內嵌的子系：
...
|標記類型名稱|為此類別的專案所產生的 xsd 型別名稱。 XSD 為 Dsl\Generated 程式**代碼 \\ \* 架構 .xsd**|
...
|表示法|如果屬性，則會將屬性序列化為 xml 屬性;如果專案，則會將它序列化為元素;如果忽略，則不會序列化。|
