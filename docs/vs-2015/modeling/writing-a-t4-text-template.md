 ...
 **編輯圖表或表單中的模型檔案。** [!INCLUDE[dsl](../includes/dsl-md.md)] 提供的工具可讓您以圖表或 Windows form 形式來編輯模型。 這樣可以更輕鬆地與所產生應用程式的使用者討論此模型。 [!INCLUDE[dsl](../includes/dsl-md.md)] 也會建立一組強型別類別，以反映模型的結構。 如需詳細資訊，請參閱 [從特定領域語言產生程式碼](../modeling/generating-code-from-a-domain-specific-language.md)。
