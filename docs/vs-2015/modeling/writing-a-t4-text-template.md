---
title: 撰寫 T4 文字模板 |Microsoft Docs
ms.date: 11/15/2016
...
> [!NOTE]
> 當您在複合陳述式（例如迴圈或條件式）內內嵌文字區塊時，請一律使用大括弧 {...} 包含文字區塊。
...
```
 ...
 如需詳細資訊，請參閱[T4 Import](../modeling/t4-import-directive.md)指示詞。

### <a name="Include"></a>包含程式碼和文字
...
```
 ...
 將檔案**載入為可導覽的模型**。 功能較強大的方法是將資料讀取為您文字範本程式碼可以巡覽的模型。 例如，您可以載入 XML 檔案，並使用 XPath 運算式對其進行巡覽。 您也可以使用[xsd.exe](https://docs.microsoft.com/dotnet/standard/serialization/xml-schema-definition-tool-xsd-exe)來建立一組可供您讀取 XML 資料的類別。
