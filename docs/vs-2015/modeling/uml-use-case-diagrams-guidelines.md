...
在 Visual Studio 中，您可以繪製 *「使用案例圖」* (Use Case Diagram)，摘要說明哪些人使用您的應用程式或系統，以及可執行的工作有哪些。 若要建立 UML 使用案例圖，請按一下 [架構] 功能表上的 [新增 UML 或分層圖]。
...
- 您系統的範圍。
  ...
  使用案例圖不會顯示使用案例的詳細資料：這只會摘要說明使用案例、執行者和系統之間的某些關聯性。 具體而言，該圖表不會顯示為了達成每一個使用案例目標所執行步驟的順序。 您可以在連結到每一個使用案例的其他圖表或文件中描述這些詳細資料。 如需詳細資訊，請參閱本主題內的[詳細描述使用案例](#Details)。

  您針對使用案例提供的描述將用到許多與系統執行所在領域相關的詞彙，例如「銷售」、「菜單」、「顧客」等。 請務必清楚定義這些詞彙及其關聯性，您可以透過「UML 類別圖」來協助這方面的工作。 如需詳細資訊，請參閱 [UML 類別圖表：方針](../modeling/uml-class-diagrams-guidelines.md)。
...
- *「執行者」* (Actor) (1) 是與您的系統互動的人員、組織、裝置或外部軟體元件的類別。 範例執行者包括**客戶**、**餐廳**、**溫度感應器**、**信用卡授權者。**

- *「使用案例」* (Use Case) (2) 代表一個或多個執行者為追求特定目標所執行的動作。 使用案例範例包括**訂單飲食**、**更新功能表**、**處理付款**。

   在使用案例圖上，使用案例會與執行使用案例的執行者相關聯 (3)。

- *「系統」* (System) (4) 是您開發中的任何系統。 系統可能是小型軟體元件，其執行者就是其他軟體的元件；它也可能是完整的應用程式，或者是部署到許多電腦或裝置的大型分散式應用程式套件。 範例子系統包括**飲食訂購網站**、**飲食傳遞商務**、**網站版本 2**。
...
1. 從工具箱將 [子系統] 界限拖曳至該圖表上，藉此代表整個系統或是其主要元件。
...
2. 從工具箱將 [執行者] 拖曳至該圖表 (將它們放置在任何系統界限外)。

    - 執行者代表與您系統互動的使用者、組織和外部系統的類別。

    - 重新命名該執行者。 例如： [**客戶]、[餐廳]、[信用卡機構]。**

3. 從工具箱將 [使用案例] 拖曳到適當的系統上。
...
4. 按一下工具箱上的 [關聯]，並按一下使用案例，然後按一下參與此使用案例的執行者。 以此方式將每一個執行者連結到其使用案例。

5. 以 [包含]、[擴充] 和 [一般化] 關聯性建構使用案例。 若要建立這些連結，請依序按一下 [工具]、[來源使用案例] 和 [目標]。 請參閱下列標題為[建構使用案例](#Structuring)的章節。

6. 更加詳細地描述使用案例。 請參閱下列標題為[詳細描述使用案例](#Details)的章節。
...
- 建立 [執行者]，代表與您系統或子系統互動的人員、組織、其他系統、軟體或裝置的類別。
...
1. 以滑鼠右鍵按一下此執行者，然後按一下 [屬性]。

     [屬性] 視窗隨即出現。

2. 將 [影像路徑] 屬性設定為影像檔的位置。
...
### <a name="multiplicities-between-actors-and-use-cases"></a>執行者和使用案例之間的多重性
 執行者和使用案例之間的關聯可在兩端顯示 *「多重性」* (Multiplicity)。

 ![搭配執行者使用案例1](../modeling/media/uml-ucguidemulti1.png "UML_UCGuideMulti1")

> [!NOTE]
> 如果兩者都是 **1**，則會隱藏使用案例圖上關聯的多重性。

 根據預設，每個多重性皆為 **1**。 在嚴謹的模型解譯中，多重性為 1 表示每一種餐點都只有一位顧客訂購，而且每一位顧客一次只訂購一種餐點 (舉例而言)。
...
- 將此使用案例連結至該專案中的其他圖表。
  ...
  - 活動圖可幫助您說明更複雜的流程，其中包含迴圈、分支和平行執行緒。 此外還可顯示部分程序之間的資料流程。 如需詳細資訊，請參閱 [UML 活動圖表：方針](../modeling/uml-activity-diagrams-guidelines.md)。
...
2. 將文件加入方案。
    ...
    2. 在 [方案總管] 中，以滑鼠右鍵按一下此方案，並指向 [加入]，然後按一下 [現有項目]。

    3. 巡覽至此 Word 文件，然後按一下 [加入]。
...
2. 在此工具箱中，按一下 [成品]，然後按一下該使用案例圖。

3. 選取新的成品之後，請將 URL 輸入或貼入 [超連結] 屬性。
...
### <a name="Include"></a>以 Include 顯示使用案例的詳細資料
 使用 [包含] 關聯可顯示某一個使用案例描述另一個使用案例的部分詳細資料。 在圖例中，**訂購食物**包含 [**付款**]、 **[選擇] 功能表**，然後**選擇 [功能表項目**]。 每一項被包含之更加詳細的使用案例都是執行者為了達成此包含的使用案例之整體目標，而可能必須執行的步驟。 箭號應指向更詳細的被包含之使用案例。
...
#### <a name="Steps"></a>定義詳細步驟的順序
 ...
 為釐清步驟的順序，您可以使用 [成品] 將個別文件附加至此包含使用案例。 在下列範例中，會將活動圖附加至「訂購餐點」使用案例。 或者，您可以使用具有步驟清單或一系列螢幕擷取畫面的文字文件。 如需詳細資訊，請參閱[詳細描述使用案例](#Details)。
...
- 活動圖中動作的名稱與被包含使用案例的名稱相同。
  ...
  如需詳細資訊，請參閱 [UML 活動圖表：方針](../modeling/uml-activity-diagrams-guidelines.md)。

  ![連結活動圖表中顯示的使用案例步驟](../modeling/media/uml-ucguidesteps.png "UML_UCGuideSteps")

### <a name="Inheritance"></a>使用一般化共用目標
 對於另一個 *「一般」* (General) 使用案例所表示之目標， *「特製化」* (Specialized) 使用案例是達成該目標的一種特定方式，這可使用 [一般化] 關聯加以顯示。 開放式箭頭應指向更一般的使用案例。
...
3. 將現有使用案例拖入或拖出此子系統中，即可調整其內容。
   ...
   若要直接在子系統中建立新的使用案例，請按一下工具箱中的 [使用案例]，然後按一下此子系統內部。
