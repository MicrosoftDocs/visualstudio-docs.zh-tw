...
在 Visual Studio 中，您可以繪製 *使用案例圖* 來摘要說明使用您的應用程式或系統的人員，以及他們可以使用的功能。 若要建立 UML 使用案例圖，請按一下 [ **架構** ] 功能表上的 [ **新增 UML 或分層圖**]。

 如需影片示範，請參閱 [將功能組織成使用案例](https://channel9.msdn.com/blogs/clinted/uml-with-vs-2010-part-2-organizing-features-into-use-cases)。
...
- 您系統的範圍。
  ...
  使用案例圖不會顯示使用案例的詳細資料：這只會摘要說明使用案例、執行者和系統之間的某些關聯性。 具體而言，該圖表不會顯示為了達成每一個使用案例目標所執行步驟的順序。 您可以在連結到每一個使用案例的其他圖表或文件中描述這些詳細資料。 如需詳細資訊，請參閱本主題中的 [詳細描述使用案例](#Details) 。
...
- 動作 *專案 (1*) 是與您系統互動的人員、組織、裝置或外部軟體元件類別。 範例執行者為 **客戶**、 **餐廳**、 **溫度感應器**、 **信用卡授權者。**

- *使用案例* (2) 代表一或多個執行者為了達成特定目標所執行的動作。 使用案例範例包括 **訂單餐費**、 **更新功能表**、 **處理付款**。

   在使用案例圖上，使用案例會與執行使用案例的執行者相關聯 (3)。

- 您的 *系統 (4) * 是您正在開發的任何內容。 系統可能是小型軟體元件，其執行者就是其他軟體的元件；它也可能是完整的應用程式，或者是部署到許多電腦或裝置的大型分散式應用程式套件。 範例子系統包括 **食物訂購網站**、 **飲食交付公司**、 **網站第2版**。

   使用案例圖可顯示您的系統或其子系統支援哪些使用案例。

## <a name="basic-steps-for-drawing-use-case-diagrams"></a><a name="BasicSteps"></a> 繪製使用案例圖的基本步驟
...
1. 從 [工具箱] 將 **子系統** 界限拖曳至圖表上，以代表整個系統或其主要元件。
...
2. 將 [動作專案] 從 [工具箱] 拖曳到圖表上， (將它們 **放在任何** 系統界限) 之外。

    - 執行者代表與您系統互動的使用者、組織和外部系統的類別。

    - 重新命名該執行者。 例如： **客戶、餐廳、信用卡機構。**

3. 將 [ **使用案例** ] 從 [工具箱] 拖曳到適當的系統上。

    - 使用案例代表執行者藉助您的系統所執行的活動。

    - 使用這些執行者本身能夠了解的標題重新命名該使用案例。 請不要使用與程式碼相關的標題。 例如： **訂購食物、支付餐費、交貨食物**。

    - 從主要的交易（例如 **訂單餐費**）開始，一直到較小的互動，例如 **選取功能表項目**。

    - 將每一個使用案例放置到支援它的系統或主要子系統中 (忽略任何僅涉及連接該使用者的外觀或元件)。

    - 您可以在系統界限外繪製使用案例，表示您的系統 (也許是特定版本或發行) 不支援該使用案例。

4. 按一下 [工具箱] 上的 [ **關聯** ]，然後按一下使用案例，再按一下參與使用案例的執行者。 以此方式將每一個執行者連結到其使用案例。

5. 使用 **Include**、 **擴充** 和 **一般化** 關聯性來建立使用案例的結構。 若要建立這些連結，請依序按一下 [工具]、[來源使用案例] 和 [目標]。 請參閱下一節標題為 [使用案例的結構](#Structuring)。

6. 更加詳細地描述使用案例。 請參閱下列標題 [詳細說明使用案例](#Details)的章節。

7. 分別繪製重點為不同子系統或是相關使用案例之不同群組的圖表。 一個模型專案中的所有圖表為相同模型的檢視。

## <a name="drawing-actors-and-use-cases"></a><a name="Actors"></a> 繪圖執行者和使用案例
 使用案例圖的主要目的在於顯示哪些人與您的系統互動，以及他們藉此達到的主要目標。

- 建立 **動作專案，以** 代表與您的系統或子系統互動的人員、組織、其他系統、軟體或裝置的類別。

  - 若要瞭解如何繪製動作專案和其他元素，請參閱 [編輯 UML 模型和圖表](../modeling/edit-uml-models-and-diagrams.md)。

  - 針對各組相異的目標，依類型或角色識別執行者 (即使實際的人員或實體可能相同)。 例如，「餐廳」和「顧客」是不同的執行者，即使餐廳員工有時可能會是顧客。

- 為每個動作專案所搜尋的每個目標建立 **使用案例** ，以達成系統。
...
### <a name="inheritance-between-actors"></a>執行者之間的繼承
 ...
 您可以在執行者之間繪製 **一般化** 連結。 特製化執行者 (例如在此範例中的「俱樂部顧客」) 會繼承一般化執行者 (例如「顧客」) 的使用案例。 箭頭應指向更一般的執行者，例如「顧客」。 當您建立此連結時，請先指向較為特製化的執行者。
...
### <a name="multiplicities-between-actors-and-use-cases"></a>執行者和使用案例之間的多重性
 動作專案與使用案例之間的關聯可以顯示每一端的 *多重性* 。

 ![包含行動之一對一的使用案例](../modeling/media/uml-ucguidemulti1.png "UML_UCGuideMulti1")

> [!NOTE]
> 在使用案例圖上，如果關聯的多重性是 **1**，則會隱藏。
...
- 若要指出相同類別的數個動作專案可以參與一個使用案例，請將關聯動作專案端的多重性設為**1 ... \* **

   在此圖中，一家或多家餐廳可以參與提供相同餐點訂購服務的工作。

- 若要顯示每個動作專案可以在數個使用案例中同時參與，請將關聯的使用案例結尾的多重性設為 **\*** 。
...
3. 設定多重性屬性，方法是從此清單中選擇：
   ...
   - **1** 表示此角色剛好有一個實例參與每個連結。

   - **1. \* **表示此角色的一個或多個實例參與每個連結。
...
3. 將 [動作專案] 或 [使用案例] 從 [ **模型瀏覽器** ] 拖曳至新圖表。

    > [!NOTE]
    > 如果您將已相關聯的執行者和使用案例放置到新圖表上，則兩者之間的關聯將自動出現在新圖表上。

## <a name="describing-use-cases-in-detail"></a><a name="Details"></a> 詳細描述使用案例
 使用案例代表：

- 使用系統之動作專案的目標，例如 **購買飲食**;和

- 一或多個 *案例*，也就是為了達成目標所執行的步驟順序，例如： {**訂單餐費、付款、交付**}。 除了成功的案例，可能會有幾個例外狀況或失敗案例，例如已 **拒絕的信用卡**。
...
- 將此使用案例連結至該專案中的其他圖表。
  ...
  - 循序圖可幫助您說明不同執行者之間一系列複雜的互動。 您還可以用它來顯示此系統內部對每一個使用案例的回應。 如需詳細資訊，請參閱 [UML 順序圖表：指導方針](../modeling/uml-sequence-diagrams-guidelines.md)。
...
### <a name="linking-use-cases-to-work-items"></a>將使用案例連結至工作項目
 如果您的專案使用， [!INCLUDE[vstsTfsRosarioLong](../includes/vststfsrosariolong-md.md)] 而且您有 [!INCLUDE[esprtfc](../includes/esprtfc-md.md)] ，則可以將每個使用案例連結至中的工作專案 [!INCLUDE[esprfound](../includes/esprfound-md.md)] 。 若要瞭解如何建立這些連結，請參閱 [連結模型專案和工作專案](../modeling/link-model-elements-and-work-items.md)。
...
## <a name="structuring-use-cases"></a><a name="Structuring"></a> 結構化使用案例
...
- [藉助 [包含] 顯示使用案例的詳細資料](#Include)

- [藉助 [一般化] 共用目標](#Inheritance)

- [藉助 [擴充] 將變異的案例分隔](#Extend)

### <a name="showing-the-details-of-a-use-case-with-include"></a><a name="Include"></a> 使用 Include 顯示使用案例的詳細資料
 使用 [ **包含** 關聯性] 來顯示一個使用案例描述另一個使用案例的部分詳細資料。 在圖例中，依 **序** 選擇 [ **付費**]、 **[功能表**] 及 **[選擇功能表項目**]。 每一項被包含之更加詳細的使用案例都是執行者為了達成此包含的使用案例之整體目標，而可能必須執行的步驟。 箭號應指向更詳細的被包含之使用案例。

> [!CAUTION]
> 您不應建立包含關聯性迴圈，其造成使用案例包含自己本身。 迴圈會產生錯誤。

 您可以共用被包含的使用案例。 在此範例中，「 **訂餐** 」和「 **訂閱評論** 」使用案例都包含「 **付款**」。

 ![使用包含關聯來分解的使用案例](../modeling/media/uml-ucguideinclude.png "UML_UCGuideInclude")
...
#### <a name="defining-the-order-of-the-detailed-steps"></a><a name="Steps"></a> 定義詳細步驟的順序
 使用案例圖不會說明詳細步驟必須執行的順序，也不會說明每一個步驟是否總是必要。

 若要讓步驟的順序更明確，您可以使用成品 **將不同** 的檔附加至包含的使用案例。 在下列範例中，會將活動圖附加至「訂購餐點」使用案例。 或者，您可以使用具有步驟清單或一系列螢幕擷取畫面的文字文件。 如需詳細資訊，請參閱 [詳細描述使用案例](#Details)。
...
- 活動圖中動作的名稱與被包含使用案例的名稱相同。
  ...
  ![已連結之活動圖表中顯示的使用案例步驟](../modeling/media/uml-ucguidesteps.png "UML_UCGuideSteps")

### <a name="sharing-goals-with-generalization"></a><a name="Inheritance"></a> 使用一般化共用目標
 使用一般化關聯性來顯示 *特製* 化的使用案例是達成其他 *一般* 使用案例所表示之目標的特定方式。 開放式箭頭應指向更一般的使用案例。

 ![顯示一般化關聯的使用案例](../modeling/media/uml-ucguidegeneral.png "UML_UCGuideGeneral")

 例如，**按信用卡****支付一般化**款項，並**依現金付款**。
...
2. 建立具有指向新一般使用案例之大型箭號的 **一般化** 關聯。

    1. 按一下 [工具箱] 中的 [ **一般化** ]。

    2. 按一下範例) 中的特殊使用案例 (**信用卡付款** 。

    3. 按一下 [一般使用案例] (在範例) 中 **付費** 。

3. 如果您已描述特製化使用案例的目標，請將共同部分移到一般使用案例的描述中。

4. 特製化使用案例之間共用的執行者可以移到一般使用案例中。

### <a name="separating-variant-cases-with-extend"></a><a name="Extend"></a> 使用 Extend 分隔變異案例
 使用 [擴充] 連結可顯示，使用案例在某些情況下可將功能加入另一個使用案例。 此箭號應指向擴充的主要使用案例。

 ![一個使用案例擴充另一個使用案例](../modeling/media/uml-ucguideextend.png "UML_UCGuideExtend")

> [!CAUTION]
> 您不應建立擴充關聯性迴圈，造成執行者自身的一般化。 迴圈可能會產生錯誤。

 例如，一般網站的 **登** 入使用案例可以包括 [ **註冊新使用者** ]，但只有在使用者還沒有帳戶時才會這麼做。

##### <a name="to-separate-a-use-case-into-main-and-extending-parts"></a>將使用案例分隔成主要和擴充部分

1. 建立新的擴充使用案例並加以命名。

2. 使用箭號指向擴充的使用案例來建立 **延伸** 關聯。

   1. 按一下 [工具箱] 中的 [ **擴充** ]。

   2. 按一下 [擴充使用案例 (在範例) 中 **註冊新的使用者** 。

   3. 按一下範例) 中的擴充使用案例 (**登** 入。
...
4. 擴充功能 (在範例) 中 **註冊新使用者** 的描述，應該包含在主要使用案例中發生的詳細資料，以及在何種情況下會發生的詳細資料。 您可將此視為修改主要案例的描述。
...
## <a name="using-subsystem-boundaries"></a><a name="Subsystems"></a> 使用子系統界限
...
### <a name="multiple-subsystems"></a>多個子系統
 您可以建立數個子系統界限，顯示不同的系統元件如何處理不同的使用案例。 例如， **新增餐廳評價** 可能會在個別的論壇網站上處理。 務必記得，使用案例圖應處理使用者可看見的內容。 如果您要說明系統內部的細部工作，請考慮使用元件圖。

### <a name="system-versions"></a>系統版本
 您可以使用不同的子系統界限表示不同版本的系統。 例如，「付款」使用案例可能包含在第 2 版網站內，但不包含在第 1 版網站內。這表示此系統可幫助顧客訂購。 不過，顧客必須直接付款給餐廳。

 使用 **相依** 性關聯來連結代表不同版本或變體的子系統。

 ![顯示不同系統版本的子系統](../modeling/media/uml-ucguidesystem.png "UML_UCGuideSystem")

## <a name="see-also"></a>另請參閱
 [模型使用者需求](../modeling/model-user-requirements.md) [uml 順序圖表：指導方針](../modeling/uml-sequence-diagrams-guidelines.md) [：編輯 Uml 模型和圖表](../modeling/edit-uml-models-and-diagrams.md) [Uml 使用案例圖：](../modeling/uml-use-case-diagrams-reference.md)參考 uml[類別圖表](../modeling/uml-class-diagrams-reference.md)：參考 Uml[元件圖](../modeling/uml-component-diagrams-reference.md)：參考 Uml[活動圖：指導方針](../modeling/uml-activity-diagrams-guidelines.md)[影片：將功能組織成使用案例](https://channel9.msdn.com/blogs/clinted/uml-with-vs-2010-part-2-organizing-features-into-use-cases)
