...
- 將範本指示詞的 [繼承] 屬性設定為 [ [ModelingTextTransformation](/previous-versions/bb893209(v=vs.140))]。 這可提供存放區的存取權。

- 針對您想要存取的 DSL 指定指示詞處理器。 這會載入您 DSL 的元件，讓您可以在文字模板的程式碼中使用其網域類別、屬性和關聯性。 它也會載入您指定的模型檔案。

  `.tt`當您 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 從 [DSL 最基礎語言] 範本建立新的方案時，會在偵錯工具專案中建立類似下列範例的檔案。
...
- 雖然撰寫程式碼片段的語言是 c #，但您可以產生任何種類的文字。 您也可以 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 將屬性加入至指示詞，以撰寫中的程式碼 `language="VB"` `template` 。

- 若要對範本進行 debug，請加入至指示詞 `debug="true"` `template` 。 如果發生例外狀況，則範本會在的另一個實例中開啟 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 。 如果您想要在程式碼中的特定點中斷偵錯工具，請插入語句 `System.Diagnostics.Debugger.Break();`
...
## <a name="loading-models-dynamically"></a>以動態方式載入模型
 ...
 若要載入檔案，最有效的方法是使用 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus。 在一般案例中，您的文字模板將使用 DSL 特定的指示詞，以一般方式載入第一個模型。 該模型包含另一個模型的 ModelBus 參考。 您可以使用 ModelBus 來開啟參考的模型，並存取特定的元素。 如需詳細資訊，請參閱 [使用文字模板中的 Visual Studio ModelBus](../modeling/using-visual-studio-modelbus-in-a-text-template.md)。

 在較不常使用的情況下，您可能會想要開啟只有檔案名，而且可能不在目前專案中的模型檔案 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 。 在此情況下，您可以使用 [如何：在程式碼中從檔案開啟模型（可能為程式碼中的檔案開啟模型](../modeling/how-to-open-a-model-from-file-in-program-code.md)）中所述的技術來開啟檔案。
