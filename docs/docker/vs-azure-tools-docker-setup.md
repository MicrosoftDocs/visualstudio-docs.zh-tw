...
ms.locfileid: "55139801"
---
# <a name="configure-a-docker-host-with-virtualbox"></a>使用 VirtualBox 設定 Docker 主機
## <a name="overview"></a>總覽
本文會逐步引導您使用 Docker 機器和 VirtualBox 來設定預設 Docker 執行個體。
如果您使用 [Docker for Windows](https://www.docker.com/products/docker-desktop)，則不需要此組態。
...
2. 確認預設執行個體已完成設定且正在執行。 (您應該會看到名為「default」的執行個體正在執行。
    ...
    ![docker-machine ls 輸出](media/vs-azure-tools-docker-setup/docker-machine-ls.png)
...
4. 顯示作用中的 Docker 容器。 清單應該是空的。
    ...
    ![docker ps 輸出](media/vs-azure-tools-docker-setup/docker-ps.png)
