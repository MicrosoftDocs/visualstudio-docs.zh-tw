...
ms.locfileid: "68926365"
---
# <a name="context-menus-xml-schema-explorer"></a>內容功能表 (XML 架構瀏覽器)

內容功能表是以滑鼠右鍵按一下某個專案時所出現的功能表。 下列操作功能表項目用於執行結構描述特定搜尋和其他作業。
...
|**顯示全域類型**|尋找並反白顯示結構描述集中的所有全域型別。|
|**顯示全域元素**|尋找並反白顯示結構描述集中的所有全域項目。|
...
|選項|描述|
|-|-----------------|
|**顯示所有輸入參考**|尋找並反白顯示匯入選取之命名空間的檔案。|
|**顯示所有輸出參考**|針對選取之命名空間內的每個檔案，尋找並反白顯示下列項目：<br /><br /> -不含`schemaLocation`屬性的 import 語句中所參考的所有命名空間。<br />-在匯入和包含語句的`schemaLocation`屬性中所指定的命名空間以外的所有檔案。|
|**顯示全域類型**|尋找並反白顯示所選命名空間中的所有全域型別。|
|**顯示全域元素**|尋找並反白顯示所選命名空間中的所有全域項目。|
|**屬性視窗**|開啟 [**屬性**] 視窗 (如果尚未開啟)。 這個視窗會顯示與節點有關的資訊。|
...
|選項|描述|
|-|-----------------|
|**顯示所有輸入參考**|尋找並反白顯示在 include 和 import 陳述式之 `schemaLocation` 屬性中指定選取檔案的所有檔案。|
|**顯示所有輸出參考**|尋找並反白顯示下列項目：<br /><br /> -所有不具有`schemaLocation`屬性的 import 語句之 namespace 屬性中指定的所有命名空間。<br />-所有 import 和 include 語句`schemaLocation`之屬性中指定的所有檔案。|
|**顯示全域類型**|尋找並反白顯示此檔案中的所有全域型別。|
|**顯示全域元素**|尋找並反白顯示此檔案中的所有全域項目。|
|**檢視程式碼**|在 XML 編輯器中開啟包含選取之節點的檔案。 在 xml 架構瀏覽器中選取的專案也會在 XML 編輯器中選取。|
|**屬性視窗**|開啟 [**屬性**] 視窗 (如果尚未開啟)。 這個視窗會顯示與節點有關的資訊。|
...
|選項|描述|
|-|-----------------|
|**在圖表視圖中顯示**|開啟圖表檢視。 如果所選的節點不在工作空間中，請將其加入至工作空間並選取該節點。|
|**在內容模型視圖中顯示**|開啟內容模型檢視。 如果所選的節點不在工作空間中，請將其加入至工作空間並選取該節點。|
|**檢視程式碼**|在 XML 編輯器中開啟包含選取之節點的檔案。 在 xml 架構瀏覽器中選取的專案也會在 XML 編輯器中選取。|
|**屬性視窗**|開啟 [**屬性**] 視窗 (如果尚未開啟)。 這個視窗會顯示與節點有關的資訊。|

## <a name="node-type-element"></a>節點類型:項目
除了上述的全域節點選項外，項目節點的操作功能表還包含下列選項：

|選項|描述|
|-|-----------------|
|**移至類型定義**|瀏覽至選取之項目的型別定義。 當用於此項目的型別是全域型別時，這個選項就會適用。|
|**移至原始元素**|針對項目參考，瀏覽至此項目的實際定義。|
...
|**顯示替代群組標頭**|針對屬於替代群組之成員的全域項目，尋找並反白顯示選取之項目的所有直接和間接標頭，例如下列項目：<br /><br /> -在選取的元素上指定的替代群組標頭。<br />-在其 head 元素上指定的替代群組標頭。|
...
- [搜尋架構集](../xml-tools/searching-the-schema-set.md)