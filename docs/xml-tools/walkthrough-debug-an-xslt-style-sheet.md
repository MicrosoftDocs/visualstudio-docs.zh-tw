...
6. 在功能表列上，選擇 [ **XML**] [  >  **啟動 XSLT 調試**] （或按**Alt** + **F5**）。
   ...
   在編輯器中，偵錯工具位於 `xsl:if` 樣式表單的元素上。 另一個名為*below-average.xml*的檔案會在編輯器中開啟;這是在處理輸入檔中的每個節點時，將填入的輸出檔案*books.xml* 。
...
1. 從 [**調試**] 功能表中，選擇 [ **Windows**  >  **監**看式  >  **] [watch 1**]。
...
3. 在下一行中，于 `self::node()` [**名稱**] 欄位中輸入，然後按**enter**。

   `self::node()`這是評估為目前內容節點的 XPath 運算式。 `self::node()` XPath 運算式的值為第一個書籍節點。 它會隨著轉換的進行而變更。

4. 展開 `self::node()` 節點，然後展開值為的節點 `price` 。

   ![Visual Studio 中 XSLT 調試期間的監看式視窗](media/xslt-debugging-watch-window.png)

   您可以查看目前 book 節點的書籍價格值，並將其與值進行比較 `$bookAverage` 。 因為書籍價格低於平均值，所以 `xsl:if` 當您繼續進行偵錯工具時，條件應該會成功。

## <a name="step-through-the-code"></a>逐步執行程式碼

1. 按 **F5** 繼續。

   因為第一個書籍節點符合 `xsl:if` 條件，所以會將 [書籍] 節點加入至*below-average.xml*的輸出檔。 偵錯工具繼續執行，直到其再次定位於樣式表的 `xsl:if` 項目上為止。 偵錯工具現在位於*books.xml*檔案的第二個書籍節點上。

   在 [**監看式 1** ] 視窗中， `self::node()` 值會變更為第二個書籍節點。 藉由檢查價格項目的值，您可以確定價格高於平均值，因此 `xsl:if` 條件應該會失敗。

2. 按 **F5** 繼續。

   因為第二個書籍節點不符合 `xsl:if` 條件，所以不會將 [書籍] 節點新增至*below-average.xml*的輸出檔。 偵錯工具會繼續執行，直到它再次放置在 `xsl:if` 樣式表單中的元素上。 偵錯工具現在位於books.xml檔案中的第三個 `book` 節點上。 *books.xml*

   在 [**監看式 1** ] 視窗中， `self::node()` 值會變更為第三個書籍節點。 藉由檢查元素的值 `price` ，您可以判斷價格低於平均值。 `xsl:if`條件應該會成功。

3. 按 **F5** 繼續。

   因為 `xsl:if` 已滿足條件，所以會將第三本書加入*below-average.xml*輸出檔中。 當處理過 XML 文件中的所有書籍後，偵錯工具會停止。
