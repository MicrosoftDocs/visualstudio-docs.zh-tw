...
1. 建立新的類別庫專案。
    ...
    從頂端功能表列中，選擇 [檔案] >  [新增] >  [專案]。 在 [**新增專案**] 對話方塊的左窗格中，選擇 [ **Visual Basic**] 底下的 [ **.NET Standard**]，然後在中間窗格中選擇 [**類別庫（.NET Standard）** ]。
    ::: moniker-end
...
2. 編輯程式碼使其看來如下所示：
   ...
   `DialogDebuggerVisualizer` 有一個您必須覆寫的抽象方法，`Show`。
