...
**使用舊版C#和 VB 運算式評估工具**： 偵錯工具會使用 Visual Studio 2013 C#/VB 運算式評估工具，而不是 Visual Studio 2015 roslyn 運算式評估工具。    
...
- **將變更套用在繼續執行 （僅限機器碼）**: Visual Studio 會自動編譯和適用於從中斷狀態繼續執行此程序時，您所做的未處理的程式碼變更。 如果未選取，您可以選擇套用變更，使用**套用程式碼變更**項目底下**偵錯**功能表。  
