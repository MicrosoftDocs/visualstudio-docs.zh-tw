...
description: 在 Visual Studio 中檢查偵測執行緒和進程的工具。 執行緒和進程代表必須依特定循序執行的指令序列。
...
- 針對 <xref:System.Threading.Tasks.Task> 在工作平行程式庫中使用 [ (TPL)](/dotnet/standard/parallel-programming/task-parallel-library-tpl)的程式碼，[並行執行階段](/cpp/parallel/concrt/concurrency-runtime/) (機器碼) 中，調試多執行緒應用程式的主要工具是 [**平行堆疊**] 視窗、[**平行監看** 式 ] 視窗和 [ 工作] 視窗 ([工作] 視窗也支援 JavaScript 承諾物件) 。
