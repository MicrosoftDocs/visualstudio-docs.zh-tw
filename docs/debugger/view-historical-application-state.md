...
1. 打開**工具** > **選項** > **"斯特爾"** 設置，然後選擇 **"Intelli 跟蹤事件和快照**"選項。

    從 Visual Studio 2017 Enterprise 15.9 Preview 2 版開始，此選項是 [IntelliTrace 快照 (受控與原生)]****。

    ![啟用 Intelli 跟蹤事件和快照模式](../debugger/media/intellitrace-enable-snapshots.png "啟用 Intelli 跟蹤事件和快照模式")

1. 如果要配置用於查看異常快照的選項，請從 **"選項**"對話方塊中選擇 **"IntelliTrace** > **高級**"。
    ...
    > [!NOTE]
    > 僅會對 IntelliTrace 記錄的例外狀況事件建立快照。 對於託管代碼，您可以通過選擇 **"工具** > **選項** > **IntelliTrace 事件**"來指定 IntelliTrace 記錄的事件。

1. 在專案中設定一或多個中斷點，並開始偵錯 (按下 **F5**)，或藉由逐步執行程式碼 (**F10** 或 **F11**) 開始偵錯。

    IntelliTrace 會在每個偵錯工具步驟、中斷點事件和未處理的例外狀況事件上，建立應用程式程序的快照。 這些事件會與其他 IntelliTrace 事件一起記錄在 [診斷工具]**** 視窗的 [事件]**** 索引標籤中。 要打開此視窗，請選擇 **"調試** > **視窗** > **顯示診斷工具**"。

    快照可供使用的事件旁會顯示相機圖示。

    ![包含快照的事件選項卡](../debugger/media/intellitrace-events-tab-with-snapshots.png "具有中斷點和步驟快照的事件選項卡")
...
1. 您可以使用偵錯工具列的 [回溯]**** 和 [快轉]**** 按鈕，在事件之間巡覽。
    ...
    這些按鈕導航"**診斷工具"視窗中**"**事件**"選項卡中顯示的事件。 後退或向前步向事件會自動啟動所選事件[上的歷史調試](../debugger/historical-debugging.md)。

    ![後退和向前按鈕](../debugger/media/intellitrace-step-back-icons-description.png "後退和向前一步按鈕")
    ...
    ![使用快照在事件上啟動歷史模式](../debugger/media/intellitrace-historical-mode-with-snapshot.png "使用快照在事件上啟動歷史模式")

2. 若要返回即時執行，請選擇 [繼續 (F5)]**** 或按一下資訊列中的 [返回即時偵錯]**** 連結。

3. 您還可以從"**事件"** 選項卡查看快照。為此，請選擇具有快照的事件，然後按一下 **"啟動歷史調試**"。

    ![啟動事件上的歷史調試](../debugger/media/intellitrace-activate-historical-debugging.png "啟動事件上的歷史調試")

    與 **Set Next Statement** 命令不同，檢視快照不會重新執行您的程式碼，而會為您提供過去發生的應用程式狀態靜態檢視。

    ![IntelliTrace 退步概述](../debugger/media/intellitrace-step-back-overview.png "IntelliTrace 退步概述")
