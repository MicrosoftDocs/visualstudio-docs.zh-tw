...
如果您在嘗試附加的期間，于 [**輸出**] 視窗中看到下列錯誤，可能是下面所列的已知問題。 請嘗試建議的解決方案，如果問題持續存在，請洽詢先前的別名。
...
這是當使用者嘗試針對已啟用 ApplicationInsights 的 AppService 附加快照偵錯工具時，會發生的已知問題。 ApplicationInsights 會以不同于 Visual Studio 的大小寫來設定 AppSettings，而造成此問題。
