...
以下是一個 Code Map 的快速流覽：
 ...
 ![在 code map 上使用呼叫堆疊進行偵錯工具](../debugger/media/debuggermap_overview.png "DebuggerMap_Overview")

## <a name="MapStack"></a> 對應呼叫堆疊

1. C#在 Visual Studio Enterprise、Visual Basic、 C++或 JavaScript 專案中，選取 [ **Debug** ] > [開始] [**啟動**] 或按**F5**，開始進行偵錯工具。
...
- 或者，取消選取 [啟用 Visual Studio**工具**（或**Debug**）中的**Just My Code** ] >**選項** > [**調試**]：
...
若要新增批註，請以滑鼠右鍵按一下 Code Map，然後選取 [**編輯** > **新增批註**]，然後輸入批註。
...
若要停止對應自動加入新的呼叫堆疊，請選取 [Code Map] 工具列上的 [![自動顯示 Code Map 上的呼叫堆疊](../debugger/media/debuggermap_automaticupdateicon.gif "在 Code Map 自動顯示呼叫堆疊")]。 地圖會繼續反白顯示現有的呼叫堆疊。 若要以手動方式將目前的呼叫堆疊加入至地圖，請按**Ctrl**+**Shift**+ **`** 。
...
若要將您要追蹤的專案加入至地圖，請以滑鼠右鍵按一下方法，然後選取您要追蹤的專案。最近新增的專案會以綠色顯示。

![與呼叫堆疊上的方法相關的欄位 Code Map](../debugger/media/debuggermap_showedfields.png "DebuggerMap_ShowedFields")
...
![與其他人共用呼叫堆疊 Code Map](../debugger/media/debuggermap_sharewithothers.png "與其他人共用堆疊呼叫 Code Map")
