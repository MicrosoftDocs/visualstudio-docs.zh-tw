...
1. 選取 [ **Debug**Windows **Watch**  >  **Windows**  >  **watch**  >  **watch 1**]，或按**Ctrl** + **Alt** + **W**  >  **1**以開啟 [監看式] 視窗。
...
如果出現重新整理圖示是因為屬性的自動評估和隱含函式呼叫已停用，您可以在 [**工具**選項] 中選取 [**啟用屬性評估及其他隱含函式呼叫**] 來啟用它  >  **Options**  >  **Debugging**  >  ** **。

若要示範如何使用重新整理圖示：

1. 在 [**工具**  >  **選項**] 的  >  **Debugging**  >  **[一般**] 選項中，清除 [**啟用屬性評估及其他隱含函式呼叫**] 核取方塊。
...
防止產生副作用的其中一種方法是關閉自動函式評估。 在 [**工具**  >  **選項**] 的  >  **Debugging**  >  **[一般**] 選項中，取消選取 [**啟用屬性評估及其他隱含函式呼叫**]。
...
1. 當執行在中斷點暫停時，請選擇**Locals** [ **Debug**  >  **Windows**  >  **區域變數**] 來開啟 [區域變數] 視窗。
...
1. 繼續偵錯。 當執行在方法中暫停時 `DoSomething()` ，[ **監看** 式] 視窗會顯示 `Person` 物件。
   ...
   > [!NOTE]
   > 如果您想要查看物件的屬性，例如 `Person.Name` ，您必須選取 [**工具**  >  **選項**]、  >  **Debugging**  >  **[一般]、[**  >  **啟用屬性評估及其他隱含函式呼叫**] 來啟用屬性評估。
