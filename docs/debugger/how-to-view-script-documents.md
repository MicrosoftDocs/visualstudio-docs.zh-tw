...
針對動態產生頁面的某些應用程式類型，當您從瀏覽器中載入的指令檔設定中斷點時，進入中斷模式和 debug 會比較容易。 同樣地，您可以 `debugger` 從載入的指令檔加入語句，進入中斷模式。 本文說明如何查看這些檔。

> [!NOTE]
> 在之前 [!INCLUDE[vs_dev11_long](../data-tools/includes/vs_dev11_long_md.md)] ，從伺服器端腳本產生的用戶端腳本檔案會出現在 [腳本瀏覽器] 視窗中。
