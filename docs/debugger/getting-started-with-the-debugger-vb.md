---
title: 了解如何使用 Visual Studio 偵錯工具對 Visual Basic 程式碼進行偵錯
...
2. 在 [Visual Basic] 下方，選擇 [Windows 桌面]，然後在中間窗格中選擇 [主控台應用程式]。
...
1. 在 `Main` 函式的 `For Each` 迴圈中，按一下下列程式碼行的左邊界來設定中斷點：
    ...
    ![設定中斷點](../debugger/media/get-started-set-breakpoint-vb.png)
...
2. 按下 **F5** 鍵或 [開始偵錯] 按鈕 ![開始偵錯](../debugger/media/dbg-tour-start-debugging.png 「開始偵錯」，應用程式會啟動，而偵錯工具會執行到您設定中斷點的程式碼行。
    ...
    ![叫用中斷點](../debugger/media/get-started-hit-breakpoint-vb.png)
...
1. 在 `Main` 函式的 `shape.Draw` 方法呼叫中暫停時，請按下 **F11** 鍵 (或選擇 [偵錯] > [逐步執行]) 推進到 `Rectangle` 類別的程式碼。
     ...
     ![使用 F11 鍵來逐步執行程式碼](../debugger/media/get-started-f11-vb.png "F11 鍵逐步執行")
...
2. 按幾次 **F10** 鍵 (或選擇 [偵錯] > [不進入函式])，直到偵錯工具在 `MyBase.Draw` 方法呼叫上停止為止，然後再次按下 **F10** 鍵。
     ...
     ![使用 F10 鍵不進入程式碼](../debugger/media/get-started-step-over-vb.png "F10 鍵不進入函式")

     請注意，這次偵錯工具不會逐步執行基底類別 (`Shape`) 的 `Draw` 方法。 **F10** 鍵會推進偵錯工具，而不需要逐步執行應用程式程式碼中的函式或方法 (此程式碼仍會執行)。 藉由在 `MyBase.Draw` 方法呼叫上按下 F10 鍵 (而非 **F11** 鍵)，我們略過了 `MyBase.Draw` 的實作程式碼 (現在對我們可能不太重要)。
...
1. 在程式碼編輯器中，向下捲動並停留在 `Triangle` 類別的 `Console.WriteLine` 方法上，直到綠色的 [執行至點選處] 按鈕 ![執行至點選處](../debugger/media/dbg-tour-run-to-click.png "RunToClick") 出現在左側為止。
     ...
     ![使用 [執行至點選處] 功能](../debugger/media/get-started-run-to-click-vb.png "執行至點選處")
...
1. 展開 `shapes` 物件以查看其所有屬性，例如陣列 `[0]` 的第一個索引，其具有 `Rectangle` 值。
     ...
     ![檢視資料提示](../debugger/media/get-started-data-tip-vb.png "檢視資料提示")
...
1. 查看程式碼編輯器底部的 [自動變數] 視窗。
     ...
     ![檢查 [自動變數] 視窗中的變數](../debugger/media/get-started-autos-window-vb.png "[自動變數] 視窗")
...
2. 按幾下 **F11** 鍵，直到您看到偵錯工具在程式碼編輯器中 `Rectangle` 類別的 `MyBase.Draw` 方法中暫停為止。 查看 [呼叫堆疊] 視窗。
    ...
    ![檢查呼叫堆疊](../debugger/media/get-started-call-stack-vb.png "ExamineCallStack")
