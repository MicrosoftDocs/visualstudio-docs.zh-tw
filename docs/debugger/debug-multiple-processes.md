...
ms.locfileid: "37057963"
---
# <a name="debug-multiple-processes"></a>偵錯多重處理序
以下是如何開始偵錯程序、 處理序之間切換、 中斷和繼續執行、 逐步執行來源、 停止偵錯，並終止或中斷處理序連結。  
...
##  <a name="BKMK_Find_the_source_and_symbol___pdb__files"></a> 尋找來源和符號 (.pdb) 檔  
 ...
 如果您無法存取處理程序的檔案，您可以使用 [反組譯碼] 視窗進行巡覽。 請參閱[How to： 使用反組譯碼視窗](../debugger/how-to-use-the-disassembly-window.md)  
...
-   [在方案中啟動多個專案](#BKMK_Start_multiple_projects_in_a_solution)  
  
-   [附加至處理序](#BKMK_Attach_to_a_process)  
  
-   [自動啟動偵錯工具中的 處理序](#BKMK_Automatically_start_an_process_in_the_debugger)  
...
####  <a name="BKMK_Start_a_specific_project_in_a_solution"></a> 在方案中啟動特定專案  
 ...
 ![回到頁首](../debugger/media/pcs_backtotop.png "PCS_BackToTop") [VS 方案中啟動多個處理序、 附加至處理序、 自動啟動偵錯工具中的 處理序](../debugger/debug-multiple-processes.md#BKMK_Start_multiple_processes_in_a_VS_solution__attach_to_a_process__automatically_start_a_process_in_the_debugger)  
 ...
 **附加至正在本機電腦執行的處理序**  
 ...
 按一下 **偵錯 > připojit k procesu**。 在上**připojit k procesu**對話方塊方塊中，選取的程序**可用的處理序**清單，然後再選擇**附加**。  
  
 ![附加至處理序 對話方塊](../debugger/media/dbg_attachtoprocessdlg.png "DBG_AttachToProcessDlg")  
...
7.  在 編輯字串 對話方塊中，輸入`vsjitdebugger.exe`中**數值資料** 方塊中。  
 ...
 ![自動偵錯工具起始項目在 regedit.exe](../debugger/media/dbg_execution_automaticstart_result.png "DBG_Execution_AutomaticStart_Result")  
...
|**處理序**視窗操作功能表：<br /><br /> -   **偵錯停止時中斷連結**|切換的行為**偵錯**，**停止偵錯**所選處理序：<br /><br /> -Checked： 偵錯工具會中斷連結程序。<br />-清除： 處理序終止。|  
 ...
 ![回到頁首](../debugger/media/pcs_backtotop.png "PCS_BackToTop") [停止偵錯、 終止或中斷處理序連結](../debugger/debug-multiple-processes.md#BKMK_Stop_debugging__terminate_or_detach_from_processes)  
