...
## <a name="more-help"></a>詳細的說明
 若要取得更多遠端偵錯程式說明，包括命令列參數，請按一下 [說明] > [遠端偵錯程式] 視窗中的**使用**方式。 如果您沒有開啟，您可以將下列程式程式碼複製到 [檔案**瀏覽器**] 視窗，以看到網頁。 （您必須將 \<Visual Studio 安裝目錄 > 取代為 Visual Studio 安裝的位置）。
