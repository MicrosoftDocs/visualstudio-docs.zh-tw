...
儘管演示應用C++，但大多數功能都適用于 C#、可視基本功能、F#、Python、JavaScript 和 Visual Studio 支援的其他語言（F# 不支援編輯和繼續）。 F# 和 JavaScript 不支援 [自動變數]**** 視窗)。 螢幕截圖位於C++。
...
如果您需要安裝工作負載，但已經擁有視覺化工作室，請轉到 **"工具** > **獲取工具和功能..."，** 這將打開視覺化工作室安裝程式。 Visual Studio 安裝程式即會啟動。 選擇 [使用 C++ 的桌面開發]**** 工作負載，然後選擇 [修改]**** 按鈕。

## <a name="create-a-project"></a>建立專案

首先，您將創建一個C++主控台應用程式專案。 在您新增任何項目之前，專案類型會隨附您需要的所有範本檔案！
...
3. 在左側窗格中的 **"新專案**"對話方塊中，展開**視覺化C++，** 然後選擇**Windows 桌面**。 在中間窗格中，選擇**Windows 主控台應用程式**。 然後命名專案*開始調試*。

     如果看不到**主控台應用**專案範本，請選擇 **"新專案**"對話方塊左側窗格中的 **"打開視覺化工作室安裝程式"** 連結。 Visual Studio 安裝程式即會啟動。 選擇 [.NET Core 跨平台開發]**** 工作負載，然後選擇 [修改]****。
...
1. 在 [建立新專案]**** 視窗的搜尋方塊中輸入或鍵入 ASP.NET**。 接下來，從"語言"清單中選擇**C++，** 然後從"平臺"清單中選擇**Windows。** 

   應用語言和平臺篩選器後，選擇**主控台應用**範本，然後選擇 **"下一步**"。

   ![選擇主控台應用的C++範本](../debugger/media/vs-2019/get-started-create-console-project-cpp.png)

   > [!NOTE]
   > 如果看不到**主控台應用**範本，則可以從 **"創建新專案"** 視窗安裝該範本。 在 [找不到您要找的資料嗎?]**** 訊息中，選擇 [安裝更多工具和功能]**** 連結。 然後，在視覺化工作室安裝程式中，選擇**具有C++工作負載的桌面開發**。
...
1. 再按**F11**一次以步`SendMessage`進方法。
     ...
     ![使用 F11 進入代碼](../debugger/media/get-started-f11-cpp.png "F10 步進")
...
1. 在方法調用時暫停時，按**F10（** 或選擇**調試>單步**執行） 一次。
     ...
     ![使用 F10 單一步驟代碼](../debugger/media/get-started-step-over-cpp.png "F10 步長")
...
1. 在代碼編輯器中，向下滾動並懸停在方法`std::wcout`中的函數上`SendMessage`，直到左側出現綠色 **"運行以按一下**!["](../debugger/media/dbg-tour-run-to-click.png "運行點擊")按鈕。 按鈕的工具提示會顯示「執行到這裡」。

     ![使用"運行到按一下"功能](../debugger/media/get-started-run-to-click-cpp.png "執行至點選處")
...
1. 暫停語句時`name += letters[i]`，將滑鼠懸停在變數上`letters`，您將看到它的預設值`size={10}`。
...
1. 展開`letters`變數以顯示它包含的元素。
     ...
     ![檢查區域變數視窗中的變數](../debugger/media/get-started-locals-window-cpp.png "本機視窗")
...
2. 按一下**F11**幾次，直到看到調試器在`SendMessage`方法中暫停。 查看 [呼叫堆疊]**** 視窗。
    ...
    ![檢查呼叫堆疊](../debugger/media/get-started-call-stack-cpp.png "檢查呼叫堆疊")
...
1. 按**F11**兩次`std::wcout`以運行該功能。

1. 當調試器在`SendMessage`方法調用中暫停時，使用滑鼠在左側抓取黃色箭頭（執行指標），並將黃色箭頭向上移動一行，返回回`std::wcout`。

1. 按**F11**.

    調試器重新運行`std::wcout`該函數（在主控台視窗輸出中可以看到這一功能）。
