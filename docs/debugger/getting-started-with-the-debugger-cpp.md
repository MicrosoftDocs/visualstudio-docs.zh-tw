...
雖然示範應用程式是 c + +，但大部分的功能都適用于 c #、Visual Basic、F #、Python、JavaScript，以及其他 Visual Studio 所支援的語言 (F # 不支援編輯後繼續。 F# 和 JavaScript 不支援 [自動變數]**** 視窗)。 螢幕擷取畫面是在 c + + 中。
...
如果您需要安裝工作負載，但已有 Visual Studio，請移至 [**工具**  >  **取得工具和功能**]，這會開啟 Visual Studio 安裝程式。 Visual Studio 安裝程式即會啟動。 選擇 [使用 C++ 的桌面開發]**** 工作負載，然後選擇 [修改]**** 按鈕。
...
1. 在 [建立新專案]**** 視窗的搜尋方塊中輸入或鍵入 ASP.NET**。 接著，從 [語言] 清單中選擇 [ **c + +** ]，然後從 [平臺] 清單中選擇 [ **Windows** ]。 

   套用語言和平臺篩選器之後，請選擇 [ **主控台應用程式** ] 範本，然後選擇 [ **下一步]**。

   ![選擇主控台應用程式的 c + + 範本](../debugger/media/vs-2019/get-started-create-console-project-cpp.png)
...
1. 再按一次 **F11** ，逐步執行 `SendMessage` 方法。
     ...
     ![使用 F11 來逐步執行程式碼](../debugger/media/get-started-f11-cpp.png "F10 逐步執行")
...
1. 在方法呼叫中暫停時，請按 **F10** (或選擇 [ **Debug >** 不進入) 一次]。
     ...
     ![使用 F10 來跳過程式碼](../debugger/media/get-started-step-over-cpp.png "F10 跳過")
...
1. 在 [程式碼編輯器] 中，將滑鼠游標移至方法的函式上，直到按一下 [執行] 按鈕，然後按一下 [ `std::wcout` `SendMessage` ![執行](../debugger/media/dbg-tour-run-to-click.png ">runtoclick")] 按鈕才會出現在左邊。 **Run to Click** 按鈕的工具提示會顯示「執行到這裡」。

     ![使用執行至按一下功能](../debugger/media/get-started-run-to-click-cpp.png "執行至點選處")
...
1. 在語句上暫停時 `name += letters[i]` ，將滑鼠停留在變數上， `letters` 您會看到它是預設值 `size={10}` 。
...
1. 按 **F11** 兩次以執行 `std::wcout` 函數。

1. 當偵錯工具在方法呼叫中暫停時 `SendMessage` ，請使用滑鼠抓取左側的黃色箭號 (執行指標) 左邊，然後將黃色箭號向左移動一行 `std::wcout` 。

1. 按 **F11**鍵。

    偵錯工具會重新開機函式 `std::wcout` (您在主控台視窗輸出) 中看到此函式。
