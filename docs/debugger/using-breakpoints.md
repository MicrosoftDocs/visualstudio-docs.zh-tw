...
##  <a name="BKMK_Overview"></a> 在原始程式碼中設定中斷點  
 ...
 當偵錯工具停止於中斷點時，您可以查看應用程式，包括變數值和呼叫堆疊的目前狀態。 如需有關呼叫堆疊的詳細資訊，請參閱[如何： 使用 [呼叫堆疊] 視窗](../debugger/how-to-use-the-call-stack-window.md)。  
...
如需有關呼叫堆疊的詳細資訊，請參閱[如何： 使用 [呼叫堆疊] 視窗](../debugger/how-to-use-the-call-stack-window.md)。 
...
2.  在 **地址**方塊中，輸入記憶體位址或評估為記憶體位址的運算式。 例如，輸入當變數 `&avar` 的內容變更時要中斷的 `avar` 。  
...
>  
>- 如果您對區域變數設定資料中斷點，中斷點會維持啟用狀態時函式結束，但記憶體位址不再適用，所以中斷點的行為無法預測。 如果您對區域變數設定資料中斷點，您應該刪除或停用函式結束前的中斷點。  

##  <a name="BKMK_Specify_advanced_properties_of_a_breakpoint_"></a> 在 [中斷點] 視窗中管理中斷點 
...
3. 在您想要調查; 加入新的中斷點例如，當物件是要加入至集合。 以滑鼠右鍵按一下中斷點，然後選取**條件**。  
   
4. 使用中的物件識別碼**條件運算式**欄位。 比方說，如果變數`item`是要加入至集合中，選取的物件**成立**並輸入**item = = $\<n >**，其中\<n > 是物件 ID 號碼.  
...
##  <a name="BKMK_Print_to_the_Output_window_with_tracepoints"></a> 中斷點動作和追蹤點  
 A*追蹤點*會將訊息列印至中斷點**輸出**視窗。 追蹤點在程式語言中的行為可以像是暫存追蹤陳述式。  
