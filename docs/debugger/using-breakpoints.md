...
對於大部分的語言C#（包括），會自動反白顯示中斷點和目前的執行行。 針對C++程式碼，您可以藉由選取 [**工具**] （或 [ **Debug**]） >**選項** >  >  **調試**程式來開啟反白顯示中斷點和目前的行 **。語句（C++僅限）** 。

![設定中斷點](../debugger/media/basicbreakpoint.png "基本中斷點")

當您進行 debug 時，執行會在中斷點上暫停，然後才執行該行上的程式碼。 中斷點符號會顯示黃色箭號。

在下列範例的中斷點，`testInt` 的值仍然是1。 因此，由於變數已初始化（設定為值為1），因此值不會變更，因為黃色的語句尚未執行。
...
「追蹤點」是將訊息列印至 [輸出] 視窗的中斷點。 追蹤點的作用就像是程式設計語言中的暫時性追蹤語句，而且不會暫停程式碼的執行。 您可以在 [**中斷點設定**] 視窗中設定特殊動作來建立追蹤點。 如需詳細指示，請參閱在[Visual Studio 偵錯工具中使用追蹤點](../debugger/using-tracepoints.md)。
...
### <a name="use-object-ids-in-conditional-expressions-c-and-f-only"></a>在條件運算式中使用物件識別碼C# （ F#僅限和）
