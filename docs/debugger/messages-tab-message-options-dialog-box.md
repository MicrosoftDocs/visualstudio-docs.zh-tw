 ...
 **若要檢視訊息**選取的檢視特定訊息。 當您建立新的 [訊息] 視窗時，它可以顯示所有訊息。 當您篩選來自**訊息**索引標籤上，該篩選條件只適用於新的訊息，而不顯示在 [Windows] 檢視中的訊息。

 **訊息群組**選取檢視的訊息群組。 可用的群組包含：
...
- 未知： 範圍介於 0 到 (WM_USER-1) 中的未知的訊息
  ...
  **將設定儲存為預設**儲存供稍後使用目前設定為訊息的搜尋選項。 結束 Spy + + 時，也會儲存這些設定。