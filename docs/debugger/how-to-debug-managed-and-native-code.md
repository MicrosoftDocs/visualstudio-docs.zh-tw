...
1. 開啟 Visual Studio 並建立專案。
    ...
    按 **Esc** 來關閉開始視窗。 輸入**Ctrl + Q**開啟 [搜尋] 方塊，輸入**空白專案**，選擇 [**範本**]，然後選擇 [ C++**空白專案**]。 在出現的對話方塊中，選擇 [建立]。 然後，鍵入像 **Mixed_Mode_Debugging** 的名稱，並按一下 [建立]。
    ::: moniker-end
...
1. 開啟 Visual Studio 並建立新專案。
    ...
    ::: moniker range=">=vs-2019"
    按 **Esc** 來關閉開始視窗。 輸入**Ctrl + Q**來開啟搜尋方塊，輸入**主控台**，選擇 [**範本**]，然後選擇 [**主控台應用程式（.Net Core）** ] 或 [**主控台應用程式（.NET Framework）** ] C#。 在出現的對話方塊中，選擇 [建立]。
...
1. 在 *launchsettings.json* 檔案中新增下面一行：
