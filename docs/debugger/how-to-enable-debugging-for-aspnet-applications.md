...
<a name="iis"></a>
- 安裝 Visual Studio 時，請選取 [ **開發階段 IIS 支援** ]。  (必要時，請重新執行 Visual Studio 安裝程式、選取 [ **修改**]，然後新增此元件。 ) 
- 以系統管理員身分執行 Visual Studio。
- 使用 ASP.NET 和/或 ASP.NET Core 的適當版本 () 安裝並正確設定 IIS。 如需詳細資訊和指示，請參閱 [使用 ASP.NET 3.5 和 ASP.NET 4.5 的 iis 8.0，或使用](/iis/get-started/whats-new-in-iis-8/iis-80-using-aspnet-35-and-aspnet-45) [iis 在 Windows 上裝載 ASP.NET Core](/aspnet/core/host-and-deploy/iis/index)。
...
1. 在 Visual Studio**方案總管**中選取 ASP.NET 專案，然後按一下 [**屬性**] 圖示，按下**Alt** + **enter**，或按一下滑鼠右鍵並選擇 [**屬性**]。
...
1. 選取 Visual Studio**方案總管**中的 ASP.NET Core 專案，然後按一下 [**屬性**] 圖示，按下**Alt** + **鍵**，或按一下滑鼠右鍵並選擇 [**屬性**]。
...
例如，如果您 `debug="true"` 在<em>www.microsoft.com/aaa/web.config</em>中指定，則 aaa 資料夾或*aaa* *aaa*之任何子資料夾中的任何應用程式都會繼承該設定，除非其中一個應用程式以自己的*web.config*檔覆寫設定。
