...
應用程式所使用的記憶體的任一邊的"NoMansLand"緩衝區目前填入 0xFD NoMansLand (0xFD)。

釋放的區塊 (0xDD) 的連結保持在偵錯堆積中未使用的釋放的區塊清單時`_CRTDBG_DELAY_FREE_MEM_DF`設定旗標目前填入 0xDD。

新的物件 (0xCD) 新的物件填入的 0xCD 時配置。
...
**_NORMAL_BLOCK**呼叫[malloc](/cpp/c-runtime-library/reference/malloc)或是[calloc](/cpp/c-runtime-library/reference/calloc)建立一般區塊。 如果您只要使用一般區塊，而且不需要用戶端區塊，建議您定義 [_CRTDBG_MAP_ALLOC](/cpp/c-runtime-library/crtdbg-map-alloc)，它會造成所有堆積配置呼叫都對應至它們在偵錯組建裡的偵錯對等用法。 這可將每個配置呼叫的相關檔名和行號資訊儲存在對應的區塊標頭裡。
...
**_FREE_BLOCK**一般來說，會釋放的區塊會從清單中移除。 若要檢查釋放記憶體是否仍然不能寫入，或模擬低記憶體條件，您可以選擇保留連結串列上的釋放區塊，將其標記為可用，並填入已知位元組值 (目前是 0xDD)。

**_IGNORE_BLOCK**就可以關閉一段時間的偵錯堆積作業。 在這段期間，記憶體區塊會保留於清單終上，但是標記為忽略區塊。
