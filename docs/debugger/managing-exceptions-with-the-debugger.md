...
提供回應最重要例外狀況的處理常式。 如果您需要知道如何新增例外狀況的處理常式，請參閱透過[撰寫更好的 c # 程式碼來修正 bug](../debugger/write-better-code-with-visual-studio.md)。 此外，您也可以瞭解如何設定偵錯工具，一律中斷執行某些例外狀況。

發生例外狀況時，偵錯工具會將例外狀況訊息寫入至 [**輸出**] 視窗。 當下列情況發生時，可能會中斷執行：
...
- 在下列 c # 主控台應用程式中，Main 方法會在區塊內擲回**AccessViolationException** `try/catch` 。
  ...
  如果您已**AccessViolationException**簽入**例外狀況設定**， `throw` 則當您在偵錯工具中執行此程式碼時，執行將會在行上中斷。 然後，您可以繼續執行。 主控台應該會顯示這兩行：
...
- C # 主控台應用程式會使用具有兩個方法的類別來參考類別庫。 其中一個方法會擲回例外狀況並加以處理，而第二個方法則擲回相同的例外狀況，但不會處理它。
  ...
  如果您已**AccessViolationException**核取 [**例外狀況設定**]， `throw` 當您在偵錯工具中執行此程式碼時，執行將會在**ThrowHandledException （）** 和**ThrowUnhandledException （）** 中的那一行中斷。

若要將例外狀況設定還原為預設值，請選擇 [將**清單還原為預設設定**] 按鈕：

![將例外狀況設定還原成預設值](../debugger/media/restoredefaultexceptions.png "RestoreDefaultExceptions")

## <a name="tell-the-debugger-to-continue-on-user-unhandled-exceptions"></a><a name="BKMK_UserUnhandled"></a>告訴偵錯工具在使用者未處理的例外狀況時繼續進行
...
> [!TIP]
> 請檢查您的拼字！ [**例外狀況設定**] 視窗不會檢查加入的例外狀況是否存在。 因此，如果您輸入**system.uritemplatematchexception**，您會收到該例外狀況的專案（而不是**system.uritemplatematchexception**）。
...
- [不使用 C 執行時間程式庫進行執行時間檢查](../debugger/using-run-time-checks-without-the-c-run-time-library.md)<br/>
