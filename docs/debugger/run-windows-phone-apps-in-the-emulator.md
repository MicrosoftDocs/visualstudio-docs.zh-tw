---
title: "在模擬器中執行 Windows Phone 8.1 應用程式 |Microsoft 文件"
ms.custom: 
ms.date: 07/18/2017
ms.reviewer: 
ms.suite: 
...
在模擬器中執行 Windows 10 行動裝置上的資訊，請參閱[測試與 Microsoft 模擬器](/windows/uwp/debug-test-perf/test-with-the-emulator)。
  
如需您可以在 Windows Phone 8.1 模擬器中測試之功能的詳細資訊，請參閱[Windows Phone 模擬器中測試應用程式功能](http://msdn.microsoft.com/en-us/c1b2b0ec-b8cc-4a98-84c1-701428e45cb1)。  
  
模擬器與 Visual Studio 搭配使用，能提供讓您用來設計、開發、偵錯和測試 Windows Phone 應用程式的完整環境。  
  
##  <a name="BKMK_run"></a>在模擬器中執行 Windows Phone 應用程式  
 雖然您正在開發 Windows Phone 應用程式，您可以用於 Windows Phone 模擬器快速部署和測試您的應用程式。 不過，建議您先在實際 Windows Phone 裝置上測試應用程式，再將應用程式發行至 Windows Phone 市集。 這樣可讓您以使用者的身分體驗您的應用程式。  
...
> [!TIP]
>  當您要測試您的應用程式在模擬器上時，保持模擬器開啟，因此您可以快速重新執行您的應用程式偵錯工作階段之間。  
...
2.  在**標準**工具列上，選取其中一個模擬器選項。  
     ...
     ![Windows Phone 模擬器映像清單](../debugger/media/wp_emulator_list.png "WP_Emulator_list")  
  
3.  部署和執行您的應用程式進行偵錯，**偵錯**功能表上，按一下 **開始偵錯**，或按 F5。  
  
     部署和執行您的應用程式但不偵錯，**偵錯**功能表上，按一下 **啟動但不偵錯**，或按 Ctrl + F5。  
  
     隨即會部署和啟動您的應用程式。  
  
     若要部署您的應用程式但不執行，在**建置**功能表上，按一下 **部署方案**。  
...
-   若要停止執行中的應用程式，請執行下列其中一項：  
    ...
    -   在模擬器中，按**回**按鈕結束應用程式。 如果應用程式的使用中頁面不是應用程式的起始頁，您可能需要按**回**按鈕一次以上。  
...
1.  執行您的應用程式之前，請在 Visual Studio 中，以滑鼠右鍵按一下方案總管 中的專案，然後選取**屬性**開啟**專案設計工具**。  
  
2.  在**專案設計工具**上**偵錯**頁面上，保留**偵錯時停用的標記**核取方塊未選取，如果您希望應用程式進入休眠狀態停用。 如果您想要在停用時為應用程式加上標記，請核取此核取方塊。  
  
3.  在**偵錯**功能表上，按一下 **開始偵錯**，或按 F5 執行應用程式。  
  
4.  在模擬器中，按**啟動** 按鈕。 [開始] 畫面即會出現，並停用應用程式。 應用程式會進入休眠狀態或加上標記，根據設定**偵錯時停用的標記**核取方塊。  
  
##### <a name="to-reactivate-a-dormant-or-tombstoned-app"></a>重新啟用休眠或加上標記的應用程式  
  
-   在模擬器中，按**回**按鈕返回應用程式。 如果您已巡覽至其他頁面，或開啟其他應用程式，您可能需要按**回**按鈕多次以重新啟動應用程式。  
  
     偵錯工作階段即會繼續。 如果偵錯工具已中斷與應用程式的連結，則可能需要按 F5 繼續偵錯工作階段。  
  
###  <a name="BKMK_depltool"></a>使用應用程式部署工具執行應用程式  
 您也可以使用 Windows Phone 應用程式部署工具 (**AppDeploy.exe**) 若要在模擬器中執行您的應用程式。 此工具是一個獨立應用程式，可在安裝 Windows Phone 開發工具時一併安裝。  
  
 如需詳細資訊，請參閱[應用程式部署工具部署 Windows Phone 8.1 應用程式](http://msdn.microsoft.com/Library/23700f82-1399-44d9-bc0c-714be4a48ee6)。  
...
|工具列按鈕|組態選項|  
|---------------------|---------------------------|  
|![輸入 Windows Phone 模擬器工具列上的選項](../debugger/media/wp_emulator_.png "WP_Emulator_")|**設定單點或多點輸入**<br /><br /> 當您啟用多點輸入時，可以按一下滑鼠右鍵移動觸控點，而不需觸控螢幕。 接著，您可以按滑鼠左鍵同時移動兩個觸控點。|  
|![Windows Phone 模擬器工具列上的方向](../debugger/media/wp_emulator_rotation.png "WP_Emulator_rotation")|**設定模擬器方向**<br /><br /> 您可以將 Windows Phone 模擬器中的方向變更為其中一個方向 (共三個方向)：直向、橫向 (左) 或橫向 (右)。 當您變更方向時，模擬器的大小並不會變更。<br /><br /> 若要變更方向，請按一下**向左旋轉**按鈕或**向右旋轉** 按鈕。|  
|![調整大小的 Windows Phone 模擬器工具列上的選項](../debugger/media/wp_emulator_size.png "WP_Emulator_size")|**設定模擬器大小**<br /><br /> 您可以在主機電腦畫面上變更模擬器大小。 模擬器的 DPI 是根據主機監視器 DPI (不論縮放值為何)。<br /><br /> -若要調整模擬器以畫面，按一下**全螢幕** 按鈕。<br />-若要變更縮放設定，請按一下**縮放** 按鈕。 **縮放**對話方塊隨即開啟。 在**縮放**對話方塊方塊中，輸入 33 與 100 之間的縮放值。|  
  
##  <a name="BKMK_buttons"></a>使用模擬器上的模擬的硬體按鈕  
 使用模擬器螢幕右側的模擬硬體按鈕，以模擬電話硬體按鈕的使用。  
  
-   按一下**電源**按鈕可模擬顯示關閉和開啟。 按住可模擬關閉電話。  
  
-   按一下**音量提高**或**音量降低**按鈕可模擬變更通話和通知的電話擴音器音量。  
...
2.  中間影像顯示模擬器後點選**電源**按鈕關閉顯示器。  
  
3.  右側影像顯示點選之後的模擬器畫面**音量提高**提高音量按鈕。  
  
 ![Windows Phone 模擬器上的按鈕](../debugger/media/wp_emulator_buttons.png "WP_Emulator_buttons")  
...
##  <a name="BKMK_tasks_kbd"></a>使用電腦鍵盤與模擬器  
 ...
 若要停止使用您的電腦鍵盤在模擬器中，請按 PAGE DOWN 鍵或 PAUSE/BREAK 鍵 （Windows 8/8.1 模擬器） 或 F4 （Windows 10 模擬器）。  
...
|F2|START|長按會如預期運作。|  
|F3|SEARCH||  
|F4|在 Windows 10 模擬器中，會使用本機電腦的鍵盤與不使用本機電腦的鍵盤之間切換。|不適用於 Windows 8/8.1 模擬器。|  
|F5|不適用。||  
...
##  <a name="BKMK_checkpoints"></a>儲存和載入自訂檢查點  
 使用儲存模擬器狀態的快照集**檢查點** 索引標籤的模擬器 的**其他工具**。 如果您經常使用相同的資料和設定來測試應用程式，則此功能十分有用。  
  
 例如，如果您的應用程式需要數個連絡人，您可以一次建立連絡人記錄，並儲存模擬器的快照。 否則，每次啟動模擬器時，都需要重新建立連絡人記錄。  
  
-   按一下**新的檢查點**來擷取模擬器狀態的資料與稍後重新測試應用程式所需設定的新快照集。 新的檢查點加入至**檢查點**清單。  
  
     將偵錯工具附加至模擬器時，您無法擷取檢查點。  
  
-   選取中的檢查點**檢查點**清單以檢視檢查點的相關資訊。  
  
-   選取選項按鈕，在**預設**設為作用中的模擬器已儲存的檢查點的預設檢查點的資料行。  
  
-   按一下**還原**重新啟動 Windows Phone 上的作業系統模擬器並載入選取的快照集。  
  
-   按一下**刪除**以刪除您不再需要的快照。  
  
 原始模擬器影像一律顯示為中的第一個項目**檢查點**清單和無法變更或刪除。 不過，您可以選取不同的快照做為預設模擬器影像。  
  
 ![檢查點] 索引標籤的 [Windows Phone 模擬器](../debugger/media/wp_emulator_checkpoints.png "WP_Emulator_checkpoints")  
  
##  <a name="BKMK_tasks_shot"></a>擷取模擬器中的螢幕擷取畫面  
 您可以使用 [其他工具] 視窗中的螢幕擷取畫面工具，即可建立 Windows Phone 應用程式的螢幕擷取畫面。 此工具會建立符合執行中模擬器解析度的 PNG 檔案。  
  
 ![螢幕擷取畫面，從 Windows Phone 模擬器](../debugger/media/wp_emulator_screenshots.png "WP_Emulator_screenshots")  
...
3.  模擬器工具列上，按一下展開按鈕開啟**其他工具**視窗。  
  
4.  按一下**螢幕擷取畫面** 索引標籤。  
  
5.  您的應用程式就緒時按一下**擷取** 按鈕。  
  
     螢幕擷取畫面即會顯示在工作區中。  
  
6.  按一下**儲存** 按鈕以開啟**存** 對話方塊。  
  
7.  選擇的位置和**檔案名稱**，然後再按一下**儲存**。  
...
-   在 Visual Studio 中指定版本組建。 指定版本組建之後，請選取 啟動應用程式**部署*[應用程式名稱]*** 連結**建置**功能表。  
