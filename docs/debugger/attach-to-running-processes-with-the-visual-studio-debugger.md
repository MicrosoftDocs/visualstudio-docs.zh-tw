...
1. 在 Visual Studio 中，選取 [**調試**程式] [  >  **附加至進程**] （或按**Ctrl** + **Alt** + **P**）以開啟 [**附加至進程**] 對話方塊。
...
選擇 [ **Debug**] [重新  >  **附加至進程**] （**Shift** + **Alt** + **P**），可以快速地重新附加至您先前附加的進程。 當您選擇此命令時，偵錯工具會先嘗試比對先前的處理序識別碼，如果失敗，則會比對先前的進程名稱，以立即嘗試附加至您附加的最後一個進程。 如果找不到相符專案，或有數個進程具有相同的名稱，則會開啟 [**附加至進程**] 對話方塊，讓您可以選取正確的進程。
...
針對用戶端腳本的偵錯工具，必須在瀏覽器中啟用腳本的偵錯工具。 若要在 Chrome 上進行用戶端腳本的偵錯工具，請選擇 [ **javascript （Chrome）** ] 或 [ **JAVAscript （Microsoft Edge-Chromium）** ] 作為 [程式碼類型]，視您的應用程式類型而定，您可能需要關閉所有 Chrome 實例，並在 [偵錯模式] 中啟動瀏覽器（ `chrome.exe --remote-debugging-port=9222` 從命令列輸入）。 在舊版的 Visual Studio 中，適用于 Chrome 的腳本偵錯工具是**Web 套件**。
...
|在本機 IIS 伺服器上，針對支援的應用程式類型進行用戶端腳本的偵錯工具 |使用 [**附加至進程**]|*chrome.exe*、 *MicrosoftEdgeCP.exe*或*iexplore.exe*|必須啟用腳本的調試。 對於 Chrome，您也必須在 [偵錯工具] 模式中執行 Chrome （ `chrome.exe --remote-debugging-port=9222` 從命令列輸入），然後在 [**附加至**] 欄位中選取 [ **JavaScript （Chrome）** ]。|
...
在某些情況下，偵錯工具可能需要協助，以正確識別要進行 debug 錯的程式碼類型。 如果已正確設定連接值（您可以在 [**可使用的進程**] 清單中查看正確的程式），但偵錯工具無法附加，請嘗試在 [連線**類型**] 清單中選取最適當的連線類型，例如，如果您正在對 Linux 或 Python 應用程式進行偵錯工具。 如果您使用預設連線類型，則可以選取要連接的特定程式碼類型，如本節稍後所述。
