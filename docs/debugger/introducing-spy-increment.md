...
## <a name="64-bit-operating-systems"></a>64 位元作業系統  
 Spy++ 有兩個版本。 第一個版本名為 Spy++ (spyxx.exe)。如果視窗是在 32 位元處理序中執行，就非常適合用這個版本來顯示傳送至視窗的訊息。 例如，Visual Studio 會在 32 位元處理序中執行。 因此，您可以使用 Spy++ 來顯示傳送至方案總管 的訊息。 Visual Studio 中大多數組建的預設組態是在 32 位元處理程序中執行，因為此第一個版本的 Spy + + 是指[上可用**工具**功能表](../debugger/how-to-start-spy-increment.md)Visual Studio 中。  
