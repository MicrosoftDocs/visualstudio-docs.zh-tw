...
- 使用複雜的訊息記錄檔選取參數來設定[訊息選項](../debugger/how-to-open-messages-view-from-find-window.md)。
...
## <a name="64-bit-operating-systems"></a>64 位元作業系統
 Spy++ 有兩個版本。 第一個版本名為 Spy++ (spyxx.exe)。如果視窗是在 32 位元處理序中執行，就非常適合用這個版本來顯示傳送至視窗的訊息。 例如，Visual Studio 會在 32 位元處理序中執行。 因此，您可以使用 Spy++ 來顯示傳送至方案總管的訊息。 因為 Visual Studio 中大多陣列建的預設設定是在32位進程中執行，所以如果[已安裝必要元件](../debugger/how-to-start-spy-increment.md)，則此第一個版本的 Spy + + 就是 Visual Studio 中的 [**工具**] 功能表上可用的。
