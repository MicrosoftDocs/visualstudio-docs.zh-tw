...
例外狀況協助程式是可協助您偵錯錯誤的絶佳功能。 您也可以執行檢視錯誤詳細資料，以及從例外狀況協助程式新增監看式等作業。 或者；如有必要，您可以變更擲回特定例外狀況的條件。 如需如何在程式碼中處理例外狀況的詳細資訊，請參閱[透過撰寫更好的 C# 程式碼來修正錯誤 (Bug)](../debugger/write-better-code-with-visual-studio.md)。
