---
title: "在 Visual Studio （VB、 C#、 c + + 和 XAML） 中啟動的 UWP 應用程式的偵錯工作階段 |Microsoft 文件"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
![適用於 Windows 和 Windows Phone](../debugger/media/windows_and_phone_content.png "windows_and_phone_content")  
 ...
 本主題描述如何啟動以 XAML 和 Visual c + +、 Visual C# 或 Visual Basic 撰寫的 UWP 應用程式的偵錯工作階段。 要對應用程式進行偵錯，必須設定偵錯工作階段並選擇應用程式的啟動方式。  
  
> [!NOTE]
>  如需以 JavaScript 和 HTML 撰寫的應用程式請參閱[啟動偵錯工作階段 (JavaScript)](../debugger/start-a-debugging-session-for-store-apps-in-visual-studio-javascript.md)。  
...
2.  執行下面步驟以開啟專案的偵錯屬性頁：  
    ...
    -   針對 Visual C# 與 Visual Basic 應用程式，請選擇 [ **偵錯**]。  
         ...
         ![& #35。&#47;VB 專案偵錯屬性頁](../debugger/media/dbg_csvb_debugpropertypage.png "DBG_CsVb_DebugPropertyPage")  
  
    -   針對 Visual C++ 應用程式，請展開 [ **組態屬性**  ] 節點，然後選擇 [ **偵錯**]。  
  
         ![C# 43; &#43;UWP 應用程式偵錯屬性頁](../debugger/media/dbg_cpp_debugpropertypage.png "DBG_CPP_DebugPropertyPage")  
...
###  <a name="BKMK_Choose_the_deployment_target"></a> 選擇部署目標  
 ...
 您可以部署和偵錯在 Visual Studio 電腦、 本機電腦或遠端裝置上的 Visual Studio 模擬器上的 UWP 應用程式。  
...
-   從 [ **選取遠端偵錯工具連接** ] 對話方塊中選擇遠端電腦。  
    ...
    -   針對 c + + 應用程式中，選擇向下的箭號，在**機器名稱**方塊，然後選擇**\<尋找...> >**。  
 ...
 您可以部署和偵錯 Windows Phone 應用程式在裝置上或其中一個 Visual Studio phone 模擬器。 從 [目標裝置]  清單選取裝置或模擬器。  
...
|**僅限 GPU**|對在圖形處理器 (GPU) 上執行的原生 C++ 程式碼進行偵錯。|  
 ...
 Windows Phone 應用程式，您也可以選擇偵錯工具来用於背景處理序，從**背景工作處理序**。  
