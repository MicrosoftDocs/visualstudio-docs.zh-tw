---
title: 瀏覽程式碼與 Visual Studio 中偵錯工具 |Microsoft Docs
...
ms.locfileid: "49933764"
---
# <a name="navigate-code-with-the-visual-studio-debugger"></a>瀏覽 Visual Studio 偵錯工具的程式碼
熟悉命令和快速鍵中偵錯工具巡覽程式碼，可就變得更快速且輕易地尋找和解決您的應用程式中的問題。 當您瀏覽程式碼偵錯工具中的時，您可以檢查您的應用程式的狀態，或深入了解其執行流程。  
...
## <a name="start-debugging"></a>開始偵錯  
 ...
 大部分的偵錯工具功能，例如 [區域變數] 視窗中檢視變數的值，或在 [監看式] 視窗中評估運算式可用，只在偵錯工具暫停時，才 (也稱為*中斷模式*)。 偵錯工具暫停時，應用程式狀態已暫止時函式、 變數和物件保留在記憶體中。 在中斷模式時，您可以檢查項目的位置和狀態，以尋找違規或錯誤。 針對某些專案類型，您也可以進行調整應用程式處於中斷模式時。 若要觀看示範這些功能的影片，請參閱[Getting Started with 偵錯工具](https://www.youtube.com/watch?v=FtGCi5j30YU&list=PLReL099Y5nRfw6VNvzMkv0sabT2crbSpK&index=6)。
...
-   **執行至點選處** 
    ...
    若要執行的點，在您的程式碼中偵錯工具暫停時，選取**執行到這裡**（您將看到程式碼行暫留時的圖示） 的綠色箭號圖示。 這樣就不需要設定暫時中斷點。
...
-   **執行至名稱所指定的函式**  
     ...
     ![[新增中斷點] 對話方塊](../debugger/media/dbg_execution_newbreakpoint.png "DBG_Execution_NewBreakpoint")  
...
>   -   啟用 [編輯後繼續] 時，如果您進行了 [編輯後繼續] 無法立即重新對應的編輯作業，[ **設定下一個陳述式** ] 就會失敗。 舉例來說，如果您編輯了 catch 區塊內的程式碼，就會發生這種情況。 當發生這種情況時，您會看到錯誤訊息，告訴您此作業不支援。  
> 
> [!NOTE]
