...
ms.locfileid: "52389652"
---
# <a name="project-settings-for-a-c-debug-configuration"></a>C++ 偵錯組態的專案設定
您可以變更專案設定中的 C 或 Visual c + + 偵錯組態**屬性頁** 對話方塊中所述[如何： 設定偵錯和發行組態](../debugger/how-to-set-debug-and-release-configurations.md)。 下表顯示 [屬性頁] 對話方塊中與偵錯工具相關的設定位置。  
...
| **其他要部署的檔案** (遠端 Windows 偵錯工具) | 如果部署目錄 屬性設定，這是要複製到部署目錄中的其他檔案以分號分隔清單。 預設設定為空白，表示不會將其他檔案複製到部署目錄。 若要啟用檔案部署，您也必須選取 [組態管理員] 對話方塊中的 [部署] 核取方塊。 如需詳細資訊，請參閱[如何：建立和編輯組態](../ide/how-to-create-and-edit-configurations.md)。 |
