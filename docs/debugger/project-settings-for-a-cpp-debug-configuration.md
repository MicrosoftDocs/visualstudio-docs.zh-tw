...
| **部署目錄** (遠端 Windows 偵錯工具) | 指定遠端電腦上的路徑，在啟動之前專案輸出會複製到這個路徑中。 路徑可以是遠端電腦上的網路共用，或是遠端電腦上資料夾的路徑。 預設設定為空白，表示專案輸出不會複製到網路共用。 若要啟用檔案部署，您也必須選取 [組態管理員] 對話方塊中的 [部署] 核取方塊。 如需詳細資訊，請參閱[＜How to：建立及編輯組態](../ide/how-to-create-and-edit-configurations.md)。 |
| **其他要部署的檔案** (遠端 Windows 偵錯工具) | 如果部署目錄 屬性設定，這是要複製到部署目錄中的其他檔案以分號分隔清單。 預設設定為空白，表示不會將其他檔案複製到部署目錄。 若要啟用檔案部署，您也必須選取 [組態管理員] 對話方塊中的 [部署] 核取方塊。 如需詳細資訊，請參閱[＜How to：建立及編輯組態](../ide/how-to-create-and-edit-configurations.md)。 |
...
## <a name="see-also"></a>另請參閱  
 ...
 [建立及管理 Visual c + + 專案](/cpp/ide/creating-and-managing-visual-cpp-projects)   
