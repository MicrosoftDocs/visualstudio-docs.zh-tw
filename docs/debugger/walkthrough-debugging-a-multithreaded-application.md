...
ms.locfileid: "51607714"
---
# <a name="view-threads-in-the-visual-studio-debugger-by-using-the-threads-window"></a>在 Visual Studio 偵錯工具中使用 [執行緒] 視窗中檢視往來文章
在 [**執行緒**] 視窗中，您可以檢查及使用您要偵錯應用程式中的執行緒。 如需如何使用的逐步指引**執行緒** 視窗中，請參閱[逐步解說： 使用執行緒視窗進行偵錯](../debugger/how-to-use-the-threads-window.md)。

## <a name="use-the-threads-window"></a>使用執行緒視窗 
 **執行緒**視窗包含的資料表，其中每個資料列描述您的應用程式中的個別執行緒。 根據預設，資料表會列出您的應用程式中的所有執行緒，但是您可以篩選清單，顯示您感興趣的執行緒。 每個資料行描述不同類型的資訊。 您也可以隱藏部分資料行。 如果您顯示所有資料行時，下列資料行出現，請從左到右：  
  
- **旗標**： 在此未標記的專欄中，您可以將標記您要特別注意的執行緒。 如需如何在執行緒的旗標的資訊，請參閱[如何： 旗標並將執行緒取消標幟](../debugger/how-to-flag-and-unflag-threads.md)。  
...
- **處理序名稱**: 進階資料行 （預設為隱藏），顯示每個執行緒所屬的程序。 當您偵錯的許多程序，此資料行中的資料可以是很有用。  
...
## <a name="display-flagged-threads"></a>顯示已標幟的執行緒  
 您可以加上旗標的執行緒，您想要特別注意的標記中的圖示**執行緒**視窗。 如需詳細資訊，請參閱 <<c0> [ 如何： 將執行緒加上旗標和取消](../debugger/how-to-flag-and-unflag-threads.md)。 在 [**執行緒**] 視窗中，您可以選擇顯示所有執行緒或僅已標幟的執行緒。  
...
## <a name="group-and-sort-threads"></a>群組和排序執行緒  
 當您群組執行緒時，標題會出現在每個群組的表格中。 標題包含群組描述，例如**背景工作執行緒**或是**未標幟的執行緒**，和樹狀目錄控制項。 每個群組的成員執行緒會出現在群組標題下方。 如果您想要隱藏群組的成員執行緒，可用於樹狀結構控制項中摺疊群組。  
...
## <a name="search-for-specific-threads"></a>搜尋特定執行緒  
 您可以搜尋符合指定的字串中的執行緒**執行緒**視窗。 當您搜尋執行緒時，視窗會顯示比對任何資料行中的搜尋字串的所有執行緒。 這項資訊包括出現在呼叫堆疊中頂端的執行緒位置**位置**資料行。 根據預設，不被搜尋的完整呼叫堆疊。  
  
### <a name="to-search-for-specific-threads"></a>若要搜尋特定執行緒  
  
1. 在頂端的工具列**執行緒**視窗中，移至**搜尋**方塊，並：  
...
2. （選擇性）若要完整的呼叫堆疊併入搜尋中，選取**搜尋呼叫堆疊**。   
