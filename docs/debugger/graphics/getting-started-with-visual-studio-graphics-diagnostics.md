...
在繼續之前，請確定您可以建立並執行應用程式。 選擇 [**組建**  >  **組建方案**] 以確定它的組建不會發生錯誤。 然後選擇 [ **Debug**] [  >  **啟動但不**進行偵測] （**Ctrl + F5**），以確定它能正確執行。 視您使用工具測試的機器而定，您可能需要調整範例的平臺和偵錯工具目標。 例如，若要對 Visual Studio 主機電腦上的 x64 平臺進行測試，請選擇 [ **x64** ] 做為 [方案平臺] 和 [**本機電腦**] 作為您的偵錯工具目標。 
