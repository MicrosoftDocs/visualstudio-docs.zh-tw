 ...
 **輸入**組合語言輸入組合器會讀取您的應用程式所指定的索引和頂點資料，並將其組裝以供圖形硬體使用。
 ...
 **頂點著色器**頂點著色器階段會處理頂點，通常是執行轉換、外觀和光源等作業。 頂點著色器所產生的頂點數目會與它們採用為輸入的頂點數目相同。
...
> 如果 `POSITION` 或 `SV_POSITION` 語意未出現在頂點著色器輸出中，[頂點著色器] 階段就不會顯示任何內容。
 ...
 輪廓**著色器**（僅限 direct3d 11 和 direct3d 12）：輪廓著色器階段會處理定義低順序介面（例如線條、三角形或四個）的控制點。 而輸出結果是，它會產生傳遞給固定函式鑲嵌式階段的較高階幾何修補和修補常數。

 在 [管線階段] 視窗中，不會視覺化輪廓著色器階段。

 **鑲嵌階段**（僅限 direct3d 11 和 direct3d 12）鑲嵌階段是固定的函式（不可程式化）硬體單位，可預先設置由輪廓著色器輸出所代表的網域。 而輸出結果是，它會建立網域的取樣模式以及一組連接這些樣本的較小基本項目 (點、線條、三角形)。

 在 [管線階段] 視窗中，不會視覺化鑲嵌階段。

 **網域著色器**（僅限 direct3d 11 和 direct3d 12）網域著色器階段會處理來自「鑲嵌」階段的更高順序幾何修補程式，並將鑲嵌式貼圖的因素組成一維。 鑲嵌式因素可以包含鑲嵌輸入因素和輸出因素。 而輸出結果是，它會根據鑲嵌因素計算輸出修補上某個點的頂點位置。
 ...
 **資料流程輸出階段**資料流程輸出階段可以在點陣化之前攔截已轉換的基本專案，並將它們寫入記憶體;從該處，可以將資料 recirculated 為圖形管線先前階段的輸入，或由 CPU 讀回。

 在 [管線階段] 視窗中，不會視覺化資料流輸出階段。

 轉譯器**階段**「轉譯器」階段是固定的函式（不可程式化）硬體單位，會藉由執行掃描行轉換，將向量基本專案（點、線條、三角形）轉換成點陣影像。 在點陣化期間，頂點會轉換成同質剪輯空間並進行剪輯。 而輸出結果是，會對應像素著色器，而且每個頂點屬性都會內插到基本項目，並供像素著色器使用。

 在 [管線階段] 視窗中，不會視覺化轉譯器階段。

 **圖元著色器**圖元著色器階段會處理柵格化的基本專案以及插補頂點資料，以產生每個圖元的值，例如色彩和深度。

 在 [管線階段] 視窗中，像素著色器輸出會視覺化為全彩點陣影像。 若要進一步查看結果，請選取 [圖形管線階段] 視窗中的 [像素著色器]，以使用影像編輯器檢視所處理的基本項目。

 **輸出合併**輸出合併階段會將新呈現圖元的效果與其對應緩衝區的現有內容（色彩、深度和樣板）結合，以在這些緩衝區中產生新的值。
...
### <a name="vertex-and-geometry-shader-preview"></a>頂點和幾何著色器預覽
 當您在 [**管線階段**] 視窗中選取 [頂點] 或 [幾何著色器] 階段時，您可以在下方的面板中，看到著色器的輸入和輸出。  在這裡，您可以找到在輸入組合語言階段組合之後，提供給著色器之頂點清單的詳細資料。
