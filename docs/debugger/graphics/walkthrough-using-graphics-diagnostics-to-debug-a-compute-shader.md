...
2.  從轉譯模擬結果的 Draw 事件開始，會從每個先前的 `CSSetShader` 事件往前移動。 然後，在**圖形事件呼叫堆疊** 視窗中，選擇 瀏覽至 呼叫站台的最上層函式。 在呼叫位置，您可以使用的第一個參數[CSSetShader](/windows/desktop/api/d3d11/nf-d3d11-id3d11devicecontext-cssetshader)函式來判斷哪個計算著色器會執行下一個呼叫`Dispatch`事件。  
