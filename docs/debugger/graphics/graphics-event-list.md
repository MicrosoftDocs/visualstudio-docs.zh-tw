...
### <a name="event-list-views"></a>事件清單檢視
 事件清單支援兩個不同的檢視，而這兩個檢視透過不同的方式來組織圖形事件，以支援您的工作流程和喜好設定。 第一個視圖是*GPU 工作視圖*，它分層地組織事件及其關聯的狀態。 第二個檢視是 *「時間軸檢視」* (timeline view)，其依時間順序組織事件，且格式為一般清單。

 **GPU 工作**視圖在層次結構中顯示捕獲的事件及其狀態。 階層最上層是由事件 (例如繪製呼叫、清除、存在以及處理檢視的繪製呼叫、清除、存在) 所構成。 在事件清單中，您可以展開繪製呼叫，以顯示繪製呼叫時的目前裝置狀態，而且可以進一步展開每種狀態類型，以顯示設定其值的事件。 您也可以在此層級中，查看先前的畫面格中是否有設定特定狀態，或是否在最後一次繪製呼叫之後多次設定特定狀態。

 "**時間軸"** 視圖按時間順序顯示每個捕獲的事件。 這種組織事件清單的方式與舊版 Visual Studio 相同。

##### <a name="to-change-the-event-list-view-mode"></a>變更事件清單檢視模式

- 在事件清單上方的 **"圖形事件清單"** 視窗中，找到 **"視圖**"下拉式功能表並選擇 **"時間軸"** 視圖或**GPU 工作**視圖。
...
Visual Studio 2017 和更高包含**資源歷史記錄**視窗。  在 **"事件清單"** 視窗中](media/gfx_watch.png)的條目旁邊選擇手錶圖示![手錶圖示將打開如下所示**的資源歷史記錄**視窗：

![資源歷程記錄](media/gfx_diag_resource_history.png)

此視窗允許您查看事件清單中所選項的歷史記錄。  頂部的下拉可用於選擇要查看 的歷史記錄的其他項。  視窗的上半部分包含**幀設置事件**。  這些事件屬於 *"創建*類型"類別，是通常初始化和創建資源的調用。  視窗的下半部分包含 **"幀事件**"部分。  這些是使用資源期間發生的正常讀取和寫入事件。

| 資料行 | 描述 |
|-----------| - |
| **類型** | 顯示條目的類型，通常*創建*、*讀取*和*寫入*。 |
| **檢視** | 顯示該時間時刻的資源縮略圖。  按兩下縮略圖可打開此時資源的詳細資訊視圖。 |
| **事件** | 顯示建置事件的方法調用。  通過選擇相應行上的手錶圖示![手錶圖示](media/gfx_watch.png)，可以查看各個專案的任何其他歷史記錄。  此外，任何以藍色文本繪製的專案（如`m_commandList`上面的螢幕截圖中）都可以選擇以瞭解更多詳細資訊。 |
