...
### <a name="event-list-views"></a>事件清單檢視  
 事件清單支援兩個不同的檢視，而這兩個檢視透過不同的方式來組織圖形事件，以支援您的工作流程和喜好設定。 第一個檢視是*GPU 工作檢視*其中以階層方式組織事件以及其相關聯的狀態。 第二個檢視是 *「時間軸檢視」* (timeline view)，其依時間順序組織事件，且格式為一般清單。  
...
-   在**圖形事件清單**視窗上方列出事件，找出**檢視**下拉式清單並選擇**時間表**檢視或**GPU 工作**檢視。  
