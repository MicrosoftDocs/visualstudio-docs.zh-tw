...
### <a name="device-context-object"></a>裝置內容物件
 在 Direct3D 11 和 Direct3D 10 中，裝置內容 ([D3D11 裝置內容] 或 [D3D10 裝置]) 物件特別重要，因為其保存最重要的狀態資訊，而且會連結到目前設定的其他狀態物件。 裝置內容詳細資料會顯示在新的文件視窗中，而每種類別的資訊會顯示在其索引標籤上。選取新的事件時，裝置內容會變更，以反映目前的裝置狀態。
