...
圖形記錄文件是在圖形診斷工作階段下執行應用程式時所發生圖形事件的記錄。 錄製之後，您可以在 Visual Studio 圖形分析器中檢查記錄來診斷轉譯和效能問題。
 ...
 ![包含兩個已捕獲框架的圖形記錄檔。](media/gfx_diag_demo_graphics_log_orientation.png "gfx_diag_demo_graphics_log_orientation")
 ...
 **Direct3D 資訊**列出在 capture 會話期間所使用之顯示配接器的硬體和驅動程式功能的相關資訊。
 ...
 **顯示資訊**列出在 capture 會話期間所使用之顯示配接器的相關資訊。
 ...
 **實驗**檔案列出與 capture 會話相關聯的實驗檔案的資訊。

|屬性|描述|
|--------------|-----------------|
|**路徑**|.vsglog 檔案的路徑。 **注意：** 在舊版 capture 底下，未使用此屬性。|

 **模組資訊**列出應用程式在 capture 會話期間載入的動態連結程式庫（Dll）的名稱和版本。
 ...
 **目標應用程式**列出作為 capture 會話主旨之應用程式的相關資訊。
