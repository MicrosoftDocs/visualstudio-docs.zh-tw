...
1. 在 [**加入新專案**] 對話方塊中，選取 [ **Visual C++**  >  **公用程式**  >  **偵錯工具] 視覺效果檔案（. natvis）**。
...
Natvis 視覺化使用 C++ 運算式來指定要顯示的資料項目。 除了[內容運算子（c + +）](../debugger/context-operator-cpp.md)中描述之 c + + 運算式的增強功能和限制之外，請注意下列事項：
...
- 在 [**工具**  >  **選項**] （或 [**調試**程式  >  **選項**]）下 > **Debugging**  >  **輸出視窗**]，將**Natvis 診斷訊息（僅限 c + +）** 設定為 [**錯誤**]、[**警告**] 或 [**詳細**資訊 **]**，然後選取
...
1. 視覺效果應用於哪種類型（ `Name` 屬性）。

2. 該類型物件的值外觀應該為何 ( `DisplayString` 項目)。

3. 當使用者在變數視窗（節點）中展開類型時，該類型的成員應該看起來的樣子 `Expand` 。
...
在 `DisplayString` 運算式中， `x` 和 `y` （其為的成員 `CPoint` ）位於大括弧內，因此會評估其值。 此範例也會示範如何使用雙大括弧（或）來對大括弧進行換用 `{{` `}}` 。
...
- `ValuePointer`指向第一個專案的運算式（必須是不是元素類型的指標 `void*` ）。

該陣列的下限預設值為 0。 若要覆寫值，請使用 `LowerBound` 元素。 Visual Studio 隨附的*natvis*檔案有範例。

>[!NOTE]
>例如，您可以使用 `[]` 運算子搭配使用的 `vector[i]` 任何一維陣列視覺效果 `ArrayItems` ，即使類型本身（例如 `CATLArray` ）不允許這個運算子也一樣。
...
>[!NOTE]
>例如，您可以使用 `[]` 運算子搭配使用的 `vector[i]` 任何一維陣列視覺效果 `IndexListItems` ，即使類型本身（例如 `CATLArray` ）不允許這個運算子也一樣。
...
偵錯工具 `NextPointer` `ValueNode` 會在 node 元素的內容中評估和運算式 `LinkedListItems` ，而不是父清單類型。 在上述範例中， `CAtlList` 有一個 `CNode` 類別（在中找到 `atlcoll.h` ），它是連結清單的節點。 `m_pNext`和 `m_element` 是該類別的欄位，而不是類別的欄位 `CNode` `CAtlList` 。
...
Natvis 不支援基本類型（例如， `int` 、）或基本型別指標的視覺化檢視 `bool` 。 在此案例中，其中一個選項是使用適用于您使用案例的[格式規範](../debugger/format-specifiers-in-cpp.md)。 例如，如果您在程式 `double* mydoublearray` 代碼中使用，則可以在偵錯工具的 **[監看**式] 視窗中使用陣列格式規範，例如運算式 `mydoublearray, [100]` ，這會顯示前100個元素。
