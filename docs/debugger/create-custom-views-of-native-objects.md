...
---
# <a name="create-custom-views-of-c-objects-in-the-debugger-using-the-natvis-framework"></a>使用 Natvis 架構，在偵錯工具中建立 c + + 物件的自訂視圖
...
套用 `TextBox` Natvis 自訂視覺化檢視規則時，在變數視窗中的外觀會變得更簡單。 類別的重要成員會一起出現，而偵錯工具會顯示自訂字串類型的基礎字串值。

![使用視覺化檢視的 TextBox 資料](../debugger/media/dbg_natvis_textbox_visualizer.png "使用視覺化檢視的 TextBox 資料")

## <a name="use-natvis-files-in-c-projects"></a><a name="BKMK_Using_Natvis_files"></a>使用 c + + 專案中的 natvis 檔案

Natvis 會使用 *Natvis* 檔案來指定視覺效果規則。 *Natvis*檔案是副檔名為*natvis*的 XML 檔案。 Natvis 架構定義于 *%VSINSTALLDIR%\Xml\Schemas\natvis.xsd*中。

*Natvis*檔案的基本結構是一或多個 `Type` 代表視覺效果專案的元素。 每個元素的完整名稱 `Type` 會在其屬性中指定 `Name` 。
...
1. 在**方案總管**中選取 c + + 專案節點，然後選取 [**專案**] [  >  **加入新專案**]，或以滑鼠右鍵按一下專案，然後選取 [**加入**  >  **新專案**]。

1. 在 [**加入新專案**] 對話方塊中，選取**Visual C++**  >  **公用程式**  >  **偵錯工具視覺效果檔案 (. natvis) **。

1. 將檔案命名為，然後選取 [ **新增**]。

   新的檔案會新增至 **方案總管**，並在 [Visual Studio 檔] 窗格中開啟。

Visual Studio 偵錯工具會自動載入 c + + 專案中的 *natvis* 檔案，而且根據預設，在專案建立時，也會將這些檔案包含在 *.pdb* 檔案中。 如果您在建立應用程式時，偵錯工具會從 *.pdb*檔案載入*natvis*檔案，即使您沒有開啟專案也一樣。 如果您不想要將 *natvis* 檔案包含在 *.pdb*中，可以將它從建立的 *.pdb* 檔案中排除。
...
>[!NOTE]
>針對可執行檔專案，請使用方案專案來加入不在 *.pdb*中的任何*natvis*檔案，因為沒有可用的 c + + 專案。

>[!NOTE]
>從 *.pdb* 載入的 Natvis 規則僅適用于 *.pdb* 所參考模組中的類型。 例如，如果 [ *Module1* ] 具有名為之類型的 Natvis 專案 `Test` ，它只會套用至 `Test` *Module1.dll*中的類別。 如果另一個模組也定義了名為的類別 `Test` ，則 Natvis 專案不適用。 *Module1.pdb*
...
2. 載入的 c + + 專案或最上層方案中的任何 *natvis* 檔案。 這個群組包括所有已載入的 c + + 專案，包括類別庫，但不包含其他語言的專案。

3. 透過 VSIX 封裝安裝和註冊的任何 *natvis* 檔案。

::: moniker range="vs-2017"

4. 使用者專屬的 Natvis 目錄 (例如 *%USERPROFILE%\Documents\Visual Studio 2017 \ 視覺化* 程式) 。

::: moniker-end

::: moniker range=">= vs-2019"

4. 使用者特定的 Natvis 目錄 (例如， *%USERPROFILE%\Documents\Visual Studio 2019 \*) 的視覺化程式。
...
如果您修改 Visual Studio 外部的 *natvis* 檔案，變更不會自動生效。 若要更新偵錯工具視窗，您可以**在 [即時**運算] 視窗中重新評估 **.natvisreload**命令。 然後，變更就會生效，而不需要重新開機調試進程。
...
Natvis 視覺化使用 C++ 運算式來指定要顯示的資料項目。 除了在偵錯工具中的 c + + 運算式的增強功能和限制（如內容運算子中所述） [ (c + +) ](../debugger/context-operator-cpp.md)，請注意下列事項：

- Natvis 運算式是在正在視覺化之物件的內容 (而非目前的堆疊框架) 中進行評估。 例如， `x` 在 Natvis 運算式中，會參考正在視覺化的物件中名為 **x** 的欄位，而不是目前函式中名為 **x** 的本機變數。 您無法存取 Natvis 運算式中的區域變數，雖然您可以存取全域變數。
...
- 若要控制運算式的顯示方式，您可以使用 [c + + 中的格式](format-specifiers-in-cpp.md#BKMK_Visual_Studio_2012_format_specifiers)規範中所述的任何格式規範。 Natvis 內部使用專案時，會忽略格式規範，例如 `Size` [ArrayItems 擴充](../debugger/create-custom-views-of-native-objects.md#BKMK_ArrayItems_expansion)中的運算式。

## <a name="natvis-views"></a>Natvis 檢視

您可以定義不同的 Natvis 視圖，以不同的方式來顯示類型。 例如，以下的視覺效果會 `std::vector` 定義名為的簡化視圖 `simple` 。 `DisplayString`和元素會 `ArrayItems` 顯示在預設的視圖和視圖中 `simple` ，而 `[size]` 和專案則 `[capacity]` 不會顯示在 `simple` 視圖中。
...
- 在 [**工具**  >  **選項**] (或 [**調試**程式  >  **選項**]) > 的**調試**程式]  >  **輸出視窗**，將 [ **Natvis 診斷訊息 (c + + 僅) ** ] 設定為 [**錯誤** **]、[****警告**] 或 [**詳細**資訊]
...
`AutoVisualizer`元素是*natvis*檔案的根節點，且包含 namespace `xmlns:` 屬性。
...
1. 視覺效果應該用來 (屬性) 的類型 `Name` 。

2. 該類型物件的值外觀應該為何 ( `DisplayString` 項目)。

3. 當使用者在變數視窗中展開類型 (節點) 時，該類型的成員應會顯示的樣子 `Expand` 。

#### <a name="templated-classes"></a>樣板化類別
`Name`元素的屬性 `Type` 接受星號 `*` 做為可用於樣板化類別名稱的萬用字元。

在下列範例中，不論物件是或，都會使用相同的視覺效果 `CAtlArray<int>` `CAtlArray<float>` 。 如果有特定的視覺效果專案，則 `CAtlArray<float>` 其優先順序會高於泛型一個。
...
您可以使用宏 $T 1、$T 2 等等，來參考視覺效果專案中的樣板參數。 若要尋找這些宏的範例，請參閱 Visual Studio 隨附的 *natvis* 檔案。

#### <a name="visualizer-type-matching"></a><a name="BKMK_Visualizer_type_matching"></a> 視覺化類型對應
如果視覺效果專案無法驗證，則會使用下一個可用的視覺效果。

#### <a name="inheritable-attribute"></a>Inheritable 屬性
選擇性 `Inheritable` 屬性會指定視覺效果只適用于基底類型，還是套用至基底類型和所有衍生類型。 `Inheritable` 的預設值為 `true`。

在下列範例中，視覺效果只適用于 `BaseClass` 類型：
...
`Priority`如果定義無法剖析，選擇性屬性會指定使用替代定義的順序。 可能的值 `Priority` 為： `Low` 、 `MediumLow` 、 `Medium` 、 `MediumHigh` 和 `High` 。 預設值是 `Medium`。 `Priority`屬性只會區分同一個*natvis*檔案中的優先順序。
...
您可以將 `Optional` 屬性放在任何節點上。 如果選擇性節點內的子運算式無法剖析，則偵錯工具會忽略該節點，但會套用其餘的 `Type` 規則。 在下列類型中， `[State]` 是非選擇性的，但 `[Exception]` 是選擇性的。  如果 `MyNamespace::MyClass` 具有名為 _ 的欄位， `M_exceptionHolder` 則 `[State]` 會同時顯示節點和 `[Exception]` 節點，但如果沒有任何 `_M_exceptionHolder` 欄位，則只 `[State]` 會顯示節點。
...
選擇性 `Condition` 屬性適用于許多視覺效果元素，並指定使用視覺效果規則的時機。 如果 condition 屬性內的運算式解析為 `false` ，則不會套用視覺效果規則。 如果評估為 `true` ，或沒有 `Condition` 屬性，則會套用視覺效果。 您可以將這個屬性用於視覺效果專案中的 else 邏輯。

例如，下列視覺效果有兩個 `DisplayString` 適用于智慧型指標類型的元素。 當 `_Myptr` 成員是空的時，第一個專案的條件會 `DisplayString` 解析為 `true` ，如此表單就會顯示。 當 `_Myptr` 成員不是空的時，條件會評估為 `false` ，而第二個 `DisplayString` 元素會顯示。
...
您可以在 `IncludeView` `ExcludeView` 類型和個別成員上使用和屬性。

### <a name="version-element"></a><a name="BKMK_Versioning"></a> Version 元素
元素會將 `Version` 視覺化專案範圍設為特定模組和版本。 `Version`元素有助於避免名稱衝突、減少不小心的不符，並允許不同類型版本的不同視覺效果。

如果不同模組所使用的通用標頭檔定義類型，則只有當類型在指定的模組版本中時，才會顯示版本化的視覺效果。

在下列範例中，視覺效果僅適用于 `DirectUI::Border` `Windows.UI.Xaml.dll` 從版本1.0 到1.5 中找到的類型。
...
表示類型的變數會 `CPoint` 顯示，如下圖所示：

 ![使用 DisplayString 元素](../debugger/media/dbg_natvis_cpoint_displaystring.png "使用 DisplayString 元素")

在 `DisplayString` 運算式中， `x` 和 `y` （其為的成員 `CPoint` ）位於大括弧內，因此會評估其值。 此範例也會示範如何使用雙大括弧 ( 或 ) 來對大括弧進行 escape `{{` `}}` 。
...
`StringView`元素會定義偵錯工具可以傳送至內建文字視覺化檢視的值。 例如，假設有下列類型的視覺效果 `ATL::CStringT` ：
...
加入 `StringView` 元素會告訴偵錯工具它可以將值顯示為文字視覺效果。
...
- 如果 `Expand` 視覺效果專案中未指定節點，則子系會使用預設的擴充規則。

- 如果 `Expand` 指定的節點沒有子節點，則在偵錯工具視窗中不會展開該類型。

#### <a name="item-expansion"></a><a name="BKMK_Item_expansion"></a> 專案展開

 `Item`元素是節點中最基本和通用的元素 `Expand` 。 `Item` 定義單一子項目。 例如， `CRect` 具有欄位 `top` 、、 `left` `right` 和 `bottom` 的類別具有下列視覺效果專案：
...
#### <a name="arrayitems-expansion"></a><a name="BKMK_ArrayItems_expansion"></a> ArrayItems 擴充
...
- `ValuePointer`指向第一個專案 (的運算式，此專案必須是不) 之元素類型的指標 `void*` 。

該陣列的下限預設值為 0。 若要覆寫值，請使用 `LowerBound` 元素。 Visual Studio 隨附的 *natvis* 檔案有範例。

>[!NOTE]
>例如，您可以使用 `[]` 運算子，搭配 `vector[i]` 使用的任何一維陣列視覺效果 `ArrayItems` ，即使類型本身 (例如 `CATLArray`) 不允許這個運算子。
...
- `Rank` 指定陣列的陣序。
- `Size` 元素接受隱含的 `$i` 參數，並將參數取代為維度索引，以便在該維度中尋找陣列的長度。 在上述範例中，運算式 `_M_extent.M_base[0]` 應提供第0個維度的長度、 `_M_extent._M_base[1]` 第1個，依此類推。
...
`ArrayItems`只有在陣列元素在記憶體中連續配置時，您才可以使用擴充。 偵錯工具會藉由只遞增其指標來取得下一個元素。 如果您需要操作值節點的索引，請使用 `IndexListItems` 節點。 以下是具有節點的視覺效果 `IndexListItems` ：
...
和之間的唯一差異在於 `ArrayItems` `IndexListItems` `ValueNode` ，它預期<sup>第</sup> i 個元素的完整運算式具有隱含 `$i` 參數。

>[!NOTE]
>例如，您可以使用 `[]` 運算子，搭配 `vector[i]` 使用的任何一維陣列視覺效果 `IndexListItems` ，即使類型本身 (例如 `CATLArray`) 不允許這個運算子。

#### <a name="linkedlistitems-expansion"></a><a name="BKMK_LinkedListItems_expansion"></a> LinkedListItems 擴充

如果視覺化類型代表連結清單，則偵錯工具可以使用 `LinkedListItems` 節點顯示其子系。 下列類型的視覺效果 `CAtlList` 使用 `LinkedListItems` ：
...
偵錯工具 `NextPointer` `ValueNode` 會在 node 元素的內容中評估和運算式 `LinkedListItems` ，而不是父清單類型。 在上述範例中， `CAtlList` 有一個 `CNode` 類別 (在 `atlcoll.h` 連結清單的節點) 中找到。 `m_pNext` 和 `m_element` 是該類別的欄位，而不是類別的欄位 `CNode` `CAtlList` 。

`ValueNode` 可以保留空白，或使用 `this` 來參考 `LinkedListItems` 節點本身。

#### <a name="customlistitems-expansion"></a>CustomListItems 展開

`CustomListItems` 展開可讓您撰寫周遊資料結構 (例如雜湊表) 的自訂邏輯。 使用 `CustomListItems` 來視覺化資料結構，可以針對您需要評估的所有專案使用 c + + 運算式，但不適合 `ArrayItems` 、 `IndexListItems` 或的模具 `LinkedListItems` 。

您可以使用在擴充中 `Exec` `CustomListItems` 定義的變數和物件，在擴充內執行程式碼。 您可以搭配使用邏輯運算子、算術運算子和指派運算子 `Exec` 。 您不能使用 `Exec` 來評估函式，但 c + + 運算式評估工具支援的 [偵錯工具內建函式](../debugger/expressions-in-the-debugger.md#BKMK_Using_debugger_intrinisic_functions_to_maintain_state) 除外。

的下列視覺化 `CAtlMap` 程式是適合的絕佳範例 `CustomListItems` 。
...
#### <a name="treeitems-expansion"></a><a name="BKMK_TreeItems_expansion"></a> TreeItems 擴充
...
語法類似于 `LinkedListItems` 節點。 `LeftPointer`、 `RightPointer` 和 `ValueNode` 會在樹狀節點類別的內容下進行評估。 `ValueNode` 可以保留空白或用 `this` 來參考 `TreeItems` 節點本身。

#### <a name="expandeditem-expansion"></a><a name="BKMK_ExpandedItem_expansion"></a> ExpandedItem 展開
 `ExpandedItem`元素會藉由顯示基類或資料成員的屬性，來產生匯總的子視圖，就好像它們是視覺化類型的子系。 偵錯工具會評估指定的運算式，並將結果的子節點附加至視覺化類型的子清單。
...
下列範例顯示如何從衍生類別中的基類匯總屬性。 假設 `CPanel` 類別衍生自 `CFrameworkElement`。 節點視覺效果不會重複來自基類的屬性，而是將 `CFrameworkElement` `ExpandedItem` 這些屬性附加至類別的子清單 `CPanel` 。
...
此處需要 **nd** 格式規範，以關閉衍生類別的視覺化比對。 否則，運算式會 `*(CFrameworkElement*)this` 導致再次套用 `CPanel` 視覺效果，因為預設的視覺效果類型比對規則會將它視為最適合的類別。 使用 **nd** 格式規範來指示偵錯工具使用基類視覺效果，如果基類沒有視覺效果，則為預設展開。
...
### <a name="hresult-element"></a><a name="BKMK_HResult"></a> HResult 元素
 `HResult`元素可讓您自訂偵錯工具視窗中的**HRESULT**所顯示的資訊。 `HRValue`元素必須包含要自訂之**HRESULT**的32位值。 `HRDescription`元素包含要在偵錯工具視窗中顯示的資訊。
...
- 屬性組會 `ServiceId`  -  `Id` 識別 `UIVisualizer` 。 `ServiceId`是視覺化檢視封裝所公開之服務的 GUID。 `Id` 如果服務提供一個以上的視覺化檢視，這就是區分其的唯一識別碼。 在上述範例中，相同的視覺化服務提供兩種視覺化檢視。
...
*Natvis*檔案中定義的每個類型都必須明確列出可以顯示它的任何 UI 視覺化程式。 偵錯工具會比對類型專案中的視覺化檢視參考與已註冊的視覺化檢視。 例如，下列的類型專案會 `std::vector` 參考 `UIVisualizer` 上述範例中的。
...
```
 ...
 您可以 `UIVisualizer` 在 [ [影像監看式]](https://marketplace.visualstudio.com/search?term=%22Image%20Watch%22&target=VS&category=All%20categories&vsVersion=&sortBy=Relevance) 延伸模組中查看用來查看記憶體中點陣圖的範例。

### <a name="customvisualizer-element"></a><a name="BKMK_CustomVisualizer"></a>CustomVisualizer 項目
 `CustomVisualizer` 是指定 VSIX 延伸模組的擴充點，您可以撰寫它來控制 Visual Studio 程式碼中的視覺效果。 如需撰寫 VSIX 擴充功能的詳細資訊，請參閱 [VISUAL STUDIO SDK](../extensibility/visual-studio-sdk.md)。

撰寫自訂的視覺化程式比 XML Natvis 定義更多，但您不受限於 Natvis 不支援的條件約束。 自訂的視覺化程式可以存取一組完整的偵錯工具擴充性 Api，這可查詢和修改偵錯工具的進程，或與 Visual Studio 的其他部分通訊。

 您可以在 `Condition` `IncludeView` 元素上使用、和 `ExcludeView` 屬性 `CustomVisualizer` 。

## <a name="limitations"></a>限制

Natvis 自訂會使用類別和結構，但不能用於 typedef。

Natvis 不支援基本類型 (的視覺化檢視，例如， `int` `bool`) 或基本型別的指標。 在此案例中，其中一個選項是使用適用于您使用案例的 [格式規範](../debugger/format-specifiers-in-cpp.md) 。 例如，如果您在程式 `double* mydoublearray` 代碼中使用，則可以在偵錯工具的 **[監看** 式] 視窗中使用陣列格式規範，例如運算式 `mydoublearray, [100]` ，這會顯示前100個元素。
