...
- 如果您想要從原生 DLL 中斷 managed 呼叫應用程式中的程式碼，反之亦然，請啟用 [混合模式的調試](../debugger/how-to-debug-in-mixed-mode.md)程式。

- 在某些情況下，您可能需要告訴偵錯工具哪裡可以找到原始程式碼。 如需詳細資訊，請參閱 [使用未載入符號/沒有來源載入頁面](../debugger/specify-symbol-dot-pdb-and-source-files-in-the-visual-studio-debugger.md#use-the-no-symbols-loadedno-source-loaded-pages)。
