...
1. 開啟 Visual Studio 並建立新專案。
   ...
   在 [**設定您的新專案**] 視窗的 [**專案名稱**] 方塊中，輸入名稱或使用預設名稱。 然後選擇 [**建立**]。
...
1. 若要繼續執行至叫用第四個中斷點為止，請按一下 [偵錯]**** 功能表的 [繼續]****。
     ...
     ![[平行堆疊] 視窗中的鳥瞰&#45;眼睛](../debugger/media/pdb_walkthrough_5.png "PDB_Walkthrough_5")
...
1. 若要繼續執行至叫用第二個中斷點為止，請按一下 [偵錯]**** 功能表的 [繼續]****。
     ...
     工作 4 又在等待指派給工作 2 的執行緒所擁有的監視器。 （以滑鼠右鍵按一下標題**欄**  >  ，然後選擇 [資料行]**執行緒指派**，用來查看工作2的執行緒指派值）。
...
1. 若要在遇到第三個中斷點之前繼續執行，請在 [偵錯]**** 功能表上，按一下 [繼續]****。
     ...
     ![[工作] 視窗中的父&#45;子視圖](../debugger/media/pdb_walkthrough_9.png "PDB_Walkthrough_9")
...
1. 若要在遇到第三個中斷點之前繼續執行，請在 [偵錯]**** 功能表上，按一下 [繼續]****。 按一下 [識別碼]**** 資料行標頭，依識別碼排序。 您應該會看到下圖。
     ...
     因為工作 5 已完成，所以不會再出現。 如果您的電腦不是這種情況，而且未顯示鎖死，請按**F11**逐步執行一次。
