...
1. 使用 [以**系統管理員身分執行**] 選項啟動 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]。
...
1. 在**方案總管**中，開啟*WebPart1* （在 Visual Basic 中）或*WebPart1.cs* （在C#中）。
...
4. 將下列程式碼加入 `WebPart1` 類別。 此程式碼會將名為 `DataFilePath` 的自訂屬性新增至 Web 元件。 自訂屬性是可由使用者在 SharePoint 中設定的屬性。 這個屬性會取得和設定用來填入資料方格之 XML 資料檔的位置。
...
按一下 [Web 元件] 動詞功能表中所顯示的專案，顯示和隱藏非管理員的員工。
...
[如何：使用設計工具建立 sharepoint web](../sharepoint/how-to-create-a-sharepoint-web-part-by-using-a-designer.md)元件 
