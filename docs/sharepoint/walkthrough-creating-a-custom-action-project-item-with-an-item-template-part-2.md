...
# <a name="walkthrough-create-a-custom-action-project-item-with-an-item-template-part-2"></a>逐步解說：使用專案範本建立自訂動作專案專案（第2部分）
 ...
 在此逐步解說中，您會將嚮導加入至自訂動作專案專案，此專案會在[逐步解說：使用專案範本建立自訂動作專案專案（第1部分）](../sharepoint/walkthrough-creating-a-custom-action-project-item-with-an-item-template-part-1.md)中示範。 當使用者將自訂動作專案加入至 SharePoint 專案時，此 wizard 會收集自訂動作的相關資訊（例如，在使用者選擇時要流覽的位置和 URL），並將此資訊加入至新專案專案中的*Elements.xml*檔案。
...
5. 選擇 [ **WPF 使用者控制項程式庫**] 專案範本，將專案命名為**ItemTemplateWizard**，然後選擇 [**確定]** 按鈕。
     ...
     [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]將**ItemTemplateWizard**專案新增至方案。
...
## <a name="define-the-default-location-and-id-strings-for-custom-actions"></a>定義自訂動作的預設位置和識別碼字串
 每個自訂動作都有一個位置和識別碼，它是在Elements.xml檔案中專案的 `GroupID` 和屬性中指定 `Location` `CustomAction` 。 *Elements.xml* 在此步驟中，您會在 ItemTemplateWizard 專案中定義這些屬性的一些有效字串。 當您完成本逐步解說時，這些字串會在使用者于 wizard 中指定位置和識別碼時，寫入自訂動作專案專案中的*Elements.xml*檔案。
...
2. 在 XAML 視圖中，以下列 XAML 取代目前的 XAML。 XAML 會定義 UI，其中包含標題、用來指定自訂動作行為的控制項，以及視窗底部的導覽按鈕。
    ...
    > [!NOTE]
    > 在此 XAML 中建立的視窗衍生自 <xref:Microsoft.VisualStudio.PlatformUI.DialogWindow> 基類。 當您將自訂 WPF 對話方塊新增至 Visual Studio 時，建議您從這個類別衍生您的對話方塊，使其在 Visual Studio 和中的其他對話方塊具有一致的樣式，以避免出現強制回應對話方塊可能發生的問題。 如需詳細資訊，請參閱[建立和管理模式對話方塊](../extensibility/creating-and-managing-modal-dialog-boxes.md)。

3. 如果您正在開發 Visual Basic 專案，請 `ItemTemplateWizard` `WizardWindow` 在元素的屬性中移除類別名稱的命名空間 `x:Class` `Window` 。 這個元素位於 XAML 的第一行。 當您完成時，第一行應該類似下列程式碼：
...
## <a name="implement-the-wizard"></a>執行嚮導
 藉由執行介面來定義 wizard 的功能 <xref:Microsoft.VisualStudio.TemplateWizard.IWizard> 。
...
1. 在 Visual Studio 命令提示字元] 視窗中，執行下列命令，將*PathToWizardAssembly*取代為開發電腦上 ItemTemplateWizard 專案內建 ItemTemplateWizard.dll 元件的完整路徑。

    ```xml
    sn.exe -T PathToWizardAssembly
    ```

     *ItemTemplateWizard.dll*元件的公開金鑰 token 會寫入 [Visual Studio 命令提示字元] 視窗中。
...
2. 在檔案結尾附近， `WizardExtension` 于和標記之間加入下列專案 `</TemplateContent>` `</VSTemplate>` 。 將屬性的*YourToken*值取代 `PublicKeyToken` 為您在上一個程式中取得的公開金鑰 token。
...
## <a name="add-replaceable-parameters-to-the-elementsxml-file-in-the-item-template"></a>將可取代的參數新增至專案範本中的*Elements.xml*檔案
 將數個可取代的參數新增至 ItemTemplate 專案中的*Elements.xml*檔案。 這些參數會在您稍 `PopulateReplacementDictionary` 早定義之類別的方法中初始化 `CustomActionWizard` 。 當使用者將自訂動作專案新增至專案時，Visual Studio 會自動以在 [wizard] 中指定的值取代新專案專案中*Elements.xml*檔案中的這些參數。
...
1. 在 ItemTemplate 專案中，將*Elements.xml*檔案的內容取代為下列 XML。
...
6. 在功能表列上，選擇 [**組建**] [組建] [  >  **方案**]，然後確定方案會編譯而不會發生錯誤。
...
2. 在 ItemTemplateWizard 專案中，開啟 CustomActionWizard 程式碼檔案，然後在方法的第一行程式碼中加入中斷點 `RunStarted` 。
...
2. 展開 [ **Visual c #** ] 或 [ **Visual Basic** ] 節點（視專案範本支援的語言而定），展開 [ **SharePoint** ] 節點，然後選擇 [ **2010** ] 節點。
...
8. 確認 Visual Studio 的另一個實例中的程式碼會在您稍早于方法中設定的中斷點上停止 `RunStarted` 。

9. 選擇**F5**鍵，或在功能表列上選擇 [ **debug**Continue]，繼續進行專案的 debug  >  ** **。
