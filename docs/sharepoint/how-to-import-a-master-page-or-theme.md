...
# <a name="how-to-import-a-master-page-or-theme"></a>如何：匯入主版頁面或主題
  您可以建立和使用主版頁面和主題，讓 SharePoint 網站上的頁面具有一致的外觀。 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]不會提供這些專案的範本，但您可以在 SharePoint Designer 中建立這些專案，然後將它們匯入至 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 。 如需詳細資訊，請參閱 Microsoft 網站上的[建立區塊：頁面和使用者介面](/previous-versions/office/developer/sharepoint-2010/ee539040(v=office.14))。
...
7. 在功能表列上，選擇 [**專案**] [新增] [  >  **現有專案**]，然後選擇主版頁面或主題檔案。
...
9. 在模組中，開啟 [ *Elements.xml*]。
     ...
     您必須更新*Elements.xml*檔案，以參考您新增的主版頁面或主題。
...
10. 若為主版頁面，請將現有的模組標記取代為下列標記。
    ...
    ```
     ...
     屬性 `Type="GhostableInLibrary"` 會指出專案已加入至內容資料庫，而模組的屬性會 `Url` 指定要將檔案儲存在 SharePoint 內容資料庫中的位置。
