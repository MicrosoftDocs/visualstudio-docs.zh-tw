...
### <a name="to-add-a-bdc-model-file-to-a-sharepoint-project"></a>將 BDC 模型檔案加入至 SharePoint 專案

1. 在 [**方案總管**中，選擇 SharePoint 專案的資料夾。

2. 在功能表列上，選擇 [**專案**] [新增] [  >  **現有專案**]。

3. 在 [**加入現有專案**] 對話方塊中，流覽至您想要加入至專案之模型定義檔的位置，選擇檔案，然後選擇 [**加入**] 按鈕。

    如果模型未定義 *.net 元件類型的企業營運（LOB）系統*，則會開啟 [**加入 .net 元件 LobSystem** ] 對話方塊。
