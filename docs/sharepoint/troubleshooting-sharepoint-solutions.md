...
## <a name="error-the-site-location-is-not-valid"></a>錯誤：站台位置無效
...
### <a name="error-message"></a>錯誤訊息
 部署步驟啟用功能時發生錯誤：具有識別碼的欄位 [*Guid*] 功能中所定義 [*Guid*] 在目前網站集合或子網站中，找不到。
...
### <a name="error-message"></a>錯誤訊息
 建置錯誤：部署步驟啟用功能時發生錯誤：檔案 Template\Features\\[*匯入專案*<em>功能</em>*名稱*] \Files\Lists\\[*舊*<em>清單名稱</em>] \Schema.xml 不存在。
...
### <a name="error-message"></a>錯誤訊息
 無法載入 Web.config 組態檔。 檢查任何格式不正確的 XML 項目的檔案，並再試一次。 發生下列錯誤:指定的索引鍵不存在於字典中。
