...
1. 建立具有下列設定的類別庫專案：
    ...
    > [!NOTE]
    > 您無法在定義 SharePoint 工具延伸模組的相同專案中執行 SharePoint 命令，因為 SharePoint 命令的目標是 .NET Framework 3.5，而 SharePoint 工具延伸模組的目標為 [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] 。 您必須在個別的專案中定義擴充功能所使用的任何 SharePoint 命令。 如需詳細資訊，請參閱[在 Visual Studio 中部署 SharePoint 工具的擴充](../sharepoint/deploying-extensions-for-the-sharepoint-tools-in-visual-studio.md)功能。
...
## <a name="example"></a>範例
 下列程式碼範例示範具有識別碼的 SharePoint 命令 `Contoso.Commands.UpgradeSolution` 。 此命令會使用伺服器物件模型中的 Api 來升級至已部署的解決方案。
...
## <a name="deploying-the-command"></a>部署命令
 若要部署命令，請在相同的 [!include[vsprvs](../sharepoint/includes/vsprvs-md.md)] 擴充功能（*vsix*）封裝中包含命令元件，並搭配使用命令的延伸模組元件。 您也必須在 extension.vsixmanifest 檔案中新增命令元件的專案。 如需詳細資訊，請參閱[在 Visual Studio 中部署 SharePoint 工具的擴充](../sharepoint/deploying-extensions-for-the-sharepoint-tools-in-visual-studio.md)功能。
