...
---
# <a name="how-to-add-a-finder-method"></a>如何：加入 Finder 方法
  若要啟用商務資料連線（BDC）服務，以顯示 web 元件或清單中的實體清單，您必須建立*Finder*方法。 Finder 方法是一個特殊方法，會傳回實體實例的集合。 如需詳細資訊，請參閱[設計商務資料連線模型](../sharepoint/designing-a-business-data-connectivity-model.md)。
...
3. 在 [**加入方法**] 清單中，選擇 [**建立搜尋方法**]。

    Visual Studio 加入方法、傳回參數和型別描述元。

4. 將型別描述元設定為實體集合型別描述項。 如需如何建立實體集合類型描述元的詳細資訊，請參閱[如何：定義參數的類型描述](../sharepoint/how-to-define-the-type-descriptor-of-a-parameter.md)元。

   > [!NOTE]
   > 如果您已將特定的搜尋工具方法加入至實體，則不需要執行此步驟。 Visual Studio 會使用您在特定搜尋工具方法中所定義的類型描述元。
...
6. 將程式碼新增至搜尋工具方法。 此程式碼會執行下列工作：
   ...
   - 傳回 BDC 服務的實體清單。
     ...
     下列範例會 `Contact` 使用 SQL Server 的 AdventureWorks 範例資料庫中的資料，傳回實體的集合。
