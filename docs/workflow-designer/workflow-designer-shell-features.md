...
ms.locfileid: "72649726"
---
# <a name="workflow-designer-shell-features"></a>工作流程設計工具殼層功能

工作流程設計工具是由三個主要的 UI 區域所組成：設計工具介面、其上方的階層連結列和其底下的 shell。 位於畫面上方的階層連結列，會用於顯示目前根活動的祖系清單。 如需詳細資訊，請參閱[如何：使用](../workflow-designer/how-to-use-breadcrumb-navigation.md)階層連結流覽。 位於畫面中央的設計介面，會用於撰寫工作流程。 位於畫面下方的殼層，會包含許多管理目前檢視的按鈕。
...
Visual Studio 內工作流程設計工具所使用的字型是由環境字型所控制。 如果您針對作業系統主題使用高對比色彩配置，則顯示在工作流程設計工具變更中的色彩。 變更字型或色彩設定之後，您必須重新開機 Visual Studio，變更才會在工作流程設計工具中生效。