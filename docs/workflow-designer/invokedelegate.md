...
description: 瞭解 InvokeDelegate 設計工具，以及如何使用 InvokeDelegate 設計工具來建立和設定 InvokeDelegate 活動。
...
|<xref:System.Activities.Statements.InvokeDelegate.Delegate%2A>|對|活動執行時要呼叫之 <xref:System.Activities.ActivityDelegate> 的名稱。 這個屬性可以在設計工具介面上編輯，而且是必要的。|
