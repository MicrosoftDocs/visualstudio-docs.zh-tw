...
description: 瞭解如何使用 SendAndReceiveReply 範本來建立一對預先設定的傳送和 ReceiveReply 活動。
...
| <xref:System.ServiceModel.Activities.ReceiveReply.Request%2A> | 對 | 參考到與這個 <xref:System.ServiceModel.Activities.Send> 活動成對的 <xref:System.ServiceModel.Activities.ReceiveReply> 活動。 這個屬性不得為 **null** 。 <xref:System.ServiceModel.Activities.Send> 和 <xref:System.ServiceModel.Activities.ReceiveReply> 活動會一起用於用戶端上，以建立要求/回應訊息模式的模型。 這個屬性會指定哪個 <xref:System.ServiceModel.Activities.Send> 活動為成對的活動。 在設計工具中，您無法編輯這個屬性，因為它會自動系結至 <xref:System.ServiceModel.Activities.Send> 您用來建立 <xref:System.ServiceModel.Activities.ReceiveReply> 活動的活動。 |
