...
[ **CorrelationScope** ] 活動設計工具位於 [**工具箱**] 的 [**訊息**] 類別中，若要存取，請按一下工作流程設計工具左側的 [**工具箱**] 索引標籤。 或者，從  **View**  功能表中選取 **工具箱**，或按**Ctrl** +**Alt** +**X**。

[ **CorrelationScope** ] 活動設計工具可以從 [**工具箱**] 拖曳出來，放到工作流程設計工具介面上。 這會建立具有 CorrelationScope 預設**DisplayName**的 <xref:System.ServiceModel.Activities.CorrelationScope> 活動。 @No__t_0 可以在 [ **CorrelationScope** ] 活動設計工具的標頭中編輯，或是在 [**屬性**] 視窗的 [ **DisplayName** ] 方塊中編輯。

若要指定子訊息活動所使用的 <xref:System.ServiceModel.Activities.CorrelationHandle>，請選取 [**屬性**] 視窗中 [ **CorrelatesWith** ] 欄位旁邊的省略號按鈕，以顯示 [**運算式編輯器**] 對話方塊。 這個屬性也可以在活動設計工具介面上設定。
...
下表顯示 <xref:System.ServiceModel.Activities.CorrelationScope> 屬性，並且描述屬性在設計工具中的使用方式。 這些屬性可以在 [**屬性**] 視窗中或在工作流程設計工具介面上編輯，通常是在這兩者中。
