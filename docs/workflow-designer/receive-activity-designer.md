...
| <xref:System.ServiceModel.Activities.Receive.OperationName%2A> | 對 | 指定這個 <xref:System.ServiceModel.Activities.Receive> 活動實作之服務作業的名稱。 如果未明確設定 **action** 屬性，就會使用這個屬性來建立 **action** 屬性的預設值。 |
