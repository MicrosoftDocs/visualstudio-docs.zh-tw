...
description: 瞭解如何使用 [加入相互關聯初始化運算式] 對話方塊來設定傳送、接收和 SendReply 活動的 CorrelationInitializers 屬性。
