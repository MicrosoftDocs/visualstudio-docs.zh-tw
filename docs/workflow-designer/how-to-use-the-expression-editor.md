...
運算式是常值，或是繫結到引數或屬性的 Visual Basic 程式碼。 它們包含與作業結合的值元素（例如變數、常數、常值、屬性），以產生新的值。 運算式會使用 VB.NET 語法撰寫，即使應用程式使用 C# 也是如此。 這表示大小寫並不重要，比較是使用單一等號（"="，而不是 "= ="）來執行，而布林運算子是 "and" 和 "or"，而**不是符號**"& &" 和 "| |"，而不是使用**null**。 如需 Visual Basic 中的運算式和運算子和一些範例的詳細資訊，請參閱[Visual Basic 中的運算子和運算式](/previous-versions/visualstudio/visual-studio-2010/a1w3te48(v=vs.100))。
