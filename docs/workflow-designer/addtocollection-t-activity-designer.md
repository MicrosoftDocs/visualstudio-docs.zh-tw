...
[ **AddToCollection \<T >** ] 活動設計**工具**位於 [工具箱] 的 [**集合**] 類別中，若要存取，請按一下工作流程設計工具的 [**工具箱**] 索引標籤。 或者，從  **View**  功能表中選取 **工具箱**，或按**Ctrl** +**Alt** +**X**。

[ **AddToCollection \<T >** ] 活動設計工具可以從 [**工具箱**] 拖曳出來，放到工作流程設計工具介面上放置活動的任一處，例如 <xref:System.Activities.Statements.Sequence> 內部。 卸載**AddToCollection \<T >** 活動設計工具會建立 <xref:System.Activities.Statements.AddToCollection%601> 活動，預設 <xref:System.Activities.Activity.DisplayName%2A> 為 AddToCollection < Int32 \>。 （根據預設， *TypeArgument*為**Int32**。 TypeArgument 可以在屬性方格中變更）。@No__t_0 值可以在**AddToCollection < t \>** 活動設計工具的標頭中編輯，或是在屬性方格的 [ **DisplayName** ] 方塊中編輯。 其他的屬性必須在屬性方格上進行編輯。
