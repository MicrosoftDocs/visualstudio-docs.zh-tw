...
description: 瞭解如何使用 AddToCollection <T> 活動設計工具來建立和設定 AddToCollection <T> 活動。
...
|<xref:System.Activities.Statements.AddToCollection%601.Item%2A>|對|要加入至集合的專案 \<T> 。 此專案的類型為 *T* ，其類型為 *TypeArgument* 。 若要指定項目，請在屬性方格中輸入 Visual Basic 運算式。|
|<xref:System.Activities.Statements.AddToCollection%601.Collection%2A>|對|應該加入項目的集合。 此集合的類型為 **ICollection<TypeArgument \>** 。 若要指定集合，請在屬性方格中輸入 Visual Basic 運算式。|
