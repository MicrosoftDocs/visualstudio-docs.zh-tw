...
編輯器也與 Visual Studio 中的互動式視窗整合，在兩者間交換程式碼變得更輕鬆。 如需詳細資訊，請參閱 [使用者入門 - 使用互動式 REPL 視窗])(getting-started.md#using-the-interactive-repl-window) 和[使用互動式視窗 - 將程式碼傳送至互動式命令](interactive-repl.md#send-code-to-interactive-command)。
...
> [!Tip]
> 您可以透過 [工具] > [選項] > [文字編輯器] > [Python] > [進階] 來設定自動完成的行為。 其中，[依據搜尋字串篩選清單]︰在您輸入時套用自動完成建議的篩選 (預設為核取)，而 [成員自動完成顯示成員的交集] 只會顯示所有可能的類型支援的自動完成 (預設為未核取)。 請參閱[選項 - 完成結果](options.md#completion-results)。
...
> [!Tip]
> 若要停用程式碼著色，請移至 [工具] > [選項] > [文字編輯器] > [Python] > [進階]，清除 [其他選項] > [根據類型為名稱著色]。 請參閱[選項 - 其他選項](options.md#miscellaneous-options)。
...
透過[建立程式碼片段](https://msdn.microsoft.com/library/ms165394.aspx)可自訂程式碼片段，並透過...匯入 
