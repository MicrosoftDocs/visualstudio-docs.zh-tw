---
title: 適用於 Python 的 Cookiecutter 延伸模組
...
請注意，只有在加入檔案到現有專案時，`Python.InstallProjectRequirements` 命令才有效。 此限制的存在，是因為命令是由 [方案總管] 中的 Python 專案處理，而在 [方案總管] - [資料夾檢視] 中沒有可接收訊息的專案。 我們希望未來的版本中能解除此限制 (並在整體上提供更好的 [資料夾檢視] 支援)。
