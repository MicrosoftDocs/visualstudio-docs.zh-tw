...
1. `import` `from` 在 .py 檔案中選取、和 `make_dot_string` 函數語句 *.py* 。 以滑鼠右鍵按一下選取的文字，然後選擇 [**傳送到 Interactive** (或按**Ctrl** + **enter**) 。 程式碼片段會立即貼入 **互動式** 視窗並執行。 由於程式碼已定義函式，因此您可以藉由呼叫它幾次來快速測試該函式：
