...
1. 下載並執行最新的 Visual Studio 2017 安裝程式：
...
1. 安裝程式會顯示一份工作負載清單，而工作負載是特定開發區域的相關選項群組。 針對 Python，選取 [Python 開發] 工作負載。
    ...
    選擇性：如果您要使用資料科學，也請考慮 [資料科學與分析應用程式] 工作負載。 此工作負載支援 Python 以及 R 和 F# 語言。 如需詳細資訊，請參閱[資料科學與分析應用程式工作負載](../rtvs/data-science-workload.md)。

    > [!Note]
    > 只有 Visual Studio 2017 15.2 版和更新版本才提供 Python 和資料科學工作負載。
