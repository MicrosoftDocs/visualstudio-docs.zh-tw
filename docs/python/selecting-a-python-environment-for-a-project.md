...
Visual Studio 中的所有新 Python 專案最初配置為使用預設全域環境，該環境顯示在**解決方案資源管理器**中的**Python 環境**節點下：

![顯示在 [方案總管] 中的全域預設 Python 環境](media/environments/environments-project.png)

::: moniker range="vs-2017"
要更改專案的環境，請按右鍵 Python**環境**節點並選擇 **"添加/刪除 Python 環境**"。 在含有全域、虛擬和 Conda 環境的顯示清單中，選取您想要出現在 [Python 環境]**** 節點底下的所有項目：
...
| Command | 描述 |
| --- | --- |
| **添加虛擬環境** | 開始在專案中新建虛擬環境的程序。 請參閱[建立虛擬環境](#create-a-virtual-environment)。 |
| **添加現有虛擬環境** | 提示您選取包含虛擬環境的資料夾，並將它新增至 [Python 環境]**** 底下的清單，但不會加以啟用。 [啟用現有的虛擬環境](#activate-an-existing-virtual-environment)。 |
| **創建康達環境** | 切換至 [Python 環境]** 視窗** **，您可在其中輸入環境名稱並指定其基底解譯器。 請參閱 [Conda 環境](managing-python-environments-in-visual-studio.md#conda-environments)。 |
::: moniker-end
...
使用虛擬環境的其中一個優點是，即使經過一段時間的專案開發，虛擬環境仍會反映專案的確切相依性。 （另一方面，共用的全域環境包含任意數量的庫，無論您是否在專案中使用它們。然後，可以輕鬆地從虛擬環境創建*需求.txt*檔，然後用於在另一個開發或生產電腦上重新安裝這些依賴項。 如需詳細資訊，請參閱[使用 requirements.txt 管理必要套件](managing-required-packages-with-requirements-txt.md)。
...
1. 按右鍵**解決方案資源管理器**中的**Python 環境**並選擇 **"添加虛擬環境**"，該對話方塊將彈出以下對話方塊：
...
>
> 無論哪種方式，結果都與您使用"**添加現有虛擬環境"** 命令相同。

### <a name="activate-an-existing-virtual-environment"></a>啟用現有的虛擬環境

如果您已在其他位置建立虛擬環境，則可以依照下列方式為專案啟用虛擬環境：

1. 按右鍵**解決方案資源管理器**中的**Python 環境**，然後選擇"**添加現有虛擬環境**"。

1. 在顯示的 **"流覽"** 對話方塊中，導航到並選擇包含虛擬環境的資料夾，然後選擇 **"確定**"。 如果 Visual Studio 在該環境中偵測到 *requirements.txt* 檔案，便會詢問是否要安裝那些套件。

1. 幾分鐘後，虛擬環境將顯示在**解決方案資源管理器**中的 Python**環境**節點下。 根據預設不會啟用虛擬環境，因此請以滑鼠右鍵按一下虛擬環境並選取 [啟用環境]****。
::: moniker-end
...
> [!Tip]
> pip 無法安裝包的常見情況是，當包包含*\*.pyd*檔中本機組件的原始程式碼時。 如果未安裝所需的 Visual Studio 版本，pip 就無法編譯這些元件。 此情況中顯示的錯誤訊息是「錯誤: 找不到 vcvarsall.bat」****。 `easy_install`通常能夠下載預編譯的二進位檔案，並且可以從 下載適用于舊版本的 Python 的編譯器[https://www.microsoft.com/download/details.aspx?id=44266](https://www.microsoft.com/download/details.aspx?id=44266)。 如需詳細資訊，請參閱 Python 工具小組部落格中的[如何處理「找不到 vcvarsallbat」的困擾 (英文)](https://devblogs.microsoft.com/python/unable-to-find-vcvarsall-bat/)。
