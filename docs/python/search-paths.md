...
不過，即使針對整個系統設定了搜尋路徑環境變數，Visual Studio 也會將它忽略。 事實上，它被忽略，*正是因為*它被設置為整個系統，從而引發了某些無法自動回答的問題：引用的模組是針對 Python 2.7 還是 Python 3.6+？ 它們是否將覆寫標準程式庫程式庫模組？ 開發人員是否知道此行為，或它是否是惡意的劫持嘗試？
