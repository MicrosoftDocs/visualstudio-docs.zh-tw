...
為提供完整的偵錯體驗，Visual Studio 中的[混合模式 Python 偵錯工具](debugging-mixed-mode.md)需要使用 Python 解譯器的偵錯符號，以便剖析許多內部資料結構。 以 python27.dll 為例，對應的符號檔為 python27.pdb；若是 python36.dll，符號檔則為 python36.pdb。 每個版本的解譯器也提供各種不同模組的符號檔。
