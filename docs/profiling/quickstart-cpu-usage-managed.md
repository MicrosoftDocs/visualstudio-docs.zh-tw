...
ms.locfileid: "77558193"
---
# <a name="quickstart-analyze-cpu-usage-data-in-visual-studio-c-visual-basic"></a>快速入門：在 Visual Studio 中分析 CPU 使用量C#資料（，Visual Basic）
...
1. 開啟 Visual Studio 並建立專案。
   ...
   在 [建立新專案] 視窗的搜尋方塊中輸入或鍵入 ASP.NET。 接下來， **C#** 從 [語言] 清單中選擇或**Visual Basic** ，然後從 [平臺] 清單中選擇 [ **Windows** ]。
   ...
   在 [**設定您的新專案**] 視窗中，于 [**專案名稱**] 方塊中鍵入或輸入*MyProfilerApp* 。 接著，選擇 [建立]。
...
2. 接下來，在 `Main` 函式結尾的右大括弧上，設定第二個中斷點：
     ...
     ![設定程式碼剖析的中斷點](../profiling/media/quickstart-cpu-usage-breakpoints.png "設定中斷點進行分析")
...
1. 在函式清單中，檢查執行最多工作的函式。
     ...
     ![[診斷工具 CPU 使用量] 索引標籤](../profiling/media/quickstart-cpu-usage-cpu.png "DiagToolsCPUUsageTab")
...
2. 在函式清單中，按兩下 `ServerClass::GetNumber` 函式。
    ...
    ![診斷工具呼叫端被呼叫端視圖](../profiling/media/quickstart-cpu-usage-caller-callee.png "DiagToolsCallerCallee")
