...
ms.locfileid: "79550080"
---
# <a name="quickstart-analyze-cpu-usage-data-in-visual-studio-c-visual-basic"></a>快速入門：分析視覺化工作室中的 CPU 使用率資料（C#，視覺化基礎）
...
1. 打開視覺化工作室並創建專案。
   ...
   在左側窗格中的 **"新專案**"對話方塊中，展開**C#** 或**視覺化基本**，然後選擇 **.NET Core**。 在中間窗格中，選擇 [主控台應用程式 (.NET Core)]****。 然後命名專案*MyProfilerApp*。
   ...
   在 [建立新專案]**** 視窗的搜尋方塊中輸入或鍵入 ASP.NET**。 接下來，從"語言"清單中選擇**C#** 或**視覺化基本，** 然後從"平臺"清單中選擇**Windows。**
   ...
   在 **"配置新專案**"視窗中，在 **"專案名稱**"框中鍵入或輸入*MyProfilerApp。* 然後，選擇 **"創建**"。

   ::: moniker-end

   Visual Studio 會隨即開啟您的新專案。

2. 打開*Program.cs，* 並將所有代碼替換為以下代碼：
    ...
    > [!NOTE]
    > 在`Sub Main`視覺化基本中，請確保啟始物件設置為 （**屬性** > **應用程式** > **啟始物件**）。
