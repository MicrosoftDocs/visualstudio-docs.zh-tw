...
-  **Bytes**   預設不會顯示 [位元組] 和 [ **平均大小] (位元組]) **的資料   行。 若要顯示它們，請以滑鼠右鍵按一下 [ **類型**] 或 [配置] 資料    **Allocations**   行，然後選取 [**位元組**]   和 [ **平均大小] ([位元組]) **   選項，將它們加入至圖表。 

   這兩個數據行與 ** (配置) ** 和 ** (自助配置) **的總計相似，不同之處在于它們會顯示所佔用的記憶體數量，而不是佔用記憶體的物件數目。 這些資料行只會出現在 **配置** 視圖中。

- [ **模組名稱**] 資料行會   顯示模組，其中包含正在呼叫的函式或進程。

這些資料行都可以排序。 針對 [ **類型** ] 和 [ **模組名稱** ] 資料行，您可以依字母順序以遞增或遞減順序排序專案。 針對 **配置**、**位元組**   和 **平均大小 (位元組) **，您可以藉由增加或減少數值來排序專案。
...
-  ** (配置總計) **和 **總大小 (位元組數) **資料   行顯示已配置的物件數目，以及函式和其所呼叫之所有其他函式所使用的記憶體數量。
- [ **自我 (** 配置]) 和 [ **自我大小 (位元組]) ** 資料行顯示已配置的物件數目，以及單一選取的函式或配置類型所使用的記憶體數量。
- [ **平均大小 (位元組) ** ] 資料行顯示的資訊與 [配置] **視圖中的相同** 。
- [ **模組名稱**] 資料行會   顯示模組，其中包含正在呼叫的函式或進程。
