...
1. 開啟 Visual Studio 並建立專案。
   ...
   在 [ **建立新專案** ] 視窗的 [搜尋] 方塊中，輸入或輸入 *asp.net* 。 接下來，從語言清單中選擇 **C#**，然後從平台清單中選擇 **Windows**。

   套用語言和平臺篩選器之後，請選擇 **( .Net Core) 範本的 ASP.NET Web 應用程式** ，然後選擇 **[下一步]**。
   ...
   在 [**設定您的新專案**] 視窗中，于 [**專案名稱**] 方塊中輸入或輸入 *MyProfilingApp_MVC* 。 然後，選擇 [ **建立**]。
