...
1. 開啟 Visual Studio 並建立專案。
   ...
   > [!NOTE]
   > 如果您沒有看到 **ASP.NET Web 應用程式 ( .Net Core)** 專案範本，請選擇 [**新增專案**] 對話方塊左窗格中的 [**開啟 Visual Studio 安裝程式**] 連結。 Visual Studio 安裝程式即會啟動。 選擇 [ASP.NET 與網頁程式開發] 工作負載，然後選擇 [修改]。
   ...
   在 [搜尋] 方塊中輸入 **web 應用程式** ，選擇 [ **c #** ] 作為 [語言]，選擇 [ **ASP.NET Core Web 應用程式] (模型-視圖控制器])**，然後選擇 **[下一步]**。 在下一個畫面中，將專案命名為 *MyProfilingApp_MVC*，然後選擇 **[下一步]**。
   ...
   Visual Studio 會建立並開啟您的新專案。

1. 在 [方案 Explorer] 中，以滑鼠右鍵按一下 [模型] 資料夾，然後選擇 [**加入**  >  **類別**]。
...
1. 在 [方案 Explorer] 中，開啟 *Controller/homecontrollers.cs*，並取代下列程式碼：
...
1. 除非您關閉 [診斷工具] 視窗，否則該視窗已出現。 若要再次顯示視窗，請按一下 [**調試** 程式  >  **視窗**  >  **顯示診斷工具**]。
