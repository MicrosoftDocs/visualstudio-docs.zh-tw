...
[CPU 使用量] 工具可以在開啟的 Visual Studio 專案上及已安裝的 Microsoft Store 應用程式上執行，或是附加至執行中的應用程式或處理程序。 您可以在本機或遠端電腦上，或在模擬器 (Simulator 或 Emulator) 中執行工具。 如需詳細資訊，請參閱[使用或不使用偵錯工具來執行分析工具](../profiling/running-profiling-tools-with-or-without-the-debugger.md)。 
