---
title: VSPerfCmd：將探測器附加到本機服務，以獲得應用統計資訊
ms.date: 11/04/2016
ms.topic: conceptual
...
3. 啟動分析工具。 輸入：
    ...
    **VSPerfCmd /開始：樣品**[/輸出](../profiling/output.md)`Options` **：** `OutputFile` [ ]  
   ...
   | [/交叉會話](../profiling/crosssession.md) | 在其他工作階段啟用處理序程式碼剖析。 如果應用程式在不同的工作階段中執行，則需要這個選項。 工作階段識別碼會列在 [Windows 工作管理員] 的 [處理程序] 索引標籤上的 [工作階段識別碼] 欄。 **/crosssession** 可縮寫成 **/CS**。 |
...
- 以下**VSPerfCmd**選項對開始並停止資料收集。 請在個別的命令列上指定各個選項。 您可以多次開始和停止資料收集。
    ...
    |**/附加：** `PID` [ `ProcName`&#124;] [/分離](../profiling/detach.md)[： ]`PID`&#124;`ProcName`*|**/attach** 會開始為處理序 ID 或處理序名稱指定的處理序收集資料。 **/detach** 會停止指定的處理序或所有處理序 (如果未指定處理序) 的資料收集。|
