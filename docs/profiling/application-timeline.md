...
> [!NOTE]
> 除了 [時間軸] **** 資料之外，您還可以收集並分析 CPU 使用量資料和能源消耗資料。 請參閱[運行具有或不帶調試器的分析工具](../profiling/running-profiling-tools-with-or-without-the-debugger.md)。

## <a name="collect-application-timeline-data"></a>收集應用程式時間軸資料

您可以在本機電腦、連接的裝置、Visual Studio 模擬器 (Simulator 或 Emulator) 或遠端裝置上分析應用程式的回應性。 請參閱[運行具有或不帶調試器的分析工具](../profiling/running-profiling-tools-with-or-without-the-debugger.md)。
...
2. 按一下**調試/性能探測器**。 您應該會在 .diagsession 視窗中看到一份程式碼剖析工具清單。

3. 選取 [應用程式時間軸] **** ，然後在視窗底部按一下 [開始] **** 。

   > [!NOTE]
   > 您可能會看到使用者帳戶控制視窗請求您運行*VsEtwCollector.exe*的許可權。 按一下 [是]****。
...
### <a name="report-scenarios-categories-and-events"></a><a name="BKMK_Report_scenarios_categories_and_events"></a>報告方案、類別和事件
...
#### <a name="customizing-timeline-details"></a><a name="BKMK_Customizing_Timeline_details_"></a>自訂時間表詳細資訊
...
|-|-|
|**按**|依開始時間或事件長度排序。|
|![按幀對事件進行分組](../profiling/media/timeline_groupbyframes.png "TIMELINE_GroupByFrames")|加入或移除依框架分組事件的最上層 [框架] **** 分類。|
|![篩選時間表詳細資訊清單](../profiling/media/timeline_filter.png "TIMELINE_Filter")|依選取的分類和事件長度篩選清單。|
|![自訂時間表詳細資訊](../profiling/media/timeline_viewsettings.png "TIMELINE_ViewSettings")|可讓您指定事件的註釋。|

## <a name="see-also"></a>另請參閱

- [WPF 團隊博客：適用于 WPF 應用程式的新 UI 性能分析工具](https://blogs.msdn.microsoft.com/wpf/2015/01/16/new-ui-performance-analysis-tool-for-wpf-applications/)
