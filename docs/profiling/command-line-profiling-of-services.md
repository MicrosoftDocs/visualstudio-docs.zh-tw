...
| **收集併發資料：** 使用併發方法收集資源爭用資料和執行緒活動資料，這些資料顯示 CPU 利用率、執行緒爭用、執行緒遷移、同步延遲、重疊 IO 區域和其他系統事件。 | -   [收集併發資料](../profiling/collecting-concurrency-data-for-a-service-by-using-the-profiler-command-line.md) |
| **加入階層互動資料︰** 您可以新增服務對 Microsoft [!INCLUDE[ssNoVersion](../data-tools/includes/ssnoversion_md.md)] 資料庫之同步 ADO.NET 呼叫的相關效能資料。 | -   [收集層交互資料](../profiling/adding-tier-interaction-data-from-the-command-line.md) |
...
|**分析 ASP.NET 應用程式**|-   [設定檔ASP.NET Web 應用程式](../profiling/command-line-profiling-of-aspnet-web-applications.md)|
