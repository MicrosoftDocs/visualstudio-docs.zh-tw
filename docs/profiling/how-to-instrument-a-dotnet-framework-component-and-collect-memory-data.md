---
title: Profiler 命令列-檢測用戶端 .NET 元件，取得記憶體資料
ms.date: 11/04/2016
ms.topic: how-to
...
4. 啟動分析工具。 輸入：
   ...
   - /Start 需要[/output](../profiling/output.md)**：** `OutputFile` 選項。 **/start** `OutputFile`指定程式碼剖析資料的名稱和位置（。*vsp*）檔案。
   ...
   | [事件](../profiling/events-vsperfcmd.md) **：**`Config` | 指定程式碼剖析期間要收集的 Windows 事件追蹤 (ETW) 事件。 ETW 事件會收集在不同的（。*etl*）檔案。 |
