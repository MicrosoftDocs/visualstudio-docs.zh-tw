...
ms.locfileid: "80233054"
---
# <a name="analyze-memory-usage"></a>分析記憶體使用量

要查找記憶體洩漏和記憶體使用效率低下，可以使用諸如調試器集成記憶體使用方式診斷工具或性能探測器中的工具，如 .NET 物件分配工具和事後記憶體使用工具。

記憶體使用量工具可讓您擷取 Managed 和原生記憶體堆積的一個或多個 *「快照」* (Snapshot)。 您可以收集 .NET、ASP.NET、原生或混合模式 (.NET 和原生) 應用程式的快照。 **記憶體使用方式**工具可以在打開的 Visual Studio 專案上、已安裝的 Microsoft 應用商店應用上運行，也可以附加到正在運行的應用或進程。 您可以在本機或遠端電腦上，或在模擬器 (Simulator 或 Emulator) 中執行工具。 您可以運行**記憶體使用**工具，無論是否進行調試。 有關詳細資訊，請參閱[運行具有或不帶調試器的分析工具](../profiling/running-profiling-tools-with-or-without-the-debugger.md)。 在調試器中，您可以打開和關閉記憶體分析，並查看每個物件的記憶體使用方式細目。 您可以在暫停執行時查看記憶體使用結果，例如在中斷點。

**.NET 物件分配**工具可説明您識別 .NET 代碼中的分配模式和異常。 此工具僅作為事後工具運行。
