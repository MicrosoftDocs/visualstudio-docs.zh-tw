...
## <a name="attach-the-profiler"></a>附加分析工具
 要使用探測器將探測器附加到目標應用程式，請使用**VSPerfCmd/start**和 **/attach**選項來初始化探測器並附加到目標應用程式。 您可以在單一命令列上指定 **/start** 和 **/attach** 及其個別選項。 您也可以加入 **/globaloff** 選項以在目標應用程式啟動時暫停資料收集。 然後使用 **/globalon** 開始收集資料。
...
2. 啟動分析工具。 輸入：
    ...
    **VSPerfCmd /開始：樣品/輸出：** `OutputFile` |`Options`
   ...
   | 選項 | 描述 |
   | - | - |
   | [/使用者](../profiling/user-vsperfcmd.md) **：**[ ]`Domain` **\\**`UserName` | 指定擁有程式碼剖析處理序之帳戶的網域和使用者名稱。 只有在以登入的使用者之外的使用者身分執行處理序時，才需要這個選項。 處理序擁有者會列在 [Windows 工作管理員] 的 [處理程序] 索引標籤上的 [使用者名稱] 欄。 |
   | [/交叉會話](../profiling/crosssession.md) | 在其他工作階段啟用處理序程式碼剖析。 如果 ASP.NET 應用程式在不同的工作階段中執行，則需要這個選項。 工作階段識別碼會列在 [Windows 工作管理員] 的 [處理程序] 索引標籤上的 [工作階段識別碼] 欄。 **/crosssession** 可縮寫成 **/CS**。 |
...
3. 將分析工具附加至目標應用程式。 輸入：
    ...
    **VSPerfCmd**[/附加](../profiling/attach.md)`PID` **：**[`Sample Event`&#124;`ProcName`] [ ]  
...
## <a name="control-data-collection"></a>控制資料收集
 當目標應用程式運行時，可以使用*VSPerfCmd.exe*選項啟動和停止將資料寫入探測器資料檔案。 控制資料收集可讓您收集特定程式執行 (例如啟動或關閉應用程式) 的資料。
...
- 以下**VSPerfCmd**選項對開始並停止資料收集。 請在個別的命令列上指定各個選項。 您可以多次開始和停止資料收集。
    ...
    |[/進程](../profiling/processon-and-processoff.md)**：** `PID` [/進程關閉](../profiling/processon-and-processoff.md) **：**`PID`|開始 (**/processon**) 或停止 (**/processoff**) 對 `PID` 指定的處理序收集資料。|
    |[/附加](../profiling/attach.md) **：** `PID` [/分離](../profiling/detach.md)|**/attach** 會開始對 `PID` 指定的處理序收集資料。 **/detach** 停止所有處理序的資料收集。|
