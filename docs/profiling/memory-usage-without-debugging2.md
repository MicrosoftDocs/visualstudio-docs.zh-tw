...
-  方塊中輸入名稱。 篩選條件不會區分大小寫，且它會辨識類型名稱任何部分中的指定字串。

-  下拉式清單中的 [摺疊小物件]  ，隱藏 [大小 (位元組)]  小於總記憶體 0.5% 的類型。

-  下拉式清單中的 [Just My Code]  ，隱藏外部程式碼所產生的大部分執行個體。 外部類型屬於作業系統或架構元件，或者由編輯器產生。
