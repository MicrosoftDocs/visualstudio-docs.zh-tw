...
5. 將分析工具附加至目標應用程式。 類型：
   ...
   - 根據預設，每經過 10,000,000 個未暫止處理器時脈週期，會取樣一次效能資料。 在 1GH 處理器上，這大約是每 10 秒一次。 您可以指定以下其中一個選項來變更時脈週期間隔，或指定其他取樣事件。[/targetclr](../profiling/targetclr.md) **:** `Version` 如果應用程式中載入多個版本的執行階段，可使用此選項指定進行分析的 CLR 版本。 選擇項。
