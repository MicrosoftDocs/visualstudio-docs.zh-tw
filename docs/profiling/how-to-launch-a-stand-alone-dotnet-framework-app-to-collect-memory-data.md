---
title: 探測器命令列：打開用戶端 .NET 框架應用，獲取記憶體資料
ms.date: 11/04/2016
ms.topic: conceptual
...
3. 啟動目標應用程式。 輸入：
    ...
    **VSPerfCmd**[/啟動](../profiling/launch.md)**：** `appName` **/gc：**[`Options`**分配**&#124;**存留期**]  

   - [需要 /gc](../profiling/gc-vsperfcmd.md)**：**`Keyword`選項來收集 .NET 框架記憶體資料。 關鍵字參數指定要收集記憶體配置資料，或收集記憶體配置和物件存留期資料。
...
- 下列成對的選項會開始和停止資料收集。 請在個別的命令列上指定各個選項。 您可以多次開始和停止資料收集。
    ...
    |[/附加](../profiling/attach.md) **：** `PID` [/分離](../profiling/detach.md)|**/attach** 會開始為 `PID`(處理序 ID) 指定的處理序收集資料。 **/detach** 停止所有處理序的資料收集。|
