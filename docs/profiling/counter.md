 ...
 **Start:**`Trace` 將分析工具初始化以使用檢測方法。

 **Launch:**`AppName` 啟動指定的應用程式和分析工具。 分析工具必須先初始化才能使用取樣方法。

 **Attach：**`PID` 啟動分析工具，並將它附加至處理序識別碼所指定的處理序。 分析工具必須先初始化才能使用取樣方法。
