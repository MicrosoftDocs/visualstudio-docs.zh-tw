...
使用調試器集成的**記憶體使用方式**診斷工具進行調試時，查找記憶體洩漏和記憶體效率低下。 記憶體使用量工具可讓您擷取受控 原生之記憶體堆積的一或多個「快照」**，以利了解物件類型的記憶體使用量影響。 您可以收集 .NET、原生或混合模式 (.NET 和原生) 應用程式的快照。

下圖顯示 [診斷工具]**** 視窗 (於 Visual Studio 2015 Update 1 及更新版本中提供)：

![診斷工具&#45;更新1](../profiling/media/diagnostictools-update1.png "DiagnosticTools-Update1")
...
您也可以在偵錯工具外部使用記憶體工具。 請參閱[記憶體使用方式，而不進行調試](../profiling/memory-usage-without-debugging2.md)。 您可以在 Windows 7 及更新版本使用未附加偵錯工具的分析工具。 Windows 8 及更新版本必須執行附有偵錯工具的分析工具 ([診斷工具]**** 視窗)。

> [!NOTE]
> **自訂分配器支援**本機記憶體探測器的工作原理是收集在運行時發出的分配[ETW](/windows-hardware/drivers/devtest/event-tracing-for-windows--etw-)事件資料。  在來源層級已註釋 CRT 和 Windows SDK 中的配置器，以便擷取其配置資料。 如果您正在撰寫自己的配置器，則針對任何將指標傳回最新配置之堆積記憶體的函式，都可以使用 [__declspec](/cpp/cpp/declspec)(allocator) 來裝飾，如本範例中針對 myMalloc 所示：
>
...
4. 以工具列上的 [選取工具]**** 設定選擇 [記憶體使用量]****。
     ...
     ![顯示診斷工具](../profiling/media/diag-tools-select-tool-2.png "直徑工具選擇工具")
...
5. 按一下 [偵錯/開始偵錯]**** (或工具列上的 [開始]**** 或 **F5**)。
     ...
     > [!NOTE]
     > 由於收集記憶體資料可能會影響原生或混合模式應用程式的偵錯效能，因此預設會停用記憶體快照。 若要在原生或混合模式應用程式中啟用快照，請啟動偵錯工作階段 (快速鍵：**F5**)。 當**出現"診斷工具"** 視窗時，選擇"**記憶體使用方式**"選項卡，然後選擇 **"堆分析**"。
     >
     >  ![啟用快照集](../profiling/media/dbgdiag_mem_mixedtoolbar_enablesnapshot.png "DBGDIAG_MEM_MixedToolbar_EnableSnapshot")
     >
     >  停止（快速鍵：**移動**+**F5**）並重新啟動調試。

6. 若要在偵錯工作階段開始時擷取快照，請選擇 [記憶體使用量]**** 摘要工具列上的 [擷取快照]****。 (在此設定中斷點也可能會有幫助)。

    ![拍攝快照](../profiling/media/dbgdiag_mem_mixedtoolbar_takesnapshot.png "DBGDIAG_MEM_MixedToolbar_TakeSnapshot")
...
- 要查看當前快照和上一個快照之間的差異的詳細資訊，請選擇箭頭左側的更改連結（![記憶體使用量增加](../profiling/media/prof-tour-mem-usage-up-arrow.png "記憶體使用量增加")）。 紅色箭號表示記憶體使用量增加，綠色箭號表示減少。
...
### <a name="managed-types-reports"></a>Managed 類型報表
 ...
 ![調試器託管類型報表&#45;路徑到根](../profiling/media/dbgdiag_mem_managedtypesreport_pathstoroot.png "DBGDIAG_MEM_ManagedTypesReport_PathsToRoot")
 ...
 ![託管引用物件報表檢視](../profiling/media/dbgdiag_mem_managedtypesreport_referencedtypes.png "DBGDIAG_MEM_ManagedTypesReport_ReferencedTypes")

 要在上窗格中顯示所選類型的實例，請選擇!["實例"圖示](../profiling/media/dbgdiag_mem_instanceicon.png "DBGDIAG_MEM_InstanceIcon")。

 ![Instances 檢視表](../profiling/media/dbgdiag_mem_managedtypesreport_instances.png "DBGDIAG_MEM_ManagedTypesReport_Instances")

 [執行個體] **** 檢視顯示在上方窗格的快照中選取之物件的執行個體。 [根的路徑]**** 和 [參考的物件]**** 窗格顯示參考所選執行個體的物件，以及所選執行個體參考的類型。 當調試器在拍攝快照的點停止時，可以懸停在**Value**儲存格上，以在工具提示中顯示物件的值。
...
- 選擇所選類型的實例圖示 （![物件類型列中的實例圖示](../profiling/media/dbg_mma_instancesicon.png "DBG_MMA_InstancesIcon")），以顯示有關快照中所選類型物件的資訊。
...
- 在 [診斷工具] **** 視窗中，選擇 [記憶體使用量] **** 索引標籤摘要表資料格中的變更連結。
   ...
   ![選擇差異&#41;報告&#40;更改](../profiling/media/dbgdiag_mem_choosediffreport.png "DBGDIAG_MEM_ChooseDiffReport")
...
![本機類型差異視圖](../profiling/media/dbgdiag_mem_native_typesviewdiff.png "DBGDIAG_MEM_Native_TypesViewDiff")
...
> [分析 CPU 使用率](../profiling/beginners-guide-to-performance-profiling.md)
