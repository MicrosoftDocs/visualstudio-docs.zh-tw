---
title: 探測器命令列：儀器動態ASP.NET應用，獲取計時資料
ms.date: 11/04/2016
ms.topic: conceptual
author: mikejo5000
ms.author: mikejo
...
要從[!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)]Web 應用程式收集效能資料，請修改目標應用程式的*Web.config*檔，使[VSInstr.exe](../profiling/vsinstr.md)工具能夠檢測動態編譯的應用程式檔。 然後，使用[VSPerfCLREnv.cmd](../profiling/vsperfclrenv.md)工具在 Web 服務器上設置適當的環境變數以啟用分析，然後重新開機電腦。

請啟動分析工具，然後執行目標應用程式。 程式碼剖析工具附加至應用程式時，您可以暫停和繼續收集資料。 完成分析時，請關閉應用程式、關閉 Internet Information Services (IIS) 背景工作處理序，然後關閉分析工具。 完成分析工作後，將*Web.config 檔和*Web 服務器還原到其原始狀態。
...
2. 啟動分析工具。 輸入：
   ...
   - **/輸出：**`OutputFile`選項在 **/start**時是必需的。 `OutputFile`指定分析資料 （的名稱和位置 （。*vsp*） 檔。
     ...
     | [/交叉會話](../profiling/crosssession.md) | 在其他登入工作階段啟用處理序程式碼剖析。 如果 ASP.NET 應用程式在不同的工作階段中執行，則需要這個選項。 會話識別碼列在 Windows 工作管理員的 **"進程"** 選項卡上的 **"會話 ID"** 列中。 **/crosssession** 可縮寫成 **/CS**。 |
...
完成所有分析後，請替換*Web.config*檔，清除分析環境變數，然後重新開機電腦以將應用程式和伺服器還原到分析之前處於狀態。
