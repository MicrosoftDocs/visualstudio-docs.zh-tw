---
title: CA2009：不要在 ImmutableCollection 值上呼叫 Tolmmutablecollection
ms.date: 04/29/2020
ms.topic: reference
f1_keywords:
- CA2009
...
ms.locfileid: "82587702"
---
# <a name="ca2009-do-not-call-toimmutablecollection-on-an-immutablecollection-value"></a>CA2009：不要在 ImmutableCollection 值上呼叫 Tolmmutablecollection
...
`ToImmutable`不必要地在<xref:System.Collections.Immutable>命名空間的不可變集合上呼叫方法。
...
這些類型定義了從現有<xref:System.Collections.Generic.IEnumerable%601>的集合建立新的不可變集合的擴充方法。
...
若要修正違規，請在`ToImmutable`不可變的集合上移除多餘的呼叫。 例如，下列兩個程式碼片段會顯示規則的違規，以及如何加以修正：
