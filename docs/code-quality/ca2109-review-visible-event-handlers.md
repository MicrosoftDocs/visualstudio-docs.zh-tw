...
不要公開事件處理方法，除非絕對必要。 事件處理常式，會叫用的公開的方法的委派類型，可以加入至任何事件，只要處理常式和事件簽章相符。 事件可能會引發的任何程式碼，並經常會引發以回應使用者動作，例如按一下按鈕的高度信任的系統程式碼。 加入事件處理方法的安全性檢查不會防止程式碼註冊事件處理常式叫用方法。
