...
請考慮明確實作為公用介面方法的基底類型。 衍生自基底類型的類型只能透過轉換成介面之目前實例（在中C#為 @no__t 0）的參考，來存取繼承的介面方法。 如果衍生的型別實作（明確）繼承的介面方法，就無法再存取基底實作為。 透過目前實例參考的呼叫將會叫用衍生的實值;這會導致遞迴和最終堆疊溢位。

當提供外部可見的 `Close()` 或 @no__t 2 方法時，此規則不會報告 <xref:System.IDisposable.Dispose%2A?displayProperty=fullName> 的明確實作為違規。
