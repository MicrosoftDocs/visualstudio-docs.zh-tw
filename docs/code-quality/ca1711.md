...
您可以只針對此規則、所有規則或此類別中的所有規則（命名）來設定這些選項。 如需詳細資訊，請參閱[設定 FxCop 分析器](configure-fxcop-analyzers.md)。

### <a name="allow-suffixes"></a>允許尾碼

您可以設定允許的尾碼清單，其中每個尾碼都以管線字元（"|"）分隔。 例如，若要指定規則不應針對旗標和旗標尾碼執行，請將下列機碼值組新增至專案中的 editorconfig 檔案：
