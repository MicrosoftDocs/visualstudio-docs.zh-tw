...
|旗標 | Flags|<xref:System.Enum?displayProperty=fullName>|
...
如果您是從[FxCop 分析器](install-fxcop-analyzers.md)執行此規則（而不是使用舊版分析），您可以設定允許的尾碼清單，其中每個尾碼都以管線字元（"|"）分隔。 例如，若要指定規則不應針對旗標和旗標尾碼執行，請將下列機碼值組新增至專案中的 editorconfig 檔案：
...
- [CA1710:識別項應該使用正確的後置字元](../code-quality/ca1710.md)
