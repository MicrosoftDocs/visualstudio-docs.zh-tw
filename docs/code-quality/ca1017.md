...
<xref:System.Runtime.InteropServices.ComVisibleAttribute> 屬性會判斷 COM 用戶端如何存取 managed 程式碼。 良好的設計會要求組件明確表示 COM 的可視性。 您可以針對整個元件設定 COM 可見度，然後針對個別類型和類型成員加以覆寫。 如果屬性不存在，則 COM 用戶端可以看到元件的內容。
