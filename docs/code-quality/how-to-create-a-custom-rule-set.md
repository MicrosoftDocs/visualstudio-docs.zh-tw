...
3. 選取 [ **\<Choose multiple rule sets>** 從**執行此規則集**]。

::: moniker-end

::: moniker range=">=vs-2019"

3. 選取 [ **\<Choose multiple rule sets>** 從作用中**規則**]。
...
- 如果相同的規則在具有*相同*嚴重性的規則集內列出兩次以上，您可能會在**錯誤清單**中看到下列警告：
   ...
   **CA0063：無法載入規則集檔案 ' \[ 您的]、規則集或它的其中一個相依規則集檔案。檔案不符合規則集架構。**
...
若要變更在編輯器中開啟之規則集的顯示名稱，請選取功能表列**Properties**上的 [**視圖**  >  **屬性視窗]** ，以開啟 [屬性] 視窗。 在 [**名稱**] 方塊中輸入顯示名稱。 您也可以輸入規則集的描述。
