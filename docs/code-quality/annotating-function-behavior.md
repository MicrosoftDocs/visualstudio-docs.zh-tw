...
|註釋|描述|
|----------------|-----------------|
|`_Called_from_function_class_(name)`|這個述詞不適合單獨使用，而是要搭配 `_When_` 註釋使用。 如需詳細資訊, 請參閱[指定批註的套用時機和位置](../code-quality/specifying-when-and-where-an-annotation-applies.md)。<br /><br /> 參數是任一字元串, 也會出現`_Function_class_`在某些函數之宣告的注釋中。 `name`  `_Called_from_function_class_`如果目前正在分析的函式是使用`_Function_class_`具有相同值的`name`來標注, 則傳回非零, 否則會傳回零。|
...
|`_Must_inspect_result_`|標注任何輸出值, 包括傳回值、參數和 globals。  如果在標註的物件中未接著檢查值，分析器就會報告錯誤。 「檢查」包括值是否用於條件運算式、指派給輸出參數或全域，或是做為參數傳遞。  若為傳回值`_Must_inspect_result_` , `_Check_return_`則表示。|
|`_Use_decl_annotations_`|可以用於函式定義 (也稱為函式主體) 來取代標頭中的注釋清單。  使用`_Use_decl_annotations_`時, 會使用出現在相同函式之範圍標頭中的注釋, 如同它們也出現在`_Use_decl_annotations_`具有注釋的定義中。|
