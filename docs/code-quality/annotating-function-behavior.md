...
|`_Must_inspect_result_`|任何輸出值，包括傳回值、 參數和全域變數加上附註。  如果在標註的物件中未接著檢查值，分析器就會報告錯誤。 「檢查」包括值是否用於條件運算式、指派給輸出參數或全域，或是做為參數傳遞。  傳回值，如`_Must_inspect_result_`意味著`_Check_return_`。|
|`_Use_decl_annotations_`|可在函式定義 （也稱為函式主體） 取代的標頭中的註解清單。  當`_Use_decl_annotations_`是出現在相同的函式的範圍內標頭的註解使用如同它們也會出現在已定義`_Use_decl_annotations_`註解。|

## <a name="successfailure-annotations"></a>成功/失敗附註
