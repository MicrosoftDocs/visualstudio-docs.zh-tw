...
您可以將機碼值組新增至[editorconfig](https://editorconfig.org)檔案, 以設定分析器規則。 設定檔可以是[專案特有](#per-project-configuration)的, 也可以在兩個或多個專案之間[共用](#shared-configuration)。
...
您可以在兩個或多個專案之間共用分析器設定的 editorconfig 檔案, 但它需要一些額外的步驟。
