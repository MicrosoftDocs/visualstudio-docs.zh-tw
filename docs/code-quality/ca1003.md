...
在 .NET Framework 2.0 之前，若要將自訂資訊傳遞至事件處理常式，則必須宣告新的委派，以指定衍生自類別的類別 <xref:System.EventArgs?displayProperty=fullName> 。 在 .NET Framework 2.0 和更新版本中，泛型 <xref:System.EventHandler%601?displayProperty=fullName> 委派可讓衍生自的任何類別 <xref:System.EventArgs> 與事件處理常式一起使用。

## <a name="how-to-fix-violations"></a>如何修正違規

若要修正此規則的違規，請移除委派，並使用委派取代其使用方式 <xref:System.EventHandler%601?displayProperty=fullName> 。
...
下列範例顯示違反規則的委派。 在 Visual Basic 範例中，批註會說明如何修改範例以符合規則。 在 c # 範例中，會顯示已修改程式碼的範例。

[!code-vb[FxCop.Design.CustomEventHandler#1](../code-quality/codesnippet/VisualBasic/ca1003-use-generic-event-handler-instances_1.vb)]
[!code-csharp[FxCop.Design.CustomEventHandler#1](../code-quality/codesnippet/CSharp/ca1003-use-generic-event-handler-instances_1.cs)]

下列程式碼片段會移除上一個範例中符合規則的委派宣告。 它會 `ClassThatRaisesEvent` `ClassThatHandlesEvent` 使用委派來取代和方法中的使用 <xref:System.EventHandler%601?displayProperty=fullName> 。
