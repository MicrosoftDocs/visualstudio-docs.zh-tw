...
在 .NET Framework 2.0 之前，若要將自訂資訊傳遞至事件處理常式，則必須宣告新的委派，以指定衍生自 <xref:System.EventArgs?displayProperty=fullName> 類別的類別。 在 .NET Framework 2.0 和更新版本中，泛型 <xref:System.EventHandler%601?displayProperty=fullName> 委派可讓衍生自 <xref:System.EventArgs> 的任何類別與事件處理常式一起使用。
...
下列程式碼片段會移除上一個範例中符合規則的委派宣告。 它會使用 <xref:System.EventHandler%601?displayProperty=fullName> 委派取代 `ClassThatRaisesEvent` 和 `ClassThatHandlesEvent` 方法中的使用方式。
