...
- 針對 .NET Framework 2.0，請使用 `/keyfile` 或 `/keycontainer` 編譯器選項[/KEYFILE （指定金鑰或金鑰組來簽署元件）](/cpp/build/reference/keyfile-specify-key-or-key-pair-to-sign-an-assembly)或[/KEYCONTAINER （指定金鑰容器以簽署元件）](/cpp/build/reference/keycontainer-specify-a-key-container-to-sign-an-assembly)連結器選項（在 c + + 中））。
