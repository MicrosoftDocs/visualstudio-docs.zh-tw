...
- 請確定 <xref:System.Data.DataViewManager.DataViewSettingCollectionString%2A?displayProperty=nameWithType> 已從信任的來源指派屬性。
...
> [!NOTE]
> 雖然 <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> 是建立實例的建議方式，但與有一些 <xref:System.Xml.XmlReader> 行為上的差異 <xref:System.Xml.XmlTextReader> 。 <xref:System.Xml.XmlReader>從正規化 <xref:System.Xml.XmlReader.Create%2A> `\r\n` 到 `\n` XML 值的，同時 <xref:System.Xml.XmlTextReader> 保留 `\r\n` 序列。
