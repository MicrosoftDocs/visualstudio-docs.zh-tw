...
符合下列任一條件時，就會引發此規則：
- 參考的是 <xref:System.Security.Authentication.SslProtocols?displayProperty=nameWithType> 已被取代的值。
- 表示已被取代值的整數值已指派給 <xref:System.Security.Authentication.SslProtocols> 變數，做為傳回 <xref:System.Security.Authentication.SslProtocols> 值使用，或當做 <xref:System.Security.Authentication.SslProtocols> 引數使用。
