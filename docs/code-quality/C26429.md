...
使用判斷提示來強制假設指標值的有效性，是常見的作法。 判斷提示的問題在於，它們不會透過介面公開假設（例如，在傳回類型或參數中）。 判斷提示也難以維護，並與其他程式碼變更保持同步。 建議使用指導方針支援程式庫中的 gsl：： not_null 做為資源的標記，但絕不能有 null 值。 [規則 USE_NOTNull] 有助於識別省略 null 檢查的位置，因此可以更新為使用 gsl：： not_null。
