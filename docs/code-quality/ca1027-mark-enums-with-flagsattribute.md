...
在下列範例中，`DaysEnumNeedsFlags` 是一個列舉，符合使用 <xref:System.FlagsAttribute> 但沒有它的需求。 @No__t-0 列舉不具有2的乘冪值，但不正確地指定 <xref:System.FlagsAttribute>。 這會違反規則[CA2217：請勿使用 FlagsAttribute 標記](../code-quality/ca2217.md)列舉。
