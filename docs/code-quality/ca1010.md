...
型別會執行 <xref:System.Collections.IEnumerable?displayProperty=fullName> 介面，但不會實作為 <xref:System.Collections.Generic.IEnumerable%601?displayProperty=fullName> 介面，而包含的元件則是以 .NET 為目標。 此規則會忽略實 <xref:System.Collections.IDictionary?displayProperty=fullName> 的類型。
