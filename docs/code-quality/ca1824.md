...
<xref:System.Resources.NeutralResourcesLanguageAttribute> 屬性會通知資源管理員應用程式的預設文化特性。 如果預設文化特性的資源內嵌在應用程式的主要元件中，而且 <xref:System.Resources.ResourceManager> 必須取出屬於與預設文化特性相同文化特性的資源，則 <xref:System.Resources.ResourceManager> 會自動使用位於主要元件中的資源，而不是搜尋附屬元件。 這會略過一般的元件探查、改善您載入的第一個資源的查閱效能，並可減少您的工作集。

> [!TIP]
> 如需 <xref:System.Resources.ResourceManager> 用來探查資源檔的進程，請參閱[封裝和部署資源](/dotnet/framework/resources/packaging-and-deploying-resources-in-desktop-apps)。
