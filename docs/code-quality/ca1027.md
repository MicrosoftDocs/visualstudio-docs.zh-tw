...
在下列範例中， `DaysEnumNeedsFlags` 是符合使用之需求的列舉型 <xref:System.FlagsAttribute> 別，但沒有它。 `ColorEnumShouldNotHaveFlag`列舉的值不是2的乘冪，而是不正確地指定 <xref:System.FlagsAttribute> 。 這會違反規則[CA2217：請勿使用 FlagsAttribute 標記](../code-quality/ca2217.md)列舉。
