...
## <a name="cause"></a>原因
公用或受保護的類型會實 <xref:System.Collections.IEnumerator?displayProperty=fullName>，但不會提供 <xref:System.Collections.IEnumerator.Current%2A?displayProperty=fullName> 屬性的強型別版本。 衍生自下列類型的類型不受此規則所規範：
...
此規則需要 <xref:System.Collections.IEnumerator> 的執行同時提供 @no__t 1 屬性的強型別版本，讓使用者在使用介面所提供的功能時，不需要將傳回值轉換成強式類型。 此規則假設實 <xref:System.Collections.IEnumerator> 的類型包含強于 <xref:System.Object> 之類型實例的集合。
