...
當您將 `readonly` （ `ReadOnly` in）修飾詞套用 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 至包含陣列的欄位時，無法將欄位變更為參考不同的陣列。 但是，儲存在唯讀欄位的陣列元素則可以變更。 根據可公開存取之唯讀陣列的元素，做出決策或執行作業的程式碼，可能會包含易受攻擊的安全性弱點。

請注意，具有公用欄位也會違反設計規則[CA1051：不要宣告可見的實例欄位](../code-quality/ca1051.md)。
...
如果您選擇第二種方法，請勿將欄位取代為屬性;傳回陣列的屬性會對效能造成不良影響。 如需詳細資訊，請參閱[CA1819：屬性不應傳回陣列](../code-quality/ca1819.md)。
