...
當您將`readonly` （`ReadOnly` in [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]）修飾詞套用至包含陣列的欄位時，無法將欄位變更為參考不同的陣列。 但是，儲存在唯讀欄位的陣列元素則可以變更。 根據可公開存取之唯讀陣列的元素，做出決策或執行作業的程式碼，可能會包含易受攻擊的安全性弱點。
...
這個範例會示範違反此規則的危險。 第一個部分顯示具有類型`MyClassWithReadOnlyArrayField`的範例程式庫，其中包含兩個不安全的欄位（`grades`和`privateGrades`）。 此欄位`grades`是公用的，因此容易受到任何呼叫者的影響。 此欄位`privateGrades`是私`GetPrivateGrades`用的，但仍然很容易受到攻擊，因為方法會將它傳回給呼叫端。 此`securePrivateGrades`欄位是`GetSecurePrivateGrades`由方法以安全的方式公開。 它會宣告為私用，以遵循良好的設計實務。 第二個部分所顯示的程式碼會變更`grades`儲存`privateGrades`在和成員中的值。
...
- [CA2104不要宣告唯讀的可變動參考型別](../code-quality/ca2104-do-not-declare-read-only-mutable-reference-types.md)
