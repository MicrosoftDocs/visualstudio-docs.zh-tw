...
- 無法具現化，而且只會 [`static`](/dotnet/csharp/language-reference/keywords/static) [ `Shared` 在 Visual Basic](/dotnet/visual-basic/language-reference/modifiers/shared)) 方法中定義 (的類型。

如果您將套用 <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute?displayProperty=fullName> 到要分析的元件，此規則將不會標示 [`internal`](/dotnet/csharp/language-reference/keywords/internal) [ `Friend` Visual Basic) 中](/dotnet/visual-basic/language-reference/modifiers/friend)標示為 (的類型，因為 friend 元件可能會使用欄位。
...
- 類別會以具有[ `new` 條件約束](/dotnet/csharp/language-reference/keywords/new-constraint)的型別參數傳遞。 下列範例將依規則 CA1812 進行旗標：
