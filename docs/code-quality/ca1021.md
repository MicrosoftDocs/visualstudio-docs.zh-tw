...
如果方法必須傳回不同的實例，請使用方法的傳回值來完成這項操作。 請參閱 <xref:System.String?displayProperty=fullName> 類別，以取得可在字串上運作並傳回字串之新實例的各種方法。 使用此模型時，呼叫端必須決定是否保留原始物件。
...
執行**Try\<某些 >** 模式的方法（例如 <xref:System.Int32.TryParse%2A?displayProperty=fullName>）不會引發這個違規。 下列範例顯示的結構（實值型別）會執行 <xref:System.Int32.TryParse%2A?displayProperty=fullName> 方法。
