...
| CA1301 | [CA1301:避免使用重複的快速鍵](../code-quality/ca1301.md) | 便捷鍵也稱為快速鍵，可讓鍵盤使用 ALT 鍵存取控制項。 當多個控制項具有重複的便捷鍵時，就無法妥善定義便捷鍵的行為。 |
...
| CA1403 | [CA1403:自動配置類型不應該是 COM 可見](../code-quality/ca1403.md) | COM 可見的實值型別會使用 System.runtime.interopservices.outattribute. StructLayoutAttribute 屬性設定為 Layoutkind.sequential 標示. Auto 來標記。這些類型的版面配置可以在 .NET 版本之間變更，這會中斷預期特定配置的 COM 用戶端。 |
| CA1404 | [CA1404 必須：在 P/Invoke 之後立即呼叫 GetLastError](../code-quality/ca1404.md) | 對 Marshal.getlastwin32error 方法或對等的 GetLastError 函式進行呼叫 [!INCLUDE[TLA2#tla_win32](../code-quality/includes/tla2sharptla_win32_md.md)] ，而立即先前的呼叫不是作業系統叫用方法。 |
...
| CA2107 | [CA2107:必須檢閱 Deny 和 Permit Only 的使用方式](../code-quality/ca2107.md) |PermitOnly 方法和 Codeaccesspermission.deny。只有具備 .NET 安全性相關知識的使用者才能使用此方法。 而使用這些安全性動作的程式碼應該接受安全性檢閱。 |
...
| CA2127 | [CA2136:成員不應該具有衝突的透明度註釋](../code-quality/ca2136.md) | 關鍵程式碼不能出現在100% 透明的元件中。 此規則會針對類型、欄位和方法層級的任何 SecurityCritical 批註，分析100% 透明的元件。 |
...
| CA2128 |[CA2147:CA2147：透明方法不可以使用安全性判斷提示](../code-quality/ca2147.md) | 這項規則會分析元件中的所有方法和類型，該元件中的所有方法和類型都是100% 透明或混合的透明/關鍵，並且會標示 Assert 的任何宣告式或命令式用法。 |
...
| CA2205 | [CA2205:必須使用 Win32 API 的受控對應項](../code-quality/ca2205.md) | 系統會定義作業系統叫用方法，並提供具有對等功能的 .NET 方法。 |
...
| CA2228 | [CA2228:不要使用尚未發行版本所支援的格式建置資源](../code-quality/ca2228.md) | 使用 .NET 預先發行版本所建立的資源檔，可能無法由支援的 .NET 版本使用。 |
