...
對方法進行呼叫，其名稱包含 "uri"、"Uri"、"urn"、"Urn"、"url" 或 "Url"，且方法的宣告類型包含具有參數的對應方法多載 <xref:System.Uri?displayProperty=fullName> 。
...
下列範例顯示的方法 `ErrorProne` 違反規則，以及可正確呼叫多載的方法 `SaferWay` <xref:System.Uri> ：
