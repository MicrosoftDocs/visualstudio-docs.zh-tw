...
符合下列任一條件時，就會引發此規則：
- 參考的是 <xref:System.Net.SecurityProtocolType?displayProperty=nameWithType> 已被取代的值。
- 代表已被取代值的整數值已指派給 <xref:System.Net.SecurityProtocolType> 變數。
