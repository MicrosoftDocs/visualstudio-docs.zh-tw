...
將自訂字典 XML 檔案命名為*CustomDictionary.xml*。 將字典放在工具的安裝目錄、專案目錄，或與使用者設定檔（*%USERPROFILE%\Application Data \\ ...*）下的工具相關聯的目錄中。若要瞭解如何在 Visual Studio 中將自訂字典加入至專案，請參閱[如何：自訂程式碼分析字典](../code-quality/how-to-customize-the-code-analysis-dictionary.md)。
