...
::: moniker range=">=vs-2019"
從 Visual Studio 2019 16.4 版和 CodeAnalysis. 計量（2.9.5）開始， `SourceLines` 並 `ExecutableLines` 取代先前的 `LinesOfCode` 度量。 如需新計量的說明，請參閱程式[代碼度量值](../code-quality/code-metrics-values.md)。 計量 `LinesOfCode` 可在舊版模式中使用。
::: moniker-end
::: moniker range="vs-2017"
`LinesOfCode`在新的命令列程式碼計量工具中，度量更精確且可靠。 它與任何 codegen 差異無關，當工具組或執行時間變更時，並不會變更。 新工具會計算實際的程式程式碼數，包括空白行和留言。
::: moniker-end

其他度量（例如 `CyclomaticComplexity` 和）會 `MaintainabilityIndex` 使用與舊版*Metrics.exe*相同的公式，但新的工具會計算 `IOperations` （邏輯來源指令）數目，而不是中繼語言（IL）指令。 這些數位會與 Visual Studio IDE 和舊版*Metrics.exe*所產生的數目稍有不同。
