...
CSE 指出處理序的狀態已損毀且系統不予攔截。 在損毀狀態的案例中，如果您將方法標示為 <xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute?displayProperty=fullName> 屬性，一般處理常式只會攔截例外狀況。 根據預設， [Common Language Runtime （CLR）](/dotnet/standard/clr)不會叫用 cse 的 catch 處理常式。

最安全的選項是讓進程損毀，而不會攔截這類例外狀況。 即使是記錄程式碼，也可以讓攻擊者利用記憶體損毀的錯誤。

當使用會攔截所有例外狀況的一般處理常式（例如，`catch (System.Exception e)` 或 `catch` 不含例外狀況參數）攔截 Cse 時，就會觸發此警告。
