...
如果您在程式庫或架構中擲回<xref:System.Exception>一般<xref:System.SystemException>例外狀況類型（例如或），則會強制取用者攔截所有例外狀況，包括不知道如何處理的未知例外狀況。

相反地，會擲回已經存在於架構中的衍生型別，或建立您自己的型別<xref:System.Exception>（衍生自）。
...
|無效`enum`的值|<xref:System.ComponentModel.InvalidEnumArgumentException?displayProperty=fullName>|
...
針對所有其他情況，請考慮建立您自己的衍生自<xref:System.Exception>的類型，並擲回。
