...
規則 CA1052 假設僅包含靜態成員的類型不是為了繼承而設計，因為該類型不會提供可在衍生類型中覆寫的任何功能。 不想要繼承的類型應該以中C#的 `static` 修飾詞標記，以禁止其作為基底類型使用。 此外，應該移除其預設的函式。 在 Visual Basic 中，類別應該轉換成[模組](/dotnet/visual-basic/language-reference/statements/module-statement)。
...
若要修正此規則的違規，請將類型標記為 `static` 並移除預設的函C#式（），或將它轉換成模組（Visual Basic）。
...
下列範例顯示如何在中C#使用 `static` 修飾詞標記類型，以修正此規則的違規：
