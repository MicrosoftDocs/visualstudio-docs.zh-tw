...
## <a name="cause"></a>原因
元件物件模型（COM）可見類型會標示 <xref:System.Runtime.InteropServices.ClassInterfaceAttribute> 屬性設定為 <xref:System.Runtime.InteropServices.ClassInterfaceType>的 `AutoDual` 值。
