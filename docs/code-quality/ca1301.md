...
## <a name="example"></a>範例
下列範例顯示的是包含兩個具有相同存取金鑰之控制項的最小表單。 金鑰會儲存在資源檔中，而不會顯示。 不過，它們的值會出現在已加上批註的 `checkBox.Text` 行中。 藉由交換 `checkBox.Text` 行與其批註化的對應專案，可以檢查重複快速鍵的行為。 不過，在此情況下，此範例不會從規則產生警告。
