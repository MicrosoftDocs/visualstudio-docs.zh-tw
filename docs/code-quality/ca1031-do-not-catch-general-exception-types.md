...
> [!NOTE]
> 從.NET Framework 4 開始，common language runtime (CLR) 不會再提供中的作業系統和 managed 程式碼，例如存取違規發生的損毀的狀態例外狀況[!INCLUDE[TLA#tla_mswin](../code-quality/includes/tlasharptla_mswin_md.md)]、 由 managed 程式碼。 如果您想要編譯的.NET Framework 4 中的應用程式或更新版本，並維護的損毀的狀態例外狀況處理，您可以套用<xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute>屬性處理損毀的狀態例外狀況的方法。
