...
2.  **若要依專案篩選警告**： 在多專案方案中，選擇 上方清單中的一個或多個專案方的 程式碼分析 視窗。 選擇要顯示所有警告的方案名稱。  
  
3.  **若要依嚴重性篩選警告**： 根據預設，指派給程式碼分析訊息的嚴重性**警告**。 您可以指派一或多個訊息的嚴重性**錯誤**在自訂規則集內。 選擇 **警告**或**錯誤**顯示指派個別嚴重性的訊息。 選擇**所有**顯示所有訊息。