...
- 代表安全通訊協定版本的整數值已指派給 <xref:System.Security.Authentication.SslProtocols> 變數，做為傳回值使用 <xref:System.Security.Authentication.SslProtocols> ，或當做 <xref:System.Security.Authentication.SslProtocols> 引數使用。
