...
- <xref:System.IDisposable> 是在類別中根據重新實作。

- 再次覆寫 `Finalize`。

- `Dispose()` 遭到覆寫。

- `Dispose()` 的方法不是公用、[密封](/dotnet/csharp/language-reference/keywords/sealed)或名為**Dispose**。

- `Dispose(bool)` 未受保護、虛擬或未密封。

- 在未密封的類型中，`Dispose()` 必須呼叫 `Dispose(true)`。

- 針對未密封的類型，`Finalize` 的實作為不會呼叫或兩者都是 `Dispose(bool)` 或基類完成項。

違反任何一種模式會觸發警告 CA1063 必須。

宣告和實 <xref:System.IDisposable> 介面的每個未密封型別都必須提供自己的 `protected virtual void Dispose(bool)` 方法。 `Dispose()` 應該呼叫 `Dispose(true)`，而完成項應該呼叫 `Dispose(false)`。 如果您建立宣告和執行 <xref:System.IDisposable> 介面的未密封型別，則必須定義 `Dispose(bool)` 並呼叫它。 如需詳細資訊，請參閱[清除非受控資源（.net 指南）](/dotnet/standard/garbage-collection/unmanaged)和[處置模式](/dotnet/standard/design-guidelines/dispose-pattern)。
...
- 從您的型別所實作為介面的清單中移除 <xref:System.IDisposable>，並改為覆寫基類處置執行。
...
- 修改 Dispose （），使其呼叫 Dispose （true），然後呼叫目前物件實例（`this`上的 <xref:System.GC.SuppressFinalize%2A> 或 Visual Basic 中的 `Me`），然後傳回。

- 修改您的完成項，使其呼叫 Dispose （false），然後傳回。

- 如果您建立宣告和執行 <xref:System.IDisposable> 介面的未密封類型，請確定 <xref:System.IDisposable> 的實作為本節前面所述的模式。
