...
- @No__t_0 的方法不是公用、[密封](/dotnet/csharp/language-reference/keywords/sealed)或名為**Dispose**。
...
宣告和實 <xref:System.IDisposable> 介面的每個未密封型別都必須提供自己的 `protected virtual void Dispose(bool)` 方法。 `Dispose()` 應呼叫 `Dispose(true)`，而完成項應呼叫 `Dispose(false)`。 如果您建立宣告和執行 <xref:System.IDisposable> 介面的未密封型別，則必須定義 `Dispose(bool)` 並呼叫它。 如需詳細資訊，請參閱[清除非受控資源（.net 指南）](/dotnet/standard/garbage-collection/unmanaged)和[處置模式](/dotnet/standard/design-guidelines/dispose-pattern)。
