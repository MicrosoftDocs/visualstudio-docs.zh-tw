...
下列範例顯示只能搭配 <xref:System.IO.FileStream> 物件使用的方法 `ManipulateFileStream`，這會違反此規則。 第二個方法（`ManipulateAnyStream`）藉由使用 <xref:System.IO.Stream>取代 <xref:System.IO.FileStream> 參數來滿足規則。
