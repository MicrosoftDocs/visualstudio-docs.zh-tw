...
欄位的主要用法應該是當做實作詳細資料。 欄位應 `private` 或 `internal`，而且應該使用屬性來公開。 存取屬性的方式很簡單，因為存取欄位時，屬性存取子中的程式碼可能會變更，因為類型的功能會展開，而不會引入中斷性變更。
...
- 當目標平臺利用 `ref` 來修改欄位時，例如適用于 WPF 和 UWP 的模型 viewmodel （MVVM）架構
