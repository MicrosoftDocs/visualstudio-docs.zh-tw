...
## <a name="example"></a>範例
下列範例是由程式庫類型（`SecuredTypeWithFields`）與不安全的欄位所組成，可以建立程式庫類型之實例的類型（`Distributor`），而且不會將實例傳遞給類型沒有建立它們的許可權，以及應用程式程式碼可以讀取實例的欄位，即使它沒有保護該類型的許可權也一樣。
