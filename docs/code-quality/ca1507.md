...
規則 CA1507 會將`string`常值當做引數使用於方法或函式，其中[nameof](/dotnet/csharp/language-reference/keywords/nameof) （`NameOf` Visual Basic）運算式會新增可維護性。 如果符合下列所有條件，則會引發此規則：
