...
- 適用于 <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert?displayProperty=nameWithType> 自訂擴充方法中的 `this` 參數。 @No__t_0 類別中的函式是靜態的，因此不需要存取方法主體中的 `this` 參數。
