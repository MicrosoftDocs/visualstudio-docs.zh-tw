...
但是，如果程式碼有公開例外狀況，稍後用來當做內部例外狀況的基底，則合理地假設程式碼將能夠使用基底例外狀況進行智慧型作業。 公用例外狀況會有比、或所提供的更多資訊 <xref:System.Exception> <xref:System.SystemException> <xref:System.ApplicationException> 。
