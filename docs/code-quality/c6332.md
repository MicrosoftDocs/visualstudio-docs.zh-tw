...
ms.locfileid: "72746255"
---
# <a name="c6332"></a>C6332
警告 C6332：不正確參數：不允許將零當做 dwFreeType 參數傳遞至 \<function >。 這會導致此呼叫失敗

 此警告表示將不正確參數傳遞至 VirtualFree 或 VirtualFreeEx。 VirtualFree 和 VirtualFreeEx 都會拒絕 dwFreeType 參數零。 DwFreeType 參數可以是 MEM_DECOMMIT 或 MEM_RELEASE。 不過，MEM_DECOMMIT 和 MEM_RELEASE 值可能不會在相同的呼叫中一起使用。 此外，請確定不會忽略 VirtualFree 函數的傳回值。
