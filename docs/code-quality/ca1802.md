...
呼叫宣告類型的靜態函式時，會在執行時間計算 `static readonly` 欄位的值。 如果 `static readonly` 欄位在宣告時初始化，而且未明確宣告靜態的函式，則編譯器會發出靜態的函數來初始化欄位。

@No__t_0 欄位的值會在編譯時期進行計算並儲存在中繼資料中，這會在與 `static readonly` 欄位比較時增加執行時間效能。
