...
欄位會 `static` `readonly` 在 Visual Basic) 中宣告及 (， `Shared` `ReadOnly` 並使用在編譯時期可的值進行初始化。
...
欄位的值 `const` 會在編譯時期進行計算並儲存在中繼資料中，這會在與欄位比較時增加執行時間效能 `static readonly` 。
...
您可以只針對此規則設定此選項，針對所有規則，或針對此類別中的所有規則， (效能) 。 如需詳細資訊，請參閱 [設定 FxCop 分析器](configure-fxcop-analyzers.md)。

### <a name="required-modifiers"></a>必要的修飾詞

您可以設定此規則來覆寫必要的欄位修飾詞。 根據預設， `static` 和 `readonly` 都是所分析的欄位所需的修飾詞。 您可以從下表中，將此覆寫為以逗號分隔的一個或多個修飾詞值：
