...
@No__t-0 屬性會決定 COM 用戶端存取 managed 程式碼的方式。 良好的設計會要求組件明確表示 COM 的可視性。 您可以針對整個元件設定 COM 可見度，然後針對個別類型和類型成員加以覆寫。 如果屬性不存在，則 COM 用戶端可以看到元件的內容。
...
## <a name="example"></a>範例
下列範例會顯示已套用 @no__t 0 屬性的元件，以防止 COM 用戶端看到它。
