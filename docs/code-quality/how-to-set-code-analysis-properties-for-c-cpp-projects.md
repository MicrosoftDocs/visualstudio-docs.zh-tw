...
[設定清單] 和 [**平臺**] 清單可讓您將不同的程式碼分析設定套用至不同的專案配置和平臺組合。 例如, 您可以指示程式碼分析, 將一組規則套用至您的專案, 以用於 debug 組建, 以及針對發行組建使用不同的集合。

## <a name="enabling-code-analysis"></a>啟用程式碼分析
您可以選取 [**啟用 C/C++ on Build 的程式碼分析**], 以決定是否要啟用專案的程式碼分析。 例如, 您可以將與設定清單結合, 以決定停用 debug build 的程式碼分析, 並針對發行組建加以啟用。
