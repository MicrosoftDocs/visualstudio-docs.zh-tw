...
<xref:System.ArgumentException>和其衍生類型的一個和兩個字串構造函式的簽章，與位置`message`和`paramName`參數不一致。 請確定使用正確的字串引數呼叫這些函式。 簽章如下所示：
...
若要修正此規則的違規，請呼叫接受訊息、參數名稱或兩者的函式，並確定引數對於<xref:System.ArgumentException>所呼叫的類型而言是正確的。
...
下列程式碼顯示不正確具現化實例的<xref:System.ArgumentNullException>函式。
