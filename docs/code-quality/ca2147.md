...
此規則會分析屬於100% 透明或混合透明/關鍵之元件中的所有方法和類型，並將 <xref:System.Security.CodeAccessPermission.Assert%2A>的任何宣告式或命令式用法標示為旗標。

在執行時間，從透明程式碼 <xref:System.Security.CodeAccessPermission.Assert%2A> 的任何呼叫都會導致擲回 <xref:System.InvalidOperationException>。 這可能會發生在100% 透明元件中，而且也會出現在混合的透明/關鍵元件中，其中方法或類型已宣告為透明，但包含宣告式或命令式判斷提示。
