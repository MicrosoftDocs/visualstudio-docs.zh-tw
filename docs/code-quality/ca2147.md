...
此規則會分析屬於100% 透明或混合透明/關鍵之元件中的所有方法和類型，並將的任何宣告式或命令式用法標記為旗標 <xref:System.Security.CodeAccessPermission.Assert%2A> 。
...
若要解決此問題，請使用來標記呼叫判斷提示的程式碼 <xref:System.Security.SecurityCriticalAttribute> ，或移除該判斷提示。
