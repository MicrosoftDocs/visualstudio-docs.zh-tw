...
索引子（也就是索引的屬性）應該使用整數或字串類型做為索引。 這些類型通常用於編制資料結構的索引，並增加程式庫的可用性。 <xref:System.Object> 類型的使用應該限制在設計階段無法指定特定整數或字串類型的情況下。 如果設計需要其他類型的索引，請重新考慮類型是否代表邏輯資料存放區。 如果它不代表邏輯資料存放區，請使用方法。
