...
從 analyzer 套件的版本3.0.0 開始，預設不再使用命名啟發學習法，但這是 [可](#use-naming-heuristic)設定的。
...
您可以只針對此規則、所有規則或此類別中的所有規則設定此選項， (全球化) 。 如需詳細資訊，請參閱 [設定 .net 程式碼品質分析器](configure-fxcop-analyzers.md)。

### <a name="use-naming-heuristic"></a>使用命名啟發式

您可以設定參數或屬性名稱是否包含 "Text"、"Message" 或 "Caption" 將會觸發此規則。
