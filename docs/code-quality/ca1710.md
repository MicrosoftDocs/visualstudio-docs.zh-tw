...
實 <xref:System.Collections.IDictionary> 的型別具有以「字典」字尾的名稱，即使類型也會執行 <xref:System.Collections.IEnumerable> 或 <xref:System.Collections.ICollection>。 「集合」和「字典」尾碼的命名慣例可讓使用者區分下列兩種列舉模式。
