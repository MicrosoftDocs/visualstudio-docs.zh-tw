...
<xref:System.Runtime.InteropServices.LayoutKind> 配置類型是由通用語言執行平台管理。 這些類型的配置可以變更版本之間的.NET Framework 中，這會中斷 COM 用戶端預期特定的版面配置。 如果<xref:System.Runtime.InteropServices.StructLayoutAttribute>未指定屬性，則C#，Visual Basic 中，和C++指定編譯器[實](<xref:System.Runtime.InteropServices.LayoutKind.Auto>)實值型別。
