...
|CheckId|CA1715|
|類別|Microsoft。命名|
|重大變更|重大-在介面上引發。<br /><br /> 在泛型型別參數上引發時，不會中斷。|
...
型別或方法的 [泛型型別參數](/dotnet/csharp/programming-guide/generics/generic-type-parameters) 名稱開頭不是大寫 t '。
...
您可以設定此規則分析的程式碼部分。 如需詳細資訊，請參閱 [設定 .net 程式碼品質分析器](configure-fxcop-analyzers.md)。
