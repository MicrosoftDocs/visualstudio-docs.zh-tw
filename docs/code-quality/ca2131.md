...
## <a name="cause"></a>原因
類型會參與類型等價，而類型本身或類型的成員或欄位會以 <xref:System.Security.SecurityCriticalAttribute> 屬性來標記。

> [!NOTE]
> 此舊版規則不適用於 FxCop 分析器。 如需詳細資訊，請參閱已[淘汰的規則](fxcop-rule-port-status.md#deprecated-rules)。

## <a name="rule-description"></a>規則描述
此規則會引發任何關鍵的類型或包含參與類型等價之關鍵方法或欄位的類型。 當 CLR 偵測到這種類型時，就無法在執行時間使用 <xref:System.TypeLoadException> 載入它。 通常，只有在使用者手動實作類型等價，而不是依賴 tlbimp 和編譯器進行類型等價時，就會引發這項規則。
