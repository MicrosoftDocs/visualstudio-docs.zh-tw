...
當實例欄位的名稱符合參數或區域變數名稱時，會在 `this` `Me` 方法主體內使用（in）關鍵字來存取實例欄位 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 。 維護程式碼時，很容易忘記這種差異，並假設參數/區域變數參考實例欄位，這會導致錯誤。 這種情況特別適用于冗長的方法主體。
