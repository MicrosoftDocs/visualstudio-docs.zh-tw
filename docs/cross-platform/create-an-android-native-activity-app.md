...
##  <a name="req"></a> 需求  
 請先確認您已符合所有系統需求，並已在 Visual Studio 2015 中安裝適用於跨平台行動裝置開發的 Visual C++，才能建立新的專案。 如需詳細資訊，請參閱[針對跨平台行動裝置應用程式開發安裝 Visual C++](../cross-platform/install-visual-cpp-for-cross-platform-mobile-development.md)。 請確認安裝包含所需之協力廠商工具和 SDK，並已安裝 Visual Studio Emulator for Android。  
