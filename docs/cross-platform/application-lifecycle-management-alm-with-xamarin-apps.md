...
|程式碼涵蓋範圍|N/A||
|[對程式碼進行單元測試](../test/unit-test-your-code.md)|[是]|對於 Windows 和 Android 的目標，可以使用內建的 MSTest 工具。 若要在 Windows、Android 和 iOS 上執行單元測試，Xamarin 會建議使用 NUnit。 請參閱[使用 TFVC](/azure/devops/repos/tfvc/overview?view=vsts)。|
