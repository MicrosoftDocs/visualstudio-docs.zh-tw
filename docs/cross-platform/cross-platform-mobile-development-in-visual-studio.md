...
|[使用安裝跨平臺行動開發C++](/cpp/cross-platform/install-visual-cpp-for-cross-platform-mobile-development)|
|[深入瞭解如何使用C++將目標設為多個平臺](https://visualstudio.microsoft.com/vs/cplusplus-mdd/)（VisualStudio.com）|
|[安裝您需要的內容，然後建立適用C++于 Android 的原生活動應用程式](/cpp/cross-platform/create-an-android-native-activity-app)|
|[深入了解與 Android 及 Windows 應用程式共用 C++ 程式碼](https://visualstudio.microsoft.com/vs/cplusplus-mdd/) (VisualStudio.com)|
|[適用于的跨平臺行動裝置開發範例C++](/cpp/cross-platform/cross-platform-mobile-development-examples)|
...
## <a name="build-a-cross-platform-game-for-android-ios-and-windows-by-using-visual-studio-tools-for-unity"></a>使用 Visual Studio Tools for Unity 建置 Android、iOS 和 Windows 的跨平台遊戲
 ...
 透過 Visual Studio Tools for Unity (VSTU)，您可以使用 Visual Studio 以 C# 撰寫遊戲和編輯器指令碼，然後使用其強大的偵錯工具來尋找及修正錯誤。 最新版的 VSTU 包含對 Unity 2018.1 的支援，且包括 Unity 的 ShaderLab 著色器語言的語法著色、與 Unity 的更佳同步化、更豐富的偵錯，以及 MonoBehavior 精靈的改善式程式碼產生。 VSTU 也會將您的 Unity 專案檔案、主控台訊息和啟動遊戲的功能整合到 Visual Studio 中，以便您可以在撰寫程式碼時，花較少的時間來切換 Unity Editor。
...
- [將 Office 365 API 新增至 Visual Studio 專案](/office/developer-program/office-365-developer-program)
