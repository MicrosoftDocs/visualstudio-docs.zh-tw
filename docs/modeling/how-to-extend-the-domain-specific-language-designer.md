...
如需有關此功能的範例程式碼和詳細資訊，請參閱 Visual Studio[視覺效果和模型化 SDK](https://code.msdn.microsoft.com/Visualization-and-Modeling-313535db)。
...
1. 使用 [**類別庫**] 專案範本建立新的專案。 此專案將包含您擴充功能的程式碼。
...
若要測試本主題中的任何延伸模組，請建立並執行解決方案。 Visual Studio 的實驗執行個體隨即開啟。 在此情況下，請開啟 DSL 解決方案。 編輯 [Dsldefinition.dsl 檔] 圖表。 您可以看到延伸模組行為。
...
2. 將此檔案複製到目的電腦，然後在 [Windows Explorer] （或 [檔案瀏覽器]）中按兩下該檔案。
     ...
     [Visual Studio 擴充管理員] 隨即開啟，以確認已安裝延伸模組。
