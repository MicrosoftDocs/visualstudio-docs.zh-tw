...
當使用者在 Visual Studio 中儲存特定領域語言（DSL）的實例或*模型*時，就會建立或更新 XML 檔案。 您可以重載該檔案，以在存放區中重新建立模型。
...
![DSL 定義圖表 &#45; 家族樹狀結構模型](../modeling/media/familyt_person.png)
...
- 啟用 DSL 的驗證。 在 [DSL Explorer] 中，選取 [Editor\Validation]，然後將 [**使用 ...** ] 屬性設為 `true` 。
     ...
     自動產生的驗證方法會檢查是否有多義性。 方法位於 `Load` 驗證類別目錄中。 這可確保使用者會收到警告，表示可能無法重新開啟檔案。
