...
當您設計 DSL 時，您可以將它當做 Visual Studio 整合擴充功能 (VSIX) 封裝的一部分散發。 使用者使用 Visual Studio 中的 DSL:
...
本概觀的其餘部分則介紹建立和使用 Visual Studio 中的特定領域語言的基本操作的逐步解說。
...
## <a name="create-a-dsl-solution"></a>建立 DSL 方案

若要建立新的定義域專屬語言，您可以建立新的 Visual Studio 方案使用定義域專屬語言專案範本。
...
-   **在 DslPackage 專案**此專案包含可讓以開啟及編輯 Visual Studio 中之 DSL 執行個體的程式碼。
...
1.  按一下 [**轉換所有範本**中**方案總管] 中**工具列。 這樣會重新產生大部分的 DslDefinition.dsl 中的原始程式碼。

    > [!NOTE]
    > 每當您變更*DslDefinition.dsl*，您必須按一下**轉換所有範本**重新建置方案之前。 您可以自動化此步驟。 如需詳細資訊，請參閱 <<c0> [ 如何自動執行轉換的所有範本](/previous-versions/visualstudio/visual-studio-2012/ff521399\(v\=vs.110\))。

2.  按 **F5**，或在 [偵錯]  功能表上，按一下 [開始偵錯] 。

     建置 DSL，並已安裝在 Visual Studio 的實驗執行個體。

     啟動 Visual Studio 的實驗性執行個體。 實驗執行個體需要它的設定，從個別的子樹的登錄中，Visual Studio 擴充功能註冊以進行偵錯的所在。 一般 Visual Studio 執行個體並沒有存取那里註冊的擴充功能。

3.  在 Visual Studio 的實驗性執行個體，開啟名為的模型檔案**測試**從**方案總管 中**。
...
如果您無法看到 [總管] 中的 Visual Studio 偵錯的執行個體上**檢視**功能表中的指向**其他 Windows**，然後按一下*\<您的語言 >* **Explorer**。
...
在偵錯方案中，開啟範本檔案以副檔名".tt 」。 這些範例示範如何從模型中，產生的文字，並讓您測試您的 DSL 的 API。 其中一個範例以[!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]，則在其他[!INCLUDE[csprcs](../data-tools/includes/csprcs_md.md)]。
...
1.  在 Visual Studio 的實驗性執行個體，將儲存模型檔案。
...
當您想要修改 DSL 定義中時，關閉實驗執行個體，並更新主要的 Visual Studio 執行個體中的定義。
...
3.  重建 DSL。
    ...
    3.  按 F5。 等到 Visual 的 Studio 實驗執行個體出現。

4.  在偵錯方案中的 Visual Studio 實驗執行個體中，開啟測試模型檔案。 從工具箱拖曳至其本身的項目。 請注意，已變更的工具標題和 DSL 總管 中的型別名稱。
...
9. 在 Visual Studio 的實驗性執行個體，開啟測試模型檔案。 使用新的工具來建立鄉鎮和鄉鎮與人員之間的連結。 請注意，您可以只建立正確類型的項目之間的連結。
...
## <a name="deploying-the-dsl"></a>部署 DSL
 ...
 方案 [bin] 資料夾中，找出.vsix 檔案。 請將它複製到您想要將它安裝所在的電腦。 在該電腦中，按兩下 VSIX 檔案。 DSL 可以用於該電腦上的 Visual Studio 的所有執行個體。
...
##  <a name="Reset"></a> 移除舊的實驗性 Dsl
 ...
 所有的實驗性 Dsl 和其他實驗性 Visual Studio 延伸模組，這會移除從您的電腦。 這些是已在偵錯模式執行的擴充功能。

 Dsl 或其他已藉由執行 VSIX 檔案完整安裝的 Visual Studio 擴充功能，並不會移除此程序。
...
2.  重建任何實驗的 Dsl 或要使用其他實驗性 Visual Studio 延伸模組。
