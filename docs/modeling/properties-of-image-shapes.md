...
ms.locfileid: "85520844"
---
# <a name="properties-of-image-shapes"></a>影像圖案的屬性

您可以使用「影像」圖形來指定如何在產生的設計工具中顯示網域類別。 將 `Image` 類別的屬性設定為預先定義的影像檔，以定義影像圖形。 下列為支援的格式：
...
|存取修飾詞|Geometry 圖形的存取修飾詞（公用或內部）。|公用|
...
|繼承修飾詞|描述從影像圖形（或）產生之原始程式碼類別的繼承種類 `none` `abstract` `sealed` 。|無|
|基底影像圖形|這個圖形的基類。|(無)|
|名稱|這個圖形的名稱。|目前名稱|
|命名空間|與這個圖形關聯的命名空間。|目前的命名空間|
|工具提示類型|工具提示的定義位置（[固定]、[變數] 或 [無]）。 若已修正，則 `Fixed Tooltip Text` 會使用屬性的值做為工具提示; 如果變數，則會在自訂程式碼中定義工具提示。|無|
