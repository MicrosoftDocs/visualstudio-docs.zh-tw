...
如果您撰寫的文字模板會讀取包含 Visual Studio ModelBus 參考的模型，您可能會想要解析參考以存取目標模型。 在此情況下，您必須調整文字模板和參考的特定領域語言（Dsl）：
...
若要解析文字模板中的 ModelBus 參考，目標 DSL 必須有相容的介面卡。 文字模板會在與 Visual Studio 檔編輯器不同的 AppDomain 中執行，因此，介面卡必須載入模型，而不是透過 DTE 來存取。

1. 如果目標 DSL 解決方案沒有**ModelBusAdapter**專案，請使用 Modelbus 延伸模組 wizard 建立一個：

    1. 下載並安裝 Visual Studio ModelBus 擴充功能（如果您尚未這麼做）。 如需詳細資訊，請參閱[視覺效果和模型化 SDK](https://devblogs.microsoft.com/devops/the-visual-studio-modeling-sdk-is-now-available-with-visual-studio-2017/)。
...
2. 如果您想要從文字模板和其他程式碼（例如命令）存取 DSL，請複製**ModelBusAdapter**專案：
    ...
    5. 以滑鼠右鍵按一下**方案總管**中的新專案，然後按一下 [**屬性**]。 在 [屬性編輯器] 中，變更所產生元件和預設命名空間的名稱。
...
## <a name="write-a-text-template-that-can-resolve-modelbus-references"></a>撰寫可以解析 ModelBus 參考的文字模板
...
```
 ...
 執行此文字模板時，指示詞會 `SourceDsl` 載入檔案 `Sample.source` 。 從開始，範本可以存取該模型的元素 `this.ModelRoot` 。 程式碼可以使用該 DSL 的網域類別和屬性。
...
1. 在 [檔案管理器] 中，複製並貼上包含*ModelBusAdapter*的資料夾。
...
5. 在 `DslPackage` 專案中，將專案參考加入至 `T4ModelBusAdapter` 。

6. 在 DslPackage\source.extension.tt 中，于底下新增下面這一行 `<Content>` 。
...
11. 確認 DSL 是否正常運作。 在實驗專案中，開啟 `Sample.provider` 。 關閉 Visual Studio 的實驗執行個體。
...
2. 在 DSL 專案中，新增 MBProvider DSL 元件的參考。 按一下滑鼠右鍵 `MBConsumer\Dsl\References` ，然後按一下 [**加入參考**]。 在 [**流覽**] 索引標籤中，找出`MBProvider\Dsl\bin\Debug\Company.MBProvider.Dsl.dll`
...
1. 在 MBConsumer 方案中，按下 CTRL + F5。 Visual Studio 的實驗實例會在**MBConsumer\Debugging**專案中開啟。

2. 新增範例的複本。提供給**MBConsumer\Debugging**專案。 這是必要的，因為 ModelBus 參考必須參考同一個方案中的檔案。

   1. 以滑鼠右鍵按一下 [調試] 專案，指向 [**加入**]，然後按一下 [**現有專案**]。

   2. 在 [**加入專案**] 對話方塊中，將篩選準則設定為 [**所有檔案（ \* . \* ）**]。
...
5. 儲存檔案。 （尚未關閉 Visual Studio 的實驗實例。）
...
1. 在 Visual Studio 的實驗實例中，開啟範例文字模板檔案。 設定其內容，如下所示。
    ...
    - 取用者模型會透過在該 DSL 中產生的指示詞處理器，以一般方式存取。

    - 元件和匯入指示詞必須能夠存取 ModelBus 和提供者 DSL 的類型。

    - 如果您知道許多 Mbr 都連結到相同的模型，最好只呼叫 CreateAdapter 一次。
