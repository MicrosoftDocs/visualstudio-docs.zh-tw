...
在 Visual Studio 的文字模板中，您可以使用 `<#@include#>` 指示詞，包含來自另一個檔案的文字。 您可以將 `include` 指示詞放在文字範本中第一個 `<#+ ... #>` 類別功能區塊之前的任何位置。 被納入的檔案也包含 `include` 指示詞和其他指示詞。 如此可讓您在範本之間共用範本程式碼和重複使用文字。
...
- 包含檔案的名稱不需要使用 `".tt"` 副檔名。
   ...
   您可以使用其他副檔名 (例如 `".t4"`) 做為包含檔案的副檔名。 這是因為當您將 `.tt` 檔案加入至專案時，Visual Studio 會自動將其**自訂工具**屬性設定為 [`TextTemplatingFileGenerator`]。 您通常不希望被納入的檔案進行個別轉換。
...
## <a name="msbuild"></a>在 MSBuild 和 Visual Studio 中使用專案屬性
 雖然您可以在 include 指示詞中使用 Visual Studio 的宏（例如 $ （SolutionDir）），但它們在 MSBuild 中無法使用。 如果想要轉換組建電腦中的範本，您必須改用專案屬性。
