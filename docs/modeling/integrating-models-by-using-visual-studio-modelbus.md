...
Visual Studio ModelBus 提供一種方法，可讓您在模型和其他工具之間建立連結到模型。 例如，您可以連結特定領域語言（DSL）模型和 UML 模型。 您可以建立一組整合的 DSL。
...
> [!NOTE]
> 在 ModelBus 的目前 Visual Studio 執行中，連結的模型必須是相同 Visual Studio 方案中的專案。
...
## <a name="provide"></a>提供 DSL 的存取權
 您必須定義用於 DSL 的 ModelBusAdapter，才能建立模型或其項目的 ModelBus 參考。 若要這麼做，最簡單的方法是使用 Visual Studio 模型匯流排擴充功能，將命令新增至 DSL 設計工具。
...
根據預設，Visual Studio ModelBus 介面卡會使用元素的 guid 來識別它。 因此，這些 ID 必須保存在模型檔中。
...
3. 在 Visual Studio 的實驗實例中的偵錯工具專案中，新增每個 DSL 實例的檔案。

    > [!NOTE]
    > Visual Studio ModelBus 只能解析相同 Visual Studio 方案中的專案之模型的參考。 例如，您無法建立針對檔案系統其他部分之模型檔的參考。
...
若要建立模型參考，您需要模型為其實例之 DSL 的 AdapterManager，以及模型的檔案名或 Visual Studio 專案專案。

若要建立項目參考，您需要模型檔的配接器，以及所要參考的項目。

> [!NOTE]
> 使用 Visual Studio ModelBus，您可以只對相同 Visual Studio 方案中的專案建立參考。
