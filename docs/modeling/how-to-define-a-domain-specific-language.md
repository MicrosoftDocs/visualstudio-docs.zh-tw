...
### <a name="test-the-solution"></a>測試方案
 ...
 若要測試方案，請按 F5 或 CTRL+F5。 在實驗模式中，開啟 Visual Studio 的新執行個體。

 在 Visual Studio 中，方案總管 中的新執行個體中開啟範例檔案。 此檔案會開啟為具有工具箱的圖表。
...
3.  **開啟 [總管] 中。** 在圖表的側邊是語言總管視窗中，通常名為*YourLanguage*總管。 如果您沒有看到這個視窗，可能是在 [方案總管] 下方的索引標籤上。 如果您找不到的話**檢視**功能表上，指向**其他 Windows**，然後按一下  *YourLanguage* **總管**。
...
|DSL 的工具箱未出現，但是我未變更組件名稱。<br /><br /> 或者，出現一個訊息方塊，回報載入擴充功能失敗。|重設實驗執行個體，然後重建方案。<br /><br /> 1.在 Windows 啟動 功能表底下**所有程式**，展開[!INCLUDE[vssdk_current_long](../misc/includes/vssdk_current_long_md.md)]，然後**工具**，然後按一下**重設 Microsoft Visual Studio 實驗執行個體**。<br />2.在 Visual Studio**建置**功能表上，按一下**重建方案**。|
