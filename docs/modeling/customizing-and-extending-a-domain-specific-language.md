...
|圖形色彩或其他功能（例如字型）表示目前的狀態。|請參閱 [更新圖形和連接器以反映模型](../modeling/updating-shapes-and-connectors-to-reflect-the-model.md)。<br /><br /> 建立可更新公開屬性的規則。 請參閱 [規則傳播模型內的變更](../modeling/rules-propagate-changes-within-the-model.md)。<br /><br /> 或者，使用 OnAssociatedPropertyChanged () 更新未公開的功能，例如連結箭號或字型。|
...
|在任何圖形上設定背景影像|覆寫 InitializeInstanceResources () 以新增錨定的 ImageField。|
...
|刪除專案時，刪除、重做或重新連結相關的元素。|設定關聯性角色的 [ **傳播刪除** 值]。 如需更複雜的效果，請 `ShouldVisitRelationship` 在 DomainModel 中定義的類別中，覆寫和 `ShouldVisitRolePlayer` 方法 `MyDslDeleteClosure` 。 |
...
|使用一項工具建立專案群組。 例如，具有一組固定埠的元件。|覆寫 ToolboxHelper 中的 [工具箱] 初始化方法。 建立專案群組原型 (EGP) 包含元素及其關聯性連結。 請參閱 [自訂工具和工具箱](../modeling/customizing-tools-and-the-toolbox.md)。<br /><br /> 請在 EGP 中包含主體和埠圖形，或在 EGP 具現化時，定義 BoundsRules 來放置埠圖形。|
