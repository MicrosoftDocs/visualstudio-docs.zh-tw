...
您可以將 code map 儲存為 Visual Studio 專案、影像或 XPS 檔案的一部分。
...
若要將對應儲存為特定專案的一部分，請在 [對應] 工具列上，選擇 [**共用**] [將] [  >  **移 \<CodeMapName> 至**]，然後選擇您要儲存對應的專案。

![將對應移至其他專案](../modeling/media/codemapsmovemapmenu.png)

Visual Studio 會將對應儲存為 *.dgml*檔案，讓您可以與 Visual Studio Enterprise 和 Visual Studio Professional 的其他使用者共用。
...
1. 在 [Code Map] 工具列上**Share**，選擇 [  >  **以影像形式共用電子郵件**] 或 [**複製影像**]。
...
1. 在 [Code Map] 工具列上，選擇 [**共用**  >  **電子郵件為便攜 xps** ] 或 [**另存為便攜 xps**]。
