...
4. 底下**藉由建立連結的程序合併**，新增兩個路徑：
   ...
   - 一個路徑會連結新項目連結至父模型。 您必須輸入的路徑運算式巡覽從現有的項目，向上的內嵌關聯性的父模型。 最後，它會在新的連結，將會指派新的項目中指定的角色。 路徑如下所示：

      `ExampleModelHasElements.ExampleModel/!ExampleModel/.Elements`

   - 在另一個路徑會連結新項目至現有的項目。 路徑運算式會指定參考關聯性和新的項目指派給角色。 此路徑如下所示：
...
5. 測試 DSL:
   ...
   2. 啟動 Visual Studio 的實驗執行個體之後，開啟您的 DSL 的模型檔案。 建立一些範例項目。
...
6. 測試 DSL:
    ...
    1. 按下**F5**重建解決方案。 開啟 Visual Studio 的實驗執行個體時，請開啟您的 DSL 執行個體。

    2. 以數種方式建立新的項目：

        - 從拖曳**範例項目**工具拖曳至圖表。

        - 在 **範例模型總管**，以滑鼠右鍵按一下根節點，然後按一下**新增新的範例項目**。
...
1. 建立[!INCLUDE[dsl](../modeling/includes/dsl_md.md)]解決方案使用元件模型範本。
...
10. 建置並執行方案。 Visual Studio 的新執行個體隨即出現。
