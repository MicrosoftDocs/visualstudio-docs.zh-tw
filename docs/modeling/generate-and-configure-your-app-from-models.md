...
- **啟動特定，然後一般化。** 先撰寫應用程式的特定版本。 這個版本應該作用於一組條件。 滿意其運作正常狀態時，即可讓它的一部分衍生自模型。 逐漸擴充衍生的組件。
     ...
     例如，設計一個具有一組特定網頁的網站，再設計 web 應用程式來呈現模型中所定義的頁面。

- **建立變體層面的模型。** 識別將會改變的層面 (兩個部署之間，或一段時間的需求變更)。 這些是應該衍生自模型的層面。

     例如，如果一組網頁和其間的連結變更，但是頁面的樣式和格式一律相同，則模型應該描述這些連結，但不需要描述頁面的格式。

- **不同的考慮。** 如果變動層面可以分成獨立區域，請為每個區域使用不同的模型。 使用 ModelBus，您可以定義影響兩個模型的作業以及其間的條件約束。

     例如，您可以使用一個模型來定義網頁和不同模型之間的導覽，以定義頁面的版面配置。

- **建立需求的模型，而不是解決方案。** 設計模型，使其描述使用者需求。 反之，請不要根據實作的變動層面來設計標記法。

     例如，web 導覽模型應該代表網頁和其之間的超連結。 Web 導覽模型不應代表應用程式中的 HTML 或類別片段。

- **產生或解讀？** 如果特定部署的需求極少變更，請透過模型產生程式碼。 如果需求可能經常變更，或可能共存於相同部署的多個變異中，請撰寫應用程式，使它可以讀取和轉譯模型。

     例如，如果您使用您的網站模型來開發一系列不同且獨立安裝的網站，則您應該從模型產生網站的程式碼。 但是，您可以使用您的模型來控制每天變更的網站，因此最好是撰寫可讀取模型並據此呈現網站的 web 伺服器。

- **UML 或 DSL？** 請考慮使用造型來擴充 UML 以建立模型標記法。 如果沒有符合用途的 UML 圖表，請定義 DSL。 但是，請避免破壞 UML 的標準語意。

     例如，UML 類別圖是方塊與箭號的集合；運用這個標記法，您理論上可以定義任何項目。 但是，不建議您使用類別圖，而事實上描述一組類型的位置除外。 例如，您可以調整類別圖表來描述不同類型的 web 網頁。
