...
```
 ...
 您可以指定您自己的編輯器或在提供的編輯器[!INCLUDE[dnprdnshort](../code-quality/includes/dnprdnshort_md.md)]，這類<xref:System.Windows.Forms.Design.FileNameEditor>或<xref:System.Drawing.Design.ImageEditor>。 例如，使用下列程序具有使用者可以在其中輸入檔案名稱的屬性。
