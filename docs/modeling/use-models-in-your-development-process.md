...
- 您可以偶爾使用模型來產生程式碼或其他成品，例如資料庫結構描述或文件。 例如，Visual Studio 的模型化元件會從模型產生。 如需詳細資訊，請參閱[從模型產生和設定您的應用程式](../modeling/generate-and-configure-your-app-from-models.md)。

您可以在各種不同的程序中使用模型，不論是極端敏捷式還是高度形式化都可以。

## <a name="use-models-to-reduce-ambiguity"></a>使用模型來減少不明確的情況
...
## <a name="use-models-with-other-artifacts"></a>搭配其他成品使用模型

模型本身並非需求規格或架構。 雖然它是用來針對這些事情更清楚地表達某些層面的工具，不過並無法表達軟體設計期間所需的所有概念。 因此，這些模型應該與其他通訊方式一起使用，例如 OneNote 頁面或段落、Microsoft Office 檔、Team Foundation 中的工作專案，或專案室牆上的便箋。 除了最後一個項目以外，上述所有物件類型都可以連結至該模型的項目部分。

通常與模型一起使用的其他規格層面包含下列項目。 根據您的專案規模和樣式，可能會使用其中許多層面，或完全不使用任何層面：

- 使用者劇本。 使用者劇本是系統行為之某個層面的簡短描述 (與使用者和其他專案關係人一起討論)，該層面將在其中一個專案反覆項目中傳遞。 一般使用者案例會開始「客戶將能夠 ...」使用者案例可能會引進一組使用案例，或定義先前已開發的使用案例延伸模組。 定義或擴充使用案例有助於讓使用者劇本更加清楚。
...
### <a name="sharpen-focus-as-each-iteration-approaches"></a>當每個反復專案接近時，將焦點銳化
...
在需求研討會之後，請詳述該需求模型的詳細資料，並且將此模型連結至開發工作。 若要這麼做，您可以將 Team Foundation 中的工作專案連結至模型中的元素。
...
|設計層級|圖表類型|
|-|-|
|商務程序<br /><br /> 了解使用系統的內容可協助您了解使用者的需求。|-概念類別圖表描述商務程式中所使用的商務概念。|
|使用者需求<br /><br /> 使用者對於系統需求的定義。|-商務規則和服務需求品質可以在不同的檔中加以描述。|
|高階設計<br /><br /> 系統的整體結構：主要元件以及它們如何結合在一起。|-相依性圖表描述系統如何結構化為相互相依的元件。 您可以針對相依性圖表驗證程式代碼，以確保它符合架構。|
|程式碼分析<br /><br /> 可以從程式碼產生圖表。|-相依性圖表會顯示類別之間的相依性。 更新的程式碼可以針對相依性圖表進行驗證。<br />-類別圖表會顯示程式碼中的類別。|
