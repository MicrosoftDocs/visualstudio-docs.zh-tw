...
```  
 ...
 檔案名稱從這個範本建立項目時，會根據使用者輸入的名稱**加入新項目** 對話方塊。 建立多檔案項目範本時，這非常有用。 如需詳細資訊，請參閱[＜How to：建立多檔案項目範本](../ide/how-to-create-multi-file-item-templates.md)並[範本參數](../ide/template-parameters.md)。  
