...
2. IDE 會決定目前的原始程式碼檔案和位置，在編輯器中顯示為反白顯示。

3. Debug 會話通常會藉由呼叫程式的**Continue**方法來回應第一個停止事件。

4. 程式接著會執行，直到它遇到停止條件，例如叫用中斷點。 在這種情況下，DE 會將中斷點事件傳送至 debug 會話。 中斷點事件是一個停止事件，然後再次取消執行以等待使用者回應。

5. 如果使用者想要逐步執行、跳過或跳出函式，IDE 會提示 [debug] 會話呼叫程式的 `Step` 方法。 然後，IDE 會傳遞步驟的單位（指令、語句或行）和步驟的類型（無論是逐步執行、跳過或不進入函式）。 當步驟完成時，[取消] 會將步驟完成事件傳送至 debug 會話，也就是「停止」事件。
   ...
   如果使用者想要逐步執行、跳過或跳出函式，IDE 會提示 SDM 呼叫[IDebugProgram2：： step](../../extensibility/debugger/reference/idebugprogram2-step.md)。 然後，IDE 會傳遞[STEPUNIT](../../extensibility/debugger/reference/stepunit.md) （指令、語句或行）和[STEPKIND](../../extensibility/debugger/reference/stepkind.md)，也就是要逐步執行、跳過或移出函式。 當步驟完成時，DE 會將[IDebugStepCompleteEvent2](../../extensibility/debugger/reference/idebugstepcompleteevent2.md)介面傳送至 SDM，這是停止事件。
   ...
   如果 debug 封裝是要忽略特定的停止事件，則 debug 封裝會呼叫 SDM，而後者會呼叫[IDebugProgram2：： Continue](../../extensibility/debugger/reference/idebugprogram2-continue.md)。 如果程式在遇到停止狀況時，在函式中逐步執行、跳過或跳出，它會繼續執行步驟。 這表示程式會維持逐步執行狀態，讓它知道如何繼續進行。
