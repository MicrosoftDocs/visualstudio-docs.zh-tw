...
## <a name="members"></a>成員
初始化/使用 BPRESI_BPRESLOCATION `bpResLocation` （中斷點解析位置） 欄位[BP_RESOLUTION_INFO](../../../extensibility/debugger/reference/bp-resolution-info.md)結構。

初始化/使用 BPRESI_PROGRAM`pProgram`欄位`BP_RESOLUTION_INFO`結構。

初始化/使用 BPRESI_THREAD`pThread`欄位`BP_RESOLUTION_INFO`結構。

BPRESI_ALLFIELDS 指定所有欄位。
