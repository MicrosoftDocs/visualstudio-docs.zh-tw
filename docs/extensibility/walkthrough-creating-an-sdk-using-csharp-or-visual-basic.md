...
在本逐步解說中，您將瞭解如何使用 Visual c # 建立簡單的數學程式庫 SDK，然後將 SDK 封裝為 Visual Studio 擴充功能（VSIX）。 您將完成下列程式：
...
8. 在 [**設定**] 欄中，確認 [ **SimpleMath** row] 已設為 [**發行**]，然後選擇 [**關閉**] 按鈕以接受變更。
   ...
   > [!IMPORTANT]
   > SimpleMath 元件的 SDK 僅包含一個設定。 此設定必須是發行組建，或使用元件的應用程式不會通過的認證 [!INCLUDE[win8_appstore_long](../debugger/includes/win8_appstore_long_md.md)] 。

9. 在**方案總管**中，開啟**SimpleMath**專案節點的快捷方式功能表，然後選擇 [**組建**]。

## <a name="to-create-the-simplemathvsix-extension-project"></a><a name="createVSIX"></a>若要建立 SimpleMathVSIX 擴充功能專案
...
11. 在**方案總管**中，開啟的快捷方式功能表 `SDKManifest.xml` ，選擇 [**屬性**]，然後將 [**在 VSIX 中包含**] 屬性的值變更為**True**。
...
16. 將子資料夾重新命名為 `commonconfiguration` ，在其中建立子資料夾，並為子資料夾命名 `neutral` 。
...
2. 在範本清單中，展開 [ **Visual c #** ] 或 [ **Visual Basic**]，然後選擇 [ **Windows Store** ] 節點。
...
6. 在詳細資料窗格中，選擇 [ **WinRT 數學程式庫**] 延伸模組。
    ...
    SDK 的其他相關資訊隨即出現。 您可以選擇 [**更多資訊**] 連結來開啟 https://msdn.microsoft.com/ ，如同您稍早在本逐步解說的 SDKManifest.xml 檔案中所指定。
...
13. 在應用程式中，輸入任兩個數字，選擇作業，然後選擇 **=** 按鈕。
