...
> [!NOTE]
>  注意： 對於不支援新 「 顯示語言 」 （例如，F #、 Cobol、 Fortran） 程式碼顏色標示的程式碼片段會單色。  
...
-   後援的品牌封裝 (Branding_\<地區設定 >.mshc) 安裝說明檢視器 2.3 應用程式根目錄中 (範例： C:\Program Files (x86) \Microsoft Help Viewer\v2.3) 說明檢視器語言組件。  這適用於未安裝任一產品品牌封裝的情況下 （已安裝任何內容） 或已安裝的品牌封裝已損毀的位置。  請注意使用商標套件應用程式根後援時，會忽略的 Visual Studio 項目 （標誌和意見反應）。  
...
|當 SelfBranded = False|onLoad|讀取查詢字串來檢查其是否為列印要求。  設定將使用者的慣用索引標籤的所有 codesnippets。如果是列印要求再將 isPrinterFriendly 設為 true。 檢查有高對比模式。|  
