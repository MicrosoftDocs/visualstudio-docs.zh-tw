...
## <a name="project-file"></a>專案檔
 [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)]整合式的開發環境 (IDE)，以便將範本對應至正確的專案類型需要專案副檔名。 因為 Web 專案沒有專案檔，dummy 專案檔案的副檔名.webproj 註冊要對應的專案類型範本。

 語言名稱字串 （選擇性） 加入範本，若要讓 Web 專案系統中設定預設語言**加入新項目**以範本為基礎的項目 對話方塊。 字串必須是檔案的第一行。 它必須符合在 IntelliSense 引擎註冊中，在 AddItemLanguageName 下註冊的名稱和專案 Subtype(VsTemplate) 下註冊的名稱。 如需詳細資訊，請參閱 <<c0> [ 網站支援屬性](../../extensibility/internals/web-site-support-attributes.md)。

 如果字串不存在，Web 專案系統會嘗試判斷頁面新增至 Web 專案的專案範本的語言屬性和檔案延伸模組為基礎的預設語言。
...
### <a name="empty-web-site-projects"></a>空白網站專案
 這些檔案建立新的空網站，以回應**空網站**命令，以選擇後可取得**檔案** > **New Web Site**:
...
### <a name="web-site-projects"></a>網站專案
 這些檔案建立新的網站，以回應**ASP.NET Web Site**命令，以選擇後可取得**檔案** > **新網站**:

- Default.aspx

     新的網站預設首頁。 Language 屬性會指定程式碼後置的語言，並 CodeFile 屬性指定包含此頁面相關聯的程式碼後置程式碼的相依檔案。
...
### <a name="web-service-projects"></a>Web 服務專案
 這些檔案建立新的網站，以回應**ASP.NET Web 服務**命令，以選擇後可取得**檔案** > **New Web Site**:
