...
ms.locfileid: "58149584"
---
# <a name="solutions-overview"></a>解決方案概觀

解決方案是建立應用程式一起運作的一或多個專案的群組。 屬於方案的專案和狀態資訊會儲存在兩個不同的方案檔。 [方案 (.sln) 檔](solution-dot-sln-file.md)是以文字為基礎和可放置在原始程式碼控制之下，並讓使用者彼此共用。 [方案使用者選項 (.suo) 檔案](solution-user-options-dot-suo-file.md)是二進位檔。 如此一來，.suo 檔案不能放在原始檔控制之下，並包含使用者特定資訊。
...
3. 如果載入 VSPackage 時，此環境會呼叫`QueryInterface`針對<xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage>VSPackage 所需要的介面的介面。

   - 從.sln 檔案讀取時，環境會呼叫`QueryInterface`針對`IVsPersistSolutionProps`。

   - 從.suo 檔案讀取時，環境會呼叫`QueryInterface`針對`IVsPersistSolutionOpts`。
