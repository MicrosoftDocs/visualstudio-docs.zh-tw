...
在中使用 managed package framework （MPF）語言類別來執行語言服務 [!INCLUDE[csprcs](../../data-tools/includes/csprcs_md.md)] 相當簡單。 您需要 VSPackage 來裝載語言服務、語言服務本身，以及您語言的剖析器。
...
3. 選取 [!INCLUDE[csprcs](../../data-tools/includes/csprcs_md.md)] 作為語言，並使用選項來產生新的金鑰檔。 按 [下一步] 。
...
6. 輸入 [**插入程式碼片段**] 做為**命令名稱**及 `cmdidInsertSnippet` **命令識別碼**。 按一下 [完成] 。
...
7. 將 `MyLanguageService` 類別修改為衍生自 <xref:Microsoft.VisualStudio.Package.LanguageService> 類別：
...
5. 加入類別的函式，該函式會採用至少與基類的「檢查程式」相同的參數，並將的「處理常式」參數傳遞至基類的「函式」。
     ...
     例如，衍生自類別之類別的函式 <xref:Microsoft.VisualStudio.Package.Source> 可能如下所示：
