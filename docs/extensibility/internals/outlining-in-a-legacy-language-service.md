...
## <a name="the-hidden-region"></a>隱藏的區域
 ...
 語言服務剖析器會使用<xref:Microsoft.VisualStudio.Package.AuthoringSink.AddHiddenRegion%2A>方法來加入新的隱藏的區域，搭配預設行為，隱藏的區域中，雖然<xref:Microsoft.VisualStudio.Package.AuthoringSink.AddHiddenRegion%2A>方法可讓您自訂的外觀和行為的外框。 隱藏的區域都各有隱藏的區域工作階段中，一旦[!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)]管理語言服務的隱藏的區域。
