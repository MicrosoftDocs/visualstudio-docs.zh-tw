...
- 您可以使用以 XML 為基礎的命令資料表設定編譯器（ *Vsct.exe*），從現有的 *.ctc*檔案產生檔案。
...
6. 類型
   ...
   ```
    ...
    此動作會建立新的 *.Vsct* XML 命令資料表來源檔案。 您可以使用*Vsct.exe*.vsct 編譯器來編譯檔案，就像 *.vsct*檔案一樣。
...
4. 類型
    ...
    ```
     ...
     此程式會建立新的 *.Vsct* XML 命令資料表編譯器檔案。 您可以使用*vsct.exe*.vsct 編譯器來編輯和編譯檔案，就像 *.vsct*檔案一樣。
...
2. 加入包含元素的[ItemGroup 元素](../../msbuild/itemgroup-element-msbuild.md) `VSCTCompile` ，如下列範例所示。
    ...
    ```
     ...
     `ResourceName`元素應一律設定為 `Menus.ctmenu` 。

3. 如果您的專案包含 *.resx*檔案，請加入 `EmbeddedResource` 包含元素的元素 `MergeWithCTO` ，如下列範例所示：
    ...
    ```
     ...
     此標記應該放在 `ItemGroup` 包含內嵌資源的元素內。

4. 在編輯器中，開啟封裝檔案，通常名為* \<ProjectName\> Package.cs*或* \<ProjectName\> package .vb*。
...
5. 將 `ProvideMenuResource` 屬性新增至 package 類別，如下列範例所示。
    ...
    ```
     ...
     第一個參數值必須符合 `ResourceName` 您在專案檔中定義的屬性值。
