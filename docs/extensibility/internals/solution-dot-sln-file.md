...
ms.locfileid: "58157291"
---
# <a name="solution-sln-file"></a>方案 (.sln) 檔案

方案是組織專案在 Visual Studio 中的結構。 解決方案會保留在兩個檔案的專案的狀態資訊：

- .sln 檔案以文字為基礎 （共用）

- .suo 檔案 （二進位、 使用者特定的解決方案選項）

如需.suo 檔案的詳細資訊，請參閱[方案使用者選項 (。Suo) 檔案](../../extensibility/internals/solution-user-options-dot-suo-file.md)。
...
每個專案的檔案會包含讀取的環境，以填入與該專案的項目階層的其他資訊。 階層的資料持續性是專案所控制。 資料不是通常儲存在.sln 檔案中上,，雖然您可以故意寫入專案資訊而非.sln 檔案如果您選擇這樣做。 如需持續性的詳細資訊，請參閱[專案持續性](../../extensibility/internals/project-persistence.md)並[開啟和儲存專案項目](../../extensibility/internals/opening-and-saving-project-items.md)。
...
`# Visual Studio 15`\
（最近） 儲存這個方案檔的 Visual Studio 主要版本。 這項資訊會控制方案圖示中的版本號碼。

`VisualStudioVersion = 15.0.26730.15`\
Visual Studio （最近） 儲存方案檔的完整版本。 如果具有相同主要版本的 Visual Studio 中的新版儲存方案檔，這個值不會更新以降低解決方案檔案中的客戶流失。

`MinimumVisualStudioVersion = 10.0.40219.1`\
可開啟此方案檔案的 Visual Studio 的最小值 （最舊） 的版本。
...
`VisualStudioVersion = 16.0.28701.123`\
Visual Studio （最近） 儲存方案檔的完整版本。 如果具有相同主要版本的 Visual Studio 中的新版儲存方案檔，以減少流失檔案中的將不會更新此值。
...
## <a name="file-body"></a>檔案內文

.Sln 檔案的主體包含數個區段，標示為`GlobalSection`，如下所示：
...
1. 環境讀取而非.sln 檔案全域區段，並處理標記的所有區段`preSolution`。 在此範例檔案中，還有一個這類陳述式：
