...
在此逐步解說中具現化的設定存放區是介面的實例，它會 <xref:Microsoft.VisualStudio.Shell.Interop.IVsWritableSettingsStore> 在呼叫時讀取和寫入下列登錄位置： **HKCU\Software\Microsoft\VisualStudio\14.0 \\ \<CollectionName> **
...
5. 在 [設計] 窗格中，按一下控制項的主要區域，然後按 [**刪除**]。
     ...
     此步驟會移除 <xref:System.Windows.Controls.Border> 元素和其中的所有專案，並只保留最上層 <xref:System.Windows.Controls.Grid> 元素。

6. 將控制項從 [**工具箱**] 拖曳 <xref:System.Windows.Controls.StackPanel> 至方格。
...
8. 針對、和的事件新增**x：Name**屬性， <xref:System.Windows.Controls.TextBox> `Click` <xref:System.Windows.Controls.Button> 如下列範例所示。
...
1. 在 [XAML] 窗格中，以滑鼠右鍵按一下 `Click` 元素的屬性 <xref:System.Windows.Controls.Button> ，然後按一下 [**流覽至事件處理常式**]。
...
3. 新增私用 `SettingsStore` 屬性，如下列範例所示。
    ...
    ```
     ...
     這個屬性會先 <xref:EnvDTE80.DTE2> 從使用者控制項的，取得介面的參考，其中包含 Automation 物件模型， <xref:System.Windows.FrameworkElement.DataContext%2A> 然後使用 DTE 來取得介面的實例 <xref:Microsoft.VisualStudio.Shell.Interop.IVsSettingsManager> 。 然後，它會使用該實例來傳回目前的使用者設定。
...
您可以使用不同的事件處理常式中的不同值來取得和設定屬性，以修改此使用者控制項，以儲存並抓取任意數目的自訂設定 `SettingsStore` 。 只要在每次呼叫時使用不同的 `propertyName` 參數 <xref:Microsoft.VisualStudio.Shell.Interop.IVsWritableSettingsStore.SetString%2A> ，這些值就不會覆寫登錄中的另一個。
