...
## <a name="the-visual-studio-package-project-template"></a>Visual Studio 封裝專案範本
 您可以在 [**新增專案**] 對話方塊中的 [ **c + +** 擴充性] 資料夾底下找到 [Visual Studio 封裝] 專案範本。

### <a name="to-create-a-vspackage-using-the-visual-studio-package-template"></a>若要使用 Visual Studio 封裝範本建立 VSPackage

1. 使用 Visual Studio 封裝範本來建立專案。

2. 選取 [**自訂編輯器**] 選項，然後按 **[下一步]**。 [**編輯器選項**] 頁面隨即出現。

3. 在 [**編輯器名稱**] 方塊中，輸入編輯器的名稱。 在 [**副檔名**] 方塊中，輸入您想要與編輯器相關聯的副檔名。 您的編輯器適用于具有此副檔名的檔案。 副檔名只會針對 Visual Studio 註冊，不適用於 Windows。 在 [**預設檔案名**] 方塊中，為您的編輯器所建立的新檔輸入預設的檔案名。
...
2. 在 [**新增**檔案] 對話方塊的 [**已安裝的範本**] 窗格中，依序選取 [檔案] 範本和您註冊的檔案類型。
