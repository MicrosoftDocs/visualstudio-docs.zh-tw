...
3. 藉由設定介面來執行編輯器 factory <xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory> 。
     ...
     如需詳細資訊，請參閱[編輯器工廠](/visualstudio/extensibility/editor-factories?view=vs-2015)。
...
6. 提供檔持續性和對外部檔案變更的回應：
    ...
    1. 若要保存檔案， <xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2> 請 <xref:Microsoft.VisualStudio.Shell.Interop.IPersistFileFormat> 在您的編輯器檔資料物件上執行和。

    2. 若要回應外部檔案變更，請 <xref:Microsoft.VisualStudio.Shell.Interop.IVsFileChangeEx> <xref:Microsoft.VisualStudio.Shell.Interop.IVsDocDataFileChangeControl> 在您的編輯器檔資料物件上執行和。

        > [!NOTE]
        > 呼叫 `QueryService` 上的 <xref:Microsoft.VisualStudio.Shell.Interop.SVsFileChangeEx> 以取得的指標 `IVsFileChangeEx` 。

7. 使用原始程式碼控制協調檔編輯事件。 請遵循下列步驟：

    1. 藉 `IVsQueryEditQuerySave2` 由在上呼叫來取得的指標 `QueryService` <xref:Microsoft.VisualStudio.Shell.Interop.SVsQueryEditQuerySave> 。
...
8. 啟用 [**屬性**] 視窗，以顯示在編輯器中選取之文字的屬性。 請遵循下列步驟：

    1. <xref:Microsoft.VisualStudio.Shell.Interop.ITrackSelection.OnSelectChange%2A>在每次文字選取變更時呼叫，傳入您的實作為 <xref:Microsoft.VisualStudio.Shell.Interop.ISelectionContainer> 。
...
9. 讓使用者可以在編輯器和**工具箱**之間，或在外部編輯器（例如 Microsoft Word）和 [**工具箱**] 之間拖放專案。 請遵循下列步驟：

    1. `IDropTarget`在您的編輯器上執行，以警示 IDE 您的編輯器是放置目標。

    2. 在 <xref:Microsoft.VisualStudio.Shell.Interop.IVsToolboxUser> 視圖上執行介面，讓您的編輯器可以在 [**工具箱**] 中啟用和停用專案。

    3. <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage.ResetDefaults%2A>在服務上執行並呼叫， `QueryService` <xref:Microsoft.VisualStudio.Shell.Interop.SVsToolbox> 以取得 <xref:Microsoft.VisualStudio.Shell.Interop.IVsToolbox2> 和介面的指標 <xref:Microsoft.VisualStudio.Shell.Interop.IVsToolbox3> 。
...
10. 決定您是否想要編輯器的其他選用功能。
    ...
    - 如果您想要編輯器支援 `Undo` 命令，建議的方法是使用 OLE 復原管理員模型。 或者，您可以讓編輯器直接處理此 `Undo` 命令。
...
13. 藉由執行介面，從您的編輯器公開 Automation 物件模型 `IDispatch` 。

     如需詳細資訊，請參閱 [Contributing to the Automation Model](../extensibility/internals/contributing-to-the-automation-model.md)。

## <a name="robust-programming"></a>穩固程式設計

- 當 IDE 呼叫方法時，會建立編輯器實例 <xref:Microsoft.VisualStudio.Shell.Interop.IVsEditorFactory.CreateEditorInstance%2A> 。 如果編輯器支援多個視圖，會 `CreateEditorInstance` 同時建立檔資料和檔視圖物件。 如果檔資料物件已開啟，則會將非 null `punkDocDataExisting` 值傳遞給 `IVsEditorFactory::CreateEditorInstance` 。 您的編輯器 factory 必須藉由查詢其上的適當介面，判斷現有的檔資料物件是否相容。 如需詳細資訊，請參閱[支援多個檔視圖](../extensibility/supporting-multiple-document-views.md)。
...
- 如果您決定使用就地啟用，請執行下列介面：
  ...
  > `IOleInPlaceComponent`介面是用來避免 OLE 2 功能表合併。
   ...
   您的 `IOleCommandTarget` 實作為處理命令，例如**剪**下、**複製**和**貼**上。 在執行時 `IOleCommandTarget` ，請決定您的編輯器是否需要自己的 *.vsct*檔案來定義自己的命令功能表結構，或是否可以執行所定義的標準命令 [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] 。 通常，編輯器會使用並擴充 IDE 的功能表，並定義自己的工具列。 不過，除了使用 IDE 的標準命令集之外，編輯器通常還需要定義自己的特定命令。 您的編輯器必須宣告所使用的標準命令，然後在 *.vsct*檔案中定義任何新的命令、內容功能表、最上層功能表和工具列。 如果您建立就地啟用編輯器，請 <xref:Microsoft.VisualStudio.Shell.Interop.IOleInPlaceComponent> 在 *.vsct*檔案中執行並定義編輯器的功能表和工具列，而不是使用 OLE 2 功能表合併。

- 若要防止 UI 中的功能表命令根本，您應該在發明新命令之前，先使用 IDE 中的現有命令。 共用命令定義于*SharedCmdDef. .vsct*和*ShellCmdDef. .vsct*中。 這些檔案預設會安裝在您安裝的 VisualStudioIntegration\Common\Inc 子目錄中 [!INCLUDE[vsipsdk](../extensibility/includes/vsipsdk_md.md)] 。

- `ISelectionContainer`可以同時表示單一和多重選取專案。 每個選取的物件都會實作為 `IDispatch` 物件。

- IDE 會將當做可 `IOleUndoManager` 從或存取的服務，當做 <xref:Microsoft.VisualStudio.Shell.Interop.ILocalRegistry2.CreateInstance%2A> 可透過具現化的物件 <xref:Microsoft.VisualStudio.Shell.Interop.ILocalRegistry2.CreateInstance%2A> 。 您的編輯器會 `IOleUndoUnit` 針對每個 `Undo` 動作執行介面。
