...
- Win32： HBITMAP
  ...
  **影像名字**
  ...
  影像標記（簡稱為名字）是 GUID/識別碼組，可唯一識別影像庫中的影像資產或影像清單資產。
...
1. 將 UI 中所有 \<影像 > 元素取代為 \<CrispImage > 專案。

2. 將所有來源屬性變更為 [名字] 屬性。

    - 如果影像永遠不會變更，而且您使用**KnownMonikers**，則會以靜態方式將該屬性系結至**KnownMoniker**。 （請參閱上面的範例）。
...
## <a name="how-do-i-use-image-monikers-in-a-new-tool-window"></a>如何? 在新的工具視窗中使用影像名字項嗎？
 已更新 Visual Studio 2015 的 VSIX 封裝專案範本。 若要建立新的工具視窗，請在 VSIX 專案上按一下滑鼠右鍵，**然後選取 [新增 > ** **新專案**] （**Ctrl**+**Shift**+**a**）。 在專案語言的 [擴充性] 節點底下，選取 [**自訂工具視窗]** ，指定工具視窗的名稱，然後按 [**新增**] 按鈕。

 這些是在工具視窗中使用名字標記的關鍵位置。 請遵循下列各項的指示：

1. 當索引標籤夠小（也用於**Ctrl** **+索引**標籤視窗切換器）時，[工具視窗] 索引標籤。
...
3. 當索引標籤夠小（也用於**Ctrl** **+索引**標籤視窗切換器）時，[工具視窗] 索引標籤。

   1. 在衍生自**ToolWindowPane**類型的類別的函式中，移除這些程式程式碼（如果存在的話）：
   ...
   2. 請參閱「如何? 在新工具視窗中使用影像的名字」的步驟 #1。 一節。

4. 用來開啟工具視窗的命令。

   - 請參閱「如何? 在新工具視窗中使用影像的名字」的步驟 #2。 一節。
...
3. 變更圖示的原始 VSHPROPIDs 的執行方式，以便在擴充點要求時，建立圖示的「舊版」版本。 **IVsImageService2**提供取得這些圖示所需的功能
   ...
   **如何? 使用 CPS 中的映射名字標記嗎？**
   ...
   設定 CPS （一般專案系統）中的自訂影像，可以手動方式或透過專案系統擴充性 SDK 隨附的專案範本來完成。
...
2. 如果您只使用**KnownMonikers**，請執行下列動作：
   ...
   - 新增 < 匯入資訊清單 = "$ （ManifestFolder）\\< 相對安裝目錄路徑至 *\>\Microsoft.VisualStudio.ImageCatalog.imagemanifest"/\*> 在 \<符號 > 區段的頂端。
...
## <a name="testing-your-images"></a>測試您的映射
 您可以使用影像庫檢視器工具來測試您的映射資訊清單，以確定所有專案都已正確撰寫。 您可以在[Visual Studio 2015 SDK](visual-studio-sdk.md)中找到此工具。 此工具和其他專案的檔可在[這裡](https://docs.microsoft.com/visualstudio/extensibility/internals/vssdk-utilities?view=vs-2015&redirectedfrom=MSDN)找到。
 ...
 **ManifestFromResources**
 ...
 Manifest from Resources 工具會取得影像資源（PNG 或 XAML）的清單，並產生映射資訊清單檔案，以便搭配影像服務使用這些映射。
