...
7.  AppliesTo： 指定指定適用的 Visual Studio 專案類型都可以在 [參考管理員] 中的 Sdk。 會辨識九個值： WindowsAppContainer、 VisualC、 VB、 CSharp、 WindowsXAML、 JavaScript、 受管理，與原生。 SDK 作者可以使用和 ("+')，或 ("&#124;")，不 ("！")若要指定完全 SDK 適用於專案類型的範圍運算子。  
