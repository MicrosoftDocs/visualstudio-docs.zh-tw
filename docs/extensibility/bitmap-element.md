...
|href|點陣圖的路徑。 需要 resID 或 href 屬性。<br /><br /> 指定的映像檔，內嵌在產生的二進位檔中搜尋 include 路徑。  在命令資料表合併期間複製映像，並不需要任何額外的資源查閱或負載。  UsedList 屬性不存在，在區域中的所有映像可用。 **注意︰** 映像可能會提供包含數種格式之一 *.bmp*， *.png*，和 *.gif*。  舊版編譯器不支援部分的透明度的 alpha 資訊的 32 位元點陣圖影像。 這些版本的因應措施是使用 *.png*格式。|  
