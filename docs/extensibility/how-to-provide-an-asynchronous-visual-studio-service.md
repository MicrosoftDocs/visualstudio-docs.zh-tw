...
1. 建立 VSIX**專案（[** 檔案] [  >  **新**  >  **專案**] [  >  **Visual c #**  >  **Extensiblity**  >  **VSIX 專案**]）。 將專案命名為**TestAsync**。

2. 將 VSPackage 新增至專案。 選取**方案總管**中的專案節點，然後按一下 [**加入**  >  **新專案**] [  >  **Visual c # 專案**] [擴充性]  >  **Extensibility**  >  **Visual Studio 封裝**]。 將此檔案命名為*TestAsyncPackage.cs*。
...
## <a name="register-a-service"></a>註冊服務
 若要註冊服務，請將新增 <xref:Microsoft.VisualStudio.Shell.ProvideServiceAttribute> 至提供服務的封裝。 不同于註冊同步服務，您必須確定封裝和服務都支援非同步載入：

- 您必須將**AllowsBackgroundLoading = true**欄位新增至， <xref:Microsoft.VisualStudio.Shell.PackageRegistrationAttribute> 以確保可以非同步初始化封裝，以取得有關 PackageRegistrationAttribute 的詳細資訊，請參閱[註冊及取消註冊 vspackage](../extensibility/registering-and-unregistering-vspackages.md)。

- 您必須將**IsAsyncQueryable = true**欄位加入至， <xref:Microsoft.VisualStudio.Shell.ProvideServiceAttribute> 以確保服務實例可以非同步初始化。

  以下是 `AsyncPackage` 具有非同步服務註冊的範例：
...
1. 這會在初始化運算式中顯示，但您可以在任何想要使用服務的地方取得服務。
    ...
    ```
     ...
     別忘了變更 `userpath` 為在您的電腦上有意義的檔案名和路徑！
...
1. 將功能表命令加入至您的專案。 （在 [**方案總管**中，選取專案節點，按一下滑鼠右鍵，然後選取 [**新增**  >  ]**新增專案**  > 擴充性**Extensibility**  > **自訂命令**）。將命令檔命名為*TestAsyncCommand.cs*。

2. 自訂命令範本會將方法重新加入 `Initialize()` 至*TestAsyncPackage.cs*檔案，以便初始化命令。 在 `Initialize()` 方法中，複製初始化命令的行。 它看起來應該如下所示：

    ```csharp
    TestAsyncCommand.Initialize(this);
    ```

     將這一行移至 `InitializeAsync()` *AsyncPackageForService.cs*檔案中的方法。 因為這是在非同步初始化中，所以您必須先切換到主執行緒，才能使用初始化命令 <xref:Microsoft.VisualStudio.Threading.JoinableTaskFactory.SwitchToMainThreadAsync%2A> 。 目前看起來應該如下所示：
...
6. 新增名為的非同步方法 `UseTextWriterAsync()` ，以取得服務並使用其方法：
