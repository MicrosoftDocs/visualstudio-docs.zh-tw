...
5. 在根 `Project` 元素中，加入專案， `PropertyGroup` 其中包含 `UICulture` 符合您預設語言的元素。
...
7. 在 `EmbeddedResource` 呼叫*VSPackage*的元素中，將專案取代為設定為的元素 `ManifestResourceName` `LogicalName` ，如下所示 `VSPackage.en-US.Resources` ：
...
8. 針對每個當地語系化的語言，複製的專案 `EmbeddedResource` `VsPackage.en-US` ，並將複本的**Include**屬性和**LogicalName**專案設為目標地區設定。

9. 針對每個當地語系化 `VSCTCompile` 的專案，加入 `ResourceName` 指向的元素 `Menus.ctmenu` ，如下列範例所示：
