...
## <a name="troubleshoot-office-solutions-by-using-the-event-viewer"></a>使用事件檢視器針對 Office 方案進行疑難排解
...
## <a name="change-the-assembly-name-causes-conflicts"></a>變更元件名稱導致衝突
 如果您在部署方案之後，變更 [**專案設計**工具] 之 [**應用程式**] 頁面中的 [**元件名稱**] 值，發佈工具就會修改安裝套件，使其具有一個*Setup.exe*檔案和兩個部署資訊清單。 如果您部署兩個資訊清單檔案，就可能發生下列狀況：

- 如果使用者兩個版本都安裝，應用程式就會載入兩個 VSTO 增益集。

- 如果先安裝了 VSTO 增益集才變更組件名稱，使用者永遠都收不到更新。

  若要避免這些狀況，請不要在部署方案之後變更解決方案的 [**元件名稱**] 值。

## <a name="check-for-updates-takes-a-long-time"></a>檢查更新是否需要很長的時間
 適用于 Office runtime 的 Visual Studio 2010 工具提供一個登錄專案，系統管理員可以使用它來設定下載資訊清單和解決方案的超時值。
...
## <a name="cant-update-or-publish-to-a-network-file-share"></a>無法更新或發佈至網路檔案共用
 如果在發行更新時，在進程中鎖定解決方案的*Setup.exe*檔案，則位於網路檔案共用上的 Office 解決方案可能會在更新期間顯示誤導訊息。 訊息內容可能是：「無法將 'setup.exe' 加入 Web。 Web 中已有檔案 'setup.exe'。」
...
## <a name="prerequisites-for-microsoft-office-arent-installed"></a>未安裝 Microsoft Office 的必要條件
 您可以將 .NET Framework、 [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]和 Office 主要 interop 組件加入安裝套件當做與 Office 解決方案一起部署的必要條件。 如需如何安裝主要 interop 元件的詳細資訊，請參閱[設定電腦以開發 office 方案](../vsto/configuring-a-computer-to-develop-office-solutions.md)和[如何：安裝 Office 主要 interop 元件](../vsto/how-to-install-office-primary-interop-assemblies.md)。

## <a name="publish-using-localhost-can-cause-installation-problems"></a>使用 Localhost 發行可能會導致安裝問題
 當您使用 `http://localhost` 做為檔層級解決方案的發佈或安裝位置時，[**發行嚮導]** 不會將字串轉換成實際的電腦名稱稱。 在這種情況下，解決方案必須安裝在開發電腦上。 為使得部署的方案在開發電腦上使用 IIS，所有的 HTTP/HTTPS/FTP 位置請使用完整格式名稱，不要使用 localhost。
...
3. 將**元件版本**的修訂編號（第三個欄位）設定為萬用字元（ \* ）。 例如，"1.0. *"。  然後選擇 [**確定]** 按鈕。
 ...
 [!NOTE]
> 從 Visual Studio 2017 開始，如果您嘗試在元件版本中使用萬用字元，將會發生組建錯誤。  這是因為元件版本中的萬用字元會中斷 MSBuild 具決定性的功能。 系統會指示您移除元件版本中的萬用字元，或停用確定性。  若要深入瞭解具決定性的功能，請參閱：[一般 MSBuild 專案屬性](../msbuild/common-msbuild-project-properties.md)和[自訂您的組建](../msbuild/customize-your-build.md)

## <a name="installation-fails-when-the-uri-has-characters-that-arent-us-ascii"></a>當 URI 包含非 ASCII 字元時，安裝會失敗
 當您發佈 Office 解決方案到 HTTP/HTTPS/FTP 位置時，路徑不能有不是 US-ASCII 的任何 Unicode 字元。 這種字元會造成安裝程式的不一致行為。 安裝路徑請使用 US-ASCII 字元。

## <a name="prompt-to-manually-uninstall-appears-when-you-publish-and-install-a-solution-on-the-development-computer"></a>當您在開發電腦上發佈和安裝解決方案時，出現手動卸載的提示
...
## <a name="uncaught-exception-or-method-not-found-error-when-you-install-a-solution"></a>當您安裝解決方案時，發現無法攔截的例外狀況或找不到方法的錯誤
 當您開啟部署資訊清單（ *vsto*檔案）、office 應用程式、檔或活頁簿來安裝 office 方案時，可能會出現下列狀況的錯誤訊息：
...
## <a name="manifest-registry-keys-for-add-ins-change-after-an-installshield-limited-edition-project-is-built"></a>建立 InstallShield 限量版專案之後，增益集的資訊清單登錄機碼會變更
 當您建立 InstallShield 限量版專案時，屬於 VSTO 增益集安裝程式一部分的資訊清單登錄機碼有時會從*vsto*變更為 *.dll。*
...
## <a name="the-clickonce-installer-for-your-office-solution-doesnt-install-the-primary-interop-assemblies"></a>Office 方案的 ClickOnce 安裝程式不會安裝主要 interop 元件
 ...
 如果安裝程式未正確安裝 Pia，請從安裝目錄執行名為*o2007pia.msi*的安裝程式檔案，以手動方式安裝。

## <a name="reinstall-office-solutions-causes-an-argument-out-of-range-exception"></a>重新安裝 Office 方案導致引數超出範圍例外狀況
 當您重新安裝 Office 解決方案時，可能會出現 <xref:System.ArgumentOutOfRangeException> 例外狀況及下列錯誤訊息：指定的引數超出有效值的範圍。

 如果安裝位置的 URL 大小寫不同，就會發生這種情況。 例如，如果您第一次安裝 Office 方案， `http://fabrikam.com/ExcelSolution.vsto` 然後第二次使用，就會出現此錯誤 `http://fabrikam.com/excelsolution.vsto` 。

 若要防止訊息出現，請在安裝 Office 解決方案時使用相同的大小寫。

## <a name="cant-install-a-clickonce-solution-by-opening-the-deployment-manifest-from-the-web"></a>無法從 web 開啟部署資訊清單以安裝 ClickOnce 解決方案
 使用者可以從 Web 開啟部署資訊清單，藉以安裝 Office 解決方案。 不過，某些 Internet Information Services （IIS）安裝會封鎖*vsto*副檔名。 您必須先在 IIS 中定義 MIME 類型，才能使用它來部署 Office 方案。

 如需如何在 IIS 7 中定義 MIME 類型的相關資訊，請參閱[加入 Mime 類型（IIS7）](https://technet.microsoft.com/library/cc725608(WS.10).aspx)。
