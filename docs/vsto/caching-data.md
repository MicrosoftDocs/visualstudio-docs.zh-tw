...
- 若要建置方案時，請將物件加入至資料快取中，將套用<xref:Microsoft.VisualStudio.Tools.Applications.Runtime.CachedAttribute>屬性加入物件常值。 如需詳細資訊，請參閱[如何：離線或在伺服器上快取資料以用於](../vsto/how-to-cache-data-for-use-offline-or-on-a-server.md)。

- 若要以程式設計方式加入物件的資料快取在執行階段，請使用`StartCaching`方法的許多項目，例如`ThisDocument`或`ThisWorkbook`類別。 如需詳細資訊，請參閱[如何：以程式設計方式快取 Office 文件中的資料來源](../vsto/how-to-programmatically-cache-a-data-source-in-an-office-document.md)。
