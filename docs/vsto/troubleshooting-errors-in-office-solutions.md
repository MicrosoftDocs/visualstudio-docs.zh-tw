...
### <a name="some-events-of-office-objects-are-not-accessible-when-using-c"></a>使用 C\# 時，無法存取某些 Office 物件的事件
...
- 視覺C#效果：無法內嵌「interop 類型 ' DocumentClass '。 請改用適當的介面。」
...
### <a name="references-to-office-classes-are-not-recognized"></a>無法辨識 Office 類別的參考
 某些類別名稱（例如應用程式）位於多個命名空間中，例如 <xref:Microsoft.Office.Interop.Word> 和 <xref:System.Windows.Forms>。 因此，專案範本頂端的匯**入**/**using**語句會包含簡短的合格常數，例如：

 [!code-csharp[Trin_VstcoreTroubleshootingWord#2](../vsto/codesnippet/CSharp/Trin_VstcoreTroubleshootingWordCS/ThisDocument.cs#2)]
 [!code-vb[Trin_VstcoreTroubleshootingWord#2](../vsto/codesnippet/VisualBasic/Trin_VstcoreTroubleshootingWordVB/ThisDocument.vb#2)]

 **使用**語句/匯**入**的用法，需要您以 Word 或 Excel 辨識符號來區分 Office 類別的參考，例如：
...
### <a name="threads-are-not-stopped-correctly-after-debugging"></a>在調試之後，執行緒未正確停止
 Visual Studio 中的 Office 專案會遵循執行緒命名慣例，該慣例可讓偵錯工具正確關閉程式。 如果您在方案中建立執行緒，則應在每個執行緒名稱加上前置詞 VSTA_，確保停止偵錯時能正確處理這些執行緒。 例如，您可以設定等候網路事件**VSTA_NetworkListener**之執行緒的 `Name` 屬性。
