...
3. 在 [**隱私權選項**] 索引標籤上，清除 [**儲存時移除檔案屬性中的個人資訊**] 核取方塊。

4. 儲存活頁簿並關閉 Excel。

### <a name="cannot-open-a-project-after-migration"></a>無法在遷移後開啟專案
...
### <a name="errors-in-upgraded-office-2003-document-level-projects-that-contain-windows-forms-controls"></a>升級的 Office 2003 檔層級專案中包含 Windows Forms 控制項的錯誤
 如果您升級 Microsoft Office 2003 檔層級專案，而且檔包含 Windows Forms 控制項，則升級的專案可能會發生編譯或執行階段錯誤。 若要避免這個問題，請先在開發電腦上安裝 Visual Studio 2005 Tools for Office Second Edition Runtime，再將專案升級。 您可以從下列 Microsoft 下載中心，取得這個執行階段版本的可轉散發套件： [Microsoft Visual Studio 2005 Tools for Office Second Edition Runtime (VSTO 2005 SE) (x86)](https://www.microsoft.com/download/details.aspx?id=2392)。
...
### <a name="controls-appear-as-black-rectangles-on-the-document-or-worksheet"></a>控制項在檔或工作表上顯示為黑色矩形
 如果您將文件或工作表上的控制項設為群組，Visual Studio 就不再能夠辨認此控制項。 群組控制項無法在 [**屬性**] 視窗中存取，而且會在檔或工作表上顯示為黑色矩形。 您必須將這些控制項取消群組才能還原其功能。

### <a name="controls-on-a-word-template-are-not-visible-in-visual-studio"></a>在 Word 範本上的控制項不會顯示在 Visual Studio
 如果您在 [Visual Studio 設計工具] 中開啟 Word 範本，就有可能看不到該範本上未與文字排列的控制項。 這是因為 Visual Studio 會以**一般**視圖開啟 Word 範本。 若要查看控制項，請按一下 [**視圖**] 功能表，指向**Microsoft Office Word view** ]，然後按一下 [**列印版面**配置]。

### <a name="insert-clip-art-command-does-nothing-in-the-visual-studio-designer"></a>[插入美工圖案] 命令不會在 Visual Studio 設計工具中執行任何工作
 當 Excel 或 Word 在 Visual Studio 設計工具中開啟時，按一下功能區中 [**圖例**] 索引標籤上的 [**美工圖案**] 按鈕並不會開啟 [**剪切美工圖案**] 工作窗格。 若要新增美工圖案，您必須在 Visual Studio 以外的主要專案資料夾（而不是*\bin*資料夾中的複本）中開啟活頁簿或檔的複本，並新增美工圖案，然後儲存活頁簿或檔。
...
### <a name="cannot-reference-office-pia-classes-in-projects-that-target-the-net_v40_short-or-the-net_v45"></a>無法在以或為目標的專案中參考 Office PIA 類別 [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)][!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)]
...
- Visual Basic：當使用不是 PIA 模式來連結其元件時，不允許參考類別 ' DocumentClass '。」
...
### <a name="references-to-office-classes-are-not-recognized"></a>無法辨識 Office 類別的參考
 某些類別名稱（例如應用程式）位於多個命名空間，例如 <xref:Microsoft.Office.Interop.Word> 和 <xref:System.Windows.Forms> 。 基於這個理由，專案範本頂端的匯**入** / **using**語句會包含簡短的合格常數，例如：

 [!code-csharp[Trin_VstcoreTroubleshootingWord#2](../vsto/codesnippet/CSharp/Trin_VstcoreTroubleshootingWordCS/ThisDocument.cs#2)]
 [!code-vb[Trin_VstcoreTroubleshootingWord#2](../vsto/codesnippet/VisualBasic/Trin_VstcoreTroubleshootingWordVB/ThisDocument.vb#2)]

 匯**入** / **using**語句的這種用法需要您區分 Office 類別的參考與 Word 或 Excel 辨識符號，例如：
 ...
 即使您已匯入 Word 或 Excel 命名空間，並可存取其中的所有類別，您還是必須以 Word 或 Excel 完整限定所有類型，以移除不明確的命名空間。
...
### <a name="compiler-errors-occur-after-a-namedrange-control-is-deleted"></a>刪除 NamedRange 控制項之後發生編譯器錯誤
...
## <a name="debug-projects"></a><a name="debugging"></a>Debug 專案
 當您偵錯 Office 專案時，可能會遇到下列錯誤。

### <a name="prompt-to-uninstall-appears-when-you-publish-and-install-a-solution-on-the-development-computer"></a>當您在開發電腦上發佈和安裝解決方案時，會出現 [提示卸載]
...
### <a name="document-level-projects-created-at-unc-network-locations-do-not-run-from-visual-studio"></a>在 UNC 網路位置建立的檔層級專案不會從 Visual Studio 執行
...
### <a name="threads-are-not-stopped-correctly-after-debugging"></a>在調試之後，執行緒未正確停止
 Visual Studio 中的 Office 專案會遵循執行緒命名慣例，該慣例可讓偵錯工具正確關閉程式。 如果您在方案中建立執行緒，則應在每個執行緒名稱加上前置詞 VSTA_，確保停止偵錯時能正確處理這些執行緒。 例如，您可以設定 `Name` 執行緒的屬性，以等候網路事件**VSTA_NetworkListener**。
...
### <a name="error-when-stopping-the-debugger-in-a-document-level-project-after-using-edit-and-continue"></a>使用 [編輯後繼續] 之後，停止檔層級專案中的偵錯工具時發生錯誤
 當專案處於中斷模式時，如果您使用 [**編輯**後**繼續**] 來變更 Excel 或 Word 檔層級專案中的程式碼，當您後續停止偵錯工具時，您可能會看到含有下列錯誤訊息的對話方塊。

 「在這個處理序的目前狀態終止它可能會造成不良後果，包括資料遺失和系統不穩定。」

 無論您在對話方塊中按一下 **[是]** 或 [**否**]，Visual Studio 都會終止 Excel 或 Word 進程，並停止偵錯工具。 若要停止專案偵錯而不顯示此對話方塊，請直接結束 Excel 或 Word，而不是在 Visual Studio 中停止偵錯工具。
