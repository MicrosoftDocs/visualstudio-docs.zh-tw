...
1.  將下面程式碼加入 `ThisDocument` 類別的 `ThisDocument_Startup` 方法。 此程式碼會取得 XML 字串，從**employees.xml**檔案，將 XML 字串新增至新的自訂 XML 組件，並將內容控制項繫結至自訂 XML 組件中的項目。  
