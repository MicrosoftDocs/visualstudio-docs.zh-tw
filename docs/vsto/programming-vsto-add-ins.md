...
# <a name="program-vsto-add-ins"></a>程式 VSTO 增益集
 ...
 如需 VSTO 增益集和其他類型方案的一般資訊，您可以使用 Visual Studio 中的 Office 程式開發工具來建立此功能，請參閱[Office 方案開發&#40; &#41;總覽](../vsto/office-solutions-development-overview-vsto.md)。

## <a name="use-the-thisaddin-class"></a>使用 Thisaddin.vb 類別
 您可以在 `ThisAddIn` 類別中開始撰寫 VSTO 增益集程式碼。 Visual Studio 會自動在您 VSTO 增益集專案的*thisaddin.vb* [!INCLUDE[vbprvb](../sharepoint/includes/vbprvb-md.md)]中 *，或* C#在程式碼檔案中產生此類別。 當 Microsoft Office 應用程式載入您的 VSTO 增益集時， [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] 會自動為您具現化這個類別。
...
|Microsoft Office Word|<xref:Microsoft.Office.Interop.Word.Application>|
 ...
 下列程式碼範例顯示如何使用`Application`欄位，在 Microsoft Office Excel 的 VSTO 增益集中建立新的活頁簿。 這個範例適合從 `ThisAddIn` 類別執行。
...
```
 ...
 若要從 `ThisAddIn` 類別外執行相同的動作，請使用 `Globals` 物件存取 `ThisAddIn` 類別。 如需物件的`Globals`詳細資訊，請參閱[全域存取 Office 專案中的物件](../vsto/global-access-to-objects-in-office-projects.md)。
...
### <a name="AccessingDocuments"></a>在 Office 應用程式啟動時存取檔
 當您啟動 [!INCLUDE[office14_long](../vsto/includes/office14-long-md.md)] 應用程式時，並非所有應用程式都會自動開啟文件；而當您啟動 [!INCLUDE[Office_15_short](../vsto/includes/office-15-short-md.md)] 應用程式時，所有應用程式都不會開啟文件。 因此，如果程式碼需要開啟`ThisAdd-In_Startup`檔，請勿在事件處理常式中加入程式碼。 相反地，請將程式碼加入 Office 應用程式在使用者建立或開啟文件時所引發的事件。 如此可確保程式碼對文件執行作業之前，該文件已處於開啟狀態。
...
### <a name="thisaddin-members-to-use-for-other-tasks"></a>要用於其他工作的 Thisaddin.vb 成員
...
### <a name="understand-the-design-of-the-thisaddin-class"></a>瞭解 Thisaddin.vb 類別的設計
...
## <a name="customize-the-user-interface-of-microsoft-office-applications"></a>自訂 Microsoft Office 應用程式的使用者介面
 ...
 此外，您也可以藉由產生可擴充文件和活頁簿行為的主項目，來修改 Word 文件和 Excel 活頁簿的 UI。 這可讓您將 Managed 控制項加入文件和工作表。 如需詳細資訊，請參閱[在 VSTO 增益集的執行時間中擴充 Word 檔和 Excel 活頁簿](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md)。

## <a name="call-code-in-vsto-add-ins-from-other-solutions"></a>從其他方案呼叫 VSTO 增益集的程式碼
 您可以將 VSTO 增益集中的物件公開給其他方案 (包括其他 Office 方案)。 如果您想要讓其他方案也能使用 VSTO 增益集提供的服務，這就很有用。 例如，如果您有 Microsoft Office Excel 的 VSTO 增益集，它會針對來自 web 服務的財務資料執行計算，則其他方案可以在執行時間呼叫 Excel VSTO 增益集來執行這些計算。
