...
## <a name="persist-host-controls-in-the-document"></a>將主控制項保存在檔中
...
### <a name="re-create-dynamic-host-controls-when-documents-are-opened"></a>在檔開啟時重新建立動態主控制項

每當使用者開啟文件時，您都可以重新建立動態主控制項來取代現有的原生控制項。 在文件開啟時以這種方式建立主控制項，便會模擬使用者可能預期的經驗。

若要重新建立 Word 的主<xref:Microsoft.Office.Tools.Excel.NamedRange>控制項，或 Excel 的或<xref:Microsoft.Office.Tools.Excel.ListObject>主控制項，請使用 <xref:Microsoft.Office.Tools.Excel.ControlCollection?displayProperty=fullName> `Add` \<控制項類別 > 或<xref:Microsoft.Office.Tools.Word.ControlCollection?displayProperty=fullName>物件的方法。 使用具有原生 Office 物件參數的方法。
...
<xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName>若要重新建立主控制項，您必須先刪除原生<xref:Microsoft.Office.Interop.Excel.Chart?displayProperty=fullName>，然後使用<xref:Microsoft.Office.Tools.Excel.ControlCollection.AddChart%2A>方法重新建立<xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName> 。 <xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName> <xref:Microsoft.Office.Interop.Excel.Chart?displayProperty=fullName>沒有控制項類別 > 方法，可讓您根據現有的建立新的。 `Add` \<

如果您未先刪除原生<xref:Microsoft.Office.Interop.Excel.Chart>，則當您重新建立時，將會建立第二個重複的<xref:Microsoft.Office.Tools.Excel.Chart?displayProperty=fullName>圖表。
...
### <a name="re-create-windows-forms-controls-when-documents-are-opened"></a>在檔開啟時重新建立 Windows Forms 控制項
...
當您使用 VSTO 增益集將動態 Windows Forms 控制項加入檔時，可以防止控制項的 ActiveX 包裝函式在下一次以下列方式開啟時，出現在檔中。

#### <a name="remove-activex-wrappers-when-the-document-is-opened"></a>在檔開啟時移除 ActiveX 包裝函式
...
雖然 `GetVstoObject` 方法主要是用來在執行階段產生新的主項目，但這個方法也會在首次針對特定文件呼叫時，清除文件中所有的 ActiveX 包裝函式。 如需如何使用`GetVstoObject`方法的詳細資訊，請參閱[在 VSTO 增益集的執行時間中擴充 Word 檔和 Excel 活頁簿](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md)。

如果您的 vsto 增益集會在檔開啟時建立動態控制項，則您的 vsto 增益集會在建立`GetVstoObject`控制項的進程中，呼叫方法。 您不需要將個別呼叫加入 `GetVstoObject` 方法中來移除此案例中的 ActiveX 包裝函式。
