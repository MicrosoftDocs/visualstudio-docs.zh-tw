...
### <a name="microsoft-office-version"></a>Microsoft Office 版本
 ...
 若要載入註冊于**HKEY_LOCAL_MACHINE**底下的 VSTO 增益集, 電腦必須安裝更新套件976477。 如需詳細資訊，請參閱 [http://go.microsoft.com/fwlink/?LinkId=184923](http://go.microsoft.com/fwlink/?LinkId=184923)。
 ...
 *根*\Software\Microsoft\Office\Outlook\FormRegions\\*訊息類別*
...
## <a name="LoadBehavior"></a>LoadBehavior 值
 \\*根*\Software\Microsoft\Office\\*應用程式名稱*\Addins*增益集識別碼*索引鍵底下的**LoadBehavior**專案包含值的位元組合, 這些值會指定的執行時間行為。VSTO 增益集。 最低順序位元 (值 0 和 1) 表示 VSTO 增益集目前處於卸載或載入狀態。 其他位元則表示應用程式嘗試載入 VSTO 增益集的時間。

 一般來說, 當 VSTO 增益集安裝在終端使用者電腦上時, **LoadBehavior**專案會設定為0、3或 16 (十進位)。 根據預設，當您建置或發行 VSTO 增益集時，Visual Studio 會將 VSTO 增益集的 **LoadBehavior** 項目設定為 3。
...
|值 (十進位)|VSTO 增益集狀態|VSTO 增益集載入行為|說明|
|--------------------------|-------------------------|--------------------------------|-----------------|
