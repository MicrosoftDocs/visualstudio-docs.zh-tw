...
>
>如果安裝程式是以目前的使用者為目標，則不需要安裝到 WOW6432Node，因為 HKEY_CURRENT_USER 的 \Software 路徑是共用的。
>
...
|**友好**|REG_SZ|必要。 這是 Microsoft Office 應用程式的 [COM 增益集] **** 對話方塊中，所顯示之 VSTO 增益集的描述性名稱。 預設值為 VSTO 增益集 ID。|
 ...
 *根*\Software\Microsoft\Office **LoadBehavior** \\ *應用程式名稱*\Addins \\ *增益集識別碼*索引鍵下的 LoadBehavior 專案包含值的位元組合，這些值會指定 VSTO 增益集的執行時間行為。 最低順序位元 (值 0 和 1) 表示 VSTO 增益集目前處於卸載或載入狀態。 其他位元則表示應用程式嘗試載入 VSTO 增益集的時間。
