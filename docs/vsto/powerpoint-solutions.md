...
# <a name="powerpoint-solutions"></a>PowerPoint 方案
 ...
 如需 VSTO 增益集的詳細資訊，請參閱 vsto 增益集程式[設計入門](getting-started-programming-vsto-add-ins.md)和[vsto 增益集的架構](architecture-of-vsto-add-ins.md)。如果您是使用 Microsoft Office 進行程式設計的新手，請參閱[Visual Studio &#40; &#41;中的 Office 程式開發入門](getting-started-office-development-in-visual-studio.md)。
...
- 新增或變更投影片上的圖案。
  ...
  若要從 VSTO 增益集存取 PowerPoint 物件模型，請使用專案中 `ThisAddIn` 類別的 [`Application`] 欄位。 [`Application`] 欄位會傳回代表目前 PowerPoint 實例的[應用程式](/previous-versions/office/developer/office-2010/ff764034(v=office.14))物件。 如需詳細資訊，請參閱[VSTO 增益集程式](programming-vsto-add-ins.md)設計。

  呼叫 PowerPoint 物件模型時，您使用的類型是由 PowerPoint 的主要 Interop 組件所提供。 主要 Interop 組件的作用，如同 VSTO 增益集中 Managed 程式碼與 PowerPoint 中 COM 物件模型之間的橋樑。 PowerPoint 主要 interop 元件中的所有類型都定義于[Microsoft.](/previous-versions/office/developer/office-2010/ff763170(v=office.14)) 如需主要 interop 元件的詳細資訊，請參閱[Office 方案&#40;開發&#41;總覽 VSTO](office-solutions-development-overview-vsto.md)和[Office 主要 interop 元件](office-primary-interop-assemblies.md)。
...
|將自訂群組新增至功能區上的內建索引標籤。|[如何：自訂內建索引標籤](how-to-customize-a-built-in-tab.md)|
 ...
 如需自訂 PowerPoint 和其他 Microsoft Office 應用程式之 UI 的詳細資訊，請參閱[OFFICE UI 自訂](office-ui-customization.md)。
