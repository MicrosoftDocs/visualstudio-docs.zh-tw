---
title: "逐步解說： 變更快取中的伺服器上的活頁簿的資料 |Microsoft 文件"
ms.custom: 
ms.date: 02/02/2017
ms.reviewer: 
ms.suite: 
...
---
# <a name="walkthrough-changing-cached-data-in-a-workbook-on-a-server"></a>逐步解說：變更伺服器上的活頁簿快取資料
  本逐步解說示範如何修改 Microsoft Office Excel 活頁簿中而不啟動 Excel 使用快取的資料集<xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>類別。  
...
-   執行主控台應用程式中的程式碼，而不啟動 Excel 修改快取的資料集內的資料行。  
...
2.  在 [範本] 窗格中，依序展開**Visual C#**或**Visual Basic**，然後展開**Office**。  
  
3.  在展開**Office**節點中，選取**2010年**節點。  
...
## <a name="changing-data-in-the-cached-dataset-by-using-the-console-application"></a>變更快取的資料集使用主控台應用程式中的資料  
 使用<xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>類別中的主控台應用程式，以將資料讀入本機`AdventureWorksLTDataSet`物件，修改此資料，然後將它儲存回快取的資料集。  
...
10. 在**方案總管] 中**，以滑鼠右鍵按一下**DataWriter**專案，指向**偵錯**，然後按一下 [**開始新執行個體**。  
     ...
     快取的資料集讀入本機的資料集、 修改產品價格，在本機的資料集，以及將新的值儲存至快取的資料集時，主控台應用程式會顯示訊息。 按 enter 鍵關閉應用程式。  
...
2.  開啟 AdventureWorksReport 活頁簿中的組建資料夾**AdventureWorksReport**專案。 根據預設，組建資料夾是在下列位置：  
...
3.  確認中的值**ListPrice**資料行的第一個資料列傳回<xref:Microsoft.Office.Tools.Excel.ListObject>現在是 1574.65。  
