...
description: 瞭解如何使用 ServerDocument 類別，修改在 Microsoft Excel 活頁簿中快取的資料集，而不需啟動 Excel。
...
- 在主控台應用程式中執行程式碼，而不啟動 Excel，以修改快取資料集中的資料行。
...
- Microsoft SQL Server 的執行中實例的存取權，或已附加 AdventureWorksLT 範例資料庫的 Microsoft SQL Server Express。 您可以從 [SQL Server 範例 GitHub](https://github.com/Microsoft/sql-server-samples/releases/tag/adventureworks)存放庫下載 AdventureWorksLT 資料庫。 如需附加資料庫的詳細資訊，請參閱下列主題：
...
2. 在 [範本] 窗格中，展開 [ **Visual c #** ] 或 [ **Visual Basic**]，然後展開 [ **Office**]。
...
## <a name="change-data-in-the-cached-dataset-by-using-the-console-application"></a>使用主控台應用程式變更快取資料集中的資料
 使用 <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> 主控台應用程式中的類別，將資料讀取至本機 `AdventureWorksLTDataSet` 物件、修改此資料，然後將它儲存回快取的資料集。
...
9. 將下列程式碼新增至 `Main` 方法，在您于上一個步驟中新增的程式碼之後。 此程式碼會執行下列工作：
   ...
   - 它會變更 `ListPrice` 資料集之 product 資料表中的每個產品值。
...
10. 在 **方案總管** 中，以滑鼠右鍵按一下 **>datawriter** 專案，指向 [ **Debug**]，然後按一下 [ **開始新實例**]。
     ...
     主控台應用程式會在將快取的資料集讀取至本機資料集時顯示訊息、修改本機資料集中的產品價格，並將新的值儲存至快取的資料集。 按 **Enter** 以關閉應用程式。

## <a name="test-the-workbook"></a>測試活頁簿
 當您開啟活頁簿時，現在會顯示您對快取資料 <xref:Microsoft.Office.Tools.Excel.ListObject> 集中的資料行所做的變更 `ListPrice` 。

### <a name="to-test-the-workbook"></a>測試活頁簿

1. 如果仍然開啟，請關閉 Visual Studio 設計工具中的 AdventureWorksReport 活頁簿。

2. 開啟 AdventureWorksReport 活頁簿，該活頁簿位於 **AdventureWorksReport** 專案的 [組建] 資料夾中。 根據預設，組建資料夾會在下列其中一個位置：
...
3. 確認的第一個資料列的 **ListPrice** 資料行中的值 <xref:Microsoft.Office.Tools.Excel.ListObject> 現在是1574.65。
