...
## <a name="UsingFormRegions"></a>使用表單區功能變數名稱稱
...
## <a name="DisablingInheritance"></a>停用表單區域繼承
 ...
 如果您不要表單區域與任何衍生的訊息類別相關聯，請將表單區域的 <xref:Microsoft.Office.Tools.Outlook.FormRegionManifest.ExactMessageClass%2A> 屬性設為 **true**。 例如，如果您將連續的表單區域與`IPM.Task`產生關聯， <xref:Microsoft.Office.Tools.Outlook.FormRegionManifest.ExactMessageClass%2A>並將屬性設定為**true**，則表單區域只會附加至標準工作表單的底部。 表單區域將不會附加至任何自訂的標準工作表單版本底部。

## <a name="ClassNames"></a>瞭解類型和訊息類別名稱
...
## <a name="ReadingPane"></a>設計讀取窗格的相鄰表單區域
...
- 所有其他圖示應該是 16 x 16 像素。
  ...
  **視窗**圖示會出現在通知區域，以及顯示取代或全部取代表單區域之開啟專案的 [ **Alt** + **]** 索引標籤對話方塊中。
