...
- 將文件或範本的組件繫結至資料。 您可以將內容控制項繫結至資料庫欄位、[!INCLUDE[dnprdnshort](../sharepoint/includes/dnprdnshort-md.md)] 中的 Managed 物件、儲存在文件的 XML 元素，及其他資料來源。
  ...
  在文件層級的專案中，您可以於設計階段或執行階段，將內容控制項加入文件。 在 VSTO 增益集專案中，您可以在執行階段將內容控制項加入任何開啟的文件。 如需詳細資訊，請參閱[如何：將內容控制項新增至 Word 檔](../vsto/how-to-add-content-controls-to-word-documents.md)。

> [!NOTE]
> 您只能在以 Open XML 格式儲存的檔中使用內容控制項。 您無法在以 Word 97-2003 檔（ *.doc*）格式儲存的檔中使用內容控制項。
...
### <a name="build-block-gallery"></a>組建區塊庫
 建立區塊資源庫可讓使用者從要插入檔的*檔建立區塊*清單中選取。 文件建置組塊是一段可以多次使用的內容，例如通用的封面頁、格式化的資料表或頁首。 如需詳細資訊，請參閱 <xref:Microsoft.Office.Tools.Word.BuildingBlockGalleryContentControl> 型別。 如需建立區塊的詳細資訊，請參閱[Word 2007 中適用于開發人員的新功能](/previous-versions/office/developer/office-2007/bb266218(v=office.12))。
...
### <a name="generic-content-control"></a>一般內容控制項
 ...
 您只能在執行階段建立 <xref:Microsoft.Office.Tools.Word.ContentControl> 物件，不能在設計階段建立。 如需詳細資訊，請參閱[如何：將內容控制項新增至 Word 檔](../vsto/how-to-add-content-controls-to-word-documents.md)。
...
|這項工作：|請執行：|
|--------------------|--------------|
|取得或設定控制項中要顯示的文字。|使用**Text**屬性。 **注意：** <xref:Microsoft.Office.Tools.Word.PictureContentControl> 和 <xref:Microsoft.Office.Tools.Word.ContentControl> 類型沒有此屬性。|
...
|當使用者按一下它時，取得或設定內容控制項框線中顯示的標題。|使用 [**標題**] 屬性。|
...
## <a name="Protection"></a>使用內容控制項保護檔的元件
...
## <a name="DataBinding"></a>將資料系結至內容控制項
...
- 您可以將內容控制項系結至內嵌在檔中的 XML 片段（也稱為*自訂 xml 元件*）中的元素。

  如需將 Office 方案中的主控制項系結至資料的總覽，請參閱[將資料系結至 office 方案中的控制項](../vsto/binding-data-to-controls-in-office-solutions.md)。
...
### <a name="use-the-windows-forms-data-binding-model"></a>使用 Windows Forms 資料系結模型
 ...
 在檔層級專案中，您可以使用 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]中的 [**資料來源**] 視窗，將資料系結至內容控制項。 如需如何將資料系結內容控制項加入檔的詳細資訊，請參閱[如何：使用資料庫的資料填入檔](../vsto/how-to-populate-documents-with-data-from-a-database.md)和[如何：使用物件的資料填入檔](../vsto/how-to-populate-documents-with-data-from-objects.md)。
...
|<xref:System.Drawing.Image><br /><br /> <xref:System.Byte> 陣列|<xref:Microsoft.Office.Tools.Word.PictureContentControl>|None|
 ...
 在文件層級和 VSTO 增益集專案中，您可以使用控制項的 <xref:System.Windows.Forms.IBindableComponent.DataBindings%2A> 屬性的 <xref:System.Windows.Forms.ControlBindingsCollection.Add%2A> 方法，以程式設計方式將內容控制項繫結至資料來源。 如果您這樣做，請將字串**文字**傳入 <xref:System.Windows.Forms.ControlBindingsCollection.Add%2A> 方法的*propertyName*參數。 **Text**屬性是內容控制項的預設資料系結屬性。
...
```
 ...
 如需示範如何以更詳細的方式將內容控制項系結至自訂 XML 元件的逐步解說，請參閱[逐步解說：將內容控制項系結至自訂 xml 元件](../vsto/walkthrough-binding-content-controls-to-custom-xml-parts.md)。
...
### <a name="behavior-differences-between-design-time-and-runtime"></a>設計階段與執行時間之間的行為差異
 ...
 另請注意，許多定義內容控制項行為的屬性，在設計階段不起任何作用。 例如，如果您在設計階段將內容控制項的**LockContents**屬性設定為**True** ，您仍然可以在 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 設計工具的控制項中編輯文字。 這個屬性只會防止使用者在執行階段編輯控制項。
...
### <a name="checkbox"></a>Word 專案中的核取方塊內容控制項
 Word 2010 導入了表示核取方塊的新型別內容控制項。 不過，[!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] 不會提供對應的 CheckBoxContentControl 類型，讓您在 Office 專案中使用。 若要在 [!INCLUDE[Word_15_short](../vsto/includes/word-15-short-md.md)] 或 Word 2010 專案中建立核取方塊內容控制項，請使用 <xref:Microsoft.Office.Tools.Word.ControlCollection.AddContentControl%2A> 方法建立 <xref:Microsoft.Office.Tools.Word.ContentControl> 物件，並將 <xref:Microsoft.Office.Interop.Word.WdContentControlType.wdContentControlCheckBox> 值傳遞給方法，以指定核取方塊內容控制項。 下列程式碼範例會示範如何執行這項操作。
