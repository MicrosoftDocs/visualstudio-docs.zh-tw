...
### <a name="generic-content-control"></a>泛型內容控制項  
 ...
 您只能在執行階段建立 <xref:Microsoft.Office.Tools.Word.ContentControl> 物件，不能在設計階段建立。 如需詳細資訊，請參閱[＜How to：將內容控制項加入 Word 文件](../vsto/how-to-add-content-controls-to-word-documents.md)。  
...
### <a name="use-the-windows-forms-data-binding-model"></a>使用 Windows Form 資料繫結模型  
 ...
 在文件層級專案中，您可以使用資料繫結至內容控制項**資料來源** 視窗中的[!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]。 如需如何將資料繫結內容控制項加入文件的詳細資訊，請參閱[How to:資料庫中的資料填入文件](../vsto/how-to-populate-documents-with-data-from-a-database.md)和[How to:物件的資料填入文件](../vsto/how-to-populate-documents-with-data-from-objects.md)。  
