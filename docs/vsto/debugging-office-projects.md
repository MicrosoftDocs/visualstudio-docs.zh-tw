...
# <a name="debug-office-projects"></a>Debug Office 專案
  您可以使用與偵錯其他 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 專案時所使用的相同 Microsoft [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 工具，來偵錯 Office 專案。 當您偵錯 Office 專案時，也可以使用[!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 偵錯工具功能，例如可以插入中斷點，以及在 [區域變數] 視窗中檢視變數。 如需 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 偵錯工具的詳細資訊，請參閱[Visual Studio 中的 Debug](../debugger/debugger-feature-tour.md)。
...
## <a name="start-and-stop-the-debugger"></a>啟動和停止偵錯工具
 您可以開始對 Office 專案進行偵錯工具，就像開始進行其他 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 專案的程式調試一樣。例如，您可以按**F5**鍵。 當您開始偵測 VSTO 增益集專案時，會啟動目標 Office 應用程式的新進程，並載入 VSTO 增益集。
...
## <a name="debug-word-document-projects-based-on-the-word-97-2003-document-doc-format"></a>根據 Word 97-2003 檔（* .doc）格式的檔專案進行 Debug
 若要根據 Word 97-2003 檔（ */* .doc *）格式來 Debug word 檔專案，您必須將專案資料夾新增至 [信任的資料夾] 清單。 如需如何執行此動作的詳細資訊，請參閱[授與信任給檔](../vsto/granting-trust-to-documents.md)。

## <a name="debug-disabled-add-ins"></a>Debug 停用的增益集
 Microsoft Office 應用程式可以停用無法如預期般運作的 VSTO 增益集。 Microsoft Office 應用程式會停用 VSTO 增益集，以防止在每次應用程式啟動時載入有問題的程式碼。 不過，在進行一般偵錯時，也很容易會導致未預期的行為。 如需如何重新啟用 VSTO 增益集的詳細資訊，請參閱[如何：重新啟用已停用的 Vsto 載入](../vsto/how-to-re-enable-a-vsto-add-in-that-has-been-disabled.md)宏。
