...
ms.locfileid: "72985999"
---
# <a name="how-to-add-windows-forms-controls-to-office-documents"></a>如何：將 Windows Forms 控制項加入 Office 檔
  您可以在文件層級專案的設計階段中，將 Windows Form 控制項加入 Microsoft Office Excel 和 Microsoft Office Word 文件。 在執行時間，您可以在檔層級自訂和 VSTO 增益集中加入控制項。例如，您可以將 <xref:Microsoft.Office.Tools.Excel.Controls.ComboBox> 控制項加入工作表，讓使用者可以從選項清單中選取。
...
1. 使用具有 [加入\<*控制項類別*] > 的方法（其中*控制項類別*是您想要加入之 Windows Forms 控制項的類別名稱，例如 <xref:Microsoft.Office.Tools.Word.ControlExtensions.AddButton%2A>）。

     下列程式碼範例示範如何在 Excel 的檔層級專案中，將 <xref:Microsoft.Office.Tools.Excel.Controls.Button> 加入 `Sheet1` 的儲存格**C5** 。
...
1. 使用具有 [加入\<*控制項類別*] > 的方法（其中*控制項類別*是您想要加入之 Windows Forms 控制項的類別名稱，例如 <xref:Microsoft.Office.Tools.Word.ControlExtensions.AddButton%2A>）。
    ...
    > [!NOTE]
    > 在以 [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] 或更新版本為目標的 VSTO 增益集專案中，您必須在存取 新增 @no__ 之前，先加入*Microsoft*的參考，然後再新增至 [app-v 4.0] 或 [ *app-v 4.0* ] 元件。t_3_*控制項類別*> 方法。
