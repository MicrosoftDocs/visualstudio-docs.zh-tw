...
### <a name="creating-custom-user-controls"></a>建立自訂使用者控制項  
...
### <a name="arranging-controls-at-design-time"></a>在設計階段排列控制項  
...
### <a name="matching-control-size-to-cell-size-on-an-excel-worksheet"></a>讓控制項大小符合 Excel 工作表上的儲存格大小  
...
### <a name="layout-style-of-controls-on-a-word-document"></a>在 Word 文件上的控制項配置樣式  
 ...
 當在執行階段將控制項加入 Word 文件中時，您可以使用不同指定新控制項的配置樣式`Add` \<*控制項類別*> 方法的多載<xref:Microsoft.Office.Tools.Word.ControlCollection>類別：  
...
### <a name="controls-outside-the-main-document-body"></a>主要文件主體之外的控制項  
