...
1. 在 [檔案]**** 功能表上，依序選擇 [新增]**** 和 [專案] > ****。 如果您的整合式開發環境（IDE）設定為使用 [!INCLUDE[vbprvb](../sharepoint/includes/vbprvb-md.md)] 開發設定，請在**File** [檔案] 功能表上選擇 [**新增**  >  **專案**]。
...
2. 在 [範本] 窗格中，于您想要使用之語言的節點底下，展開 [ **Office/SharePoint**]。
...
4. 在專案範本清單中，選取 VSTO 增益集專案範本。 如需可用 VSTO 增益集專案範本的清單，請參閱[Office 專案範本總覽](../vsto/office-project-templates-overview.md)。
   ...
   > [!NOTE]
   > 當您選取 [ **Office 增益集**] 節點時，如果看不到專案範本，請確定已在對話方塊頂端的下拉式方塊中選取 [ **.NET Framework 4**或更新版本]。 Office 專案範本會顯示這兩個版本的 .NET Framework。
...
4. 在專案範本清單中，選取文件層級的專案範本。 如需可用檔層級專案範本的清單，請參閱[Office 專案範本總覽](../vsto/office-project-templates-overview.md)。

   > [!NOTE]
   > 當您選取 [ **Office 增益集**] 節點時，如果看不到專案範本，請確定已選取 [ **.NET Framework 4**或更新版本]。
...
7. 如果您想要為方案建立新檔，請選取 [**建立新檔**]，如果您想要自訂現有的檔，請選取 [**複製現有檔**]。
    ...
    如果您建立新的檔，請在 [**名稱**] 方塊中指定名稱，然後使用 [**格式**] 方塊來選取檔案格式。 如需可用格式的詳細資訊，請參閱[檔層級自訂的架構](../vsto/architecture-of-document-level-customizations.md)。
   ...
   > [!NOTE]
   > 如果您要使用 [!INCLUDE[Word_15_short](../vsto/includes/word-15-short-md.md)] 專案中的現有文件，則只能使用在 [!INCLUDE[Word_15_short](../vsto/includes/word-15-short-md.md)] 中建立的文件，或必須轉換為此種文件才能使用。 同樣地，如果您要使用 Word 2010 專案中的現有文件，則只能使用在 Word 2010 中建立的文件，或必須轉換為此種文件才能使用。 如果您使用舊版 Word 中所建立的文件，文件中的特定功能將會停用。 如果您嘗試撰寫會用到這些功能的程式碼，專案可能會發生錯誤。 若要轉換檔、在或 Word 2010 中開啟，請 [!INCLUDE[Word_15_short](../vsto/includes/word-15-short-md.md)] 在**File**功能區的 [檔案] 索引標籤上，選擇 [**資訊**  >  **轉換**]。

8. 選擇 [完成]****。

9. 如有下列情況，請至 Word 的 [信任中心]，將專案資料夾及其子資料夾加入信任位置：

   - 您正在建立以*docm*檔案為基礎的 Word 檔，而且該檔包含 VBA 專案或 Windows Forms 控制項的主機。 將專案資料夾加入信任位置的清單，有助確保文件可以在設計階段如預期般運作。

   - 您正在建立以*dotx*檔案為基礎的 Word 範本專案。 您必須將專案資料夾加入信任位置的清單，才能執行和偵錯專案。

     如需如何將檔新增至信任位置的詳細資訊，請參閱 Microsoft Office Online 網站[建立、移除或變更檔案的信任位置](https://support.office.com/article/Create-remove-or-change-a-trusted-location-for-your-files-f5151879-25ea-4998-80a5-4208b3540a62)。
