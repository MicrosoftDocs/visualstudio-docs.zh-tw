...
1. 在不需要 Microsoft Office 的專案（例如主控台應用程式或 Windows Forms 專案）中，加入*Microsoft.VisualStudio.Tools.Applications.ServerDocument.dll*和*Microsoft.VisualStudio.Tools.Applications.Runtime.dll*元件的參考。
...
3. 呼叫靜態 <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument.AddCustomization%2A> 方法。
     ...
     下列程式碼範例會使用多載 <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument.AddCustomization%2A> 。 這個多載會採用檔的完整路徑，並 <xref:System.Uri> 針對您要附加至檔的自訂，指定部署資訊清單的位置。 這個範例假設名為**WordDocument1.docx**的 Word 檔位於桌面上，而且部署資訊清單位於名為**Publish**且同時也在桌面上的資料夾。
