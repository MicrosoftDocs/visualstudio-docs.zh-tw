...
- <xref:Microsoft.Office.Interop.Outlook._Application.Inspectors%2A> 屬性，您可用來存取顯示單一項目內容的視窗，例如電子郵件訊息或會議邀請。
  ...
  若要取得 <xref:Microsoft.Office.Interop.Outlook.Application> 物件的實例，請使用您專案中 `ThisAddIn` 類別的 [應用程式] 欄位。 如需詳細資訊，請參閱[VSTO 增益集程式](../vsto/programming-vsto-add-ins.md)設計。

> [!NOTE]
> 當您使用 Outlook 物件模型保護所封鎖的屬性和方法時，若要協助避免安全性警告，請從 `ThisAddIn` 類別的 [應用程式] 欄位取得 Outlook 物件。 如需詳細資訊，請參閱[Office 方案的特定安全性考慮](../vsto/specific-security-considerations-for-office-solutions.md)。
...
### <a name="folder-object"></a>資料夾物件
 ...
 如需示範如何存取預設 <xref:Microsoft.Office.Interop.Outlook.Folder> 並建立新 <xref:Microsoft.Office.Interop.Outlook.Folder>的範例，請參閱[如何：以程式設計方式建立自訂資料夾專案](../vsto/how-to-programmatically-create-custom-folder-items.md)。
...
### <a name="contactitem-object"></a>ContactItem 物件
 <xref:Microsoft.Office.Interop.Outlook.ContactItem>物件代表 [**連絡人**] 資料夾中的連絡人。 <xref:Microsoft.Office.Interop.Outlook.ContactItem> 物件含有它們所表示之人員的各種連絡資訊，例如街道地址、電子郵件地址和電話號碼。
...
## <a name="refdoc"></a>使用 Outlook 物件模型檔
...
### <a name="vba-object-model-reference"></a>VBA 物件模型參考
 ...
 VBA 物件模型參考中的所有物件和成員都會對應至 Outlook PIA 中的類型和成員。 例如，VBA 物件模型參考中的偵測器物件會對應至 Outlook PIA 中的 <xref:Microsoft.Office.Interop.Outlook.Inspector> 物件。 雖然 VBA 物件模型參考會提供大部分屬性、方法和事件的程式碼範例，但如果您想要在以 Visual Studio 建立的 Outlook VSTO 增益集專案中使用這些程式碼範例，您必須將此參考中的 VBA 程式碼改成 Visual Basic 或 Visual C# 程式碼。
