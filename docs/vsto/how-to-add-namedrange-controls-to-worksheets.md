---
title: 作法：將 NamedRange 控制項新增至工作表
ms.date: 02/02/2017
ms.topic: conceptual
dev_langs:
- VB
...
ms.locfileid: "71255617"
---
# <a name="how-to-add-namedrange-controls-to-worksheets"></a>HOW TO：將 NamedRange 控制項新增至工作表
...
2. 在 [**名稱**] 方塊中，輸入範圍的名稱，然後按**enter**鍵。
...
2. 將單一欄位從 [資料來源] 視窗拖曳至您的工作表。
     ...
     繫結至資料的 <xref:Microsoft.Office.Tools.Excel.NamedRange> 控制項隨即加入工作表。 如需詳細資訊，請參閱資料系結[和 Windows Forms](/dotnet/framework/winforms/data-binding-and-windows-forms)。

## <a name="runtimedoclevel"></a>在檔層級專案中，于執行時間加入 NamedRange 控制項
...
## <a name="runtimeaddin"></a>在 VSTO 增益集專案中，于執行時間加入 NamedRange 控制項
 您可以利用程式設計方式，在 VSTO 增益集專案中，將 <xref:Microsoft.Office.Tools.Excel.NamedRange> 控制項加入任何開啟中的工作表。 當工作表關閉時，動態建立的指定範圍不會保存為工作表中的主控制項。 如需詳細資訊，請參閱[在 VSTO 增益集的執行時間中擴充 Word 檔和 Excel 活頁簿](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md)。
