...
## <a name="assign-a-ribbon-to-an-inspector"></a>將功能區指派給偵測器  
 ...
 您可以在專案中加入多個功能區。 如果有多個功能區共用功能區 ID，請覆寫在專案 `ThisAddin` 類別中的 `CreateRibbonExtensibilityObject` 方法，以指定要在執行階段顯示哪個功能區。 如需詳細資訊，請參閱 <<c0> [ 功能區概觀](../vsto/ribbon-overview.md)。 如需有關每個功能區類型的詳細資訊，請參閱技術文件[來自訂功能區的 Outlook 2007](http://msdn.microsoft.com/946e97ea-f556-4e84-8fac-01cd9214e170)。  
...
## <a name="specify-the-ribbon-type-by-using-ribbon-xml"></a>使用功能區 XML 來指定功能區類型  
 ...
 <xref:Microsoft.Office.Core.IRibbonExtensibility.GetCustomUI%2A> 方法會在功能區程式碼檔中由 Visual Studio 自動產生。 *RibbonID*參數是識別總管或偵測器的特定類型的字串。 如需可能值的完整清單*ribbonID*參數，請參閱技術文件[自訂 Outlook 2007 功能區](http://msdn.microsoft.com/946e97ea-f556-4e84-8fac-01cd9214e170)。  
