...
## <a name="resize-controls-at-run-time"></a>在執行時間調整控制項大小
 如果您在執行時間加入 Windows Forms 控制項，並傳入做為 <xref:Microsoft.Office.Interop.Excel.Range> 控制項的位置，當包含該範圍的工作表資料格調整大小時，控制項會自動調整其大小。
...
## <a name="reset-control-placement"></a>重設控制項位置
 您可以將 `Placement` 屬性設定為下列其中一個值，以重設控制項的位置和大小 <xref:Microsoft.Office.Interop.Excel.XlPlacement> ：
