...
## <a name="accessing-the-ribbon-by-using-the-globals-class"></a>使用 Globals 類別存取功能區  
 您可以使用 `Globals` 類別，從專案中任意位置存取文件層級專案或 VSTO 增益集專案中的功能區。  

 如需有關`Globals`類別，請參閱[全域存取 Office 專案中的物件](../vsto/global-access-to-objects-in-office-projects.md)。  
...
## <a name="accessing-a-collection-of-ribbons-that-appear-in-a-specific-outlook-inspector-window"></a>存取出現在特定 [Outlook 檢查] 視窗中的功能區集合  
 您可以存取出現在 Outlook 的功能區集合*偵測器*。 [檢查] 是一個視窗，會在使用者執行特定工作 (例如建立電子郵件訊息) 時，在 Outlook 中開啟。 若要存取 [檢查] 視窗的功能區，請呼叫 `Globals` 類別的 `Ribbons` 屬性，並傳入代表 [檢查] 的 <xref:Microsoft.Office.Interop.Outlook.Inspector> 物件。  
...
## <a name="accessing-a-collection-of-ribbons-that-appear-for-a-specific-outlook-explorer"></a>存取出現在特定 [Outlook 總管] 視窗中的功能區集合  
 您可以存取出現在 Outlook 的功能區集合*總管*。 [總管] 是 Outlook 執行個體的主要應用程式使用者介面 (UI)。 若要存取 [總管] 視窗的功能區，請呼叫 `Globals` 類別的 `Ribbons` 屬性，並傳入代表 [總管] 的 <xref:Microsoft.Office.Interop.Outlook.Explorer> 物件。  
