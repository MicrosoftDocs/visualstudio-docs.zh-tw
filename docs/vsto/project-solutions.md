...
## <a name="automate-project-by-using-the-project-object-model"></a>使用專案物件模型自動化專案
 ...
 若要從 VSTO 增益集存取專案物件模型, 請使用`Application`專案中`ThisAddIn`類別的欄位。 `Application`欄位會傳回物件,代表`Microsoft.Office.Interop.MsProject.Application`專案的目前實例。 如需詳細資訊, 請參閱[VSTO 增益集程式](../vsto/programming-vsto-add-ins.md)設計。
...
## <a name="use-the-project-object-model-documentation"></a>使用 project 物件模型檔
 ...
 VBA 物件模型參考中的所有物件和成員都會對應至 Project 主要 Interop 組件 (PIA) 中的類型和成員。 例如, VBA 物件模型參考中的行事曆物件會對應至專案`Microsoft.Office.Interop.MSProject.Calendar` PIA 中的類型。 雖然 VBA 物件模型參考提供大部分屬性、方法和事件的程式碼範例, 但如果您想要在所建立的專案 VSTO 增益集專案C#中使用它們, 則必須將此參考中的 VBA 程式碼轉譯為 Visual Basic 或視覺效果。使用 Visual Studio。
...
### <a name="infrastructure-types-in-the-project-primary-interop-assembly"></a>專案主要 interop 元件中的基礎結構類型
 ...
 如需詳細資訊, 請參閱[Office 主要 interop 元件中的類別和介面總覽](http://go.microsoft.com/fwlink/?LinkId=189592)。
...
- [Office 開發中的 project 2010 和 Project Server 2010](http://go.microsoft.com/fwlink/?LinkId=199016)
