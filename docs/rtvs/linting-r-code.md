...
Visual Studio R 工具 (RTVS) 可為 R 提供內建的 Linter，其行為可透過一系列於本文中所述的選項做出控制。 這些選項位於 [工具] > [選項] > [文字編輯器] > [R] > [Lint] 中。

Lint 預設為停用。 若要啟用 Lint，請將 [全部] > [Enable lint] \(啟用 Lint\) 選項設定為 [True]。
