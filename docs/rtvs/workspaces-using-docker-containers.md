---
title: "適用於 Visual Studio 和 Docker 容器的 R 工具 |Microsoft Docs"
ms.custom: 
ms.date: 12/04/2017
ms.reviewer: 
ms.suite: 
...
- [建立容器](#creating-a-container)
- [連線到容器](#connecting-to-a-container)
- [使用自訂映像](#using-custom-built-images)
...
1. [工作區] 視窗的 [Local Running Containers] (本機執行容器) 區段會顯示在連接埠 5444 執行 RTVS 精靈的容器。 (請參閱 [Linux 的遠端 R 伺服器](workspaces-remote-r-service-for-linux.md)以取得設定精靈的詳細資料。)

    ![顯示可用容器的 [工作區] 視窗](media/workspaces-window-running-containers.png)

1. 若要連線到容器，請按兩下容器名稱，或選取容器名稱右邊的向前箭頭按鈕。 當連線時，您會看見 [R 互動] 視窗 (請參閱[使用 R 互動視窗](interactive-repl.md))：
