...
**R Tools** > **會話** > **重置**命令和互動式視窗中的重置工具列按鈕也隨時重置工作區狀態。 使用遠端工作區，重設會刪除第一次連線到遠端伺服器時所建立的使用者設定檔，這其實會刪除此處累積的任何檔案。

## <a name="local-workspaces"></a>本機工作區

[本機工作區] 清單會顯示您電腦上安裝的所有 R 解譯器。

當 Visual Studio 啟動時，它會嘗試通過查看**HKEY_LOCAL_MACHINE\軟體\R-Core\\**登錄機碼來自動檢測已安裝的所有 R 版本。 因為這項檢查只會在啟動時完成，所以如果您安裝新的 R 解譯器，就需要重新啟動 Visual Studio。
...
> [!Tip]
> 要禁用保存提示，請選擇**R Tools** > **選項**命令，並在**將工作區選項切換到 之前將"顯示確認對話方塊**`No`"設置為 。 請參閱[工作區選項](options-for-r-tools-in-visual-studio.md#workspace)。
...
根據預設，當您在本機工作區中啟動新的 R 解譯器時，目前的工作目錄會是 *%userprofile%\Documents*。 您可以隨時使用**R Tools** > **工作目錄**命令更改目錄，或者通過按右鍵 Visual Studio 解決方案資源管理器中的專案並在此處選擇"**設置工作目錄**"等命令來更改目錄。
...
亦請注意，在遠端工作區中，各個工作階段工作目錄中的所有檔案仍都保留在相同使用者設定檔原位。 如前所述，在使用遠端工作區時，可以使用**R Tools** > **會話** > **重置**命令（或互動式視窗中的重新開機按鈕）刪除這些檔。 此命令會再次刪除伺服器中的使用者設定檔，這在您重新連線時會重新建立。
...
[專案屬性](r-projects-in-visual-studio.md#project-properties)可控制 RTVS 在執行專案時是否複製檔案，且實際複製的檔案為何。 要打開此頁面，請選擇 **"專案** > **（名稱）屬性**"功能表命令，或按右鍵解決方案資源管理器中的專案並選擇 **"屬性**"。
