...
互動式視窗也會與一般 R 編輯器視窗整合。 您可以選擇代碼，然後按**Ctrl**+**Enter**，或按右鍵並選擇 **"在互動式 中執行"，** 並且代碼在互動式視窗中逐行運行，就像直接鍵入代碼一樣。 當游標位於編輯器視窗中的一行上時 **，Ctrl**+**Enter**會將該行發送到互動式視窗，然後將游標移動到下一行。 這樣，您只需重複按**Ctrl**+**Enter**即可單一步驟代碼。
...
在此多行模式下 **，"Enter"** 鍵僅在位於塊末尾時運行代碼塊，否則將插入新行。 但是，您可以在任何位置按**Ctrl**+**Enter**立即運行該代碼塊。
...
工具列命令如下所示，其中大多數具有等效鍵盤，並且在**R Tools** > **會話**和**R Tools** > **工作目錄**功能表（或如上所述）中也可用：

| 按鈕 | Command | 按鍵組合 | 描述 |
| --- | --- | --- | --- |
| ![重置按鈕](media/repl-toolbar-01-reset.png) | 重設 | **Ctrl**+**換檔**+**F10** | 重設互動式視窗工作階段，並清除所有變數和歷程記錄。 |
| ![[清除] 按鈕](media/repl-toolbar-02-clear.png) | Clear | **Ctrl**+**L** | 清除互動式視窗中所顯示的輸出；不會影響工作階段變數或歷程記錄。 |
| ![[歷程記錄] 按鈕](media/repl-toolbar-03-history.png) | 前一個歷程記錄命令<br/>下一個歷程記錄命令 | **向上鍵**、**向下鍵**<br/>**Alt**+**向上**， **Alt**+**向下** | 捲動歷程記錄，具有多行程式碼區塊的特定行為。 請參閱[歷程記錄](#history)。 |
...
| ![[來源 R 指令碼] 按鈕](media/repl-toolbar-06-source-r-script.png) | 來源 R 指令碼 | **Ctrl**+**移位**+**S** | 使用 Visual Studio 編輯器中目前作用中的 R 指令碼來呼叫 `source`，而編輯器會執行程式碼。  只有在 Visual Studio 編輯器中開啟 R 檔案時，才會出現此按鈕。 |
| ![[有回應的來源 R 指令碼] 按鈕](media/repl-toolbar-07-source-r-script-with-echo.png) | 有回應的來源 R 指令碼 | **Ctrl**+**移位**+**輸入** | 與 [來源 R 指令碼] 相同，但在互動式視窗中顯示指令碼內容。 |
| ![[插斷 R] 按鈕](media/repl-toolbar-08-interrupt-r.png)| 插斷 R | **Esc** | 停止互動式視窗中的任何執行中程式碼，例如，螢幕擷取畫面中的 `while` 迴圈會顯示在此區段的開頭。 |
| ![[附加偵錯工具] 按鈕](media/repl-toolbar-09b-attach-debugger.png)| 附加偵錯工具 | n/a | 也可使用**調試** > **附加到 R 交互**命令。 |
| ![[將工作目錄設定為來源檔案位置] 按鈕](media/repl-toolbar-10-set-working-directory-source.png)| 將工作目錄設定為來源檔案位置 | **Ctrl**+**移位**+**E** | 將工作目錄設定為載入至互動式視窗的最近來源檔案 (使用 `source`)。 請參閱[工作目錄](#working-directory)。 |
| ![[將工作目錄設定為專案位置] 按鈕](media/repl-toolbar-11-set-working-directory-to-project.png) | 將工作目錄設定為專案位置 | **Ctrl**+**移位**+**P** | 將工作目錄設定為 Visual Studio 中目前已載入專案的根目錄。 請參閱[工作目錄](#working-directory)。 |
...
開發人員通常會想要變更其在互動式工作階段時的工作目錄。 各種命令（可在工具列 **、R Tools** > **工作目錄功能表**和專案內容功能表）上提供，允許您輕鬆地將工作目錄設置為原始檔案、位置或專案或任何其他任意位置的位置。 這麼做可協助您在參考檔案時，避免鍵入完整路徑名稱或過長的相對路徑名稱。
...
此行為適應使用 Up 箭頭和**Enter**擊鍵組合重新執行歷程記錄中的最後一個專案的典型情況，同時通過按 Up 箭頭導航，自然允許編輯多行代碼塊。

為**Down**避免導航到多行代碼塊中，請使用工具列按鈕或"向上 **"和****"向下"，**-**Alt**+並且所有這些塊都被視為一行。

體驗歷程記錄功能的最簡單方式是在互動式視窗中自行嘗試即可。 下列程式碼提供數個適合的單行和多行陳述式。 不過，個別搭配使用複製並貼上與每個陳述式，可建立適當的歷程記錄 （您還可以將代碼粘貼到單獨的代碼檔中，然後使用**Ctrl**+**Enter**將行發送到互動式視窗。
