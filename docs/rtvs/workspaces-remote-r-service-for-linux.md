---
title: "在 Linux 上設定遠端 R 服務 | Microsoft Docs"
ms.custom: 
ms.date: 12/04/2017
ms.reviewer: 
ms.suite: 
...
適用於 Linux 的遠端 R 服務目前已封裝成 rtvs-daemon。 精靈已在 Ubuntu 16.04、16.10 LTS 桌面版、伺服器版，以及適用於執行 Ubuntu 之 Linux 的 Windows 子系統上進行測試並獲得支援。 本主題的一大部分會提供在這些不同系統上設定遠端 R 服務的指示。
