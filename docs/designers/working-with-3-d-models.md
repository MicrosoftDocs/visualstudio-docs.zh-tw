...
3D 模型會定義物件存在於 3D 場景中的圖形。 模型可以是基本的單獨物件、從基本物件階層所構成的複雜物件，或甚至是整個 3D 場景。 3D 物件由 3D 空間中的點（稱為*頂點*）、定義由這些點組成的三角形、線或其他基元的索引以及可能應用於每個頂點或每個基元的屬性（例如曲面法線）組成。 此外，某些資訊可能適用於每個物件；比方說，哪種著色器和紋理會給予物件獨特的外觀。
