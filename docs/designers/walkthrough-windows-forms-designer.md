...
10. 按 **Ctrl**+**V** 三次來將 `Button` 控制項的複本貼到 `TableLayoutPanel` 控制項剩下的控制列。
...
DemoCalculator 控制項上的按鈕具有事件處理常式，可用來實作大部分的計算機邏輯。 Windows Form 設計工具可讓您只需按兩下便能實作所有按鈕之所有事件處理常式的虛設常式。
