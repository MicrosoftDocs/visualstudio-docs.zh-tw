...
1. 以滑鼠右鍵按一下專案節點，然後選擇 [**加入 > 新專案**]。 選擇 [ **TYPESCRIPT JSON 設定檔**]，然後按一下 [**新增**]。
   ...
   Visual Studio 將*tsconfig*新增至專案根目錄。 您可以使用這個檔案來[設定](https://www.typescriptlang.org/docs/handbook/tsconfig-json.html)TypeScript 編譯器的選項。
...
1. 在此範例中，將 jQuery 的 TypeScript 定義檔新增至您的專案。 在您的*package. json*檔案中包含下列各項。
...
1. 如果未安裝方案總管中的封裝，請以滑鼠右鍵按一下 [npm] 節點，然後選擇 [**還原套件**]。
   ...
   >[!NOTE]
   > 在某些情況下，方案總管可能表示 npm 套件與*package*不同步，因為[這裡](https://github.com/aspnet/Tooling/issues/479)所述的已知問題。 例如，套件在安裝時可能會顯示為 [未安裝]。 在大部分情況下，您可以藉由刪除*package. json*、重新開機 Visual Studio，然後重新新增*封裝 json*檔案（如本文稍早所述）來更新方案總管。
