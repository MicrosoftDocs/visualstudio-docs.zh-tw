...
::: moniker-end
   ...
   隨即建立專案，並在**編輯器**視窗中開啟名為*server .js*的檔案。 [編輯器] 會顯示檔案的內容，而且您在 Visual Studio 中撰寫程式碼的工作大部分都在此進行。
...
1. 在搜尋方塊中鍵入**詳細資訊**。 從顯示的結果中，選擇 [選項] 類別下的 [專案和方案] --> [建置並執行]。

   ![Visual Studio 的搜尋方塊](media/quickstart-IDE-quick-launch.png)
