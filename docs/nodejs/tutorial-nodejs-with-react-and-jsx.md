...
1. 因為已執行具有中斷點的程式碼，所以請重新整理瀏覽器頁面以叫用中斷點。
    ...
    根據您的環境和瀏覽器狀態而定，您可能會叫用 app-bundle.js 的中斷點，或其在 app.tsx 中對應位置的中斷點。 不論哪一種方式，您都可以逐步執行程式碼並檢查變數。 (如果您需要中斷 *.tsx* 檔案中的程式碼但卻無法這樣做，請嘗試使用 `debugger;` 陳述式或在 Chrome Developer Tools 中設定中斷點。)
