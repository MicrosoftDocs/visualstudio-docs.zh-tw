---
title: 可用來發佈到開發與測試環境的 PowerShell 指令碼
...
* [Windows PowerShell 3.0](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770458(v=ws.10)) 或更新版本。
...
您可以遵循 [這些指示](/azure/virtual-machines/windows/classic/web-app-visual-studio)，在建立新專案時產生裝載網站之虛擬機器的發佈指令碼。 您也可以 [在 Azure App Service 中產生 Web 應用程式的發佈指令碼](/azure/app-service/scripts/app-service-powershell-deploy-github)。
...
1. 建立專案的 Web Deploy 封裝。 Web Deploy 封裝是經過壓縮的封存檔 (.zip 檔案)，內含您想要複製到網站或虛擬機器的檔案。 您可以在 Visual Studio 中為任何 Web 應用程式建立 Web Deploy 封裝。
   ...
   如需詳細資訊，請參閱 [如何：在 Visual Studio 中建立 Web 部署封裝](/previous-versions/aspnet/dd465323(v=vs.110))。 您也可以自動建立 Web Deploy 套件，如[自訂和擴充發佈指令碼](#customizing-and-extending-the-publish-scripts)中所述。
...
1. 以下列程式碼取代 `New-WebDeployPackage`，並取代建構 `$msbuildCmd` 的程式行中的預留位置。 此程式碼係用於 Visual Studio 2019。 如果您使用 Visual Studio 2017，請將 **VisualStudioVersion** 屬性變更為 `15.0`、針對 Visual Studio 2015 變更為 '14.0'，或針對 Visual Studio 2013 變更為 `12.0`。
    ...
    若要建置 Web 應用程式，使用 MsBuild.exe。 如需協助，請參閱 [MSBuild 命令列參考](../msbuild/msbuild-command-line-reference.md)
