...
## <a name="configure-security-permissions"></a>設定安全性權限  
 ...
 [專案設計工具]  中的 [安全性]  頁面包含 [啟用 ClickOnce 安全性設定]  核取方塊。 選取此核取方塊時，安全性權限要求會加入您應用程式的部署資訊清單。 在安裝期間，如果要求的權限超過從中部署應用程式之區域的預設權限，將會提示使用者授與權限。 如需詳細資訊，請參閱[＜How to：啟用 ClickOnce 安全性設定](../deployment/how-to-enable-clickonce-security-settings.md)。  
...
## <a name="debug-an-application-that-has-restricted-permissions"></a>偵錯具有限制權限的應用程式  
 ...
 如需詳細資訊，請參閱[＜How to：以受限制的權限對 ClickOnce 應用程式進行偵錯](../deployment/how-to-debug-a-clickonce-application-with-restricted-permissions.md)。  
