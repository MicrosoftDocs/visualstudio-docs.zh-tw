...
#### [針對測試和實際執行伺服器部署 ClickOnce 應用程式但不重新簽署](deploying-clickonce-applications-for-testing-and-production-without-resigning.md)
...
##### [逐步解說：手動部署不需要重新簽署而且會保留商標資訊的 ClickOnce 應用程式](walkthrough-manually-deploying-a-clickonce-app-no-re-signing-required.md)
