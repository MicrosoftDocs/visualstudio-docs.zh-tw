...
| `MSBuildRuntimeType` | 保留 | 目前執行的執行階段類型。 已在 MSBuild 15 中引入。 可能是未定義的值（在 MSBuild 15 之前），`Full` 表示 msbuild 是在桌面 .NET Framework 上執行，`Core` 指出 MSBuild 是在 .NET Core 上執行（例如在 `dotnet build`中），或 `Mono` 指出 MSBuild 是在 Mono 上執行。 |
