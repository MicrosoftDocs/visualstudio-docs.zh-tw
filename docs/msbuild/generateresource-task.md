...
|`EnvironmentVariables`|選擇性的 `String[]` 參數。<br /><br /> 指定應傳遞給生成的*resgen.exe*生成的環境變數的名稱值對陣列，以及（或有選擇地重寫）常規環境塊。|
|`ExcludedInputPaths`|選擇性 <xref:Microsoft.Build.Framework.ITaskItem>`[]` 參數。<br /><br /> 指定項目陣列，這會指定要在最新狀態檢查期間忽略所追蹤輸入的路徑。|
|`ExecuteAsTool`|選擇性的 `Boolean` 參數。<br /><br /> 如果`true`從相應的目標框架在 proc 外運行*tlbimp.exe*和*aximp.exe*以生成必要的包裝程式集。 此參數允許多目標的 `ResolveComReferences`。|
...
|`NeverLockTypeAssemblies`|選擇性的 `Boolean` 參數。<br /><br /> 獲取或設置布林值，指定是創建新[AppDomain](/dotnet/api/system.appdomain)以評估資源 *（.resx*） 檔 （true） 還是僅在資源檔引用使用者的程式集時創建新[AppDomain（false）。](/dotnet/api/system.appdomain)|
...
|`References`|選擇性的 `String[]` 參數。<br /><br /> 要從中載入 *.resx* 檔案類型的參考。 *.resx* 檔案資料項目可能具有 .NET 類型。 讀取 *.resx*檔時，必須解決此問題。 一般而言，使用標準型別載入規則即可順利解析。 如果您提供 `References` 中的組件，則會優先使用它們。<br /><br /> 強型別資源不需要此參數。|
...
|`ToolArchitecture`|選擇性的 <xref:System.String?displayProperty=fullName> 參數。<br /><br /> 用於確定*是否需要使用跟蹤器.exe*來生成*ResGen.exe*。<br /><br /> 應可剖析為 <xref:Microsoft.Build.Utilities.ExecutableType> 列舉的成員。 如果為 `String.Empty`，請使用啟發學習法以判斷預設架構。 應可剖析為 Microsoft.Build.Utilities.ExecutableType 列舉的成員。|
