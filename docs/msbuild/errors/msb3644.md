...
接下來，請注意、和中要求的 framework 識別碼和版本， `TargetFramework` `TargetFrameworkIdentifier` `TargetFrameworkVersion` 並尋找本機磁片磁碟機上該版本的參考元件資料夾。  例如，在 Windows 上，.NET Core 的32位參考元件應該位於 *C:\Program 檔 (x86) \Reference Assemblies\Microsoft\Framework \. NETCore* 和64位 .NET Framework 的參考元件資料夾必須位於 *C:\Program Files\Reference Assemblies\Microsoft\Framework \. .netframework*。 如果您的專案所需的版本資料夾不存在或似乎已損毀，您可能需要安裝或重新安裝該版本。 您可以在 [下載 .net](https://dotnet.microsoft.com/download/)上安裝所需的版本。
