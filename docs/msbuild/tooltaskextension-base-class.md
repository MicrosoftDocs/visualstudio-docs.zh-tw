...
| <xref:Microsoft.Build.Utilities.ToolTask.EchoOff%2A> | 選擇性的 `bool` 參數。<br /><br /> 當設置為`true`時，此任務將 **/Q**傳遞給*cmd.exe*命令列，以便命令列不會複製到 stdout。 |
...
| <xref:Microsoft.Build.Utilities.ToolTask.ToolPath%2A> | 選擇性的 `string` 參數。<br /><br /> 指定位置，工作會從該位置載入基礎可執行檔。 如果未指定此參數，則任務使用與運行 MSBuild 的框架版本對應的 SDK 安裝路徑。 |
