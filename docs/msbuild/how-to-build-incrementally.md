...
當您建置大型專案時，很重要的一點是，如果先前建置的元件仍是最新，就不會重建。 如果每次都建置所有目標，每次建置會花很長的時間才能完成。 若要啟用累加式組建（只會重建先前未建立的目標，或目標已過期的組建），Microsoft Build Engine （MSBuild）可以比較輸入檔的時間戳記與輸出檔案的時間戳記，以及決定要略過、建立還是部分重建目標。 不過，在輸入和輸出之間必須有一對一的對應。 您可以使用轉換，讓目標可以找出這種直接對應。 如需轉換的詳細資訊，請參閱[轉換](../msbuild/msbuild-transforms.md)。
...
MSBuild 可以比較輸入檔案的時間戳記與輸出檔案的時間戳記，並決定是否略過、建立或部分重建目標。 在下列範例中，如果 `@(CSFile)` 專案清單中的任何檔案比*hello .exe*檔案新，MSBuild 將會執行目標。否則會略過：
...
> [!NOTE]
> 在輸入和輸出之間沒有直接對應的目標，一律會比每個輸出只能對應到一個輸入的目標更常建立，因為如果某些輸入已變更，MSBuild 無法判斷哪些輸出需要重建。
...
> [!NOTE]
> 雖然 `GenerateContentFiles` 目標可以以累加方式建置，該目標的所有輸出永遠必須作為 `BuildHelp` 目標的輸入。 當您使用 `Output` 元素時，MSBuild 會自動提供一個目標的所有輸出作為另一個目標的輸入。
