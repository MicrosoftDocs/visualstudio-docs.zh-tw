...
<xref:Microsoft.Build.Tasks.Windows.GetWinFXPath> 工作會傳回目前 .NET 執行時間的目錄。

## <a name="task-parameters"></a>工作參數

| 參數 | 描述 |
|-------------------| - |
| `WinFXPath` | 選擇性的 **String** 輸出參數。<br /><br /> 指定 .NET 執行時間的實際路徑。 |
| `WinFXNativePath` | 必要的 **String** 參數。<br /><br /> 指定原生 .NET 執行時間的路徑。 |
| `WinFXWowPath` | 必要的 **String** 參數。<br /><br /> 指定64位系統上32位**windows On windows**模組中 .net 元件的路徑。 |
...
## <a name="example"></a>範例
 ...
 下列範例顯示如何使用**GetWinFXPath**工作來偵測 .net 執行時間的原生路徑。
