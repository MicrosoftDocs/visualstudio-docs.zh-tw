...
| `Text` | 選擇性的 `String` 參數。<br /><br /> 如果 `Condition` 參數評估為 `true`，MSBuild 會記錄的錯誤文字。 |

## <a name="remarks"></a>備註

`Error` 工作可讓 MSBuild 專案將錯誤文字發行到記錄器，並停止組建執行。
...
下列程式碼範例會確認已設定所有必要的屬性。 若未設定，專案就會引發錯誤事件，並記錄 `Text` 工作的 `Error` 參數值。
