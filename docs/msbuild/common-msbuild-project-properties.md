...
|DebugSymbols|布林值，指出建置是否要產生符號。<br /><br /> 在命令列上設定 **/p:DebugSymbols=false** 時，會停用程式資料庫 (.pdb) 符號檔的產生作業。|  
...
|Deterministic|布林值，指出編譯器是否應該針對相同的輸入產生相同的組件。 這個參數對應於 `vbc.exe` 和 `csc.exe` 編譯器的 `/deterministic` 參數。|
|DisabledWarnings|隱藏指定的警告。 您只需要指定警告識別碼的數值部分。 若有多個警告，請以分號分隔。 這個參數對應於 vbc.exe 編譯器的 `/nowarn` 參數。|  
...
|IntermediateOutputPath|如果沒有指定路徑，則為衍生自 `BaseIntermediateOutputPath` 的完整中繼輸出路徑。 例如 \obj\debug\\。 如果這個屬性被覆寫，則設定 `BaseIntermediateOutputPath` 沒有任何作用。|  
...
|OutputType|指定輸出檔的檔案格式。 這個參數的值可以是下列其中一個：<br /><br /> -   Library。 建立程式碼程式庫。 (預設值)。<br />-   Exe。 建立主控台應用程式。<br />-   Module。 建立模組。<br />-   Winexe。 建立 Windows 程式。<br /><br /> 這個屬性相當於 vbc.exe 編譯器的 `/target` 參數。|  
...
|ProduceReferenceAssembly|布林值，設定為 `true` 時會產生目前組件的[參考組件](https://github.com/dotnet/roslyn/blob/master/docs/features/refout.md)。 使用此功能時，`Deterministic` 應該是 `true`。 這個屬性對應於 `vbc.exe` 和 `csc.exe` 編譯器的 `/refout` 參數。|
...
|SGenToolPath|選擇性的工具路徑，指出目前版本的 SGen.exe 被覆寫時，可取得 SGen.exe 的位置。|  
...
|Satellite_CompanyName|產生附屬組件期間要傳遞到 AL.exe 的公司名稱。|  
|Satellite_Configuration|產生附屬組件期間要傳遞到 AL.exe 的組態名稱。|  
|Satellite_Description|產生附屬組件期間要傳遞到 AL.exe 的描述文字。|  
...
|VbcToolPath|選擇性路徑，指出目前的 vbc.exe 版本被覆寫時，vbc.exe 的另一個位置。|  
