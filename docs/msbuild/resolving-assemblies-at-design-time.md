...
## <a name="multi-targeting"></a>多目標
 ...
 Visual Studio 允許您定位在 .NET 框架的多個版本上運行的 .NET 框架版本。 發佈新的 .NET 框架版本時，可以使用目標包安裝框架，並會自動在 Visual Studio 中作為目標顯示。

## <a name="how-type-resolution-works"></a>類型解析如何運作

 在運行時，CLR 通過查看 GAC、bin 目錄和任何探測路徑來解決程式集中*bin*的類型。 這是由融合載入器處理。 但融合載入器如何知道所要尋找的目標呢？ 這會根據在設計階段建置應用程式時的解析結果而定。
 ...
 當您以特定的 .NET Framework 為目標時，建置系統會使用目標套件中的參考組件來解析所有類型。 在運行時，融合載入程式將這些相同的類型解析為運行時程式集，這些程式集通常位於 GAC 中。
