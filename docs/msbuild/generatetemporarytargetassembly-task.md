...
如果專案中至少有一個 XAML 頁面參考該專案中本機宣告的類型，則 <xref:Microsoft.Build.Tasks.Windows.GenerateTemporaryTargetAssembly> 工作會產生元件。 建置流程完成之後，或如果建置流程失敗，都會將產生的組件移除。

## <a name="task-parameters"></a>工作參數

| 參數 | 描述 |
|--------------------------| - |
| `AssemblyName` | 必要的 **String** 參數。<br /><br /> 指定為專案所產生之組件的簡短名稱，它也是暫時產生之目標組件的名稱。 例如，如果專案產生的 Windows 可執行檔名稱為*winexeassembly.exe*，則**AssemblyName**參數的值為**winexeassembly.exe**。 |
| `CompileTargetName` | 必要的 **String** 參數。<br /><br /> 指定用來從原始程式碼檔產生元件之 MSBuild 目標的名稱。 一般的 **CompileTargetName** 值為 **CoreCompile**。 |
...
第一個標記編譯階段（由[MarkupCompilePass1](../msbuild/markupcompilepass1-task.md)執行）會將 XAML 檔案編譯成二進位格式。 因此，編譯器需要包含 XAML 檔案所使用之類型的參考元件清單。 不過，如果 XAML 檔案使用在相同專案中定義的類型，則在建立專案之前，不會建立該專案的對應元件。 因此，在第一個標記編譯階段期間，無法提供組件參考。

相反地， **MarkupCompilePass1**會將包含相同專案中類型參考的 XAML 檔案，延遲轉換成第二個標記編譯階段，這是由[MarkupCompilePass2](../msbuild/markupcompilepass2-task.md)所執行。 在執行 **MarkupCompilePass2** 之前，會產生暫存組件。 此元件包含已延遲標記編譯階段的 XAML 檔案所使用的類型。 當執行時，會將所產生元件的參考提供給**MarkupCompilePass2** ，以允許將延遲的編譯 XAML 檔案轉換成二進位格式。
