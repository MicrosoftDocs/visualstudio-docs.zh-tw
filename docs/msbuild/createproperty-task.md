...
| `ValueSetByTask` | 選擇性的 `String` 輸出參數。<br /><br /> 包含與 `Value` 參數相同的值。 只有當您想要在因為輸出是最新狀態時，避免 MSBuild 設定 output 屬性時，才使用此參數。 |
...
下列範例使用 `CreateProperty` 工作，以使用 `NewFile` 和 `SourceFilename` 屬性值組合建立 `SourceFileExtension` 屬性。
