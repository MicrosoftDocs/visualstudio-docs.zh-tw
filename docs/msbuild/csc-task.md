...
ms.locfileid: "92796559"
---
# <a name="csc-task"></a>Csc 工作

包裝 csc.exe  ，並產生可執行檔 (.  檔案)。 如需 *csc.exe* 的詳細資訊，請參閱 [C# 編譯器選項](/dotnet/csharp/language-reference/compiler-options/index)。
...
| `Win32Icon` | 選擇性的 `String` 參數。<br /><br /> 在組件中插入  中具有所需的外觀。 如需詳細資訊，請參閱 [-win32icon (C# 編譯器選項)](/dotnet/csharp/language-reference/compiler-options/win32icon-compiler-option)。 |
