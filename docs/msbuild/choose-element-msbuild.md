...
|元素|描述|
|-------------|-----------------|
|[Otherwise](../msbuild/otherwise-element-msbuild.md)|選擇性項目。<br /><br /> 指定只有當所有 `PropertyGroup` 項目的條件評估為 `ItemGroup` 時，才需評估的程式碼 `When` 和 `false` 項目的區塊。 `Otherwise` 項目中可能有零或一個 `Choose` 項目，而且它必須是最後一個項目。|
|[When](../msbuild/when-element-msbuild.md)|必要元素。<br /><br /> 指定 `Choose` 項目可能要選取的程式碼區塊。 `When` 項目中可能有一或多個 `Choose` 項目。|
