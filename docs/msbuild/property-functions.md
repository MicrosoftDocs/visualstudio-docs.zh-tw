...
|string GetPathOfFileAbove(string file, string startingDirectory)|搜尋並傳回在目前組建檔位置上方目錄結構中檔案的完整路徑，或根據 `startingDirectory`（如有指定）。|
...
MSBuild 中的 `GetPathOfFileAbove` 屬性函式會傳回指定檔案的路徑（如果位於目前目錄上方的目錄結構中）。 它在功能上相當於呼叫
