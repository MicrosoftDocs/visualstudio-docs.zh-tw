...
MSBuild 是 Visual Studio 的建置系統。 下列連結會導致包含 MSBuild 參考資訊的主題。
...
- [命令列參考](../msbuild/msbuild-command-line-reference.md)\
 描述可以搭配 Msbuild.exe 使用的引數和參數。

- [.目標](../msbuild/msbuild-dot-targets-files.md)檔案\
 描述 *。* 包含在 MSBuild 中的目標檔案。

- [已知的項目中繼資料](../msbuild/msbuild-well-known-item-metadata.md)\
 列出與每個項目一起建立的中繼資料。

- [回應](../msbuild/msbuild-response-files.md)檔\
 說明包含命令列參數的 *.rsp* 檔案。
