...
產生 ClickOnce 部署資訊清單。 ClickOnce 部署資訊清單描述應用程式的部署，方法是定義部署的唯一身分識別、識別部署特性（例如安裝或線上模式）、指定應用程式更新設定和更新位置，以及，表示對應的 ClickOnce 應用程式資訊清單。
...
| `EntryPoint` | 選擇性的 <xref:Microsoft.Build.Framework.ITaskItem>`[]` 參數。<br /><br /> 指出所產生資訊清單組件的進入點。 針對 ClickOnce 部署資訊清單，此輸入會指定 ClickOnce 應用程式資訊清單。<br /><br />如果 `EntryPoint` 工作參數未指定，`<customHostSpecified>` 標記會插入為 `<entryPoint>` 標記的子系，例如：<br /><br /> `<entryPoint xmlns="urn:schemas-microsoft-com:asm.v2">`<br /><br /> `<co.v1:customHostSpecified />`<br /><br /> `</entryPoint>`<br /><br /> 您可以使用下列步驟，將 DLL 相依性加入至應用程式資訊清單︰<br /><br /> 1. 使用 <xref:Microsoft.Build.Tasks.ResolveAssemblyReference>的呼叫來解析元件參考。<br />2. 將上一個工作的輸出和元件本身傳遞至 <xref:Microsoft.Build.Tasks.ResolveManifestFiles>。<br />3. 使用 `Dependencies` 參數來傳遞相依性，以 <xref:Microsoft.Build.Tasks.GenerateApplicationManifest>。 |
...
| `MapFileExtensions` | 選擇性的 `Boolean` 參數。<br /><br /> 指定是否要使用 .deploy 副檔名對應。 如果此參數為 `true`，每個程式檔都是以 .deploy 副檔名發行。 此選項適用于網頁伺服器安全性，以限制必須解除封鎖的副檔名數目，以啟用 ClickOnce 應用程式部署。 此參數的預設值為 `false`。 |
