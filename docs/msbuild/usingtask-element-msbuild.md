...
> [!NOTE]
> 不同于屬性和專案，將會使用套用至的*第一個* `UsingTask` 元素 `TaskName` ; 若要覆寫工作，您必須在現有的之前定義新的 `UsingTask` *before* 。
...
|屬性|說明|
|---------------|-----------------|
|`Architecture`|選擇性屬性。<br /><br /> 指定工作必須在指定位的進程中執行。 如果目前的進程無法滿足需求，則工作將會在執行的工作主機進程中執行。<br /><br /> 支援的值為 `x86` （32位）、 `x64` （64位）、 `CurrentArchitecture` 和 `*` （任何架構）。|  
...
|`Runtime`|選擇性屬性。<br /><br /> 指定工作必須在指定版本的 .NET Framework 執行時間中執行。 如果目前的進程無法滿足需求，則工作將會在執行的工作主機進程中執行。 在 .NET Core MSBuild 中不支援。<br /><br /> 支援的值為 `CLR2` （.NET Framework 3.5）、 `CLR4` （.NET Framework 4.7.2 或更高版本）、 `CurrentRuntime` 和 `*` （任何執行時間）。|  
...
> [!NOTE]
> 專案層級屬性和專案沒有任何意義，如果 `UsingTask` 元素來自于使用 MSBuild 引擎全域註冊的其中一個 *. tasks*檔案。 專案層級值對 MSBuild 而言不是全域的。
