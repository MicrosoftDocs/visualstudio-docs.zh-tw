...
Microsoft Build Engine 是用來建立應用程式的平臺。 這個引擎也稱為 MSBuild，提供了專案檔的 XML 結構描述，以控制組建平台處理和建置軟體的方式。 Visual Studio 使用 MSBuild，但 MSBuild 不相依于 Visual Studio。 藉由在您的專案或方案檔上叫用*msbuild.exe* ，您可以在未安裝 Visual Studio 的環境中協調和建立產品。
 ...
 如需有關 MSBuild for c + + 的詳細資訊，請參閱[msbuild （c + +）](/cpp/build/msbuild-visual-cpp)。
...
## <a name="use-msbuild-at-a-command-prompt"></a>在命令提示字元中使用 MSBuild
 ...
 若要在命令提示字元中執行 MSBuild，請將專案檔與適當的命令列選項一起傳遞至*MSBuild.exe*。 命令列選項能讓您設定屬性、執行特定目標，以及設定可控制建置流程的其他選項。 例如，您可以使用下列命令列語法，在 `Configuration` 屬性設為 `Debug` 的情況下建置 *MyProj.proj* 檔案。
...
## <a name="project-file"></a>專案檔
 ...
 MSBuild 使用以 XML 為基礎的專案檔案格式，其可直接且可擴充。 MSBuild 專案檔案格式可讓開發人員描述要建立的專案，以及如何針對不同的作業系統和設定來建立它們。 此外，專案檔格式還能讓開發人員撰寫可重複使用的建置規則供個別檔案使用，讓這些組建在產品內的不同專案中仍有一致的表現。

 下列各節將說明 MSBuild 專案檔案格式的一些基本元素。 如需如何建立基本專案檔的教學課程，請參閱[逐步解說：從頭開始建立 MSBuild 專案檔案](../msbuild/walkthrough-creating-an-msbuild-project-file-from-scratch.md)。
...
### <a name="items"></a><a name="BKMK_Items"></a>Items
...
### <a name="tasks"></a><a name="BKMK_Tasks"></a>任務
 ...
 MSBuild 包含您可以修改以符合您需求的一般工作。 範例包括用於複製檔案的 [Copy](../msbuild/copy-task.md)、用於建立目錄的 [MakeDir](../msbuild/makedir-task.md)，以及用於編譯 Visual C# 原始程式碼檔的 [Csc](../msbuild/csc-task.md)。 如需可用工作的清單以及使用方式資訊，請參閱工作[參考](../msbuild/msbuild-task-reference.md)。

 工作是在 MSBuild 專案檔中執行，方法是建立一個專案，該元素的名稱為[目標](../msbuild/target-element-msbuild.md)元素的子系。 工作通常會接受參數，而這些參數會當做項目的屬性傳遞。 MSBuild 屬性和專案都可以當做參數使用。 例如，下列程式碼會呼叫 [MakeDir](../msbuild/makedir-task.md) 工作，並將前面範例中宣告的 `BuildDir` 屬性值傳遞給此工作。
...
### <a name="targets"></a><a name="BKMK_Targets"></a>攻擊
...
## <a name="build-logs"></a>組建記錄
 ...
 您可以將建置錯誤、警告和訊息記錄至主控台或另一個輸出裝置。 如需詳細資訊，請參閱[取得組建記錄](../msbuild/obtaining-build-logs-with-msbuild.md)檔和[MSBuild 中的記錄](../msbuild/logging-in-msbuild.md)。

## <a name="use-msbuild-in-visual-studio"></a>在 Visual Studio 中使用 MSBuild

 Visual Studio 使用 MSBuild 專案檔案格式來儲存 managed 專案的組建資訊。 使用 Visual Studio 介面加入或變更的專案設定，會反映在中 *。 \** 針對每個專案產生的 proj 檔案。 Visual Studio 使用 MSBuild 的主控實例來建立 managed 專案。 這表示可以 Visual Studio 或在命令提示字元中建立受控專案（即使未安裝 Visual Studio），而且結果會完全相同。
...
| [項目](../msbuild/msbuild-items.md) | 描述 MSBuild 檔案格式背後的一般概念，以及這些專案如何彼此配合。 |
...
| [工作](../msbuild/msbuild-tasks.md) | 顯示如何建立可供 MSBuild 用來執行不可部分完成之組建作業的可執行程式碼單位。 |
...
| [MSBuild 如何建立專案](build-process-overview.md) | 描述 MSBuild 中使用的內部組建進程 |
