...
**內容：** 將[開關語句](/dotnet/csharp/language-reference/keywords/switch)轉換為 C# 8.0[開關運算式](/dotnet/csharp/whats-new/csharp-8#switch-expressions)。

**何時：** 您希望將語句`switch`轉換為`switch`運算式，反之亦然。 

**原因：** 如果只使用運算式，則此重構可以輕鬆從傳統`switch`語句轉換。
...
2. 將游標放在關鍵字中`switch`，然後按**Ctrl**+**。** 以觸發 [快速動作與重構]**** 功能表。
