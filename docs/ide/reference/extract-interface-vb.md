---
title: "使用 Visual Basic 來擷取介面 | Microsoft Docs"
ms.custom: 
ms.date: 11/17/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="extract-an-interface-in-visual-basic"></a>使用 Visual Basic 來擷取介面
...
1. 醒目標示要作為動作執行對象的類別名稱，或直接將文字游標放在類別名稱中的某處。
   ...
   ![醒目標示的程式碼](media/extractinterface-highlight-vb.png)
...
1. 在 [擷取介面] 快顯對話方塊中，輸入所要求的資訊：![擷取介面](media/extractinterface-dialog-vb.png)
   | 欄位 | 描述 |
   | --- | --- |
...
1. 按一下 [確定 **Deploying Office Solutions**]。
   ...
   ![產生的類別](media/extractinterface-class-vb.png)
   ![產生的介面](media/extractinterface-interface-vb.png)
