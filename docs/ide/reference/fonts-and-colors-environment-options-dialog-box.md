...
[選項]**** 對話方塊的 [字型和色彩]**** 頁面可讓您為整合式開發環境 (IDE) 中的各種使用者介面項目建立自訂的字型和色彩配置。 您可以通過按一下 **"工具** > **選項**"來訪問此對話方塊，然後選擇 **"環境** > **字體和顏色**"。
...
- **環境字型**
     ...
     更改所有 IDE 使用者介面元素的字體樣式和大小，這些元素在 **"顯示"設置**中尚未具有單獨的選項。
...
- **[所有文字工具視窗]**
    ...
    > [!NOTE]
    > 對 **[所有文本工具 Windows]** 項的文本的更改不會在您進行這些專案的會話期間生效。 您可以藉由開啟的 Visual Studio 的另一個執行個體來評估這類變更。
...
|**指標保證金**|位於程式碼編輯器的左邊，顯示中斷點和書籤圖示的邊界。|
|**行號**|每一行程式碼旁出現的選擇性數字。|
|**可見的空白區**|空格、定位點與自動換行指標|
|**書簽**|包含書籤的行。 只有在停用指示區邊界時，才看得到 [書籤]****。|
...
|**可折疊文本**|可切換進入和離開程式碼編輯器的檢視的文字或程式碼區塊。|
|**註解**|程式碼註解。|
