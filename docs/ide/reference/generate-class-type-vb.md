---
title: "產生類別或類型 - 程式碼產生 (Visual Basic) | Microsoft Docs"
ms.custom: 
ms.date: 11/17/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="generate-a-class-or-type-in-visual-basic"></a>使用 Visual Basic 來產生類別或類型
**功能：**讓您立即產生類別或類型的程式碼。 
...
1. 將游標放在有紅色曲線的行上，該曲線代表您使用尚未存在的類別。
   ...
   ![醒目標示的程式碼](media/class-highlight-vb.png)
...
1. 接著，執行下列其中一項操作：
   ...
   * **滑鼠**
   ...
   ![「產生類別」預覽](media/class-preview-vb.png)
...
1. 如果您選取 [產生新的類型] 項目，將會顯示可讓您執行一些額外動作的快顯對話方塊。
   ...
   ![產生類型](media/class-newtype-vb.png)
...
1. 系統將會使用從類別/結構使用方式推斷的建構函式來自動建立類別/結構。
   ...
   ![「產生類別」結果](media/class-result-vb.png)
