...
當您使用簡化的組建組態時，專案系統會決定要建置偵錯或發行版本。 按一下 **"調試** > **開始調試**"（或按**F5）** 時，生成將放在調試位置，而不考慮指定的**輸出路徑**。 但是，[建置]**** 功能表上的 [建置方案]**** 命令卻會將其放在您指定的位置。 要啟用高級組建組態，在功能表列上，請選擇 **"工具** > **選項**"。 在 [選項]**** 對話方塊中，展開 [專案和方案]****，選取 [一般]****，然後清除 [顯示進階組建組態]**** 核取方塊。 這麼做可以讓您手動控制所有組態值，以及建置的是偵錯或發行版本。
