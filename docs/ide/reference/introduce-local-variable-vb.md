---
title: "引進區域變數 - 程式碼產生 (Visual Basic) | Microsoft Docs"
ms.custom: 
ms.date: 11/17/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="introduce-a-local-variable-in-visual-basic"></a>使用 Visual Basic 來引進區域參數
...
1. 醒目標示您想要指派給新區域變數的運算式。
   ...
   ![醒目標示的程式碼](media/local-highlight-vb.png)
...
1. 接著，執行下列其中一項操作：
   ...
   * **滑鼠**
   ...
   ![「引進區域」預覽](media/local-preview-vb.png)
...
1. 系統將會使用從區域變數使用方式推斷的類型來自動建立區域變數。  輸入名稱來為新區域變數提供一個新名稱。
   ...
   ![「引進區域」結果](media/local-result-vb.png)
