...
使用此選項，可自訂在編輯器中自動儲存檔案的頻率。 針對先前儲存的檔案，Visual Studio 會將檔案的複本儲存在 ***%LocalAppData%\Microsoft\VisualStudio\BackupFiles \\ [專案名稱]*** 中。 如果檔案是新檔案，但尚未儲存，Visual Studio 使用隨機產生的檔案名來 autosaves 它。
