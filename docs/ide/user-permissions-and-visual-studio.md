...
出於安全原因，應盡可能將 Visual Studio 作為典型使用者運行。

> [!WARNING]
> 您也應該確保不會編譯、啟動或偵錯任何不是來自受信任者或受信任位置的 Visual Studio 方案。

作為典型使用者，您可以在視覺化工作室 IDE 中執行幾乎所有操作。 您需要有系統管理員權限才能完成下列工作：

|區域|Task|取得詳細資訊|
|----------|----------| - |
|安裝|安裝或修改視覺化工作室。|[安裝視覺工作室](../install/install-visual-studio.md)，[修改視覺工作室](../install/modify-visual-studio.md)|
...
||對在不同使用者帳戶下執行的應用程式進行偵錯，例如 ASP.NET 網站。|[調試ASP.NET和AJAX應用程式](../debugger/how-to-enable-debugging-for-aspnet-applications.md)|
...
|效能工具|附加至提升權限的應用程式。|[性能分析入門指南](../profiling/beginners-guide-to-performance-profiling.md)|
