...
4.  您可以選擇性地指定類型等其他成員相關資訊。
     ...
     **注意：**您也可以使用鍵盤快速鍵來建立成員。 如需詳細資訊，請參閱[類別圖表和類別細節視窗中的鍵盤和滑鼠快速鍵](keyboard-and-mouse-shortcuts-in-the-class-diagram-and-class-details-window.md)。
...
-   例如，當類型位於參考組件時，整個類型會處於唯讀狀態。 當您在 [類別細節] 中選擇圖案時，[類別細節] 視窗會以唯讀狀態顯示圖案的詳細資訊。
