...
**類別設計工具**支援部分類別和方法。 類別圖表中的類型圖形即為部分方法的單一宣告位置。 如果部分類別定義于多個檔案中，您可以在 [**屬性**] 視窗中設定 [**新成員位置**] 屬性，以指定**類別設計工具**將使用的宣告位置。 也就是說，當您按兩下類別圖形時，**類別設計工具**會移至包含 [**新成員位置**] 屬性所識別之類別宣告的原始程式檔。 當您按兩下類別圖形中的部分方法時，**類別設計工具**會移至部分方法宣告。 另外，在 [屬性]**** 視窗中，[檔案名稱]**** 屬性是指宣告位置。 若是部分類別，[檔案名稱]**** 會列出所有包含該類別宣告和實作程式碼的檔案。 但若是部分方法，[檔案名稱]**** 只會列出包含部分方法宣告的檔案。
