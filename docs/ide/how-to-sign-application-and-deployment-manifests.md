...
> [!NOTE]
> *.pfx* 檔案不能包含憑證鏈結資訊。 如果是，則會出現以下導入錯誤：**找不到用於解密的證書和私密金鑰**。 要刪除憑證連結接資訊，可以使用*Certmgr.msc* [並禁用](/previous-versions/aa730868(v=vs.80))在匯出 *.pfx 檔時**包含所有證書**的選項。
