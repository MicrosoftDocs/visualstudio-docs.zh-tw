...
|||
|-|-|
|**新資料夾**|建立一個虛擬資料夾或子資料夾，可供您組織經常使用的元素。 它們保存在活動解決方案 （*.suo*） 檔中。 重新命名或刪除程式碼中的項目之後，其可能會以錯誤節點的方式出現在虛擬資料夾中。 若要修正此問題，請刪除該錯誤節點。 如果您重新命名項目，則可將其再一次從專案階層移動到資料夾內。|
|**上一步**|巡覽至先前選取的項目。|
|**轉寄**|巡覽至下一個選取的項目。|
...
**類視圖中**的快捷方式（或按右鍵）功能表可能包含以下命令，具體取決於所選項目類型：

|||
|-|-|
|**移至定義**|如果開啟的專案中未定義該項目，便在原始程式碼或 [物件瀏覽器]**** 中尋找該項目的定義。|
|**瀏覽定義**|在 [物件瀏覽器]**** 中顯示所選項目。|
|**查找所有參考**|尋找目前所選物件項目，並在 [尋找結果]**** 視窗中顯示結果。|
|**對類型篩選** (僅限 Managed 程式碼)|僅顯示所選類型或命名空間。 您可以通過選擇"查找 " 旁邊的 **"清除查找**"**X**）**Find**按鈕來刪除篩選器。|
|**複製**|複製項目的完整名稱。|
...
您可以使用視窗頂端的下拉式清單來篩選您要檢視的元件。 自訂群組件可以包括託管代碼可執行檔、庫程式集、型別程式庫和 *.ocx*檔。 無法加入 C + + 自訂元件。

::: moniker range="vs-2017"

自訂設置保存在 Visual Studio 使用者應用程式目錄中 *，%APPDATA%\微軟_VisualStudio_15.0\ObjBrowEX.dat*。
...
您可以使用視窗頂端的 [搜尋]**** 方塊，搜尋特定項目。 搜尋不區分大小寫。 搜尋結果會顯示在左窗格中。 要清除搜索，請選擇 **"搜索**"框旁邊的 **"清除搜索**"**X**） 按鈕。
...
通過使用工具列上**的物件瀏覽器設置**按鈕，可以指定以下視圖之一：
...
**物件瀏覽器**中的快捷方式（或按右鍵）功能表可能包含以下命令，具體取決於所選項目類型：
...
> [!NOTE]
> 從 Visual Studio 2015 開始，**代碼定義**視窗只能與C++代碼一起使用。

### <a name="code-definition-shortcut-menu"></a>[程式碼定義] 捷徑功能表

**代碼定義**視窗中的快捷方式（或按右鍵）功能表可能包含以下命令：
...
|**轉到聲明**||
|**查找所有參考**|在方案中尋找類型或成員的參考。|
|**檢視呼叫階層**|在 [呼叫階層]**** 視窗中顯示方法。|
|**切換標頭/程式碼檔**||
|**運行測試**|如果專案中含有單元測試，將會執行所選程式碼的測試。|
|**調試測試**||
|**中斷點**|插入中斷點 (或追蹤點)。|
|**運行到游標**|以偵錯模式執行程式，直到指標的位置為止。|
|**片段**||
|**剪下**、**複製**、**貼上**||
|**注釋**||
|**大綱**|標準大綱命令。|
...
- [類視圖和物件瀏覽器圖示](../ide/class-view-and-object-browser-icons.md)
