...
視選取的專案類型而定，**類別檢視**中的快捷方式（或滑鼠右鍵）功能表可能包含下列命令：
...
您可以使用視窗頂端的下拉式清單來篩選您要檢視的元件。 自訂群組件可包含 managed 程式碼可執行檔、程式庫元件、類型程式庫和 *.ocx*檔案。 無法加入 C + + 自訂元件。
...
藉由使用工具列上的 [**物件瀏覽器設定**] 按鈕，您可以指定下列其中一個視圖：
...
視選取的專案類型而定，**物件瀏覽器**中的快捷方式（或滑鼠右鍵）功能表可能包含下列命令：
...
[程式**代碼定義**] 視窗中的快捷方式（或滑鼠右鍵）功能表可能包含下列命令：
