...
2. 在顯示的搜尋結果中，於 [元件] 標題下，選取 [安裝 PreEmptive Protection - Dotfuscator]。
   * 但您若在「功能表」標題下看到 [PreEmptive Protection - Dotfuscator Community]，則表示已安裝 Dotfuscator Community。 選取該選項即可[開始][get-started]使用。
...
2. 在 [快速啟動] 結果顯示畫面的「安裝」標題下，選取 [PreEmptive Protection - Dotfuscator (個別元件)]。
   * 但若看到「功能表」標題下的 [工具] - [PreEmptive Protection - Dotfuscator]，則已安裝 Dotfuscator CE。 選取該選項即可[開始][get-started]使用。
