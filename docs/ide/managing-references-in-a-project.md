...
若要新增參考，請以滑鼠右鍵按一下方案總管**** 中的 [參考]**** 或 [相依性]**** 節點，然後選擇 [新增參考]****。 您還可以按右鍵專案節點並選擇 **"添加** > **參考**"。 有關詳細資訊，請參閱[如何：添加或刪除引用](../ide/how-to-add-or-remove-references-by-using-the-reference-manager.md)。
...
如果選擇將 Windows 8.1 專案重新定向到 Windows 10，請參閱[移植、遷移和升級 Visual Studio 專案](../porting/port-migrate-and-upgrade-visual-studio-projects.md)。
...
當您有會產生組件的專案時，您應該參考該專案，而不要使用檔案參考 (請參閱下文)。 專案對專案參考的優點是它會在組建系統中建立專案之間的相依性。 如果自上次建置的參考專案已變更，則將會建立相依專案。 檔案參考不會建立組建相依性，因此可以建置參考專案而不需建置相依專案，且參考可能會遭到淘汰。 （也就是說，專案可以引用以前構建的專案版本。這可能導致*bin*目錄中需要多個版本的單個 DLL，這是不可能的。 當發生此衝突時，您會看到一則訊息，例如「警告: 無法將專案 'project' 中的相依性 'file' 複製至執行目錄，因為它會覆寫參考 'file'」。 如需詳細資訊，請參閱[針對中斷參考進行疑難排解](../ide/troubleshooting-broken-references.md)以及[如何：建立和移除專案相依性](../ide/how-to-create-and-remove-project-dependencies.md)。
...
- [排除損壞的引用故障](../ide/troubleshooting-broken-references.md)
