---
title: 使用 tasks.vs.json launch.vs.json 來自訂組建偵錯工作
ms.date: 02/21/2018
ms.topic: conceptual
helpviewer_keywords:
- NMAKE [Visual Studio]
...
1. 接下來，以滑鼠右鍵按一下 [方案總管] 中的可執行檔，並選擇 [設定為啟動項目]。
   ...
   > [!IMPORTANT]
   > 如需有關「開啟資料夾」專案中C++自訂群組建和「偵錯工具」工作的詳細資訊，請參閱[Visual Studio 中的組建系統的C++開啟資料夾支援](/cpp/build/open-folder-projects-cpp)。
