...
- 根據您在 >**工具** 中的程式碼樣式設定，自動建立*editorconfig*檔案 **，>** **文字編輯器**> **C#** > 程式**代碼樣式**。
...
- 直接透過編輯器設定程式碼樣式規則的嚴重性層級。 如果您目前沒有 editorconfig 檔案，系統會為您產生一個檔案。 將游標放在 [錯誤]、[警告] 或 [建議] 上，並輸入**Ctrl**+ **。** 開啟 [快速動作與重構] 功能表。 選取 [**設定] 或 [隱藏問題**]。 然後選取規則，並選擇您想要為該規則設定的嚴重性層級。 這會以規則的新嚴重性更新現有的 EditorConfig。
