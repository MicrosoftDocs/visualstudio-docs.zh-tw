...
若要在 Visual Studio 中使用 F# 進行開發，請先安裝 **.NET 桌面開發**工作負載 (若尚未安裝的話)。 您可以通過視覺化工作室安裝程式安裝視覺化工作室工作負載，您可以通過選擇 **"工具** > **獲取工具和功能**"來打開該工作負載。
...
|[查找和替換文本](finding-and-replacing-text.md)|可讓您在檔案、專案或解決方案中搜尋，並且可能變更文字。|是|
...
|**Ctrl**+**按一下**以轉到定義|可讓您按住 **Ctrl** 並按一下 F# 符號，以叫用 [移至定義]。|是|
|從 QuickInfo 移至定義|工具提示內可叫用 [移至定義] 的可按式符號。|是|
|移至全部|通過**Ctrl**+**T**為所有 F# 構造啟用全域、模糊比對導航。|是|
...
|自動完成|啟用後，會導致單詞完成在鍵入時選擇第一個匹配項，而不是等待您選擇一個或按**Ctrl**+**空格**。|是|
