...
> [!NOTE]
> 如果您因為記憶體問題而使用產品時遇到困難，請透過 [意見反應工具](../ide/how-to-report-a-problem-with-visual-studio.md)讓我們知道。
...
- 使用 [**編輯**] [移至] [移至  >  **Go To**  >  **最後一個編輯位置**]，或按**Ctrl** + **Shift** + **倒退鍵**，跳至方案中的最後一個編輯位置。
...
- **指定要載入的符號**
    ...
    在原生的偵錯工具中，載入符號檔 (*. .pdb*) 在記憶體資源方面很昂貴。 您可以設定偵錯工具符號設定來節省記憶體。 一般而言，您會將方案設定成只從您的專案載入模組。

    若要指定符號載入，請選擇 [**工具**] [選項] [  >  **Options**  >  **調試**  >  **符號**]。
...
- **停用對應模式**
    ...
    [**對應模式**](how-to-track-your-code-by-customizing-the-scrollbar.md#display-modes) 會在捲軸上顯示程式碼的行數（以縮為單位）。 預設會啟用對應模式。

    若要停用對應模式，請移至 [**工具**] [選項] [  >  **Options**  >  **文字編輯器**]  >  [**所有語言**]  >  [**捲軸**]，然後在 [**行為**] 區段中，取消選取 [**使用垂直捲動條的地圖**

- **停用自動換行**

    [**自動換**](./reference/how-to-manage-word-wrap-in-the-editor.md) 行會顯示超出程式碼編輯器視窗目前寬度的長時間程式碼片段的部分。 預設會開啟自動換行。

    若要停用目前正在處理之專案的自動換行，請移至 [**編輯**] [  >  **先進**的  >  **自動換行**]。  (您可以使用相同的功能表命令來切換此設定。 ) 

    若要停用所有專案的自動換行，請移至 [**工具**]  >  [**選項]**  >  **[一般**  >  **文字編輯器**]  >  **[所有語言**]  >  ** **，然後在 [**設定**] 區段中取消選取 [**自動換行**
...
CLR 使用記憶體回收記憶體管理系統。 在此系統中，有時不再需要的物件會使用記憶體。 這種狀態是暫時的；記憶體回收行程會根據其效能和資源使用量啟發學習法來釋放這個記憶體。 您可以使用 Visual Studio 中的快速鍵，強制 CLR 收集任何未使用的記憶體。 如果有大量的垃圾等候收集，而您強制進行垃圾收集，您應該會在 [**工作管理員**] 中看到*devenv.exe*進程的記憶體使用量下降。 很少需要使用這種方法。 不過，完成耗費資源的作業 (例如完整建置、偵錯工作階段或方案開啟事件) 之後，即可協助您判斷處理序實際正在使用的記憶體數量。 因為 Visual Studio 是混合的 (Managed 和原生)，所以原生配置器和記憶體回收行程可能偶而會競爭有限的記憶體資源。 在高記憶體使用量的情況下，它有助於強制執行記憶體回收行程。

若要強制執行垃圾收集，請使用**快速鍵： ctrl** + **alt** + **shift** + **f12**、 **Ctrl** + **alt** + **shift** + **f12** (按兩次) 。
