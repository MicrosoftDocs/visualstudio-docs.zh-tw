...
> [!NOTE]
> 如果您因為記憶體問題而難以使用該產品，請通過[回饋工具](../ide/how-to-report-a-problem-with-visual-studio.md)告知我們。
...
1. 選擇 **"工具** > **選項**"以打開"**選項**"對話方塊。

1. 在"**專案和解決方案** > **"一般**頁面上，取消選擇**在解決方案載入上重新打開的文檔**。

若您停用自動檔案還原，則可以使用其中一個[移至](../ide/go-to.md)命令，快速瀏覽到您要開啟的檔案：

- 若是一般**移至**功能，請選取 [編輯]**** > [移至]**** > [移至全部]****，或按 **Ctrl**+**T**。

- 使用 **"編輯** > **轉到** > **最後編輯位置**"或按**Ctrl**+**移位**+**回空間**跳轉到解決方案中的最後一個編輯位置。

- 使用 [移至最近使用的檔案]****，查看最近在方案中瀏覽的檔案清單。 選擇 **"編輯** > **轉到** > **最近的檔**"，或按**Ctrl**+**1** **，Ctrl**+**R**。
...
- **指定要載入的符號**
    ...
    對於本機調試，載入符號檔 *（.pdb*） 在記憶體資源方面非常昂貴。 您可以設定偵錯工具符號設定來節省記憶體。 一般而言，您會將方案設定成只從您的專案載入模組。

    要指定符號載入，請選擇 **"工具** > **選項** > **調試** > **符號**"。

    設定 [僅限指定的模組]**** 的選項，而不是 [所有模組]****，然後指定您要載入的模組。 偵錯時，您也可以以滑鼠右鍵按一下 [模組]**** 視窗中的特定模組，以在符號載入中明確包含模組 （要在調試時打開視窗，請選擇**調試** > **Windows** > **模組**。
...
- **停用 XAML 設計工具**
    ...
    預設情況下啟用 XAML 設計器，但僅在打開 *.xaml*檔時才會消耗資源。 如果您使用 XAML 檔案，但不想要使用設計工具功能，請停用這項功能，以釋出一些記憶體。
...
CLR 使用記憶體回收記憶體管理系統。 在此系統中，有時不再需要的物件會使用記憶體。 這種狀態是暫時的；記憶體回收行程會根據其效能和資源使用量啟發學習法來釋放這個記憶體。 您可以使用 Visual Studio 中的快速鍵，強制 CLR 收集任何未使用的記憶體。 如果有大量垃圾等待收集，並且強制進行垃圾回收，則應在**工作管理員**中看到*devenv.exe*進程的記憶體使用方式下降。 很少需要使用這種方法。 不過，完成耗費資源的作業 (例如完整建置、偵錯工作階段或方案開啟事件) 之後，即可協助您判斷處理序實際正在使用的記憶體數量。 因為 Visual Studio 是混合的 (Managed 和原生)，所以原生配置器和記憶體回收行程可能偶而會競爭有限的記憶體資源。 在高記憶體使用量的情況下，它有助於強制執行記憶體回收行程。
