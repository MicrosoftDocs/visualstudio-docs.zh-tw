---
title: 改善效能的祕訣
description: 瞭解如何優化您可能不會用來協助改善效能的特定 Visual Studio 功能。
...
- **停用診斷工具**
    ...
    若要停用 **診斷工具**，請啟動偵錯工具，選取 [**工具**  >  **選項**]  >    >  **[一般**]，然後取消選取 [在 **偵錯工具時啟用診斷工具**] 選項。
...
- **停用 XAML 設計工具**
    ...
    若要停用 xaml 設計工具，請移至 [**工具**  >  **選項**  >  **xaml 設計** 工具  >  **啟用 xaml 設計** 工具]，然後取消選取此選項。
...
- **將未追蹤的檔案新增至本機 .gitignore**
    ...
    Visual Studio 會使用未追蹤的檔案執行 Git 命令， `git status` 以在您將新檔案新增至存放庫時提供順暢的體驗。 當有大量未追蹤的檔案時， `git status` 可能會耗用額外的記憶體。 若要忽略這些檔案並改善的效能 `git status` ，您可以將這些檔案或資料夾新增至您的本機 .gitignore 檔案。 若要存取檔案，請移至 **git**  >  **設定**  >  **git 存放庫設定**。 然後，在 [ **Git** 檔案] 區段中，按一下 [ **新增** ] 建立 .gitignore 檔案，或按一下 [ **編輯** ] （如果您已經有的話）。
