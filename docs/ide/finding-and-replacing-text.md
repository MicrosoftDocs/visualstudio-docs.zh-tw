...
- 按**Ctrl**+**F**作為*快捷方式，以*查找當前檔中的字串。
- 按**Ctrl**+**H**作為快捷方式，*以查找和替換*當前檔中的字串。
...
**"查找"** 控制項的版本在某些工具視窗中也可用。 例如，您可以在搜尋方塊中輸入文字，以篩選 [工具箱]**** 視窗中的控制項清單。 可讓您搜尋其內容的其他工具視窗包含 [方案總管]****、[屬性]**** 視窗和 [Team Explorer]****。

## <a name="find-in-files-and-replace-in-files"></a>檔案中尋找和檔案中取代

- 按**Ctrl**+**Shift**+**F**作為快捷方式，*以查找*多個檔中的字串。
- 按**Ctrl**+**Shift**+**H**作為快捷方式，*以查找和替換*多個檔中的字串。
...
您可以通過選擇"**查找資料夾"** 按鈕（它看起來像**Look in****...）** 來定義搜尋範圍。 在 [選擇搜尋資料夾]**** 對話方塊中，您可以指定一組要搜尋的資料夾，並且可以儲存規格，以在稍後重複使用。
...
您可以選擇 [查詢]**** 方塊旁的 [編輯自訂元件集]**** 按鈕，將元件集定義為搜尋範圍。 您可以指定已安裝的 .NET 或 COM 元件、包含在解決方案中的 Visual Studio 專案或任何程式集或型別程式庫（.dll、.tlb、.olb、.exe 或 *.ocx）。* * * *.tlb* *.olb* *.exe* 若要搜尋參考，請選取 [查詢參考]**** 方塊。
...
|**Ctrl**+**Alt** = 按一下 + 拖動|新增第二個選取範圍|
|**移位**+**Alt**+**.**|將下一個相符文字新增為選取範圍|
|**Ctrl**+**移位**+**Alt**+**，**|將所有相符的文字新增為選取範圍|
|**移位**+**Alt**+**，**|移除最後一個選取項目|
|**Ctrl**+**移位**+**Alt**+**.**|跳過下一個相符的項目|
|**Alt** + 按一下|新增方塊選取範圍|
|**Esc** 或按一下|清除所有選取範圍|
...
- [在視覺化工作室中使用正則運算式](../ide/using-regular-expressions-in-visual-studio.md)
