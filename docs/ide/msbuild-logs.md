...
1. 兩個下拉式方塊中的 [Msbuild project build verbosity] \(MSBuild 專案組建詳細資料\)  都設為 [詳細]  。 第一個控制 [輸出視窗]  中的組建詳細資料，第二個控制 \<專案名稱\>.log 檔案中的組建詳細資料，此記錄檔是在建置期間建立於每個專案的中繼目錄中。
1. 在 Visual Studio 開發人員命令提示字元中，輸入下列任一命令，替換實際的路徑和組態值：
