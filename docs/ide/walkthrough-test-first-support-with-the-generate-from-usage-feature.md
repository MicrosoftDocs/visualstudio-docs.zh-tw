...
**"從使用中生成**"功能可與與 Visual Studio 集成的測試框架一起使用。 本主題會示範 Microsoft 單元測試架構。
...
1. 測試專案包含名為*UnitTest1*的檔。 在**方案總管**中按兩下這個檔案，在程式碼編輯器中開啟它。 已產生測試類別和測試方法。
...
4. 選擇或按一下 **"快速操作**"燈泡。 您會看到錯誤訊息，說明類型 `Automobile` 未定義。 您也會看到一些解決方案。

5. 按一下 **"生成新類型**"以打開"**組建類型"** 對話方塊。 此對話方塊會提供許多選項，包括在不同的專案中產生類型。
...
8. 在**解決方案資源管理器**中，在**GFUDemo_VB**或**GFUDemo_CS**專案節點下查看，以驗證新的*汽車.vb*或*Automobile.cs*檔是否存在。 在程式碼編輯器中，焦點仍在 `AutomobileTest.DefaultAutomobileIsInitializedCorrectly`，這可讓您以最少的中斷繼續撰寫測試。
...
3. 按一下屬性的 **"快速操作**"燈泡`IsRunning`，然後按一下**生成屬性"汽車.IsRunning"。**
...
1. 在 [測試]**** 功能表上，選擇 [執行]**** > [所有測試]****。
     ...
     **運行** > **所有測試**命令運行為當前解決方案編寫的任何測試框架中的所有測試。 在這個案例中，會有兩個測試，而且兩個測試都應該要失敗。 `DefaultAutomobileIsInitializedCorrectly` 測試失敗的原因是 `Assert.IsTrue` 條件傳回 `False`。 `AutomobileWithModelNameCanStart` 測試失敗的原因是 `Start` 類別的 `Automobile` 方法擲回例外狀況。
...
- [從使用方式生成](../ide/visual-csharp-intellisense.md#generate-from-usage)
