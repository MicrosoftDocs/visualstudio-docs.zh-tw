...
1. 使用下列程式碼，將標籤參考加入至您的表單。
     ...
     當變數不跟蹤物件時，它將設置為一個特殊的保留值：`null`在 C# 和`Nothing`Visual Basic 中。 因此當程式啟動時，`firstClicked` 和 `secondClicked` 會設為 `null` 或 `Nothing`，這表示變數並未追蹤任何項目。
...
4. 選擇一個 Label 控制項，並注意什麼事也沒發生。 程式已經跟蹤玩家選擇的第一個標籤，因此`firstClicked`不等於`null`C# 或`Nothing`Visual Basic 中的標籤。 當 `if` 陳述式檢查 `firstClicked` 以判斷它是否等於 `null` 或 `Nothing` 時，若發現不相等，就不會執行 `if` 陳述式中的陳述式。 因此，只有選擇的第一個圖示變為黑色，而其他圖示是不可見的，如下圖所示。
...
- 要轉到下一個教程步驟，請參閱**[步驟 6：添加計時器](../ide/step-6-add-a-timer.md)**。
