...
**程式碼片段插入器**顯示所有可用程式碼片段的程式碼片段名稱。 程式碼片段插入器也會包含輸入對話方塊，您可以在其中輸入程式碼片段名稱或程式碼片段名稱的一部分。 程式碼片段插入器會反白顯示最接近程式碼片段名稱的相符項目。 隨時按**Tab**將關閉程式碼片段插入器並插入當前選定的程式碼片段。 在程式碼編輯器中按 **ESC** 或按一下滑鼠會關閉程式碼片段插入器，而不插入程式碼片段。
...
|已選取|建立 [checked](/dotnet/csharp/language-reference/keywords/checked) 區塊。|在方法、索引子、屬性存取子或事件存取子內。|
...
|mbox|建立 <xref:System.Windows.Forms.MessageBox.Show%2A?displayProperty=fullName> 呼叫。 您可能需要添加對*System.Windows.Forms.dll*的引用。|在方法、索引子、屬性存取子或事件存取子內。|
...
|參數|建立 [switch](/dotnet/csharp/language-reference/keywords/switch) 區塊。|在方法、索引子、屬性存取子或事件存取子內。|
...
|取消選取|建立 [unchecked](/dotnet/csharp/language-reference/keywords/unchecked) 區塊。|在方法、索引子、屬性存取子或事件存取子內。|
...
- [程式碼片段函數](../ide/code-snippet-functions.md)
- [程式碼片段](../ide/code-snippets.md)
- [範本參數](../ide/template-parameters.md)
- [如何：使用環繞碼程式碼片段](../ide/how-to-use-surround-with-code-snippets.md)
