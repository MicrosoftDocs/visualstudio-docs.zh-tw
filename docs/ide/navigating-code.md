...
您可以使用工具列上的 **"向後導航****（Ctrl**+**-**） 和 **"向前導航**+**-****"（Ctrl**+**Shift）** 按鈕將插入點移動到以前的位置，或從上一個位置返回到較新的位置。 這些按鈕會保留插入點的最後 20 個位置。 [檢視]**** 功能表的 [向後巡覽]**** 和 [向前巡覽]**** 底下也有這些命令。
...
- 要將焦點從代碼視窗轉移到巡覽列，請按快速鍵組合**Ctrl**+**F2**。
...
要隱藏巡覽列，可以在 **"文字編輯器所有語言**"設置（**工具** > **選項** > **文字編輯器** > **所有語言**）中更改**巡覽列**選項，也可以更改各個語言的設置。
...
輸入 | 函式
------------ | ---
**鍵盤** | 將文本游標放在類型名稱的某處，然後按**Shift**+**F12**
**滑鼠** | 從右鍵功能表中選取 [尋找所有參考]****

## <a name="reference-highlighting"></a>反白顯示參考

當您按一下原始程式碼中的符號時，該符號在文件的所有出現處，都將反白顯示。 反白顯示的符號可能包含宣告和參考，以及許多其他 [尋找所有參考] **** 會傳回的符號。 其中包括類別、物件、變數、方法和屬性的名稱。 在 Visual Basic 程式碼中，許多控制項結構的關鍵字也會反白顯示。 要移動到下一個或上一個突出顯示的符號，請按 **"Ctrl**+**向下移位**+**箭頭**"或 **"Ctrl**+**向上移位**+**箭頭**"。 您可以在 **"工具** > **選項** > **環境** > **"字體和顏色** > **突出顯示參考 "** 中更改突出顯示顏色。
...
- **轉到所有****（Ctrl**+**T**或**Ctrl**+**），** 移動到指定的行、類型、檔、成員或符號。

- **轉到檔****（Ctrl**+**1**， **Ctrl**+**F**）： 移動到解決方案中的指定檔。

- **轉到最近的檔****（Ctrl**+**1**， **Ctrl**+**R**）： 移動到解決方案中指定的最近訪問的檔。

- **移至類型** (**Ctrl**+**1**、**Ctrl**+**T**)：移至解決方案中的指定類型。

- **移至成員** (**Ctrl**+**1**、**Ctrl**+**M**)：移至解決方案中的指定成員。

- **轉到符號****（Ctrl**+**1**， **Ctrl**+**S**）： 移動到解決方案中的指定符號。
...
輸入 | 函式
------------ | ---
**鍵盤** | 將文本游標放在類型名稱的某處，然後按**Ctrl**+**F12**
**滑鼠** | 以滑鼠右鍵按一下類型名稱，然後選取 [移至實作]****

## <a name="go-to-base"></a>移至基底

使用"轉到基本"，可以向上導航所選元素的繼承鏈。 如果有多個結果，您將看到這些結果在 **"轉到基本"** 視窗中列出：

輸入 | 函式
------------ | ---
**鍵盤** | 將文本游標放在類型名稱的某處，然後按**Alt**+**Home**
**滑鼠** | 按右鍵類型名稱並選擇 **"轉到基本"**
...
輸入 | 函式
------------ | ---
**鍵盤** | 將文本游標放在類型名稱的某處，然後按**Ctrl**+**K** **，Ctrl**+**T**
...
代碼編輯器中的"結構視覺化器"功能顯示*結構指引線*- 垂直虛線，指示代碼庫中匹配的大括弧。 這可讓您更輕鬆地查看邏輯區塊的開始和結束位置。

![結構視覺化檢視](../ide/media/vside_structure_visualizer.png)

要禁用結構輔助線，請轉到 **"工具** > **選項** > **文字編輯器** > **"，** 然後清除 **"顯示結構指南行"** 框。
...
- [查看呼叫層次結構](../ide/reference/call-hierarchy.md)
