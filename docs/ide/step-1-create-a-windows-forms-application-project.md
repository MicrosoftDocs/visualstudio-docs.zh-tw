...
ms.locfileid: "77579909"
---
# <a name="step-1-create-a-windows-forms-app-project"></a>第 1 步：創建 Windows 表單應用專案

創建圖片檢視器時，第一步是創建 Windows 表單應用專案。

::: moniker range="vs-2017"

## <a name="open-visual-studio-2017"></a>開啟 Visual Studio 2017

1. 在功能表列上，選擇 **"檔** > **新專案** > **"。** 對話方塊看起來應該類似下列螢幕擷取畫面。

     ![[新增專案] 對話方塊](../ide/media/newprojectdialogcallouts.png)<br/>[新增專案]****** *對話方塊*

2. 在 **"新專案"** 對話方塊的左側，選擇**Visual C#** 或**視覺化基本**，然後選擇**Windows 桌面**。

3. 在專案範本清單中，選擇**Windows 表單應用 （.NET 框架）。** 將新表單命名為 *PictureViewer*，然後選擇 [確定]**** 按鈕。
...
1. 在 [建立新專案]**** 視窗中，在搜尋方塊內輸入或鍵入 *Windows Forms*。 接下來，從 **"專案類型**"清單中選擇 **"桌面**"。
   ...
   > [!NOTE]
   > 如果看不到**Windows 表單應用 （.NET Framework）** 範本，則可以從 **"創建新專案"** 視窗安裝該範本。 在 [找不到您要找的資料嗎?]**** 訊息中，選擇 [安裝更多工具和功能]**** 連結。
   >
...
1. 在 [設定您的新專案]**** 視窗的 [專案名稱]**** 方塊中鍵入或輸入 *PictureViewer*。 然後，選擇 **"創建**"。

::: moniker-end

Visual Studio 為您的應用創建解決方案。 解決方案充當應用所需的所有專案和檔的容器。 本教學課程稍後將會詳細說明這些詞彙。

## <a name="about-the-windows-forms-app-project"></a>關於 Windows 表單應用專案

1. 開發環境包含三個視窗：主視窗、[方案總管]**** 和 [屬性]**** 視窗。

     如果缺少這些視窗中的任何一個，則可以還原預設視窗佈局。 在功能表列上，選擇 **"視窗** > **重置視窗佈局**"。

     您也可以使用功能表命令顯示視窗。 在功能表列上，選擇 **"查看** > **屬性視窗**"或 **"解決方案資源管理器**"。
    ...
    * **方案總管視窗**：在這個視窗中，您可以檢視和巡覽至方案中的所有項目。
    ...
    如果您選擇一個檔案，[屬性]**** 視窗的內容會隨著變更。 如果打開代碼檔（以*C#* 中的 .cs 結尾，在 Visual Basic 中*以 .vb*結尾），則將顯示代碼檔的代碼檔或設計器。 設計工具是一個視覺化介面，您可以將控制項 (例如按鈕和清單) 加入該介面中。 Visual Studio 表單的設計工具稱為 **Windows Forms 設計工具**。
...
1. 在功能表列上，選擇 **"全部保存檔** > **Save All**"。

     作為替代方法，請選擇工具列上的"**全部保存"** 按鈕，如下圖所示。
...
* 要轉到下一個教程步驟，請參閱**[步驟 2：運行應用](../ide/step-2-run-your-program.md)**。
