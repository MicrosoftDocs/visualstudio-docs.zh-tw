...
2. 按右鍵環境並選擇**安裝 Python 包**。 此命令會開啟 [套件]**** 索引標籤上的 [Python 環境]**** 視窗。
...
2. 按右鍵環境並選擇 **"管理 Python 包..."。** 此命令在 **"包 （PyPI）"** 選項卡上打開**Python 環境**視窗。
...
1. 按右鍵**解決方案資源管理器**中的專案，然後選擇"**添加>新專案**"。
...
1. 您可能已經注意到，"**添加>新專案**"對話方塊顯示許多可以添加到 Python 專案的其他類型檔，包括 Python 類、Python 包、Python 單元測試 *、Web.config*檔等。 一般而言，這些項目範本 顧名思義是使用實用的未定案程式碼建立檔案的好方法。
...
2. 按右鍵**解決方案資源管理器**中的專案並選擇**屬性**。 然後選取 [偵錯]**** 索引標籤，將 [連接埠號碼]**** 屬性設定為 `4449`。 這個步驟可確保 Visual Studio 以 `localhost:4449` 啟動瀏覽器，以符合程式碼中的 `app.run` 引數。

3. 選擇**調試>無需調試即可啟動**（**Ctrl**+**F5**），它保存對檔的更改並運行應用程式。
