...
如需取代模式中所用規則運算式的資訊，請參閱[規則運算式中的替代項目 (.NET 指南)](/dotnet/standard/base-types/substitutions-in-regular-expressions)。 若要使用已編號的擷取群組，則指定已編號群組的語法為 `$1`，而指定考慮中群組的語法為 `(x)`。 例如，群組規則運算式 `(\d)([a-z])` 在下列字串中找到四個相符項目：**1a 2b 3c 4d**。 取代字串 `z$1` 會將該字串轉換為 **z1 z2 z3 z4**。
