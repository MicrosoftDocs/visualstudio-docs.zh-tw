...
Visual Studio 解決方案可以包含以不同 .NET 版本為目標的專案。  不過，請注意，您只能使用單一組建的參考條件，或以遞迴方式為每個版本建立不同的二進位檔，來建立單一版本的 .NET。  如需目標 Framework 的詳細資訊，請參閱[目標 Framework](/dotnet/standard/frameworks)。
