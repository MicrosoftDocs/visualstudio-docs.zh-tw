...
1. 使用下列其中一種方法來停止您的應用程式：
    ...
    > [!NOTE]
    > 當您從 IDE 內部執行應用程式時，它稱為「偵錯工具」，因為您通常會這麼做，以便找出並修正應用程式中的 bug （錯誤）。 雖然此應用程式很小，而且還不會這麼做，但它仍然是真正的程式。 您遵循相同程序執行和偵錯其他程式。 若要深入了解偵錯，請參閱[偵錯工具簡介](../debugger/debugger-feature-tour.md)。
