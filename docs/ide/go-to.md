...
根據預設，會在所有方案項目中搜尋指定的項目。 不過，您可以在搜尋詞彙前面引用特定字元，以將程式碼搜尋限制為特定項目類型。 您還可以通過選擇 **"轉到"** 對話方塊工具列上的按鈕來快速更改搜索篩選器。 變更類型篩選的按鈕是在左邊，變更搜尋範圍的按鈕則是在右邊。
...
如果您在程式碼中使用 [camel casing](https://en.wikipedia.org/wiki/Camel_case) (駝峰式命名法)，則只要輸入程式碼項目名稱的大寫字母，就可以更快速地找到程式碼項目。 例如，如果代碼的類型`CredentialViewModel`稱為 ，則可以通過選擇**Type**篩選器 **（t**）， 然後在"轉到"對話方塊中只輸入名稱 （`CVM`） 的大寫字母來縮小搜尋範圍。 如果您的程式碼名稱很長，則這個功能十分有用。
...
- [轉到"行"對話方塊](../ide/reference/go-to-line.md)
