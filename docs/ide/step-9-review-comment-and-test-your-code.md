...
ms.locfileid: "70293640"
---
# <a name="step-9-review-comment-and-test-your-code"></a>步驟 9：檢閱、註解及測試您的程式碼

您接下來要將註解加入至程式碼。 註解是一種不會變更程式運作方式的附註。 它可讓讀取程式碼的人容易了解您的程式碼有何用途。 將註解加入至您的程式碼是個好習慣。

在C#中，兩個正斜線（//）會將一行標示為批註。 在 Visual Basic 中，使用單引號 (') 將一行標示成註解。 加入註解後，請測試程式。 較好的做法是，經常在您處理專案時執行和測試程式碼，以便在程式碼變得更複雜之前早期攔截並修正所有問題。 這稱為「反覆測試」。
...
1. 執行程式之前，選擇 [**全部儲存**] 工具列按鈕來儲存您的程式，看起來應該類似下列螢幕擷取畫面。

     ![[全部儲存] 工具列按鈕](../ide/media/express_iconsaveall.png)<br>
***全部儲存****按鈕*

     或者，若要儲存您的程式，請從功能表**欄選擇** > [檔案] [**全部儲存**] （或按**Ctrl** + **Shift** + **S**）。 最好養成儘早並經常儲存的習慣。
...
1. 選擇 [顯示圖片] 按鈕，執行您剛撰寫的程式碼。 首先，程式會開啟 [開啟檔案] 對話方塊。 確認篩選出現在對話方塊底部的 [檔案類型] 下拉式清單中。 然後，瀏覽至圖片並開啟它。 在 [我的文件] 資料夾的 [我的圖片\範例圖片] 資料夾內，您通常可以找到 Windows 作業系統隨附的範例圖片。
