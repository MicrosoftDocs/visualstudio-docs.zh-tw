...
此樣式規則 (規則識別碼 IDE0003 與 IDE0009) 可以套用到欄位、屬性、方法或事件。 **true** 值表示希望程式碼符號在 C# 中以 `this.` 開頭或在 Visual Basic 中以 `Me.` 開頭。 **false** 值表示希望程式碼項目前面_不_要加上 `this.` 或 `Me.`。
...
- 當此規則設定為 **true** 時，偏好使用 autoproperties，而非包含私用支援欄位的屬性。
- 當此規則設定為 **false** 時，偏好使用包含私用支援欄位的屬性，而非 autoproperties。
...
- 當此規則設定為 **true** 時，偏好搭配使用 Null 檢查與模式比對，而非 object.ReferenceEquals。
- 當此規則設定為 **false** 時，偏好使用 object.ReferenceEquals，而非 Null 檢查與模式比對。
...
- 當此規則設為 **true** 時，建議使用具有三元條件式的指派，而不是 if else 陳述式。
- 當此規則設為 **false** 時，建議使用具有 if else 陳述式的指派，而不是三元條件式。
...
- 當此規則設為 **true** 時，建議 return 陳述式使用三元條件式，而不是 if else 陳述式。
- 當此規則設為 **false** 時，建議 return 陳述式使用 if else 陳述式，而不是三元條件式。
...
- 當此規則設定為 **true** 時，在方法宣告的空白參數清單括弧內插入空格。
- 當此規則設定為 **false** 時，在方法宣告的空白參數清單括弧內移除空格。
...
- 當此規則設定為 **true** 時，在方法呼叫名稱與左括弧之間插入空格。
- 當此規則設定為 **false** 時，在方法呼叫名稱與左括弧之間移除空格。
...
- 當此規則設定為 **true** 時，在空白引數清單括弧內插入空格。
- 當此規則設定為 **false** 時，在空白引數清單括弧內移除空格。
