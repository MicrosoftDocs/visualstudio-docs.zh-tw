...
- 當此規則設為 **always_for_clarity** 時，建議使用括號來說明關係運算子 (`>`、`<`、`<=`、`>=`、`is`、`as`、`==`、`!=`) 優先順序。
...
- 當此規則設定為 **true** 時，偏好使用 Auto 屬性，而非包含私用支援欄位的屬性。
- 當此規則設定為 **false** 時，偏好使用包含私用支援欄位的屬性，而非 Auto 屬性。
