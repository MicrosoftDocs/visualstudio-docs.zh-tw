...
您的圖片檢視器應用程式已完成，並已準備好可執行。 您可以執行應用程式，並設定 <xref:System.Windows.Forms.PictureBox>的背景色彩。 若要深入瞭解，請嘗試變更表單的色彩、自訂按鈕和核取方塊，以及變更表單的屬性，以改善應用程式。
...
1. 選擇色彩以設定 PictureBox 背景色彩。 仔細查看 `backgroundButton_Click()` （或，`BackgroundButton_Click()`）方法，以瞭解其運作方式。
