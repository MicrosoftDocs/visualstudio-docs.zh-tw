...
您可以指定自訂建置事件，在建置開始之前或完成之後自動執行命令。 例如，您可以在生成開始之前運行 *.bat*檔，或在生成完成後將新檔案複製到資料夾中。 只有在建置成功到達建置流程中的這些點時，建置事件才會執行。

如需您所使用之程式設計語言的特定資訊，請參閱下列主題：

- 視覺化基礎 --[如何：指定建置事件（可視基本）](../ide/how-to-specify-build-events-visual-basic.md)。
...
建置事件遵循與 DOS 命令相同的語法，不過您可以使用巨集更輕鬆地建立建置事件。 有關可用宏的清單，請參閱[預建置事件/生成後事件命令列對話方塊](../ide/reference/pre-build-event-post-build-event-command-line-dialog-box.md)。

為了獲得最佳結果，請依照下列格式秘訣：

- 在運行`call` *.bat*檔的所有建置事件之前添加語句。
...
- 視需要包含萬用字元。
   ...
   示例：`for %I in (*.txt *.doc *.html) do copy %I c:\`*我的目錄*`\`
...
- [MS 構建特殊字元](../msbuild/msbuild-special-characters.md)
