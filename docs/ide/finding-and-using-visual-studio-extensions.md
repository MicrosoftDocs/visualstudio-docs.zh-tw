...
使用 [延伸模組和更新] 對話方塊，即可安裝和管理 Visual Studio 延伸模組。 若要開啟 [延伸模組和更新] 對話方塊，選擇 [工具]  >  [延伸模組和更新]，或是在 [快速啟動] 視窗中輸入 [延伸模組]。

![Visual Studio 中的 [延伸模組和更新] 對話方塊](media/finding-using-visual-studio-extensions/extensions-and-updates.png)
...
若要保護您的系統以避免可能包含錯誤或惡意程式碼的擴充功能，您可以限制只在使用一般使用者權限執行 Visual Studio 時才能載入個別使用者擴充功能。 這表示當使用系統管理使用者權限執行 Visual Studio 時，個別使用者擴充功能會被停用。 若要這樣做，請移至 [延伸模組和更新] 選項頁面 ([工具/選項]  >  [環境]  >  [延伸模組和更新])。 清除 [以系統管理員身分執行時載入個別使用者擴充功能]  核取方塊，然後重新啟動 Visual Studio。

## <a name="automatic-extension-updates"></a>自動延伸模組更新

Visual Studio Marketplace 有新版本可用時，就會自動更新延伸模組。 偵測到新版本的延伸模組，並且會在背景安裝。 下一次啟動 Visual Studio 時，就會執行新版本的延伸模組。
