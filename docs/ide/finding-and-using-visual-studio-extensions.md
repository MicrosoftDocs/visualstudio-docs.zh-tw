...
延伸模組是在 Visual Studio 內執行的程式碼套件，提供新的或改良的功能。 延伸模組可能是控制項、範例、範本、工具或其他元件，可將功能新增至 Visual Studio，例如[Live Share](https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsls-vs)或[Visual Studio IntelliCode](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.VSIntelliCode)。

如需建立 Visual Studio 擴充功能的相關資訊，請參閱[VISUAL STUDIO SDK](../extensibility/visual-studio-sdk.md)。 如需使用擴充功能的相關資訊，請參閱[Visual Studio Marketplace](https://marketplace.visualstudio.com)上的個別擴充功能頁面。
...
## <a name="manage-extensions-dialog-box"></a>[管理延伸模組] 對話方塊
...
## <a name="find-and-install-extensions"></a>尋找並安裝擴充功能

::: moniker range="vs-2017"

您可以從[Visual Studio Marketplace](https://marketplace.visualstudio.com)或 Visual Studio 中的 [擴充功能和更新] 對話方塊安裝延伸模組。

若要在 Visual Studio 中安裝延伸模組：
...
您可以從[Visual Studio Marketplace](https://marketplace.visualstudio.com)或 Visual Studio 中的 [管理延伸模組] 對話方塊安裝延伸模組。
...
> [!NOTE]
> 您可以停用 VSIX 擴充功能，而不是使用 MSI 安裝的延伸模組。 MSI 安裝的延伸模組只能卸載。
...
當 Visual Studio Marketplace 上有新版本可用時，就會自動更新延伸模組。 偵測到新版本的延伸模組，並且會在背景安裝。 當您下一次開啟 Visual Studio 時，將執行新版的擴充功能。

如果您想要停用自動更新，您可以停用所有延伸模組的功能，或僅針對特定的延伸模組。
...
- 選擇 [停用此延伸模組]。 Visual Studio 會停用延伸模組，並可讓您知道是否需要重新啟動系統，以讓停用生效。 您可以視需要在 [工具] > [延伸模組和更新] 對話方塊中重新啟用延伸模組。
...
- 選擇 [一律不再顯示此訊息]。
  ...
  - 如果通知與無回應，則當此延伸模組與無回應相關聯時，整合式開發環境（IDE）就不會再顯示通知。 Visual Studio 仍會顯示此延伸模組的當機相關通知，以及其他延伸模組的當機與無回應相關通知。
...
> [!NOTE]
> UI 無回應或當機通知表示當 UI 無回應或者發生當機時，延伸模組的其中一個模組是在堆疊上。 這不一定表示延伸模組本身為問題。 延伸模組可能會呼叫屬於 Visual Studio 一部分的程式碼，進而導致無回應的 UI 或損毀。 不過，如果導致 UI 無回應或當機的延伸模組對您而言不重要，則此通知可能仍然十分有用。 在此情況下，停用延伸模組可避免未來發生 UI 無回應或損毀，而且不會影響產能。
