---
title: 逐步解說：使用 C# 或 Visual Basic 建立簡單的應用程式
ms.date: 10/03/2017
...
![選擇設定對話方塊](../ide/media/exploreide-settings.png)
...
![已套用一般設定的 IDE](../ide/media/exploreide-idewithgeneralsettings.png)
...
1. 建立新的專案。 在功能表列上，選取 [檔案] > [新增] > [專案]。
     ...
     ![在功能表上依序選擇 [檔案]、[開新檔案] 和 [專案]](../ide/media/exploreide-filenewproject.png)

1. 在 [新增專案] 對話方塊中，選取 [已安裝] > [Visual C#] (或 [Visual Basic]) > [Windows 桌面] 分類，然後選取 [WPF 應用程式 (.NET Framework)] 範本。 將專案命名為 **HelloWPFApp**。

     ![Visual Studio [新增專案] 對話方塊中的 WPF 應用程式範本](../ide/media/exploreide-newprojectcsharp.png)

1. 選取 [確定]。

Visual Studio 會建立 HelloWPFApp 專案和方案，而且**方案總管**會顯示各種不同檔案。 **WPF 設計工具**會在分割檢視中顯示 *MainWindow.xaml* 的設計檢視和 XAML 檢視。 您可以滑動分隔器來增加或減少顯示任一檢視。 您可以選擇只查看視覺檢視，或只查看 XAML 檢視。 (如需詳細資訊，請參閱[適用於 Windows Form 開發人員的 WPF 設計工具](http://msdn.microsoft.com/47ad0909-e89b-4996-b4ac-874d929f94ca))。下列項目會在 **方案總管**中出現：
...
1. 將 [ **檔案名稱** ] 屬性變更為 `Greetings.xaml`。
     ...
     ![包含反白顯示之檔名的 [屬性] 視窗](../ide/media/exploreide-filenameinpropertieswindow.png)
...
2. 在 [工具箱] 中展開 [通用 WPF 控制項] 節點以查看 TextBlock 控制項。
     ...
     ![已反白顯示 [TextBlock] 控制項的 [工具箱]](../ide/media/exploreide-textblocktoolbox.png)
...
1. 在 [工具箱] 中，尋找 **RadioButton** 控制項。
     ...
     ![已選取 [RadioButton] 控制項的 [工具箱] 視窗](../ide/media/exploreide-radiobuttontoolbox.png)
...
2. 在 XAML 檢視中，將 Button 控制項的 [內容] 值從 `Content="Button"` 變更為 `Content="Display"`，然後儲存變更。
     ...
     ![具有控制項標籤的問候表單](../ide/media/exploreide-greetingswithconrollabels.png)
...
2. 選擇 [偵錯] > [停止偵錯] 停止偵錯工具。
     ...
     ![[偵錯] 功能表上的 [停止偵錯] 命令](../ide/media/exploreide-stopdebugging.png)

我們在這個逐步解說開始時，將 *MainWindow.xaml* 重新命名為 *Greetings.xaml*，但是程式碼仍然參照 *MainWindow.xaml* 作為應用程式的啟動 URI，因此專案無法啟動。
...
2. 依序選取 [ **偵錯**] 和 [ **切換中斷點**]，以新增中斷點。
     ...
     ![[偵錯] 功能表上的 [切換中斷點] 命令](../ide/media/exploreide-togglebreakpoint.png)
...
1. 在主功能表上，選取 [組建] > [清除方案]，刪除在上一個組建期間建立的中繼檔和輸出檔。 這並非必要動作，但它可清除偵錯建置輸出。
     ...
     ![[建置] 功能表上的 [清除方案] 命令](../ide/media/exploreide-cleansolution.png)

2. 使用工具列上的下拉式控制項 (它目前的名稱是 "Debug")，將 HelloWPFApp 的建置組態從 [偵錯] 變更為 [發行]。

     ![已選取 [發行] 的 [標準] 工具列](../ide/media/exploreide-releaseversion.png)

3. 選擇 [建置] > [建置方案] 來建置方案。

     ![[建置] 功能表上的 [建置方案] 命令](../ide/media/exploreide-buildsolution.png)

恭喜您完成此逐步解說！ 您可在方案和專案目錄 (*...\HelloWPFApp\HelloWPFApp\bin\Release*) 下找到您建置的 *.exe*。
