...
工具列是 Visual Studio 功能表列下的按鈕列，有助您提高撰寫程式碼的效率。 例如，您可以切換 IntelliSense 完成模式、增加或減少行縮排，也可以為不要編譯的程式碼加上註解 ([IntelliSense](../../ide/using-intellisense.md) 是程式碼撰寫的輔助工具，可顯示其他項目中符合的方法清單)。在本節中，我們會為一些程式碼加上註解。
