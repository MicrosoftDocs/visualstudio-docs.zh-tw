...
雖然示範應用程式是 Visual Basic，但大部分的功能都適用于C#、 C++、 F#、Python、JavaScript 和 Visual Studio 支援的其他語言（不F#支援 [編輯後繼續]）。 F# 和 JavaScript 不支援 [自動變數] 視窗)。 螢幕擷取畫面位於 Visual Basic。
...
1. 在 [建立新專案] 視窗的搜尋方塊中輸入或鍵入 ASP.NET。 接下來，從語言清單中選擇 **Visual Basic**，然後從平台清單中選擇 **Windows**。 
   ...
   ![選擇主控台應用程式的 [Visual Basic] 範本（.NET Core）](../visual-basic/media/vs-2019/get-started-create-console-project.png)

   > [!NOTE]
   > 如果您未看到 [主控台應用程式 (.NET Core)] 範本，您可以從 [建立新專案] 視窗中安裝。 在 [找不到你要尋找的項目嗎?] 訊息中，選擇 [安裝更多工具和功能] 連結。 接下來，在 Visual Studio 安裝程式中選擇 **.NET Core 跨平台開發**工作負載。
...
1. 在*Program .vb*中，以下列程式碼取代所有的預設程式碼：
...
1. 再按一次**F11**鍵，逐步執行 `SendMessage` 方法。
     ...
     ![使用 F11 逐步執行程式碼](../visual-basic/media/get-started-f11-vb.png "F10 逐步執行")
...
1. 在方法呼叫中暫停時，請按下**F10**鍵（或選擇 [ **Debug >** 不進入函式]）一次。
     ...
     ![使用 F10 來跳過程式碼](../visual-basic/media/get-started-step-over-vb.png "F10 不進入函式")
...
1. 在 [程式碼編輯器] 中，將滑鼠游標移至 `SendMessage` 方法中的 `Console.WriteLine` 方法上，直到您**按一下**[![執行](../../debugger/media/dbg-tour-run-to-click.png "處 runtoclick")] 按鈕的綠色回合，才會出現在左邊。 按鈕的工具提示會顯示「執行到這裡」。
...
1. 在 `name += letters[i]` 語句上暫停時，將滑鼠停留在 `letters` 變數上，您會看到它是預設值，也就是陣列中第一個元素的值，`"f"c`。
