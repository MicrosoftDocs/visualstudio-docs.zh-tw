...
1. 在 `Main` 函式的 `shape.Draw` 方法呼叫中暫停時，請按下 **F11** 鍵 (或選擇 [偵錯] > [逐步執行]) 推進到 `Rectangle` 類別的程式碼。
     ...
     ![使用 F11 逐步執行程式碼](../visual-basic/media/get-started-f11-vb.png "F11 逐步執行")
...
2. 按一下 [**執行] 按一下**[![執行] 按鈕，然後按一下](../../debugger/media/dbg-tour-run-to-click.png "處 runtoclick")。
    ...
    偵錯工具將會推進到 `Triangle` 類別的 `Console.WriteLine` 方法實作。 （如果偵錯工具會先在您稍早設定的中斷點上暫停，請使用 [執行] 再次**按一下**以讓偵錯工具前進到 `Console.WriteLine`）。
...
1. 查看程式碼編輯器底部的 [自動變數] 視窗。
     ...
     ![檢查 [自動變數] 視窗中的變數](../visual-basic/media/get-started-autos-window-vb.png "自動變數視窗")
