...
1. 在左窗格中，展開 [Visual C#]，然後選擇 [測試] 類別。 在中間窗格中，選擇 [單元測試專案 (.NET Framework)] 專案範本。 將專案命名為 **QuickTest**，然後選擇 [確定] 按鈕。

   ![Visual Studio 中測試專案的 [新增專案] 對話方塊](media/tutorial-projects-new-test-project-cs.png)

   第二個專案會新增至 [方案總管]，並在編輯器中開啟名為 *UnitTest1.cs* 的檔案。

   ![包含兩個專案的 Visual Studio 方案總管](media/tutorial-projects-solution-explorer-cs.png)
...
1. 選擇 [QuickTest] 專案中的 [參考] 節點，然後從右鍵功能表或操作功能表中選擇 [新增參考]。
   ...
   ![新增參考功能表](media/tutorial-projects-add-reference-cs.png)
...
1. 回到 **QuickDate** 專案，開啟尚未開啟的 *Calendar.cs* 檔案，並將下列 [using 陳述式](/dotnet/csharp/language-reference/keywords/using-statement)和 <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute> 屬性新增至檔案頂端，以解決測試專案中的錯誤。
   ...
   ![CSharp 程式碼](media/tutorial-projects-code-cs.png)
...
1. 在方案總管中，選取 **QuickTest** 專案。 從右鍵功能表或操作功能表中，選擇 [屬性]，或只按 **Alt**+**Enter** 鍵。
   ...
   ![專案屬性](media/tutorial-projects-properties-cs.png)

1. 探索專案屬性頁的一些其他索引標籤 (例如 [建置] 和 [設定])。 不同專案類型的這些索引標籤不同。
...
如果您想要進一步探索 Visual Studio，請考慮遵循其中一個 [C# 教學課程](index.yml)，來建立應用程式。
