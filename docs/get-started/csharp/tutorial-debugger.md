...
1. 展開 `letters` 變數以查看其屬性，包括變數包含的所有元素。
     ...
     ![醒目提示 [名稱 + = 字母 [I]] 語句的 Visual Studio 偵錯工具的螢幕擷取畫面，以及顯示信件陣列中元素的下拉式清單。](../csharp/media/get-started-view-data-tip.png)
...
1. 按下 **F10** (或 **Debug**  >  ) 幾次，以逐一查看迴圈中的數次 `for` 、在中斷點再次暫停，然後在 `name` 變數上每次檢查其值時將滑鼠停留在變數上。

     ![Visual Studio 偵錯工具的動畫屏幕快照，顯示在偵錯工具中按下 F10 鍵來「跳過」並逐一查看迴圈的效果。](../csharp/media/get-started-data-tip.gif)
