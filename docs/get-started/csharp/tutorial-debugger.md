...
3. 在 [新增專案]**** 對話方塊的左窗格中，展開 [C#]****，然後選擇 [.NET Core]****。 在中間窗格中，選擇 [主控台應用程式 (.NET Core)]****。 然後命名專案*開始調試*。
...
1. 在 [建立新專案]**** 視窗的搜尋方塊中輸入或鍵入 ASP.NET**。 接下來，從語言清單中選擇 **C#**，然後從平台清單中選擇 **Windows**。 
   ...
   ![為主控台應用（.NET 核心）選擇 C# 範本](../csharp/media/vs-2019/get-started-create-console-project.png)
...
1. 在"**配置新專案**"視窗中，在 **"專案名稱**"框中鍵入或輸入 *"入門調試*"。 然後，選擇 **"創建**"。
...
1. 在*Program.cs*中，將所有預設代碼替換為以下代碼：
...
1. 再按**F11**一次以步`SendMessage`進方法。
     ...
     ![使用 F11 進入代碼](../csharp/media/get-started-f11.png "F10 步進")
...
1. 在方法調用時暫停時，按**F10（** 或選擇**調試>單步**執行） 一次。
     ...
     ![使用 F10 單一步驟代碼](../csharp/media/get-started-step-over.png "F10 步長")
...
1. 在代碼編輯器中，向下滾動並懸停在方法`Console.WriteLine`中的方法上`SendMessage`，直到左側出現綠色 **"運行以按一下**!["](../../debugger/media/dbg-tour-run-to-click.png "運行點擊")按鈕。 按鈕的工具提示會顯示「執行到這裡」。
     ...
     ![使用"運行到按一下"功能](../csharp/media/get-started-run-to-click.png "執行至點選處")
...
1. 暫停`name += letters[i]`語句時，將滑鼠懸停在變數上`letters`，您將看到它的預設值，即陣列中第一個元素的值`char[10]`。
...
1. 展開`letters`變數以顯示它包含的元素。
     ...
     ![檢查區域變數視窗中的變數](../csharp/media/get-started-locals-window.png "本機視窗")
...
2. 按一下**F11**幾次，直到看到調試器在`SendMessage`方法中暫停。 查看 [呼叫堆疊]**** 視窗。
    ...
    ![檢查呼叫堆疊](../csharp/media/get-started-call-stack.png "檢查呼叫堆疊")
