...
[**容器**] 視窗會在您啟動容器化 .net 專案時自動開啟。 若要在 Visual Studio 中隨時查看您的容器，請使用**Ctrl** + **Q**來啟動 [Visual Studio 搜尋] 方塊，然後輸入 `Containers` 並選擇第一個專案。 您也可以從主功能表開啟 [**容器**] 視窗。 使用功能表路徑**視圖**[  >  **其他 Windows**  >  **容器**]。  
...
> [!NOTE]
> 當您執行時，Visual Studio 會自動將 stdout 和 stderr 重新導向至 [**輸出**] 視窗，因此，使用**Ctrl**F5 從 Visual Studio 啟動的 windows 容器 + **F5**將不會在此索引標籤中顯示記錄; 請改用 [**輸出**] 視窗。
