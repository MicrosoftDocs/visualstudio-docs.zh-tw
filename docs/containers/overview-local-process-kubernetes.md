...
ms.date: 06/02/2020
ms.topic: conceptual
description: 描述使用本機進程搭配 Kubernetes 來將您的開發電腦連接到 Kubernetes 叢集的流程
...
> [!WARNING]
> Kubernetes 的本機程式僅供開發和測試案例使用。 不適用於生產叢集或主動使用中的即時服務。

## <a name="using-local-process-with-kubernetes"></a>使用本機進程搭配 Kubernetes

若要在 Visual Studio 中使用本機進程搭配 Kubernetes，您需要在已安裝*ASP.NET 和 網頁程式開發*工作負載的 Windows 10 上執行[Visual Studio 2019][visual-studio] 16.7 版 Preview 4 或更新版本，並安裝[本機進程 Kubernetes 延伸][lpk-extension]模組。 當您搭配 Kubernetes 使用本機進程來建立 Kubernetes 叢集的連線時，您可以選擇將叢集中現有 pod 的所有流量，重新導向至您的開發電腦。

> [!NOTE]
> 搭配使用本機進程與 Kubernetes 時，系統會提示您輸入服務的名稱，以重新導向至您的開發電腦。 此選項可讓您輕鬆地識別要重新導向的 pod。 Kubernetes 叢集與開發電腦之間的所有重新導向皆適用于 pod。

使用 Kubernetes 的本機進程建立與您叢集的連線時，會：
...
* 更新您的主機檔案，將叢集上的服務對應至開發電腦上的本機 IP 位址。 這些主機檔案專案可讓程式碼在您的開發電腦上執行，以對叢集中執行的其他服務提出要求。 若要更新主機檔案，使用 Kubernetes 的本機進程會在連線到您的叢集時，要求您的開發電腦上有系統管理員存取權。
* 開始在您的開發電腦上執行和偵錯工具代碼。 如有必要，使用 Kubernetes 的本機進程將會停止目前正在使用這些埠的服務或處理常式，以釋放開發電腦上所需的埠。

建立與叢集的連線之後，您可以在電腦上以原生方式執行和偵測程式碼，而不需要容器化，而且程式碼可以直接與叢集的其餘部分互動。 遠端代理程式接收的任何網路流量都會重新導向至連線期間指定的本機埠，讓原生執行的程式碼可以接受和處理該流量。 您叢集的環境變數、磁片區和秘密可供開發電腦上執行的程式碼使用。 此外，由於主機檔案專案和埠轉送是由本機程式以 Kubernetes 新增至您的開發人員電腦，因此您的程式碼可以使用叢集中的服務名稱，將網路流量傳送至叢集上執行的服務，並將流量轉送至叢集中正在執行的服務。 在您的開發電腦與您的叢集之間進行連線時，會在您的整個叢集中路由傳送流量。

此外，Kubernetes 的本機進程提供一種方法，可透過檔案，將環境變數和裝載的檔案複寫至您的開發電腦上叢集內的 pod `KubernetesLocalProcessConfig.yaml` 。 您也可以使用這個檔案來建立新的環境變數和磁片區裝載。

> [!NOTE]
> 在連線到叢集的持續期間 (加上15分鐘的時間) ，Kubernetes 的本機進程會使用本機電腦上的系統管理員許可權，執行名為 *EndpointManager* 的進程。

## <a name="additional-configuration-with-kuberneteslocalprocessconfigyaml"></a>使用 KubernetesLocalProcessConfig 的其他設定。 yaml

此檔案可 `KubernetesLocalProcessConfig.yaml` 讓您將環境變數和已掛接的檔案複寫至叢集中的 pod。 如需其他設定選項的詳細資訊，請參閱 [使用 Kubernetes 設定本機進程][using-config-yaml]。

## <a name="using-routing-capabilities-for-developing-in-isolation"></a>使用路由功能來以隔離方式進行開發

依預設，Kubernetes 的本機進程會將服務的所有流量重新導向至您的開發電腦。 您也可以選擇使用路由功能，僅將源自子域的要求重新導向至您的開發電腦。 這些路由功能可讓您使用本機進程搭配 Kubernetes，以隔離方式進行開發，並避免中斷叢集中的其他流量。

下列動畫顯示兩位開發人員在相同叢集上進行隔離的開發人員：

![示範隔離的動畫 GIF](media/local-process-kubernetes/lpk-graphic-isolated.gif)

當您啟用隔離工作時，除了連接到您的 Kubernetes 叢集之外，Kubernetes 的本機進程還會執行下列作業：
...
如果 Kubernetes 的本機進程偵測到您的 Kubernetes 叢集上已啟用 Azure Dev Spaces，系統會提示您停用 Azure Dev Spaces，您才能搭配 Kubernetes 使用本機進程。
...
當您中斷與叢集的連線時，依預設，使用 Kubernetes 的本機進程將會移除所有 envoy pod 和重複的服務。 

> 記路由管理員部署和服務將會在您的命名空間中繼續執行。 若要移除部署和服務，請針對您的命名空間執行下列命令。
>
> ```azurecli
...
具有 Kubernetes 的本機進程有下列限制：

* Kubernetes 的本機進程會將單一服務的流量重新導向至您的開發電腦。 您無法使用本機進程搭配 Kubernetes 來重新導向多個服務。
* 服務必須由單一 pod 支援，才能連接至該服務。 您無法連接到具有多個 pod 的服務，例如具有複本的服務。
* Pod 可能只有在該 pod 中執行的單一容器，才能讓具有 Kubernetes 的本機進程順利連接。 使用 Kubernetes 的本機進程無法連線到具有其他容器（例如服務網格所插入的側車容器）之 pod 的服務。
* 使用 Kubernetes 的本機進程需要較高的許可權，才能在您的開發電腦上執行，以編輯主機檔案。
* 使用 Kubernetes 的本機進程無法在已啟用 Azure Dev Spaces 的叢集上使用。

### <a name="local-process-with-kubernetes-and-clusters-with-azure-dev-spaces-enabled"></a>Kubernetes 和叢集的本機進程已啟用 Azure Dev Spaces

您無法在已啟用 Azure Dev Spaces 的叢集上使用本機進程搭配 Kubernetes。 如果您想要在啟用 Azure Dev Spaces 的叢集上使用本機進程搭配 Kubernetes，則必須先停用 Azure Dev Spaces，再連線到叢集。

## <a name="next-steps"></a>後續步驟

若要開始使用本機進程搭配 Kubernetes，以連接到您的本機開發電腦至您的叢集，請參閱 [使用本機進程與 Kubernetes](local-process-kubernetes.md)。
