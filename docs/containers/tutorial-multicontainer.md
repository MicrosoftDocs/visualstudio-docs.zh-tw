...
將專案新增至相同的方案，並呼叫它*MyWebAPI*。 選取 [ **API** ] 做為專案類型，並清除 [針對**HTTPS 設定**] 核取方塊。 在此設計中，我們只會使用 SSL 來與用戶端通訊，而不是在相同 web 應用程式中的容器之間進行通訊。 只`WebFrontEnd`需要 HTTPS。
...
1. 在專案中，開啟*Index.cshtml.cs*檔案`OnGet` ，並以下列程式碼取代方法。 `WebFrontEnd`
...
1. 現在，在 Web API 專案中，將程式碼新增至 [值] 控制器，以自訂 API 針對您從*webfrontend*新增的呼叫所傳回的訊息。
...
1. 在專案中，選擇 **新增 > 容器協調器支援**。 `WebFrontEnd` [ **Docker 支援選項**] 對話方塊隨即出現。
...
1. 立即在本機執行網站（F5 或 Ctrl + F5）以確認其如預期般運作。 如果所有專案都已正確設定，您會看到訊息 "Hello from webfrontend and webapi （with value 1）"。
