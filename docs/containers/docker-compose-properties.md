...
您可以使用設定來執行命令或自訂腳本，然後再啟動應用程式 `entrypoint` ，並使其相依于設定。 例如，如果您只需要執行，而不是在發行模式中，**只在「** 偵測」模式中設定憑證 `update-ca-certificates` ，您只能在 >docker-compose.yml 中新增下列程式碼*yml*： **Release**
