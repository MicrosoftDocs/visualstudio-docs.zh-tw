...
- 如果您的測試必須使用不是由 DLL 專案匯出的成員，並且受測專案是建置為動態程式庫，請考慮將它轉換成靜態程式庫。
  ...
  1. 在**解決方案資源管理器**中，在所測試專案的快顯功能表上，選擇**屬性**。 專案的 [屬性]**** 視窗便會開啟。
...
- 如果 DLL 不會匯出您要測試的函式，您可以將輸出 *.obj* 或 *.lib* 檔案新增至測試專案的相依性。
  ...
  2. 在 **"解決方案資源管理器**"中，在測試專案的快顯功能表上，選擇**屬性**。

  3. 選擇**配置屬性** > **連結器** > **輸入** > **其他依賴項**。

       選擇 [編輯]****，然後新增 **.obj** 或 **.lib** 檔案的名稱。 不要使用完整路徑名稱。

  4. 選擇**配置屬性** > **連結器** > **常規** > **附加庫目錄**。

       選擇 [編輯]****，然後新增 **.obj** 或 **.lib** 檔案的目錄路徑。 該路徑通常是位於受測專案的組建資料夾內。

  5. 選擇**配置屬性** > **VC++ 目錄** > **包括目錄**。
...
- [微軟.VisualStudio.測試控管.CppUnitTestFrameworkAPI參考](../test/microsoft-visualstudio-testtools-cppunittestframework-api-reference.md)
