...
即時單元測試可用於測試針對 .NET 框架或 .NET 核心的解決方案。 在本教程中，您將通過創建面向 .NET 標準的簡單類庫來學習使用即時單元測試，您將創建一個針對 .NET Core 的 MSTest 專案來測試它。
...
本教程要求您安裝了帶有 **.NET Core 跨平臺開發工作負載的**Visual Studio 企業版。

## <a name="create-the-solution-and-the-class-library-project"></a>建立方案和類別庫專案

首先創建名為實用程式庫的視覺化工作室解決方案，該解決方案由單個 .NET 標準類庫專案 StringLibrary 組成。

方案只是一或多個專案的容器。 若要建立空白方案，請開啟 Visual Studio 並執行下列作業：

1. **從頂級**視覺化工作室功能表中選擇 **"檔** > **新專案** > "。
...
現在，您已經創建了解決方案，您將創建一個名為 StringLibrary 的類庫，其中包含許多用於處理字串的擴充方法。

1. 在**解決方案資源管理器中**，按右鍵實用程式庫解決方案並選擇"**添加新** > **專案**"。
...
3. 在右側窗格中選擇**類庫 （.NET 標準）** 範本，並在 **"名稱**"文字方塊中輸入**StringLibrary，** 如下圖所示：
...
6. 從頂級視覺化工作室功能表中選擇 > **"生成生成解決方案**"。 **Build** 生成應成功。
...
3. 在右側窗格中選擇 **"單元測試專案 （.NET Core）"** 範本，並在 **"名稱**"文字方塊中輸入**StringLibrary 測試**，如下圖所示：
...
5. 單元測試專案無法自動存取所測試的類別程式庫。 您可以新增類別庫專案的參考，以提供測試程式庫存取權。 為此，請按右鍵`StringLibraryTests`專案並選擇"**添加** > **參考**"。 在 **"參考管理器"** 對話方塊中，請確保選擇了 **"解決方案**"選項卡，並選擇 StringLibrary 專案，如下圖所示。
...
8. 由於單元測試代碼包含一些非 ASCII 字元，Visual Studio 將顯示以下對話方塊，以警告如果以預設 ASCII 格式保存檔，某些字元將丟失。 選擇 [用其他編碼儲存]**** 按鈕。

   ![選擇檔案編碼](media/lut-start/ascii-encoding.png)

9. 在 **"提前保存選項"** 對話方塊的 **"編碼**"下拉清單中，選擇**Unicode（UTF-8，無簽名） - 字碼頁 65001，** 如下圖所示：

   ![選擇 UTF-8 編碼](media/lut-start/utf8-encoding.png)

10. 通過從頂級視覺化工作室功能表中選擇**生成** > **重建解決方案**來編譯單元測試專案。
...
1. 或者，選擇包含 StringLibrary 代碼的代碼視窗。 這要麼是 C# 專案的*Class1.cs，* 要麼是 Visual Basic 專案的*Class1.vb。* （此步驟允許您在啟用即時單元測試後直觀地檢查測試結果和代碼覆蓋率。

1. 從頂級視覺化工作室功能表中選擇 **"測試** > **即時單元測試** > **開始**"。

1. Visual Studio 會啟動 Live Unit Test，以自動執行所有測試。

完成執行測試之後，**測試總管**會顯示整體結果和個別測試結果。 此外，程式碼視窗會以圖形方式顯示測試程式碼涵蓋範圍和測試結果。 如下圖所示，所有三個測試都已成功執行。 它也會顯示我們的測試已涵蓋 `StartsWithUpper` 方法中的所有程式碼路徑，而且已成功執行這些測試 (以綠色核取記號 "✓" 指出)。 最後，它表明 StringLibrary 中的其他方法都沒有代碼覆蓋率（由藍線"➖"表示）。
...
1. 按一下 `StartsWithUpper` 方法之 `if (String.IsNullOrWhiteSpace(s))` 行中的綠色核取記號。 如下圖所示，Live 單元測試指示三個測試涵蓋該程式碼，並且所有測試都已成功執行。

   !['if' 條件陳述式的程式碼涵蓋範圍](media/lut-start/code-coverage-cs1.png)

1. 按一下 `StartsWithUpper` 方法之 `return Char.IsUpper(s[0])` 行中的綠色核取記號。 如下圖所示，Live 單元測試指示只有兩個測試涵蓋該程式碼，並且所有測試都已成功執行。
...
1. `DirectCallWithNullOrEmpty`通過在調用 方法後立即添加以下代碼來[`Microsoft.VisualStudio.TestTools.UnitTesting.Assert.IsFalse`](/dotnet/api/microsoft.visualstudio.testtools.unittesting.assert.isfalse)修改方法。

    [!code-csharp[StringLibraryTest source code](samples/snippets/csharp/lut-start/unittest2.cs#2)]

1. Live Unit Testing 會在您修改原始程式碼時自動執行新的和修改過的測試。 正如**下面的測試資源管理器**圖像所示，所有測試（包括已添加的兩個測試和已修改的測試）都已成功。

   ![擴展測試覆蓋範圍後測試資源管理器](media/lut-start/test-dynamic.png)

1. 切換到包含 StringLibrary 類原始程式碼的視窗。 Live Unit Testing 現在會顯示我們的程式碼涵蓋範圍擴充至 `StartsWithLower` 方法。
...
1. 測試執行時，即時單元測試指示`TestHasEmbeddedSpaces`該方法失敗，如下圖所示：
   ...
   ![測試資源管理器報告測試失敗](media/lut-start/test-failure.png)
...
1. 將游標停留在含 `HasEmbeddedSpaces` 方法簽章的行上方。 即時單元測試顯示一個工具提示，用於報告該方法由一個測試覆蓋，如下圖所示：
   ...
   ![失敗測試的即時單元測試資訊](media/lut-start/test-failure-info-cs.png)

1. 選取失敗的 **TestHasEmbeddedSpaces** 測試。 Live Unit 測試為您提供了許多選項，例如運行所有測試、運行所選測試、調試所有測試以及調試所選測試，如下圖所示：

   ![失敗測試的即時單元測試選項](media/lut-start/test-failure-options.png)

1. 選取 [偵錯選取的測試]****，以偵錯失敗的測試。

1. Visual Studio 會以偵錯模式執行測試。

   測試將陣列中的每個字串分配給一個變數，`phrase`並將其傳遞給`HasEmbeddedSpaces`方法。 assert 運算式第一次為 `false` 時，執行程式會暫停並叫用偵錯工具。 方法調用中[`Microsoft.VisualStudio.TestTools.UnitTesting.Assert.IsTrue`](/dotnet/api/microsoft.visualstudio.testtools.unittesting.assert.istrue)意外值產生的異常對話方塊顯示在下圖中。

   ![即時單元測試異常對話方塊](media/lut-start/exception-dialog-cs.png)

   此外，Visual Studio 提供的所有調試工具都可用於説明我們解決失敗的測試，如下圖所示：

   ![視覺化工作室調試工具](media/lut-start/debugging-tools-cs.png)
...
1. 選擇 **"調試** > **繼續**"，按**F5，** 或按一下工具列上的 **"繼續"** 按鈕以繼續執行測試程式。 因為發生無法處理的例外狀況，所以測試終止。
這提供初步調查 Bug 的足夠資訊。 `TestHasEmbeddedSpaces` (測試常式) 的假設錯誤，或 `HasEmbeddedSpaces` 未正確地辨識所有內嵌空格。

1. 要診斷和更正問題，請從 方法`StringLibrary.HasEmbeddedSpaces`開始。 查看 `HasEmbeddedSpaces` 方法中的比較。 它會將內嵌空格視為 U+0020。 不過，Unicode Standard 包含許多其他空格字元。 這可能表示不正確地測試程式庫程式碼的空格字元。

1. 將相等比較取代為 <xref:System.Char.IsWhiteSpace%2A?displayProperty=fullName> 方法呼叫：

    [!code-csharp[The TestHasEmbeddedSpaces test method](samples/snippets/csharp/lut-start/program2.cs#1)]

1. 即時單元測試自動重新運行失敗的測試方法，並在代碼視窗和**測試資源管理器**中更新結果，如下圖所示：

    ![成功的嵌入式空間測試](media/lut-start/test-success-cs.png)
