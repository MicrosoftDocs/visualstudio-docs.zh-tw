...
2. 此時，系統會針對可執行測試代理程式的方式提供兩個選項：
   ...
   - **服務**：如果不必運行與桌面交互的自動測試，例如編碼的 UI 測試或在測試回合時創建視頻錄製，請在 **"運行測試代理"下選擇****"服務**"。 測試代理程式會做為服務啟動。 選擇 [下一步]****。
   ...
   - **互動式進程**：如果要運行必須與桌面交互的自動測試，例如編碼 UI 測試或在測試回合時創建視頻錄製，請選擇 **"互動式進程**"。 測試代理程式會做為互動式處理序 (而非服務) 啟動。
...
4. 若要儲存變更，請選擇 [套用設定]****。 關閉**配置摘要**對話方塊，然後關閉**測試代理組態工具**。
...
1. 要隨時運行該工具以重新配置測試控制器，請選擇 **"開始** > **測試控制器組態工具**"。
