...
2. 此時，系統會針對可執行測試代理程式的方式提供兩個選項：
   ...
   - **服務**：如果您不需要執行與桌面互動的自動化測試，例如自動程式碼 UI 測試或在測試執行時建立影片錄製，請選取 **[以下列方式執行測試代理程式**] 底下的 [**服務**]。 測試代理程式會做為服務啟動。 選擇 [下一步]。
   ...
   - **互動式進程**：如果您想要執行必須與桌面互動的自動化測試，例如自動程式碼 UI 測試或在測試執行時建立影片錄製，請選取 [**互動式進程**]。 測試代理程式會做為互動式處理序 (而非服務) 啟動。
...
4. 若要儲存變更，請選擇 [套用設定]****。 關閉 [設定**摘要**] 對話方塊，然後關閉 [**測試代理程式**設定] 工具。
...
1. 若要執行工具，隨時重新設定測試控制器，請選擇 [**啟動**  >  **測試控制器設定工具**]。
