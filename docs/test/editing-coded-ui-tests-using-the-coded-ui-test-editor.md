---
title: 在 Visual Studio 中編輯自動程式碼 UI 測試
ms.date: 11/04/2016
 ...
 *我看不到* 自動程式碼 UI 測試編輯器。
您可能使用 2012 之前的 Visual Studio Enterprise 版本。 透過 MSDN 訂閱，自動程式碼 UI 測試編輯器也可在 Visual Studio 2010 Feature Pack 2 中取得。 如需詳細資訊，請參閱 [Microsoft Visual Studio 2010 Feature Pack 2](http://go.microsoft.com/fwlink/?LinkID=204119)。
...
- 您可以使用自動程式化 UI 測試編輯器工具列中的 [尋找] 按鈕，以開啟 Visual Studio 中的 [尋找和取代] 工具。 接著，您可以使用 [尋找] 控制項，找出自動程式碼 UI 測試編輯器中的 UI 動作。 例如，您可以嘗試尋找「按一下 [登入] 按鈕」。 在大型測試中，這樣做很有用。 請注意，您無法在自動程式碼 UI 測試編輯器的 [尋找和取代] 工具中使用取代功能。 如需詳細資訊，請參閱 [Finding and Replacing Text](../ide/finding-and-replacing-text.md)中的＜尋找控制項＞。

- 有時候，很難視覺化控制項在受測試應用程式之 UI 中的位置。 自動程式碼 UI 測試編輯器的其中一項功能，就是您可以選取 UI 控制項對應中列出的控制項，並檢視其在受測試應用程式中的位置。 如需詳細資訊，請參閱這個主題更下方位置的[尋找受測試應用程式中的 UI 控制項](#CodedUITestEditor_LocateUIControl)。

- 它可能需要展開包含您想要編輯之控制項的容器控制項。 如需詳細資訊，請參閱這個主題更下方位置的[尋找控制項和其子系](#CodedUITestEditor_LocateDecendants)。
