...
在**解決方案資源管理器**中，打開*UIMap.uitest 的*快顯功能表，然後選擇 **"打開**"。 編碼的 UI 測試顯示在編碼的**UI 測試編輯器**中。 您現在可以檢視和編輯自動程式碼 UI 測試中錄製的方法、動作和對應控制項。
...
修改控制項屬性的方法與修改 UI 動作的方法一樣。 在 **"UI 控制項映射"** 窗格中，選擇要編輯的控制項，並使用"**屬性"** 視窗修改其屬性。
...
- 要撤銷所做的屬性更改，請從 **"編輯"** 功能表**中選擇"撤銷"，** 或按**Ctrl**+**Z**。

- 您可以使用"編碼的 UI 測試編輯器"工具列中的 **"查找**"按鈕在視覺化工作室中打開 **"查找和替換**"工具。 然後，可以使用 **"查找"** 控制項在編碼的 UI 測試編輯器中查找 UI 操作。 例如，您可以嘗試尋找「按一下 [登入] 按鈕」。 在大型測試中，這樣做很有用。 您無法在 [自動程式碼 UI 測試編輯器] 的 [尋找和取代]**** 工具中使用取代功能。 如需詳細資訊，請參閱[尋找和取代文字](../ide/finding-and-replacing-text.md)中的＜尋找控制項＞。
...
- 要撤銷拆分，請從 **"編輯"** 功能表**中選擇"撤銷"，** 或按**Ctrl**+**Z**。

- 您可以重新命名新方法。 在 **"UI 操作"** 窗格中選擇它，並在"編碼的 UI 測試編輯器"工具列中選擇 **"重命名**"按鈕。
...
要撤銷移動，請從 **"編輯"** 功能表**中選擇"撤銷"，** 或按**Ctrl**+**Z**。 但是，您必須手動從*UIMap.cs*或*UIMap.vb*檔中刪除代碼。
...
- [創建資料驅動的編碼 UI 測試](../test/creating-a-data-driven-coded-ui-test.md)
- [演練：創建、編輯和維護編碼的 UI 測試](../test/walkthrough-creating-editing-and-maintaining-a-coded-ui-test.md)
