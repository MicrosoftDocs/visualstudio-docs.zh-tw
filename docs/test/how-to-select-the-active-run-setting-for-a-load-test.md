...
如果您的負載測試在 [回合設定]**** 資料夾下只有一個回合設定節點，該節點則一律為使用中的節點。 如果您的負載測試包含多個回合設定節點，則可以在執行負載測試時選取其中一個使用。 [請參閱操作操作操作操作操作操作設置如何：將其他回合設定添加到負載測試](../test/how-to-add-additional-run-settings-to-a-load-test.md)。
