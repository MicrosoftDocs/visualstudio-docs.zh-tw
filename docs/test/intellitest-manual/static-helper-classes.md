...
[參數化單元測試](test-generation.md#precondition)中表達假設用的靜態類別，例如[先決條件](test-generation.md#parameterized-unit-testing)。 此類別的方法可以用來篩選掉不要的測試輸入。
...
[參數化單元測試](test-generation.md#postcondition)中表達判斷提示用的靜態類別，例如[後置條件](test-generation.md#parameterized-unit-testing)。
