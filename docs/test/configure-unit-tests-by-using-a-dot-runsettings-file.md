...
您可以使用 \*.runsettings 檔案來設定 Visual Studio 中的單元測試。 (檔案名稱並不重要，但前提是您使用副檔名 '.runsettings'。)例如，您可以變更執行測試的 .NET Framework 版本、傳遞測試結果的所在目錄，以及測試回合期間所收集的資料。

如果不想要任何特殊設定，即不需要 \*.runsettings 檔案。 \*.runsettings 檔案最常見的用法是自訂[程式碼涵蓋範圍](../test/customizing-code-coverage-analysis.md)。
