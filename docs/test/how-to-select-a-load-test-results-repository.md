...
您不必侷限於本機結果存放區。 負載測試通常會在遠端代理程式電腦集合上執行。 代理程式加上控制器能夠產生比任何單一電腦更接近的模擬負載。 有關詳細資訊，請參閱[測試控制器和測試代理](configure-test-agents-and-controllers-for-load-tests.md)。
...
3. 在 **"載入測試結果存儲"中**，按一下 **（...）** 以顯示"**連接屬性**"對話方塊。
