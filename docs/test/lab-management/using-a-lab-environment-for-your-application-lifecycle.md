...
- **虛擬機器範本：** 虛擬機器範本是已移除其名稱及其他識別碼的虛擬機器。 在 SCVMM 環境中部署 VM 範本時，Microsoft Test Manager 會產生新的識別項。 這允許您在相同環境或多個環境中部署多份虛擬機器的複本，然後同時執行虛擬機器。

- **預存虛擬機器：** 儲存在您專案程式庫且包含唯一識別碼的虛擬機器。
