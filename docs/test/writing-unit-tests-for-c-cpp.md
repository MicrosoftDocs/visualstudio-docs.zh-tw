...
除了使用已安裝的框架外，您還可以為在 Visual Studio 中使用的任何框架編寫自己的測試配接器。 測試配接器可以將單元測試與**測試總管**視窗整合。 [Visual Studio Marketplace](https://marketplace.visualstudio.com) 上提供幾個協力廠商配接器。 有關詳細資訊，請參閱[安裝協力廠商單元測試框架](install-third-party-unit-test-frameworks.md)。
...
- **Google 測試配接器**包含在**具有C++工作負載的桌面開發**預設元件中。 它有一個專案範本，您可以添加到解決方案。 使用**解決方案資源管理器**中解決方案節點上的"**添加新專案**"按右鍵功能表來添加它。 它還具有您可以通過**工具** > **選項配置的選項**。 有關詳細資訊，請參閱[：在視覺工作室中使用 Google 測試](how-to-use-google-test-for-cpp.md)。

- **Boost.Test**是桌面開發預設元件，**具有C++** 工作負載。 它與**測試資源管理器**集成，但當前沒有專案範本。 必須手動設定它。 有關詳細資訊，請參閱[：在視覺化工作室中使用 Boost.Test](how-to-use-boost-test-for-cpp.md)。

- **CTest** 支援已隨附於 [使用 C++ 的桌面開發]**** 工作負載之一部分的 [C++ CMake 工具]**** 元件。 有關詳細資訊，請參閱[：在視覺化工作室中使用 CTest。](how-to-use-ctest-for-cpp.md)

**Visual Studio 2015 和更早版本**

您可以在視覺化工作室應用商店下載 Google 測試配接器和增強器.測試配接器擴展。 在[測試配接器找到增強.測試](https://marketplace.visualstudio.com/items?itemName=VisualCPPTeam.TestAdapterforBoostTest)和[測試配接器的谷歌測試](https://marketplace.visualstudio.com/items?itemName=VisualCPPTeam.TestAdapterforGoogleTest)。
...
在一個或多個測試專案中定義和運行測試。 在要測試的代碼相同的解決方案中創建專案。 要向現有解決方案添加新測試專案，請按右鍵**解決方案資源管理器**中的"解決方案"節點。 在彈出式功能表中，選擇 **"添加新** > **專案**"。 將 [語言]**** 設為 C++，然後在搜尋方塊中鍵入 "test"。 下圖顯示安裝 [使用 C++ 進行桌面開發]**** 及 [UWP 開發]**** 工作負載後，可使用的測試專案：
...
在一個或多個測試專案中定義和運行測試。 在要測試的代碼相同的解決方案中創建專案。 要添加新的測試專案，請按右鍵**解決方案資源管理器**中的"解決方案"節點，然後選擇"**添加新** > **專案**"。 在左側窗格中，選擇 **"可視C++測試**"。 然後，從中心窗格中選擇一個專案類型。 下圖顯示安裝 [使用 C++ 的桌面開發]**** 工作負載時可用的測試專案：
...
要啟用對受測專案中的函數的訪問，請添加對測試專案中專案的引用。 按右鍵**解決方案資源管理器**中的測試專案節點，查看彈出式功能表。 選擇 **"添加** > **參考**"。 在"添加參考"對話方塊中，選擇要測試的專案。

![新增參考](media/cpp-add-ref-test-project.png)

### <a name="link-to-object-or-library-files"></a>連結至物件或程式庫檔案

若測試程式碼沒有匯出您要測試的函式，您可以將輸出 .obj 或 .lib 檔案新增至測試專案的相依性。 有關詳細資訊，請參閱[將測試連結到物件或庫檔](/visualstudio/test/how-to-use-microsoft-test-framework-for-cpp#object_files)。
...
為了避免必須在原始檔案中的每個包含語句中鍵入完整路徑，可以在**專案** > **屬性** > **C/C++** > **常規** > **附加包含目錄中**添加所需的資料夾。
...
測試專案中的 *.cpp*檔具有為您定義的存根類和方法。 它們顯示了如何編寫測試代碼的示例。 簽名使用TEST_CLASS和TEST_METHOD宏，這使得這些方法可以從**測試資源管理器**視窗中發現。
...
1. 在 [測試]**** 功能表上，選擇 [Windows]**** > [測試總管]****。 下圖顯示其測試尚未執行的測試專案。
   ...
   ![運行測試前的測試資源管理器](media/cpp-test-explorer.png)
...
1. 在**測試資源管理器中**，選擇 **"全部運行**"或選擇要運行的特定測試。 以滑鼠右鍵按一下測試即可顯示其他選項，包括在啟用中斷點的偵錯模式中執行測試。 執行所有測試之後，視窗會顯示哪些測試成功及哪些測試失敗：

![執行測試後的 [測試總管]](media/cpp-test-explorer-passed.png)

針對失敗的測試，此訊息會提供詳細資料以協助診斷原因。 按右鍵彈出功能表的失敗測試。 選擇 **"調試所選測試**"以單一步驟發生故障的函數。
