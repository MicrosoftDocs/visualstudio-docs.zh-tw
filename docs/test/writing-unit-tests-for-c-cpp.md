...
- **Google Test 介面卡** 隨附為 **使用 c + +** 工作負載的桌面開發預設元件。 它有一個可新增至方案的專案範本。 使用**方案總管**中方案節點上的 [**加入新的專案**] 右鍵功能表，將它加入。 它也有可透過 [工具] **Tools**  >  **選項**設定的選項。 如需詳細資訊，請參閱 [如何：使用 Visual Studio 中的 Google Test](how-to-use-google-test-for-cpp.md)。
