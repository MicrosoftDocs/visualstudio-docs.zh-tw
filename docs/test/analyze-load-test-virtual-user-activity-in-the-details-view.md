...
> [!NOTE]
> 在執行要使用 [虛擬使用者活動圖] 的負載測試之前，您必須使用負載測試編輯器確認 [計時詳細資料儲存區] 屬性已設為 [所有個別細節] 選項。 如需詳細資料，請參閱[如何：設定收集完整詳細資料以便啟用虛擬使用者活動圖](../test/how-to-configure-load-tests-to-collect-full-details.md)。
