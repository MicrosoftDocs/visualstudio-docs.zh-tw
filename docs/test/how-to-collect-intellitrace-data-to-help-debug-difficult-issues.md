...
ms.locfileid: "80880217"
---
# <a name="how-to-collect-intellitrace-data-to-help-debug-difficult-issues"></a>如何：收集 IntelliTrace 資料以協助偵錯困難的問題

您可以為 IntelliTrace 配置診斷數據適配器,以在 Visual Studio 中收集特定的診斷追蹤資訊。 測試可以使用此配接器，如此測試就可以收集應用程式的重大診斷事件，讓開發人員之後使用這些事件追蹤整個程式碼，找出 Bug 的原因。 IntelliTrace 的診斷資料配接器可以用於手動或自動化測試。
...
下列程序說明如何設定您想要收集的 IntelliTrace 資料。 這些步驟適用於可視化工作室中的「測試設置」對話框。
::: moniker-end
...
5. 選擇「**一般**」選項卡。僅選擇**IntelliTrace 事件**,以記錄在測試時對性能影響最小的重大診斷事件。
...
7. 選擇 **「模組」** 選項卡。選擇 **「從除以下模組外的所有模組收集資料**」,並使用 **「新增」** 添加到模組清單中 **,然後「刪除」** 以刪除模組。 此選項可讓您納入系統上執行的所有模組，但您指定的模組例外。
...
8. 選擇「**行程**」 選項卡。選擇 **「從除以下行程之外的所有行程收集資料」**,並使用 **「新增」** 新增到行程清單中,**然後「刪除」** 以刪除進程。 此選項可讓您納入系統上執行的所有處理序，但您指定的處理序例外。

     -或-

     選取 [只從指定的處理序收集資料]****，然後使用 [新增]**** 以新增至處理序清單，並使用 [移除]**** 以移除處理序。 此選項可讓您確切地指定想要的處理序。

9. ( 選擇性的 )選擇 **「Intelli 追蹤事件**」選項卡。選擇或清除在收集診斷事件時要包括或排除的每個 IntelliTrace 事件類別。

10. (選擇性) 展開每個 IntelliTrace 事件分類，並選取或清除您想要在 IntelliTrace 事件中納入或排除的每個特定事件。

11. ( 選擇性的 )選擇 **「高級」** 選項卡。接下來,選擇**用於錄製的最大磁碟空間量**旁邊的箭頭,並選擇要啟用的 IntelliTrace 檔的最大大小。

    > [!NOTE]
    > 如果您增加了記錄的大小，當您將這項記錄與測試結果儲存在一起時，可能會發生逾時問題。

12. 如果您使用的是 Microsoft 測試管理員(在 Visual Studio 2017 中棄用),請選擇 **「儲存**」。 如果您要使用 Visual Studio，請選擇 [確定]****。 針對測試設定，現在已經設定和儲存 IntelliTrace 設定。

    ::: moniker range="vs-2017"
    > [!NOTE]
    > 要重置此診斷資料適配器的配置,請選擇 **「重置為**Visual Studio 的預設配置」,為 Microsoft 測試管理器選擇 **「重置為預設值**」。
    ::: moniker-end
    ::: moniker range=">=vs-2019"
    > [!NOTE]
