...
5. 選擇"**常規**"選項卡。僅選擇**IntelliTrace 事件**，以記錄在測試時對性能影響最小的重大診斷事件。
...
7. 選擇 **"模組"** 選項卡。選擇 **"從除以下模組外的所有模組收集資料**"，並使用 **"添加"** 添加到模組清單中 **，然後"刪除"** 以刪除模組。 此選項可讓您納入系統上執行的所有模組，但您指定的模組例外。
...
8. 選擇"**進程**"選項卡。選擇 **"從除以下進程之外的所有進程收集資料"，** 並使用 **"添加"** 添加到進程清單中，**然後"刪除"** 以刪除進程。 此選項可讓您納入系統上執行的所有處理序，但您指定的處理序例外。

     -或-

     選取 [只從指定的處理序收集資料]****，然後使用 [新增]**** 以新增至處理序清單，並使用 [移除]**** 以移除處理序。 此選項可讓您確切地指定想要的處理序。

9. （可選）選擇 **"Intelli 跟蹤事件**"選項卡。選擇或清除在收集診斷事件時要包括或排除的每個 IntelliTrace 事件類別。

10. (選擇性) 展開每個 IntelliTrace 事件分類，並選取或清除您想要在 IntelliTrace 事件中納入或排除的每個特定事件。

11. （可選）選擇 **"高級"** 選項卡。接下來，選擇**用於錄製的最大磁碟空間量**旁邊的箭頭，並選擇要啟用的 IntelliTrace 檔的最大大小。
...
12. 如果您要使用 Microsoft Test Manager，請選擇 [儲存]****。 如果您要使用 Visual Studio，請選擇 [確定]****。 針對測試設定，現在已經設定和儲存 IntelliTrace 設定。
    ...
    > [!NOTE]
    > 要重置此診斷資料配接器的配置，請選擇 **"重置為**Visual Studio 的預設配置"，為 Microsoft 測試管理器選擇 **"重置為預設值**"。
