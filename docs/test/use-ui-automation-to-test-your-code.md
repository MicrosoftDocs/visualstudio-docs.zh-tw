...
1. 安裝**自動程式化 UI 測試**元件。
   ...
   如果尚未安裝，請安裝 Visual Studio 的**自動程式化 UI 測試**元件。 選擇 [工具] > [取得工具和功能] 以啟動 **Visual Studio 安裝程式**。 在 **Visual Studio 安裝程式**中，選擇 [個別元件] 索引標籤，然後向下捲動至 [偵錯和測試] 區段。 選取 [自動程式化 UI 測試]元件，並選擇 [修改]。
