...
- 您可以使用**載入測試分析器**中的表視圖或圖形視圖來分析網頁回應時間：
...
1. 在**負載測試分析器**中，選擇工具列上的**表**以確保顯示表網格。
...
3. 每一個頁面的資料會顯示在方格中。 通常會顯示下列資料行。
   ...
   |**網路**|網路類型。<br /><br /> 根據預設並未收集這個資料。 若要收集此資料，請在 [負載測試編輯器]**** 的 [回合設定]**** 節點下，選取回合設定節點予以變更。 在 [屬性]**** 視窗中的 [計時詳細資料儲存區]**** 屬性，選取 [所有個別細節]****。|
   |**總**|對網頁發出的要求總數。 這是負載測試中之所有反覆項目的總數。|
   ...
   |**麥克斯**|頁面回應時間的最大值。<br /><br /> 根據預設並未收集這個資料。 若要收集此資料，請在 [負載測試編輯器]**** 的 [回合設定]**** 節點下，選取回合設定節點予以變更。 在 [屬性]**** 視窗中的 [計時詳細資料儲存區]**** 屬性，選取 [所有個別細節]****。|
...
1. 在**負載測試分析器**中，選擇工具列上的**圖形**以確保顯示圖形。
...
7. （可選）重複上一步，以**平均頁面時間**、**頁面回應時間目標和****總頁**。
