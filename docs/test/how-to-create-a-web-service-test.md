...
下列程序使用 Commerce Starter Kit 內所包含的 Web 服務。 您可以從[ASP.NET commerce 入門套件](https://sourceforge.net/projects/ppcsk/)下載它。
...
4. 開啟瀏覽器的個別會話，然後在 [**位址**] 工具列中輸入 *.ASMX*頁面的 URL。 請選取要測試的方法，並檢查 SOAP 訊息， 其中包含 `SOAPAction`。
...
9. 返回**Web 效能測試編輯器**，然後選擇 [**字串主體**] 屬性中的省略號 **（...）** 。 將剪貼簿的內容貼到屬性中。
