...
**測試資源管理器**可以從解決方案中的多個測試專案和作為生產代碼專案的一部分的測試類運行測試。 測試專案可以使用不同的單元測試架構。 當進行測試的程式碼是為 .NET 撰寫時，測試專案可以用任何同樣以 .NET 為目標的語言撰寫，而不管目標程式碼的語言為何。 原生 C/C++ 程式碼專案必須使用 C++ 單元測試架構進行測試。 如需詳細資訊，請參閱[撰寫 C/C++ 的單元測試](writing-unit-tests-for-c-cpp.md)。
...
- 要在預設組中運行所有測試，請選擇 **"運行**"，然後選擇功能表上的組。
...
要在 Visual Studio 編輯器中顯示測試方法的原始程式碼，請選擇測試，然後在按右鍵功能表中選擇 **"打開測試**"（鍵盤 **：F12**）。
...
|群組|描述|
|-|-----------------|
|**時間**|按執行時間對測試進行分組：**快速**、**中**和**慢。**|
|**狀態**|按執行結果對測試進行分組：**失敗的測試**、**跳過的測試**、**通過的測試**、**未運行**|
|**目標框架** | 依其專案的目標架構將測試分組 |
...
**若要建立播放清單**，請在測試總管中選擇一或多項測試。 在按右鍵功能表上，選擇 **"添加到播放清單** > **新播放清單**"。 以您在 [建立新播放清單] **** 對話方塊中指定的名稱和位置儲存檔案。
...
::: moniker-end
::: moniker range=">=vs-2019"
您可以建立和儲存想要執行或檢視為群組的測試清單。 選擇播放清單時，清單中的測試將顯示在新的測試資源管理器選項卡中。您可以將測試添加到多個播放清單。

**若要建立播放清單**，請在測試總管中選擇一或多項測試。 在按右鍵功能表上，選擇 **"添加到播放清單** > **新播放清單**"。

![建立播放清單](../test/media/vs-2019/test-explorer-playlist-16-2.png)

播放清單將在新的測試資源管理器選項卡中打開。您可以使用此播放清單一次，然後丟棄它，也可以按一下播放清單視窗工具列中的 **"保存**"按鈕，然後選擇名稱和位置來保存播放清單。

![播放清單會在個別的測試總管索引標籤中開啟](../test/media/vs-2019/test-explorer-playlist-tab-16-2.png)

**若要建立播放清單**，請在測試總管中選擇一或多項測試。 按右鍵並選擇"**添加到播放清單** > **新播放清單**"。
...
|**完整名稱**|在測試命名空間、類別和方法的完整名稱中搜尋相符項目。|
|**專案**|在測試專案名稱中搜尋相符項目。|
|**目標框架**|在測試總管分類名稱中搜尋相符項目：[失敗的測試] ****、[略過的測試] ****、[成功的測試] ****。|
...
* 在測試資源管理器中按右鍵，然後選擇 **"分析所選測試的代碼覆蓋率**"
...
測試可以通過在測試的代碼編輯器中按右鍵並選擇**運行測試**或使用 Visual Studio 中的預設[測試資源管理器快捷方式](../ide/default-keyboard-shortcuts-in-visual-studio.md#bkmk_testexplorerGLOBAL)從測試資源管理器運行。 部分快速鍵是以內容為基礎。 這表示它們會根據您的游標在程式碼編輯器中的位置來執行或偵錯測試。 如果游標在測試方法內，則該測試方法便會執行。 如果游標是在類別層級，則該類別中的所有測試便會執行。 對於命名空間層級也是如此。
