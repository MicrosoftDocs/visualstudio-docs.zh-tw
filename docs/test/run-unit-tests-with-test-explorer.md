...
**若要建立播放清單**，請在測試總管中選擇一或多項測試。 在右鍵功能表上，選擇 [**新增至播放清單**  >  **]**]。 以您在 [建立新播放清單] **** 對話方塊中指定的名稱和位置儲存檔案。
...
::: moniker-end
::: moniker range=">=vs-2019"
您可以建立和儲存想要執行或檢視為群組的測試清單。 當您選取播放清單時，清單中的測試會顯示在新的 [測試瀏覽器] 索引標籤中。您可以將測試加入一個以上的播放清單。

**若要建立播放清單**，請在測試總管中選擇一或多項測試。 在右鍵功能表上，選擇 [**加入] 播放清單**  >  **新播放清單**。

![建立播放清單](../test/media/vs-2019/test-explorer-playlist-16-2.png)

播放清單會在新的 [測試瀏覽器] 索引標籤中開啟。您可以使用此播放清單一次，然後捨棄它，或者您可以按一下播放清單視窗工具列上的 [**儲存**] 按鈕，然後選取要儲存播放清單的名稱和位置。

![播放清單會在個別的測試總管索引標籤中開啟](../test/media/vs-2019/test-explorer-playlist-tab-16-7.png)

**若要建立播放清單**，請在測試總管中選擇一或多項測試。 按一下滑鼠右鍵，然後選擇 [新增**至播放清單**]  >  **新播放清單**。

**若要開啟播放清單**，選擇 Visual Studio 工具列中的播放清單圖是，然後從功能表中選取先前儲存的播放清單檔案。

**若要編輯播放清單**，您可以在任何測試上按一下滑鼠右鍵，然後使用功能表選項，從播放清單中新增或移除它。

從 Visual Studio 2019 16.7 版開始，您可以選擇工具列中的 [**編輯**] 按鈕。 測試旁會出現核取方塊，顯示播放清單中包含和排除的測試。 視需要編輯群組。

![編輯播放清單按鈕](../test/media/vs-2019/test-explorer-playlist-edit-16-7.png)

您也可以勾選或取消核取階層中父群組的方塊。 此動作會建立動態播放清單，一律根據該群組中的測試來更新播放清單。 例如，如果您在類別旁放置核取記號，從該類別新增的任何測試都會成為此播放清單的一部分。 如果您從該類別刪除測試，則會從播放清單中移除它。 您可以使用工具列中的 [儲存] 按鈕來儲存播放清單，並開啟在磁片上建立的*播放清單*檔案，以深入瞭解規則。 此檔案會列出組成播放清單的所有規則和個別測試。
...
* 在測試瀏覽器中按一下滑鼠右鍵，然後選取 [**分析所選測試的程式碼涵蓋範圍**]
