...
|Tests|顯示負載測試期間個別測試回合的詳細資料。 如需詳細資訊，請參閱本主題中的[測試資料表](../test/analyze-load-test-results-and-errors-in-the-tables-view.md#the-tests-table)。|
|閾值|顯示負載測試回合期間發生的臨界值規則違規清單。 如需詳細資訊，請參閱[分析臨界值規則違規](../test/analyze-threshold-rule-violations-in-load-tests.md)。|
