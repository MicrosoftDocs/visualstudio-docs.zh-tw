...
有關鍵盤協助工具的資訊，請參閱[：使用鍵盤導航](~/accessibility-keyboard.md)。

只需使用適當的鍵盤快速鍵，您就可以在 Visual Studio for Mac 中存取各種命令。 本文檔列出了 Mac 金鑰綁定方案**的 Visual Studio**的預設快捷方式。 如需如何變更按鍵繫結關係配置的相關詳細資訊，請參閱[自訂 IDE](~/customizing-the-ide.md#key-bindings)。 您還可以使用"Mac > **首選項**"功能表項目**的視覺化工作室**自訂命令的快捷方式，並在 **"首選項"** 視窗中選擇**環境** > **鍵綁定**。
...
我們已經將最常用的、最有用的鍵盤快速鍵編譯為可下載的 pdf 檔。 我們有兩個版本，一個顯示適用于 Mac 的 Visual Studio 的快捷方式，另一個版本為在使用 Windows 時具有 Visual Studio 體驗過經驗的使用者提供視覺工作室短片。

 - [Visual Studio 用於 Mac 鍵盤快捷參考](https://aka.ms/vsm-keys)
 - [視覺工作室&視覺工作室為Mac鍵盤快捷參考](https://aka.ms/vsm-vs-keys)
