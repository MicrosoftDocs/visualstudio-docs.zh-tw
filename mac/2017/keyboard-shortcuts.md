...
有關鍵盤協助工具的資訊，請參閱[：使用鍵盤導航](accessibility.md#how-to-use-keyboard-navigation)。

只需使用適當的鍵盤快速鍵，您就可以在 Visual Studio for Mac 中存取各種命令。 此文件列出 **Visual Studio** 按鍵繫結關係配置的預設快速鍵。 如需如何變更按鍵繫結關係配置的相關詳細資訊，請參閱[自訂 IDE](customizing-the-ide.md#key-bindings)。 您還可以使用"Mac > **首選項**"功能表項目**的視覺化工作室**自訂命令的快捷方式，並在 **"首選項"** 視窗中選擇**環境** > **鍵綁定**。
