...
22. 現在您可以新增一些程式碼來驅動敵人的邏輯，使它追逐玩家。 以滑鼠右鍵按一下 [ **專案** ] 視窗中的 [ **資產** ] 資料夾，然後選取 [ **建立 > c # 腳本** ]。
...
5. Visual Studio 方案很簡單。 它包含 [Assets] 資料夾 (來自 [Finder] 的相同資料夾) 和稍早建立的 **EnemyAI.cs** 指令碼。 在更複雜的專案中，階層可能與您在 Unity 中看到的不同。
    ...
    ![Visual Studio for Mac 中的方案視窗](media/unity-image24.png)
...
2. 按一下 [開始偵錯] 按鈕或按 **F5** 。 這會建置專案，並將其附加至 Unity 進行偵錯。
    ...
    ![Visual Studio for Mac 中的 [開始] 按鈕](media/unity-image26.png)
...
4. 應該會叫用中斷點，您現在可以使用 Visual Studio for Mac 偵錯工具。
    ...
    ![Visual Studio for Mac 中遇到中斷點](media/unity-image28.png)

5. 從 [ **區域變數** ] 視窗中，找出參考 **enemyai.cs** 物件的 **this** 指標。 展開參考，並查看您可以瀏覽相關聯的成員，例如 [Speed] \(速度\)。

    ![Visual Studio for Mac 中的 [區域變數] 視窗](media/unity-image29.png)

6. 從 **Start** 方法中移除中斷點，方法與新增它的方式相同 - 透過在邊界中按一下它，或選取行並按 **F9** 。

    ![按一下 Visual Studio for Mac 移除中斷點](media/unity-image30.png)

7. 按 **F10** 不進入使用標記作為參數尋找 **Player** 遊戲物件的第一行程式碼。

8. 將滑鼠游標暫留在程式碼編輯器視窗中的 **player** 變數上，以檢視其相關聯的成員。 您甚至可以展開重疊以檢視子系屬性。

    ![Visual Studio for Mac 編輯器中的調試時間範圍](media/unity-image31.png)
...
10. 切換回 **Visual Studio for Mac** 並在 **Update** 方法的第一行設定中斷點。 它應該會立即叫用。
    ...
    ![在 Visual Studio for Mac 中移除中斷點](media/unity-image33.png)
...
6. Visual Studio for Mac 也可讓您輕鬆地定義新的著色器。 從 [方案總管] 中，以滑鼠右鍵按一下 [資產]，然後選取 [新增] > [新增著色器]。
    ...
    ![Visual Studio for Mac 中的新著色器動作](media/unity-image43.png)
