...
1. 在 [ **web 和主控台 > 應用**程式] 下，選擇 [ **web 應用程式**] 範本： ![ 建立新的 ASP.NET 應用程式](media/docker-quickstart-1.png)
...
1. 輸入專案詳細資料，例如 [專案名稱] （在此範例中為_MyWebAPI_ ）。
...
1. 在 `DockerDemoFrontEnd` 專案中，開啟*Pages/Index. cshtml*檔案，並 `OnGet` 以下列程式碼取代方法：
   ...
   ```
    ...
    > [!NOTE]
    > 在實際執行的程式碼中，您不應該在 `HttpClient` 每個要求之後處置。 如需最佳作法，請參閱[使用 HttpClientFactory 來執行可復原的 HTTP 要求](https://docs.microsoft.com/dotnet/architecture/microservices/implement-resilient-applications/use-httpclientfactory-to-implement-resilient-http-requests)。
...
1. 在前端和 Web API 專案中，在 Startup.cs 的方法中批註[AspNetCore](https://docs.microsoft.com/dotnet/api/microsoft.aspnetcore.builder.httpspolicybuilderextensions.usehttpsredirection)的呼叫， `Configure` 因為此範例程式碼會使用 HTTP （而*Startup.cs*非 HTTPS）來呼叫 Web API。
