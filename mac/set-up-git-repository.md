...
3. 選擇 [ **從註冊的選取** ] 索引標籤，然後按 [ **新增** ] 按鈕：

    ![[加入已註冊的存放庫] 對話方塊。](media/version-control-git5.png)
...
10. 當您開始進行其他變更之後，請先使用 **版本控制 > [審查] 和 [認可** ] 功能表開啟 [狀態] view。 在您選取並認可變更之後，請選取 [ **推送** ] 以將變更推送至遠端存放庫。 這將允許所有適當的使用者在 github.com 上進行檢視：
...
1. 前往功能表列中的 [ **版本控制] > [審查並認可]，** 建立您的第一個認可。
...
2. 這會顯示 [ **使用 Url 連接** ] 索引標籤：
    ...
    ![已輸入詳細資料的 [與 Url 連接] 索引標籤](media/version-control-git13.png)

3. 在遠端存放庫的 GitHub 頁面上，按 [複製或下載]**** 按鈕，然後複製提供的 URL：

    ![顯示的 GitHub URL](media/version-control-git14.png)

4. 取代 [**以 Url 連接**] 索引標籤中 [ **url** ] 專案欄位中的所有文字。這會為您填入此索引標籤中的大部分其他欄位，如步驟 #2 中的影像所示。

5. 輸入您要將存放庫複製到其中的目錄，然後按下 [ **複製**]。
