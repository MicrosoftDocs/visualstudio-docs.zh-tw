...
Visual Studio for Mac 提供對 Razor 編輯的支援，包含 *.cshtml* 檔案中的 IntelliSense 和語法醒目提示。 適用于 Mac 8.3 + 的 Visual Studio 2019 中的新功能，是在 Razor 檔案中具有內容感知 IntelliSense 的功能，因此您會收到符合您目前在檔中編輯之語言的 IntelliSense。
...
1. 選取您的 .NET Core 目標 framework (我們建議2.2 版或更新版本的) ，然後選取 **[下一步]**。 選擇專案的名稱，並視需要新增 Git 支援。 選取 [Create] \(建立\) 以建立專案。

   ![Razor 專案名稱](media/razor-new-project2.png)

   Visual Studio for Mac 會在程式碼配置視窗中開啟您的專案。
1. 使用 **命令 + Option + F5**來執行專案，而不需進行任何偵錯工具。

   Visual Studio 開始 [Kestrel](/aspnet/core/fundamentals/servers/kestrel)、將瀏覽器開啟至 `https://localhost:5001` ，並顯示您的第一個 Razor web 應用程式。
...
此資料夾包含專案的網頁，以及每個專案的程式碼後端：
- HTML 標籤和 Razor 語法的* \* cshtml 檔案。*
- C # 程式碼後端用來處理頁面事件的* \* cshtml.cs 檔案。*
...
如需建立 Razor web 應用程式的完整指南，請參閱 [ASP.NET Core 中的 Razor Pages 簡介](/aspnet/core/razor-pages/index)。
