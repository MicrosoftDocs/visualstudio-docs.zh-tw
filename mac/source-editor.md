...
ms.locfileid: "63000238"
---
# <a name="source-editor"></a>原始檔編輯器

可靠的原始檔編輯器是以簡潔且具效率的方式撰寫程式碼的必要項目。 Visual Studio for Mac 提供一個精細的原始檔編輯器，其為您與 IDE 互動的核心。 原始檔編輯器會提供您可能預期及輕鬆工作所需要的功能：範圍從語法醒目提示、程式碼片段和程式碼摺疊等基礎功能，到其 Roslyn 編譯器整合的優點，例如完成完整功能的 IntelliSense 程式碼。
...
巡覽至 [Visual Studio] > [喜好設定] > [文字編輯器] > [標記與尺規]，來設定顯示空白的選項。 選取此選項允許設定「何時」 顯示不可見的字元：絕不、選取時，或一律顯示：
