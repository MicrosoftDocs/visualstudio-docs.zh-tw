...
2. 下載完成後，按一下 **VisualStudioforMacInstaller.dmg** 裝載安裝程式，然後按兩下箭號標誌加以執行：
    ...
    [![按一下大箭頭開始安裝](media/install-installer-sml.png)](media/install-installer.png#lightbox)

3. 您可能會看到要從網際網路下載應用程式的相關警告。 按一下 [開啟]****。
4. 等候安裝程式檢查您的系統：

    [![安裝程式檢查系統已安裝的元件](media/install-checking-sml.png)](media/install-checking.png#lightbox)

5. 系統會出現要求您確認隱私權和授權條款的警示。 請前往連結並加以閱讀；如果您同意即可按 [繼續]****：

    [![按照隱私和條款的連結，然後繼續，如果你同意](media/install-privacy.png)](media/install-privacy.png#lightbox)

6. 即會顯示可用工作負載的清單。 選取您想要使用的元件：

    [![選擇要安裝的可選工作負載功能](media/install-selection.png)](media/install-selection.png#lightbox)
...
|應用程式類型  |目標  |選取項目  |注意  |
|---------|---------|---------|---------|
|**使用 Xamarin 的應用**| Xamarin.Forms|選擇**Android**和**iOS**平臺 |您需要安裝[**Xcode**](https://developer.apple.com/xcode/) |
||僅限 iOS|選擇**iOS**平臺|您需要安裝[**Xcode**](https://developer.apple.com/xcode/)|
||僅 Android|選擇**Android**平臺|請注意，您還應選擇相關的依賴項|
||僅限 Mac|選擇**macOS（可哥）** 平臺|您需要安裝[**Xcode**](https://developer.apple.com/xcode/)|
|**.NET Core 應用程式**|         |選擇 **.NET 核心**平臺。|         |
|**ASP.NET Core Web 應用程式**|         |選擇 **.NET 核心**平臺。|         |
|**Azure Functions**|         |選擇 **.NET 核心**平臺。|         |
|**跨平台 Unity 遊戲開發**|         |除了適用于 Mac 的視覺化工作室之外，無需安裝其他平臺。| 如需安裝 Unity 延伸模組的詳細資訊，請參閱 [Unity 安裝指南](/visualstudio/mac/setup-vsmac-tools-unity)。|


7. 選取完成之後，請按 [安裝]**** 按鈕。
8. 安裝程式會顯示下載和安裝 Visual Studio for Mac 的進度，並顯示選取的工作負載。 系統將提示您輸入密碼以授予安裝所需的許可權：

    [![選擇要安裝的可選工作負載功能](media/installation-progress.png)](media/installation-progress.png#lightbox)

9. 安裝後，適用于 Mac 的 Visual Studio 將通過登錄並選擇要使用的金鑰綁定來提示您個人化安裝：

    [![登錄到 IDE](media/ide-tour-2019-start-signin.png)](media/ide-tour-2019-start-signin.png#lightbox)

    [![選擇要使用的鍵盤快速鍵](media/ide-tour-2019-keyboard-shortcut.png)](media/ide-tour-2019-keyboard-shortcut.png#lightbox)
